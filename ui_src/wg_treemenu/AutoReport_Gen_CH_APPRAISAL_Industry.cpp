...
Report_Para_CH* AutoReport_Gen_CH_APPRAISAL_Industry::Para() const
{
	return AutoReport_Gen_Mgr::Inst()->Para();
}

CString AutoReport_Gen_CH_APPRAISAL_Industry::MakePicture_1_1()
{
...
int AutoReport_Gen_CH_APPRAISAL_Industry::TempletID()
{
	return 0;
}
BOOL AutoReport_Gen_CH_APPRAISAL_Industry::PreMakeData()
{
	Initial();
	m_Segments.clear();
...
void AutoReport_Gen_CH_APPRAISAL_Industry::InsertCoverPage()
{
	GEN_IFTAG_STR pSrc;
	GEN_IFTAG pTgt;
...
void AutoReport_Gen_CH_APPRAISAL_Industry::InsertCatalogPage()
{
	CString strText = _T("");
	strText = _T("");
	CString strTemp = _T("");
...
void AutoReport_Gen_CH_APPRAISAL_Industry::MakeContent()
{
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt1()
{
	...
	str.Format(_T("为了解结构的可靠性，委托单位委托检测单位对该厂房进行安全性鉴定。"));
	MakeText(genElements, str);
	m_Segments.push_back(genElements);
}
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt2()
{
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt2_1(GenSegmentElements& genElements)
{
	MakeTitle(genElements, _T("鉴定的目的和内容"), 2, 1);
}
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt2_2(GenSegmentElements& genElements)
{
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt3()
{
	GenSegmentElements Elems;
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt3_1(GenSegmentElements& genElements)
{
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt3_2(GenSegmentElements& genElements)
{
	...
	{
		strName = _T("荷载组合");
		MakeTitle(genElements, strName, 3, 2, _T(""), 4);
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt4()
{
	GenSegmentElements Elems;
	MakeTitle(Elems, _T("构件的可靠性鉴定"), 4, 0);
	CString str;
	str.Format(_T("    依据《工业建筑可靠性鉴定标准》GB 50144-2019，该厂房结构的构件的可靠性评级需要根据其安全性评级和使用性评级两部分分别来考虑。"));
	MakeText(Elems, str);
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt4_1(GenSegmentElements& genElements)
{
	MakeTitle(genElements, _T("构件的安全性鉴定"), 4, 1);
	CString str;
	str.Format(_T("    钢构件的安全性等级按照承载能力、构造两个项目评定。其中，承载能力评级按照规范6.3.2条执行，具体评定规则见下表："));
	MakeText(genElements, str);
	//表4.1-1  钢构件承载能力评定等级
	{
		T_TABLE_D TableD;
		...
		//
		CString strType[] = { _T("重要构件、连接"), _T("次要构件") };
		T_TAIF_D TaifD;
	...
	str.Format(_T("    各构件的构造评定等级按照规范6.3.4条执行，具体评定规则见下表："));
	MakeText(genElements, str);
	//表4.1-2  钢构件构造评定等级
	{
		T_TABLE_D TableD;
		...
		TableD.HeaderD.aTitleName.Add(_T("a级或b级"));
		TableD.HeaderD.aTitleName.Add(_T("c级或d级"));
	...
	str.Format(_T("    根据规范的要求，结构中各构件的安全性评价等级结果见下表（仅输出安全性评价等级小于a级的构件）："));
	MakeText(genElements, str);
	...
	T_IMEL_D ImelD;
	CString strMemb[] = { _T("重要构件"), _T("次要构件") };
	CString strGrade[] = { _T("a"), _T("b"), _T("c"), _T("d") };
	...
	//表4.1-3  钢梁构件安全性评定等级
	{
		...
		TableD.RecordD.aRecordData.SetSize(m_lstBeam_Dsas.GetCount()+m_lstBeamCFS_Dsas.GetCount());
		auto& record = TableD.RecordD.aRecordData;

		for (int i=0; i<m_lstBeam_Dsas.GetCount(); i++)
		{	
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBeam_Dsas[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBeam_Dsas[i], data);
			record[i].SetSize(8);
			record[i][0].Format(_T("%d"), m_lstBeam_Dsas[i]);
			record[i][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];

			if (m_pDoc->m_pPostCtrl->GetDesignResult()->ReadSteelDesignResult(m_lstBeam_Dsas[i], 0, 1, 0, SteelResult) && SteelResult.bCheck)
			{

				if (m_pDoc->m_pPostCtrl->StlDgnMgr()->ReadDesignResult(m_lstBeam_Dsas[i], ResData))
				{
			...
			record[i][6] = str;
			record[i][7] = data.GetLevel() == -1 ? _T("-") : strGrade[data.GetLevel()];
		}
		int nIndex = m_lstBeam_Dsas.GetCount();
		int k = 0;
		for (int i = 0; i < m_lstBeamCFS_Dsas.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBeamCFS_Dsas[i], ImelD))
				ImelD.Initialize();
			ResData.Initial();
			k = i + nIndex;
			m_pDoc->m_pPostCtrl->CfsDgnMgr()->ReadDesignResult(m_lstBeamCFS_Dsas[i], CfsData);
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBeamCFS_Dsas[i], data);
			record[k].SetSize(8);
			record[k][0].Format(_T("%d"), m_lstBeamCFS_Dsas[i]);
			...
			str = data.nEvaluateLevel[1] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[1]];
			record[k][6] = str;
			record[k][7] = data.GetLevel() == -1 ? _T("-") : strGrade[data.GetLevel()];
		}
		
		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = m_lstBeam_Dsas.GetCount() + m_lstBeamCFS_Dsas.GetCount();
		TableD.RecordD.nRecordColCount = 8;
		if (m_lstBeam_Dsas.GetCount() + m_lstBeamCFS_Dsas.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢梁构件安全性评定等级"), _T("CHARPT_4_1_3_TBL1"));
	}
	...
	//表4.1-4  钢柱构件安全性评定等级
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("构造评价结果"));
		TableD.HeaderD.aTitleName.Add(_T("安全性评价等级"));
		TableD.RecordD.aRecordData.SetSize(m_lstCol_Dsas.GetCount() + m_lstColCFS_Dsas.GetCount());
		auto& record = TableD.RecordD.aRecordData;

		for (int i = 0; i < m_lstCol_Dsas.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstCol_Dsas[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstCol_Dsas[i], data);
			record[i].SetSize(8);
			record[i][0].Format(_T("%d"), m_lstCol_Dsas[i]);
			record[i][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			if (m_pDoc->m_pPostCtrl->GetDesignResult()->ReadSteelDesignResult(m_lstCol_Dsas[i], 0, 1, 0, SteelResult) && SteelResult.bCheck)
			{

				if (m_pDoc->m_pPostCtrl->StlDgnMgr()->ReadDesignResult(m_lstCol_Dsas[i], ResData))
				{
					str.Format(_T("%.3g"), 1.0 / ResData.m_Sect.m_Data[T_RSTL_MEMB_SECT::RLT_COMP].GetRatio());
					record[i][2] = str;
					str = data.nEvaluateLevel[0] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[0]];
		...
		int nIndex = m_lstCol_Dsas.GetCount();
		int k = 0;
		for (int i = 0; i < m_lstColCFS_Dsas.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstColCFS_Dsas[i], ImelD))
				ImelD.Initialize();
			ResData.Initial();
			k = i + nIndex;
			m_pDoc->m_pPostCtrl->CfsDgnMgr()->ReadDesignResult(m_lstColCFS_Dsas[i], CfsData);
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstColCFS_Dsas[i], data);
			record[k].SetSize(8);
			record[k][0].Format(_T("%d"), m_lstColCFS_Dsas[i]);
			record[k][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			str.Format(_T("%.3g"), 1.0 / CfsData.m_Sect.m_Data[CH_RCFS_SECT::RLT_COMP].GetRatio());
			record[k][2] = str;
			str = data.nEvaluateLevel[0] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[0]];
		...
		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = m_lstCol_Dsas.GetCount() + m_lstColCFS_Dsas.GetCount();
		TableD.RecordD.nRecordColCount = 8;
		if (m_lstCol_Dsas.GetCount() + m_lstColCFS_Dsas.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢柱构件安全性评定等级"), _T("CHARPT_4_1_4_TBL1"));
	}
	...
	//表4.1-5  钢支撑构件安全性评定等级
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("构造评价结果"));
		TableD.HeaderD.aTitleName.Add(_T("安全性评价等级"));
		TableD.RecordD.aRecordData.SetSize(m_lstBraceCFS_Dsas.GetCount() + m_lstBrace_Dsas.GetCount());
		auto& record = TableD.RecordD.aRecordData;

		for (int i = 0; i < m_lstBrace_Dsas.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBrace_Dsas[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBrace_Dsas[i], data);
			record[i].SetSize(8);
			record[i][0].Format(_T("%d"), m_lstBrace_Dsas[i]);
			record[i][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			if (m_pDoc->m_pPostCtrl->GetDesignResult()->ReadSteelDesignResult(m_lstBrace_Dsas[i], 0, 1, 0, SteelResult) && SteelResult.bCheck)
			{

				if (m_pDoc->m_pPostCtrl->StlDgnMgr()->ReadDesignResult(m_lstBrace_Dsas[i], ResData))
				{
					str.Format(_T("%.3g"), 1.0 / ResData.m_Sect.m_Data[T_RSTL_MEMB_SECT::RLT_COMP].GetRatio());
					record[i][2] = str;
					str = data.nEvaluateLevel[0] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[0]];
		...
		int nIndex = m_lstBrace_Dsas.GetCount();
		int k = 0;
		for (int i = 0; i < m_lstBraceCFS_Dsas.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBraceCFS_Dsas[i], ImelD))
				ImelD.Initialize();
			ResData.Initial();
			k = i + nIndex;
			m_pDoc->m_pPostCtrl->CfsDgnMgr()->ReadDesignResult(m_lstBraceCFS_Dsas[i], CfsData);
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBraceCFS_Dsas[i], data);
			record[k].SetSize(8);
			record[k][0].Format(_T("%d"), m_lstBraceCFS_Dsas[i]);
			record[k][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			str.Format(_T("%.3g"), 1.0 / CfsData.m_Sect.m_Data[CH_RCFS_SECT::RLT_COMP].GetRatio());
			record[k][2] = str;
			str = data.nEvaluateLevel[0] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[0]];
		...
		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = m_lstBraceCFS_Dsas.GetCount() + m_lstBrace_Dsas.GetCount();
		TableD.RecordD.nRecordColCount = 8;
		if (m_lstBraceCFS_Dsas.GetCount() + m_lstBrace_Dsas.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢支撑构件安全性评定等级"), _T("CHARPT_4_1_5_TBL1"));
	}
	...
	//表4.1-6  钢桁架构件安全性评定等级
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("构造评价结果"));
		TableD.HeaderD.aTitleName.Add(_T("安全性评价等级"));
		TableD.RecordD.aRecordData.SetSize(m_lstTrussCFS_Dsas.GetCount() + m_lstTruss_Dsas.GetCount());
		auto& record = TableD.RecordD.aRecordData;

		for (int i = 0; i < m_lstTruss_Dsas.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstTruss_Dsas[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstTruss_Dsas[i], data);
			record[i].SetSize(8);
			record[i][0].Format(_T("%d"), m_lstTruss_Dsas[i]);
			record[i][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			if (m_pDoc->m_pPostCtrl->GetDesignResult()->ReadSteelDesignResult(m_lstTruss_Dsas[i], 0, 1, 0, SteelResult) && SteelResult.bCheck)
			{

				if (m_pDoc->m_pPostCtrl->StlDgnMgr()->ReadDesignResult(m_lstTruss_Dsas[i], ResData))
				{
					str.Format(_T("%.3g"), 1.0 / ResData.m_Sect.m_Data[T_RSTL_MEMB_SECT::RLT_COMP].GetRatio());
					record[i][2] = str;
					str = data.nEvaluateLevel[0] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[0]];
		...
		int nIndex = m_lstTruss_Dsas.GetCount();
		int k = 0;
		for (int i = 0; i < m_lstTrussCFS_Dsas.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstTrussCFS_Dsas[i], ImelD))
				ImelD.Initialize();
			ResData.Initial();
			k = i + nIndex;
			m_pDoc->m_pPostCtrl->CfsDgnMgr()->ReadDesignResult(m_lstTrussCFS_Dsas[i], CfsData);
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstTrussCFS_Dsas[i], data);
			record[k].SetSize(8);
			record[k][0].Format(_T("%d"), m_lstTrussCFS_Dsas[i]);
			record[k][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			str.Format(_T("%.3g"), 1.0 / CfsData.m_Sect.m_Data[CH_RCFS_SECT::RLT_COMP].GetRatio());
			record[k][2] = str;
			str = data.nEvaluateLevel[0] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[0]];
		...
		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = m_lstTruss_Dsas.GetCount() + m_lstTrussCFS_Dsas.GetCount();
		TableD.RecordD.nRecordColCount = 8;
		if (m_lstTruss_Dsas.GetCount() + m_lstTrussCFS_Dsas.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢桁架构件安全性评定等级"), _T("CHARPT_4_1_6_TBL1"));
	}
}
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt4_2(GenSegmentElements& genElements)
{
	MakeTitle(genElements, _T("构件的使用性鉴定"), 4, 2);
	CString str;
	str.Format(_T("    钢构件的使用性等级按照变形、偏差、一般构造和腐蚀等项目进行。钢构件的变形项目按照规范6.3.9执行，具体评定规则见下表："));
	MakeText(genElements, str);
	...
	//表4.2-1  钢构件变形评定等级
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("评定等级"));
		TableD.HeaderD.aTitleName.Add(_T("评定标准"));
		TableD.RecordD.aRecordData.SetSize(3);
		auto& record = TableD.RecordD.aRecordData;
		record[0].SetSize(2);
		record[0][0] = _T("a");
		record[0][1] = _T("满足国家现行相关标准规定和设计要求");
		record[1].SetSize(2);
		record[1][0] = _T("b");
		record[1][1] = _T("超过a级要求，尚不影响正常使用");
		record[2].SetSize(2);
		record[2][0] = _T("c");
		record[2][1] = _T("超过a级要求，对正常使用有明显影响");

		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = 3;
		TableD.RecordD.nRecordColCount = 2;
		MakeTableCenterText(genElements, TableD, _T("钢构件变形评定等级"), _T("CHARPT_4_2_1_TBL"));
	}

	str.Format(_T("    根据规范的要求，结构中各构件的各构件的使用性评定结果如下(仅输出使用性评价等级小于a级的构件)："));
	MakeText(genElements, str);
	...
	//4.2.1
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("构件号"));
		TableD.HeaderD.aTitleName.Add(_T("构件种类"));
		TableD.HeaderD.aTitleName.Add(_T("变形评价结果"));
		TableD.HeaderD.aTitleName.Add(_T("偏差"));
		TableD.HeaderD.aTitleName.Add(_T("一般构造"));
		TableD.HeaderD.aTitleName.Add(_T("腐蚀"));
		TableD.HeaderD.aTitleName.Add(_T("其它"));
		TableD.HeaderD.aTitleName.Add(_T("使用性评价等级"));
		...
		for (int i = 0; i < m_lstBeam_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBeam_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBeam_Dsau[i], data);
			record[i].SetSize(8);
			...
			str = data.nEvaluateLevel[1] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[1]];
			record[i][3] = str;
			str = data.nEvaluateLevel[2] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[2]];
			record[i][4] = str;
			str = data.nEvaluateLevel[3] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[3]];
			record[i][5] = str;
			str = data.nEvaluateLevel[4] == -1 ? _T("-") : strGrade[data.nEvaluateLevel[4]];
			record[i][6] = str;
			record[i][7] = data.GetLevel() == -1 ? _T("-") : strGrade[data.GetLevel()];
		}
		...
		for (int i = 0; i < m_lstBeamCFS_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBeamCFS_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBeamCFS_Dsau[i], data);
			k = i + nIndex;
			record[k].SetSize(8);
		...
		if (m_lstBeam_Dsau.GetCount() + m_lstBeamCFS_Dsau.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢梁构件使用性评定等级"), _T("CHARPT_4_2_2_TBL"));
	}
	...
	//表4.2.3  钢柱构件安全性评定等级
	{
		...
		for (int i = 0; i < m_lstCol_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstCol_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstCol_Dsau[i], data);
			record[i].SetSize(8);
		...
		for (int i = 0; i < m_lstColCFS_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstColCFS_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstColCFS_Dsau[i], data);
			k = i + nIndex;
			record[k].SetSize(8);
		...
		if (m_lstCol_Dsau.GetCount() + m_lstColCFS_Dsau.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢柱构件使用性评定等级"), _T("CHARPT_4_2_3_TBL"));
	}
	...
	//表4.2.4  钢支撑构件安全性评定等级
	{
		...
		for (int i = 0; i < m_lstBrace_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBrace_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBrace_Dsau[i], data);
			record[i].SetSize(8);
		...
		for (int i = 0; i < m_lstBraceCFS_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBraceCFS_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBraceCFS_Dsau[i], data);
			k = i + nIndex;
			record[k].SetSize(8);
		...
		if (m_lstBraceCFS_Dsau.GetCount() + m_lstBrace_Dsau.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢支撑构件使用性评定等级"), _T("CHARPT_4_2_4_TBL"));
	}
	...
	//表4.2.5  钢桁架构件使用性评定等级
	{
		...
		for (int i = 0; i < m_lstTruss_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstTruss_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstTruss_Dsau[i], data);
			record[i].SetSize(8);
		...
		for (int i = 0; i < m_lstTrussCFS_Dsau.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstTrussCFS_Dsau[i], ImelD))
				ImelD.Initialize();
			data.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstTrussCFS_Dsau[i], data);
			k = i + nIndex;
			record[k].SetSize(8);
		...
		if (m_lstTruss_Dsau.GetCount() + m_lstTrussCFS_Dsau.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢桁架构件使用性评定等级"), _T("CHARPT_4_2_5_TBL"));
	}
}
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt4_3(GenSegmentElements& genElements)
{
	MakeTitle(genElements, _T("构件的可靠性鉴定评级"), 4, 3);
	CString str;
	str.Format(_T("    根据各构件的安全性评级和使用性评级，最终得到的可靠性等级如下表所示(仅输出安全性或使用性评价等级小于a级的构件)："));
	MakeText(genElements, str);
	...
	//4.3.1
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("构件号"));
		TableD.HeaderD.aTitleName.Add(_T("构件种类"));
		TableD.HeaderD.aTitleName.Add(_T("安全性评级"));
		TableD.HeaderD.aTitleName.Add(_T("使用性评级"));
		TableD.HeaderD.aTitleName.Add(_T("可靠性评级"));
		...
		for (int i = 0; i < m_lstBeam.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBeam[i], ImelD))
				ImelD.Initialize();
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBeam[i], sData);
			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBeam[i], uData);
			record[i].SetSize(5);
			record[i][0].Format(_T("%d"), m_lstBeam[i]);
			record[i][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			record[i][2] = strGrade[sData.GetLevel()];
			record[i][3] = strGrade[uData.GetLevel()];
			record[i][4] = sData.GetLevel() > uData.GetLevel() ? strGrade[sData.GetLevel()] : strGrade[uData.GetLevel()];
		}
		...
		for (int i = 0; i < m_lstBeamCFS.GetCount(); i++)
		{
			...
			k = i + nIndex;
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBeamCFS[i], sData);
			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBeamCFS[i], uData);
			record[k].SetSize(5);
			record[k][0].Format(_T("%d"), m_lstBeamCFS[i]);
			record[k][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			record[k][2] = strGrade[sData.GetLevel()];
			record[k][3] = strGrade[uData.GetLevel()];
			record[k][4] = sData.GetLevel() > uData.GetLevel() ? strGrade[sData.GetLevel()] : strGrade[uData.GetLevel()];
		}
		...
		if (m_lstBeam.GetCount() + m_lstBeamCFS.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢梁构件可靠性评定等级"), _T("CHARPT_4_3_1_TBL"));
	}
	...
	//表4.2.3  钢柱构件
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("使用性评级"));
		TableD.HeaderD.aTitleName.Add(_T("可靠性评级"));
		TableD.RecordD.aRecordData.SetSize(m_lstColCFS.GetCount() + m_lstCol.GetCount());
		auto& record = TableD.RecordD.aRecordData;
		...
		for (int i = 0; i < m_lstCol.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstCol[i], ImelD))
				ImelD.Initialize();
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstCol[i], sData);
			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstCol[i], uData);
			record[i].SetSize(5);
		...
		for (int i = 0; i < m_lstColCFS.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstColCFS[i], ImelD))
				ImelD.Initialize();
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstColCFS[i], sData);
			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstColCFS[i], uData);
			k = i + nIndex;
			record[k].SetSize(5);
		...
		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = m_lstColCFS.GetCount() + m_lstCol.GetCount();
		TableD.RecordD.nRecordColCount = 5;
		if (m_lstCol.GetCount() + m_lstColCFS.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢柱构件可靠性评定等级"), _T("CHARPT_4_3_2_TBL"));
	}
	...
	//表4.3.3  钢支撑构件
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("使用性评级"));
		TableD.HeaderD.aTitleName.Add(_T("可靠性评级"));
		TableD.RecordD.aRecordData.SetSize(m_lstBrace.GetCount() + m_lstBraceCFS.GetCount());
		auto& record = TableD.RecordD.aRecordData;
		...
		for (int i = 0; i < m_lstBrace.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBrace[i], ImelD))
				ImelD.Initialize();
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBrace[i], sData);

			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBrace[i], uData);
			record[i].SetSize(5);
			record[i][0].Format(_T("%d"), m_lstBrace[i]);
			record[i][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			record[i][2] = sData.GetLevel() == -1 ? _T("-") : strGrade[sData.GetLevel()];
			record[i][3] = uData.GetLevel() == -1 ? _T("-") : strGrade[uData.GetLevel()];
			if (sData.GetLevel() == -1 && uData.GetLevel() == -1)
				record[i][4] = _T("-");
			else
				record[i][4] = sData.GetLevel() > uData.GetLevel() ? strGrade[sData.GetLevel()] : strGrade[uData.GetLevel()];
		}
		...
		for (int i = 0; i < m_lstBraceCFS.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstBraceCFS[i], ImelD))
				ImelD.Initialize();
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstBraceCFS[i], sData);

			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstBraceCFS[i], uData);
			k = i + nIndex;
			record[k].SetSize(5);
			record[k][0].Format(_T("%d"), m_lstBraceCFS[i]);
			record[k][1] = ImelD.bImportant ? strMemb[0] : strMemb[1];
			record[k][2] = sData.GetLevel() == -1 ? _T("-") : strGrade[sData.GetLevel()];
			record[k][3] = uData.GetLevel() == -1 ? _T("-") : strGrade[uData.GetLevel()];
			if (sData.GetLevel() == -1 && uData.GetLevel() == -1)
				record[i][4] = _T("-");
			else
				record[k][4] = sData.GetLevel() > uData.GetLevel() ? strGrade[sData.GetLevel()] : strGrade[uData.GetLevel()];
		...
		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = m_lstBrace.GetCount() + m_lstBraceCFS.GetCount();
		TableD.RecordD.nRecordColCount = 5;
		if (m_lstBraceCFS.GetCount() + m_lstBrace.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢支撑构件可靠性评定等级"), _T("CHARPT_4_3_3_TBL"));
	}
	...
	//表4.3.4  钢桁架构件
	{
		...
		TableD.HeaderD.aTitleName.Add(_T("使用性评级"));
		TableD.HeaderD.aTitleName.Add(_T("可靠性评级"));
		TableD.RecordD.aRecordData.SetSize(m_lstTruss.GetCount() + m_lstTrussCFS.GetCount());
		auto& record = TableD.RecordD.aRecordData;
		...
		for (int i = 0; i < m_lstTruss.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstTruss[i], ImelD))
				ImelD.Initialize();
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstTruss[i], sData);
			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstTruss[i], uData);
			record[i].SetSize(5);
		...
		for (int i = 0; i < m_lstTrussCFS.GetCount(); i++)
		{
			if (!m_pDoc->m_pAttrCtrl->GetImel(m_lstTrussCFS[i], ImelD))
				ImelD.Initialize();
			sData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(m_lstTrussCFS[i], sData);
			uData.Initialize();
			CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(m_lstTrussCFS[i], uData);
			k = i + nIndex;
			record[k].SetSize(5);
		...
		TableD.RecordD.aRecordData.FreeExtra();
		TableD.RecordD.nRecordRowCount = m_lstTruss.GetCount() + m_lstTrussCFS.GetCount();
		TableD.RecordD.nRecordColCount = 5;
		if (m_lstTruss.GetCount() + m_lstTrussCFS.GetCount() > 0)
			MakeTableCenterText(genElements, TableD, _T("钢桁架构件可靠性评定等级"), _T("CHARPT_4_3_4_TBL"));
	}
}

void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt5()
{
	GenSegmentElements Elems;
...
void AutoReport_Gen_CH_APPRAISAL_Industry::WriteCharpt6()
{
	GenSegmentElements genElements;
	MakeTitle(genElements, _T("处理建议"), 6, 0);
	CString str;
	str.Format(_T("    根据规范3.3.1条，对于可靠性评级为c级的构件，应该采取措施，结构中可靠性评级为c的构件见下表："));
	MakeText(genElements, str);
	...
	int nCountD = m_lstTypeD.GetCount();
	{
		...
		TableD.RecordD.nRecordColCount = 6;
		if (nCountC > 0)
			MakeTable(genElements, TableD, _T("可靠性评级为c级的构件"), _T("CHARPT_6_1_TBL"));

	}

	str.Format(_T("    对于可靠性评级为d级的构件，必须立即采取措施，结构中可靠性评级为d的构件见下表："));
	MakeText(genElements, str);
	...
	//
	{
		...
		TableD.RecordD.nRecordColCount = 6;
		if (nCountD > 0)
			MakeTable(genElements, TableD, _T("可靠性评级为d级的构件"), _T("CHARPT_6_2_TBL"));
...
int AutoReport_Gen_CH_APPRAISAL_Industry::SetRltInfo(int key, CArray<CStringArray, CStringArray&>& strArray, int& nIndex)
{
	CString strKey;
	...
	T_DSAS_D sData;
	T_DSAU_D uData;
	CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(key, sData);
	CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(key, uData);
	...
	if (sData.GetLevel() == -1)
		...
		strArray[nIndex].Add(m_strFormat + strGrade[sData.GetLevel()]);
	if (uData.GetLevel() == -1)
		strArray[nIndex].Add(m_strFormat + _T("-"));
	else
		strArray[nIndex].Add(m_strFormat + strGrade[uData.GetLevel()]);
	int nLevel = sData.GetLevel() > uData.GetLevel() ? sData.GetLevel() : uData.GetLevel();
	if (nLevel == -1)
		strArray[nIndex].Add(m_strFormat + _T("-"));
	else
		strArray[nIndex].Add(m_strFormat + strGrade[nLevel]);

	return nIndex;
}
int AutoReport_Gen_CH_APPRAISAL_Industry::SetStlInfo(int key, const T_MATD_D& data, CArray<CStringArray, CStringArray&>& strArray, int& nIndex)
{
	CString strKey;
	strKey.Format(_T("%d"), key);
	strKey = m_strFormat + strKey;
...
void AutoReport_Gen_CH_APPRAISAL_Industry::setTableFormatString(int nlocation /* = 0 */, int ncolor /* = 16777215 */)
{
...
void AutoReport_Gen_CH_APPRAISAL_Industry::MakeStlMatlInfo()
{
	m_mapstlInfo.clear();
	wstring str[][20] = {
		//GB03(S)
...
BOOL AutoReport_Gen_CH_APPRAISAL_Industry::GetStlList()
{
	T_KEY_LIST lstStl;
	...
	for (int i = 0; i < lstStl.GetCount(); i++)
	{
		...
		switch (iMembType)
		{
		case 1:	m_lstCol_Dsas.Add(lstStl[i]);	break;
		case 2:	m_lstBeam_Dsas.Add(lstStl[i]);	break;
		case 3:	m_lstBrace_Dsas.Add(lstStl[i]);	break;
		case 5:	m_lstTruss_Dsas.Add(lstStl[i]);	break;
		default:
			break;
		}
	}
...
BOOL AutoReport_Gen_CH_APPRAISAL_Industry::GetCFSList()
{
	T_KEY_LIST lstCFS;
	...
	{
		if (!m_pDoc->m_pAttrCtrl->GetDsas(lstCFS[i], sData))
			sData.Initialize();
		if (sData.GetLevel() < 1)
			continue;
		...
		switch (iMembType)
		{
		case 1: m_lstColCFS_Dsas.Add(lstCFS[i]);	break;
		case 2: m_lstBeamCFS_Dsas.Add(lstCFS[i]);	break;
		case 3: m_lstBraceCFS_Dsas.Add(lstCFS[i]);	break;
		case 5: m_lstTrussCFS_Dsas.Add(lstCFS[i]);	break;
		default:
			break;
		}
	}
	...
	{
		sData.Initialize();
		CReinforceDgnMgr_CH::Instance()->GetEvaluateSecurity(lstCFS[i], sData);
		uData.Initialize();
		CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(lstCFS[i], uData);
...
BOOL AutoReport_Gen_CH_APPRAISAL_Industry::GetTypeList()
{
	T_KEY_LIST lstStl;
	CString strCode = CDBLib::GetStlCodeName();
	if (strCode == _T("GB50017-17"))
	...
	for (int i = 0; i < lstStl.GetCount(); i++)
	{
		...
		uData.Initialize();
		CReinforceDgnMgr_CH::Instance()->GetEvaluateUsability(lstStl[i], uData);
		nType = sData.GetLevel() > uData.GetLevel() ? sData.GetLevel() : uData.GetLevel();
		if (nType == 2)
