...
CStldWindPressureAreaMgr::CStldWindPressureAreaMgr(CWnd* pParent, void* pData)
...
CStldWindPressureAreaMgr::~CStldWindPressureAreaMgr()
{
}

void CStldWindPressureAreaMgr::CreateCodeData( void* pData )
{
	if(m_aCodeDlg.GetSize() != EN_WIND_PRESSURE_CODE_TYPE_NUMBER)
		return;

	auto L_SetCodeData = [this](const EN_WIND_PRESSURE_CODE_TYPE& enCode, CCMWindPressureBaseChildDlg* pChildDlg)
	{
		const int nCode = GetDBCodeIdx(enCode);
		m_aCodeName[enCode] = GetCodeStrByDBCode(nCode);
		m_aCodeDlg [enCode] = pChildDlg;
