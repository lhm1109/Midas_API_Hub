...
CStldWindPressureAreaDlg_IS875_2015::CStldWindPressureAreaDlg_IS875_2015(int nCodeType, CWnd* pParent /*=NULL*/)
	: CCMWindPressureBaseChildDlg(CStldWindPressureAreaDlg_IS875_2015::IDD, pParent)
{
	m_pData = NULL;
	m_nCodeType = nCodeType;
}

void CStldWindPressureAreaDlg_IS875_2015::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_TM_KA_FACTOR_EDIT, m_editKaFact);

	// Radio buttons for mode
	DDX_Control(pDX, IDC_TM_PRESS_COEF_RADIO, m_radioPressCoef);
	DDX_Control(pDX, IDC_TM_FORCE_COEF_RADIO, m_radioForceCoef);
}
BEGIN_MESSAGE_MAP(CStldWindPressureAreaDlg_IS875_2015, CCMWindPressureBaseChildDlg)
	ON_BN_CLICKED(IDC_TM_PRESS_COEF_RADIO, OnCoefModeRadioClicked)
	ON_BN_CLICKED(IDC_TM_FORCE_COEF_RADIO, OnCoefModeRadioClicked)
	ON_BN_CLICKED(IDC_WINDP_AUTOCALC_KAFACT, OnChkAutoCalcKaFact)
END_MESSAGE_MAP()

BOOL CStldWindPressureAreaDlg_IS875_2015::OnInitDialog()
{
	...
	m_editKaFact.SetUnitType(D_UNITSYS_NONE);
	m_radioPressCoef.SetCheck(TRUE);
	m_radioForceCoef.SetCheck(FALSE);

	if(m_pData)
	{
		m_editKaFact.EnableWindow(!( m_pData->CodeParam.IS875_2015.bAutoKaCalc ));
	}
	else
	{
		m_editKaFact.EnableWindow(FALSE);
	}
...
BOOL CStldWindPressureAreaDlg_IS875_2015::DestroyWindow()
{
	return CCMWindPressureBaseChildDlg::DestroyWindow();
}


void CStldWindPressureAreaDlg_IS875_2015::ShowCoefFields(int mode)
{
...
void CStldWindPressureAreaDlg_IS875_2015::OnChkAutoCalcKaFact()
{
	CButton* pBtn = (CButton*)GetDlgItem(IDC_WINDP_AUTOCALC_KAFACT);
	int nCheck = pBtn->GetCheck();
	GetDlgItem(IDC_TM_KA_FACTOR_EDIT)->EnableWindow(nCheck == 0);
	m_pData->CodeParam.IS875_2015.bAutoKaCalc = nCheck;

}

void CStldWindPressureAreaDlg_IS875_2015::OnCoefModeRadioClicked()
{
	int mode = m_radioPressCoef.GetCheck() ? 0 : 1;
	ShowCoefFields(mode);
}
BOOL CStldWindPressureAreaDlg_IS875_2015::Dlg2Data(void* pData, BOOL bWarning)
{
	...
	BOOL bAutoCalcKa = ( (CButton*)GetDlgItem(IDC_WINDP_AUTOCALC_KAFACT) )->GetCheck() == 1;
	pISCode->bAutoKaCalc = bAutoCalcKa == 1 ? TRUE : FALSE;
	CString csVal;
	m_editKaFact.GetWindowText(csVal);
	pISCode->dKafact = bAutoCalcKa ? 1.0 : _tstof(csVal);
...
void CStldWindPressureAreaDlg_IS875_2015::Data2Dlg(void* pData, BOOL bWarning)
{
	m_pData = (T_WDPR_D*)pData;
	...
	CButton* pBtn = (CButton*)GetDlgItem(IDC_WINDP_AUTOCALC_KAFACT);
	pBtn->SetCheck(pISCode->bAutoKaCalc);
	if(!pISCode->bAutoKaCalc)
	{
		m_editKaFact.SetEditUnit(pISCode->dKafact, 4);
	}
	else
	{
		m_editKaFact.SetEditUnit(1.0, 4);
	}
...
void CStldWindPressureAreaDlg_IS875_2015::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	HWND hWnd;
...
void CStldWindPressureAreaDlg_IS875_2015::UpdateBuffer()
{
