// CSeisCvlJpSetInvestSimdDlg_H24.cpp : implementation file
//
#include "stdafx.h"
...
void CSeisCvlJpSetInvestSimdDlg::OnInitDialog_H24()
{
	InitControl();
	InitCaseNameCtrl();
	InitGroupNameCtrl();
...
void CSeisCvlJpSetInvestSimdDlg::OnChangedCasePulsCmb_H24()
{
	UpdateLoadCaseStepCtrl(m_this_plus, m_stepDeltaLs_plus);
}

void CSeisCvlJpSetInvestSimdDlg::OnChangedCaseMinusCmb_H24()
{
	UpdateLoadCaseStepCtrl(m_this_minus, m_stepDeltaLs_minus);
}

void CSeisCvlJpSetInvestSimdDlg::InitColmGroupCtrl_H24(const MComboBox& groupNameCmb, MComboBox& rComboBox)
{
	CString strGroup;
...
void CSeisCvlJpSetInvestSimdDlg::UpdateGroupLowNameCtrl_H24(MComboBox& rComboBox, BOOL bReadOnly/*=FALSE*/)
{
...
void CSeisCvlJpSetInvestSimdDlg::UpdateColmGroupGroupCtrl_H24()
{
	CString strCalcType;
	m_calcType.GetWindowText(strCalcType);
	if (strCalcType == COMBO_CALC_TYPE_INPUT_VALUE) {
		//교갹별 조사시 δMax 를 알기위해 기둥하단으로 부터 관성력 작용위치까지의 거리 h를 구하기 위해 유저입력이 선택되더라도 활성화 함.
		m_colmGroup_plus.EnableWindow(m_nGroupType == T_SIGR_D::kLower);
		m_colmGroup_minus.EnableWindow(FALSE);// - 방향은 항상 비활성 + 방향 값을 따라감
		return;
	}

	//H24 기준이고 기초변위 영향분에 대해 안전계수 고려 할 경우 비활성화
	CButton* pCheckBtn = (CButton*)GetDlgItem(IDC_TM_SIMD_SAFETY_COEFFICIENT_CHK);
	if (pCheckBtn->GetCheck() == TRUE) {
		m_colmGroup_plus.EnableWindow(m_nGroupType == T_SIGR_D::kLower);
...
void CSeisCvlJpSetInvestSimdDlg::UpdateStepDeltaYCtrl_H24()
{
	CString str;
...
void CSeisCvlJpSetInvestSimdDlg::InitStepDeltaLsCtrl_H24()
{
...
void CSeisCvlJpSetInvestSimdDlg::InitStepDeltaACtrl_H24()
{
...
void CSeisCvlJpSetInvestSimdDlg::UpdateAlphaCtrl_H24()
{
	...
	m_alpha.SetWindowText(_T("1.2"));

	if (bearArray.GetCount() < 1)
		return;
	...
	if (bSeismicIsolation == TRUE) {
		...
		m_alpha.SetWindowText(_T("2.4"));
	}
}

void CSeisCvlJpSetInvestSimdDlg::UpdateConsiderAlpha_H24()
{
	CButton* pCheckBtn = (CButton*)GetDlgItem(IDC_TM_SIMD_SAFETY_COEFFICIENT_CHK);
	...
	BOOL bSteel = (m_nMatlType_plus == T_SIGR_LOWER_D::kSteel && m_nMatlType_minus == T_SIGR_LOWER_D::kSteel);
	pCheckBtn->EnableWindow(!bSteel);
}

BOOL CSeisCvlJpSetInvestSimdDlg::UddateControlByCalcTypeAuto_H24(const UINT/*QIehp::eCom*/& comp,
																...
																OUT MComboBox& stepDeltaLsCmb)
{
	...
	if (iIndex >= 0) {
		...
		ASSERT(FALSE);//ASSERT(sidcD.nDesignCode == BrdgSE_CodeJP::JROAD5_H14);
		colmGroupCmb.SetCurSel(0);
	}
...
BOOL CSeisCvlJpSetInvestSimdDlg::DlgToData_H24(OUT T_SIMD_D& rData)
{
	CString str;
	//조사 케이스
