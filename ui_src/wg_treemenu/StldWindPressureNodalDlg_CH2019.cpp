...
/////////////////////////////////////////////////////////////////////////////
// CStldWindPressureNodalDlg_CH2019 dialog

CStldWindPressureNodalDlg_CH2019::CStldWindPressureNodalDlg_CH2019(CWnd* pParent /*=NULL*/)
: CCMWindPressureBaseChildDlg(CStldWindPressureNodalDlg_CH2019::IDD, pParent)
{
	m_pData = NULL;
	m_nPeriodType = 1;
}

void CStldWindPressureNodalDlg_CH2019::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CStldWindPressureNodalDlg_CH2019, CCMWindPressureBaseChildDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CStldWindPressureNodalDlg_CH2019 message handlers

BOOL CStldWindPressureNodalDlg_CH2019::OnInitDialog() 
{
...
BOOL CStldWindPressureNodalDlg_CH2019::DestroyWindow() 
{
...
BOOL CStldWindPressureNodalDlg_CH2019::Dlg2Data( void* pData, BOOL bWarning )
{
	m_pData = (T_WDPR_D*)pData;
	
	if(((CButton*)GetDlgItem(IDC_TM_TARGET_RDO2))->GetCheck())
		m_pData->CodeParam.CH2019.nStructMethod = 2;
	else
		m_pData->CodeParam.CH2019.nStructMethod = 1;

	m_pData->CodeParam.CH2019.dDampingRatio = m_edtDampingRatio.GetEditValue();
	m_pData->CodeParam.CH2019.dBasicPeriod = m_edtBasePeriod.GetEditValue();
	m_pData->CodeParam.CH2019.dBasicPeriod_Cross = m_edtBasePeriod2.GetEditValue();
	if(bWarning, m_pData->CodeParam.CH2019.dBasicPeriod <= 1E-5 || m_pData->CodeParam.CH2019.dBasicPeriod_Cross <= 1E-5)
	{
		AfxMessageBox(_LS(IDS_TM_WINDP_NO_PERIOD));
		return FALSE;
	}
	m_pData->CodeParam.CH2019.dWindwardCoef = m_edtWindwardCf.GetEditValue();
	m_pData->CodeParam.CH2019.dLeewardCoef = m_edtLeewardCf.GetEditValue();
	
	if(((CButton*)GetDlgItem(IDC_TM_WIND_CONSIDER_PARTITION))->GetCheck())
	{
		m_pData->CodeParam.CH2019.bConsiderPartition = TRUE;
		m_pData->CodeParam.CH2019.dPartitionCoef = m_edtPartitionPresCf.GetEditValue();
	}
	else
		m_pData->CodeParam.CH2019.bConsiderPartition = FALSE;
	
	if(((CButton*)GetDlgItem(IDC_TM_WIND_CONSIDER_WINDSHIELD))->GetCheck())
	{
		m_pData->CodeParam.CH2019.bConsiderWindShield = TRUE;
		m_pData->CodeParam.CH2019.dWindShieldCoef = m_edtWindshieldCf.GetEditValue();
	}
	else
		m_pData->CodeParam.CH2019.bConsiderWindShield = FALSE;

	if (((CButton*)GetDlgItem(IDC_TM_WIND_ALONG_CHECK))->GetCheck())
	{
		m_pData->CodeParam.CH2019.bAlongVibration = TRUE;
		m_pData->CodeParam.CH2019.nAlongType = ((CButton*)GetDlgItem(IDC_TM_WIND_ALONG_RADIO1))->GetCheck() ? 0 : 1;
	}	
	else
		m_pData->CodeParam.CH2019.bAlongVibration = FALSE;
	
	if(((CButton*)GetDlgItem(IDC_TM_WIND_CROSS_CHECK))->GetCheck())
		m_pData->CodeParam.CH2019.bCrossVibration = TRUE;
	else
		m_pData->CodeParam.CH2019.bCrossVibration = FALSE;
	
//   if(((CButton*)GetDlgItem(IDC_TM_WIND_CFT_CHECK))->GetCheck())
//   {
//     m_pData->CH2019.bWindComfort = TRUE;
//     m_pData->CH2019.dCftWindPressure = m_edtCftWinPressure.GetEditValue();
//     m_pData->CH2019.dCftDampingRatio = m_edtCftDampingRatio.GetEditValue();
//   }
//   else
//     m_pData->CH2019.bWindComfort = FALSE;

	return TRUE; 
}

void CStldWindPressureNodalDlg_CH2019::Data2Dlg( void* pData, BOOL bWarning )
{
	m_pData = (T_WDPR_D*)pData;

	if(m_pData->CodeParam.CH2019.nStructMethod == 2)
	{
	...
	m_edtDampingRatio.SetEditUnit(m_pData->CodeParam.CH2019.dDampingRatio);
	m_edtBasePeriod.SetEditUnit(m_pData->CodeParam.CH2019.dBasicPeriod);
	m_edtBasePeriod2.SetEditUnit(m_pData->CodeParam.CH2019.dBasicPeriod_Cross);
	m_edtWindwardCf.SetEditUnit(m_pData->CodeParam.CH2019.dWindwardCoef);
	m_edtLeewardCf.SetEditUnit(m_pData->CodeParam.CH2019.dLeewardCoef);

	if(m_pData->CodeParam.CH2019.bConsiderPartition)
	...
	m_edtPartitionPresCf.SetEditUnit(m_pData->CodeParam.CH2019.dPartitionCoef);
	
	if(m_pData->CodeParam.CH2019.bConsiderWindShield)
	...
	m_edtWindshieldCf.SetEditUnit(m_pData->CodeParam.CH2019.dWindShieldCoef);
	
	if (m_pData->CodeParam.CH2019.bAlongVibration)
	{
		...
		if (m_pData->CodeParam.CH2019.nAlongType == 0)
	...
	if(m_pData->CodeParam.CH2019.bCrossVibration)
...
//   if(m_pData->CodeParam.bWindComfort)
//     ((CButton*)GetDlgItem(IDC_TM_WIND_CFT_CHECK))->SetCheck(TRUE);
//   else
//     ((CButton*)GetDlgItem(IDC_TM_WIND_CFT_CHECK))->SetCheck(FALSE);
//   m_edtCftWinPressure.SetEditUnit(m_pData->CH2019.dCftWindPressure);
//   m_edtCftDampingRatio.SetEditUnit(m_pData->CH2019.dCftDampingRatio);
//   OnChkConsiderComfort();
}

int CStldWindPressureNodalDlg_CH2019::GetType()
{
	int nType = 1;
...
void CStldWindPressureNodalDlg_CH2019::InitControl()
{
...
void CStldWindPressureNodalDlg_CH2019::OnChkConsiderPartition()
{
	BOOL bEnable;
	if(((CButton*)GetDlgItem(IDC_TM_WIND_CONSIDER_PARTITION))->GetCheck())
		bEnable = TRUE;
...
void CStldWindPressureNodalDlg_CH2019::OnChkConsiderWindShield()
{
	BOOL bEnable;
	if(((CButton*)GetDlgItem(IDC_TM_WIND_CONSIDER_WINDSHIELD))->GetCheck())
		bEnable = TRUE;
...
void CStldWindPressureNodalDlg_CH2019::OnChkConsiderCrossWind()
{
	BOOL bEnable;
	if(((CButton*)GetDlgItem(IDC_TM_WIND_CROSS_CHECK))->GetCheck())
		bEnable = TRUE;
...
// void CStldWindPressureNodalDlg_CH2019::OnChkConsiderComfort()
// {
//   BOOL bEnable;
//   if(((CButton*)GetDlgItem(IDC_TM_WIND_CFT_CHECK))->GetCheck())
//     bEnable = TRUE;
...
void CStldWindPressureNodalDlg_CH2019::OnBtnPeriod()
{
	int nStore;
...
void CStldWindPressureNodalDlg_CH2019::OnBtnCrossWind()
{
	CStldWindPressureAreaDlg_CH2019_CrossTorsion dlg;
	dlg.SetData(m_pData->CodeParam.CH2019);
	if(dlg.DoModal() == IDOK)
	{
		dlg.GetData(m_pData->CodeParam.CH2019);
	}
}

void CStldWindPressureNodalDlg_CH2019::OnBtnPeriod2()
{
	int nStore;
	double dHeight, dWidth;  
...
void CStldWindPressureNodalDlg_CH2019::OnCmdWindAlongCheck()
{
	int nCheck = ((CButton*)GetDlgItem(IDC_TM_WIND_ALONG_CHECK))->GetCheck();
	m_pData->CodeParam.CH2019.bAlongVibration = nCheck;
