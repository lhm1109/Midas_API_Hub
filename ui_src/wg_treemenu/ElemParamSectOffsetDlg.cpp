...
#include "stdafx.h"
#include "ElemParamSectOffsetDlg.h"
...
/////////////////////////////////////////////////////////////////////////////
// ElemParamSectOffsetDlg dialog


ElemParamSectOffsetDlg::ElemParamSectOffsetDlg(T_SECT_SECTION_D* pData, BOOL bTapered/*=FALSE*/, CWnd* pParent/*=NULL*/)
	: CDialogMove(ElemParamSectOffsetDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(ElemParamSectOffsetDlg)
	...
	m_aCtrlCenter.Add(IDC_TM_CENTROID_RD);
	m_aCtrlCenter.Add(IDC_TM_CENTER_RD);
	m_aCtrlVertical.Add(IDC_TM_VSIZE_RD);
	m_aCtrlVertical.Add(IDC_TM_VUSER_RD);
	m_aCtrlHorizont.Add(IDC_TM_HSIZE_RD);
	m_aCtrlHorizont.Add(IDC_TM_HUSER_RD);
	m_aCtrlUserOffsetRef.Add(IDC_TM_RDO_REF_CENT);
	m_aCtrlUserOffsetRef.Add(IDC_TM_RDO_REF_BEDGE);
}


void ElemParamSectOffsetDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(ElemParamSectOffsetDlg)
	DDX_Control(pDX, IDC_TM_VJ_UNIT, m_unitVJ);
	DDX_Control(pDX, IDC_TM_VJ_EDIT, m_editVJ);
	DDX_Control(pDX, IDC_TM_VI_UNIT, m_unitVI);
	DDX_Control(pDX, IDC_TM_VI_EDIT, m_editVI);
	DDX_Control(pDX, IDC_TM_OFFSET_CB, m_wndOffset);
	DDX_Control(pDX, IDC_TM_HJ_UNIT, m_unitHJ);
	DDX_Control(pDX, IDC_TM_HJ_EDIT, m_editHJ);
	DDX_Control(pDX, IDC_TM_HI_UNIT, m_unitHI);
	DDX_Control(pDX, IDC_TM_HI_EDIT, m_editHI);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(ElemParamSectOffsetDlg, CDialogMove)
	//{{AFX_MSG_MAP(ElemParamSectOffsetDlg)
	ON_BN_CLICKED(IDC_TM_VUSER_RD, OnCmdVerticalRadio)
	ON_BN_CLICKED(IDC_TM_HSIZE_RD, OnCmdHorizontalRadio)
	ON_CBN_SELCHANGE(IDC_TM_OFFSET_CB, OnSelchangeCmdOffsetCb)
	ON_BN_CLICKED(IDC_TM_VSIZE_RD, OnCmdVerticalRadio)
	ON_BN_CLICKED(IDC_TM_HUSER_RD, OnCmdHorizontalRadio)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// ElemParamSectOffsetDlg message handlers

BOOL ElemParamSectOffsetDlg::OnInitDialog() 
{
...
void ElemParamSectOffsetDlg::OnOK() 
{
...
void ElemParamSectOffsetDlg::OnCancel() 
{
	// TODO: Add extra cleanup here
	CDialogMove::OnCancel();
}

void ElemParamSectOffsetDlg::OnCmdVerticalRadio() 
{
...
void ElemParamSectOffsetDlg::OnCmdHorizontalRadio() 
{
	// TODO: Add your control notification handler code here
	int nOption;
...
void ElemParamSectOffsetDlg::InitOffsetCombo()
{
...
void ElemParamSectOffsetDlg::Data2Dlg()
{
...
BOOL ElemParamSectOffsetDlg::Dlg2Data()
{
...
void ElemParamSectOffsetDlg::OnSelchangeCmdOffsetCb() 
{
	// TODO: Add your control notification handler code here
	BOOL bEanbleH = TRUE;
	BOOL bEnableV = TRUE;
	...
	GetDlgItem(IDC_TM_HUSER_RD)->EnableWindow(bEanbleH);
	GetDlgItem(IDC_TM_HI_EDIT)->EnableWindow(bEanbleH);
	GetDlgItem(IDC_TM_HJ_EDIT)->EnableWindow(bEanbleH);
	if (bEanbleH) OnCmdHorizontalRadio();
	else CDlgUtil::CtrlRadioSetCheck(this, m_aCtrlHorizont, 0);

	GetDlgItem(IDC_TM_VUSER_RD)->EnableWindow(bEnableV);
	GetDlgItem(IDC_TM_VI_EDIT)->EnableWindow(bEnableV);
	GetDlgItem(IDC_TM_VJ_EDIT)->EnableWindow(bEnableV);	
...
void ElemParamSectOffsetDlg::EnableDisableCenterIs()
{
...
void ElemParamSectOffsetDlg::EnableDisableUserOffsetRef()
{
	int nVert, nHorz;
