#if !defined(__SECTRVBEMGRID_H__)
#define __SECTRVBEMGRID_H__
...
// CSectRVbemGrid window
#include "HeaderPre.h"

class CSectRVbemDlg;

class __MY_EXT_CLASS__ CSectRVbemGrid :  public CTBCommon, public CDBUpdateConnector
{
public:
// Construction
public:
	CSectRVbemGrid();
	CSectRVbemGrid(CWnd* pParent);
...
public:
	void Initialize();
	BOOL DeleteRecordByKey(const int &key, const T_VBEM_D &data);
	BOOL InsertRecordByKey(const int &key, const T_VBEM_D &data);
	...
	BOOL ValidateAndMakeRecord(CStringArray &value, CRowColArray &aCols, int &key, T_VBEM_D &data);
	...
	BOOL GetValue(ROWCOL nRow, UINT &key, T_VBEM_D &data);
	BOOL SetValue(ROWCOL nRow, const UINT &key, const T_VBEM_D &data);
	BOOL SetValue(CGXData* pData, ROWCOL nRow, const UINT &key, const T_VBEM_D &data, CRowColArray &awCols);
	void ConvStrToData(int i, CString& value, T_VBEM_D& data);
	void ConvDataToStr(int i, const T_VBEM_D& data, CString& value, const T_VBEM_K VbemK);

	void MakeSearchKey(const int &key, const T_VBEM_D &data, CStringArray &aKey);
...
public:
	...
	void ConvertUnit();
	virtual ~CSectRVbemGrid();
	std::map<T_VBEM_K, T_SECT_D> m_mapVbemToSect;
	std::map<T_VBEM_K, T_SECV_D> m_mapVbemToSecv;

	// Generated message map functions
protected:
	//{{AFX_MSG(CSectRVbemGrid)
		// NOTE - the ClassWizard will add and remove member functions here.
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()

public:
	static UINT WM_SECT_RFORC_GRID_VBEM_CLICKED;

public:
	int GetSelectedVbemKList(CArray<UINT, UINT>& aVbemK);
...
#endif // !defined(__SECTRVBEMGRID_H__)
