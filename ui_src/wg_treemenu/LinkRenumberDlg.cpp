// LinkRenumberDlg.cpp : implementation file
//
...
CLinkRenumberDlg::CLinkRenumberDlg(CWnd* pParent /*=NULL*/)
	: CMenuBarChildDlg(CLinkRenumberDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CLinkRenumberDlg)
	//m_NumTarget = 1;
	m_SelType = 1;
	//m_tmStartNode = "1";
	m_tmStartNlnk = _T("1");
...
void CLinkRenumberDlg::DoDataExchange(CDataExchange* pDX)
{
	CMenuBarChildDlg::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CLinkRenumberDlg)
	...
	DDX_Text(pDX, IDC_TM_START_LINK, m_tmStartNlnk);
...
BEGIN_MESSAGE_MAP(CLinkRenumberDlg, CMenuBarChildDlg)
	//{{AFX_MSG_MAP(CLinkRenumberDlg)
	...
	//ON_BN_CLICKED(IDC_TM_RENUM_LINK, OnTmRenumTarget)
	//ON_BN_CLICKED(IDC_TM_RENUM_LINKNODE, OnTmRenumTarget)
...
/////////////////////////////////////////////////////////////////////////////
// CLinkRenumberDlg message handlers

BOOL CLinkRenumberDlg::OnInitDialog() 
{
...
void CLinkRenumberDlg::AlignControl()
{
	CRect rRef;
	CRect rToMove;
	int nDistY;	
...
void CLinkRenumberDlg::SetInitCombString()
{
...
//void CLinkRenumberDlg::OnTmRenumTarget() 
//{		
//	UpdateData(TRUE);
//	SetEditControl();	
//}

void CLinkRenumberDlg::OnSelchangeSortMethodCmb() 
{  
	SetFrameSize();
	SetEditControl();
}

void CLinkRenumberDlg::SetFrameSize()
{
	...
	((CStructBndrDlg*)(GetParent()->GetParent()))->SetHolderSize(nDlgHeight);
}

void CLinkRenumberDlg::OnTmCylinderAxisRdo() 
{
	UpdateData(TRUE);
	SetEditControl();
}

void CLinkRenumberDlg::SetEditControl()
{
	int nSortMethod = m_SortMethodCmb.GetCurSel();
...
void CLinkRenumberDlg::OnSelchangeCarOrder1st() 
{
...
void CLinkRenumberDlg::OnSelchangeCarOrder2nd() 
{  
	CString strTemp;
...
void CLinkRenumberDlg::OnSelchangeCynOrder1st()
{
...
void CLinkRenumberDlg::OnSelchangeCynOrder2nd()
{  
	CString strTemp;
...
void CLinkRenumberDlg::OnSelchangeCynAxisOrder()
{
...
BOOL CLinkRenumberDlg::GetRot1stPoint(double &Px, double &Py, double &Pz)
{
...
BOOL CLinkRenumberDlg::GetRot2ndPoint(double &Px, double &Py, double &Pz)
{
...
BOOL CLinkRenumberDlg::GetPol1stPoint(double &Px, double &Py, double &Pz)
{
...
BOOL CLinkRenumberDlg::GetPol2ndPoint(double &Px, double &Py, double &Pz)
{
...
BOOL CLinkRenumberDlg::CheckOrthogonal()
{
...
void CLinkRenumberDlg::ExecuteCartesian()
{
 	UpdateData(TRUE);
	CArray<T_NODE_K, T_NODE_K> aNodeKey;
	CArray<T_ELEM_K, T_ELEM_K> aElemKey;
	//CArray<T_LKKY_K, T_LKKY_K> aLkkyKey;
	...
	//StartNode = (T_NODE_K)atoi(m_tmStartNode);
	StartNlnk = (T_NLNK_K)_ttoi(m_tmStartNlnk);
	// Numbering Target 을 체크한다. Node인지 Lkky인지 Both인지.
	// 동시에 Select Type을 체크. All 인지 By Select 인지.
	I_GENModelBase * pIGM = CTreeMenuBarBase::GetI_GENModelST();  

	if(m_SelType == 0)  //Link만 전부 Renumbering
	{
	...
	if (m_pDoc->m_pDataCtrl->RenumberingNlnk(aNodeKey, StartNlnk,OrderTemp))
		m_pDoc->m_pViewCtrl->UnselectAll(NULL);
}

void CLinkRenumberDlg::ExecuteCylindrical()
{
//	UpdateData(TRUE);
//	CArray<T_NODE_K, T_NODE_K> aNodeKey;
//	CArray<T_ELEM_K, T_ELEM_K> aElemKey;
...
//
//	StartNode = (T_NODE_K)atoi(m_tmStartNode);
//	StartLkky = (T_LKKY_K)atoi(m_tmStartLkky);
...
//		  {
//			  m_pDoc->m_pAttrCtrl->GetNodeKeyList(aNodeKey);
//			  m_pDoc->m_pDataCtrl->RenumberingCyn(aNodeKey,aElemKey,aLkkyKey,StartNode,StartElem,StartLkky,OrderTemp,m_org,m_rot,m_pol);
//		  }
//		  else  // By Selection
//		  {
//			  if(!pIGM->GetSelectedNodeKeyList(aNodeKey,TRUE))
...
//			  }
//			  if(m_pDoc->m_pDataCtrl->RenumberingCyn(aNodeKey,aElemKey,aLkkyKey,StartNode,StartElem,StartLkky,OrderTemp,m_org,m_rot,m_pol))
//				  m_pDoc->m_pViewCtrl->UnselectAll(NULL);
//		  }
//		  break;
//	  case 1:  // Link
//		  if(m_SelType == 0)  // All
//		  {
//			  m_pDoc->m_pAttrCtrl->GetLkkyKeyList(aLkkyKey);
//			  m_pDoc->m_pDataCtrl->RenumberingCyn(aNodeKey,aElemKey,aLkkyKey,StartNode,StartElem,StartLkky,OrderTemp,m_org,m_rot,m_pol);
//		  }
//		  else  // By Selection
//		  {
//				ASSERT(0); // 뷰 선택 가능해지면 작업
//// 			  if(!pIGM->GetSelectedLkkyKeyList(aLkkyKey,TRUE))
//// 			  {
//// 				  GSaveHistoryNF(_LS(IDS_WG_TREEMENU_Error___No_selected_link_)); return; 
//// 			  }
//// 			  if(m_pDoc->m_pDataCtrl->RenumberingCyn(aNodeKey,aElemKey,aLkkyKey,StartNode,StartElem,StartLkky,OrderTemp,m_org,m_rot,m_pol))
//// 				  m_pDoc->m_pViewCtrl->UnselectAll(NULL);
//		  }
//		  break;
//	  case 2:  // Node and Link
//		  if(m_SelType == 0)  // All
//		  {
//			  m_pDoc->m_pAttrCtrl->GetNodeKeyList(aNodeKey);
//			  m_pDoc->m_pAttrCtrl->GetLkkyKeyList(aLkkyKey);
...
//// 			  if(!pIGM->GetSelectedLkkyKeyList(aLkkyKey,TRUE)) SelectCheckLkky = TRUE;
//// 			  if(SelectCheckNode && SelectCheckLkky)
//// 			  {
...
void CLinkRenumberDlg::OnTmExecute()
{
	Execute();
}

void CLinkRenumberDlg::OnTmClose()
{
	CloseDlg();
}

int CLinkRenumberDlg::GetOrderOpt()
{
	int nRet = 0;
