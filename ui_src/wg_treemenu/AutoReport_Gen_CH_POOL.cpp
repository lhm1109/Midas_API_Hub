...
Report_Para_CH* AutoReport_Gen_CH_POOL::Para() const
{
		return AutoReport_Gen_Mgr::Inst()->Para();
}


int AutoReport_Gen_CH_POOL::TempletID()
{
		return 0;
}
BOOL AutoReport_Gen_CH_POOL::PreMakeData()
{
	Initial();
	m_Segments.clear();
...
void AutoReport_Gen_CH_POOL::InsertCoverPage()
{
	GEN_IFTAG_STR pSrc;
	GEN_IFTAG pTgt;
...
void AutoReport_Gen_CH_POOL::InsertCatalogPage()
{
	CString strText = _T("");
	strText = _T("");
	CString strTemp=_T("");
...
void AutoReport_Gen_CH_POOL::MakeContent()
{   
...
void AutoReport_Gen_CH_POOL::MakeTable_CH(GenSegmentElements &genElements, T_TABLE_D &TableD,  LPCTSTR lpCaption, LPCTSTR lpBookMark)
{
...
void AutoReport_Gen_CH_POOL::WriteCharpt1()
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+1,0) == FALSE)
		return;
	GenSegmentElements Elems;
	MakeTitle(Elems, _T("基本资料"),1,0);
	WriteCharpt1_1(Elems);
	WriteCharpt1_2(Elems);
	WriteCharpt1_3(Elems);
	WriteCharpt1_4(Elems);
	m_Segments.push_back(Elems);
}
void AutoReport_Gen_CH_POOL::WriteCharpt1_1(GenSegmentElements &Elems)
{    
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+1,1) == FALSE)
		return;
	MakeTitle(Elems, _T("工程概况"), 1, 1);  
	CString str1,strTol;
	T_DCON_D dconD;
	...
	if(dconD.bSpecialSeismic)
	{
		...
		str1.Format(_T("    结构设计安全等级: %s"),GetBold(strLevel[dconD.nClass]));
	}
	else
	{
	...
	//
	{
		...
		str1.Format(_T("    抗震设防类别: 地震分组为第%s组;抗震设防烈度%s度;设计基本地震加速度值为%sg，场地类别%s类;按%s度进行抗震构造措施。"),GetBold(strSDC),GetBold(nSfi),GetBold(dSfi),GetBold(strSC),GetBold(nSfi));
		MakeCompleteText(strTol, str1);
	}
	//
	{
		str1.Format(_T("    混凝土结构的环境类别：设计地面以及池体为__类，设计地面以上为__类；结构构件的裂缝控制等级及最大裂缝宽度：____级，ωmax=____mm"));
		MakeCompleteText(strTol, str1);
		str1.Format(_T("    建筑耐火等级为____级"));
		MakeCompleteText(strTol, str1);
		str1.Format(_T("    砌体砌筑质量等级要求为____级"));
		MakeCompleteText(strTol, str1);
		str1.Format(_T("    结构计算软件: midas Gen（2021 V2.1版本)"));
		MakeCompleteText(strTol, str1);
		CString strBook = _T("CHARPT_1_1_TXT");
		MakeText(Elems, strTol, strBook);
	}
	return;
}
void AutoReport_Gen_CH_POOL::WriteCharpt1_2(GenSegmentElements &Elems)
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+1,2) == FALSE)
		return;
	MakeTitle(Elems, _T("设计参数"), 1, 2);
	MakeTitle(Elems, _T("几何参数"), 1, 2, _T(""), 1);  
	MakeText(Elems,GetBold(_T("#用户自行输入#")));
	MakeTitle(Elems, _T("土层参数"), 1, 2, _T(""), 2);  
	CString str;
	MakeAlignText(str,_T("土的类别"),_T(""));
	MakeAlignText(str,_T("土的容重"),_T(""));
	MakeAlignText(str,_T("饱和土容重"),_T(""));
	MakeAlignText(str,_T("土壤内摩擦角"),_T(""));
	MakeAlignText(str,_T("地基承载力特征值"),_T(""));
	MakeAlignText(str,_T("水容重"),_T("10.00KN/m3"));
	MakeText(Elems, str);
	//
	MakeTitle(Elems, _T("材料参数"),1, 2, _T(""), 3);  
	//

	{
		...
		MakeTable(Elems, TableD ,_T("混凝土参数"), _T("CHARPT_1_2_TBL1"));  
	}
	//
	{
		T_DCRB_D dcrbD;
		...
		TableD.HeaderD.aTitleName.Add(_T("顶板上层"));
		TableD.HeaderD.aTitleName.Add(_T("顶板下层"));
		TableD.HeaderD.aTitleName.Add(_T("底板上层"));
		TableD.HeaderD.aTitleName.Add(_T("底板下层"));
		TableD.HeaderD.aTitleName.Add(_T("壁板内侧"));
		TableD.HeaderD.aTitleName.Add(_T("壁板外侧"));
		TableD.HeaderD.aTitleName.Add(_T("柱"));

		TableD.RecordD.aRecordData.SetSize(1);
		auto& record = TableD.RecordD.aRecordData;
		record[0].Add(Double2String_g(msrbD.dT_Slab_X));
		record[0].Add(Double2String_g(msrbD.dB_Slab_X));
		record[0].Add(Double2String_g(msrbD.dT_Slab_X));
		record[0].Add(Double2String_g(msrbD.dB_Slab_X));
		record[0].Add(Double2String_g(msrbD.dT_Slab_X));
		record[0].Add(Double2String_g(msrbD.dB_Slab_X));
		record[0].Add(Double2String_g(dcrbD.d0_Column));
		TableD.RecordD.nRecordRowCount = 1;
		TableD.RecordD.nRecordColCount = 7;
		MakeTable(Elems, TableD ,_T("砼保护层厚度（单位：mm）"), _T("CHARPT_1_2_TBL2"));  
	}
	//
	{
		CAutoRecoveryUnit unitSet(D_UNITSYS_LENGTH_INDEX_M, D_UNITSYS_FORCE_INDEX_KN, m_pDoc->m_pUnitCtrl);
		...
		MakeTable(Elems, TableD ,_T("设计用钢筋参数"), _T("CHARPT_1_2_TBL3"));  
	}
}
void AutoReport_Gen_CH_POOL::WriteCharpt1_3(GenSegmentElements &Elems)
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+1,3) == FALSE)
		return;
	MakeTitle(Elems, _T("设计依据"), 1, 3);
	CString str;
	MakeAlignText(str,_T("《给水排水工程钢筋混凝土水池结构设计规程》"),_T("CECS 138:2002"));
	MakeAlignText(str,_T("《给水排水工程构筑物设计规范》"),_T("GB50069-2002"));
	MakeAlignText(str,_T("《混凝土结构设计规范》"),_T("GB50010—2010"));
	MakeAlignText(str,_T("《地基基础设计规范》"),_T("GB50007—2011"));
	MakeAlignText(str,_T("《给排水工程结构设计手册》"),_T("中国建筑工业出版社"));
	MakeText(Elems, str);
}
void AutoReport_Gen_CH_POOL::WriteCharpt1_4(GenSegmentElements &Elems)
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+1,4) == FALSE)
		return;
	MakeTitle(Elems, _T("计算模型"), 1, 4);

	SetViewPoint(0);
	Report_Key_LIST aKeys;
	Para()->GroupKeys(aKeys,1000*ARGC_POOL);
	CString strBookMark;
	int nIndex = 1;
	CString strViewName[] = { _T("结构模型图"),_T("顶视图"),_T("左立面图"),_T("右立面图"),_T("前立面图"),_T("后立面图") };
	CString strGroupName,strName;
	int nDir[] = { 0,3,1,4,2,5 };
	for(int j = 0;j < aKeys.GetSize();j++)
	{
		...
		{
			SetViewPoint(nDir[i]);
			strName = strGroupName + _T("_") + strViewName[i];
			Replace_StrPath(strName);
			MakeImage_Current(strName, FALSE);
			strBookMark.Format(_T("CHARPT_1_4_FIG%d"), nIndex++);
			MakePicture(Elems, strName, strBookMark);
		}
	}
	SetViewPoint(0);
}
void AutoReport_Gen_CH_POOL::WriteCharpt2()
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+2,0) == FALSE)
		return;
	GenSegmentElements Elems;
	MakeTitle(Elems, _T("荷载计算"), 2, 0);
	CString str;
	str.Format(_T("    %s"),GetBold(_T("#用户自行输入#")));
	MakeText(Elems,str);
	str.Empty();
	MakeCompleteText(str,_T("    1)顶板重:Pt = ____"));
	MakeCompleteText(str,_T("    2)底板重:Pb = ____;池底压重:____"));
	MakeCompleteText(str,_T("    3)壁板重:Pw = ____"));
	MakeCompleteText(str,_T("    4)水池自重:Pp = ____"));
	MakeCompleteText(str,_T("    5)水重:Ps = ____"));
	MakeCompleteText(str,_T("    6)覆土重:Pf = ____"));
	MakeCompleteText(str,_T("    7)底板面积: ____"));
	MakeCompleteText(str,_T("    8)池体范围内水浮力: ____"));
	MakeCompleteText(str,_T("    9)总压力:Fk = ____"));
	MakeText(Elems, str);
	m_Segments.push_back(Elems);
}
void AutoReport_Gen_CH_POOL::WriteCharpt3()
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+3,0) == FALSE)
		return;
	GenSegmentElements Elems;
	MakeTitle(Elems, _T("地基承载力"), 3, 0);
	CString str;
	str.Format(_T("    %s"),GetBold(_T("#用户自行输入#")));
	MakeText(Elems,str);
	m_Segments.push_back(Elems);
}
void AutoReport_Gen_CH_POOL::WriteCharpt4()
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+4,0) == FALSE)
		return;
	GenSegmentElements Elems;
	MakeTitle(Elems, _T("抗浮验算"), 4, 0);
	CString str;
	str.Format(_T("    %s"),GetBold(_T("#用户自行输入#")));
	MakeText(Elems,str);
	m_Segments.push_back(Elems);
}
void AutoReport_Gen_CH_POOL::WriteCharpt5()
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+5,0) == FALSE)
		return;
	GenSegmentElements Elems;
	MakeTitle(Elems, _T("程序计算结果"), 5, 0);
	WriteCharpt5_1(Elems);
	WriteCharpt5_2(Elems);
	m_Segments.push_back(Elems);
}
void AutoReport_Gen_CH_POOL::WriteCharpt6()
{
	if (Para()->SegmentIsCheck(1000 * ARGC_POOL + 6, 0) == FALSE)
		return;
	GenSegmentElements Elems;
	MakeTitle(Elems, _T("结构计算"), 6, 0);

	WriteCharpt6_1(Elems);
	WriteCharpt6_2(Elems);
	m_Segments.push_back(Elems);
}
void AutoReport_Gen_CH_POOL::WriteCharpt5_1(GenSegmentElements &Elems)
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+5,1) == FALSE)
		return;
	MakeTitle(Elems, _T("荷载工况及组合") ,5, 1);   
	//DrawNodeAll(FALSE);
	CString str;
	...
	m_pDoc->m_pAttrCtrl->GetStldKeyList(rKeyList);
	{
		...
		TableD.RecordD.nRecordRowCount = TableD.RecordD.aRecordData.GetSize();
		TableD.RecordD.nRecordColCount = 4;
		MakeTable(Elems, TableD ,_T("荷载工况"), _T("CHARPT_5_1_TBL1"));  
	}
	//
	...
	T_GRUP_D groupD;
	{
		...
		Para()->GroupKeys(aGroupKey,1000*ARGC_POOL);//1:Mode Group
...

	{
		T_PRES_K  presKey;
		T_PRES_D  presData;
		CMap<T_STLD_K,T_STLD_K,BOOL,BOOL> mPresCount;
		auto pos = m_pDoc->m_pAttrCtrl->GetStartPres();
		while(pos)
		{
			m_pDoc->m_pAttrCtrl->GetNextPres(pos,presKey,presData);
			if(!mPresCount.PLookup(presData.LoadCaseKey))
				mPresCount.SetAt(presData.LoadCaseKey,TRUE);
		}

		T_ARPR_K  arprKey;
		T_ARPR_D  arprData;
		CMap<T_STLD_K,T_STLD_K,BOOL,BOOL> mArprCount;
		pos = m_pDoc->m_pAttrCtrl2->GetStartArpr();
		while(pos)
		{
			m_pDoc->m_pAttrCtrl2->GetNextArpr(pos,arprKey,arprData);
			if(!mArprCount.PLookup(arprData.StldK))
				mArprCount.SetAt(arprData.StldK,TRUE);
		}

		T_SDSP_K  SdspKey;
		T_SDSP_D  SdspData;
		CMap<T_STLD_K, T_STLD_K, BOOL, BOOL> mSdspCount;
		pos = m_pDoc->m_pAttrCtrl->GetStartSdsp();
		while (pos)
		{
			m_pDoc->m_pAttrCtrl->GetNextSdsp(pos, SdspKey, SdspData);
			if (!mSdspCount.PLookup(SdspData.LoadCaseKey))
				mSdspCount.SetAt(SdspData.LoadCaseKey, TRUE);
		}
		...
		dspOpt.m_Load.bAllGroup = TRUE;
		dspOpt.m_Load.nLoadCaseType = D_LOADCASE_STATIC;
		
		dspOpt.m_Load.nLoadCaseIndex = 0;//pos;
		dspOpt.m_Load.bTxtAll = TRUE;
		dspOpt.m_Load.m_bIsChanged = TRUE;
		int nIndex = 0;
		CString strBook;
		...
		for(int j = 0;j < aGroupK.GetSize();j++)
		{
			...
			{
				m_pDoc->m_pAttrCtrl->GetStld(rKeyList[i],stldD);
				dspOpt.m_Load.nLoadCaseIndex = rKeyList[i];
				dspOpt.m_Load.nLoadCase = rKeyList[i];
				if(mPresCount.PLookup(rKeyList[i]))
				{
					strBook.Format(_T("CHARPT_5_1_FIG_%d"),++nIndex);
					str.Format(_T("压力荷载_%s_%s"),stldD.LoadCaseName, strGroupName);
					Replace_StrPath(str);
					dspOpt.m_Load.bPres = TRUE;
					ExportDisplaySetting::SET(&dspOpt);
					MakeImage_Current(str, FALSE);
					MakePicture(Elems, str,strBook);
					dspOpt.m_Load.bPres = FALSE;
				}
				//
				if(mArprCount.PLookup(rKeyList[i]))
				{
					strBook.Format(_T("CHARPT_5_1_FIG_%d"),++nIndex);
					str.Format(_T("区域压力荷载_%s_%s"),stldD.LoadCaseName, strGroupName);
					Replace_StrPath(str);
					dspOpt.m_Load.bArpr = TRUE;
					ExportDisplaySetting::SET(&dspOpt);
					MakeImage_Current(str, FALSE);
					MakePicture(Elems, str,strBook);
					dspOpt.m_Load.bArpr = FALSE;
				}
				
				if (mSdspCount.PLookup(rKeyList[i]))
				{
					strBook.Format(_T("CHARPT_5_1_FIG_%d"), ++nIndex);
					str.Format(_T("节点强制位移_%s_%s"), stldD.LoadCaseName, strGroupName);
					Replace_StrPath(str);
					dspOpt.m_Load.bDisplacement = TRUE;
					ExportDisplaySetting::SET(&dspOpt);
					MakeImage_Current(str, FALSE);
					MakePicture(Elems, str, strBook);
					dspOpt.m_Load.bDisplacement = FALSE;
				}
...

	{
		...
		MakeTable(Elems, TableD ,_T("荷载组合"), _T("CHARPT_5_1_TBL2"));  
	}
}
void AutoReport_Gen_CH_POOL::WriteCharpt5_2(GenSegmentElements &Elems)
{
	if(Para()->SegmentIsCheck(1000*ARGC_POOL+5,2) == FALSE)
		return;
	MakeTitle(Elems, _T("板单元设计内力结果") ,5, 2);   
	//-----------------------------------------------------------------------//
	Report_Key_LIST aKeys;
	Para()->ListReportOption(aKeys,ARGC_POOL);
	if(aKeys.IsEmpty())
		return;
	SetViewPoint(0);
	...
	CString strRltTypeName[] = {_T("板单元内力(中心值)"),_T("板单元内力(最大值)"),_T("配筋结果"),_T("裂缝验算结果")};
	...
	for(int i = 0;i < aKeys.GetSize();i+=4)
	{
		...
		//ForceD.m_PForcComponent = nForceType[aKeys[i + 2].m_nKey];
...
void AutoReport_Gen_CH_POOL::WriteCharpt6_1(GenSegmentElements &Elems)
{
	if (Para()->SegmentIsCheck(1000 * ARGC_POOL + 6, 1) == FALSE)
		return;
	MakeTitle(Elems, _T("水池构件配筋计算"), 6, 1);
	//-----------------------------------------------------------------------//
	Report_Key_LIST aKeys;
	Para()->ListReportOption(aKeys, ARGC_POOL);
	if (aKeys.IsEmpty())
	...
	for (int k = 0; k < 2; k++)
	{
		...
		for (int i = 0; i < aReportKeys[k].GetSize(); i += 4)
		{
			...
			for (int m = 0; m < nSbdo; m++)
			{
				...
				for (int n = 0; n < 2; n++)
				{
					...
					gm->m_InvalidateFlag = TRUE;
					MakeImage_Current(str, FALSE);
					WriteCharpt6_1_TableMaxElemInSbdo(TableD.RecordD.aRecordData,SbdoD,aSbdoElem, nIndex, strLoad,n,pCodeCheck);
		...
		MakeText(Elems, str);
		if (k == 0)
			MakeTable(Elems, TableD, _T("单元中心值配筋结果"), _T("CHARPT_6_2_TBL1"));
		else
			MakeTable(Elems, TableD, _T("单元最大值配筋结果"), _T("CHARPT_6_2_TBL2"));
...
void AutoReport_Gen_CH_POOL::WriteCharpt6_1_TableMaxElemInSbdo(CArray<CStringArray, CStringArray &>& recond, const T_SBDO_D& SbdoD,const T_ELEM_K_LIST& aSbdoElem, 
...
void AutoReport_Gen_CH_POOL::WriteCharpt6_2(GenSegmentElements &Elems)
{
	if (Para()->SegmentIsCheck(1000 * ARGC_POOL + 6, 2) == FALSE)
		return;
	MakeTitle(Elems, _T("板正常使用状态验算"), 6, 2);
	//-----------------------------------------------------------------------//
	Report_Key_LIST aKeys;
	Para()->ListReportOption(aKeys, ARGC_POOL);
	if (aKeys.IsEmpty())
	...
	for (int k = 0; k < 2; k++)
	{
		...
		for (int i = 0; i < aReportKeys[k].GetSize(); i += 4)
		{
			...
			for (int m = 0; m < nSbdo; m++)
			{
				...
				for (int n = 0; n < 2; n++)
				{
					if(!WriteCharpt6_2_TableMaxElemInSbdo(TableD.RecordD.aRecordData, SbdoD, aSbdoElem, nIndex, strLoad, n, pCodeCheck))
						continue;
					strDir = n == 0 ? _T("方向1") : _T("方向2");
		...
		MakeText(Elems, str);
		if(k == 0)
			MakeTable(Elems, TableD, _T("单元中心值裂缝"), _T("CHARPT_6_2_TBL1"));
		else
			MakeTable(Elems, TableD, _T("单元最大值裂缝"), _T("CHARPT_6_2_TBL2"));
		for (int i = 0; i < aStrPath.GetSize(); i++)
		{
			MakePicture(Elems, aStrPath[i], aStrBook[i]);
		}
...
BOOL AutoReport_Gen_CH_POOL::WriteCharpt6_2_TableMaxElemInSbdo(CArray<CStringArray, CStringArray &>& recond, const T_SBDO_D& SbdoD, const T_ELEM_K_LIST& aSbdoElem, 
	int nIndex, const CString& strLoad, int nDir, CRCSCodeCheck* pCodeCheck)
{
...
void AutoReport_Gen_CH_POOL::setTableFormatString(int nlocation /* = 0 */,int ncolor /* = 16777215 */)
{
