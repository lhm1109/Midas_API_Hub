...
void CDB_THRD::Add(T_THRD_K Key,T_THRD_D& rData,CDB_NODE* pNode,CDB_ELEM* pElem)
{
	T_THRD_D Data;
	BOOL bExist=m_thrd.Lookup(Key, Data);
	if(bExist)
	{
		if(Data.nGraphStep == 0)
		{
			...
			{
				if(pNode != NULL)VERIFY(pNode->DelListItem(Data.nEntity, LT_THRD_CMD, Key));
			}
			else if(Data.nFunctionType >= 3 && Data.nFunctionType <= 7)
			{
				if(pElem != NULL)VERIFY(pElem->DelListItem(Data.nEntity, LT_THRD_CMD, Key));
			}
			...
			{
				if(Data.nElemType4Iehg == 0 || Data.nElemType4Iehg == 2 || Data.nElemType4Iehg == 3) // beam or truss or wall
				{
					if(pElem != NULL)VERIFY(pElem->DelListItem(Data.nEntity, LT_THRD_CMD, Key));
				}
			}
	...
	if(rData.nGraphStep == 0)
	{
		...
		{
			if(pNode != NULL)pNode->AddListItem(rData.nEntity, LT_THRD_CMD, Key);
		}
		else if(rData.nFunctionType >= 3 && rData.nFunctionType <= 7)
		{
			if(pElem != NULL)pElem->AddListItem(rData.nEntity, LT_THRD_CMD, Key);
		}    
		else if(rData.nFunctionType == 9)
		{
			if(rData.nElemType4Iehg == 0 || rData.nElemType4Iehg == 2 || rData.nElemType4Iehg == 3) // beam or truss or wall
			{
				if(pElem != NULL)pElem->AddListItem(rData.nEntity, LT_THRD_CMD, Key);
			}
		}
	...
	{
		if (!bExist) m_pDoc->m_pStagCtrl->ActiveEtc(LT_THRD_CMD, Key);
	}
}

BOOL CDB_THRD::Del(T_THRD_K Key,CDB_NODE* pNode,CDB_ELEM* pElem)
{
	T_THRD_D Data;
	m_thrd.Lookup(Key, Data);
	BOOL ret=m_thrd.RemoveKey(Key);
	ASSERT(ret);
	if(ret)
	{
		if(Data.nGraphStep == 0)
		...
		if (m_pDoc && m_pDoc->m_pStagCtrl->GetCurStag() != 0)
		{
			m_pDoc->m_pStagCtrl->DeactiveEtc(LT_THRD_CMD, Key);
		}
	}
	return ret;
}
...
//-------------------------------------------------------------------------
BOOL CDB_THRD::Get(T_THRD_K Key,T_THRD_D& rData)
{
	if (m_pDoc && m_pDoc->m_pStagCtrl->GetCurStag() != 0)
	{
		UINT uTemp;
		if (!m_pDoc->m_pStagCtrl->m_thrd->Lookup(Key, uTemp)) return FALSE;
	}
	return m_thrd.Lookup(Key,rData);
}

int CDB_THRD::GetCount()
{
	if (m_pDoc && m_pDoc->m_pStagCtrl->GetCurStag() != 0)
	{
		return m_pDoc->m_pStagCtrl->m_thrd->GetCount();
	}
	return m_thrd.GetCount();
}

POSITION CDB_THRD::GetStart()
{
	if (m_pDoc && m_pDoc->m_pStagCtrl->GetCurStag() != 0)
	{
		return m_pDoc->m_pStagCtrl->m_thrd->GetStartPosition();
	}
	return m_thrd.GetStartPosition();
}

void CDB_THRD::GetNext(POSITION& rNextPosition,T_THRD_K& rKey,T_THRD_D& rData)
{
	if (m_pDoc && m_pDoc->m_pStagCtrl->GetCurStag() != 0)
	{
		UINT uTemp;
		m_pDoc->m_pStagCtrl->m_thrd->GetNextAssoc(rNextPosition, rKey, uTemp);
		m_thrd.Lookup(rKey, rData);
		return;
	}
