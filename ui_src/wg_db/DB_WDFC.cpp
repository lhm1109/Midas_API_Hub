...
CDB_WDFC::CDB_WDFC()
{
	m_nStartNum = 1;
	m_nLastNum  = 0;
	m_wvep.InitHashTable(HASHSIZEWDFC);
	m_wveplist.InitHashTable(HASHSIZELISTWDFC);
}
...
void CDB_WDFC::Add(T_WDFC_K Key,T_WDFC_D& rData)
{
	T_WDFC_D data;
...
BOOL CDB_WDFC::Del(T_WDFC_K Key)
{
...
void CDB_WDFC::AddList(T_WDFC_K Key)
{
	WdfcList* pList = new WdfcList;
	m_wveplist.SetAt(Key, pList);
}

BOOL CDB_WDFC::DelList(T_WDFC_K Key)
{
	WdfcList* pList;
...
void CDB_WDFC::AddListItem(T_WDFC_K Key, int nCmd, int nKey)
{
	LaplList* pList;
...
BOOL CDB_WDFC::DelListItem(T_WDFC_K Key, int nCmd, int nKey)
{
	LaplList* pList;
...
BOOL CDB_WDFC::GetList(T_WDFC_K Key,WdfcList*& rpList)
{
