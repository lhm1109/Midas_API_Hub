// CMRebarPscMomentGrid.cpp : implementation file
//
// 2004. 03. 09    by TAE
/////////////////////////////////////////////////////////////////////////////
//   1        2         3      4      5       6        7  
// ---------------------------------------------------------
//  Dia |  Number |  Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
...
/////////////////////////////////////////////////////////////////////////////
// CCMRebarPscMomentGrid

CCMRebarPscMomentGrid::CCMRebarPscMomentGrid(CCMRebarPscMoment* pParent)
{
	ASSERT(pParent);
	m_pParent = pParent;
...
CCMRebarPscMomentGrid::~CCMRebarPscMomentGrid()
{
}

//--------------------------------------------------------------------------
// Interface
void CCMRebarPscMomentGrid::Initialize(CArray<T_RPSC_MBAR, T_RPSC_MBAR&> *pData, BOOL bSym)
{
	ASSERT(pData);
	m_pData = pData;
	m_bSymmetry = bSym;
...
void CCMRebarPscMomentGrid::MakeItemEx()
{
	CWaitCursor wait;
...
void CCMRebarPscMomentGrid::ChangeSymmetry(BOOL bSym)
{
	m_bSymmetry = bSym;
...
void CCMRebarPscMomentGrid::UpdateBuffer(int nCmd, CArray<int, int> &aOldIndex, CArray<int, int> &aIndex, CArray<T_RPSC_MBAR, T_RPSC_MBAR&> &aData)
{
...
CString CCMRebarPscMomentGrid::GetArrangeStr(int data)
{
...
int CCMRebarPscMomentGrid::GetArrangeData(CString &value)
{
...
CString CCMRebarPscMomentGrid::GetRefPosStr(int data)
{
...
int CCMRebarPscMomentGrid::GetRefPosData(CString &value)
{
...
//--------------------------------------------------------------------------
// Implementation
void CCMRebarPscMomentGrid::SetColInfo()
{
	...
	// z, spacing
	SetStyleRange(CGXRange().SetCols(6, 7), CGXStyle()
			.SetFormat(GX_FMT_FIXED)
			.SetPlaces(2));
...
void CCMRebarPscMomentGrid::SetValue(ROWCOL nRow, int index, T_RPSC_MBAR &data)
{
	CStringArray aValue;
...
void CCMRebarPscMomentGrid::SetHeaderTitle()
{
	BOOL bOldLock = LockUpdate(TRUE);
	...
	CString aTitle[] = {_LS(IDS_CMD_FPSC_Dia), _LS(IDS_CMD_FPSC_Number), _LS(IDS_CMD_FPSC_Ref_Y), _LS(IDS_CMD_FPSC_Y), 
		_LS(IDS_CMD_FPSC_Ref_Z), _LS(IDS_CMD_FPSC_Z), _LS(IDS_CMD_FPSC_Spacing)};
	int aWidth[] = {60, 60, 65, 60, 60, 60, 60};
...
void CCMRebarPscMomentGrid::ConvDataToStrArray(T_RPSC_MBAR &data, CStringArray &aValue)
{
//   1        2         3      4      5       6        7  
// ---------------------------------------------------------
//  Dia |  Number |  Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
	...
	aValue[0] = data.strBarNa;
	aValue[1].Format(_T("%d"), data.iBarNum);
	aValue[2] = GetArrangeStr(data.iStaYRef);
	aValue[3].Format(DATA_FMT, data.dStaY);
	aValue[4] = GetRefPosStr(data.iStaZRef);
	aValue[5].Format(DATA_FMT, data.dStaZ);
	aValue[6].Format(DATA_FMT, data.dPitch);
}

BOOL CCMRebarPscMomentGrid::InsertRecordByKey(int index, T_RPSC_MBAR &data)
{
...
BOOL CCMRebarPscMomentGrid::DeleteRecordByKey(int index, T_RPSC_MBAR &data)
{
	ROWCOL ncRow, ncCol;
...
void CCMRebarPscMomentGrid::SelectGridRow(long* aRow, long nSize)
{
...
BOOL CCMRebarPscMomentGrid::GetValue(ROWCOL nRow, T_RPSC_MBAR &data)
{
	data.Initialize();
...
BOOL CCMRebarPscMomentGrid::OnValidateField(ROWCOL nRow, ROWCOL nCol)
{
	CString value;
	int nColID;
...
// CTBBrowserWnd::ProcessKeys 참조
BOOL CCMRebarPscMomentGrid::OnValidateRecord(ROWCOL nRow)
{
...
BOOL CCMRebarPscMomentGrid::ValidateField(CString& value, int nColID)
{
//   1        2         3      4      5       6        7  
// ---------------------------------------------------------
//  Dia |  Number |  Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
	...
	if (nColID == 0 || nColID == 2 || nColID == 4) return TRUE;
	else if (nColID == 1)
	{
	...
	else if (nColID == 3 || nColID == 5 || nColID == 6)
	{
...
BOOL CCMRebarPscMomentGrid::ValidateRecord(CStringArray &aValue, CRowColArray &aCols)
{
	CString aTitle[] = {_LS(IDS_CMD_FPSC_Dia), _LS(IDS_CMD_FPSC_Number), _LS(IDS_CMD_FPSC_Ref_Y), _LS(IDS_CMD_FPSC_Y),
		_LS(IDS_CMD_FPSC_Ref_Z), _LS(IDS_CMD_FPSC_Z), _LS(IDS_CMD_FPSC_Spacing)};
...
BOOL CCMRebarPscMomentGrid::OnAddNewRecord(ROWCOL nRow)
{
	BOOL bOldLock = LockUpdate(TRUE);
	if (!AddToDB(nRow))
	{
...
BOOL CCMRebarPscMomentGrid::AddToDB(ROWCOL nRow)
{
	int nIndex;
	T_RPSC_MBAR data;
...
BOOL CCMRebarPscMomentGrid::AddData(int nIndex, T_RPSC_MBAR &data)
{
...
BOOL CCMRebarPscMomentGrid::OnModifyRecord(ROWCOL nRow)
{
	// 수정한다. 실패하면 에러 리턴
	BOOL bOldLock = LockUpdate(TRUE);
	if (!ModifyToDB(nRow))
...
BOOL CCMRebarPscMomentGrid::ModifyToDB(ROWCOL nRow)
{
	int nIndex;
	T_RPSC_MBAR data;
...
BOOL CCMRebarPscMomentGrid::ModifyData(int nOldIndex, int nIndex, T_RPSC_MBAR &data)
{
	CArray<int, int> aOldIndex, aIndex;
	CArray<T_RPSC_MBAR, T_RPSC_MBAR&> aData;
...
BOOL CCMRebarPscMomentGrid::OnDeleteRecord(ROWCOL nRow)
{
	// save current row
	ROWCOL ncRow, ncCol;
	GetCurrentCell(ncRow, ncCol);
...
BOOL CCMRebarPscMomentGrid::OnDeleteRecords(CRowColArray &awRows)
{
	// save current row
	ROWCOL ncRow, ncCol;
	GetCurrentCell(ncRow, ncCol);
...
BOOL CCMRebarPscMomentGrid::DeleteFromDB(CRowColArray &awRows)
{
	int nIndex;
...
void CCMRebarPscMomentGrid::OnChangedSelection(const CGXRange *changedRect, BOOL bIsDraggin, BOOL bKey)
{
	long nSize, nTmpSize;
	if (changedRect)
	{
...
BOOL CCMRebarPscMomentGrid::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
	if (IsEdit()) 
	{
...
BOOL CCMRebarPscMomentGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CCMRebarPscMomentGrid::PasteFromClipboard(const CGXRange &range, CString* aDefVal, int nKeyCol, 
...
BOOL CCMRebarPscMomentGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bReadOnlyMode;
	BOOL bCurrentRow;
	BOOL bCurrentCell;
...
BOOL CCMRebarPscMomentGrid::MakeDataArray(CStringArray& value, CRowColArray& awCols, 
																		CArray<T_RPSC_MBAR, T_RPSC_MBAR&>& aData)
{
	T_RPSC_MBAR data;
	data.Initialize();
...
BOOL CCMRebarPscMomentGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, T_RPSC_MBAR &data)
{
...
BOOL CCMRebarPscMomentGrid::ConvStrArrayToData(CStringArray &aValue, T_RPSC_MBAR& data)
{
//   1        2         3      4      5       6        7  
// ---------------------------------------------------------
//  Dia |  Number |  Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 

	data.strBarNa = aValue[0];
	data.iBarNum = _ttoi(aValue[1]);
	data.iStaYRef = GetArrangeData(aValue[2]);
	data.dStaY = _tstof(aValue[3]);
	data.iStaZRef = GetRefPosData(aValue[4]);
	data.dStaZ = _tstof(aValue[5]);
	data.dPitch = _tstof(aValue[6]);

	return TRUE;
}


BEGIN_MESSAGE_MAP(CCMRebarPscMomentGrid, CTBBrowserWnd)
	//{{AFX_MSG_MAP(CCMRebarPscMomentGrid)
	//ON_WM_LBUTTONDOWN()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CCMRebarPscMomentGrid message handlers

/*
// 마우스 클릭시 번호 컬럼이 헤더 컬럼처럼 동작하게 한다.
void CCMRebarPscMomentGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
	ROWCOL nhRow, nhCol;
...
BOOL CCMRebarPscMomentGrid::OnInsertEmptyRecord(ROWCOL nRow)
{
...
CString CCMRebarPscMomentGrid::GetDefValue(int ix)
{
	if (ix == 0) return _T("");
	else if (ix == 1) return _T("0");
	else if (ix == 2) return D_RPSC_REF_CENTROID;
	else if (ix == 3) return _T("0");
	else if (ix == 4) return D_RPSC_REF_TOP;
...
BOOL CCMRebarPscMomentGrid::HideColsByIntend(int nFromCol, int nToCol, BOOL bHide)
{
...
BOOL CCMRebarPscMomentGrid::IsHiddenByIntend(int nCol)
{
	CGXStyle style;
	CString csHide;
...
int CCMRebarPscMomentGrid::HitTest(CPoint& pt, ROWCOL* pnRow, ROWCOL* pnCol, CRect* rectHit)
{
...
// HideColsByIntend()함수를 도와주는 함수 
BOOL CCMRebarPscMomentGrid::OnTrackColWidth(ROWCOL nCol)
{
...
// HideColsByIntend()함수를 도와주는 함수
BOOL CCMRebarPscMomentGrid::HideCols(ROWCOL nFromCol, ROWCOL nToCol, BOOL bHide, BOOL *abHideArray, UINT flags, GXCmdType ctCmd)
{
...
BOOL CCMRebarPscMomentGrid::Copy()
{
