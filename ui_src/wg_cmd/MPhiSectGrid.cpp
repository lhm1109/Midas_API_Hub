// CMPhiSectGrid.cpp : implementation file
//
...
#define D_COL_DIR 	  6
#define D_COL_D 	    7
#define D_COL_ALPHA 	8
#define D_COL_BETA 	  9
#define D_COL_015H 	  10
#define D_COL_NS 	    11
#define D_COL_D2 	    12
#define D_COL_L 	    13
#define D_COL_A 	    14
...
void CMPhiSectGrid::SetData(CArray<T_SECT_DATA, T_SECT_DATA&>& aData)
{
	BOOL bOldLock = LockUpdate(TRUE);

	SetStyleRange(CGXRange().SetCols(D_COL_RF, D_COL_C), CGXStyle()
		.SetReadOnly(FALSE));
	...
	for (int i = 0; i < nSize; i += 2)
	{
		...
		SetCoveredCellsRowCol(nRow, D_COL_POS, nRow + 1, D_COL_POS);
		SetCoveredCellsRowCol(nRow, D_COL_C, nRow + 1, D_COL_C);
		SetCoveredCellsRowCol(nRow, D_COL_USE, nRow + 1, D_COL_USE);
	}
	...
	for (int i = 0; i < nSize; ++i)
	{
		...
		if (i % 2 == 0)
		{
			...
			aValue[D_COL_POS - 1] = GetPosString(aData[nDataIdx]);
			...
			aValue[D_COL_L - 1].Format(_T("%g"), aData[nDataIdx].MpstD.dlY);
			aValue[D_COL_A - 1].Format(_T("%g"), aData[nDataIdx].MpstD.da);
			aValue[D_COL_RF - 1].Format(_T("%.3f"), aData[nDataIdx].dRFY); // 일본 OS에서 실행하면 소수점 3째자리까지 안 짤리고 표현이 되고, 이를 넘어가면 2줄로 표현이 되어 버리기 때문에... 소수점 3째자리 까지만 표현하고 나머지 값은 날려버리도록 협의 함
			aValue[D_COL_RR - 1].Format(_T("%.3f"), aData[nDataIdx].dRRY);
			aValue[D_COL_C - 1].Format(_T("%.3f"), aData[nDataIdx].MpstD.dc);
			aValue[D_COL_USE - 1].Format(_T("%d"), aData[nDataIdx].MpstD.bUse);
		}
		else
		{
			...
			aValue[D_COL_L - 1].Format(_T("%g"), aData[nDataIdx].MpstD.dlZ);
			aValue[D_COL_A - 1].Format(_T("%g"), aData[nDataIdx].MpstD.daZ);
			aValue[D_COL_RF - 1].Format(_T("%.3f"), aData[nDataIdx].dRFZ); // 일본 OS에서 실행하면 소수점 3째자리까지 안 짤리고 표현이 되고, 이를 넘어가면 2줄로 표현이 되어 버리기 때문에... 소수점 3째자리 까지만 표현하고 나머지 값은 날려버리도록 협의 함
			aValue[D_COL_RR - 1].Format(_T("%.3f"), aData[nDataIdx].dRRZ);
			aValue[D_COL_C - 1].Format(_T("%.3f"), aData[nDataIdx].MpstD.dc);
			aValue[D_COL_USE - 1].Format(_T("%d"), aData[nDataIdx].MpstD.bUse);
		}
	...
	SetStyleRange(CGXRange().SetCols(D_COL_RF, D_COL_RR), CGXStyle()
		.SetReadOnly(TRUE));
...
int CMPhiSectGrid::GetPosVal(CString strPos)
{
...
CString CMPhiSectGrid::GetPosString(const T_SECT_DATA& data)
{
	CString strReturn;
	if(!data.bCandidate)
	{
...
BOOL CMPhiSectGrid::GetData(CArray<T_SECT_DATA, T_SECT_DATA&>& aData)
{
	//aData.RemoveAll();
	...
	for (int i = 0; i < nCount - 1; ++i)
	{
		...
		if (i % 2 == 0)
		{
			...
			aData[nDataIdx].MpstD.nPosition = GetPosVal(aValue[D_COL_POS - 1]);
			...
			aData[nDataIdx].MpstD.dlY = _tstof(aValue[D_COL_L - 1]);
			aData[nDataIdx].MpstD.da = _tstof(aValue[D_COL_A - 1]);
			//aData[nDataIdx].dRFY       = _tstof(aValue[D_COL_RF     - 1]);
			//aData[nDataIdx].dRRY       = _tstof(aValue[D_COL_RR     - 1]);
			aData[nDataIdx].MpstD.dc = _tstof(aValue[D_COL_C - 1]);
			aData[nDataIdx].MpstD.bUse = _ttoi(aValue[D_COL_USE - 1]);
		}
		else
		{
			//aData[nDataIdx].SectK            = _ttoi(aValue[D_COL_ID     - 1]);
			...
			aData[nDataIdx].MpstD.dlZ = _tstof(aValue[D_COL_L - 1]);
			aData[nDataIdx].MpstD.daZ = _tstof(aValue[D_COL_A - 1]);
			//aData[nDataIdx].dRFZ       = _tstof(aValue[D_COL_RF     - 1]);
			//aData[nDataIdx].dRRZ       = _tstof(aValue[D_COL_RR     - 1]);
			//aData[nDataIdx].MpstD.bUse       = _ttoi(aValue[D_COL_USE    - 1]);
		}
	}
	return TRUE;
...
void CMPhiSectGrid::SetDataToRow(ROWCOL nRow, CStringArray& raValue)
{
...
BOOL CMPhiSectGrid::GetRowToData(ROWCOL nRow, CStringArray& raValue)
{
	int nColCount = GetColCount();
	ASSERT(nColCount == raValue.GetSize());
...
CString CMPhiSectGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString strNum(_T(""));
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CMPhiSectGrid, CTBBrowserWnd)
	//{{AFX_MSG_MAP(CMPhiSectGrid)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CMPhiSectGrid message handlers

void CMPhiSectGrid::Initialize(BOOL bCodeExist[6])
{
	CTBBrowserWnd::Initialize();

	for (int i = 0; i < 6; i++)
	{
		m_bCodeExist[i] = bCodeExist[i];
	}
	...
	//szChoiceList.Format(_T("%s\n%s\n%s\n%s\n"), _LS(IDS_CMD_MPHI_NONE), _LS(IDS_CMD_MPHI_SELECT_I), _LS(IDS_CMD_MPHI_SELECT_M), _LS(IDS_CMD_MPHI_SELECT_J));
	szChoiceList.Format(_T("%s\n%s\n%s\n"), _LS(IDS_CMD_MPHI_SELECT_I), _LS(IDS_CMD_MPHI_SELECT_M), _LS(IDS_CMD_MPHI_SELECT_J));
	SetStyleRange(CGXRange().SetCols(D_COL_POS), CGXStyle()
	...
	SetStyleRange(CGXRange().SetCols(D_COL_D, D_COL_C), CGXStyle()
			.SetHorizontalAlignment(DT_RIGHT)
			.SetValueType(GX_VT_NUMERIC)
			.SetEnabled(TRUE));  

	SetStyleRange(CGXRange().SetCols(D_COL_USE), CGXStyle()
	...
	aWidth[D_COL_NAME  - 1] = 94;
	aWidth[D_COL_SHAPE - 1] = 44;
	aWidth[D_COL_REBAR - 1] = 42;
	aWidth[D_COL_POS 	 - 1] = 33;
	aWidth[D_COL_DIR 	 - 1] = 33;
	aWidth[D_COL_D     - 1] = 35;
	aWidth[D_COL_ALPHA - 1] = 35;
	aWidth[D_COL_BETA  - 1] = 35;
	aWidth[D_COL_015H  - 1] = 35;
	aWidth[D_COL_NS 	 - 1] = 35;
	aWidth[D_COL_D2 	 - 1] = 35;
	aWidth[D_COL_L 	   - 1] = 35;
	aWidth[D_COL_A 	   - 1] = 35;
	aWidth[D_COL_RF 	 - 1] = 35;
	aWidth[D_COL_RR 	 - 1] = 35;
	aWidth[D_COL_C  	 - 1] = 35;
	aWidth[D_COL_USE   - 1] = 30;
	...
	SetValueRange(CGXRange(0, D_COL_POS), _LS(IDS_CMD_MPHI_POSITION));
	...
	SetValueRange(CGXRange(0, D_COL_L), _LS(IDS_CMD_MPHI_L));
	SetValueRange(CGXRange(0, D_COL_A), _LS(IDS_CMD_MPHI_A_CM));
	SetValueRange(CGXRange(0, D_COL_RF), _LS(IDS_CMD_MPHI_RF));
	SetValueRange(CGXRange(0, D_COL_RR), _LS(IDS_CMD_MPHI_RR));
	SetValueRange(CGXRange(0, D_COL_C), _LS(IDS_CMD_MPHI_c));
	SetValueRange(CGXRange(0, D_COL_USE), _LS(IDS_CMD_MPHI_USE));

	LockUpdate(FALSE);
}

void CMPhiSectGrid::SetSectionDataEdit(ROWCOL nRow, T_SECT_DATA& data)
{
    ...
    if(data.strShape == D_SECT_SHAPE_REG_P && m_bCodeExist[5] && bBrace) // 일본 pipe 경사말뚝 예외처리
        bCol = TRUE;
	...
	if(nRow % 2 == 1) // y축
	{
		...
		// Common
		//SetStyleRange(CGXRange().SetCells(nRow, D_COL_POS), CGXStyle().SetEnabled(data.bCandidate));  // 입력할 Data가 하나도 없으면 disable 하는 것이 정상이지만.. 이 복잡한 상황에서 재대로 체크할려면 너무 지저분해지니깐.. 그냥 열어놔도 되겠지.. 어차피 Use 열도 마찬가지로 열려 있으니..
		
		BOOL bIsTrueExcept5 = FALSE;
		for (int i = 0; i < 6; i++)
		{
			if(i == 5) continue;
			if(m_bCodeExist[i])
		...
		// RC
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_D), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate));
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_ALPHA), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate));
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_BETA), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate));
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_015H), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && m_bCodeExist[4]));
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_NS), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && m_bCodeExist[4]));
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_D2), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && m_bCodeExist[4]));
		// Steel,SRC
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_L), CGXStyle().SetEnabled(((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate && m_bCodeExist[4]));
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_A), CGXStyle().SetEnabled(((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate && 
			data.strShape != D_SECT_SHAPE_REG_P && data.strShape != D_SECT_SHAPE_REG_PSTF && data.strShape != D_SECT_SHAPE_SRC_PSTF && data.strShape != D_SECT_SHAPE_SRC_EPC)); // PMS.5568 180313 일본내진 m-phi EPC 단면 추가
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_RF), CGXStyle().SetEnabled(((((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate)) && (!(data.strShape == D_SECT_SHAPE_REG_P && m_bCodeExist[5]) || bIsTrueExcept5)));
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_RR), CGXStyle().SetEnabled(((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate && 
			data.strShape != D_SECT_SHAPE_REG_P && data.strShape != D_SECT_SHAPE_REG_PSTF && data.strShape != D_SECT_SHAPE_SRC_PSTF && data.strShape != D_SECT_SHAPE_SRC_EPC)); // PMS.5568 180313 일본내진 m-phi EPC 단면 추가
		SetStyleRange(CGXRange().SetCells(nRow, D_COL_C), CGXStyle().SetEnabled((data.bStl && bStlSect) && data.bCandidate && data.strShape == D_SECT_SHAPE_REG_P && m_bCodeExist[5]));
		
		//if(!data.bCandidate)
		//{
		//  SetStyleRange(CGXRange().SetCells(nRow, D_COL_POS), CGXStyle().SetValue(_LS(IDS_CMD_MPHI_SELECT_I)));
		//}
	}
	else // z축
	{
		BOOL bSameAlphaZDir, bSameBetaZDir;
		bSameAlphaZDir = bSameBetaZDir = FALSE;
		for (int i = 0; i < 6; i++)
		{
			if(m_bCodeExist[i])
			{
				if(CDbToolMphi::IsSameAlphaZDir(data.strShape, i)) bSameAlphaZDir = TRUE;
				if(CDbToolMphi::IsSameBetaZDir(data.strShape, i)) bSameBetaZDir = TRUE;
			}
		}
		...
		if(bCol)
		{
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_D), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && !CDbToolMphi::IsSameDZDir(data.strShape)));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_ALPHA), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && !bSameAlphaZDir));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_BETA), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && !bSameBetaZDir));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_015H), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && m_bCodeExist[4]));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_NS), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate  && m_bCodeExist[4] && !CDbToolMphi::IsSameNsZDir(data.strShape)));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_D2), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate  && m_bCodeExist[4] && !CDbToolMphi::IsSameD2ZDir(data.strShape)));
		}
		if(bBeam)
		{
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_D), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_ALPHA), CGXStyle().SetEnabled(FALSE));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_BETA), CGXStyle().SetEnabled(FALSE));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_015H), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && m_bCodeExist[4]));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_NS), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && m_bCodeExist[4]));
			SetStyleRange(CGXRange().SetCells(nRow, D_COL_D2), CGXStyle().SetEnabled(data.bRC && bRCSect && data.bCandidate && m_bCodeExist[4]));      
		}

		BOOL bCode5Only = FALSE;
		if(m_bCodeExist[5])
		{
			bCode5Only = TRUE;
			for (int i = 0; i < 6; i++)
		...
		// Steel,SRC
        if(data.strShape == D_SECT_SHAPE_REG_P && bCode5Only)
        {
		    SetStyleRange(CGXRange().SetCells(nRow, D_COL_L), CGXStyle().SetEnabled(FALSE));
		    SetStyleRange(CGXRange().SetCells(nRow, D_COL_A), CGXStyle().SetEnabled(FALSE));
		    SetStyleRange(CGXRange().SetCells(nRow, D_COL_RF), CGXStyle().SetEnabled(FALSE));
		    SetStyleRange(CGXRange().SetCells(nRow, D_COL_RR), CGXStyle().SetEnabled(FALSE));
        }
        else
        {
            SetStyleRange(CGXRange().SetCells(nRow, D_COL_L), CGXStyle().SetEnabled(((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate && m_bCodeExist[4]));
            SetStyleRange(CGXRange().SetCells(nRow, D_COL_A), CGXStyle().SetEnabled(((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate && 
                data.strShape != D_SECT_SHAPE_REG_P && data.strShape != D_SECT_SHAPE_REG_PSTF && data.strShape != D_SECT_SHAPE_SRC_PSTF && data.strShape != D_SECT_SHAPE_SRC_EPC)); // PMS.5568 180313 일본내진 m-phi EPC 단면 추가
            SetStyleRange(CGXRange().SetCells(nRow, D_COL_RF), CGXStyle().SetEnabled(((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate &&
                data.strShape != D_SECT_SHAPE_REG_P && data.strShape != D_SECT_SHAPE_REG_PSTF && data.strShape != D_SECT_SHAPE_SRC_PSTF && data.strShape != D_SECT_SHAPE_SRC_EPC)); // PMS.5568 180313 일본내진 m-phi EPC 단면 추가
            SetStyleRange(CGXRange().SetCells(nRow, D_COL_RR), CGXStyle().SetEnabled(((data.bStl && bStlSect) || (data.bSRC && bSrcSect)) && data.bCandidate &&
                data.strShape != D_SECT_SHAPE_REG_P && data.strShape != D_SECT_SHAPE_REG_PSTF && data.strShape != D_SECT_SHAPE_SRC_PSTF && data.strShape != D_SECT_SHAPE_SRC_EPC)); // PMS.5568 180313 일본내진 m-phi EPC 단면 추가
        }
	...
	CGXStyle style;	
	for (int nCol = D_COL_D; nCol <= D_COL_C; nCol++)
	{ 
	...
	{
		CGXStyle style;	
		GetStyleRowCol(nRow, D_COL_RF, style);
		if(!style.GetIncludeEnabled() || (style.GetIncludeEnabled() && style.GetEnabled()))
		{
			m_setRedFont.insert(std::pair<ROWCOL, ROWCOL>(nRow, D_COL_RF));
		}

		GetStyleRowCol(nRow, D_COL_RR, style);
		if(!style.GetIncludeEnabled() || (style.GetIncludeEnabled() && style.GetEnabled()))
		{
			m_setRedFont.insert(std::pair<ROWCOL, ROWCOL>(nRow, D_COL_RR));
		}
	}
	/*
...
BOOL CMPhiSectGrid::OnStartEditing(ROWCOL nRow, ROWCOL nCol)
{
	return CTBBrowserWnd::OnStartEditing(nRow, nCol);
}

void CMPhiSectGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
	CTBBrowserWnd::OnModifyCell(nRow, nCol);
}

BOOL CMPhiSectGrid::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
	return CTBBrowserWnd::OnEndEditing(nRow, nCol);
}

BOOL CMPhiSectGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CMPhiSectGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CMPhiSectGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CMPhiSectGrid::CopyRange(const CGXRangeList& selList)
{
...
BOOL CMPhiSectGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CMPhiSectGrid::IsPasteToAllRange(const CGXRange &range, CString szData) // copy한 Cell이 1개 이고, 동일한 행의 cell 여러개로 붙여넣은 상황일 때 TRUE.. 이 경우엔 붙여넣을 Range의 모든 Cell에 복사한 Cell의 값을 붙여 넣어 줘야 함
{
...
void CMPhiSectGrid::SetShapeBitmap(ROWCOL nRow, T_SECT_DATA data)
{	
	UINT aRegBitmapID[] = {    
		...
		IDB_CMD_SEC_REG_10, IDB_CMD_SEC_REG_11, IDB_CMD_SEC_REG_12,
		IDB_CMD_SEC_REG_13, IDB_CMD_SEC_REG_14, IDB_CMD_SEC_REG_15,
		IDB_CMD_SEC_REG_16, IDB_CMD_SEC_REG_17, IDB_CMD_SEC_REG_18,
		/*IDB_CMD_SEC_REG_19, IDB_CMD_SEC_REG_20,*/ IDB_CMD_SEC_REG_21,
		IDB_CMD_SEC_REG_22, IDB_CMD_SEC_REG_23
	};

	UINT aPscBitmapID[] = {  
		IDB_CMD_SEC_PSC_1C,	IDB_CMD_SEC_PSC_2C,	IDB_CMD_SEC_PSC_3C,
		IDB_CMD_SEC_PSC_4C,	IDB_CMD_SEC_PSC_5C,	IDB_CMD_SEC_PSC_6C,
		IDB_CMD_SEC_PSC_7C,	IDB_CMD_SEC_PSC_8C,	IDB_CMD_SEC_PSC_9C,
		IDB_CMD_SEC_PSC_10C,	IDB_CMD_SEC_PSC_11C,	IDB_CMD_SEC_PSC_12C
	};
	...
	else if(data.nStype == D_SECT_TYPE_TAPERED)
	{
		if(data.strShape == D_SECT_SHAPE_REG_B ||
			...
			data.strShape == D_SECT_SHAPE_REG_T ||
			...
			data.strShape == D_SECT_SHAPE_REG_HTRK ||
...
BOOL CMPhiSectGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
