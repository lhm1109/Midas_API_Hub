...
void CDragDropListCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_pDragImage)
	{
		...
		if (pParent)
		{
			switch(m_nGrupType)
			{
			case GRUP_TYPE_STRT:	((CCMStrtGrupDefDlg*)pParent)->UpdateListData();	break;
			case GRUP_TYPE_LOAD:	((CCMLoadGroupDlg*)pParent)->UpdateListData();	break;
			case GRUP_TYPE_BNDR:	((CCMBndrGrupDefDlg*)pParent)->UpdateListData();	break;
			case GRUP_TYPE_TNDN:	((CCMTendonDefDlg*)pParent)->UpdateListData();	break;
...
void CDragDropListCtrl::DropItem()
{
	...
	int nColumns = 1;
	CHeaderCtrl* pHeader = (CHeaderCtrl*)GetDlgItem(0);
	if (pHeader) { nColumns = pHeader->GetItemCount(); }
...
int CDragDropListCtrl::GetSelItems(CArray<int, int>& aSelItem)
{
	aSelItem.RemoveAll();
	...
	while (pos)
	{
		...
		aSelItem.Add(nSelItem);
	}

	return aSelItem.GetSize();
}
