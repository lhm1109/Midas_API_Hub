...
//-------------------------------------------
int CDrawPoint::GetClassType()
{
	return EN_DRCLASS_TYPE_POINT;
}

int CDrawPoint::GetFormatType()
{
	return EN_DRFORMAT_TYPE_DRAW;
}

void CDrawPoint::Draw(HDC DC, CDrawFunc* pDrawFunc, void* pDrawFormat)
{
	T_DRAW_FORMAT& DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);
	pDrawFunc->DrawPoint(DC, DrawFormat, m_Data.List); 
}

void CDrawPoint::Convert(BOOL bCDC, CDrawFunc* pDrawFunc, T_PAPER_FORMAT* pPaperFormat, void* pDrawFormat)
{
	if(bCDC)
	{
		// 실제크기의 Data를 HDC용 Data로 변환
		m_Data.Init();
		pDrawFunc->ConvertToArPOINT(*pPaperFormat, m_DataR.List, m_Data.List);					
	}
	else
	{
		// HDC용 Data를 실제크기의 Data로 변환
		m_DataR.Init();
		pDrawFunc->ConvertToArTPoint(*pPaperFormat, m_Data.List, m_DataR.List);
	}
}

void CDrawPoint::GetCalcBoundaryR(double Boundary[4])
{
...
void CDrawPoint::GetCalcBoundary(int Boundary[4])
{
...
void CDrawPoint::Move(BOOL bCDC, double Vector[2], T_PAPER_FORMAT* pPaperFormat)
{
	double disVectorR[2];
	int    disVector[2];
	int    i;
...
BOOL CDrawPoint::IsSelectCheck(POINT sPoint, BOOL bFill, int iTol)
{
	...
	for(i = 0 ; i < m_Data.List.GetSize(); i++ )
	{	
		if(abs(m_Data.List[i].x - sPoint.x) <= dTol && abs(m_Data.List[i].y - sPoint.y) <= dTol)
		{
			return TRUE;				
		}
	}		
...
BOOL CDrawPoint::IsSnapCheck(POINT currPoint, POINT& snapPoint, T_POINT& snapPointR)
{
	...
	if(m_SnapType == 1)
	{
		...
		{
			if(IsInSnapArea(m_Data.List[i], currPoint))
			{
				snapPoint = m_Data.List[i];
				if(bIsRData) snapPointR = m_DataR.List[i];
				return TRUE;
			}
		}
	}
...
BOOL CDrawPoint::GetCenterPoint(int nID, POINT& CenterP)
{
	if(nID >= 0 && nID < m_Data.List.GetSize())
	{
		CenterP = m_Data.List[nID];
		return TRUE;
	}
