...
//-------------------------------------------
int CDrawPolyline::GetClassType()
{
	return EN_DRCLASS_TYPE_POLYLINE;
}

int CDrawPolyline::GetFormatType()
{
	return EN_DRFORMAT_TYPE_DRAW;
}

void CDrawPolyline::Draw(HDC DC, CDrawFunc* pDrawFunc, void* pDrawFormat)
{
	T_DRAW_FORMAT DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);

	if(!DrawFormat.bHatching)
	{
		pDrawFunc->DrawPolyline(DC, DrawFormat, m_Data.List); 
	}
	else
	{
		...
		{
			if(i < nDataSize ) pDrawFunc->DrawPolyline(DC, DrawFormat, m_Data.List[i]);
			if(i < nHatchSize) pDrawFunc->DrawHatching(DC, DrawFormat, m_Hatching.List[i]);
		}
	}
}

void CDrawPolyline::Convert(BOOL bCDC, CDrawFunc* pDrawFunc, T_PAPER_FORMAT* pPaperFormat, void* pDrawFormat)
{
	if(bCDC)
	{
		// 실제크기의 Data를 HDC용 Data로 변환
		m_Data.Init();
		pDrawFunc->ConvertToArPolyline(*pPaperFormat, m_DataR.List, m_Data.List);						
		T_DRAW_FORMAT& DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);
	...
	else
	{
		...
		m_DataR.Init();
		pDrawFunc->ConvertToArPolylineR(*pPaperFormat, m_Data.List, m_DataR.List);
	}
}

void CDrawPolyline::GetCalcBoundaryR(double Boundary[4])
{
...
void CDrawPolyline::GetCalcBoundary(int Boundary[4])
{
...
void CDrawPolyline::Move(BOOL bCDC, double Vector[2], T_PAPER_FORMAT* pPaperFormat)
{
	double disVectorR[2];
	int    disVector[2];
	int    i;
...
BOOL CDrawPolyline::IsSelectCheck(POINT sPoint, BOOL bFill, int iTol)
{
	...
	{
		if(IsPointOfPolyLine(m_Data.List[i].arPoint, sPoint, FALSE, bFill, iTol)) return TRUE;
	}
	return FALSE;
}

BOOL CDrawPolyline::IsSnapCheck(POINT currPoint, POINT& snapPoint, T_POINT& snapPointR)
{
...
BOOL CDrawPolyline::GetCenterPoint(int nID, POINT& CenterP)
{
