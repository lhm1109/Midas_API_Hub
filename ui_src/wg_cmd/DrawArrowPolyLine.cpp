...
CDrawArrowPolyline::CDrawArrowPolyline()
{
	m_DataR.Init();
	m_Data.Init();
}
...
//-------------------------------------------
int CDrawArrowPolyline::GetClassType()
{
	return EN_DRCLASS_TYPE_ARROWPOLYLINE;
}

int CDrawArrowPolyline::GetFormatType()
{
	return EN_DRFORMAT_TYPE_DRAW;
}

void CDrawArrowPolyline::Draw(HDC DC, CDrawFunc* pDrawFunc, void* pDrawFormat)
{
	T_DRAW_FORMAT DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);
	pDrawFunc->DrawArrowPolyline(DC, DrawFormat, m_Data.List); 
}

void CDrawArrowPolyline::Convert(BOOL bCDC, CDrawFunc* pDrawFunc, T_PAPER_FORMAT* pPaperFormat, void* pDrawFormat)
{
	if(bCDC)
	{
...
void CDrawArrowPolyline::GetCalcBoundaryR(double Boundary[4])
{
...
void CDrawArrowPolyline::GetCalcBoundary(int Boundary[4])
{
...
void CDrawArrowPolyline::Move(BOOL bCDC, double Vector[2], T_PAPER_FORMAT* pPaperFormat)
{
	double disVectorR[2];
	int    disVector[2];
...
BOOL CDrawArrowPolyline::IsSelectCheck(POINT sPoint, BOOL bFill, int iTol)
{
	...
	for( i = 0 ; i < m_Data.List.GetSize(); i++)
	{
		T_POLYLINE* itr = &m_Data.List[i];
		if(IsPointOfPolyLine(itr->arPoint, sPoint, FALSE, bFill, iTol)) return TRUE;
	}
	return FALSE;
}

BOOL CDrawArrowPolyline::IsSnapCheck(POINT currPoint, POINT& snapPoint, T_POINT& snapPointR)
{
...
BOOL CDrawArrowPolyline::GetCenterPoint(int nID, POINT& CenterP)
{
