...
/////////////////////////////////////////////////////////////////////////////
// CHmtpGrid
CString CHmtpGrid::m_aCurDefVal[] = {_T("0.0"), _T("0.0")};
CHmtpGrid::CHmtpGrid()
{
	m_pData = 0;
...
BEGIN_MESSAGE_MAP(CHmtpGrid, CTBCommon)
	//{{AFX_MSG_MAP(CHmtpGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CHmtpGrid message handlers

void CHmtpGrid::Initialize(T_HMTP_D *pData)
{
	ASSERT(pData);
	m_pData = pData;
...
BOOL CHmtpGrid::InsertRecordByKey(const double &key, const double &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CHmtpGrid::DeleteRecordByKey(const double &key, const double &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
/////////////////////////////////////////////////////////////////////////////
// CHmtpGrid Overridable Functions
void CHmtpGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	return;
}

long CHmtpGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
	return 0;
}

BOOL CHmtpGrid::ValidateField(CString value, int nColID)
{
...
BOOL CHmtpGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CHmtpGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, double &key, double &data)
{
	/*
...
BOOL CHmtpGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CHmtpGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
...
BOOL CHmtpGrid::DeleteFromDB(ROWCOL nRow)
{
	double key;
	double data;
...
BOOL CHmtpGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CHmtpGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
	VERIFY(GetColumnNoFor(0, nCol));
...
BOOL CHmtpGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CHmtpGrid::GetNewKey()
{
	CString value(_T("0"));
	return value;
}

CString CHmtpGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CHmtpGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CHmtpGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CHmtpGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	double* pKey;
	double* pData;
...
long CHmtpGrid::GetCountData()
{
	return m_pData->aFuncData.GetSize();
}

void CHmtpGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0); // no filtering
}

void CHmtpGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	double Key;
	double Data;
...
void CHmtpGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	double Key;
	double Data;
...
/////////////////////////////////////////////////////////////////////////////
// CHmtpGrid Implementation
BOOL CHmtpGrid::GetValue(ROWCOL nRow, double &key, double &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CHmtpGrid::SetValue(ROWCOL nRow, const double &key, const double &data)
{
	//
	CRowColArray awCols;
...
BOOL CHmtpGrid::SetValue(CGXData* pData, ROWCOL nRow, const double &key, const double &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CHmtpGrid::ConvStrToData(int i, CString& value, double& data)
{
	if (i==1) data = _tstof(value);
}

void CHmtpGrid::ConvDataToStr(int i, const double& data, CString& value)
{
	if (i==1) value.Format(DATA_FMT, data);
}

void CHmtpGrid::MakeSearchKey(const double &key, const double &data, CStringArray &aKey)
{
	int nColID;
...
void CHmtpGrid::SetHeaderTitle(T_UNIT_SYSTEM &us, BOOL bInit)
{
	// Set Title
	CString csTitle, csUnit;
	int aUnit[2] = {
		CUnitCtrl::m_HMTP_UNIT.dTime, CUnitCtrl::m_HMTP_UNIT.dValue 
	};
...
BOOL CHmtpGrid::FindData(double key, int& nPos, int nStart)
{
	int i;
...
BOOL CHmtpGrid::ExistAllKey(CArray<double, double>& aKey, double& errorKey)
{
...
BOOL CHmtpGrid::AddData(double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(data);
...
BOOL CHmtpGrid::AddData(CArray<double, double> &aKey, CArray<double, double> &aData, BOOL bNotify)
{
...
BOOL CHmtpGrid::DelData(double key)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(0.0);
...
BOOL CHmtpGrid::DelData(CArray<double, double> &aKey, BOOL bNotify)
{
...
BOOL CHmtpGrid::ModifyData(double oldKey, double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aOldKey.Add(oldKey);
	aKey.Add(key);
...
BOOL CHmtpGrid::ModifyData(CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
	CArray<T_HMTP_BASE, T_HMTP_BASE&> aOrgData;
...
void CHmtpGrid::UpdateBuffer(int nCmd, CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
...
void CHmtpGrid::DBChanged()
{
	CHmtpItemDlg* pDlg = (CHmtpItemDlg*)GetParent();
	if (!pDlg) return;
