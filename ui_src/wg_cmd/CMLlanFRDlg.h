#if !defined(__CMLLANFRDLG_H__)
#define __CMLLANFRDLG_H__
...
// CCMLlanFRDlg dialog
#include "HeaderPre.h"
class __MY_EXT_CLASS__ CCMLlanFRDlg : public CCMDlgBase
{
// Construction
public:
	CCMLlanFRDlg(CWnd* pParent = NULL);   // standard constructor
...
// Interface
public:
	void SetParamData(T_LLANfr_D& data);
	void SetInitModify();
	BOOL ExternalSetting(T_LLANfr_K LlanKey);
...
protected:
	...
	BOOL InsertItem(T_LLANfr_K Key, T_LLANfr_D &Data);
	BOOL DeleteItem(T_LLANfr_K Key, T_LLANfr_D &Data);
	BOOL ModifyItem(T_LLANfr_K KeyOld, T_LLANfr_K Key, T_LLANfr_D &Data);
	CString DataToStr(int i, T_LLANfr_K Key, T_LLANfr_D &Data);
	...
	void Grid_InsertItem(int nPos, BOOL bFirst, T_LLANfr_BASE &item);
...
public:
// Dialog Data
	//{{AFX_DATA(CCMLlanFRDlg)
	enum { IDD = IDD_CMD_ML_LLAN_FRANCE };
	MEdit	    m_wndLaneName;
	CListCtrl	m_List;
...
#endif // !defined(__CMLLANFRDLG_H__)
