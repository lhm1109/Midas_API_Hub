// CMSpecModeGrid.cpp : implementation file
//
...
CCMSpecModeGrid::CCMSpecModeGrid()
{
	m_pData = 0;
	m_bInitDone = FALSE;
...
BEGIN_MESSAGE_MAP(CCMSpecModeGrid, CGXGridWnd)
	//{{AFX_MSG_MAP(CCMSpecModeGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CCMSpecModeGrid message handlers

////////////////////////////////외부에서 호출되는 함수////////////////////////
void CCMSpecModeGrid::InitGrid()
{
...
void CCMSpecModeGrid::SetDataSource(T_SPLC_D* pData)
{
	ASSERT(pData);
	m_pData = pData;
	m_nModeNum=m_pData->aUseMode.GetSize();
	SetStyleOnSizeCell();
	ShowData();
...
void CCMSpecModeGrid::SetStyleOnSizeCell()
{
...
#ifdef _RUS
	SetColWidth(2, 2, globalUtils.ScaleByDPI(60));
#else
	SetColWidth(2, 2, globalUtils.ScaleByDPI(35));
#endif
	SetColWidth(3, 3, globalUtils.ScaleByDPI(130));
...
*/
	...
	SetStyleRange(CGXRange(0, 3), CGXStyle()
		...
		.SetEnabled(FALSE)
		.SetInterior(::GetSysColor(COLOR_BTNFACE)) 
		.SetValue(_LS(IDS_WG_CMD__ADDD__Mode_Shape_Factor)));
...
void CCMSpecModeGrid::ShowData()
{
	...
	{
		BOOL bUseSelect=m_pData->bSelectUse;
		BOOL bUseMode=m_pData->aUseMode[i];
		double dMSFactor=m_pData->aMSFactor[i];
		
		SetStyleRange(CGXRange(i+D_START_ROW, 3), CGXStyle()
				.SetReadOnly(!bUseMode||!bUseSelect)
				.SetEnabled(bUseMode&&bUseSelect)
				.SetInterior(::GetSysColor((bUseMode&&bUseSelect)?COLOR_WINDOW:COLOR_BTNFACE)) 
				.SetValue(dMSFactor));
		SetStyleRange(CGXRange(i+D_START_ROW, 2), CGXStyle()
				.SetReadOnly(!bUseSelect)
				.SetEnabled(bUseSelect)
				.SetInterior(::GetSysColor((bUseSelect)?COLOR_WINDOW:COLOR_BTNFACE)));
		if(nRow==i+D_START_ROW&&nCol==2&&m_bOnStartEditing)  continue;
		SetValueRange(CGXRange(i+D_START_ROW,2),((short)m_pData->aUseMode[i]));
	}  
	GetParam()->SetLockReadOnly(bOldReadOnlyLock);
...
BOOL CCMSpecModeGrid::SaveData()
{
	...
	for(int i=0;i<m_nModeNum;i++)
	{
		...
		if (GetDoubleValue(csNum, dNum)) 
			m_pData->aUseMode[i] = (BOOL)dNum;
		else m_pData->aUseMode[i] = TRUE;
		csNum = GetEditingValue(i+D_START_ROW,3);
		if (GetDoubleValue(csNum, dNum)) 
			 m_pData->aMSFactor[i]=dNum;
		else m_pData->aMSFactor[i]=0.0;  
...
BOOL CCMSpecModeGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
...
BOOL CCMSpecModeGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
...
BOOL CCMSpecModeGrid::SetDownValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i,j;
	i=nRow+1;
	j=nCol;
...
BOOL CCMSpecModeGrid::SetUpValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i,j;
	i = nRow-1;
	j = nCol;
...
BOOL CCMSpecModeGrid::CopyRange(const CGXRangeList& selList)
{
...
BOOL CCMSpecModeGrid::OnPasteFromClipboard(const CGXRange &range)
{
	CGXGridWnd x;
	CGXRange r;
	CGXRange toRange;
...
BOOL CCMSpecModeGrid::VerifyPasteRange(CGXRange& from,CGXRange& to)
{
...
int CCMSpecModeGrid::GetSelectedRowsCount(int top,int bottom)
{
	int toRows;
	toRows=bottom-top+1;
...
int CCMSpecModeGrid::GetSelectedColsCount(int left,int right)
{
	int toCols;
	toCols=0;
	if(left<=2&&right>=2)
...
void CCMSpecModeGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMSpecModeGrid::OnStartEditing(ROWCOL nRow, ROWCOL nCol)
{
...
////////////////////////////////////////안 바뀌는 부분//////////////////////////////
BOOL CCMSpecModeGrid::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
...
CString CCMSpecModeGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString csNum;
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BOOL CCMSpecModeGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CCMSpecModeGrid::OnValidateCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMSpecModeGrid::OnActivateGrid(BOOL bActivate)
{
...
BOOL CCMSpecModeGrid::GetDoubleValue(CString& csNum, double& dNum)
{
	int ecode;
...
BOOL CCMSpecModeGrid::IsValidDoubleNumber(CString value, int &nErrCode)
{
	value.TrimLeft();
	value.TrimRight();
...
BOOL CCMSpecModeGrid::ConvertFormulaToValue(CString& value)
{
...
BOOL CCMSpecModeGrid::IsNumeric(CString str, int& ecode)
{
...
BOOL CCMSpecModeGrid::InvalidDoubleNumber(int nErrCode)
{
	switch(nErrCode)
	{
...
BOOL CCMSpecModeGrid::ValidateError(LPCTSTR lpszError)
{
