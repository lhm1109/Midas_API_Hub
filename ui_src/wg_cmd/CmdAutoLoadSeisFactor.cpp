//
// CmdAutoLoadSeisFactor.cpp : implementation file
#include "stdafx.h"
#include "wg_cmd.h"
...
/////////////////////////////////////////////////////////////////////////////
// CCmdAutoLoadSeisFactor dialog


CCmdAutoLoadSeisFactor::CCmdAutoLoadSeisFactor(CWnd* pParent /*=NULL*/)
	: CDialogMove(CCmdAutoLoadSeisFactor::IDD, pParent)
{
	//{{AFX_DATA_INIT(CCmdAutoLoadSeisFactor)
	//}}AFX_DATA_INIT
}

void CCmdAutoLoadSeisFactor::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCmdAutoLoadSeisFactor)
	DDX_Control(pDX, IDC_CMD_OSF_CASE_CMB, m_LoadCase);
	DDX_Control(pDX, IDC_CMD_SEIS_OSF_LIST, m_ListCtrl);
	DDX_Control(pDX, IDC_CMD_VERT_FAC_EDIT, m_VLFactor);
	DDX_Control(pDX, IDC_CMD_SPS_SDS_EDIT, m_SDS);
	DDX_Control(pDX, IDC_CMD_OVER_FAC_EDIT, m_OSFactor);
	//}}AFX_DATA_MAP
}

#define CDialog CDialogMove
BEGIN_MESSAGE_MAP(CCmdAutoLoadSeisFactor, CDialog)
#undef CDialog

	//{{AFX_MSG_MAP(CCmdAutoLoadSeisFactor)
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_CMD_SEIS_OSF_LIST, OnChangedCurrentItem)
	ON_BN_CLICKED(IDC_CMD_SEIS_LC_BTN, OnClickLoadCaseBtn)
	ON_BN_CLICKED(IDC_CMD_OSF_ADD_BTN, OnCmdGenSeisAdd)
	ON_BN_CLICKED(IDC_CMD_OSF_MODIFY_BTN, OnCmdGenSeisModify)
	ON_BN_CLICKED(IDC_CMD_OSF_DELETE_BTN, OnCmdGenSeisDelete)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CCmdAutoLoadSeisFactor message handlers

void CCmdAutoLoadSeisFactor::SetListCtrlHeader()
{
...
BOOL CCmdAutoLoadSeisFactor::GetSelectedItem(int &nIndex)
{
...
void CCmdAutoLoadSeisFactor::MakeItemEx()
{
...
CString CCmdAutoLoadSeisFactor::DataToStr(int i, T_SIFT_BASE &data)
{
	CString value;
	if (i == 0)
	{
...
void CCmdAutoLoadSeisFactor::SetItem(int nIndex, T_SIFT_BASE &data)
{
	LVITEM lvitem;
	CString str;
	for (int i = 0; i < 2; i++)
...
BOOL CCmdAutoLoadSeisFactor::ValidItem(T_SIFT_BASE& data)
{
...
BOOL CCmdAutoLoadSeisFactor::Dlg2Item(T_SIFT_BASE &data)
{
...
BOOL CCmdAutoLoadSeisFactor::Item2Dlg(int nIndex, T_SIFT_BASE &data)
{
...
void CCmdAutoLoadSeisFactor::Initial_LCaseComboBox()
{
...
BOOL CCmdAutoLoadSeisFactor::OnInitDialog()
{
...
void CCmdAutoLoadSeisFactor::OnOK()
{
	UpdateData(TRUE);
	...
	if (m_pDoc->m_pDataCtrl->AddSift(m_Data))
	{
		CDialogMove::OnOK();
	}
}

void CCmdAutoLoadSeisFactor::OnCancel()
{
	// TODO: Add your control notification handler code here
	CDialogMove::OnCancel();
}

void CCmdAutoLoadSeisFactor::OnClickLoadCaseBtn()
{
	AfxGetMainWnd()->SendMessage(WM_COMMAND, MAKEWPARAM(ID_STRUCT_STATIC_LOADCASE, 0));
}

void CCmdAutoLoadSeisFactor::OnCmdGenSeisAdd()
{
	T_SIFT_BASE data;
	data.Initialize();
	...
	{
		T_SIFT_BASE SiftBase = m_Data.aLoadPattern.GetAt(i);
		if (SiftBase.LoadCaseKey == data.LoadCaseKey && SiftBase.nLoadCaseType == data.nLoadCaseType)
		{
			CString csLoadCase;
...
void CCmdAutoLoadSeisFactor::OnCmdGenSeisModify()
{
	int nIndex;
	...
	for (int i = 0; i < m_Data.aLoadPattern.GetSize(); i++)
	{
		...
		if (SiftBase.LoadCaseKey == data.LoadCaseKey && data.nLoadCaseType == data.nLoadCaseType) // Coded by Seungjun MNet:No.2516 ('20061018)
		{
			CString csLoadCase;
			m_LoadCase.GetWindowText(csLoadCase);
			CString msg;
...
void CCmdAutoLoadSeisFactor::OnCmdGenSeisDelete()
{
	int nIndex;
	if (!GetSelectedItem(nIndex))
	{
...
void CCmdAutoLoadSeisFactor::OnChangedCurrentItem(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CCmdAutoLoadSeisFactor::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	HWND hWnd;
...
void CCmdAutoLoadSeisFactor::UpdateBuffer()
{
...
BOOL CCmdAutoLoadSeisFactor::CheckLoadCaseName(CString str, int& Index)
{
	BOOL bCheck = TRUE;
...
int CCmdAutoLoadSeisFactor::GetLoadCaseType(int iIndex)
{
	int iType;
	for (int i = 0; i < m_arLCtype.GetSize(); i++)
	{
		_CMD_SEIS_LCTP LCtype = m_arLCtype.GetAt(i);
