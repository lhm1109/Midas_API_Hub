// CMLlanDlg.cpp : implementation file
//
...
double GetInitialWheelSpacing(int nLaneType)
{
	...
	CVehlDB VehlDB(pDoc);
	if (!VehlDB.GetWheelInfoByCode(MvcdD, nNumWheel, dWheelSpace, nLaneType)) return 0.0;
	return dWheelSpace;
}
...
CCMLlanDlg::CCMLlanDlg(CWnd* pParent /*=NULL*/)
	: CCMDlgBase(CCMLlanDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CCMLlanDlg)
	m_nLoadDistType = 0;
	m_nEccentDir = 0;
	m_nMovDir = -1;
...
//	m_aCtrlData.Add(IDC_CMD_ST_ECCENDIR);
//	m_aCtrlData.Add(IDC_CMD_ELEM_NORMAL);
//	m_aCtrlData.Add(IDC_CMD_VECTOR);
//	m_aCtrlData.Add(IDC_CMD_EDT_VECTOR);
//	m_aCtrlData.Add(IDC_CMD_ST_VECTOR);
...
//Added by mylee(Scale Factor) 2006.10.21 end
	...
	m_aCtrlData.Add(IDC_CMD_ECCEN_VERT_LOAD_EDT);
...
void CCMLlanDlg::DoDataExchange(CDataExchange* pDX)
{
	CCMDlgBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMLlanDlg)
	...
	DDX_Control(pDX, IDC_CMD_ECCEN_VERT_LOAD_EDT, m_edtEccenVertLoad);
...
BEGIN_MESSAGE_MAP(CCMLlanDlg, CCMDlgBase)
	//{{AFX_MSG_MAP(CCMLlanDlg)
...
#define COLCOUNT 1
/////////////////////////////////////////////////////////////////////////////
// CCMLlanDlg implementation functions
void CCMLlanDlg::AlignControl()
{
	CRect rRef;
	CRect rToMove;
	int nDistY= 0;
	...
	aControls.Add(IDC_CMD_ECCEN_VERT_LOAD_EDT);
	...
	{
		CRect rRef;
		GetDlgItem(IDC_CMD_WIDTH_STC)->GetWindowRect(rRef);
		GetDlgItem(IDC_CMD_LANE_TYPE_GRP)->GetWindowRect(rToMove);
		nDistX = rRef.left - rToMove.left;
		CDlgUtil::CtrlMoveDistX(this, m_aCtrlLaneType, nDistX);

		GetDlgItem(IDC_CMD_WHEEL_TXT)->GetWindowRect(rRef);
		GetDlgItem(IDC_CMD_FACTOR_TITLE)->GetWindowRect(rToMove);
		int nDistY2 = rRef.bottom - rToMove.top;

		GetDlgItem(IDC_CMD_LANE_TYPE_GRP)->GetWindowRect(rRef);
		GetDlgItem(IDC_CMD_WIDTH_STC)->GetWindowRect(rToMove);
	...
	{
		GetDlgItem(IDC_CMD_WIDTH_EDT)->GetWindowRect(rToMove);
	}

	GetDlgItem(IDC_CMD_BMP_STATIC_GRP)->GetWindowRect(rRef);

	aControls.Append(m_aCtrlLaneType);
...
// 	}
	...
	if(  nCode == D_MOVE_CODE_AASHTO_STAN || nCode == D_MOVE_CODE_TAIWAN    || 
		 nCode == D_MOVE_CODE_KOREA       || /*nCode == D_MOVE_CODE_BS        ||*/
		 nCode == D_MOVE_CODE_RUSSIA      || nCode == D_MOVE_CODE_AUSTRALIA ||
		 nCode == D_MOVE_CODE_SOUTH_AFRICA || nCode == D_MOVE_CODE_BRAZIL)

	{
		...
		rectResize.bottom = rRef.bottom + globalUtils.ScaleByDPI(8);
		      ...
		      nCode == D_MOVE_CODE_PENDOT      || nCode == D_MOVE_CODE_KOREA_LRFD_2011 ||
					nCode == D_MOVE_CODE_POLAND      || nCode == D_MOVE_CODE_NEWZEALAND ||
					nCode == D_MOVE_CODE_BRAZIL)
	{
		GetDlgItem(IDC_CMD_WHEEL_EDT)->GetWindowRect(rRef);
		rectResize.bottom = rRef.bottom + globalUtils.ScaleByDPI(8);
	}
	else if(nCode == D_MOVE_CODE_EURO_BS)
	{
		...
		GetDlgItem(IDC_CMD_WHEEL_EDT)->GetWindowRect(rRef);
		GetDlgItem(IDC_CMD_ECCEN_VERT_LOAD_STC)->GetWindowRect(rToMove);
		nDistY = rRef.bottom - rToMove.top + globalUtils.ScaleByDPI(8);
        CDlgUtil::CtrlMoveDistY(this, aControls, nDistY);

		GetDlgItem(IDC_CMD_ECCEN_VERT_LOAD_EDT)->GetWindowRect(rRef);
		rectResize.bottom = rRef.bottom + globalUtils.ScaleByDPI(8);
	}
	else if(nCode == D_MOVE_CODE_AASHTO_LRFD || nCode == D_MOVE_CODE_CANADA)
	{
...
void CCMLlanDlg::SetHeaderTitle()
{
...
void CCMLlanDlg::MakeItemEx()
{
	...
	T_LLAN_D LlanD;
	CArray<T_LLAN_K, T_LLAN_K> aLlanK;
...
void CCMLlanDlg::OnUpdate(CWnd* pSender, LPARAM lHint, CObject* pHint)
{
	HWND hWnd;
...
void CCMLlanDlg::UpdateBuffer()
{
	...
	while(pos != NULL)
	{
		...
		case(UR_LLAN_ADD):
			{
				...
				pViewBuff->GetLlan(nKey, Key, Data);
				InsertItem(Key, Data);
			}
			break;
		case(UR_LLAN_DEL):
		...
		case(UR_LLAN_MFD):
			{
				pViewBuff->GetLlan(nKey, KeyBak, DataBak);
			}
			bMFD = TRUE;
			break;
		case(UR_LLAN_MFS):
...
int CCMLlanDlg::FindInsertionPos(CString& strName)
{
	T_LLAN_K key;
	T_LLAN_D data;
...
BOOL CCMLlanDlg::InsertItem(T_LLAN_K Key, T_LLAN_D &Data)
{
	LVITEM lvitem;
	CString str;
...
BOOL CCMLlanDlg::DeleteItem(T_LLAN_K Key, T_LLAN_D &Data)
{
	LVFINDINFO FindInfo;
...
BOOL CCMLlanDlg::ModifyItem(T_LLAN_K KeyOld, T_LLAN_K Key, T_LLAN_D &Data)
{
	LVFINDINFO FindInfo;
	FindInfo.flags = LVFI_PARAM;
	FindInfo.lParam = (LPARAM)Key;
...
CString CCMLlanDlg::DataToStr(int i, T_LLAN_K Key, T_LLAN_D &Data)
{
	CString str;
...
//#endif
	...
	double dLineWidth = m_Data.dLineWidth;
	if(!m_bModify)
	{
		if(nCode == D_MOVE_CODE_AUSTRALIA)
		{
			dLineWidth = pDoc->m_pUnitCtrl->ConvertTgtUnitData2CurUnit(D_UNITSYS_FORCE_INDEX_N, D_UNITSYS_LENGTH_INDEX_M, D_UNITSYS_BASE_LENGTH, 3.2);
		}
	}

	m_edtWidth.SetEditUnit(dLineWidth);
...
BOOL CCMLlanDlg::Dlg2Data()
{
	T_LLAN_BASE item; item.Initialize();
...
//#endif
	...
	if (IsUseLaneTypeRdo())
	{
		...
		{
			m_Data.dLineWidth = max(m_Data.dLineWidth, m_Data.dWheelSpace);
		}
	}
...
void CCMLlanDlg::InitItemDataVar()
{
...
//#if defined(_US) || defined(_ORG)
	...
	if(IsUseEccenVertLoad())
	{
		m_wndGrid.SetColWidthByDPI(5, 5, 80);
	}
//#endif
	if(IsUseCentrifugalForce())
	{
		m_wndGrid.SetColWidthByDPI(6, 6, 35);
	}
...
BOOL CCMLlanDlg::Grid_IsExistKey(int nKey)
{
	int nCount = m_wndGrid.GetRowCount();
	for (int i = 0; i < nCount; i++)
	{
...
void CCMLlanDlg::Grid_InsertItem(int nPos, T_LLAN_BASE &item)
{
	ROWCOL nRow;
...
void CCMLlanDlg::InsertBaseItem(int nPos, int nType)
{
	T_LLAN_BASE item;
...
/////////////////////////////////////////////////////////////////////////////
// CCMLlanDlg message handlers

BOOL CCMLlanDlg::OnInitDialog() 
{
	CCMDlgBase::OnInitDialog();

	// Auto Effect 끄자 (Auto Effect 가 왜 안먹을까...)
	m_chkBmp.InitControl(this, CCMLlanDlg::IDD, IDC_CMD_BMP_STATIC_GRP,FALSE,TRUE);
...
//  m_wndEccenEdit.SetUnitType(CUnitCtrl::m_LLAN_UNIT.Eccentricity);  //CMouseEdit로 바꿈.
	...
	//m_wndElemSelectEdit.EnableWindow(nSelect==2);
...
/*
	...
	m_edtVector.SetWindowText(_LS(IDS_WG_CMD__ADDD__0_0_0));
*/
	...
	EccentCtrlMan();
	SelectMethodMan();
	// Cross Beam , Vector option 사용 불가로....
	//CDlgUtil::CtrlEnableDisable(this,IDC_CMD_CROSSBEAM,FALSE);
	...
	GetDlgItem(IDC_CMD_ECCEN_VERT_LOAD_EDT)->EnableWindow(FALSE);
...
BOOL CCMLlanDlg::IsUseImpactSpan()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
BOOL CCMLlanDlg::IsUseSpanStart()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
BOOL CCMLlanDlg::IsUseEccenVertLoad()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
BOOL CCMLlanDlg::IsUseCentrifugalForce()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
BOOL CCMLlanDlg::IsUseLaneTypeRdo()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
void CCMLlanDlg::ShowHideByCode_LaneWidth()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
	...
	BOOL bShow = FALSE;
	if (nCode == D_MOVE_CODE_BS          ) bShow = TRUE;
	if (nCode == D_MOVE_CODE_EURO_BS     ) bShow = TRUE;
	if (nCode == D_MOVE_CODE_RUSSIA      ) bShow = TRUE;
	if (nCode == D_MOVE_CODE_AUSTRALIA   ) bShow = TRUE;
	if (nCode == D_MOVE_CODE_POLAND      ) bShow = TRUE;
	if (nCode == D_MOVE_CODE_SOUTH_AFRICA) bShow = TRUE;
	if (nCode == D_MOVE_CODE_BRAZIL      ) bShow = TRUE;
	
	//if (m_chkLaneOptim.GetCheck()) bShow = TRUE; // Allowable Width 를 사용하기로 변경
	
	//CDlgUtil::CtrlShowHide(this, aControls, bShow);
	CDlgUtil::CtrlEnableDisable(this, aControls, bShow && m_Data.nLaneType == 0);
}

void CCMLlanDlg::ShowHideByCode()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
	...
	**/
	...
	m_wndGrid.HideCols(6, D_NUM_COL, !bShowCF);
	if(bShowImpactSpan)
	{
		m_wndGrid.SetColWidthByDPI(0, 0, 25);
		m_wndGrid.SetColWidthByDPI(1, 1, 35);
...
// 			m_wndGrid.SetColWidthByDPI(4, 4, 35);
		}
		else
		{
			m_wndGrid.SetColWidthByDPI(0, 0, 30);
	...
	BOOL bShow = FALSE;
	if (nCode == D_MOVE_CODE_CHINA        || nCode == D_MOVE_CODE_PENDOT      || 
			nCode == D_MOVE_CODE_CANADA       || nCode == D_MOVE_CODE_AASHTO_LRFD ||
			nCode == D_MOVE_CODE_AASHTO_STAN  || nCode == D_MOVE_CODE_BS          ||
			nCode == D_MOVE_CODE_EURO_BS      || nCode == D_MOVE_CODE_RUSSIA      ||
			nCode == D_MOVE_CODE_KOREA        || nCode == D_MOVE_CODE_KOREA_LRFD_2011 ||
			nCode == D_MOVE_CODE_AUSTRALIA    || nCode == D_MOVE_CODE_POLAND      || 
			nCode == D_MOVE_CODE_SOUTH_AFRICA || nCode == D_MOVE_CODE_NEWZEALAND ||
			nCode == D_MOVE_CODE_BRAZIL) bShow = TRUE;
	//CDlgUtil::CtrlShowHide(this, aControls, bShow);
...
void CCMLlanDlg::SetParamData(T_LLAN_D& data)
{
	m_bModify = TRUE;
	m_Data = data;
}

void CCMLlanDlg::SetInitModify()
{
...
void CCMLlanDlg::OnCmdBtnModify() 
{
	// TODO: Add your control notification handler code here
	T_LLAN_K Key;
	...
	Key = (T_LLAN_K)m_List.GetItemData(iItem);
	if (!m_pDoc->m_pAttrCtrl->GetLlan(Key, m_Data)) 
	{
		CString msg;
...
void CCMLlanDlg::OnCmdBtnDelete() 
{
	// TODO: Add your control notification handler code here
	...
	if (!m_pDoc->m_pDataCtrl->DelLlan(csLineLaneName)) return;
...
void CCMLlanDlg::OnCmdBtnClose() 
{
	// TODO: Add your control notification handler code here
	DestroyWindow();	
}

void CCMLlanDlg::OnDblclkCmdList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
	OnCmdBtnModify();
	*pResult = 0;
...
void CCMLlanDlg::OnCmdBtnOk() 
{
	...
	if (!Dlg2Data()) return;
	if (m_bModify) CheckLlan = m_pDoc->m_pDataCtrl->ModifyLlan(m_csOldLineLaneName, m_Data);
	else CheckLlan = m_pDoc->m_pDataCtrl->AddLlan(m_Data);
...
void CCMLlanDlg::OnCmdBtnCancel() 
{
	// TODO: Add your control notification handler code here
	m_bListMode = TRUE;
	ResizeDialog();
...
void CCMLlanDlg::OnCmdBtnApply() 
{
	...
	if (!Dlg2Data()) return;
	if (m_bModify) m_pDoc->m_pDataCtrl->ModifyLlan(m_csOldLineLaneName, m_Data);
	else m_pDoc->m_pDataCtrl->AddLlan(m_Data);
}

void CCMLlanDlg::OnCmdChangeSelectBy() 
{
	// TODO: Add your control notification handler code here
	/*
	int nSelect;
...
LRESULT CCMLlanDlg::OnMViewLButtonDown(WPARAM wParam, LPARAM lParam)
{
	if (!m_bListMode)
	{
	...
	else if (wParam == m_wndElemSelectEdit.GetDlgCtrlID()) nType = 2;
	else ASSERT(0);
...
BOOL CCMLlanDlg::PreTranslateMessage(MSG* pMsg) 
{
...
void CCMLlanDlg::OnCmdLoadDistType() 
{
	UpdateData(TRUE);
...
void CCMLlanDlg::OnCmdEccentDir() 
{
/*
	UpdateData(TRUE);
	if(m_nEccentDir == 0)
...
BOOL CCMLlanDlg::GetEQDistance(CString& EQStr, double& Dx, double& Dy, double& Dz)
{
	MyParser Parser;
...
void CCMLlanDlg::SelectMethodMan()
{
...
// by TAE
BOOL CCMLlanDlg::ExternalSetting(T_LLAN_K LlanKey)
{
	BOOL bFlag = TRUE;
	bFlag = ::IsWindow(this->GetSafeHwnd());                // 윈도우가 생성되지 않았으면
	bFlag = m_pDoc->m_pAttrCtrl->ExistLlan(LlanKey);        // 키값이 존재하지 않으면
	bFlag = m_pDoc->m_pAttrCtrl->GetLlan(LlanKey, m_Data);  // Data를 가져올 수 없으면
...
LRESULT CCMLlanDlg::OnFoldButtonClicked(WPARAM wParam, LPARAM lParam)
{
	AlignControl();
	SelectMethodMan();
	ShowHideByCode();
...
void CCMLlanDlg::OnCmdChangeOptimChk() 
{
	ShowHideByCode();
	//ShowHideByCode_LaneWidth();	
}

void CCMLlanDlg::OnSelLaneTypeRdo()
{
