...
CSpfcTWBR89HUtil::CSpfcTWBR89HUtil()
{
	m_nSoil = 0;
	m_nZone = 0;
...
bool CSpfcTWBR89HUtil::CheckValid()
{
...
void CSpfcTWBR89HUtil::MakeSpectrumData(bool bOnlyCalc)
{
	double st = PRD_ST;
	...
	int result = std::swprintf(buffer, SpfcUtil::BUFFER_SIZE, _T("TaiwanBrg(89)H: Z=%s, S=%s, I=%4.2f, A=%4.2f, R=%4.2f, Damp=%4.2f"), strZone[m_nZone], strSoil[m_nSoil], m_dI, m_dAlpha, m_dRStar, m_dDamping);
	if (result >= 0)
	{
		SetDescript(buffer);
	}
	SetFuncName(L"TaiwanBrg(89)H");
}

void CSpfcTWBR89HUtil::MakePOSpectrumData(double dDamping, double SRa, double SRv, CArray<double, double>& aPeriod, CArray<double, double>& aAccel)
{
	aPeriod.RemoveAll();
	aAccel.RemoveAll();
...
void CSpfcTWBR89HUtil::SetParamFromCode(const T_SPFC_CODE& codeParam)
{
	m_nZone = codeParam.TAIWAN89H.nSeisZone;
	m_nSoil = codeParam.TAIWAN89H.nSoilType;
	m_dI = codeParam.TAIWAN89H.dIe;
	m_dAlpha = codeParam.TAIWAN89H.dAlpha;
	m_dMaxPeriod = codeParam.TAIWAN89H.dMaxPeriod;
	m_dRStar = codeParam.TAIWAN89H.dCoef;
	m_dDamping = codeParam.TAIWAN89H.dDamping;
}

void CSpfcTWBR89HUtil::SetParamFromCalc()
{
	m_dZ = TWBR89H::ZONE[m_nZone];
...
double CSpfcTWBR89HUtil::GetTaiwan86CFactor(int nSoil, double dDamping, double dT)
{
	double dC;
...
double CSpfcTWBR89HUtil::GetTaiwan86POCFactor(int nSoil, double dDamping, double dT, double dTg)
{
	double dC;
	double dCd = 1.5 / (40.0 * dDamping + 1.0) + 0.5;
	if (dT < 0.03) dCd = 1.0;
...
double CSpfcTWBR89HUtil::GetTaiwan86FuFactor(int nSoil, double dR, double dT)
{
	double dFu;
	if (dR < 0.) dR = 0.;
...
double CSpfcTWBR89HUtil::GetTaiwan86SpectralAcc(int nSoil, double dZ, double dDamping, double dI, double dR, double dAlpha, double dT)
{
...
double CSpfcTWBR89HUtil::GetTaiwan86ModifiedTg(int nSoil, double dR, double SRa, double SRv)
{
	double dRa = 1.0 + (dR - 1) / 1.5;
	double dTg;
	/*
...
double CSpfcTWBR89HUtil::GetTaiwan86POSpectralAcc(int nSoil, double dZ, double dI, double dR, double dAlpha, double dT, double dTg)
{
	//double dRa = 1.0 + (dR - 1) / 1.5;
...
void CSpfcTWBR89HUtil::SetNumRFromDoubleR()
{
