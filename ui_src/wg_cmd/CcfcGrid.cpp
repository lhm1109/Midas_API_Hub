...
/////////////////////////////////////////////////////////////////////////////
// CCcfcGrid
CString CCcfcGrid::m_aCurDefVal[] = {_T("0.0"), _T("0.0")};
CCcfcGrid::CCcfcGrid()
{
	m_pData = 0;

	AddColInfo(_LS(IDS_CMD_CCFC_time),        t_real, 0);
	AddColInfo(_LS(IDS_CMD_CCFC_coefficient), t_real, 0);

//18japan  AddColInfo(_T("Time"),        t_real, 0);
//18japan  AddColInfo(_T("Coefficient"), t_real, 0);
...
BEGIN_MESSAGE_MAP(CCcfcGrid, CTBCommon)
	//{{AFX_MSG_MAP(CCcfcGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CCcfcGrid message handlers

void CCcfcGrid::Initialize(T_CCFC_D *pData)
{
	ASSERT(pData);
	m_pData = pData;
...
BOOL CCcfcGrid::InsertRecordByKey(const double &key, const double &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CCcfcGrid::DeleteRecordByKey(const double &key, const double &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
/////////////////////////////////////////////////////////////////////////////
// CCcfcGrid Overridable Functions
void CCcfcGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	return;
}

long CCcfcGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
	return 0;
}

BOOL CCcfcGrid::ValidateField(CString value, int nColID)
{
	if (nColID >= GetColCount())
	{
		SetWarningText(_LS(IDS_WG_CMD__ADDD___CCcfcGrid__ValidateField__Error_));
		return FALSE;
	}
...
BOOL CCcfcGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CCcfcGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, double &key, double &data)
{
	/*
...
BOOL CCcfcGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CCcfcGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
...
BOOL CCcfcGrid::DeleteFromDB(ROWCOL nRow)
{
	double key;
	double data;
...
BOOL CCcfcGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CCcfcGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
	VERIFY(GetColumnNoFor(0, nCol));
...
BOOL CCcfcGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CCcfcGrid::GetNewKey()
{
	CString value(_T("0"));
	return value;
}

CString CCcfcGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CCcfcGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CCcfcGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CCcfcGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	double* pKey;
	double* pData;
...
long CCcfcGrid::GetCountData()
{
	return m_pData->aFuncData.GetSize();
}

void CCcfcGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0); // no filtering
}

void CCcfcGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	double Key;
	double Data;
...
void CCcfcGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	double Key;
	double Data;
...
/////////////////////////////////////////////////////////////////////////////
// CCcfcGrid Implementation
BOOL CCcfcGrid::GetValue(ROWCOL nRow, double &key, double &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CCcfcGrid::SetValue(ROWCOL nRow, const double &key, const double &data)
{
	//
	CRowColArray awCols;
...
BOOL CCcfcGrid::SetValue(CGXData* pData, ROWCOL nRow, const double &key, const double &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CCcfcGrid::ConvStrToData(int i, CString& value, double& data)
{
	if (i==1) data = _tstof(value);
}

void CCcfcGrid::ConvDataToStr(int i, const double& data, CString& value)
{
	if (i==1) value.Format(DATA_FMT, data);
}

void CCcfcGrid::MakeSearchKey(const double &key, const double &data, CStringArray &aKey)
{
	int nColID;
...
void CCcfcGrid::SetHeaderTitle(T_UNIT_SYSTEM &us, BOOL bInit)
{
	// Set Title
	CString csTitle, csUnit;
	int aUnit[2] = {
		CUnitCtrl::m_CCFC_UNIT.dTime, CUnitCtrl::m_CCFC_UNIT.dValue 
	};
	...
	if (bInit) SetColWidthByDPI(0, 0, 28);
}

BOOL CCcfcGrid::FindData(double key, int& nPos, int nStart)
{
	int i;
...
BOOL CCcfcGrid::ExistAllKey(CArray<double, double>& aKey, double& errorKey)
{
...
BOOL CCcfcGrid::AddData(double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(data);
...
BOOL CCcfcGrid::AddData(CArray<double, double> &aKey, CArray<double, double> &aData, BOOL bNotify)
{
...
BOOL CCcfcGrid::DelData(double key)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(0.0);
...
BOOL CCcfcGrid::DelData(CArray<double, double> &aKey, BOOL bNotify)
{
...
BOOL CCcfcGrid::ModifyData(double oldKey, double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aOldKey.Add(oldKey);
	aKey.Add(key);
...
BOOL CCcfcGrid::ModifyData(CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
	CArray<T_CCFC_BASE, T_CCFC_BASE&> aOrgData;
...
void CCcfcGrid::UpdateBuffer(int nCmd, CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
...
void CCcfcGrid::DBChanged()
{
	CCcfcItemDlg* pDlg = (CCcfcItemDlg*)GetParent();
	if (!pDlg) return;
