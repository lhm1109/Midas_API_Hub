// ETC_GDS_TAIWAN86V.cpp : implementation file
//
...
CETC_GDS_TAIWAN86V::CETC_GDS_TAIWAN86V(T_SPFC_D* pParamData, CWnd* pParent /*=NULL*/)
	: CMyChildDialog(CETC_GDS_TAIWAN86V::IDD, pParent, std::make_shared<CSpfcTAIWAN99VUtil>())
{
	//{{AFX_DATA_INIT(CETC_GDS_TAIWAN86V)
	m_nSoil = 0;
	m_nZone = 0;
	m_dMaxPeriod = PRD_ED;
	//}}AFX_DATA_INIT
	...
	m_pSpfcUtil = std::dynamic_pointer_cast<CSpfcTAIWAN99VUtil>(m_pSpfcBaseUtil);
}


void CETC_GDS_TAIWAN86V::DoDataExchange(CDataExchange* pDX)
{
	CMyChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CETC_GDS_TAIWAN86V)
...
BEGIN_MESSAGE_MAP(CETC_GDS_TAIWAN86V, CMyChildDialog)
	//{{AFX_MSG_MAP(CETC_GDS_TAIWAN86V)
		// NOTE: the ClassWizard will add message map macros here
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CETC_GDS_TAIWAN86V message handlers

BOOL CETC_GDS_TAIWAN86V::OnInitDialog() 
{
...
void CETC_GDS_TAIWAN86V::UpdateDampingWnd(CWnd* pDampingWnd)
{
	CString csDamping;
...
BOOL CETC_GDS_TAIWAN86V::ISVALID()
{  
...
void CETC_GDS_TAIWAN86V::SetPushOverData(BOOL bInit/*=TRUE*/)
{ 
	m_nZone = m_pParamData->CodeParam.TAIWAN99V.nSeisZone;
	m_nSoil = m_pParamData->CodeParam.TAIWAN99V.nSoilType;
	m_dMaxPeriod = m_pParamData->CodeParam.TAIWAN99V.dMaxPeriod;

	const double dI = m_pParamData->CodeParam.TAIWAN99V.dIe;
	const double dAlpha = m_pParamData->CodeParam.TAIWAN99V.dAlpha;
	const double dR = 1.0;
...
BOOL CETC_GDS_TAIWAN86V::Dlg2Data()
{
	m_pParamData->CodeParam.TAIWAN99V.nSeisZone      = m_nZone         ;
	m_pParamData->CodeParam.TAIWAN99V.nSoilType      = m_nSoil         ;
	m_pParamData->CodeParam.TAIWAN99V.dIe            = m_pSpfcUtil->GetI();
	m_pParamData->CodeParam.TAIWAN99V.dAlpha         = m_pSpfcUtil->GetAlpha();
	m_pParamData->CodeParam.TAIWAN99V.dMaxPeriod     = m_dMaxPeriod    ;
	m_pParamData->CodeParam.TAIWAN99V.dCoef = m_pSpfcUtil->GetR();
	m_pParamData->CodeParam.TAIWAN99V.dDamping = m_pSpfcUtil->GetDamping();

	m_pParamData->dResModCoef = m_pSpfcUtil->GetR();
	return TRUE;
}

void CETC_GDS_TAIWAN86V::Dlg2UtilData()
{
	CString strI, strR, strAlpha;
	...
	m_pSpfcUtil->SetZ(TW99V::ZONE[m_nZone]);
