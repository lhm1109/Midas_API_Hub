...
//-------------------------------------------
int CDrawPolygon::GetClassType()
{
	return EN_DRCLASS_TYPE_POLYGON;
}

int CDrawPolygon::GetFormatType()
{
	return EN_DRFORMAT_TYPE_DRAW;
}

void CDrawPolygon::Draw(HDC DC, CDrawFunc* pDrawFunc, void* pDrawFormat)
{
	T_DRAW_FORMAT DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);

	if(!DrawFormat.bHatching)
	{
		pDrawFunc->DrawPolygon(DC, DrawFormat, m_Data.List); 
	}
	else
	{
		int nDataSize  = (int)m_Data.List.GetSize();
		int nHatchSize = (int)m_Hatching.List.GetSize();
		int nSize = max(nDataSize, nHatchSize);
		
		for(int i = 0; i < nSize; i++)
		{
			if(i < nDataSize ) pDrawFunc->DrawPolygon(DC, DrawFormat, m_Data.List[i]);
			if(i < nHatchSize) pDrawFunc->DrawHatching(DC, DrawFormat, m_Hatching.List[i]);
		}
	}
}

void CDrawPolygon::Convert(BOOL bCDC, CDrawFunc* pDrawFunc, T_PAPER_FORMAT* pPaperFormat, void* pDrawFormat)
{
	if(bCDC)
	{
		// 실제크기의 Data를 HDC용 Data로 변환
		m_Data.Init();
		pDrawFunc->ConvertToArPolygon(*pPaperFormat, m_DataR.List, m_Data.List);	
		T_DRAW_FORMAT& DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);
	...
	else
	{
		...
		m_DataR.Init();
		pDrawFunc->ConvertToArPolygonR(*pPaperFormat, m_Data.List, m_DataR.List);
	}
}

void CDrawPolygon::GetCalcBoundaryR(double Boundary[4])
{
...
void CDrawPolygon::GetCalcBoundary(int Boundary[4])
{
...
void CDrawPolygon::Move(BOOL bCDC, double Vector[2], T_PAPER_FORMAT* pPaperFormat)
{
	double disVectorR[2];
	int    disVector[2];
	int    i;
	...
	if(disVector[0] != 0 || disVector[1] != 0)
	{
		...
		nSize = max(nDataSize, nHatchSize);
...
BOOL CDrawPolygon::IsSelectCheck(POINT sPoint, BOOL bFill, int iTol)
{
	...
	{
		if(IsPointOfPolyLine(m_Data.List[i].arPoint, sPoint, TRUE, bFill, iTol)) return TRUE;
	}
	return FALSE;
}

BOOL CDrawPolygon::IsSnapCheck(POINT currPoint, POINT& snapPoint, T_POINT& snapPointR)
{
...
BOOL CDrawPolygon::GetCenterPoint(int nID, POINT& CenterP)
{
