// CMPhiSectGrid_JP.cpp : implementation file
//
...
CMPhiSectGrid_JP::CMPhiSectGrid_JP(CMPhiSectDetailGrid_JP* pGrid)
{
	m_pDetailGrid = pGrid;
	for (int i = 0; i < 6; i++)
	{
...
CMPhiSectGrid_JP::~CMPhiSectGrid_JP()
{
}

///////////////////////////////////////////////////////////////////////////////
// User Defined Function
void CMPhiSectGrid_JP::SetData(CArray<T_SECT_DATA_JP, T_SECT_DATA_JP&> &aData)
{
	...
	for(int i=0; i<nSize; ++i)
	{
		...
		CString strMatl;
		if (aData[i].bRC)  strMatl = _T("RC");
		if (aData[i].bStl)  strMatl = _T("S");
		if (aData[i].bSRC)  strMatl = _T("SRC");

		//int nDataIdx = i/2;
		aValue[D_COL_ID - 1].Format(_T("%d"), aData[i].SectK);
		aValue[D_COL_NAME - 1].Format(_T("%s"), aData[i].strName);
		aValue[D_COL_MATL - 1] = strMatl;
		//aValue[D_COL_POS - 1] = GetPosString(aData[i]);
		aValue[D_COL_USE - 1].Format(_T("%d"), aData[i].MpstD.bUse);

		SetDataToRow(i+1, aValue);

		SetShapeBitmap(i + 1, aData[i]);
		SetSectionDataEdit(i + 1, aData[i]);
	}

	SetRowHeightByDPI(1, nSize, 28);

	LockUpdate(bOldLock);
	if(!bOldLock) Redraw();
} 
 
int CMPhiSectGrid_JP::GetPosVal(CString strPos)
{
...
CString CMPhiSectGrid_JP::GetPosString(const T_SECT_DATA_JP& data)
{
	CString strReturn;
	if(!data.bCandidate)
	{
...
BOOL CMPhiSectGrid_JP::GetData(CArray<T_SECT_DATA_JP, T_SECT_DATA_JP&> &aData)
{
	...
	for(int i=0; i<nCount - 1; ++i)
	{
		...
		aData[i].SectK = _ttoi(aValue[D_COL_ID - 1]);
		//aData[i].MpstD.nPosition = GetPosVal(aValue[D_COL_POS - 1]);
		aData[i].MpstD.bUse = _ttoi(aValue[D_COL_USE - 1]);
	}
	return TRUE;
}

void CMPhiSectGrid_JP::SetDataToRow(ROWCOL nRow, CStringArray& raValue)
{
...
BOOL CMPhiSectGrid_JP::GetRowToData(ROWCOL nRow, CStringArray& raValue)
{
	int nColCount = GetColCount();
	ASSERT(nColCount == raValue.GetSize());
...
CString CMPhiSectGrid_JP::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString strNum(_T(""));
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CMPhiSectGrid_JP, CTBBrowserWnd)
	//{{AFX_MSG_MAP(CMPhiSectGrid_JP)
	ON_WM_LBUTTONDOWN()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CMPhiSectGrid_JP message handlers

void CMPhiSectGrid_JP::Initialize(BOOL bCodeExist[6])
{
	...
	ChangeColHeaderStyle(CGXStyle()
		...
		// 컬럼 스타일 변경
	SetStyleRange(CGXRange().SetCols(D_COL_ID, D_COL_MATL), CGXStyle()
			.SetControl(GX_IDS_CTRL_STATIC)
			.SetEnabled(FALSE));

	SetStyleRange(CGXRange().SetCols(D_COL_SHAPE, D_COL_MATL), CGXStyle()
	...
	aWidth[D_COL_ID		- 1] = 20;
	aWidth[D_COL_NAME	- 1] = 123;
	aWidth[D_COL_SHAPE	- 1] = 42;
	aWidth[D_COL_MATL	- 1] = 40;
	...
	SetValueRange(CGXRange(0, D_COL_MATL), _LS(IDS_CMD_MPHI_MATL));
...
void CMPhiSectGrid_JP::SetSectionDataEdit(ROWCOL nRow, T_SECT_DATA_JP& data)
{
...
BOOL CMPhiSectGrid_JP::OnStartEditing(ROWCOL nRow, ROWCOL nCol)
{
	if (nCol == D_COL_USE)
	{
		CString strReverse = GetEditingValue(nRow, nCol) == _T("1") ? _T("0") : _T("1"); // 반대로 설정
		auto iter = std::find(m_SelectedRow.begin(), m_SelectedRow.end(), nRow);
		if (iter != m_SelectedRow.end())
		{
			for (int i = 0; i < m_SelectedRow.size(); i++)
			{
				SetValueRange(CGXRange(m_SelectedRow[i], nCol), strReverse);
				SelectRange(CGXRange().SetRows(m_SelectedRow[i]), TRUE);
			}
		}
		else
		{
			SetValueRange(CGXRange(nRow, nCol), strReverse);
		}
	}

	//CTBBrowserWnd::OnStartEditing(nRow, nCol);
	return FALSE;
}

void CMPhiSectGrid_JP::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
	CTBBrowserWnd::OnModifyCell(nRow, nCol);
}

BOOL CMPhiSectGrid_JP::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
	return CTBBrowserWnd::OnEndEditing(nRow, nCol);
}

BOOL CMPhiSectGrid_JP::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	return CTBBrowserWnd::ProcessKeys(pSender, nMessage, nChar, nRepCnt, flags);
}

BOOL CMPhiSectGrid_JP::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CMPhiSectGrid_JP::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CMPhiSectGrid_JP::CopyRange(const CGXRangeList& selList)
{
...
BOOL CMPhiSectGrid_JP::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CMPhiSectGrid_JP::IsPasteToAllRange(const CGXRange &range, CString szData) // copy한 Cell이 1개 이고, 동일한 행의 cell 여러개로 붙여넣은 상황일 때 TRUE.. 이 경우엔 붙여넣을 Range의 모든 Cell에 복사한 Cell의 값을 붙여 넣어 줘야 함
{
...
void CMPhiSectGrid_JP::SetShapeBitmap(ROWCOL nRow, T_SECT_DATA_JP& data)
{	
	UINT aRegBitmapID[] = {    
	...
	else if(data.nStype == D_SECT_TYPE_TAPERED)
	{
		...
		if (data.strShape == D_SECT_SHAPE_REG_GEN)
		{
			nBitmapIndex = IDB_CMD_SEC_REG_24;
		}

	}
	else if (
		data.nStype == D_SECT_TYPE_USER
		)
	{
		if (data.strShape == D_SECT_SHAPE_REG_GEN)
		{
...
BOOL CMPhiSectGrid_JP::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
...
BOOL CMPhiSectGrid_JP::SetCurrentCell(ROWCOL nRow, ROWCOL nCol, UINT flags)
{
	if (m_pDetailGrid->m_SectCurrentRow != nRow)
	{
		m_pDetailGrid->setInitRow();
		m_pDetailGrid->InitCursor(nRow);
		m_pDetailGrid->RedrawWindow();
	}
	return CTBBrowserWnd::SetCurrentCell(nRow, nCol, flags);
}

void CMPhiSectGrid_JP::OnLButtonDown(UINT nFlags, CPoint point)
{
	// 셀 좌표 가져오기
	ROWCOL nRow, nCol;
	...
	if (ht != GX_NOHIT && ht != GX_VERTLINE)
	{
		...
		if (nFlags & MK_SHIFT)
		{
			// Shift키가 눌린 상태이면 현재 셀을 기준으로 범위 선택
			if (m_pDetailGrid->m_SectCurrentRow != nRow)
			{
				auto nStart = m_pDetailGrid->m_SectCurrentRow > nRow ? nRow : m_pDetailGrid->m_SectCurrentRow;
				auto nEnd = m_pDetailGrid->m_SectCurrentRow > nRow ? m_pDetailGrid->m_SectCurrentRow : nRow;
				for (int row = nStart; row <= nEnd; row++)
				{
					SelectRange(CGXRange().SetRows(row), TRUE); // 선택영역
					auto iter2 = std::find(m_SelectedRow.begin(), m_SelectedRow.end(), row);
					if(iter2 == m_SelectedRow.end()) m_SelectedRow.push_back(row);
				}
			}
		}
		else if (nFlags & MK_CONTROL)
		{
			if (iter == m_SelectedRow.end())
			{
				SelectRange(CGXRange().SetRows(nRow), TRUE); // 선택영역
				m_SelectedRow.push_back(nRow);
			}
			else
			{
				SelectRange(CGXRange().SetRows(nRow), FALSE); // 선택영역
				m_SelectedRow.erase(iter);
				return;
			}
		}
		else
		{
			if (iter == m_SelectedRow.end() || nCol != D_COL_USE)// 리셋
			{
				m_SelectedRow.clear();
		...
		// 현재 셀로 설정
		nCol = nCol <= D_COL_MATL ? D_COL_USE : nCol;
