...
CCMThrdStepBeamDlg::CCMThrdStepBeamDlg(CWnd* pParent /*=NULL*/)
	: CCMChildBarBase(CCMThrdStepBeamDlg::IDD)
{
	//{{AFX_DATA_INIT(CCMThrdStepBeamDlg)
	m_strName = _T("");
	m_nResultType = 0;
	//}}AFX_DATA_INIT
}
...
void CCMThrdStepBeamDlg::DoDataExchange(CDataExchange* pDX)
{
	CCMChildBarBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMThrdStepBeamDlg)
...
BEGIN_MESSAGE_MAP(CCMThrdStepBeamDlg, CCMChildBarBase)
	//{{AFX_MSG_MAP(CCMThrdStepBeamDlg)
	...
	ON_BN_CLICKED(IDC_CMD_BAR_THTYPE_STRESS, OnChangeType)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CCMThrdStepBeamDlg message handlers

void CCMThrdStepBeamDlg::OnCmdCancel()
{
	DataInit();
	((CCMThMainDlg*)(GetParent()->GetParent()))->SetThrdMain(1, 3);
}

void CCMThrdStepBeamDlg::OnCmdOK()
{
	if (OnExecute())
	{
		DataInit();
...
void CCMThrdStepBeamDlg::OnCmdApply()
{
	if (OnExecute())
	{
		DataInit();
...
void CCMThrdStepBeamDlg::DataInit()
{
...
void CCMThrdStepBeamDlg::Data2Dlg()
{
...
BOOL CCMThrdStepBeamDlg::Dlg2Data()
{
	UpdateData();

	m_pDlgData.nGraphStep = 1;
	m_pDlgData.nStepFunction = 4;
...
BOOL CCMThrdStepBeamDlg::OnExecute()
{
	if (!Dlg2Data())
		return FALSE;
...
BOOL CCMThrdStepBeamDlg::OnInitDialog() 
{
...
void CCMThrdStepBeamDlg::MakeCombo()
{
...
void CCMThrdStepBeamDlg::MakeComponentCombo(int nType)
{
...
void CCMThrdStepBeamDlg::OnChangeType()
{
	UpdateData();
	MakeComponentCombo(m_nResultType+1);
}

BOOL CCMThrdStepBeamDlg::ExternalInit(UINT key)
{
