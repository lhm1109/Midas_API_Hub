// CMSlanOpDlg.cpp : implementation file
//
...
CCMSlanOpDlg::CCMSlanOpDlg(CWnd* pParent /*=NULL*/)
	: CCMDlgBase(CCMSlanOpDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CCMSlanOpDlg)
	m_bStadd = FALSE;
	m_nOffsetType  = 0;
	m_nMovDir = -1;
...
void CCMSlanOpDlg::DoDataExchange(CDataExchange* pDX)
{
	CCMDlgBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMSlanOpDlg)
...
BEGIN_MESSAGE_MAP(CCMSlanOpDlg, CCMDlgBase)
	//{{AFX_MSG_MAP(CCMSlanOpDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CCMSlanOpDlg message handlers
#define COLCOUNT 1

void CCMSlanOpDlg::AlignControl()
{
	CRect rRef;
	CRect rToMove;
	int nDistY=0;
...
// 	}
	...
	{
		GetDlgItem(IDC_CMD_SPAN_LENGTH_EDT)->GetWindowRect(rRef);
		GetDlgItem(IDC_CMD_STATIC_G)->GetWindowRect(rToMove);
		nDistY = rRef.bottom - rToMove.top + globalUtils.ScaleByDPI(7);
		CDlgUtil::CtrlMoveDistY(this, aControls, nDistY);
	...
	else if(nCode == D_MOVE_CODE_RUSSIA)
	{
		GetDlgItem(IDC_CMD_STATIC_G2)->GetWindowRect(rRef);
	}
	else
...
void CCMSlanOpDlg::ResizeDialog()
{
	UINT nLastCtrlID;
...
void CCMSlanOpDlg::SetHeaderTitle()
{
...
void CCMSlanOpDlg::SetGridTitle()
{
	CString csOffsetUnit, csSpanUnit;
	m_pDoc->m_pUnitCtrl->GetUnitSystem(CUnitCtrl::m_SLANop_UNIT.Offset, csOffsetUnit);
	m_pDoc->m_pUnitCtrl->GetUnitSystem(CUnitCtrl::m_SLANop_UNIT.dSpanLength, csSpanUnit);
																			 
	BOOL bOldLock = m_wndGrid.LockUpdate(TRUE);

	m_wndGrid.SetValueRange(CGXRange(0, 0), _LS(IDS_CMD_SLAN_no));
	m_wndGrid.SetValueRange(CGXRange(0, 1), _LS(IDS_CMD_SLAN_node));
	m_wndGrid.SetValueRange(CGXRange(0, 2), CString(_LS(IDS_CMD_SLAN_unit1))+csOffsetUnit+_T(")"));
	m_wndGrid.SetValueRange(CGXRange(0, 3), CString(_LS(IDS_CMD_SLAN_unit2))+csSpanUnit+_T(")"));
	m_wndGrid.SetValueRange(CGXRange(0, 4), _LS(IDS_CMD_SLAN_impact_factor));
	m_wndGrid.SetValueRange(CGXRange(0, 5), _LS(IDS_CMD_SLAN_SPAN_START));
	m_wndGrid.SetValueRange(CGXRange(0, 6), _LS(IDS_WG_CMD__ADDD__Centrifugal_Force));

	m_wndGrid.LockUpdate(bOldLock);
	if (!bOldLock) m_wndGrid.Redraw();
}

void CCMSlanOpDlg::MakeItemEx()
{
	CWaitCursor Cursor;
	...
	T_SLANop_D SlanD;
	CArray<T_SLANop_K, T_SLANop_K> aSlanK;
...
void CCMSlanOpDlg::InitItemDataVar()
{
...
BOOL CCMSlanOpDlg::InsertItem(T_SLANop_K Key, T_SLANop_D &Data)
{
	LVITEM lvitem;
	CString str;
...
CString CCMSlanOpDlg::DataToStr(int i, T_SLANop_K Key, T_SLANop_D &Data)
{
	CString str;
...
void CCMSlanOpDlg::OnUpdate(CWnd* pSender, LPARAM lHint, CObject* pHint)
{
	HWND hWnd;
...
int CCMSlanOpDlg::FindInsertionPos(CString& strName)
{
	T_SLANop_K key;
	T_SLANop_D data;
	...
	for (i = 0; i < nCount; i++)
	{
		...
		m_pDoc->m_pAttrCtrl->GetSlanop(key, data);
...
void CCMSlanOpDlg::UpdateBuffer()
{
	...
	T_SLANop_K Key, KeyBak;
	T_SLANop_D Data, DataBak;
	...
	while(pos != NULL)
	{
	  ...
	  switch(nCmd)
	  {
		case(UR_SLANop_ADD):
			{
				...
				pViewBuff->GetSlanop(nKey, Key, Data);
				InsertItem(Key, Data);
			}
			break;
		case(UR_SLANop_DEL):
		...
		case(UR_SLANop_MFD):
			{
				pViewBuff->GetSlanop(nKey, KeyBak, DataBak);
			}
			bMFD = TRUE;
			break;
		case(UR_SLANop_MFS):
...
void CCMSlanOpDlg::UpdateUnit() 
{
	...
	double dblSpan = 0.0;
	int nUnitTypeOffset = CUnitCtrl::m_SLANop_UNIT.Offset;  
	int nUnitTypeSpan = CUnitCtrl::m_SLANop_UNIT.dSpanLength;  
...
BOOL CCMSlanOpDlg::DeleteItem(T_SLANop_K Key, T_SLANop_D &Data)
{
	LVFINDINFO FindInfo;
...
BOOL CCMSlanOpDlg::Dlg2Data()
{
	UpdateData(TRUE);
	T_SLANop_BASE item; item.Initialize();
	m_wndLaneName.GetWindowText(m_Data.LaneName);

	m_Data.dOptimWidth = m_edtOptimWidth.GetEditValue();
	CFormulaEdit::GetEditValue(&m_wndLWidthEdit,m_Data.dLaneWidth);
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		item.dCentF = _tstof(m_wndGrid.GetValueRowCol(i+1, 6));
...
void CCMSlanOpDlg::Data2Dlg()
{
	...
	CString csVal;
	csVal.Format(_T("%g"), m_Data.dLaneWidth);
...
BOOL CCMSlanOpDlg::Grid_IsExistKey(int nKey)
{
	int nCount = m_wndGrid.GetRowCount();
	for (int i = 0; i < nCount; i++)
	{
...
void CCMSlanOpDlg::Grid_InsertItem(int nPos, T_SLANop_BASE &item)
{
	ROWCOL nRow;
...
void CCMSlanOpDlg::InsertBaseItem(int nPos, int nType)
{
	T_SLANop_BASE item;

	CFormulaEdit::GetEditValue(&m_wndOffsetEdit,item.Offset);
	CFormulaEdit::GetEditValue(&m_wndFactor,item.fact);
	CFormulaEdit::GetEditValue(&m_edtSpanLength,item.dSpanLength);
...
BOOL CCMSlanOpDlg::OnInitDialog() 
{
	CCMDlgBase::OnInitDialog();

	// Auto Effect 끄자 (Auto Effect 가 왜 안먹을까...)
	m_chkBmp.InitControl(this, CCMSlanOpDlg::IDD, IDC_CMD_BMP_STATIC_GRP,FALSE,TRUE);
	...
	m_wndOffsetUnit.SetUnitType(CUnitCtrl::m_SLANop_UNIT.Offset);
	m_edtSpanLength.SetUnitType(CUnitCtrl::m_SLANop_UNIT.dSpanLength);
	m_untSpanLength.SetUnitType(CUnitCtrl::m_SLANop_UNIT.dSpanLength);
	m_wndLWidthUnit.SetUnitType(CUnitCtrl::m_SLANop_UNIT.dLaneWidth );  
	m_edtSkewStart .SetUnitType(CUnitCtrl::m_SLANop_UNIT.dSkewStart);
	m_edtSkewEnd   .SetUnitType(CUnitCtrl::m_SLANop_UNIT.dSkewEnd  );
	// this  
	...
	m_csTitle1 = _LS(IDS_CMD_SLANOP_TITLE1);
	m_csTitle2 = _LS(IDS_CMD_SLANOP_TITLE2);
...
BOOL CCMSlanOpDlg::IsRelevantCode(int nCode)
{
...
BOOL CCMSlanOpDlg::IsUseImpactSpan()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
BOOL CCMSlanOpDlg::IsUseSpanStart()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
BOOL CCMSlanOpDlg::IsUseSpanLength()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
BOOL CCMSlanOpDlg::IsUseCentrifugalForce()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
...
void CCMSlanOpDlg::ShowHideByCode()
{
	T_MVCD_D MvcdData;
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	if (!pDoc->m_pAttrCtrl->GetMvcd(MvcdData)) MvcdData.Initialize();
	...
	m_wndGrid.HideCols(6, 6, !bShowCF);
	...
	aControls.RemoveAll();
	aControls.Add(IDC_CMD_STRADD_CHK);
	CDlgUtil::CtrlShowHide(this, aControls, (nCode == D_MOVE_CODE_EURO_BS || nCode == D_MOVE_CODE_BS  || nCode == D_MOVE_CODE_AUSTRALIA));
...
void CCMSlanOpDlg::SetParamData(T_SLANop_D& data)
{
	m_bModify = TRUE;
	m_Data = data;
}

void CCMSlanOpDlg::SetInitModify()
{
...
void CCMSlanOpDlg::OnCmdBtnAdd() 
{
	...
	{
		m_Data.dOptimWidth = M_InitValueCurUnit(10.0, LBF, FT, CUnitCtrl::m_SLANop_UNIT.dOptimWidth);
		m_Data.dLaneWidth = M_InitValueCurUnit(10.0, LBF, FT, CUnitCtrl::m_SLANop_UNIT.dLaneWidth);
	}
	...
	{
		m_Data.dOptimWidth = M_InitValueCurUnit(3.0, KN, M, CUnitCtrl::m_SLANop_UNIT.dOptimWidth);
		m_Data.dLaneWidth = M_InitValueCurUnit(3.0, KN, M, CUnitCtrl::m_SLANop_UNIT.dLaneWidth);
	}
...
void CCMSlanOpDlg::OnAnalOffsetType()
{
	UpdateData(TRUE);
...
void CCMSlanOpDlg::OnCmdBtnClose() 
{
	// TODO: Add your control notification handler code here
	DestroyWindow();
}

void CCMSlanOpDlg::OnDblclkCmdList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
	OnCmdBtnModify();	
	*pResult = 0;
...
void CCMSlanOpDlg::OnCmdBtnCancel() 
{
	// TODO: Add your control notification handler code here
	m_bListMode = TRUE;
	ResizeDialog();
...
void CCMSlanOpDlg::OnCmdChangeSelectBy() 
{
	UpdateData(TRUE);
	SelectMethodMan();
...
void CCMSlanOpDlg::OnCmdBtnModify() 
{
	// TODO: Add your control notification handler code here
	T_SLANop_K Key;
	...
	Key = (T_SLANop_K)m_List.GetItemData(iItem);
	if (!m_pDoc->m_pAttrCtrl->GetSlanop(Key, m_Data)) 
	{
		CString msg;
...
void CCMSlanOpDlg::OnCmdBtnCopy() 
{
	// TODO: Add your control notification handler code here
	int iItem = m_List.GetNextItem( -1, LVNI_ALL | LVNI_SELECTED);
	if (iItem == -1) 
	...
	T_SLANop_D LaneD, SourceLaneD;
	if(!CDBDoc::GetDocPoint()->m_pAttrCtrl->GetSlanop(csSLaneName,SourceLaneD))
	{
		CString StrTe;
	...
	CDBDoc::GetDocPoint()->m_pDataCtrl->AddSlanop(LaneD);
}

void CCMSlanOpDlg::OnCmdBtnDelete() 
{
	// TODO: Add your control notification handler code here
	int iItem = m_List.GetNextItem( -1, LVNI_ALL | LVNI_SELECTED);
	if (iItem == -1) 
	...
	if (!m_pDoc->m_pDataCtrl->DelSlanop(csLaneName)) return;
...
LRESULT CCMSlanOpDlg::OnMViewLButtonDown(WPARAM wParam, LPARAM lParam)
{
	if (!m_bListMode)
	{
		int nType;
...
void CCMSlanOpDlg::OnCmdOpInsert() 
{
	// TODO: Add your control notification handler code here
	ROWCOL nRow, nCol;
...
void CCMSlanOpDlg::OnCmdOpDelete() 
{
	// TODO: Add your control notification handler code here
	ROWCOL ncRow, ncCol;
	BOOL bCurrent = m_wndGrid.GetCurrentCell(ncRow, ncCol);
...
void CCMSlanOpDlg::OnCmdBtnOk() 
{
	...
	if (!Dlg2Data()) return;
	if (m_bModify) CheckSlan = m_pDoc->m_pDataCtrl->ModifySlanop(m_csOldLaneName, m_Data);
	else CheckSlan = m_pDoc->m_pDataCtrl->AddSlanop(m_Data); 
...
void CCMSlanOpDlg::OnCmdBtnApply() 
{
	CString strTemp;
	m_wndLaneName.GetWindowText(strTemp);
	...
	if (!Dlg2Data()) return;
	if (m_bModify) m_pDoc->m_pDataCtrl->ModifySlanop(m_csOldLaneName, m_Data);
	else m_pDoc->m_pDataCtrl->AddSlanop(m_Data); 
}

BOOL CCMSlanOpDlg::PreTranslateMessage(MSG* pMsg) 
{
...
void CCMSlanOpDlg::SelectMethodMan()
{
...
// by TAE
BOOL CCMSlanOpDlg::ExternalSetting(T_SLANop_K SlanKey)
{
	BOOL bFlag = TRUE;
	bFlag = ::IsWindow(this->GetSafeHwnd());                 // 윈도우가 생성되지 않았으면
	bFlag = m_pDoc->m_pAttrCtrl->ExistSlanop(SlanKey);       // 키값이 존재하지 않으면
	bFlag = m_pDoc->m_pAttrCtrl->GetSlanop(SlanKey, m_Data); // Data를 가져올 수 없으면
...
LRESULT CCMSlanOpDlg::OnFoldButtonClicked(WPARAM wParam, LPARAM lParam)
{
	AlignControl();
	SelectMethodMan();
	ShowHideByCode();
