// CCMMvhlItemSouthAfricaLoadGrid.cpp : implementation file
//
...
CCMMvhlItemSouthAfricaLoadGrid::CCMMvhlItemSouthAfricaLoadGrid()
{
	m_pMvhl = 0;
	m_nIndex = 0;
	m_nFrom = 0;
...
void CCMMvhlItemSouthAfricaLoadGrid::InitGrid()
{
...
void CCMMvhlItemSouthAfricaLoadGrid::SetStyleOnSizeCell()
{
	int count;
	int i;
	count=GetLoadSize();
...
void CCMMvhlItemSouthAfricaLoadGrid::SetDataSource(T_MVHL_D* pMvhl,int nIndex, int nFrom)
{
	ASSERT(pMvhl);
	m_pMvhl = pMvhl;
	m_nIndex = nIndex;
...
void CCMMvhlItemSouthAfricaLoadGrid::ShowData()
{
	CString str;
	double *dSpacing;
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::SaveData()
{
	CString csNum;
	double dNum;
	double *dSpacing;
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::SetDownValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i,j;
	i=nRow+1;
	j=nCol;
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::SetUpValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i,j;
	i = nRow-1;
	j = nCol;
...
CString CCMMvhlItemSouthAfricaLoadGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString csNum;
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CCMMvhlItemSouthAfricaLoadGrid, CGXGridWnd)
	//{{AFX_MSG_MAP(CCMMvhlItemSouthAfricaLoadGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CCMMvhlItemSouthAfricaLoadGrid message handlers
/////////////////////////////////////////////////////////////////////////////

BOOL CCMMvhlItemSouthAfricaLoadGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::OnValidateCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::OnActivateGrid(BOOL bActivate)
{
...
void CCMMvhlItemSouthAfricaLoadGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::GetDoubleValue(CString& csNum, double& dNum)
{
	int ecode;
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::IsValidDoubleNumber(CString value, int &nErrCode)
{
	value.TrimLeft();
	value.TrimRight();
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::ConvertFormulaToValue(CString& value)
{
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::IsNumeric(CString str, int& ecode)
{
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::InvalidDoubleNumber(int nErrCode)
{
	switch(nErrCode)
	{
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::ValidateError(LPCTSTR lpszError)
{
	SetWarningText(lpszError);
	return FALSE;
}

BOOL CCMMvhlItemSouthAfricaLoadGrid::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
...
int  CCMMvhlItemSouthAfricaLoadGrid::GetLoadSize()
{
	 return m_pMvhl->nNumLoad3[m_nIndex];
}
void CCMMvhlItemSouthAfricaLoadGrid::SetEnable(BOOL bEnable)
{
	m_bEnable = bEnable;
	SetStyleOnSizeCell();
}
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::CopyRange(const CGXRangeList& selList)
{
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::OnPasteFromClipboard(const CGXRange &range)
{
	CGXGridWnd x;
	CGXRange r;
	CGXRange toRange;
...
BOOL CCMMvhlItemSouthAfricaLoadGrid::VerifyPasteRange(CGXRange& from,CGXRange& to)
{
...
int CCMMvhlItemSouthAfricaLoadGrid::GetSelectedRowsCount(int top,int bottom)
{
	int toRows;
	toRows=bottom-top+1;
	return toRows;
...
int CCMMvhlItemSouthAfricaLoadGrid::GetSelectedColsCount(int left,int right)
{
	int toCols;
	toCols= right -left +1;
	return toCols;
