...
//-------------------------------------------
int CDrawDonut::GetClassType()
{
	return EN_DRCLASS_TYPE_DONUT;
}

int CDrawDonut::GetFormatType()
{
	return EN_DRFORMAT_TYPE_DRAW;
}

void CDrawDonut::Draw(HDC DC, CDrawFunc* pDrawFunc, void* pDrawFormat)
{
	T_DRAW_FORMAT DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);
	pDrawFunc->DrawDonut(DC, DrawFormat, m_Data.List); 
}

void CDrawDonut::Convert(BOOL bCDC, CDrawFunc* pDrawFunc, T_PAPER_FORMAT* pPaperFormat, void* pDrawFormat)
{
	if(bCDC)
	{
		// 실제크기의 Data를 HDC용 Data로 변환
		m_Data.Init();
		pDrawFunc->ConvertToArDonut(*pPaperFormat, m_DataR.List, m_Data.List);					
	}
	else
	{
		// HDC용 Data를 실제크기의 Data로 변환
		m_DataR.Init();
		pDrawFunc->ConvertToArDonutR(*pPaperFormat, m_Data.List, m_DataR.List);
	}
}

void CDrawDonut::GetCalcBoundaryR(double Boundary[4])
{
...
void CDrawDonut::GetCalcBoundary(int Boundary[4])
{
...
void CDrawDonut::Move(BOOL bCDC, double Vector[2], T_PAPER_FORMAT* pPaperFormat)
{
	double disVectorR[2];
	int    disVector[2];
	int    i;
...
BOOL CDrawDonut::IsSelectCheck(POINT sPoint, BOOL bFill, int iTol)
{
	...
	for(i = 0 ; i < m_Data.List.GetSize() ;i++)
	{
		T_DONUT* itr = &m_Data.List[i];
		dDis = CMathFunc::mathLength(abs(itr->CenterP.x - sPoint.x), abs(itr->CenterP.y - sPoint.y));
	
		if(dDis <= itr->OutRadius + dTol && dDis >= itr->InRadius - dTol)
		{
			if(bFill) return TRUE;
			if(fabs(dDis - itr->OutRadius) <= dTol) return TRUE;
			if(fabs(dDis - itr->InRadius ) <= dTol) return TRUE;
			continue;
		}
	}
	return FALSE;
...
BOOL CDrawDonut::IsSnapCheck(POINT currPoint, POINT& snapPoint, T_POINT& snapPointR)
{
...
BOOL CDrawDonut::GetCenterPoint(int nID, POINT& CenterP)
{
