...
/////////////////////////////////////////////////////////////////////////////
// CTdmeGrid
CString CTdmeGrid::m_aCurDefVal[] = {_T("0.0"), _T("0.0"), _T("0.0"), _T("0.0")};
CTdmeGrid::CTdmeGrid()
{
	m_pData = 0;

	AddColInfo(_LS(IDS_CMD_HMTP_time),       t_real, 0);
	AddColInfo(_LS(IDS_CMD_COMPRESSION),       t_real, 0);
	AddColInfo(_LS(IDS_CMD_TENSION),       t_real, 0);
	AddColInfo(_LS(IDS_CMD_HMTP_elasticity), t_real, 0);

//18japan  AddColInfo(_T("Time"),       t_real, 0);
//18japan  AddColInfo(_T("Elasticity"), t_real, 0);
...
BEGIN_MESSAGE_MAP(CTdmeGrid, CTBCommon)
	//{{AFX_MSG_MAP(CTdmeGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CTdmeGrid message handlers

void CTdmeGrid::Initialize(T_TDME_D *pData)
{
	ASSERT(pData);
	m_pData = pData;
...
BOOL CTdmeGrid::InsertRecordByKey(const double &key, const T_TDME_BASE &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CTdmeGrid::DeleteRecordByKey(const double &key, const T_TDME_BASE &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
/////////////////////////////////////////////////////////////////////////////
// CTdmeGrid Overridable Functions
void CTdmeGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	return;
}

long CTdmeGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
	return 0;
}

BOOL CTdmeGrid::ValidateField(CString value, int nColID)
{
	if (nColID >= GetColCount())
	{
		SetWarningText(_LS(IDS_WG_CMD__ADDD___CHmtpGrid__ValidateField__Error_));
		return FALSE;
	}

	int nErrCode;
	if (nColID == 0 || nColID == 1 || nColID == 2 || nColID == 3)
	{
...
BOOL CTdmeGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CTdmeGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, double &key, T_TDME_BASE &data)
{
	/*
...
BOOL CTdmeGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CTdmeGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
	...
	for (int i = 0; i < raKey.GetSize(); i++)
	{
		...
		aKey.Add(tKey);
		aData.Add(*((T_TDME_BASE*)raData.GetAt(i)));
	}
	return AddData(aKey, aData);
}


BOOL CTdmeGrid::DeleteFromDB(ROWCOL nRow)
{
	double key;
	T_TDME_BASE data;
...
BOOL CTdmeGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CTdmeGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
	VERIFY(GetColumnNoFor(0, nCol));
...
BOOL CTdmeGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CTdmeGrid::GetNewKey()
{
	CString value(_T("0"));
	return value;
}

CString CTdmeGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CTdmeGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CTdmeGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
																	CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	double* pKey;
	T_TDME_BASE* pData;
	pKey = new double;
	pData = new T_TDME_BASE;
	aKey.Add((void*)pKey);
...
void CTdmeGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	double* pKey;
	T_TDME_BASE* pData;
	...
	for (int i = 0; i < aKey.GetSize(); i++)
	{
		...
		pData = (T_TDME_BASE*)aData.GetAt(i);
		delete pKey;
		delete pData;
	}
}

long CTdmeGrid::GetCountData()
{
	return m_pData->aFuncData.GetSize();
}

void CTdmeGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0); // no filtering
}

void CTdmeGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	double Key;
	T_TDME_BASE Data;
	...
	for(nCount=0 ; nCount < nNumData; nCount++)
	{
		...
		Data.dTensile = m_pData->aFuncData[nCount].dTensile;
...
void CTdmeGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	double Key;
	T_TDME_BASE Data;
...
/////////////////////////////////////////////////////////////////////////////
// CTdmeGrid Implementation
BOOL CTdmeGrid::GetValue(ROWCOL nRow, double &key, T_TDME_BASE &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CTdmeGrid::SetValue(ROWCOL nRow, const double &key, const T_TDME_BASE &data)
{
	//
	CRowColArray awCols;
...
BOOL CTdmeGrid::SetValue(CGXData* pData, ROWCOL nRow, const double &key, const T_TDME_BASE &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CTdmeGrid::ConvStrToData(int i, CString& value, T_TDME_BASE& data)
{
	if (i==1) data.dComp = _tstof(value);
	else if (i==2) data.dTensile = _tstof(value);
	else if (i==3) data.dElast = _tstof(value);
}

void CTdmeGrid::ConvDataToStr(int i, const T_TDME_BASE& data, CString& value)
{
	if (i==1) value.Format(DATA_FMT, data.dComp);
	else if (i==2) value.Format(DATA_FMT, data.dTensile);
	else if (i==3) value.Format(DATA_FMT, data.dElast);
}

void CTdmeGrid::MakeSearchKey(const double &key, const T_TDME_BASE &data, CStringArray &aKey)
{
	int nColID;
...
void CTdmeGrid::SetHeaderTitle(T_UNIT_SYSTEM &us, BOOL bInit)
{
	// Set Title
	CString csTitle, csUnit;
	int aUnit[4] = {
		CUnitCtrl::m_TDME_UNIT.dTime, CUnitCtrl::m_TDME_UNIT.dValue, CUnitCtrl::m_TDME_UNIT.dValue, CUnitCtrl::m_TDME_UNIT.dValue 
	};

	int aSize[] = {50, 85, 65, 65};
...
BOOL CTdmeGrid::FindData(double key, int& nPos, int nStart)
{
	int i;
...
BOOL CTdmeGrid::ExistAllKey(CArray<double, double>& aKey, double& errorKey)
{
...
BOOL CTdmeGrid::AddData(double key, T_TDME_BASE& data)
{
	CArray<double, double> aOldKey, aKey;
	T_TDME_DT aData;
	aKey.Add(key);
...
BOOL CTdmeGrid::AddData(CArray<double, double> &aKey, T_TDME_DT &aData, BOOL bNotify)
{
	...
	for (int i = 0; i < nSize; i++)
	{
		...
		item.dTensile = aData[i].dTensile;
...
BOOL CTdmeGrid::DelData(double key)
{
	CArray<double, double> aOldKey, aKey;
	T_TDME_DT aData;
	T_TDME_BASE tmp;
...
BOOL CTdmeGrid::DelData(CArray<double, double> &aKey, BOOL bNotify)
{
	CArray<double, double> aOldKey;
	T_TDME_DT aData;
	T_TDME_BASE tmp;
...
BOOL CTdmeGrid::ModifyData(double oldKey, double key, T_TDME_BASE data)
{
	CArray<double, double> aOldKey, aKey;
	T_TDME_DT aData;
	aOldKey.Add(oldKey);
	...
	if (oldKey == key)
	{
		...
		m_pData->aFuncData[nPos].dTensile = data.dTensile;
...
BOOL CTdmeGrid::ModifyData(CArray<double, double> &aOldKey, CArray<double, double> &aKey, T_TDME_DT  &aData)
{
	CArray<T_TDME_BASE, T_TDME_BASE&> aOrgData;
...
void CTdmeGrid::UpdateBuffer(int nCmd, CArray<double, double> &aOldKey, CArray<double, double> &aKey, T_TDME_DT &aData)
{
...
void CTdmeGrid::DBChanged()
{
	CTdmeItemDlg* pDlg = (CTdmeItemDlg*)GetParent();
	if (!pDlg) return;
