...
CSeisItemTaiwan2022Sub::CSeisItemTaiwan2022Sub(CWnd* pParent)
	: CDialogMove(CSeisItemTaiwan2022Sub::IDD, pParent)
{
	...
	m_aCtrlMethod_SMF.Add(IDC_CMD_TAIWAN02_FSV_EDT);
	m_aCtrlMethod_SMF.Add(IDC_CMD_TAIWAN02_FSA_EDT);
	...
	m_aCtrlMethod_SMF.Add(IDC_CMD_TAIWAN02_SMF03_STC);
	m_aCtrlMethod_SMF.Add(IDC_CMD_TAIWAN02_SMF04_STC);
...
void CSeisItemTaiwan2022Sub::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CMD_TAIWAN02_FSV_EDT, m_edtFsv);
	DDX_Control(pDX, IDC_CMD_TAIWAN02_FSA_EDT, m_edtFsa);
...
BEGIN_MESSAGE_MAP(CSeisItemTaiwan2022Sub, CDialogMove)
	...
	ON_EN_CHANGE(IDC_CMD_TAIWAN02_NV_EDT, OnEdtChangeNSFactor)
	ON_EN_CHANGE(IDC_CMD_TAIWAN02_NA_EDT, OnEdtChangeNSFactor)
	ON_EN_CHANGE(IDC_CMD_TAIWAN02_NV_EDT2, OnEdtChangeNSFactor)
	ON_EN_CHANGE(IDC_CMD_TAIWAN02_NA_EDT2, OnEdtChangeNSFactor)
END_MESSAGE_MAP()

void CSeisItemTaiwan2022Sub::AlignCtrl()
{
	...
	switch(m_nSeisZone)
	{
		case enSeisZoneGEN:	// General Zone
		{
		...
		break;
		case enSeisZoneNFZ: // Near fault zone
		{
		...
		break;
		case enSeisZoneTPB:	// Taipei Basin
		{
			// Align Taipei
...
BOOL CSeisItemTaiwan2022Sub::OnInitDialog()
{
	...
	m_edtFsv.SetUnitType(D_UNITSYS_NONE);
	m_edtFsa.SetUnitType(D_UNITSYS_NONE);

	AlignCtrl();

	CDlgUtil::CtrlShowHide(this, m_aCtrlMethod0, m_nSeisZone != enSeisZoneTPB);
	CDlgUtil::CtrlShowHide(this, m_aCtrlMethod1, m_nSeisZone == enSeisZoneNFZ);
	CDlgUtil::CtrlShowHide(this, m_aCtrlMethod_SMF, m_nSeisZone != enSeisZoneTPB);
	CDlgUtil::CtrlShowHide(this, m_aCtrlMethod2, m_nSeisZone == enSeisZoneTPB);
...
void CSeisItemTaiwan2022Sub::OnOk()
{
	if ( !Dlg2Data() ) return;
...
void CSeisItemTaiwan2022Sub::OnCancel()
{
	CDialogMove::OnCancel();
}

void CSeisItemTaiwan2022Sub::OnSelChangeSpecAccel()
{
...
void CSeisItemTaiwan2022Sub::OnEdtChangeSpecAccel()
{
	SetSiteMagnifyFactor();
}

void CSeisItemTaiwan2022Sub::OnSelChangeSoilType()
{
	CtrlManager();
	SetSiteMagnifyFactor();
}

void CSeisItemTaiwan2022Sub::OnSelChangeSubZone()
{
	CtrlManager();
	SetChangeTaipai();
}

void CSeisItemTaiwan2022Sub::OnEdtChangeNSFactor()
{
	SetSiteMagnifyFactor();
}

void CSeisItemTaiwan2022Sub::Data2Dlg()
{
	...
	m_edtNdv.SetEditUnitAuto(m_pData->dNdv);
	m_edtNda.SetEditUnitAuto(m_pData->dNda);
	m_edtNmv.SetEditUnitAuto(m_pData->dNmv);
	m_edtNma.SetEditUnitAuto(m_pData->dNma);
	m_cbxSoilType.SetCurSel(m_pData->nSoilType);

	m_edtFdv.SetEditUnitAuto(m_pData->dFdv);
	m_edtFda.SetEditUnitAuto(m_pData->dFda);
	m_edtFmv.SetEditUnitAuto(m_pData->dFmv);
	m_edtFma.SetEditUnitAuto(m_pData->dFma);
	m_edtFsv.SetEditUnitAuto(m_pData->dFsv);
	m_edtFsa.SetEditUnitAuto(m_pData->dFsa);

	m_cbxSubZone_taipei.SetCurSel(m_pData->nSubZone);
	m_edtSds_T.SetEditUnitAuto(m_pData->dSds_t);
	m_edtSms_T.SetEditUnitAuto(m_pData->dSms_t);
	m_edtTm0.SetEditUnitAuto(m_pData->dTm0);
	m_edtTd0.SetEditUnitAuto(m_pData->dTd0);

	SetSiteMagnifyFactor();
}

BOOL CSeisItemTaiwan2022Sub::Dlg2Data()
{
	...
	double dFdv, dFda, dFmv, dFma, dFsv, dFsa;
	...
	CFormulaEdit::GetEditValue(&m_edtFsv, dFsv);
	CFormulaEdit::GetEditValue(&m_edtFsa, dFsa);
	...
	if ( dNdv < 0.0 || dNda < 0.0 || dNmv < 0.0 || dNma < 0.0 )
	{
		bError = TRUE;
		AfxMessageBox(_LS(IDS_WG_CMD__ADDD__Error__TAIWAN22_NFSE));
	}
	if ( dFdv < 0.0 || dFda < 0.0 || dFmv < 0.0 || dFma < 0.0 || dFsv < 0.0 || dFsa < 0.0 )
	{
		bError = TRUE;
...
void CSeisItemTaiwan2022Sub::InitSpecAccel()
{
...
void CSeisItemTaiwan2022Sub::InitSoilType()
{
	for ( int i = 0; i < 4; i++ )
	{
...
void CSeisItemTaiwan2022Sub::InitTaipei()
{
...
void CSeisItemTaiwan2022Sub::SetSiteMagnifyFactor()
{
	int nSoilType = m_cbxSoilType.GetItemData(m_cbxSoilType.GetCurSel());
	if ( nSoilType != 3 ) // user type 아닐 경우
	{
		double dFda = 0.0, dFdv = 0.0, dFma = 0.0, dFmv = 0.0, dFna = 0.0, dFnv = 0.0;
		if ( m_nSeisZone == enSeisZoneGEN || m_nSeisZone == enSeisZoneNFZ )
		{
			double dSds, dSd1, dSms, dSm1;
			CFormulaEdit::GetEditValue(&m_cbxSds, dSds);
			CFormulaEdit::GetEditValue(&m_cbxSd1, dSd1);
			...
			if ( !pDoc->m_pAttrCtrl->CalcTaiwan22SiteMagnifyFactor(TAIWAN_E_2022, m_nSeisZone, nSoilType,
				dSds, dSd1, dSms, dSm1, dNda, dNdv, dNma, dNmv,
				dFda, dFdv, dFma, dFmv, dFna, dFnv) ) return;
			// NearFaultZone의 V* 산정시 Na=Nv=1.0 일때 Fa_1,Fv_1은 LateralLoad.cpp에서 계산
			// Fa_1,Fv_1은 출력하지 않음.

			m_edtFda.SetEditUnitAuto(dFda);
			m_edtFdv.SetEditUnitAuto(dFdv);
			m_edtFma.SetEditUnitAuto(dFma);
			m_edtFmv.SetEditUnitAuto(dFmv);
			m_edtFsa.SetEditUnitAuto(dFna);
			m_edtFsv.SetEditUnitAuto(dFnv);
		}
	}
	//User type 경우에는 Dlg2Data에서 처리
}

void CSeisItemTaiwan2022Sub::SetChangeTaipai()
{
	m_edtSds_T.SetEditUnitAuto(0.6);
	m_edtSms_T.SetEditUnitAuto(0.8);
	...
	case 1:
		{
			m_edtTd0.SetEditUnitAuto(1.3);
			m_edtTm0.SetEditUnitAuto(1.3);
		}
		break;
	case 2:
		{
			m_edtTd0.SetEditUnitAuto(1.05);
			m_edtTm0.SetEditUnitAuto(1.05);
		}
		break;
	default:
		{
			m_edtTd0.SetEditUnitAuto(1.0);
			m_edtTm0.SetEditUnitAuto(1.0);
		}
		break;
	}
}

void CSeisItemTaiwan2022Sub::CtrlManager()
{
	if ( m_nSeisZone == enSeisZoneNFZ )
	{
		m_cbxSms.EnableWindow(FALSE);
		m_cbxSm1.EnableWindow(FALSE);
	}
	...
	if ( nSoilType != 3 )
	{
		m_edtFdv.SetReadOnly(true);
		m_edtFda.SetReadOnly(true);
		m_edtFmv.SetReadOnly(true);
		m_edtFma.SetReadOnly(true);
		m_edtFsv.SetReadOnly(true);
		m_edtFsa.SetReadOnly(true);
	}
	else
	{
		m_edtFdv.SetReadOnly(false);
		m_edtFda.SetReadOnly(false);
		m_edtFmv.SetReadOnly(false);
		m_edtFma.SetReadOnly(false);
		m_edtFsv.SetReadOnly(false);
		m_edtFsa.SetReadOnly(false);
	}
...
void CSeisItemTaiwan2022Sub::ChangeCaption()
{
	auto L_ChangeCaption = [ this ] (UINT nCtrlID, const CString& csCaption)
	{
		CWnd* pWnd = GetDlgItem(nCtrlID);
		if ( pWnd == nullptr ) { ASSERT(0); return; }
		pWnd->SetWindowText(csCaption);
	};
	L_ChangeCaption(IDC_CMD_TAIWAN02_GEN_STC, _T("Horizontal Spectral Accel. (Table 2-1)"));
	L_ChangeCaption(IDC_CMD_TAIWAN02_NSF_STC, _T("Near Fault Seismic Effect (Table 2-3-1 ~ 2-3-4)"));
	L_ChangeCaption(IDC_CMD_TAIWAN02_NA_STC, _T("Short Period (Ss)"));
	L_ChangeCaption(IDC_CMD_TAIWAN02_NV_STC, _T("1sec Period (S1)"));
}

CString CSeisItemTaiwan2022Sub::GetSeismicZoneName(int nCode)
{
...
CString CSeisItemTaiwan2022Sub::GetSoilTypeName(int nCode)
{
