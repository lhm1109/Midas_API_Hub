...
CThisItemDlg::CThisItemDlg(CWnd* pParent /*=NULL*/)
	: CCMDlgBase(CThisItemDlg::IDD, pParent)
{
	m_pDoc = 0;
	m_bModify = FALSE;
	...
	m_aCtrlUpdate.Add(IDC_CMD_DAMP_UPDATE_RD1);
...
CThisItemDlg::~CThisItemDlg()
{
	if(m_pGrid) delete m_pGrid;
}

BOOL CThisItemDlg::ExternalSetting(const unsigned key)
{
	BOOL bFlag = TRUE;
	bFlag = ::IsWindow(this->GetSafeHwnd());                // �����찡 �������� �ʾ�����
	bFlag = m_pDoc->m_pAttrCtrl->ExistThis(key);        // Ű���� �������� ������
	bFlag = m_pDoc->m_pAttrCtrl->GetThis(key, m_Data);  // Data�� ������ �� ������
...
void CThisItemDlg::DoDataExchange(CDataExchange* pDX)
{
	CCMDlgBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CThisItemDlg)  
...
BEGIN_MESSAGE_MAP(CThisItemDlg, CCMDlgBase)
	//{{AFX_MSG_MAP(CThisItemDlg)
...
// Interfaces
//////////////////////////////////////////////////////////////////////////////
void CThisItemDlg::SetParamData(T_THIS_D &data) 
{ 
	m_Data = data; 
	m_csOldName = m_Data.LoadCaseName;
	m_nOldID = m_Data.LoadCaseId;
...
int CThisItemDlg::GetFinalStateIndex(int nSubLoadCaseType, UINT KeyFinalState)
{
	int nSelect = 0;
	T_THIS_D DataThis;
	T_STLD_D DataStld;
...
/*
int CThisItemDlg::GetFinalStateIndex(T_THIS_K KeyThis)
{
	int nSelect = 0;
	T_THIS_D DataThis;
	int nCount = m_wndFinalStateCmb.GetCount();
...
/////////////////////////////////////////////////////////////////////////////
// CThisItemDlg message handlers

BOOL CThisItemDlg::OnInitDialog() 
{
	...
	m_wndInitLoadGroupBox.InitControl(this, CThisItemDlg::IDD, IDC_CMD_INITIAL_CONDITION_FRM, TRUE, TRUE);
	...
	SetGridDeadLoadCmb();
	// 호출
	DialogActionReactor::DoDlgReact(DialogActionReactor::kOnInitAfter, CThisItemDlg::IDD, this);
	Data2Dlg();  
	CtrlManager();
...
void CThisItemDlg::SetGridListCtrlHeader()
{
...
void CThisItemDlg::MakeGridItemEx()
{
...
void CThisItemDlg::SetGridItem(int nIndex, T_GILC_K key)
{
	T_GILC_D GilcD;
...
void CThisItemDlg::AlignControl()
{
	...
	if (!m_bJapanMode)
	{
		...
		GetDlgItem(IDC_CMD_CUMULATE_DVA_CHK     )->GetWindowRect(rRef);
...
//  if(m_nAnalMethodRdo == 1 && m_nDampingTypeRdo == 0) m_nDampingTypeRdo = 1;
	...
	//CDlgUtil::CtrlEnableDisableByRect(this, IDC_CMD_CONTROL_OPTION_FRM, m_nStaticIncrementRdo == 1, FALSE);
...
void CThisItemDlg::SetInitLoadFoldControl(const bool isShowJP, const int initMethod)
{
...
void CThisItemDlg::OnStaticIncMethod()
{
	CtrlManager();
}

void CThisItemDlg::OnStaticCtrlOpt()
{
	CtrlManager();
}

void CThisItemDlg::OnCallAutoCalcCoef()
{
	AutoCalcCoef();
}

void CThisItemDlg::OnChangeAnalType() 
{
	UpdateData(TRUE);
	MakeItemInitialLoadCmb(m_nAnalTypeRdo, m_nAnalMethodRdo);
	CtrlManager();
...
void CThisItemDlg::OnChangeAnalMethod()
{
	// 일본버전 초기 모드 일때 
...
void CThisItemDlg::OnChangeTimeHistoryType()
{
	CtrlManager();
}

void CThisItemDlg::OnChangeGeomNonlinearType()
{
	CtrlManager();
	SetFinalStateCmb();
}

void CThisItemDlg::OnChangeSequenceLoad()
{
	CtrlManager();
}

void CThisItemDlg::OnChangeCoefDefine()
{
	CtrlManager();  
	AutoCalcCoef();  // Mass and Stiffness Coefficient Calculation
}

void CThisItemDlg::OnChangeFreqPeriod()
{
	CtrlManager();
	AutoCalcCoef();  // Mass and Stiffness Coefficient Calculation
}

void CThisItemDlg::OnMassStiffChk()
{
	CtrlManager();
	AutoCalcCoef();  // Mass and Stiffness Coefficient Calculation
}

void CThisItemDlg::OnChangeIntegrationPara()
{
	CtrlManager();
...
void CThisItemDlg::OnDeltaposDouble(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CThisItemDlg::OnDeltaposInteger(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	// TODO: Add your control notification handler code here
	const MSG* pMsg = GetCurrentMessage();
...
void CThisItemDlg::SetFinalStateCmb()
{
	UpdateData();
	...
	T_THIS_K KeyThis;
	T_THIS_D DataThis;
	m_pDoc->m_pAttrCtrl->GetThisKeyList(rThisKeyList, FALSE);
...
void CThisItemDlg::AutoCalcCoef()
{
	UpdateData(TRUE);
	T_THIS_D rData;
	rData.Initialize();
...
void CThisItemDlg::OnShowDampingBtn()
{
	UpdateData();
...
void CThisItemDlg::OnChangeIteration()
{
	CtrlManager();
}

void CThisItemDlg::OnSelchangeCmdThisDampMethodCmb() 
{
	int nIndex = m_wndDampingMethodCmb.GetCurSel();
...
void CThisItemDlg::OnSelchangeCmdFinalStateCmb()
{
	CtrlManager();
}

void CThisItemDlg::OnCmdSubsequentTypeRdo() 
{
	CtrlManager();	
}

void CThisItemDlg::OnCmdSubsequentChk() 
{
	CtrlManager();	
}

void CThisItemDlg::OnCmdIterationCtrlBtn()
{
	if(!Dlg2Data()) return;
	//if(!ValidData()) return;

	CThisIterationCtrlDlg dlg(&m_Data);
	dlg.SetInitPos(D_INIT_POS_LT);
	dlg.DoModal();
}

void CThisItemDlg::OnCmdDescBtn()
{
...
BOOL CThisItemDlg::CheckValidNllp()
{
...
BOOL CThisItemDescDlg::OnInitDialog()
{
	...
	if(nLang==2)
	{
		GetDlgItem(IDC_CMD_THIS_ITEM_DESC_STC3)->ShowWindow(SW_HIDE);
	}

	return TRUE;
}

void CThisItemDlg::OnChkUseInitial()
{
	SetInitialLoadText();
	CtrlManager();
}

void CThisItemDlg::OnCmdInitLoadMethod()
{
	CtrlManager();
}

void CThisItemDlg::SetInitialLoadText()
{
...
void CThisItemDlg::MakeItemInitialLoadCmb(int nAnalType, int nAnalMethod)
{
...
void CThisItemDlg::SetGridDeadLoadCmb()
{
	T_GILC_D data;
	...
	for (int i = 0; i < nSize; i++)
	{
		...
		{
			ASSERT(!_T("CThisItemDlg::SetSecondaryDeadLoadCmb"));
		}
	}
	if (nSize != 0)
...
void CThisItemDlg::OnChkKeepFinalStep()
{
	CtrlManager();
}

void CThisItemDlg::OnCmdBtnLdGridAdd()
{
...
void CThisItemDlg::OnCmdBtnLdGridDel()
{
...
LRESULT CThisItemDlg::OnBnClickedInitLoadFoldButton(WPARAM wParam, LPARAM lParam)
{
	if (m_wndInitLoadGroupBox.GetFoldState())
		return 0L;
