...
CIehpItemDlg::CIehpItemDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CIehpItemDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CIehpItemDlg)
	m_strDesc = _T("");
...
void CIehpItemDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CIehpItemDlg)
...
BEGIN_MESSAGE_MAP(CIehpItemDlg, CDialogMove)
	//{{AFX_MSG_MAP(CIehpItemDlg)  
...
/////////////////////////////////////////////////////////////////////////////
// CIehpItemDlg Data & Window handlers
void CIehpItemDlg::SetParamData(T_IEHP_K Key, T_IEHP_D &data)
{
	m_Key = m_OldKey = Key;
	m_bModify	= TRUE;
	m_Data = data; 
...
void CIehpItemDlg::AlignCtrl()
{
	...
	{
		CWnd* pWndRef = GetDlgItem(IDC_STATIC_WALL_TYPE);   ASSERT(pWndRef);
        ...
        auto* pRadio1 = GetDlgItem(IDC_MATL_TYPE_RC_SRC_ENCASED_RDO);
        CRect rcRadio1;
        pRadio1->GetWindowRect(rcRadio1);
        rcRadio1.right = rcRef.right - 2l;
...
void CIehpItemDlg::HideStiffnessMethodCtrl()
{
	CRect rRef, rMov;
	int nDistY;
...
void CIehpItemDlg::InitSectComboCtrl()
{
...
void CIehpItemDlg::InitSectComboCtrl_Sect()
{
...
void CIehpItemDlg::InitSectComboCtrl_Thik()
{
...
void CIehpItemDlg::HingeComboCtrl()
{
	// Fx
	int nIndex=0;
...
void CIehpItemDlg::ModelComboCtrl()
{
	for (int i = 0; i < 6; i++)
	{
	...
	BOOL bNewIehpJP = TRUE; //CTestEnvMgr::GetTestEnvST(_T("NewIehpJP")) == _T("yes");
	...
	for(int i=0; i<6; i++)
	{
		...
		if((m_nDefinitionType==MOMENT_ROTATION && (m_nInteractionType==0 || m_nInteractionType==1)) || m_nElementType==HT_TRUSS || m_nElementType==HT_SPRING)
		{
...
CString CIehpItemDlg::GetHysModelName(int nHysModel)
{
	CString strModel = _T("");
	switch(nHysModel)
	{
...
int CIehpItemDlg::GetCategory(int nHysModel)
{
	enum {
		D_IEHP_CAT_SIMP=0,
		D_IEHP_CAT_DEGR,
...
void CIehpItemDlg::Data2Dlg()
{
	...
	m_nDefinitionType=0;
	if(m_Data.nHingeType==0 || m_Data.nHingeType==1) // beam(lumped, distribution)
	{
		m_nElementType = HT_BEAM_COLM; // beam
		m_nDefinitionType = m_Data.nHingeType; // lumped, distribution
	}
	else if(m_Data.nHingeType==2) m_nElementType = HT_SPRING;   // spring
	else if(m_Data.nHingeType==3) m_nElementType = HT_TRUSS;    // truss
	else if(m_Data.nHingeType==4) m_nElementType = HT_WALL_CRB; // Wall CRB
	else ASSERT(0);
...
void CIehpItemDlg::Dlg2CommonData()
{
	UpdateData();
	...
	if     (m_nElementType==HT_BEAM_COLM) m_Data.nHingeType = m_nDefinitionType; // beam(lumped, distribution)
	else if(m_nElementType==HT_SPRING     ) m_Data.nHingeType=2; // spring
	else if(m_nElementType==HT_TRUSS      ) m_Data.nHingeType=3; // truss
	else if(m_nElementType==HT_WALL_CRB   ) m_Data.nHingeType=4; // truss
	else ASSERT(0);
...
/////////////////////////////////////////////////////////////////////////////
// CIehpItemDlg Message handlers
BOOL CIehpItemDlg::OnInitDialog() 
{
#if defined (_MEC)
	ASSERT(0); // MEC 대화상자 사용 요함.
#endif
...
void CIehpItemDlg::OnCmdElementType()
{
	CtrlManager();
}

void CIehpItemDlg::OnCmdDefinitionType()
{
	CtrlManager();
}

void CIehpItemDlg::OnCmdNewHingeType() 
{  
	CtrlManager();
}

void CIehpItemDlg::OnFlexStifType()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnInteractionType() 
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnMaterialType() 
{
	CtrlManager();
}

void CIehpItemDlg::OnCmdWallType()
{
	UpdateData(TRUE);
	CtrlManager();
}

void CIehpItemDlg::CtrlElementType() 
{
	EnableDisableCtrls();
}

void CIehpItemDlg::CtrlNewHingeType()
{
	EnableDisableCtrls();
}

void CIehpItemDlg::CtrlInteractionType() 
{  
...
void CIehpItemDlg::CtrlMaterialType() 
{  
	UpdateData(TRUE);
...
void CIehpItemDlg::CtrlDefinition() 
{
	UpdateData(TRUE);
...
void CIehpItemDlg::CtrlWallType()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::CtrlLocation()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnCmdApply() 
{
	if(!Dlg2Data()) return;
}

void CIehpItemDlg::EnableDisableCtrlDofDx() 
{
	// 체크온 된 상태로 Fix
...
void CIehpItemDlg::EnableDisableCtrlDofDy() 
{
...
void CIehpItemDlg::EnableDisableCtrlDofDz() 
{  
...
void CIehpItemDlg::EnableDisableCtrlDofRx() 
{
...
void CIehpItemDlg::EnableDisableCtrlDofRy() 
{ 
	// Fiber일 때는 Fx와 연동
	BOOL bMyFollowsFx = (m_nNewHingeType == 0 && m_nInteractionType == 2) ||  // Skeleton && P-M-M
		(m_nNewHingeType == 1); // Fiber
...
void CIehpItemDlg::EnableDisableCtrlDofRz() 
{ 
	// Fx와 My/Mz를 연동 처리 
	BOOL bMyFollowsFx = (m_nNewHingeType == 0 && m_nInteractionType == 2) ||  // Skeleton && P-M-M
		(m_nNewHingeType == 1); // Fiber
...
void CIehpItemDlg::OnCmdDofDxChk() { UpdateData(TRUE);  EnableDisableCtrlDofDx();  UpdateData(FALSE); }
void CIehpItemDlg::OnCmdDofDyChk() { UpdateData(TRUE);  EnableDisableCtrlDofDy();  UpdateData(FALSE); }
void CIehpItemDlg::OnCmdDofDzChk() { UpdateData(TRUE);  EnableDisableCtrlDofDz();  UpdateData(FALSE); }
void CIehpItemDlg::OnCmdDofRxChk() { UpdateData(TRUE);  EnableDisableCtrlDofRx();  UpdateData(FALSE); }
void CIehpItemDlg::OnCmdDofRyChk() { UpdateData(TRUE);  EnableDisableCtrlDofRy();  ChangebyCtrlDofRy(); UpdateData(FALSE); }
void CIehpItemDlg::OnCmdDofRzChk() { UpdateData(TRUE);  EnableDisableCtrlDofRz();  ChangebyCtrlDofRz(); UpdateData(FALSE); }

void CIehpItemDlg::OnCmdPropDxBtn() 
{	
...
void CIehpItemDlg::OnCmdPropDyBtn() 
{  
...
void CIehpItemDlg::OnCmdPropDzBtn() 
{	
...
void CIehpItemDlg::OnCmdPropRxBtn() 
{	
...
void CIehpItemDlg::OnCmdPropRyBtn() 
{
...
void CIehpItemDlg::OnCmdPropRzBtn() 
{
...
bool CIehpItemDlg::PropBtnProc(T_IEHP_D data, int n, int nSel, int nFg, int nTabStyle, bool bUpdateNeeded)
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnCmdPropYsBtn() 
{
	UpdateData();
	Dlg2CommonData();
...
void CIehpItemDlg::ChangeNumofSectionDx() 
{  
	UpdateData();
...
void CIehpItemDlg::OnChangeSectDxEdit() 
{  
	ChangeNumofSectionDx();
}

void CIehpItemDlg::OnSelchangeLocDxCmb() 
{
	UpdateData();
...
void CIehpItemDlg::OnSelchangeStiffLocDxCmb()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnSelchangeStiffLocDyCmb()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnSelchangeStiffLocDzCmb()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnSelchangeStiffLocRxCmb()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnSelchangeStiffLocRyCmb()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnSelchangeStiffLocRzCmb()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnSelchangeModelCmb(int nDof)
{
...
void CIehpItemDlg::OnSelchangeModelDxCmb() 
{
...
void CIehpItemDlg::OnSelchangeModelDyCmb() 
{
...
void CIehpItemDlg::OnSelchangeModelDzCmb() 
{
...
void CIehpItemDlg::OnSelchangeModelRxCmb() 
{
...
void CIehpItemDlg::OnSelchangeModelRyCmb() 
{
...
void CIehpItemDlg::OnSelchangeModelRzCmb() 
{
...
void CIehpItemDlg::MakeFibrCombo(BOOL bModify)
{  
	...
	else if(m_nElementType==HT_WALL_CRB)
	{
		...
		CArray<T_THIK_K, T_THIK_K> KeyThik; KeyThik.Add(ThikK);
		CArray<T_ELEM_K, T_ELEM_K> KeyElem;
		m_pDoc->m_pAttrCtrl->GetKeyListThikUser(&KeyThik, NULL, &KeyElem);

		CStoryData* pStoryData = m_pDoc->GetStoryData();

		CArray< T_FIBW_K, T_FIBW_K> aFibwK;
		UINT unInternalWallD = 0;
		for (int i=0; i<KeyElem.GetSize(); i++)
		{
			T_ELEM_K CurElemK = KeyElem[i];
			T_ELEM_D CurElemD;
			if (!m_pDoc->m_pAttrCtrl->GetElem(CurElemK, CurElemD)) { ASSERT(0); continue; }
			if (!m_pDoc->m_pAttrCtrl->IsWall(CurElemD.eltyp)) {  continue; }

			T_STOR_K StorK;
			m_pDoc->m_pAttrCtrl->GetStorKeyByElemKey(StorK, CurElemK, pStoryData);
			aFibwK.Add(m_pDoc->m_pAttrCtrl->GetFibwKeyByWallID(CurElemD.nWallId, StorK, D_FIBW_TYPE_IEHP));
		}
		CArrayUtil::RemoveDup(aFibwK);
		for (int i=0; i<aFibwK.GetSize(); i++)
		{
			T_FIBW_D FibwD;
			if(!m_pDoc->m_pAttrCtrl->GetFibw(aFibwK[i], FibwD)) { ASSERT(0); continue; }
			CDlgUtil::CobxAddItem(m_cmbFibrName, FibwD.strName, aFibwK[i]);
		}
...
void CIehpItemDlg::OnSelchangeSectCmb() 
{
	MakeFibrCombo(FALSE);	
}

void CIehpItemDlg::CtrlTypeManager()
{
	UpdateData(TRUE);
...
void CIehpItemDlg::OnModelChangedDx() 
{ 
	DWORD nItemType;
...
void CIehpItemDlg::OnModelChangedDy() 
{ 
	DWORD nItemType;
...
void CIehpItemDlg::OnModelChangedDz()
{ 
	DWORD nItemType;
...
void CIehpItemDlg::OnModelChangedRx()
{ 
	DWORD nItemType;
...
void CIehpItemDlg::OnModelChangedRy()
{ 
	DWORD nItemType;
...
void CIehpItemDlg::OnModelChangedRz()
{ 
	DWORD nItemType;
...
void CIehpItemDlg::OnLocationChangedRyCmb()
{
	if (m_nNewHingeType == 0 && m_nInteractionType == 1) //Moment-Rotation && P-M
	{
...
void CIehpItemDlg::OnLocationChangedRzCmb()
{
	if (m_nNewHingeType == 0 && m_nInteractionType == 1) //Moment-Rotation && P-M
	{
...
void CIehpItemDlg::ChangebyCtrlDofRy()
{
	if (m_nNewHingeType == 0 && m_nInteractionType == 1) //Moment-Rotation && P-M
	{
		DWORD nItemType;
...
void CIehpItemDlg::ChangebyCtrlDofRz()
{
	if (m_nNewHingeType == 0 && m_nInteractionType == 1) //Moment-Rotation && P-M
	{
		DWORD nItemType;
...
void CIehpItemDlg::MakeBtnMenu(int ix)
{
	CString aCat[] = {
...
void CIehpItemDlg::SetCurSelBtnMenu(int ix)
{
	DWORD nItemType;
...
int CIehpItemDlg::GetTabStyle(int nIndex)
{
	UpdateData(TRUE);
...
void CIehpItemDlg::SetAssignHingeMode(BOOL bAssignHingeMode, T_IEHG_K IehgKey)
{
	m_bAssignHingeMode = bAssignHingeMode;
	m_AssignedIehgKey  = IehgKey;
}

void CIehpItemDlg::EnableDisableCtrls_WallType()
{
...
void CIehpItemDlg::EnableDisableCtrls_MatlType()
{  
...
void CIehpItemDlg::EnableDisableCtrls_RefLoc()
{
	BOOL bEnable = FALSE;
	if(m_nElementType==HT_BEAM_COLM)
	{
...
void CIehpItemDlg::EnableDisableCtrls_DefType()
{
...
void CIehpItemDlg::EnableDisableCtrls_HingeType()
{
...
void CIehpItemDlg::EnableDisableCtrls_Interact()
{
...
void CIehpItemDlg::EnableDisableCtrls_Comp()
{
	BOOL bEnableComp[6];
	for(int i=0; i<6; i++) bEnableComp[i] = TRUE;  
	bEnableComp[3] = m_nElementType==HT_SPRING;
...
void CIehpItemDlg::EnableDisableCtrls_YldSurf()
{
	BOOL bEnable = FALSE;
...
void CIehpItemDlg::EnableDisableCtrls_FiberName()
{
	BOOL bEnable = TRUE;
	...
	// move control
	CArray<UINT, UINT> aOK;
	aOK.Add(IDOK);
	aOK.Add(IDCANCEL);
	aOK.Add(IDC_CMD_APPLY);

	CRect rRef, rToMove;
	if(bFiber)
	{
		GetDlgItem(IDC_CMD_PROP_SHEAR_R_BTN)->GetWindowRect(rRef);
	}
	else
	{
		GetDlgItem(IDC_CMD_PROP_YS_BTN)->GetWindowRect(rRef);
	}
	GetDlgItem(IDC_WG_CMD_STATIC2)->GetWindowRect(rToMove);
	rToMove.bottom = rRef.bottom + globalUtils.ScaleByDPI(5);
	GetDlgItem(IDC_WG_CMD_STATIC2)->SetWindowPos(NULL, rToMove.left, rToMove.top, rToMove.Width(), rToMove.Height(), SWP_NOMOVE | SWP_NOZORDER);

	rRef = rToMove;
	GetDlgItem(IDOK)->GetWindowRect(rToMove);
	int nDistY = rRef.bottom - rToMove.top + globalUtils.ScaleByDPI(16);

	CDlgUtil::CtrlMoveDistY(this, aOK, nDistY);
	// resize self size
	CRect rectLast;
...
void CIehpItemDlg::EnableDisableCtrls()
{
	LockWindowUpdate();
	UpdateData(TRUE);
...
void CIehpItemDlg::OnStiffnessOptionBtn()
{
...
void CIehpItemDlg::OnMphiLocBtn()
{ 
...
void CIehpItemDlg::OnFiberNameBtn()
{ 
	UpdateData(TRUE);
...
void CIehpItemDlg::OnFiberOutplBtn()
{ 
...
void CIehpItemDlg::InitCtrl()
{
	m_edtShearR.SetUnitType(D_UNITSYS_NONE);
	m_edtShearR.SetReadOnly(TRUE);
}

void CIehpItemDlg::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	HWND hWnd;
...
void CIehpItemDlg::UpdateBuffer()
{
...
BOOL CIehpItemDlg::IsHysModel_Unable1stCtrl(int nHysModel)
{
	switch(nHysModel)
	{
	case D_IEHP_CLOU:
...
void CIehpItemDlg::InitCtrlTexts()
{
...
bool CIehpItemDlg::UpdatePropData(int nDof)
{
	...
	int nFg = nDof;
	if (m_nInteractionType != 0 && nDof > 3)
	{
		nFg = nDof + 3;
	}
