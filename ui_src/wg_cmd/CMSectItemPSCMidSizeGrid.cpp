// CMSectItemPSCMidSizeGrid.cpp : implementation file
//
...
CCMSectItemPSCMidSizeGrid::CCMSectItemPSCMidSizeGrid()
{
	m_pSect = 0;
	m_bInitDone = FALSE;
	m_pParent=0;
...
BEGIN_MESSAGE_MAP(CCMSectItemPSCMidSizeGrid, CGXGridWnd)
	//{{AFX_MSG_MAP(CCMSectItemPSCMidSizeGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CCMSectItemPSCMidSizeGrid message handlers

////////////////////////////////외부에서 호출되는 함수////////////////////////
void CCMSectItemPSCMidSizeGrid::InitGrid()
{
...
void CCMSectItemPSCMidSizeGrid::SetDataSource(T_SECT_D* pSect,CWnd* pParent)
{
	ASSERT(pSect);
	m_pSect = pSect;
	m_pParent=pParent;
...
void CCMSectItemPSCMidSizeGrid::SetStyleOnSizeCell()
{
	...
	CString aTitleLeft[] = {
		_T("H1"),_T("HL1"),_T("HL2"),_T("HL2-1"),_T("HL2-2"),_T("HL3"),_T("HL4"),_T("HL4-1"),_T("HL4-2"),_T("HL5"),_T("BL1"),_T("BL2"),_T("BL2-1"),_T("BL2-2"),_T("BL4-1"),_T("BL4-2"),
	};
	CString aTitleRight[] = {
		_T("HR1"),_T("HR2"),_T("HR2-1"),_T("HR2-2"),_T("HR3"),_T("HR4"),_T("HR4-1"),_T("HR4-2"),_T("HR5"),_T("BR1"),_T("BR2"),_T("BR2-1"),_T("BR2-2"),_T("BR4-1"),_T("BR4-2"),
	};
	...
	for(i=D_SIZE_START_ROW+1;i<=D_SIZE_END_ROW;i++)
	{
		SetValueRange(CGXRange(i,3),aTitleRight[i-D_SIZE_START_ROW-1]);
	}
	DecideEnable();
...
/////////////////////////데이터를 그리드에 보여주는 함수////////////////////
void CCMSectItemPSCMidSizeGrid::ShowData()
{
	...
	if (pControl->IsActive()) bActive = TRUE;
	
	for (i = D_SIZE_START_ROW; i <= D_SIZE_END_ROW; i++)
	{
		if (bActive && i == ncRow && 2 == ncCol && m_bStarted) continue;
		SetValueRange(CGXRange(i, 2), pSize[i-D_SIZE_START_ROW]);
	}
	for (i = D_SIZE_START_ROW+1; i <= D_SIZE_END_ROW; i++)
	{
		if (bActive && i == ncRow && 4 == ncCol) continue;
		SetValueRange(CGXRange(i, 4), pSize[i-D_SIZE_START_ROW-1+plusNum]);
	}
	m_bStarted=TRUE;
}

///////////////////////////그리드의 데이터를 저장하는 함수////////////////////
BOOL CCMSectItemPSCMidSizeGrid::SaveData()
{
	CString csNum;
	double dNum;
	...
	for (i = D_SIZE_START_ROW+1; i <= D_SIZE_END_ROW; i++)
	{
		...
		if (GetDoubleValue(csNum, dNum)) 
		{
			pSize[i-D_SIZE_START_ROW-1+plusNum] = dNum;
		}
		else
		{
			pSize[i-D_SIZE_START_ROW-1+plusNum] = 0.0;
		}
...
void CCMSectItemPSCMidSizeGrid::OnSizeChange()
{
	// 임시(다시 그려주기)
...
BOOL CCMSectItemPSCMidSizeGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
...
BOOL CCMSectItemPSCMidSizeGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
...
BOOL CCMSectItemPSCMidSizeGrid::SetDownValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i,j;
	i=nRow+1;
	j=nCol;
...
BOOL CCMSectItemPSCMidSizeGrid::SetUpValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i,j;
	i = nRow-1;
	j = nCol;
...
BOOL CCMSectItemPSCMidSizeGrid::CopyRange(const CGXRangeList& selList)
{
	...
	while(TRUE)
	{
		...
		if(mRange->IntersectRange(CGXRange(D_SIZE_START_ROW+1,4,D_SIZE_END_ROW,4),selList.GetAt(position)))
		{
			mSelList.AddTail(mRange);
		}    
		else
...
BOOL CCMSectItemPSCMidSizeGrid::OnPasteFromClipboard(const CGXRange &range)
{
	CGXGridWnd x;
	CGXRange r;
	CGXRange toRange;
	...
	for(toi=toRange.top;toi<=toRange.bottom;toi++)
	{
		...
		for(toj=toRange.left;toj<=toRange.right;toj++)
		{
			if(toj==1||toj==3)
				continue;
			BOOL bCondition=0;
			bCondition=(toi==(D_SIZE_START_ROW)&&toj==4);
...
BOOL CCMSectItemPSCMidSizeGrid::VerifyPasteRange(CGXRange& from,CGXRange& to)
{
...
int CCMSectItemPSCMidSizeGrid::GetSelectedRowsCount(int top,int bottom)
{
	int toRows;
	toRows=bottom-top+1;
	if(top<=1&&bottom>=1)
...
int CCMSectItemPSCMidSizeGrid::GetSelectedColsCount(int left,int right)
{
	int toCols;
	toCols=0;
	if(left<=2&&right>=2)
...
void CCMSectItemPSCMidSizeGrid::DecideEnable()
{
	
	T_SECT_D_PSC_MID m_Cell;
	m_Cell.Set(*m_pSect,true,true);
	SetStyleRange(CGXRange(D_SIZE_START_ROW,2), CGXStyle()
		.SetEnabled(m_Cell.J1).SetInterior(::GetSysColor((m_Cell.J1)?COLOR_WINDOW:COLOR_BTNFACE)));
	...
	SetStyleRange(CGXRange(D_SIZE_START_ROW+4,2), CGXStyle()
		.SetEnabled(m_Cell.JL2).SetInterior(::GetSysColor((m_Cell.JL2)?COLOR_WINDOW:COLOR_BTNFACE)));
	SetStyleRange(CGXRange(D_SIZE_START_ROW+13,2), CGXStyle()
		.SetEnabled(m_Cell.JL2).SetInterior(::GetSysColor((m_Cell.JL2)?COLOR_WINDOW:COLOR_BTNFACE)));
	
	SetStyleRange(CGXRange(D_SIZE_START_ROW+8,2), CGXStyle()
		.SetEnabled(m_Cell.JL3).SetInterior(::GetSysColor((m_Cell.JL3)?COLOR_WINDOW:COLOR_BTNFACE)));
	SetStyleRange(CGXRange(D_SIZE_START_ROW+15,2), CGXStyle()
		.SetEnabled(m_Cell.JL3).SetInterior(::GetSysColor((m_Cell.JL3)?COLOR_WINDOW:COLOR_BTNFACE)));

	SetStyleRange(CGXRange(D_SIZE_START_ROW+7,2), CGXStyle()
		.SetEnabled(m_Cell.JL4).SetInterior(::GetSysColor((m_Cell.JL4)?COLOR_WINDOW:COLOR_BTNFACE)));
	SetStyleRange(CGXRange(D_SIZE_START_ROW+14,2), CGXStyle()
		.SetEnabled(m_Cell.JL4).SetInterior(::GetSysColor((m_Cell.JL4)?COLOR_WINDOW:COLOR_BTNFACE)));  
	
	SetStyleRange(CGXRange(D_SIZE_START_ROW+3,4), CGXStyle()
		.SetEnabled(m_Cell.JR1).SetInterior(::GetSysColor((m_Cell.JR1)?COLOR_WINDOW:COLOR_BTNFACE)));
	SetStyleRange(CGXRange(D_SIZE_START_ROW+12,4), CGXStyle()
		.SetEnabled(m_Cell.JR1).SetInterior(::GetSysColor((m_Cell.JR1)?COLOR_WINDOW:COLOR_BTNFACE)));

	SetStyleRange(CGXRange(D_SIZE_START_ROW+4,4), CGXStyle()
		.SetEnabled(m_Cell.JR2).SetInterior(::GetSysColor((m_Cell.JR2)?COLOR_WINDOW:COLOR_BTNFACE)));
	SetStyleRange(CGXRange(D_SIZE_START_ROW+13,4), CGXStyle()
		.SetEnabled(m_Cell.JR2).SetInterior(::GetSysColor((m_Cell.JR2)?COLOR_WINDOW:COLOR_BTNFACE)));

	SetStyleRange(CGXRange(D_SIZE_START_ROW+8,4), CGXStyle()
		.SetEnabled(m_Cell.JR3).SetInterior(::GetSysColor((m_Cell.JR3)?COLOR_WINDOW:COLOR_BTNFACE)));
	SetStyleRange(CGXRange(D_SIZE_START_ROW+15,4), CGXStyle()
		.SetEnabled(m_Cell.JR3).SetInterior(::GetSysColor((m_Cell.JR3)?COLOR_WINDOW:COLOR_BTNFACE)));
	
	SetStyleRange(CGXRange(D_SIZE_START_ROW+7,4), CGXStyle()
		.SetEnabled(m_Cell.JR4).SetInterior(::GetSysColor((m_Cell.JR4)?COLOR_WINDOW:COLOR_BTNFACE)));
	...
	int strRight,strLeft;
	strRight=m_Cell.nRightShape;
	strLeft=m_Cell.nLeftShape;
	...
	{
		strRight = strLeft;
		for(int i=D_SIZE_START_ROW+1;i<=D_SIZE_END_ROW;i++)
		{
			SetStyleRange(CGXRange(i,4), CGXStyle()
				.SetEnabled(!m_Cell.bSymmetric).SetInterior(::GetSysColor((!m_Cell.bSymmetric)?COLOR_WINDOW:COLOR_BTNFACE)));
		}
...
void CCMSectItemPSCMidSizeGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMSectItemPSCMidSizeGrid::OnStartEditing(ROWCOL nRow, ROWCOL nCol)
{
	if (!CGXGridWnd::OnStartEditing(nRow, nCol)) return FALSE;
	return TRUE;
}
////////////////////////////////////////안 바뀌는 부분//////////////////////////////
BOOL CCMSectItemPSCMidSizeGrid::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
...
CString CCMSectItemPSCMidSizeGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString csNum;
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BOOL CCMSectItemPSCMidSizeGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CCMSectItemPSCMidSizeGrid::OnValidateCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMSectItemPSCMidSizeGrid::OnActivateGrid(BOOL bActivate)
{
...
BOOL CCMSectItemPSCMidSizeGrid::GetDoubleValue(CString& csNum, double& dNum)
{
	int ecode;
...
BOOL CCMSectItemPSCMidSizeGrid::IsValidDoubleNumber(CString value, int &nErrCode)
{
	value.TrimLeft();
	value.TrimRight();
...
BOOL CCMSectItemPSCMidSizeGrid::ConvertFormulaToValue(CString& value)
{
...
BOOL CCMSectItemPSCMidSizeGrid::IsNumeric(CString str, int& ecode)
{
...
BOOL CCMSectItemPSCMidSizeGrid::InvalidDoubleNumber(int nErrCode)
{
	switch(nErrCode)
	{
...
BOOL CCMSectItemPSCMidSizeGrid::ValidateError(LPCTSTR lpszError)
{
