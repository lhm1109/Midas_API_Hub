...
/////////////////////////////////////////////////////////////////////////////
// CSpfcItemGrid
CString CSpfcItemGrid::m_aCurDefVal[] = {_T("0.0"), _T("0.0")};
CSpfcItemGrid::CSpfcItemGrid(BOOL bPsov)
{
	m_pData = 0;

	AddColInfo(_LS(IDS_WG_CMD__ADD2__Period),        t_real, 0);
	AddColInfo(_LS(IDS_WG_CMD__ADDD__Spectral_Data), t_real, 0);

	m_aPrimaryKey.Add(0);
	m_bPushOver = bPsov;
}

CSpfcItemGrid::~CSpfcItemGrid()
{
}


BEGIN_MESSAGE_MAP(CSpfcItemGrid, CTBCommon)
	//{{AFX_MSG_MAP(CSpfcItemGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CSpfcItemGrid message handlers

void CSpfcItemGrid::Initialize(T_SPFC_D *pData, BOOL bShowReadOnly)
{
	ASSERT(pData);
	m_pData = pData;
...
BOOL CSpfcItemGrid::InsertRecordByKey(const double &key, const double &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CSpfcItemGrid::DeleteRecordByKey(const double &key, const double &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
/////////////////////////////////////////////////////////////////////////////
// CSpfcItemGrid Overridable Functions
void CSpfcItemGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	return;
}

long CSpfcItemGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
	return 0;
}

BOOL CSpfcItemGrid::ValidateField(CString value, int nColID)
{
...
BOOL CSpfcItemGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CSpfcItemGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, double &key, double &data)
{
	/*
...
BOOL CSpfcItemGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CSpfcItemGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
...
BOOL CSpfcItemGrid::DeleteFromDB(ROWCOL nRow)
{
	double key;
	double data;
...
BOOL CSpfcItemGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CSpfcItemGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
	VERIFY(GetColumnNoFor(0, nCol));
...
BOOL CSpfcItemGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CSpfcItemGrid::GetNewKey()
{
	CString value(_T("0"));
	return value;
}

CString CSpfcItemGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CSpfcItemGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CSpfcItemGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CSpfcItemGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	double* pKey;
	double* pData;
...
long CSpfcItemGrid::GetCountData()
{
	return m_pData->arSpecFuncData.GetSize();
}

void CSpfcItemGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0); // no filtering
}

void CSpfcItemGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	double Key;
	double Data;
	...
	{
		Key = m_pData->arSpecFuncData[nCount].dblPeriod;
		Data = m_pData->arSpecFuncData[nCount].dblValue;

		SetValue(GetParam()->GetData(), nCount+1, Key, Data, awCols);
	} // end of loop nCount
}

void CSpfcItemGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	double Key;
	double Data;
...
/////////////////////////////////////////////////////////////////////////////
// CSpfcItemGrid Implementation
BOOL CSpfcItemGrid::GetValue(ROWCOL nRow, double &key, double &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CSpfcItemGrid::SetValue(ROWCOL nRow, const double &key, const double &data)
{
	//
	CRowColArray awCols;
...
BOOL CSpfcItemGrid::SetValue(CGXData* pData, ROWCOL nRow, const double &key, const double &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CSpfcItemGrid::ConvStrToData(int i, CString& value, double& data)
{
	if (i==1) data = _tstof(value);
}

void CSpfcItemGrid::ConvDataToStr(int i, const double& data, CString& value)
{
	if (i==1) value.Format(DATA_FMT, data);
}

void CSpfcItemGrid::MakeSearchKey(const double &key, const double &data, CStringArray &aKey)
{
	int nColID;
...
void CSpfcItemGrid::SetHeaderTitle(T_UNIT_SYSTEM &us, BOOL bInit)
{
	// Set Title
	...
	int aUnit[][2] = {
		{CUnitCtrl::m_SPFC_UNIT.dblPeriod, CUnitCtrl::m_SPFC_UNIT.dblValue[0]}, 
		{CUnitCtrl::m_SPFC_UNIT.dblPeriod, CUnitCtrl::m_SPFC_UNIT.dblValue[1]}, 
		{CUnitCtrl::m_SPFC_UNIT.dblPeriod, CUnitCtrl::m_SPFC_UNIT.dblValue[2]}, 
		{CUnitCtrl::m_SPFC_UNIT.dblPeriod, CUnitCtrl::m_SPFC_UNIT.dblValue[3]}, 
	};

	double dDPIFactor = globalUtils.GetDPIScale()>1?globalUtils.GetDPIScale()*0.9:1.;
	int aSize[] = {60, int(85 * dDPIFactor) };
	if(m_bPushOver)
	{
		aSize[0] = 100;
		aSize[1] = int(140 * dDPIFactor);
	}
	...
	if (bInit) SetColWidth(0, 0, int(35 * dDPIFactor));
}

BOOL CSpfcItemGrid::FindData(double key, int& nPos, int nStart)
{
	int i;
	int nSize = m_pData->arSpecFuncData.GetSize();
	...
	{
		csVal.Format(DATA_FMT, m_pData->arSpecFuncData[i].dblPeriod);
		dblVal = _tstof(csVal);
...
BOOL CSpfcItemGrid::ExistAllKey(CArray<double, double>& aKey, double& errorKey)
{
...
BOOL CSpfcItemGrid::AddData(double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(data);
...
BOOL CSpfcItemGrid::AddData(CArray<double, double> &aKey, CArray<double, double> &aData, BOOL bNotify)
{
...
BOOL CSpfcItemGrid::DelData(double key)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(0.0);
...
BOOL CSpfcItemGrid::DelData(CArray<double, double> &aKey, BOOL bNotify)
{
...
BOOL CSpfcItemGrid::ModifyData(double oldKey, double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aOldKey.Add(oldKey);
	aKey.Add(key);
...
BOOL CSpfcItemGrid::ModifyData(CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
...
void CSpfcItemGrid::UpdateBuffer(int nCmd, CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
...
void CSpfcItemGrid::DBChanged()
{
	if(m_bPushOver)
	{
		CETC_GDS_UserPage *pDlg = (CETC_GDS_UserPage*)GetParent();
		if (!pDlg) return;
		if (pDlg->GetSafeHwnd() == 0) return;
		if (!IsWindow(pDlg->GetSafeHwnd())) return;
		
		// data가 변경되었으므로 user로 변경
		m_pData->nSpecCodeType = D_SPFC_CODE_USER;
		pDlg->SpectralDataChanged(); 
	}
	else
	{
		...
		// DB가 변경되면 SpecCodeType을 User로 세팅  
		// OnDataChanged()가 호출되면 거기에서 Description을 null로 변경함.
		m_pData->nSpecCodeType = D_SPFC_CODE_USER;
		
		pDlg->OnDataChanged();
		const auto data = pDlg->GetParamData();
		if (data.nSpecCodeType == D_SPFC_CODE_USER)
		{
			pDlg->SetDescription(L"");
		}
	}
}

BOOL CSpfcItemGrid::OnPasteFromClipboard(const CGXRange &range)
{
	if (IsLockEdit()) return FALSE; // display only mode이면 바로 리턴
	return CTBCommon::OnPasteFromClipboard(range);
}

BOOL CSpfcItemGrid::Copy()
{
	// Gen Trial은 Copy 막음
