// CMSectScaleFactorDlgNew.cpp : implementation file
//
...
CCMSectScaleFactorDlgNew::CCMSectScaleFactorDlgNew(CWnd* pParent /*=NULL*/)
	: CDialogMove(CCMSectScaleFactorDlgNew::IDD, pParent)
{
	//{{AFX_DATA_INIT(CCMSectScaleFactorDlgNew)
	//}}AFX_DATA_INIT
	m_pDoc = CDBDoc::GetDocPoint();
	ASSERT(m_pDoc);

	m_pGrid = new CCMSectScaleFactorGrid();

	m_aCtrlEditJ.RemoveAll();
	m_aCtrlEditJ.Add(IDC_CMD_EDIT_AREA2);
	m_aCtrlEditJ.Add(IDC_CMD_EDIT_ASY2);
	m_aCtrlEditJ.Add(IDC_CMD_EDIT_ASZ2);
	m_aCtrlEditJ.Add(IDC_CMD_EDIT_LXX2);
	m_aCtrlEditJ.Add(IDC_CMD_EDIT_LYY2);
	m_aCtrlEditJ.Add(IDC_CMD_EDIT_LZZ2);
	m_aCtrlEditJ.Add(IDC_CMD_EDIT_WEIGHT2);

	EnableLayout();
}

CCMSectScaleFactorDlgNew::~CCMSectScaleFactorDlgNew()
{
	if (m_pGrid != 0)
	{
		delete m_pGrid;
...
void CCMSectScaleFactorDlgNew::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMSectScaleFactorDlgNew)
	...
	UINT aCtrlEdit[7] = { IDC_CMD_EDIT_AREA, IDC_CMD_EDIT_ASY, IDC_CMD_EDIT_ASZ,
												...
												IDC_CMD_EDIT_WEIGHT };
	UINT aCtrlEdit2[7] = { IDC_CMD_EDIT_AREA2, IDC_CMD_EDIT_ASY2, IDC_CMD_EDIT_ASZ2,
												IDC_CMD_EDIT_LXX2 , IDC_CMD_EDIT_LYY2, IDC_CMD_EDIT_LZZ2,
												IDC_CMD_EDIT_WEIGHT2 };


	for (int i = 0; i < 7; ++i) 
	{
		DDX_Control(pDX, aCtrlEdit[i], m_editStif[i]);
		DDX_Control(pDX, aCtrlEdit2[i], m_editStif2[i]);
	}
	DDX_Control(pDX, IDC_CMD_BEFORE_CHK, m_chkBefore);
	DDX_Control(pDX, IDC_CMD_AFTER_CHK, m_chkAfter);
	DDX_Control(pDX, IDC_CMD_IJ_CHK, m_chkIJ);
	DDX_Control(pDX, IDC_CMD_GRID, *m_pGrid);
}


BEGIN_MESSAGE_MAP(CCMSectScaleFactorDlgNew, CDialogMove)
	//{{AFX_MSG_MAP(CCMSectScaleFactorDlgNew)
	ON_WM_SIZING()
	ON_WM_SIZE()
	//ON_WM_PAINT()
	...
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_CMD_LIST_SECTION, OnClickCmdListSection) // MNET:XXXX 20070620 mylee
	ON_BN_CLICKED(IDC_CMD_IJ_CHK, OnCmdIChkIJ)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CCMSectScaleFactorDlgNew message handlers

BOOL CCMSectScaleFactorDlgNew::OnInitDialog() 
{
...
void CCMSectScaleFactorDlgNew::OnCmdBtnAdd() 
{
	...
	CArray<T_BNGR_K, T_BNGR_K> aKeyBngr; aKeyBngr.RemoveAll(); aKeyBngr.Add(GetSelectedBngr());
	if (!m_pDoc->m_pAttrCtrl->GetSecfBngr(FALSE, aKeyBngr, key, Data)) return; 

	// 공통 입력값을 Data 에 세팅한다.
	Data.GroupKey = GetSelectedBngr(); 
	
	// MNET:XXXX 20070328 mylee -  I, J 관련 세팅
	Data.bSameIJ = TRUE;
	for (int i = 0; i < 7; ++i)
		Data.dScaleFactor[i] = Data.dScaleFactorJ[i] = m_editStif[i].GetEditValue();
	
	if (m_chkIJ.IsWindowEnabled()==TRUE && m_chkIJ.GetCheck()==FALSE)
	{
		Data.bSameIJ = FALSE;
		for (int i = 0; i < 7; ++i) 
			Data.dScaleFactorJ[i] = m_editStif2[i].GetEditValue();
	}

	pos = m_listSect.GetFirstSelectedItemPosition();
...
void CCMSectScaleFactorDlgNew::OnCmdBtnStiff() 
{
	T_SECF_K CurKey;
	...
	// SectData 는 여기에서만 쓰이고 끝이므로 변경 해줘도 OK
	...
	if (SecfData.bBefore == TRUE)
	{
		...
		ApplyScaleFactor(SectData.SectBefore.SectJ.Stiffness, SecfData, TRUE); // TRUE 는 J단 Factor 반영
	}
	if (SecfData.bAfter == TRUE)
	{
		...
		ApplyScaleFactor(SectData.SectAfter.SectJ.Stiffness, SecfData, TRUE);
	}

	// bMultipleElastic 이 FALSE 일때는 내부계산 한번 더 안하므로 Apply 된 상태 유지
	CCMStiffDlg dlg(m_pDoc, &SectData, NULL, this);
	dlg.DoModal();	
}

void CCMSectScaleFactorDlgNew::OnCmdBtnBdgr() 
{
	// TODO: Add your control notification handler code here
	CreateOrActivateDlg(CDBDoc::GetDocPoint(),CCMBndrGrupDefDlg::IDD,this);
}

void CCMSectScaleFactorDlgNew::OnClickCmdListSection(NMHDR* pNMHDR, LRESULT* pResult) 
{
	m_btnAdd.EnableWindow(FALSE);
	m_chkBefore.EnableWindow(FALSE); 
	m_chkAfter.EnableWindow(FALSE);
	...
	{ 
		while (pos)
		{
			...
			if (!m_pDoc->m_pAttrCtrl->GetSect(SectKey, SectData)) { ASSERT(0); return; }
			if (!SectKey) return; 
			else 
			{
				...
				bAllTapered = bAllTapered && (SectData.nStype==D_SECT_TYPE_TAPERED);
			}
		}
	}
	// 전부 Tapered 인 경우만 I, J 버튼 Enable
	ChangeCheckIJ(bAllTapered);
	OnCmdIChkIJ();
...
void CCMSectScaleFactorDlgNew::OnCmdIChkIJ() 
{
	// TODO: Add your control notification handler code here
	if (m_chkIJ.IsWindowEnabled()==TRUE && m_chkIJ.GetCheck()==FALSE)
	{
		CDlgUtil::CtrlEnableDisable(this, m_aCtrlEditJ, TRUE);
	}
	else
	{
		CDlgUtil::CtrlEnableDisable(this, m_aCtrlEditJ, FALSE);
		for (int i=0 ; i<7 ; ++i)
			m_editStif2[i].SetEditUnit(m_editStif[i].GetEditValue());
	}
}

void CCMSectScaleFactorDlgNew::OnCmdIChkIJ2() // 인위적으로 불러주는 함수
{
	// TODO: Add your control notification handler code here
	if (m_chkIJ.IsWindowEnabled()==TRUE && m_chkIJ.GetCheck()==FALSE)
	{
...
/////////////////////////////////////////////////////////////////////////////
void CCMSectScaleFactorDlgNew::OnChangeCurrentSecf(T_SECF_K key)
{
	...
	{
		m_editStif[i].SetEditUnit(data.dScaleFactor[i]);
		m_editStif2[i].SetEditUnit(data.dScaleFactorJ[i]);
	}

	// I = J 체크 관련 : Tapered 일 경우만 Enable 시키고 현재 Data의 값에 따라 Check/Uncheck
	T_SECT_D SectData;
	if (!m_pDoc->m_pAttrCtrl->GetSect(key.key.entity, SectData)) { ASSERT(0); return; }
	ChangeCheckIJ(SectData.nStype==D_SECT_TYPE_TAPERED);
	if (m_chkIJ.IsWindowEnabled()) m_chkIJ.SetCheck(data.bSameIJ); // 현재 데이터의 값으로 Check
	OnCmdIChkIJ2(); // EditJ 활성/비활성
	

	// Before/After 체크 관련 : Enable/Disable Buttons - OnClickCmdListSection 함수와 같은 동작 하도록
...
void CCMSectScaleFactorDlgNew::SetHeaderTitle()
{
...
void CCMSectScaleFactorDlgNew::MakeItemEx(CArray<T_SECT_K, T_SECT_K>* paKey/*=NULL*/)
{
	CWaitCursor Cursor;
...
BOOL CCMSectScaleFactorDlgNew::InsertItem(T_SECT_K Key, T_SECT_D& Data, T_SECF_D* WorstSecfData)
{
	LVITEM lvitem;
	CString str;
...
int CCMSectScaleFactorDlgNew::FindInsertionPos(T_SECT_K nID)
{
	T_SECT_K key;
...
CString CCMSectScaleFactorDlgNew::DataToStr(int i, T_SECT_K Key, T_SECT_D &Data)
{
	CString str;
...
void CCMSectScaleFactorDlgNew::ApplyScaleFactor(T_SECT_STIFFNESS& Stiff, T_SECF_D& Secf, BOOL bJ)
{
	if (!bJ)
	{
	...
	{
		Stiff.Area *= Secf.dScaleFactorJ[0];
		Stiff.Asy  *= Secf.dScaleFactorJ[1];
		Stiff.Asz  *= Secf.dScaleFactorJ[2];
		Stiff.Rxx  *= Secf.dScaleFactorJ[3];
		Stiff.Ryy  *= Secf.dScaleFactorJ[4];
		Stiff.Rzz  *= Secf.dScaleFactorJ[5];
		Stiff.WArea*= Secf.dScaleFactorJ[6];
	}
}

void CCMSectScaleFactorDlgNew::SetBngrCombo()
{
...
T_BNGR_K CCMSectScaleFactorDlgNew::GetSelectedBngr()
{
...
void CCMSectScaleFactorDlgNew::ChangeSelect(T_BNGR_K Key)
{
...
int CCMSectScaleFactorDlgNew::TypeByDefaultSecfValue(T_SECF_K key)
{
	return TypeByDefaultSecfValue(key.key.entity);
}

// Composite Box, I / 기타 Composite / Non Composite의 세 타입으로 나눔
int CCMSectScaleFactorDlgNew::TypeByDefaultSecfValue(T_SECT_K key)
{
	T_SECT_D SectData;
	...
	if (nStype==D_SECT_TYPE_COMPO_B      || nStype==D_SECT_TYPE_COMPO_I      || nStype==D_SECT_TYPE_COMPO_TUB      ||
		...
		return 0; // Composite Box, I + Tapered Composite 추가
	else if (nStype==D_SECT_TYPE_COMPO_CI || nStype==D_SECT_TYPE_COMPO_CT ||
					 nStype==D_SECT_TYPE_COMPO_PC || nStype==D_SECT_TYPE_COMPO_G || IsCmpTapCICT(SectData)) 
		return 1; // Composite Others + Tapered Composite 추가
	else 
		return 2; // Not Composite
}

void CCMSectScaleFactorDlgNew::ChangeCheckIJ(BOOL bTapered)
{
	if (!bTapered)
	{
		m_chkIJ.SetCheck(TRUE); 
		m_chkIJ.EnableWindow(FALSE);
		for (int i=0 ; i<7 ; ++i)
			m_editStif2[i].SetEditUnit(m_editStif[i].GetEditValue());
		CDlgUtil::CtrlEnableDisable(this, m_aCtrlEditJ, FALSE);
	...
	{
		m_chkIJ.EnableWindow(TRUE);
		CDlgUtil::CtrlEnableDisable(this, m_aCtrlEditJ, TRUE);
	}
}

void CCMSectScaleFactorDlgNew::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	HWND hWnd;
...
void CCMSectScaleFactorDlgNew::UpdateBuffer()
{
	int nModified = 0;
	...
	while(pos != NULL)
	{
		...
		{
		case(UR_SECF_ADD):
				// 삽입될 위치(nRow)를 찾아 넣는다. -> 다 지우고 다시 뿌림
				nCount = m_pGrid->GetRowCount();
				m_pGrid->SetRecordCount(0);
				//m_pGrid->DeleteRows(1, nCount);
				m_pGrid->MakeItemEx();
				m_pGrid->SetCurrentCell(1, 1);
				//m_pGrid->InsertRecordByKey(key, data);
				nModified |= 0x01;
				break;
		case(UR_SECF_DEL):
				// 삭제될 위치(nRow)를 찾아서 지운다. -> 다 지우고 다시 뿌림
				nCount = m_pGrid->GetRowCount();
				m_pGrid->SetRecordCount(0);
				//m_pGrid->DeleteRows(1, nCount);
				m_pGrid->MakeItemEx();
...
// Size 조절
void CCMSectScaleFactorDlgNew::OnSizing(UINT fwSide, LPRECT pRect) 
{
	CRect r(pRect);
...
void CCMSectScaleFactorDlgNew::OnSize(UINT nType, int cx, int cy) 
{
...
void CCMSectScaleFactorDlgNew::OnPaint() 
{
...
void CCMSectScaleFactorDlgNew::SaveWindowSizeData()
{
	...
	CRect r;
	GetDlgItem(IDC_CMD_GRID)->GetWindowRect(r);
	m_nGridFrmHeight = r.Height();
}

void CCMSectScaleFactorDlgNew::ArrangeControls(LPRECT pRect)
{
