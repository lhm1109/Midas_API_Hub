...
CSpfcCH2019Util::CSpfcCH2019Util()
{
	m_nER = 0;
	m_nSDC = 0;
	m_nSFI = 0;
...
bool CSpfcCH2019Util::CheckValid()
{
...
void CSpfcCH2019Util::MakeSpectrumData(bool bOnlyCalc)
{
	double st = PRD_ST;
	...
	int result = std::swprintf(buffer, SpfcUtil::BUFFER_SIZE, _LS(IDS_WG_CMD__ADD_CH2019_PROFILE), m_nSDC + 1, strSFI[m_nSFI], strSC[m_nSC], strER[m_nER], m_dTg, m_dXi);
	if (result >= 0)
	{
		SetDescript(buffer);
	}
	SetFuncName(L"China(GB50011-19)");
}

void CSpfcCH2019Util::MakePOSpectrumData(double dDamping, double SRa, double SRv, CArray<double, double>& aPeriod, CArray<double, double>& aAccel)
{
	aPeriod.RemoveAll();
	aAccel.RemoveAll();
...
void CSpfcCH2019Util::SetParamFromCode(const T_SPFC_CODE& codeParam)
{
	m_nSDC = codeParam.CH2019.nSeisDgnCategory;
	m_nSFI = codeParam.CH2019.nSeisFortification;
	m_nSC = codeParam.CH2019.nSiteClass;
	m_nER = codeParam.CH2019.nEarthResponse;

	m_dTg = codeParam.CH2019.dTg;
	m_dXi = codeParam.CH2019.dDamping;
	m_nLForce = codeParam.CH2019.nLForce;
	m_dMaxEQE = codeParam.CH2019.dMaxEQE;
	m_dMaxPeriod = codeParam.CH2019.dMaxPeriod;
}

void CSpfcCH2019Util::SetParamFromCalc()
{
	// 지진영향계수 곡선 변수.
...
void CSpfcCH2019Util::CalcEarthquakeCoefFromPushOver()
{
	m_dGamma = 0.9 + (0.05 - m_dXi) / (0.3 + 6.0 * m_dXi); // 곡선하강영역의 감쇄계수.
	m_dEta1 = 0.02 + (0.05 - m_dXi) / (4.0 + 32.0 * m_dXi); // 직선하강영역의 경사조정계수.
	m_dEta2 = 1.0 + (0.05 - m_dXi) / (0.08 + 1.6 * m_dXi); // Damping Ratio 조정계수.
