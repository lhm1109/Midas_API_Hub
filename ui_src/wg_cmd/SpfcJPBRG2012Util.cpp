...
CSpfcJPBRG2012Util::CSpfcJPBRG2012Util(int nCode)
	: m_nCode(nCode)
{
	m_nMethod = 0;
	m_nSoil = 0;
...
bool CSpfcJPBRG2012Util::CheckValid()
{
...
void CSpfcJPBRG2012Util::MakeSpectrumData(bool bOnlyCalc)
{
	double st = PRD_ST;
	...
	GetSpectrumValue(dValue, dExp, m_nMethod, m_nCode);
	dT1 = dValue[0];  // 0.05
	dT2 = dValue[1];  // 0.1
	dT3 = dValue[2];  // 1.1
	dSm = dValue[3];  // 160.
	...
	CString strZone[] = { _LSX(A1), _LSX(A2), _LSX(B1), _LSX(B2), _LSX(C) };
	CString strSoil[] = { _LSX(I), _LSX(II), _LSX(III) };

	wchar_t buffer[SpfcUtil::BUFFER_SIZE];
	if (m_nCode == 0)
	{
		int result = swprintf(buffer, SpfcUtil::BUFFER_SIZE, __T("JP-Bridge2012: Method=%s, Zone=%s, Soil=%s, Damp=%4.2f"), strMethod[m_nMethod], strZone[m_nZone], strSoil[m_nSoil], m_dDamping);

		if (result >= 0)
			SetDescript(buffer);

		SetFuncName(L"JP-Bridge12");
	}
	else if (m_nCode == 1)
	{
		int result = swprintf(buffer, SpfcUtil::BUFFER_SIZE, _T("JP-Bridge2017: Method=%s, Zone=%s, Soil=%s, Damp=%4.2f"), strMethod[m_nMethod], strZone[m_nZone], strSoil[m_nSoil], m_dDamping);

		if (result >= 0)
			SetDescript(buffer);

		SetFuncName(L"JP-Bridge17");
	}
	else
	{
		ASSERT(false);
...
void CSpfcJPBRG2012Util::MakePOSpectrumData(double dDamping, double SRa, double SRv, CArray <double, double>& aPeriod, CArray <double, double>& aAccel)
{
...
void CSpfcJPBRG2012Util::SetParamFromCode(const T_SPFC_CODE& codeParam)
{
...
void CSpfcJPBRG2012Util::SetParamFromCalc()
{
	m_dCd = 1.5 / (40.0 * m_dDamping + 1.0) + 0.5;
}


void CSpfcJPBRG2012Util::GetSpectrumValue(double* dValue, double* dExp, int nMethod, int nCode)
{
	double dTable1[3][7] = { {0.05, 0.1 , 1.1, 160.,  431.,  200.,  220.}, // I
							{0.1,  0.2 , 1.3, 200.,  427.,  250.,  325.}, // II
							{0.17, 0.34, 1.5, 240.,  430.,  300.,  450.} };// III

	double dTable2[3][7] = { {0.0,  0.16, 0.6,   0., 2579., 1400.,  840.},
							{0.0,  0.22, 0.9,   0., 2153., 1300., 1170.},
							{0.0,  0.34, 1.4,   0., 1719., 1200., 1680.} };
