...
#define D_COL_ELEV 			3
#define D_COL_PRESS_X 	    4
#define D_COL_PRESS_Y 	    5
#define D_COL_LOADH 	    6
#define D_COL_LOADB_X 	    7
#define D_COL_LOADB_Y 	    8
#define D_COL_NUM 	        9

CCMWindItemUserGrid::CCMWindItemUserGrid()
{
	...
	AddColInfo(_LS(IDS_CMD_WINDP_PROFILE_WIND_PRESSURE), t_real, 0);
	AddColInfo(_LS(IDS_WG_CMD_DIR_X), t_real, 0);
	AddColInfo(_LS(IDS_WG_CMD_DIR_Y), t_real, 0);
	AddColInfo(_LS(IDS_CMD_WIND_LOAD_USER_LOADED_H), t_real, 0);
	AddColInfo(_LS(IDS_CMD_WIND_LOAD_USER_LOADED_B), t_real, 0);
	AddColInfo(_LS(IDS_WG_CMD_DIR_X), t_real, 0);
	AddColInfo(_LS(IDS_WG_CMD_DIR_Y), t_real, 0);
}
...
void CCMWindItemUserGrid::SetData(CArray<T_WIND_USER, T_WIND_USER&>& aUserD)
{
	...
	for (int i = 0; i < nNum; ++i)
	{
		...
		if (!m_pDoc->m_pAttrCtrl->GetStor(aUserD[i].KeyStor, StorD, bExistModl)) continue;
		...
		aValue[D_COL_PRESS_X - 1].Format(_T("%g"), aUserD[i].dPressX);
		aValue[D_COL_PRESS_Y - 1].Format(_T("%g"), aUserD[i].dPressY);
		aValue[D_COL_LOADH - 1].Format(_T("%g"), aUserD[i].dLoadH);
		aValue[D_COL_LOADB_X - 1].Format(_T("%g"), aUserD[i].dLoadBX);
		aValue[D_COL_LOADB_Y - 1].Format(_T("%g"), aUserD[i].dLoadBY);
...
BOOL CCMWindItemUserGrid::GetData(CArray<T_WIND_USER, T_WIND_USER&>& aUserD)
{
	aUserD.RemoveAll();
	...
	for (int i = 0; i < nDataCount; ++i)
	{
		...
		aUserD[i].dPressX = _tstof(aValue[D_COL_PRESS_X - 1]);
		aUserD[i].dPressY = _tstof(aValue[D_COL_PRESS_Y - 1]);
		aUserD[i].dLoadH = _tstof(aValue[D_COL_LOADH - 1]);
		aUserD[i].dLoadBX = _tstof(aValue[D_COL_LOADB_X - 1]);
		aUserD[i].dLoadBY = _tstof(aValue[D_COL_LOADB_Y - 1]);
	}
	return TRUE;
}

void CCMWindItemUserGrid::SetDataToRow(ROWCOL nRow, CStringArray& raValue)
{
...
BOOL CCMWindItemUserGrid::GetRowToData(ROWCOL nRow, CStringArray& raValue)
{
	int nColCount = GetColCount();
	ASSERT(nColCount == raValue.GetSize());
...
CString CCMWindItemUserGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString strNum(_T(""));
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CCMWindItemUserGrid, CTBCommon)
	//{{AFX_MSG_MAP(CCMWindItemUserGrid)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CCMWindItemUserGrid message handlers

void CCMWindItemUserGrid::Initialize(int nWidth)
{
	...
	SetCoveredCellsRowCol(0, D_COL_PRESS_X, 0, D_COL_PRESS_Y);
	SetCoveredCellsRowCol(0, D_COL_LOADH, 1, D_COL_LOADH);
	SetCoveredCellsRowCol(0, D_COL_LOADB_X, 0, D_COL_LOADB_Y);
	...
	*/
	...
	SetStyleRange(CGXRange().SetCols(D_COL_PRESS_X, D_COL_LOADB_Y), CGXStyle()
		.SetValueType(GX_VT_NUMERIC)
		.SetHorizontalAlignment(DT_RIGHT)
		.SetVerticalAlignment(DT_VCENTER)
		.SetPlaces(5)
	...
	aWidth[D_COL_STORY - 1] = bExistModl ? nWidth * 0.25 : nWidth * 0.15;
	aWidth[D_COL_ELEV - 1] = nWidth * 0.15;
	aWidth[D_COL_PRESS_X - 1] = nWidth * 0.18;
	aWidth[D_COL_PRESS_Y - 1] = nWidth * 0.18;
	aWidth[D_COL_LOADH - 1] = nWidth * 0.18;
	aWidth[D_COL_LOADB_X - 1] = nWidth * 0.18;
	aWidth[D_COL_LOADB_Y - 1] = nWidth * 0.18;
	...
	SetValueRange(CGXRange(0, D_COL_PRESS_X), _LS(IDS_CMD_WINDP_WINDPRESSURE));
	SetValueRange(CGXRange(1, D_COL_PRESS_X), _LS(IDS_WG_CMD_DIR_X));
	SetValueRange(CGXRange(1, D_COL_PRESS_Y), _LS(IDS_WG_CMD_DIR_Y));
	SetValueRange(CGXRange(0, D_COL_LOADH), _LS(IDS_CMD_WIND_LOAD_USER_LOADED_H));
	SetValueRange(CGXRange(0, D_COL_LOADB_X), _LS(IDS_CMD_WIND_LOAD_USER_LOADED_B));
	SetValueRange(CGXRange(1, D_COL_LOADB_X), _LS(IDS_WG_CMD_DIR_X));
	SetValueRange(CGXRange(1, D_COL_LOADB_Y), _LS(IDS_WG_CMD_DIR_Y));

	LockUpdate(FALSE);
	EnableAppendRow(FALSE);
}

BOOL CCMWindItemUserGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bCurrentRow;
	BOOL bCurrentCell;
	BOOL bEnabled;
...
BOOL CCMWindItemUserGrid::OnStartEditing(ROWCOL nRow, ROWCOL nCol)
{
	return CTBCommon::OnStartEditing(nRow, nCol);
}

void CCMWindItemUserGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
	CTBCommon::OnModifyCell(nRow, nCol);
}

BOOL CCMWindItemUserGrid::DoLButtonUp(UINT flags, CPoint point)
{
	BOOL bOldLock = LockUpdate(TRUE);
...
BOOL CCMWindItemUserGrid::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
	return CTBCommon::OnEndEditing(nRow, nCol);
}

BOOL CCMWindItemUserGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CCMWindItemUserGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMWindItemUserGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMWindItemUserGrid::CopyRange(const CGXRangeList& selList)
{
...
BOOL CCMWindItemUserGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CCMWindItemUserGrid::IsPasteToAllRange(const CGXRange &range, CString szData) // copy한 Cell이 1개 이고, 동일한 행의 cell 여러개로 붙여넣은 상황일 때 TRUE.. 이 경우엔 붙여넣을 Range의 모든 Cell에 복사한 Cell의 값을 붙여 넣어 줘야 함
{
...
void CCMWindItemUserGrid::SetEnableDisableCells(ROWCOL nRow)
{
	if (nRow <= 1) return;
	...
	SetStyleRange(CGXRange().SetCells(nRow, D_COL_LOADH), CGXStyle().SetReadOnly(TRUE));
	SetStyleRange(CGXRange().SetCells(nRow, D_COL_LOADB_X), CGXStyle().SetReadOnly(TRUE));
	SetStyleRange(CGXRange().SetCells(nRow, D_COL_LOADB_Y), CGXStyle().SetReadOnly(TRUE));
}

BOOL CCMWindItemUserGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CCMWindItemUserGrid::ValidateField(CString value, int nColID)
{
