...
CCMThrdWall::CCMThrdWall(CWnd* pParent /*=NULL*/)
	: CCMChildBarBase(CCMThrdWall::IDD)
{
	//{{AFX_DATA_INIT(CCMThrdWall)
	m_Name = _T("");
	m_KeyNdEl = _T("");
	m_ModNum = 0;
	m_Type = 0;
...
void CCMThrdWall::DoDataExchange(CDataExchange* pDX)
{
	CCMChildBarBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMThrdWall)
...
BEGIN_MESSAGE_MAP(CCMThrdWall, CCMChildBarBase)
	//{{AFX_MSG_MAP(CCMThrdWall)
...
/////////////////////////////////////////////////////////////////////////////
// CCMThrdWall message handlers

BOOL CCMThrdWall::PreTranslateMessage(MSG* pMsg)
{
...
void CCMThrdWall::OnCmdCancel()
{
	DataInit();
	((CCMThMainDlg*)(GetParent()->GetParent()))->SetThrdMain(0, 11);
}

void CCMThrdWall::OnCmdOK()
{
	if (OnExecute())
	{
		DataInit();
...
BOOL CCMThrdWall::Dlg2Data()
{
	UpdateData(TRUE);

	m_pDlgData.nGraphStep = 0;
	m_pDlgData.nFunctionType = 13;
...
void CCMThrdWall::OnGpsBarThType()
{
...
void CCMThrdWall::MakeMenuCombo(int nType)
{
...
void CCMThrdWall::OnGpsBarThmodnum()
{
	// TODO: Add your control notification handler code here
...
void CCMThrdWall::OnSelchangeGpsBarThcomp() 
{
	UpdateData(TRUE);	
...
void CCMThrdWall::OnSelchangeGpsBarThloadcase() 
{
	UpdateModeNumCtrl();
}

void CCMThrdWall::UpdateModeNumCtrl(void)
{
	UINT nLType,nLKey,nMnMxType;
	if(!m_cbxLComb.GetSelectedLoad(nLType, nLKey, &nMnMxType))
	{
...
int  CCMThrdWall::GetThisLdcType(UINT ThisK)
{
	CDBDoc *pDoc = CDBDoc::GetDocPoint();
...
void  CCMThrdWall::EnableModeNumCtrl(BOOL bEnable)
{
...
BOOL CCMThrdWall::ExternalInit(UINT key)
{
