...
CSpfcKBC2009Util::CSpfcKBC2009Util()
{
	m_dFa = 1.36;
	m_dFv = 1.96;
	m_dSds = 0.52668;
...
bool CSpfcKBC2009Util::CheckValid()
{
	CString strMsg;
	if (m_dFa <= 0.)
	{
...
void CSpfcKBC2009Util::MakeSpectrumData(bool bOnlyCalc)
{
	double st = PRD_ST;
	...
	int result = std::swprintf(buffer, SpfcUtil::BUFFER_SIZE, _LS(IDS_WG_CMD__ADD_KBC2009_PROFILE), m_nZone + 1, m_dZoneFactor, strSite[m_nSite], m_dFa, m_dFv, m_dSds, m_dSd1, m_dI, m_dR);
	...
	SetFuncName(_T("KBC2009"));
}

void CSpfcKBC2009Util::MakePOSpectrumData(double dDamping, double SRa, double SRv, CArray<double, double>& aPeriod, CArray<double, double>& aAccel)
{
	aPeriod.RemoveAll();
	aAccel.RemoveAll();
...
void CSpfcKBC2009Util::SetParamFromCode(const T_SPFC_CODE& codeParam)
{
	m_nZone = codeParam.KBC2009.nSeisZone;
	m_nSite = codeParam.KBC2009.nSiteClass;
	m_dZoneFactor = codeParam.KBC2009.dZoneFactor;
	m_dFa = codeParam.KBC2009.dFa;
	m_dFv = codeParam.KBC2009.dFv;
	m_dSds = codeParam.KBC2009.dSds;
	m_dSd1 = codeParam.KBC2009.dSd1;
	m_dI = codeParam.KBC2009.dIe;
	m_dR = codeParam.KBC2009.dCoef;
	m_dMaxPeriod = codeParam.KBC2009.dMaxPeriod;
}

void CSpfcKBC2009Util::SetParamFromCalc()
{
	m_dTs = m_dSd1 / m_dSds;
	m_dT0 = 0.2 * m_dTs;
}
