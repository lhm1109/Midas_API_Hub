...
BEGIN_MESSAGE_MAP(CButtonSTNew, CButton)
		//{{AFX_MSG_MAP(CButtonSTNew)
...
DWORD CButtonSTNew::SetIcon(int nIconInId, int nIconOutId)
{
	HICON		hIconIn;
	HICON		hIconOut;
...
DWORD CButtonSTNew::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
	...
	if (hIconIn != NULL)
	{
		...
		m_csIcons[0].dwWidth	= (DWORD)(ii.xHotspot * 1);
		m_csIcons[0].dwHeight	= (DWORD)(ii.yHotspot * 1);
		...
		if (hIconOut != NULL)
		{
			...
			m_csIcons[1].dwWidth	= (DWORD)(ii.xHotspot * 1);
			m_csIcons[1].dwHeight	= (DWORD)(ii.yHotspot * 1);
			::DeleteObject(ii.hbmMask);
			::DeleteObject(ii.hbmColor);
		} // if
	} // if
...
BOOL CButtonSTNew::SetBtnCursor(int nCursorId)
{
	HINSTANCE hInstResource;
...
void CButtonSTNew::SetFlat(BOOL bState)
{
	m_bIsFlat = bState;
	Invalidate();
} // End of SetFlat
...
BOOL CButtonSTNew::GetFlat()
{
	return m_bIsFlat;
} // End of GetFlat


void CButtonSTNew::SetAlign(int nAlign)
{
	switch (nAlign)
	{    
		case ST_ALIGN_HORIZ:
...
int CButtonSTNew::GetAlign()
{
	return m_nAlign;
} // End of GetAlign


void CButtonSTNew::DrawBorder(BOOL bEnable)
{
	m_bDrawBorder = bEnable;
} // End of DrawBorder

void CButtonSTNew::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CButtonSTNew::OnKillFocus(CWnd * pNewWnd)
{
...
void CButtonSTNew::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CButtonSTNew::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
...
void CButtonSTNew::OnEnable(BOOL bEnable) 
{
...
void CButtonSTNew::OnCancelMode() 
{
...
void CButtonSTNew::OnCaptureChanged(CWnd *pWnd) 
{
	if (m_bMouseOnButton == TRUE)
	{
		ReleaseCapture();
...
void CButtonSTNew::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
...
void CButtonSTNew::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CButtonSTNew::PreSubclassWindow() 
{
	UINT nBS;
...
BOOL CButtonSTNew::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
LRESULT CButtonSTNew::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
void CButtonSTNew::SetFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
BOOL CButtonSTNew::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CButtonSTNew::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
void CButtonSTNew::SetTooltipText(int nId, BOOL bActivate)
{
	CString sText;
...
void CButtonSTNew::ActivateTooltip(BOOL bActivate)
{
...
BOOL CButtonSTNew::GetDefault()
{
	return m_bIsDefault;
} // End of GetDefault


void CButtonSTNew::DrawTransparent(BOOL bRepaint)
{
...
void CButtonSTNew::PaintBk(CDC* pDC)
{
...
HBRUSH CButtonSTNew::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor


void CButtonSTNew::OnSysColorChange() 
{
...
void CButtonSTNew::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox == TRUE)
	{
...
int CButtonSTNew::GetCheck()
{
	return m_nCheck;
} // End of GetCheck

void CButtonSTNew::FreeResources(BOOL bCheckForNULL)
{
...
DWORD CButtonSTNew::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
DWORD CButtonSTNew::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
void CButtonSTNew::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
HBITMAP CButtonSTNew::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CButtonSTNew::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
	...
	switch (m_nAlign)
	{
		...
		case ST_ALIGN_VERT:
			// Center image horizontally
			{
				UINT width = rpImage->Width();
				UINT height = rpImage->Height();
			}
			rpImage->left += ((rpImage->Width() - dwWidth)/2);
			if (bHasTitle == FALSE /*spTitle->IsEmpty()*/)
			{
...
//
DWORD CButtonSTNew::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonSTNew::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonSTNew::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//			Function executed successfully.
//
DWORD CButtonSTNew::SetURL(LPCTSTR lpszURL)
{
	if (lpszURL != NULL)
	{
		// Store the URL
...
//			Function executed successfully.
//
DWORD CButtonSTNew::SetAutoRepeat(BOOL bSet, DWORD dwMilliseconds)
{
	m_bAutoRepeat = bSet;
...
//			Function executed successfully.
//
DWORD CButtonSTNew::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CButtonSTNew::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
