...
class CDBDoc;
class CCMWindItemUserGrid : public CTBCommon
{
public:
	CCMWindItemUserGrid();
	virtual ~CCMWindItemUserGrid();

public:
	void Initialize(int nWidth);
	void SetData(CArray<T_WIND_USER, T_WIND_USER&>& aUserD);
	BOOL GetData(CArray<T_WIND_USER, T_WIND_USER&>& aUserD);
