// CMThrdSolid.cpp : implementation file
//
...
CCMThrdSolid::CCMThrdSolid(CWnd* pParent /*=NULL*/)
	: CCMChildBarBase(CCMThrdSolid::IDD)
{
	//{{AFX_DATA_INIT(CCMThrdSolid)
	m_Name = _T("");
	m_KeyNdEl = _T("");
	m_ModNum = 0;
	m_Type = 0;
...
void CCMThrdSolid::DoDataExchange(CDataExchange* pDX)
{
	CCMChildBarBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMThrdSolid)
...
BEGIN_MESSAGE_MAP(CCMThrdSolid, CCMChildBarBase)
	//{{AFX_MSG_MAP(CCMThrdSolid)
...
/////////////////////////////////////////////////////////////////////////////
// CCMThrdSolid message handlers

BOOL CCMThrdSolid::PreTranslateMessage(MSG* pMsg)
{
...
void CCMThrdSolid::OnCmdCancel()
{
	DataInit();
	((CCMThMainDlg*)(GetParent()->GetParent()))->SetThrdMain(0, 6);
}

void CCMThrdSolid::OnCmdOK()
{
	if (OnExecute())
	{
		DataInit();
...
BOOL CCMThrdSolid::Dlg2Data()
{
	UpdateData(TRUE);

	m_pDlgData.nGraphStep = 0;
	m_pDlgData.nFunctionType = 7;
...
BOOL CCMThrdSolid::OnInitDialog()
{
...
void CCMThrdSolid::OnGpsBarThType()
{
...
void CCMThrdSolid::OnGpsBarThmodnum()
{
	// TODO: Add your control notification handler code here
...
void CCMThrdSolid::SetPosCompList(int nType,BOOL bDefault/*=FALSE*/)
{
	...
	if(nType == 0) 
	{
		...
		m_PointCb.AddString(_LS(IDS_WG_CMD_M_NODE));
		m_PointCb.AddString(_LS(IDS_WG_CMD_N_NODE));
		m_PointCb.AddString(_LS(IDS_WG_CMD_O_NODE));
		m_PointCb.AddString(_LS(IDS_WG_CMD_P_NODE));
	...
	else 
	{
		...
		m_CompCb.AddString(_LS(IDS_WG_GPS_LEGEND_Sig_yz));
		m_CompCb.AddString(_LS(IDS_WG_GPS_LEGEND_Sig_xz));
		if(bDefault)m_CompCb.SetCurSel(0);
	}
}

BOOL CCMThrdSolid::ExternalInit(UINT key)
{
