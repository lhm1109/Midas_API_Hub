...
/////////////////////////////////////////////////////////////////////////////
// CEtfcGrid
CString CEtfcGrid::m_aCurDefVal[] = {_T("0.0"), _T("0.0")};
CEtfcGrid::CEtfcGrid()
{
	m_pData = 0;

	AddColInfo(_LS(IDS_CMD_ETFC_time),        t_real, 0);
	AddColInfo(_LS(IDS_CMD_ETFC_temperature), t_real, 0);

//18japan	AddColInfo(_T("Time"),        t_real, 0);
//18japan  AddColInfo(_T("Temperature"), t_real, 0);
...
BEGIN_MESSAGE_MAP(CEtfcGrid, CTBCommon)
	//{{AFX_MSG_MAP(CEtfcGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CEtfcGrid message handlers

void CEtfcGrid::Initialize(T_ETFC_D *pData)
{
	ASSERT(pData);
	m_pData = pData;
...
BOOL CEtfcGrid::InsertRecordByKey(const double &key, const double &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CEtfcGrid::DeleteRecordByKey(const double &key, const double &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
/////////////////////////////////////////////////////////////////////////////
// CEtfcGrid Overridable Functions
void CEtfcGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	return;
}

long CEtfcGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
	return 0;
}

BOOL CEtfcGrid::ValidateField(CString value, int nColID)
{
	if (nColID >= GetColCount())
	{
		SetWarningText(_LS(IDS_WG_CMD__ADDD___CEtfcGrid__ValidateField__Error_));
		return FALSE;
	}
...
BOOL CEtfcGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CEtfcGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, double &key, double &data)
{
	/*
...
BOOL CEtfcGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CEtfcGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
...
BOOL CEtfcGrid::DeleteFromDB(ROWCOL nRow)
{
	double key;
	double data;
...
BOOL CEtfcGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CEtfcGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
	VERIFY(GetColumnNoFor(0, nCol));
...
BOOL CEtfcGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CEtfcGrid::GetNewKey()
{
	CString value(_T("0"));
	return value;
}

CString CEtfcGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CEtfcGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CEtfcGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CEtfcGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	double* pKey;
	double* pData;
...
long CEtfcGrid::GetCountData()
{
	return m_pData->aFuncData.GetSize();
}

void CEtfcGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0); // no filtering
}

void CEtfcGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	double Key;
	double Data;
...
void CEtfcGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	double Key;
	double Data;
...
/////////////////////////////////////////////////////////////////////////////
// CEtfcGrid Implementation
BOOL CEtfcGrid::GetValue(ROWCOL nRow, double &key, double &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CEtfcGrid::SetValue(ROWCOL nRow, const double &key, const double &data)
{
	//
	CRowColArray awCols;
...
BOOL CEtfcGrid::SetValue(CGXData* pData, ROWCOL nRow, const double &key, const double &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CEtfcGrid::ConvStrToData(int i, CString& value, double& data)
{
	if (i==1) data = _tstof(value);
}

void CEtfcGrid::ConvDataToStr(int i, const double& data, CString& value)
{
	if (i==1) value.Format(DATA_FMT, data);
}

void CEtfcGrid::MakeSearchKey(const double &key, const double &data, CStringArray &aKey)
{
	int nColID;
...
void CEtfcGrid::SetHeaderTitle(T_UNIT_SYSTEM &us, BOOL bInit)
{
	// Set Title
	int nDataType = m_pData->nFuncType;
	...
	int aUnit[][2] = {
		{CUnitCtrl::m_ETFC_UNIT.dTime, CUnitCtrl::m_ETFC_UNIT.dValue}, 
		{CUnitCtrl::m_ETFC_UNIT.dTime, CUnitCtrl::m_ETFC_UNIT.dValue}, 
		{CUnitCtrl::m_ETFC_UNIT.dTime, CUnitCtrl::m_ETFC_UNIT.dValue}, 
		{CUnitCtrl::m_ETFC_UNIT.dTime, CUnitCtrl::m_ETFC_UNIT.dValue},
	};
...
BOOL CEtfcGrid::FindData(double key, int& nPos, int nStart)
{
	int i;
...
BOOL CEtfcGrid::ExistAllKey(CArray<double, double>& aKey, double& errorKey)
{
...
BOOL CEtfcGrid::AddData(double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(data);
...
BOOL CEtfcGrid::AddData(CArray<double, double> &aKey, CArray<double, double> &aData, BOOL bNotify)
{
...
BOOL CEtfcGrid::DelData(double key)
{
	CArray<double, double> aOldKey, aKey, aData;
	aKey.Add(key);
	aData.Add(0.0);
...
BOOL CEtfcGrid::DelData(CArray<double, double> &aKey, BOOL bNotify)
{
...
BOOL CEtfcGrid::ModifyData(double oldKey, double key, double data)
{
	CArray<double, double> aOldKey, aKey, aData;
	aOldKey.Add(oldKey);
	aKey.Add(key);
...
BOOL CEtfcGrid::ModifyData(CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
	CArray<T_ETFC_BASE, T_ETFC_BASE&> aOrgData;
...
void CEtfcGrid::UpdateBuffer(int nCmd, CArray<double, double> &aOldKey, CArray<double, double> &aKey, CArray<double, double> &aData)
{
...
void CEtfcGrid::DBChanged()
{
	CEtfcItemDlg* pDlg = (CEtfcItemDlg*)GetParent();
	if (!pDlg) return;
