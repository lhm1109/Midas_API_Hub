...
CString CNXLoadStepGrid::m_aCurDefVal[] = { _T(""),_T(""), _T("0") };
CNXLoadStepGrid::CNXLoadStepGrid()
{
	m_nRowCount = 0;
	m_nOldRowCount = 0;
...
BEGIN_MESSAGE_MAP(CNXLoadStepGrid, CTBCommon)

END_MESSAGE_MAP()

void CNXLoadStepGrid::SetRowCountUser(int nRowCount)
{
	ASSERT(nRowCount >= 0);
...
void CNXLoadStepGrid::SetDefaultValue(ROWCOL nFromRow, ROWCOL nToRow)
{
...
void CNXLoadStepGrid::GenerateValue(double dValue)
{
	T_NLCT_LOADSTEP LoadStep;
...
BOOL CNXLoadStepGrid::DeleteRecordByKey(const int &key, const T_NLCT_LOADSTEP &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
BOOL CNXLoadStepGrid::InsertRecordByKey(const int &key, const T_NLCT_LOADSTEP &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
void CNXLoadStepGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	return;
}

long CNXLoadStepGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
	return 0;
}

BOOL CNXLoadStepGrid::AddToDB(ROWCOL nRow)
{
	int key;
...
BOOL CNXLoadStepGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
...
BOOL CNXLoadStepGrid::DeleteFromDB(ROWCOL nRow)
{
	int key;
	T_NLCT_LOADSTEP data;
	GetValue(nRow, key, data);
...
BOOL CNXLoadStepGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CNXLoadStepGrid::ModifyToDB(ROWCOL nRow)
{
...
BOOL CNXLoadStepGrid::ModifyToDB(CStringArray& raOldKey, CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
	ASSERT(raKey.GetSize() == raData.GetSize());
...
CString CNXLoadStepGrid::GetNewKey()
{
	CString value(_T(""));
	return value;
}

CString CNXLoadStepGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CNXLoadStepGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CNXLoadStepGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	int* pKey;
...
void CNXLoadStepGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	int* pKey;
	T_NLCT_LOADSTEP* pData;
...
long CNXLoadStepGrid::GetCountData()
{
	return m_pData->aLoadSteps.GetCount();
}

void CNXLoadStepGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0); //no Filtering
}

void CNXLoadStepGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	int Key;
	T_NLCT_LOADSTEP Data;
...
void CNXLoadStepGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
...
BOOL CNXLoadStepGrid::OnPasteFromClipboard(const CGXRange &range)
{
	if (IsLockEdit()) return FALSE; // display only mode이면 바로 리턴
	return CTBCommon::OnPasteFromClipboard(range);
}

BOOL CNXLoadStepGrid::Copy()
{
	return CTBCommon::Copy();
}

BOOL CNXLoadStepGrid::OnInsertEmptyRecord(ROWCOL nRow)
{
	BOOL bActive;
...
void CNXLoadStepGrid::Initialize(T_ACTL_D* pData)
{
	ASSERT(pData);
	m_pData = pData;
...
BOOL CNXLoadStepGrid::GetValue(ROWCOL nRow, int &key, T_NLCT_LOADSTEP &data)
{
	data.Initialize();
	ROWCOL nCol;
	CString str;
...
BOOL CNXLoadStepGrid::SetValue(ROWCOL nRow, const int &key, const T_NLCT_LOADSTEP &data)
{
	CRowColArray awCols;
...
BOOL CNXLoadStepGrid::SetValue(CGXData* pData, ROWCOL nRow, const int &key, const T_NLCT_LOADSTEP &data, CRowColArray &awCols)
{
	CString str;
	for (int i = 0; i < GetColCount(); i++)
	{
...
void CNXLoadStepGrid::ConvStrToData(int i, CString& value, T_NLCT_LOADSTEP& data)
{
	if (i == 1)data.dLoadIncFactor = _tstof(value);
	if (i == 2)data.bSave = _ttoi(value) == 0 ? FALSE : TRUE;
}

void CNXLoadStepGrid::ConvDataToStr(int i, const T_NLCT_LOADSTEP& data, CString& value)
{
	if (i == 1) value.Format(DATA_FMT, data.dLoadIncFactor);
	if (i == 2)value.Format(_T("%d"), data.bSave == TRUE ? 1 : 0);
}

BOOL CNXLoadStepGrid::AddData(int key, T_NLCT_LOADSTEP data)
{
...
BOOL CNXLoadStepGrid::AddData(CArray<int, int> &aKey, CArray<T_NLCT_LOADSTEP, T_NLCT_LOADSTEP&> &aData, BOOL bNotify /*= TRUE*/)
{
...
BOOL CNXLoadStepGrid::DelData(int key)
{
	CArray<int, int> aOldKey, aKey;
	CArray<T_NLCT_LOADSTEP, T_NLCT_LOADSTEP&> aData;
	aKey.Add(key);
...
BOOL CNXLoadStepGrid::DelData(CArray<int, int> &aKey, BOOL bNotify /*= TRUE*/)
{
	CArray<int, int> aOldKey;
	CArray<T_NLCT_LOADSTEP, T_NLCT_LOADSTEP&> aData;
	int nSize = aKey.GetSize();
...
BOOL CNXLoadStepGrid::ModifyData(int oldKey, int key, T_NLCT_LOADSTEP data)
{
	CArray<int, int> aOldKey, aKey;
	CArray<T_NLCT_LOADSTEP, T_NLCT_LOADSTEP&> aData;
	aOldKey.Add(oldKey);
...
BOOL CNXLoadStepGrid::ModifyData(CArray<int, int> &aOldKey, CArray<int, int> &aKey, CArray<T_NLCT_LOADSTEP, T_NLCT_LOADSTEP&> &aData)
{
...
void CNXLoadStepGrid::MakeSearchKey(const int &key, const T_NLCT_LOADSTEP &data, CStringArray &aKey)
{
	int nColID;
...
BOOL CNXLoadStepGrid::ExistAllKey(CArray<int, int>& aKey, int& errorKey)
{
...
BOOL CNXLoadStepGrid::FindData(int key, int& nPos, int nStart/*=0*/)
{
	int i;
...
void CNXLoadStepGrid::SetHeaderTitle(T_UNIT_SYSTEM &UnitSystem, BOOL bInit /*= FALSE*/)
{
	int aSize[] = { 50, 75, 65 };
	CString strTitle;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CNXLoadStepGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CNXLoadStepGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray &aCols, int &key, T_NLCT_LOADSTEP &data)
{
...
BOOL CNXLoadStepGrid::ValidateField(CString value, int nColID)
{
	if (nColID >= GetColCount())
	{
		SetWarningText(_LS(IDS_WG_CMD__ADDD___CNSLoadStepGrid__ValidateField__Er));
		return FALSE;
	}
...
void CNXLoadStepGrid::UpdateBuffer(int nCmd, CArray<int, int> &aOldKey, CArray<int, int> &aKey, CArray<T_NLCT_LOADSTEP, T_NLCT_LOADSTEP&> &aData)
{
...
void CNXLoadStepGrid::DBChanged()
{
	CMNXLoadStepDlg* pDlg = (CMNXLoadStepDlg*)GetParent();
	if (!pDlg)return;
