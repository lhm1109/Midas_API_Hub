// WindProfileIS875GUSTDlg.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CWindProfileIS875GUSTDlg dialog
...
CWindProfileIS875GUSTDlg::CWindProfileIS875GUSTDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CWindProfileIS875GUSTDlg::IDD, pParent), m_LateralLoad(0, TRUE)
{
	//{{AFX_DATA_INIT(CWindProfileIS875GUSTDlg)
	m_nComponent = 0;
	m_nProfileType = 0;
	m_strFileName = _T("");
	//}}AFX_DATA_INIT
...
void CWindProfileIS875GUSTDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CWindProfileIS875GUSTDlg)
...
BEGIN_MESSAGE_MAP(CWindProfileIS875GUSTDlg, CDialogMove)
	//{{AFX_MSG_MAP(CWindProfileIS875GUSTDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CWindProfileIS875GUSTDlg message handlers


void CWindProfileIS875GUSTDlg::InitGrid(int nCode)
{
...

	{
		...
		m_WindPfGrid.SetStyleRange(CGXRange(0, 5), CGXStyle().SetValue(_LS(IDS_WG_CMD__ADDD__Gust_n_Factor)));
		m_WindPfGrid.SetStyleRange(CGXRange(0, 6), CGXStyle().SetValue(_LS(IDS_WG_CMD__ADD2__Wind_Force)));
		m_WindPfGrid.SetStyleRange(CGXRange(0, 7), CGXStyle().SetValue(_LS(IDS_WG_CMD__ADDD__Added_n_Force)));
		m_WindPfGrid.SetStyleRange(CGXRange(0, 8), CGXStyle().SetValue(_LS(IDS_WG_CMD__ADDD__Story_n_Force)));
		m_WindPfGrid.SetStyleRange(CGXRange(0, 9), CGXStyle().SetValue(_LS(IDS_WG_CMD__ADDD__Story_n_Shear)));
		m_WindPfGrid.SetStyleRange(CGXRange(0, 10), CGXStyle().SetValue(_LS(IDS_WG_CMD__ADD2__Overturn_g_nMoment)));
	}

	m_WindPfGrid.SetStyleRange(CGXRange().SetCols(0, 10), CGXStyle()
		.SetWrapText(TRUE)
		.SetEnabled(FALSE));
...
BOOL CWindProfileIS875GUSTDlg::OnInitDialog()
{
...
BOOL CWindProfileIS875GUSTDlg::PreTranslateMessage(MSG* pMsg)
{
...
/*
void CWindProfileIS875GUSTDlg::OnDataChanged()
{
...
void CWindProfileIS875GUSTDlg::InitChart()
{
	////////////////////////////////////////////////////////////////
	// Initialize m_Graph
...
void CWindProfileIS875GUSTDlg::SetValue(int nIndex, int nGroup, double x, double y)
{
...
void CWindProfileIS875GUSTDlg::SetAxisLabelText(int nComponent, LPCTSTR lbl)
{
	SRGraphLabel* pLabel = (SRGraphLabel*)m_Graph.GetComponent(nComponent, IDS_SRG_LABELTYPE);
	pLabel->SetAnnotation(lbl);
}
/*
void CWindProfileIS875GUSTDlg::OnETCSPFCCHARTXLog()
{
...
void CWindProfileIS875GUSTDlg::OnETCSPFCCHARTYLog()
{
	SRGraphDisplay	*pD=(SRGraphDisplay *) m_Graph.GetComponent(0, IDS_SRG_DISPLAYTYPE);
...
*/
void CWindProfileIS875GUSTDlg::InitWindProfile(T_WIND_K nKey, T_WIND_D& rDataWind, T_WNAT_D* pWnatD/*=NULL*/)
{
	m_Key = nKey;
...
void CWindProfileIS875GUSTDlg::OnChangeCompont()
{
	UpdateData(TRUE);
	UpdateGrid();
	UpdateChart();
...
void CWindProfileIS875GUSTDlg::OnChangeProfileType()
{
	UpdateData(TRUE);
	UpdateGrid();
	UpdateChart();
...
void CWindProfileIS875GUSTDlg::UpdateGrid()
{
	stWindLoad windload, windload_x, windload_y;
	CString sLevel, sPressure, sHeight, sBreadth, sForce, sAddedFrc, sWindLoad, sStoryShear, sOTMoment, sGustFact;
	...
	for (int i = 0; i < m_LateralLoad.m_aWindLoad_X.GetSize(); i++)
	{
		...
		case 0: // X
			...
			sGustFact = ConvertValToFmtStr(windload_x.dGustFactIS875, 10);
			break;
		case 1: // Y
			sBreadth = ConvertValToFmtStr(windload.dBreadth, 10);
			sForce = ConvertValToFmtStr(windload.dForce, 10);
			...
			sStoryShear = ConvertValToFmtStr(windload.dStoryShear, 10);
			sOTMoment = ConvertValToFmtStr(windload.dOTMoment, 10);
			sGustFact = ConvertValToFmtStr(windload_y.dGustFactIS875, 10);
			break;
		case 2: // XY
			sBreadth = _T("  --  ");
			sForce = _T("   --   ");
		...
		case 3: // SRSS
			...
			sStoryShear = ConvertValToFmtStr(windload.dStoryShear, 10);
			sOTMoment = ConvertValToFmtStr(windload.dOTMoment, 10);
			sGustFact = ConvertValToFmtStr(sqrt(windload_x.dGustFactIS875 * windload_x.dGustFactIS875
				+ windload_y.dGustFactIS875 * windload_y.dGustFactIS875), 10);
			//sGustFact = ConvertValToFmtStr(windload.dGustFactIS875, 10);
			break;
		}
		...
		m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 5), CGXStyle().SetValue(sGustFact));
		m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 6), CGXStyle().SetValue(sForce));
		m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 7), CGXStyle().SetValue(sAddedFrc));
		m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 8), CGXStyle().SetValue(sWindLoad));
		m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 9), CGXStyle().SetValue(sStoryShear));
		m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 10), CGXStyle().SetValue(sOTMoment));
		...
		if (i == m_LateralLoad.m_aWindLoad_X.GetSize() - 1)
		{
			...
			m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 6), CGXStyle().SetValue(sForce));
			m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 7), CGXStyle().SetValue(sAddedFrc));
			m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 8), CGXStyle().SetValue(_T("   --   ")));
			m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 9), CGXStyle().SetValue(sStoryShear));
			m_WindPfGrid.SetStyleRange(CGXRange(i + 1, 10), CGXStyle().SetValue(sOTMoment));
		}
	}
...
void CWindProfileIS875GUSTDlg::UpdateChart()
{
	stWindLoad windload, windload_x, windload_y;
	m_Graph.KillData();
	double dValue;
...
void CWindProfileIS875GUSTDlg::OnBtnDyGen()
{
	T_UCHT_D UchtD;
...
CString CWindProfileIS875GUSTDlg::ConvertValToFmtStr(double Val, int nLimit)
{
...
void CWindProfileIS875GUSTDlg::OnWINDPFViewCalcSheet()
{
	UpdateData(TRUE);
...
void CWindProfileIS875GUSTDlg::OnWindPFBrowse()
{
