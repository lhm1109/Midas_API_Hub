// SeisChinaCalcDlg.cpp : implementation file
//
...
CSeisChinaCalcDlg::CSeisChinaCalcDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CSeisChinaCalcDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CSeisChinaCalcDlg)
	m_nXMethod = 0;
	m_nYMethod = 0;
	//}}AFX_DATA_INIT
	m_nNX = 0;
...
void CSeisChinaCalcDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CSeisChinaCalcDlg)
...
BEGIN_MESSAGE_MAP(CSeisChinaCalcDlg, CDialogMove)
	//{{AFX_MSG_MAP(CSeisChinaCalcDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CSeisChinaCalcDlg message handlers

BOOL CSeisChinaCalcDlg::OnInitDialog() 
{
	...
	m_wndBX.EnableWindow(m_nXMethod==0 || m_nXMethod==1 || m_nXMethod==2);	
	m_wndNX.EnableWindow(m_nXMethod==3);	
	m_wndHY.EnableWindow(m_nYMethod==0 || m_nYMethod==1 || m_nYMethod==2);	
	m_wndBY.EnableWindow(m_nYMethod==0 || m_nYMethod==1 || m_nYMethod==2);	
	m_wndNY.EnableWindow(m_nYMethod==3);	
...
void CSeisChinaCalcDlg::OnRdoXmethod() 
{
	UpdateData(TRUE);
	m_wndHX.EnableWindow(m_nXMethod==0 || m_nXMethod==1 || m_nXMethod==2);	
	m_wndBX.EnableWindow(m_nXMethod==0 || m_nXMethod==1 || m_nXMethod==2);	
...
void CSeisChinaCalcDlg::OnRdoYmethod() 
{
	UpdateData(TRUE);
	m_wndHY.EnableWindow(m_nYMethod==0 || m_nYMethod==1 || m_nYMethod==2);	
	m_wndBY.EnableWindow(m_nYMethod==0 || m_nYMethod==1 || m_nYMethod==2);	
...
BOOL CSeisChinaCalcDlg::CalcPeriod(double dH, double dB, int nN, int nMethod, double& dPeriod)
{
	...
	switch(nMethod)
	{
		case 0: dPeriod = 0.22 + 0.035 * dH / pow(dB, (1.0/3.0));	break;
		case 1: dPeriod = 0.25 + 0.00053 * dH * dH / pow(dB, (1.0/3.0));	break;
		case 2: dPeriod = 0.03 + 0.03 * dH / pow(dB, (1.0/3.0));	break;
		case 3: dPeriod = 0.1 * nN;	break;		
	}
	return TRUE;
}

void CSeisChinaCalcDlg::OnOK() 
{
	UpdateData(TRUE);
