...
CCMWindPressureProfileDlg::CCMWindPressureProfileDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CCMWindPressureProfileDlg::IDD, pParent), m_LateralLoad(0, TRUE)
{
	m_strFileName = _T("");
	m_bGridInit = FALSE;
}
...
void CCMWindPressureProfileDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CCMWindPressureProfileDlg, CDialogMove)
...
/////////////////////////////////////////////////////////////////////////////
// CCMWindPressureProfileDlg message handlers

BOOL CCMWindPressureProfileDlg::OnInitDialog() 
{  
...
// BOOL CCMWindPressureProfileDlg::PreTranslateMessage(MSG* pMsg) 
// {
...
void CCMWindPressureProfileDlg::InitChart()
{
	////////////////////////////////////////////////////////////////
	// Initialize m_Graph
...
void CCMWindPressureProfileDlg::SetValue(int nIndex, int nGroup, double x,double y)
{
...
void CCMWindPressureProfileDlg::SetAxisLabelText(int nComponent, LPCTSTR lbl)
{
	SRGraphLabel *pLabel=(SRGraphLabel *) m_Graph.GetComponent(nComponent, IDS_SRG_LABELTYPE);
	pLabel->SetAnnotation(lbl);
}
/*
void CCMWindPressureProfileDlg::OnETCSPFCCHARTXLog() 
{
...
void CCMWindPressureProfileDlg::OnETCSPFCCHARTYLog() 
{
	SRGraphDisplay	*pD=(SRGraphDisplay *) m_Graph.GetComponent(0, IDS_SRG_DISPLAYTYPE);
...
*/
void CCMWindPressureProfileDlg::InitWindProfile(T_WDPR_D &rDataWind, double* pWindDir)
{
	m_LateralLoad.MakeWindPressure(rDataWind, FALSE, _T(""), TRUE, FALSE, pWindDir);
	m_pWdprD = &rDataWind;
}

void CCMWindPressureProfileDlg::OnBtnDyGen()
{  
//   T_UCHT_D UchtD;
...
CString CCMWindPressureProfileDlg::ConvertValToFmtStr(double Val,int nLimit)
{
...
void CCMWindPressureProfileDlg::OnWINDPFViewCalcSheet() 
{
	UpdateData(TRUE);
...
void CCMWindPressureProfileDlg::OnWindPFBrowse() 
{
...
void CCMWindPressureProfileDlg::UpdateChart()
{
	int i, nSize;
	...
	double dMinPres = m_pWdprD->arPresProfile[0].dPressure + m_pWdprD->arPresProfile[0].dAdditionalPressure;
	double dMaxPres = m_pWdprD->arPresProfile[0].dPressure + m_pWdprD->arPresProfile[0].dAdditionalPressure;
	
	for(i = 0 ; i < nSize; i++)
	{
		double dLevel = m_pWdprD->arPresProfile[i].dLevel;
		double dPressure = m_pWdprD->arPresProfile[i].dPressure + m_pWdprD->arPresProfile[i].dAdditionalPressure;
...
void CCMWindPressureProfileDlg::InitGrid()
{
...
void CCMWindPressureProfileDlg::OnDataChanged()
{
...
void CCMWindPressureProfileDlg::UpdateProfileData()
{
	UpdateChart();
}