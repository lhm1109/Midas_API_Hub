...
//-------------------------------------------
int CDrawArrowLine::GetClassType()
{
	return EN_DRCLASS_TYPE_ARROWLINE;
}

int CDrawArrowLine::GetFormatType()
{
	return EN_DRFORMAT_TYPE_DRAW;
}

void CDrawArrowLine::Draw(HDC DC, CDrawFunc* pDrawFunc, void* pDrawFormat)
{
	T_DRAW_FORMAT DrawFormat = *((T_DRAW_FORMAT*)pDrawFormat);
	pDrawFunc->DrawArrow(DC, DrawFormat, m_Data.List); 
}

void CDrawArrowLine::Convert(BOOL bCDC, CDrawFunc* pDrawFunc, T_PAPER_FORMAT* pPaperFormat, void* pDrawFormat)
{
	if(bCDC)
	{
...
void CDrawArrowLine::GetCalcBoundaryR(double Boundary[4])
{
...
void CDrawArrowLine::GetCalcBoundary(int Boundary[4])
{
...
void CDrawArrowLine::Move(BOOL bCDC, double Vector[2], T_PAPER_FORMAT* pPaperFormat)
{
	double disVectorR[2];
	int    disVector[2];
...
BOOL CDrawArrowLine::IsSelectCheck(POINT sPoint, BOOL bFill, int iTol)
{
	...
	{
		T_LINE* itr = &m_Data.List[i];
		if(IsPointOfLine(itr->P1, itr->P2, sPoint, iTol, TRUE)) return TRUE;
	}
	return FALSE;
}

BOOL CDrawArrowLine::IsSnapCheck(POINT currPoint, POINT& snapPoint, T_POINT& snapPointR)
{
...
BOOL CDrawArrowLine::GetCenterPoint(int nID, POINT& CenterP)
{
