...
CCMMvhlItemStdFRNominalGrid::CCMMvhlItemStdFRNominalGrid()
{
	m_pMvhl = 0;
	m_bInitDone = FALSE;
	m_pParent = 0;
...
void CCMMvhlItemStdFRNominalGrid::InitGrid()
{
...
void CCMMvhlItemStdFRNominalGrid::SetStyleOnSizeCell()
{
	ASSERT(0);
// 	int i;
// 	int count = 4;
...
void CCMMvhlItemStdFRNominalGrid::SetDataSource(T_MVHL_D* pMvhl, CWnd* pParent, BOOL bStd)
{
	ASSERT(pMvhl);
	m_pMvhl   = pMvhl;
	m_pParent = pParent;
	...
	if (m_bStd)
	{
		...
		SetColWidth(D_COL_V0    , D_COL_V0    , globalUtils.ScaleByDPI(50));
	}
	else
	{
		ASSERT(0);
	...
	pDoc->m_pUnitCtrl->GetUnitSystem(CUnitCtrl::m_MVHL_UNIT.dNominalWidth, strUnit);
	strTemp.Format(_T("%s (%s)"), _LS(IDS_CMD_MVHL_NOMINAL_WIDTH), strUnit);  // Nominal Width
	...
	SetStyleRange(CGXRange(1, 1), CGXStyle()
		...
		.SetVerticalAlignment(DT_VCENTER)
		.SetValue(strTemp));  // Nominal Width

	SetStyleRange(CGXRange(1, D_COL_V0), CGXStyle()
		.SetInterior(::GetSysColor(COLOR_BTNFACE))
		.SetEnabled(FALSE)
		.SetHorizontalAlignment(DT_CENTER)
		.SetVerticalAlignment(DT_VCENTER)
	...
	SetStyleRange(CGXRange().SetCols(D_COL_V0, D_COL_V0), CGXStyle()
	...
	{
		// 전체 데이터 비활성화
		SetStyleRange(CGXRange(2, D_COL_V0, 4, D_COL_V0), CGXStyle()
			.SetReadOnly(FALSE)
			.SetEnabled(FALSE) 
			.SetHorizontalAlignment(DT_RIGHT)
			.SetVerticalAlignment(DT_VCENTER)
...
void CCMMvhlItemStdFRNominalGrid::ShowData()
{
	int count = 3;
	int nCol  = 1;
	CString str;
	...
	{
		str.Format(_T("%g"), m_pMvhl->dNominalWidth[i]);
		SetValueRange(CGXRange(2 + i, D_COL_V0), str);
	}
}
BOOL CCMMvhlItemStdFRNominalGrid::SaveData()
{
	CString csNum;
	double dNum;
	...
	{
		csNum = GetEditingValue(2 + i, D_COL_V0);
		if (GetDoubleValue(csNum, dNum))
			m_pMvhl->dNominalWidth[i] = dNum;
		else
			m_pMvhl->dNominalWidth[i] = 0.0;
	}
	return TRUE;
}

BOOL CCMMvhlItemStdFRNominalGrid::SetDownValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
	i = nRow + 1;
	j = nCol;
...
BOOL CCMMvhlItemStdFRNominalGrid::SetUpValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
	i = nRow - 1;
	j = nCol;
...
CString CCMMvhlItemStdFRNominalGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString csNum;
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CCMMvhlItemStdFRNominalGrid, CGXGridWnd)
	//{{AFX_MSG_MAP(CCMMvhlItemStdFRNominalGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CCMMvhlItemStdFRNominalGrid message handlers
/////////////////////////////////////////////////////////////////////////////

BOOL CCMMvhlItemStdFRNominalGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CCMMvhlItemStdFRNominalGrid::OnValidateCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMMvhlItemStdFRNominalGrid::OnActivateGrid(BOOL bActivate)
{
...
void CCMMvhlItemStdFRNominalGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMMvhlItemStdFRNominalGrid::GetDoubleValue(CString& csNum, double& dNum)
{
	int ecode;
...
BOOL CCMMvhlItemStdFRNominalGrid::IsValidDoubleNumber(CString value, int &nErrCode)
{
	value.TrimLeft();
	value.TrimRight();
...
BOOL CCMMvhlItemStdFRNominalGrid::ConvertFormulaToValue(CString& value)
{
...
BOOL CCMMvhlItemStdFRNominalGrid::IsNumeric(CString str, int& ecode)
{
...
BOOL CCMMvhlItemStdFRNominalGrid::InvalidDoubleNumber(int nErrCode)
{
	switch (nErrCode)
	{
...
BOOL CCMMvhlItemStdFRNominalGrid::ValidateError(LPCTSTR lpszError)
{
	SetWarningText(lpszError);
	return FALSE;
}

BOOL CCMMvhlItemStdFRNominalGrid::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMMvhlItemStdFRNominalGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
	ROWCOL i, j;
...
BOOL CCMMvhlItemStdFRNominalGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CCMMvhlItemStdFRNominalGrid::CopyRange(const CGXRangeList& selList)
{
	...
	while (TRUE)
	{
		...
		if (m_bStd)
		{
			if (mRange->IntersectRange(CGXRange(D_SIZE_START_ROW, D_COL_V0, D_SIZE_END_ROW, D_COL_V0), selList.GetAt(position)))
			{
...
BOOL CCMMvhlItemStdFRNominalGrid::OnPasteFromClipboard(const CGXRange &range)
{
	CGXGridWnd x;
	CGXRange r;
	CGXRange toRange;
...
BOOL CCMMvhlItemStdFRNominalGrid::VerifyPasteRange(CGXRange& from, CGXRange& to)
{
...
int CCMMvhlItemStdFRNominalGrid::GetSelectedRowsCount(int top, int bottom)
{
	int toRows;
	toRows = bottom - top + 1;
	return toRows;
...
int CCMMvhlItemStdFRNominalGrid::GetSelectedColsCount(int left, int right)
{
	int toCols;
	toCols = 0;
// 	if (m_bStd)
