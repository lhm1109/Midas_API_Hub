// CMThrdPlate.cpp : implementation file
//
...
CCMThrdPlate::CCMThrdPlate(CWnd* pParent /*=NULL*/)
	: CCMChildBarBase(CCMThrdPlate::IDD)
{
	//{{AFX_DATA_INIT(CCMThrdPlate)
	m_Name = _T("");
	m_KeyNdEl = _T("");
	m_ModNum = 0;
	m_Type = 0;
...
void CCMThrdPlate::DoDataExchange(CDataExchange* pDX)
{
	CCMChildBarBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMThrdPlate)
...
BEGIN_MESSAGE_MAP(CCMThrdPlate, CCMChildBarBase)
	//{{AFX_MSG_MAP(CCMThrdPlate)
	ON_BN_CLICKED(IDC_CMD_BAR_THTYPE_FORCE,		OnGpsBarThType)
	ON_BN_CLICKED(IDC_CMD_BAR_THTYPE_STRESS,	OnGpsBarThType)
	ON_BN_CLICKED(IDC_CMD_BAR_THTYPE_UNITFORCE,	OnGpsBarThType)
...
/////////////////////////////////////////////////////////////////////////////
// CCMThrdPlate message handlers

BOOL CCMThrdPlate::PreTranslateMessage(MSG* pMsg)
{
...
void CCMThrdPlate::OnCmdCancel()
{
	DataInit();
	((CCMThMainDlg*)(GetParent()->GetParent()))->SetThrdMain(0, 5);
}

void CCMThrdPlate::OnCmdOK()
{
	if (OnExecute())
	{
		DataInit();
...
BOOL CCMThrdPlate::Dlg2Data()
{
	UpdateData(TRUE);

	m_pDlgData.nGraphStep = 0;
	m_pDlgData.nFunctionType = 6;
...
BOOL CCMThrdPlate::OnInitDialog()
{
...
void CCMThrdPlate::OnGpsBarThType()
{
	int cur_type=m_Type;
	UpdateData(TRUE);
...
void CCMThrdPlate::SetPosCompList(int nType,BOOL bDefault/*=FALSE*/)
{
	...
	case 0:
		...
		m_CompCb.AddString(_T("Mx"));
		m_CompCb.AddString(_T("My"));
		m_CompCb.AddString(_T("Mz"));
	...
	case 1:
		...
		m_CompCb.AddString(_T("Sig-xx(Top)"));
		m_CompCb.AddString(_T("Sig-yy(Top)"));
		m_CompCb.AddString(_T("Sig-xy(Top)"));
		m_CompCb.AddString(_T("Sig-xx(Bottom)"));
		m_CompCb.AddString(_T("Sig-yy(Bottom)"));
		m_CompCb.AddString(_T("Sig-xy(Bottom)"));
		if(bDefault)m_CompCb.SetCurSel(0);
		break;
	case 2:
		m_PointCb.AddString(_LS(IDS_WG_CMD_CENTER));
		...
		m_CompCb.AddString(_T("Fxx"));
		m_CompCb.AddString(_T("Fyy"));
		m_CompCb.AddString(_T("Fxy"));
		m_CompCb.AddString(_T("Mxx"));
		m_CompCb.AddString(_T("Myy"));
		m_CompCb.AddString(_T("Mxy"));
		m_CompCb.AddString(_T("Vxx"));
		m_CompCb.AddString(_T("Vyy"));
		if(bDefault)m_CompCb.SetCurSel(0);
		break;
	}
}

void CCMThrdPlate::OnGpsBarThmodnum()
{
...
BOOL CCMThrdPlate::ExternalInit(UINT key)
{
