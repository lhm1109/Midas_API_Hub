// ETC_GDS_KBC2015.cpp : implementation file
//
...
CETC_GDS_KBC2015::CETC_GDS_KBC2015(T_SPFC_D* pParamData, CWnd* pParent /*=NULL*/)
	: CMyChildDialog(CETC_GDS_KBC2015::IDD, pParent, std::make_shared<CSpfcKBC2016Util>())
{
	//{{AFX_DATA_INIT(CETC_GDS_KBC2015)
	//}}AFX_DATA_INIT
	m_pParamData = pParamData;
	m_pSpfcUtil = std::dynamic_pointer_cast<CSpfcKBC2016Util>(m_pSpfcBaseUtil);
	SetInitValueforPOSpectrum();
}

// Pushover Spectrum은 초기값 설정전에 호출되기 때문에 생성자에서 아래 함수를 반드시 먼저 불러야 한다.
void CETC_GDS_KBC2015::SetInitValueforPOSpectrum()
{
	//m_dDepthToMR = 0.;
	//m_dFa  = 1.46;
	//m_dFv  = 1.58;
...
void CETC_GDS_KBC2015::DoDataExchange(CDataExchange* pDX)
{
	CMyChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CETC_GDS_KBC2015)
...
BEGIN_MESSAGE_MAP(CETC_GDS_KBC2015, CMyChildDialog)
	//{{AFX_MSG_MAP(CETC_GDS_KBC2015)
	...
	ON_EN_CHANGE(IDC_CMD_DEPTH_EDIT,   OnChangeCmdDepthToMREdit)
...
/////////////////////////////////////////////////////////////////////////////
// CETC_GDS_KBC2015 message handlers

BOOL CETC_GDS_KBC2015::OnInitDialog() 
{
	CMyChildDialog::OnInitDialog();

	m_unitDepthToMR.SetUnitType(CUnitCtrl::m_SEIS_UNIT.Depth2MR);
	m_editDepthToMR.SetUnitType(CUnitCtrl::m_SEIS_UNIT.Depth2MR);
...
void CETC_GDS_KBC2015::SetFaFvSdsSd1()
{
	CString str; 
...
void CETC_GDS_KBC2015::OnSelchangeCmdZoneCmb() 
{ 
	SetComboValue();
	SetFaFvSdsSd1();
}

void CETC_GDS_KBC2015::OnSelchangeCmdZoneFactorCmb() 
{
...
void CETC_GDS_KBC2015::OnSelchangeCmdSiteCmb() 
{
	SetFaFvSdsSd1();
}

void CETC_GDS_KBC2015::OnChangeCmdZoneFactor() 
{
	SetFaFvSdsSd1();
}

void CETC_GDS_KBC2015::OnChangeCmdDepthToMREdit() 
{
	SetFaFvSdsSd1();
}

void CETC_GDS_KBC2015::OnChangeCmdFaEdit() 
{
	CString str;
...
void CETC_GDS_KBC2015::OnChangeCmdFvEdit() 
{
	CString str; 
...
void CETC_GDS_KBC2015::OnChangeCmdSdsEdit() 
{
}

void CETC_GDS_KBC2015::OnChangeCmdSd1Edit() 
{
}

void CETC_GDS_KBC2015::InitZoneCombo()
{
...
void CETC_GDS_KBC2015::InitZoneFactorCombo()
{
...
void CETC_GDS_KBC2015::InitSiteCombo()
{
...
void CETC_GDS_KBC2015::InitIFactorCombo()
{
...
void CETC_GDS_KBC2015::InitRCombo()
{
...
void CETC_GDS_KBC2015::SetComboValue()
{
...
BOOL CETC_GDS_KBC2015::ISVALID()
{ 	
	GetFinalValue();
	return m_pSpfcUtil->CheckValid();
}

void CETC_GDS_KBC2015::End()
{
	CMyChildDialog::End();
}

void CETC_GDS_KBC2015::GetFinalValue()
{
	...
	m_pSpfcUtil->SetDepthToMR(m_editDepthToMR.GetEditValue());
...
BOOL CETC_GDS_KBC2015::Dlg2Data()
{
	m_pParamData->CodeParam.KBC2016.nSeisZone = m_pSpfcUtil->GetZone();
	m_pParamData->CodeParam.KBC2016.nSiteClass = m_pSpfcUtil->GetSite();
	m_pParamData->CodeParam.KBC2016.dZoneFactor = m_pSpfcUtil->GetZoneFactor();
	m_pParamData->CodeParam.KBC2016.dDepthMR = m_pSpfcUtil->GetDepthToMR();
	m_pParamData->CodeParam.KBC2016.dFa = m_pSpfcUtil->GetFa();
	m_pParamData->CodeParam.KBC2016.dFv = m_pSpfcUtil->GetFv();
	m_pParamData->CodeParam.KBC2016.dSds = m_pSpfcUtil->GetSds();
	m_pParamData->CodeParam.KBC2016.dSd1 = m_pSpfcUtil->GetSd1();
	m_pParamData->CodeParam.KBC2016.dIe = m_pSpfcUtil->GetI();
	m_pParamData->CodeParam.KBC2016.dCoef = m_pSpfcUtil->GetR();
	m_pParamData->CodeParam.KBC2016.dMaxPeriod = m_pSpfcUtil->GetMaxPeriod();
	...
	m_pParamData->Description.Format(_LS(IDS_WG_CMD__ADD_KBC2015_PROFILE), m_pSpfcUtil->GetZone() + 1, m_pSpfcUtil->GetZoneFactor(), strSite[m_pSpfcUtil->GetSite()], m_pSpfcUtil->GetDepthToMR(), 
		strUnit, m_pSpfcUtil->GetFa(), m_pSpfcUtil->GetFv(), m_pSpfcUtil->GetSds(), m_pSpfcUtil->GetSd1(), m_pSpfcUtil->GetI(), m_pSpfcUtil->GetR());

	return TRUE;
}

void CETC_GDS_KBC2015::Data2Dlg()
{
	SetPushOverData(TRUE);
}

void CETC_GDS_KBC2015::SetPushOverData(BOOL bInit/*=TRUE*/)
{
	if(m_pParamData->nSpecCodeType != D_SPFC_CODE_KBC2015) return;

	m_pSpfcUtil->SetZone(m_pParamData->CodeParam.KBC2016.nSeisZone);
	m_pSpfcUtil->SetSite(m_pParamData->CodeParam.KBC2016.nSiteClass);
	m_pSpfcUtil->SetZoneFactor(m_pParamData->CodeParam.KBC2016.dZoneFactor);
	m_pSpfcUtil->SetDepthToMR(m_pParamData->CodeParam.KBC2016.dDepthMR);
	m_pSpfcUtil->SetFa(m_pParamData->CodeParam.KBC2016.dFa);
	m_pSpfcUtil->SetFv(m_pParamData->CodeParam.KBC2016.dFv);
	m_pSpfcUtil->SetSds(m_pParamData->CodeParam.KBC2016.dSds);
	m_pSpfcUtil->SetSd1(m_pParamData->CodeParam.KBC2016.dSd1);
	m_pSpfcUtil->SetI(m_pParamData->CodeParam.KBC2016.dIe);
	m_pSpfcUtil->SetR(m_pParamData->CodeParam.KBC2016.dCoef);
	m_pSpfcUtil->SetMaxPeriod(m_pParamData->CodeParam.KBC2016.dMaxPeriod);

	m_pSpfcUtil->SetTs(m_pParamData->CodeParam.KBC2016.dSd1 / m_pParamData->CodeParam.KBC2016.dSds);
	m_pSpfcUtil->SetT0(0.2 * m_pSpfcUtil->GetTs());
	...
	if(bInit)
	{
		...
		m_editDepthToMR.SetEditUnit(m_pSpfcUtil->GetDepthToMR());
