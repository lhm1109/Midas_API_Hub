// RSModifyFactorGrid.cpp : implementation file
//
...
{ 
	...
	D_COL_MAX = D_COL_X_VALUE
};

CRSModifyFactorGrid::CRSModifyFactorGrid()
{
	...
	AddColInfo(_LS(IDS_DGN_CHAR_CHI), t_real,    0);  
}
...
void CRSModifyFactorGrid::SetData(CArray<T_RSMF_K, T_RSMF_K>& aRsmfK, CArray<T_RSMF_D, T_RSMF_D&>& aRsmfD)
{
	BOOL bOldLock = LockUpdate(TRUE);

	int nNumKey = aRsmfK.GetSize();
	int nNumData = aRsmfD.GetSize();
	...
	for(int i=0; i<nNumKey; ++i)
	{
		...
		if(!m_pDoc->m_pAttrCtrl->GetSplc(aRsmfK[i], SplcD)) { ASSERT(0); continue; }
		
		aValue[D_COL_KEY      - 1].Format(_T("%d"), aRsmfK[i]);
		aValue[D_COL_LOADCASE - 1] = SplcD.LoadCaseName;
		aValue[D_COL_C_VALUE  - 1].Format(_T("%g"), aRsmfD[i].dC);
		aValue[D_COL_J_VALUE  - 1].Format(_T("%g"), aRsmfD[i].dJ);
		aValue[D_COL_X_VALUE  - 1].Format(_T("%g"), aRsmfD[i].dX);
	...
	HideCols(D_COL_X_VALUE, D_COL_X_VALUE, m_pDoc->m_pAttrCtrl2->IsSemeMethod_KISTEC2019()==FALSE);

	SetRowHeightByDPI(1, nNumKey+1, 18);
...
BOOL CRSModifyFactorGrid::GetData(CArray<T_RSMF_K, T_RSMF_K>& raRsmfK, CArray<T_RSMF_D, T_RSMF_D&>& raRsmfD)
{
	raRsmfK.RemoveAll();
	raRsmfD.RemoveAll();
	...
	raRsmfK.SetSize(nDataCount);
	raRsmfD.SetSize(nDataCount);
	...
	for(int i=0; i<nDataCount; ++i)
	{
		...
		raRsmfK[i]    = _ttoi(aValue[D_COL_KEY      - 1]);
		raRsmfD[i].dC = _tstof(aValue[D_COL_C_VALUE  - 1]);
		raRsmfD[i].dJ = _tstof(aValue[D_COL_J_VALUE  - 1]);
		raRsmfD[i].dX = _tstof(aValue[D_COL_X_VALUE  - 1]);
	}
	return TRUE;
}

void CRSModifyFactorGrid::SetDataToRow(ROWCOL nRow, CStringArray& raValue)
{
...
BOOL CRSModifyFactorGrid::GetRowToData(ROWCOL nRow, CStringArray& raValue)
{
	int nColCount = GetColCount();
	ASSERT(nColCount == raValue.GetSize());
...
CString CRSModifyFactorGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString strNum(_T(""));
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CRSModifyFactorGrid, CTBCommon)
	//{{AFX_MSG_MAP(CRSModifyFactorGrid)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CRSModifyFactorGrid message handlers

void CRSModifyFactorGrid::Initialize()
{
	...
	SetCoveredCellsRowCol(0, D_COL_X_VALUE , 1, D_COL_X_VALUE );
	...
	*/
	...
	if(m_pDoc->m_pAttrCtrl2->IsSemeMethod_KISTEC2019())
	{
		aWidth[D_COL_C_VALUE - 1] = 60;
		aWidth[D_COL_J_VALUE - 1] = 60;
		aWidth[D_COL_X_VALUE - 1] = 60;
	}
	else
	{
		aWidth[D_COL_C_VALUE - 1] = 70;
		aWidth[D_COL_J_VALUE - 1] = 70;
		aWidth[D_COL_X_VALUE - 1] = 70;
	}
	...
	SetValueRange(CGXRange(0, D_COL_X_VALUE ), _LS(IDS_DGN_CHAR_CHI));

	LockUpdate(FALSE);
}

BOOL CRSModifyFactorGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bCurrentRow;
	BOOL bCurrentCell;
	BOOL bEnabled;
...
BOOL CRSModifyFactorGrid::OnStartEditing(ROWCOL nRow, ROWCOL nCol)
{
	return CTBCommon::OnStartEditing(nRow, nCol);
}

void CRSModifyFactorGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
	CTBCommon::OnModifyCell(nRow, nCol);
}

BOOL CRSModifyFactorGrid::DoLButtonUp(UINT flags, CPoint point)
{
	BOOL bOldLock = LockUpdate(TRUE);
...
BOOL CRSModifyFactorGrid::OnEndEditing(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CRSModifyFactorGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CRSModifyFactorGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CRSModifyFactorGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CRSModifyFactorGrid::CopyRange(const CGXRangeList& selList)
{
...
BOOL CRSModifyFactorGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CRSModifyFactorGrid::IsPasteToAllRange(const CGXRange &range, CString szData) // copy한 Cell이 1개 이고, 동일한 행의 cell 여러개로 붙여넣은 상황일 때 TRUE.. 이 경우엔 붙여넣을 Range의 모든 Cell에 복사한 Cell의 값을 붙여 넣어 줘야 함
{
...
void CRSModifyFactorGrid::SetEnableDisableCells(ROWCOL nRow)
{
	if(nRow <= 1) return;
	SetStyleRange(CGXRange().SetCells(nRow, D_COL_LOADCASE), CGXStyle().SetReadOnly(TRUE));
}

BOOL CRSModifyFactorGrid::AddToDB( ROWCOL nRow )
{
...
void CRSModifyFactorGrid::RemoveRecords()
{
