...
CSpfcIS2016Util::CSpfcIS2016Util()
{
	m_nSoil = 0;
	m_nZone = 0;
	m_dMaxPeriod = PRD_ED;
	...
	m_dR = 3.0;
	m_dDamping = 5.0;
	m_dDampingFactor = GetDampingFactorIS1893_2016(m_dDamping);
}

bool CSpfcIS2016Util::CheckValid()
{
...
void CSpfcIS2016Util::MakeSpectrumData(bool bOnlyCalc)
{
	double st = PRD_ST;
	...
	int result = std::swprintf(buffer, SpfcUtil::BUFFER_SIZE, _T("IS1893(2016): Zone=%s, Soil= %s, Damping=%5.2f, I=%4.2f, R=%4.2f"), strZone[m_nZone], strSoil[m_nSoil], m_dDamping, m_dI, m_dR);
	if (result >= 0)
	{
		SetDescript(buffer);
	}
	SetFuncName(L"IS1893(2016)");
}

void CSpfcIS2016Util::MakePOSpectrumData(double dDamping, double SRa, double SRv, CArray<double, double>& aPeriod, CArray<double, double>& aAccel)
{
	aPeriod.RemoveAll();
	aAccel.RemoveAll();
...
void CSpfcIS2016Util::SetParamFromCode(const T_SPFC_CODE& codeParam)
{
...
void CSpfcIS2016Util::SetParamFromCalc()
{
	m_dZ = IS2016::ZONE[m_nZone];
}

double CSpfcIS2016Util::GetDampingFactorIS1893_2016(double dDamping)
{
