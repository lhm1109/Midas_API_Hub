// CMStageItemLoadDlg.cpp : implementation file
//
...
CCMStageItemLoadDlg::CCMStageItemLoadDlg(CWnd* pParent /*=NULL*/)
	: CCMStageItemBase(CCMStageItemLoadDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CCMStageItemLoadDlg)
	//}}AFX_DATA_INIT
}


void CCMStageItemLoadDlg::DoDataExchange(CDataExchange* pDX)
{
	CCMStageItemBase::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCMStageItemLoadDlg)
...
BEGIN_MESSAGE_MAP(CCMStageItemLoadDlg, CDialog)

	//{{AFX_MSG_MAP(CCMStageItemLoadDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CCMStageItemLoadDlg message handlers

void CCMStageItemLoadDlg::SetListCtrlHeader()
{
...
BOOL CCMStageItemLoadDlg::OnInitDialog() 
{
...
void CCMStageItemLoadDlg::UpdateGroupList()
{
...
void CCMStageItemLoadDlg::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	if(GetSafeHwnd())
	{
		switch(lHint)
...
void CCMStageItemLoadDlg::UpdateBuffer()
{
	/*
...
void CCMStageItemLoadDlg::OnCmdDefgroupButton() 
{
	CreateOrActivateDlg(CDBDoc::GetDocPoint(),CCMLoadGroupDlg::IDD,this);
}

/*
void CCMStageItemLoadDlg::SetCurStageDataOnly(T_STAG_D *pStageD)
{
...
void CCMStageItemLoadDlg::SetCurStageData(T_STAG_D *pStageD)
{
	...
	int nStep = pStageD->aUserDefStep.GetSize();
	double dDay;
...
void CCMStageItemLoadDlg::SetDayCombo()
{
	...
	{
		StrTemp.Format(_T("%.8g"),m_pCurStageD->aUserDefStep[i]);
		m_wndActDayCombo.AddString(StrTemp);
		m_wndActDayCombo.SetItemData(i+2,(DWORD_PTR)(&m_pCurStageD->aUserDefStep[i]));
	}

	m_wndDeactDayCombo.AddString(_LS(IDS_CMD0417__First));
	m_wndDeactDayCombo.SetItemData(0,(DWORD_PTR)&FirstLast[0]);
	m_wndDeactDayCombo.AddString(_LS(IDS_CMD0417__Last));
	m_wndDeactDayCombo.SetItemData(1,(DWORD_PTR)&FirstLast[1]);

	for(int i = 0 ; i < nSize ; i++)
	{
		StrTemp.Format(_T("%.8g"),m_pCurStageD->aUserDefStep[i]);
		m_wndDeactDayCombo.AddString(StrTemp);
		m_wndDeactDayCombo.SetItemData(i+2,(DWORD_PTR)(&m_pCurStageD->aUserDefStep[i]));
	}

	m_wndActDayCombo.SetCurSel(0);
	m_wndDeactDayCombo.SetCurSel(0);
	CDlgUtil::CobxAdjustListBoxWidth(m_wndDeactDayCombo);
}

BOOL CCMStageItemLoadDlg::GetActAge(double& dAge,CString& StrAge)
{
...
BOOL CCMStageItemLoadDlg::GetDeactAge(double& dAge, CString& StrAge)
{
...
void CCMStageItemLoadDlg::UpdateGroupListByCurState()  // Active List Deactive List 설정후에 호출할것...
{
...
void CCMStageItemLoadDlg::GetLoadGroupHavingFloorOrPlaneLoad(CMapEx<UINT, UINT, int, int>& rmLdgrK)
{
	CDBDoc* pDoc = CDBDoc::GetDocPoint(); ASSERT(pDoc);
	rmLdgrK.RemoveAll();

	// Plane Load, Floor Load 몇개 안되므로 이거를 Loop돌면서 찾자
	POSITION pos = pDoc->m_pAttrCtrl->GetStartPnla();
	T_PNLA_K PnlaK;
	T_PNLA_D PnlaD;
	while (pos)
	{
		pDoc->m_pAttrCtrl->GetNextPnla(pos, PnlaK, PnlaD);
	...
	while (pos)
	{
		pDoc->m_pAttrCtrl->GetNextFbla(pos, FblaK, FblaD);
		rmLdgrK.SetAt(FblaD.GroupKey, 0);
	}
}

void CCMStageItemLoadDlg::_AddFunc(int nWhat) // (0) Act Add (1) Deact Add
{
	...
	// MNET:2846-JBSEON-20070701
	// Deactive인 경우 Floor Load, Plane Load가 속한 그룹이 있는지 조사해서 있는 경우는 에러 처리한다.
	if (nWhat == 1)
	{
		CMapEx<T_LDGR_K, T_LDGR_K, int, int> mLdgr;
		mLdgr.InitHashTable(101);
		GetLoadGroupHavingFloorOrPlaneLoad(mLdgr);
		for (int i = 0; i < SelGrupKeys.GetSize(); i++)
		{
			if (mLdgr.Exist(SelGrupKeys[i]))
			{
				CString csMsg;
				csMsg =  _LS(IDS_DB_STAG_WARN_DEACTIVE_FLOOR_PLANE_LOAD6); 
				csMsg += _LS(IDS_DB_STAG_WARN_DEACTIVE_FLOOR_PLANE_LOAD2);
				AfxMessageBox(csMsg);
			}
		}
	}
...
void CCMStageItemLoadDlg::OnActAddButton() 
{
	_AddFunc(0);
}

void CCMStageItemLoadDlg::OnActDeleteButton() 
{
...
void CCMStageItemLoadDlg::OnActModifyButton() 
{
...
void CCMStageItemLoadDlg::OnDeactAdd() 
{
	// TODO: Add your control notification handler code here
	_AddFunc(1);
}

void CCMStageItemLoadDlg::OnDeactDeleteButton() 
{
...
/*
void CCMStageItemLoadDlg::OnDeltaposCmdDelaySpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CCMStageItemLoadDlg::ResetParam()
{
	//m_wndAgeEdit.SetWindowText(_T("0"));
}

void CCMStageItemLoadDlg::OnDeactModifyButton() 
{
