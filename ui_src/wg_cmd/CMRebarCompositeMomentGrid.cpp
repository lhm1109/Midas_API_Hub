// CMRebarCompositeMomentGrid.cpp : implementation file
//
// 2004. 03. 09    by TAE 
// 2006. 12. 15    modified by mylee
/////////////////////////////////////////////////////////////////////////////
//   1      2       3         4      5      6       7       8
// ---------------------------------------------------------------
//  Part | Dia |  Number | Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
...
//#define D_RPSC_ARRANGE_ONE     _LS(IDS_CMD_RPSC_ARRANGE_ONE)
//#define D_RPSC_ARRANGE_BOTH    _LS(IDS_CMD_RPSC_ARRANGE_BOTH)
#define D_RPSC_REF_CENTROID    _LS(IDS_CMD_RPSC_REF_CENTROID)
#define D_RPSC_REF_LEFT        _LS(IDS_CMD_RPSC_REF_LEFT)
#define D_RPSC_REF_TOP         _LS(IDS_CMD_RPSC_REF_TOP)
#define D_RPSC_REF_BOP         _LS(IDS_CMD_RPSC_REF_BOTTOM)

/////////////////////////////////////////////////////////////////////////////
// CCMRebarCompositeMomentGrid

CCMRebarCompositeMomentGrid::CCMRebarCompositeMomentGrid(CCMRebarCompositeMoment* pParent)
{
	ASSERT(pParent);
	m_pParent = pParent;
...
CCMRebarCompositeMomentGrid::~CCMRebarCompositeMomentGrid()
{
}

//--------------------------------------------------------------------------
// Interface
void CCMRebarCompositeMomentGrid::Initialize(CArray<T_RPSC_MBAR, T_RPSC_MBAR&> *pData, BOOL bSym)
{
	ASSERT(pData);
	m_pData = pData;
	m_bSymmetry = bSym;
...
void CCMRebarCompositeMomentGrid::MakeItemEx()
{
	CWaitCursor wait;
	...
	EnableSelPart1(m_pParent->IsPart1Enabled());
	ChangeSymmetry(m_bSymmetry);
}

void CCMRebarCompositeMomentGrid::ChangeSymmetry(BOOL bSym)
{
	m_bSymmetry = bSym;
...
void CCMRebarCompositeMomentGrid::UpdateBuffer(int nCmd, CArray<int, int> &aOldIndex, CArray<int, int> &aIndex, CArray<T_RPSC_MBAR, T_RPSC_MBAR&> &aData)
{
...
CString CCMRebarCompositeMomentGrid::GetArrangeStr(int data)
{
	if (data == 0) return D_RPSC_REF_CENTROID;
	if (data == 1) return D_RPSC_REF_LEFT;
	return _T("");
}

int CCMRebarCompositeMomentGrid::GetArrangeData(CString &value)
{
	if (value == D_RPSC_REF_CENTROID) return 0;
	if (value == D_RPSC_REF_LEFT) return 1;
	return -1;
}

CString CCMRebarCompositeMomentGrid::GetRefPosStr(int data)
{
...
int CCMRebarCompositeMomentGrid::GetRefPosData(CString &value)
{
...
//--------------------------------------------------------------------------
// Implementation
void CCMRebarCompositeMomentGrid::EnableSelPart1(BOOL bEnable)
{
	...
	if (bEnable)
		...
		csChoiceList.Format(_T("%s\n"), _T("2"));
	SetStyleRange(CGXRange().SetCols(1), CGXStyle().SetChoiceList(csChoiceList));
}

void CCMRebarCompositeMomentGrid::SetColInfo()
{
	...
	// ref.
	csChoiceList.Format(_T("%s\n%s\n"), D_RPSC_REF_CENTROID, D_RPSC_REF_LEFT);
...
void CCMRebarCompositeMomentGrid::SetValue(ROWCOL nRow, int index, T_RPSC_MBAR &data)
{
	CStringArray aValue;
...
void CCMRebarCompositeMomentGrid::SetHeaderTitle()
{
	BOOL bOldLock = LockUpdate(TRUE);
	...
	CString aTitle[] = {_LS(IDS_CMD_FPSC_Part), _LS(IDS_CMD_FPSC_Dia), _LS(IDS_CMD_FPSC_Number), _LS(IDS_CMD_FPSC_Ref_Y), _LS(IDS_CMD_FPSC_Y), 
		_LS(IDS_CMD_FPSC_Ref_Z), _LS(IDS_CMD_FPSC_Z), _LS(IDS_CMD_FPSC_Spacing)};
	int aWidth[] = {35, 55, 55, 65, 50, 60, 50, 55};
...
void CCMRebarCompositeMomentGrid::ConvDataToStrArray(T_RPSC_MBAR &data, CStringArray &aValue)
{
//   1      2       3         4      5      6       7       8
// ---------------------------------------------------------------
//  Part | Dia |  Number | Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
	...
	aValue[0].Format(_T("%d"), data.nPart); // need change
	aValue[1] = data.strBarNa;
	aValue[2].Format(_T("%d"), data.iBarNum);
	aValue[3] = GetArrangeStr(data.iStaYRef);
	aValue[4].Format(DATA_FMT, data.dStaY);
	aValue[5] = GetRefPosStr(data.iStaZRef);
	aValue[6].Format(DATA_FMT, data.dStaZ);
	aValue[7].Format(DATA_FMT, data.dPitch);
}

BOOL CCMRebarCompositeMomentGrid::InsertRecordByKey(int index, T_RPSC_MBAR &data)
{
...
BOOL CCMRebarCompositeMomentGrid::DeleteRecordByKey(int index, T_RPSC_MBAR &data)
{
	ROWCOL ncRow, ncCol;
...
void CCMRebarCompositeMomentGrid::SelectGridRow(long* aRow, long nSize)
{
...
BOOL CCMRebarCompositeMomentGrid::GetValue(ROWCOL nRow, T_RPSC_MBAR &data)
{
	data.Initialize();
...
BOOL CCMRebarCompositeMomentGrid::OnValidateField(ROWCOL nRow, ROWCOL nCol)
{
	CString value;
	int nColID;
...
// CTBBrowserWnd::ProcessKeys 참조
BOOL CCMRebarCompositeMomentGrid::OnValidateRecord(ROWCOL nRow)
{
...
BOOL CCMRebarCompositeMomentGrid::ValidateField(CString& value, int nColID)
{
//   0      1       2         3      4      5       6       7
// ---------------------------------------------------------------
//  Part | Dia |  Number | Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
	...
	if (nColID == 0 || nColID == 1 || nColID == 3 || nColID == 5) return TRUE;
	else if (nColID == 2)
	{
	...
	else if (nColID == 4 || nColID == 6 || nColID == 7)
	{
...
BOOL CCMRebarCompositeMomentGrid::ValidateRecord(CStringArray &aValue, CRowColArray &aCols)
{
	CString aTitle[] = {_LS(IDS_CMD_FPSC_Part), _LS(IDS_CMD_FPSC_Dia), _LS(IDS_CMD_FPSC_Number), _LS(IDS_CMD_FPSC_Ref_Y), _LS(IDS_CMD_FPSC_Y),
		_LS(IDS_CMD_FPSC_Ref_Z), _LS(IDS_CMD_FPSC_Z), _LS(IDS_CMD_FPSC_Spacing)};
...
BOOL CCMRebarCompositeMomentGrid::OnAddNewRecord(ROWCOL nRow)
{
	BOOL bOldLock = LockUpdate(TRUE);
	if (!AddToDB(nRow))
	{
...
BOOL CCMRebarCompositeMomentGrid::AddToDB(ROWCOL nRow)
{
	int nIndex;
	T_RPSC_MBAR data;
...
BOOL CCMRebarCompositeMomentGrid::AddData(int nIndex, T_RPSC_MBAR &data)
{
	CArray<int, int> aOldIndex, aIndex;
	CArray<T_RPSC_MBAR, T_RPSC_MBAR&> aData;
...
BOOL CCMRebarCompositeMomentGrid::OnModifyRecord(ROWCOL nRow)
{
	// 수정한다. 실패하면 에러 리턴
	BOOL bOldLock = LockUpdate(TRUE);
	if (!ModifyToDB(nRow))
...
BOOL CCMRebarCompositeMomentGrid::ModifyToDB(ROWCOL nRow)
{
	int nIndex;
	T_RPSC_MBAR data;
...
BOOL CCMRebarCompositeMomentGrid::ModifyData(int nOldIndex, int nIndex, T_RPSC_MBAR &data)
{
	CArray<int, int> aOldIndex, aIndex;
	CArray<T_RPSC_MBAR, T_RPSC_MBAR&> aData;
...
BOOL CCMRebarCompositeMomentGrid::OnDeleteRecord(ROWCOL nRow)
{
	// save current row
	ROWCOL ncRow, ncCol;
	GetCurrentCell(ncRow, ncCol);
...
BOOL CCMRebarCompositeMomentGrid::OnDeleteRecords(CRowColArray &awRows)
{
	// save current row
	ROWCOL ncRow, ncCol;
	GetCurrentCell(ncRow, ncCol);
...
BOOL CCMRebarCompositeMomentGrid::DeleteFromDB(CRowColArray &awRows)
{
	int nIndex;
...
void CCMRebarCompositeMomentGrid::OnChangedSelection(const CGXRange *changedRect, BOOL bIsDraggin, BOOL bKey)
{
	long nSize, nTmpSize;
	if (changedRect)
	{
...
BOOL CCMRebarCompositeMomentGrid::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
	if (IsEdit()) 
	{
...
BOOL CCMRebarCompositeMomentGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CCMRebarCompositeMomentGrid::PasteFromClipboard(const CGXRange &range, CString* aDefVal, int nKeyCol, 
...
BOOL CCMRebarCompositeMomentGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bReadOnlyMode;
	BOOL bCurrentRow;
	BOOL bCurrentCell;
...
BOOL CCMRebarCompositeMomentGrid::MakeDataArray(CStringArray& value, CRowColArray& awCols, 
																		CArray<T_RPSC_MBAR, T_RPSC_MBAR&>& aData)
{
	T_RPSC_MBAR data;
	data.Initialize();
...
BOOL CCMRebarCompositeMomentGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, T_RPSC_MBAR &data)
{
...
BOOL CCMRebarCompositeMomentGrid::ConvStrArrayToData(CStringArray &aValue, T_RPSC_MBAR& data)
{
//   1      2       3         4      5      6       7       8
// ---------------------------------------------------------------
//  Part | Dia |  Number | Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 

	data.nPart = _ttoi(aValue[0]); // need change
	data.strBarNa = aValue[1];
	data.iBarNum = _ttoi(aValue[2]);
	data.iStaYRef = GetArrangeData(aValue[3]);
	data.dStaY = _tstof(aValue[4]);
	data.iStaZRef = GetRefPosData(aValue[5]);
	data.dStaZ = _tstof(aValue[6]);
	data.dPitch = _tstof(aValue[7]);

	return TRUE;
}


BEGIN_MESSAGE_MAP(CCMRebarCompositeMomentGrid, CTBBrowserWnd)
	//{{AFX_MSG_MAP(CCMRebarCompositeMomentGrid)
	//ON_WM_LBUTTONDOWN()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CCMRebarCompositeMomentGrid message handlers

/*
// 마우스 클릭시 번호 컬럼이 헤더 컬럼처럼 동작하게 한다.
void CCMRebarCompositeMomentGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
	ROWCOL nhRow, nhCol;
...
BOOL CCMRebarCompositeMomentGrid::OnInsertEmptyRecord(ROWCOL nRow)
{
...
CString CCMRebarCompositeMomentGrid::GetDefValue(int ix)
{
	if (ix == 0) return _T("1");
	else if (ix == 1) return _T("");
	else if (ix == 2) return _T("0");
	else if (ix == 3) return D_RPSC_REF_CENTROID;
	else if (ix == 4) return _T("0");
	else if (ix == 5) return D_RPSC_REF_TOP;
	else if (ix == 6) return _T("0");
	else if (ix == 7) return _T("0");
	else ASSERT(0);
...
BOOL CCMRebarCompositeMomentGrid::HideColsByIntend(int nFromCol, int nToCol, BOOL bHide)
{
...
BOOL CCMRebarCompositeMomentGrid::IsHiddenByIntend(int nCol)
{
	CGXStyle style;
	CString csHide;
...
int CCMRebarCompositeMomentGrid::HitTest(CPoint& pt, ROWCOL* pnRow, ROWCOL* pnCol, CRect* rectHit)
{
...
// HideColsByIntend()함수를 도와주는 함수 
BOOL CCMRebarCompositeMomentGrid::OnTrackColWidth(ROWCOL nCol)
{
...
// HideColsByIntend()함수를 도와주는 함수
BOOL CCMRebarCompositeMomentGrid::HideCols(ROWCOL nFromCol, ROWCOL nToCol, BOOL bHide, BOOL *abHideArray, UINT flags, GXCmdType ctCmd)
{
...
BOOL CCMRebarCompositeMomentGrid::Copy()
{
