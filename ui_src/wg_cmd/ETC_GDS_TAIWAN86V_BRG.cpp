// ETC_GDS_TAIWAN86V_BRG.cpp : implementation file
//
...
CETC_GDS_TAIWAN86V_BRG::CETC_GDS_TAIWAN86V_BRG(T_SPFC_D* pParamData, CWnd* pParent /*=NULL*/)
	: CMyChildDialog(CETC_GDS_TAIWAN86V_BRG::IDD, pParent, std::make_shared<CSpfcTWBR89VUtil>())
{
	//{{AFX_DATA_INIT(CETC_GDS_TAIWAN86V_BRG)
	m_nSoil = 0;
	m_nZone = 0;
	m_dMaxPeriod = PRD_ED;
	//}}AFX_DATA_INIT
	...
	m_pSpfcUtil = std::dynamic_pointer_cast<CSpfcTWBR89VUtil>(m_pSpfcBaseUtil);
}


void CETC_GDS_TAIWAN86V_BRG::DoDataExchange(CDataExchange* pDX)
{
	CMyChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CETC_GDS_TAIWAN86V_BRG)
...
BEGIN_MESSAGE_MAP(CETC_GDS_TAIWAN86V_BRG, CMyChildDialog)
	//{{AFX_MSG_MAP(CETC_GDS_TAIWAN86V_BRG)
		// NOTE: the ClassWizard will add message map macros here
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CETC_GDS_TAIWAN86V_BRG message handlers

BOOL CETC_GDS_TAIWAN86V_BRG::OnInitDialog() 
{
...
void CETC_GDS_TAIWAN86V_BRG::UpdateDampingWnd(CWnd* pDampingWnd)
{
	CString csDamping;
...
void CETC_GDS_TAIWAN86V_BRG::End()
{
	UpdateData(TRUE);
	Dlg2UtilData();
...
BOOL CETC_GDS_TAIWAN86V_BRG::ISVALID()
{  
...
void CETC_GDS_TAIWAN86V_BRG::SetPushOverData(BOOL bInit/*=TRUE*/)
{ 
	m_nZone          = m_pParamData->CodeParam.TAIWAN89V.nSeisZone;
	m_nSoil          = m_pParamData->CodeParam.TAIWAN89V.nSoilType ;
	m_dMaxPeriod     = m_pParamData->CodeParam.TAIWAN89V.dMaxPeriod ;

	const double dI             = m_pParamData->CodeParam.TAIWAN89V.dIe ;
	const double dR             = 1.0 ;
	const double dAlpha         = m_pParamData->CodeParam.TAIWAN89V.dAlpha ;
...
BOOL CETC_GDS_TAIWAN86V_BRG::Dlg2Data()
{
	m_pParamData->CodeParam.TAIWAN89V.nSeisZone      = m_nZone         ;
	m_pParamData->CodeParam.TAIWAN89V.nSoilType      = m_nSoil         ;
	m_pParamData->CodeParam.TAIWAN89V.dIe = m_pSpfcUtil->GetI();
	m_pParamData->CodeParam.TAIWAN89V.dAlpha = m_pSpfcUtil->GetAlpha();
	m_pParamData->CodeParam.TAIWAN89V.dMaxPeriod     = m_dMaxPeriod    ;
	m_pParamData->CodeParam.TAIWAN89V.dCoef = m_pSpfcUtil->GetRStar();
	m_pParamData->CodeParam.TAIWAN89V.dDamping = m_pSpfcUtil->GetDamping();

	m_pParamData->dResModCoef = m_pSpfcUtil->GetR();
	return TRUE;
}

void CETC_GDS_TAIWAN86V_BRG::Dlg2UtilData()
{
	CString strI, strR, strAlpha;
	...
	m_pSpfcUtil->SetR(dR);
	m_pSpfcUtil->SetZ(TWBR89V::ZONE[m_nZone]);
