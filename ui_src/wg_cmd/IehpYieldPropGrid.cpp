// IehpYieldPropGrid.cpp : implementation file
// 
// 2003. 04. 04   by TAE
/////////////////////////////////////////////////////////////////////////
...
CIehpYieldPropGrid::CIehpYieldPropGrid(CIehpYieldPropDlgNew* pParent)
{
	m_pParent = NULL;
	ASSERT(pParent);
	m_pParent = pParent;
...
CIehpYieldPropGrid::~CIehpYieldPropGrid()
{
}
...
void CIehpYieldPropGrid::SetFormatMode(int nMode)
{
...
void CIehpYieldPropGrid::SetDataToGrid(double dP1st[][11], double dM1st[][11], double dP2nd[][11], double dM2nd[][11], double dP3rd[][11], double dM3rd[][11],BOOL bAssignProperties, BOOL bAuto)
{
...
BOOL CIehpYieldPropGrid::GetGridToData(double dP1st[][11], double dM1st[][11], double dP2nd[][11], double dM2nd[][11], double dP3rd[][11], double dM3rd[][11])
{
	CStringArray aValue;
	aValue.SetSize(D_COL_NUM);
...
int  CIehpYieldPropGrid::GetColHeaderNum()
{
	return 4;
}

void CIehpYieldPropGrid::SetEnabled(BOOL bAuto, BOOL bConc, BOOL bAssignProperties)
{
...
void CIehpYieldPropGrid::SetDataToRow(ROWCOL nRow, CStringArray& raValue)
{
...
BOOL CIehpYieldPropGrid::GetRowToData(ROWCOL nRow, CStringArray& raValue)
{
	int nColCount = GetColCount();
	ASSERT(nColCount == raValue.GetSize());
...
CString CIehpYieldPropGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString csNum;
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
void CIehpYieldPropGrid::SetHeaderTitle()
{ 
	/*
	if (m_bConc)
	{
...
BEGIN_MESSAGE_MAP(CIehpYieldPropGrid, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CIehpYieldPropGrid)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CIehpYieldPropGrid message handlers

// 그리드 초기화 
void CIehpYieldPropGrid::Initialize()
{
...
BOOL CIehpYieldPropGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bCurrentRow;
	BOOL bCurrentCell;
	BOOL bEnabled;
...
void CIehpYieldPropGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
/*
void CIehpYieldPropGrid::ChangeColumnStatus(ROWCOL nRow, ROWCOL nCol)
{
	if (nCol < 2) return;
...
void CIehpYieldPropGrid::UpdateParentChart(ROWCOL nRow, ROWCOL nCol)
{
	ASSERT(m_pParent);
...
BOOL CIehpYieldPropGrid::SetValueMode( BOOL bValueMode)
{
...
BEGIN_MESSAGE_MAP(CIehpYieldPropGraph, CWnd)
	//{{AFX_MSG_MAP(CIehpYieldPropGraph)
	ON_WM_PAINT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

CIehpYieldPropGraph::CIehpYieldPropGraph() : m_bDraw1stCurve(TRUE), m_bDraw3rdCurve(FALSE), m_bDrawCrack(false)
{
	memset(m_dV_Pcrst, 0, sizeof(m_dV_Pcrst));
	memset(m_dV_Mcrst, 0, sizeof(m_dV_Mcrst));
}
CIehpYieldPropGraph::~CIehpYieldPropGraph()
{
}

BOOL CIehpYieldPropGraph::Create(CWnd *pParentWnd, const RECT& rect, UINT nID)
{
	CREATESTRUCT cs;
...
void CIehpYieldPropGraph::FitToParent(CRect rect)
{
	SetWindowPos(NULL, 0, 0, rect.Width(), rect.Height(), SWP_NOZORDER | SWP_SHOWWINDOW | SWP_NOMOVE);
}

void CIehpYieldPropGraph::ClearStrengthBkgrnd(CDC* pDC)
{
...
void CIehpYieldPropGraph::ClearSurfaceBkgrnd(CDC* pDC)
{
	CBrush BackBrush(D_COLOR_WHITE);
	CBrush* pOldBrush = pDC->SelectObject(&BackBrush);
	CRect rect;
...
double CIehpYieldPropGraph::CalcScale(CRect rect, double dX, double dY, double dZoomRat/*0.8*/)
{
	double dScale=0.0;
...
CPoint CIehpYieldPropGraph::CalcOriginPoint(CRect rect, double dScale, double dX, double dY)
{
	CPoint orPoint;
...
void CIehpYieldPropGraph::DrawSurfaceGraph()
{
	CClientDC dc(this);
...
void CIehpYieldPropGraph::DrawStrengthGraph()
{
	CClientDC dc(this);
...
void CIehpYieldPropGraph::DrawMultiLinearGraph()
{
	CClientDC dc(this);
	ClearStrengthBkgrnd(&dc);
	dc.SetBkMode(TRANSPARENT);
...
void CIehpYieldPropGraph::DrawText(CDC* pDC, CString str, double dx, double dy)
{
	
}

void CIehpYieldPropGraph::SetDefaultData(int nMethod, double dx[], double dy[], double dOp, double dOm)
{
	m_nMethod = nMethod;
	for(int i=0; i<9; i++)
	{
...
/*
void CIehpYieldPropGraph::SetDefaultData(int nMethod, int nMat, double dPC0, double dMC0[],
...
*/
void CIehpYieldPropGraph::SetDefaultData(int nMethod, double dV_P1st[][11], double dV_M1st[][11],
...
void CIehpYieldPropGraph::SetCrackData(double dV_Pcrst[][11], double dV_Mcrst[][11])
{
	m_bDrawCrack = true;
...
void CIehpYieldPropGraph::SetDefaultData(int nMethod, T_IEHP_DT& arMultiData, BOOL bPmmc)
{
	m_bPmmc = bPmmc;
	m_nMethod = nMethod;
...
void CIehpYieldPropGraph::OnPaint() 
{
