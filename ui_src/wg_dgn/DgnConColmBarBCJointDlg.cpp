// DgnConColmBarBCJointDlg.cpp : implementation file
//
...
CDgnConColmBarBCJointDlg::CDgnConColmBarBCJointDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CDgnConColmBarBCJointDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDgnConColmBarBCJointDlg)
	m_strNqrb = _T("");
	m_strNrow = _T("");
	m_strBcjr = _T("");
	//}}AFX_DATA_INIT
...
void CDgnConColmBarBCJointDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnConColmBarBCJointDlg)
	...
	DDX_Control(pDX, IDC_DGN_RC_BCJRSPIN, m_BcjrSpin);
	DDX_Control(pDX, IDC_DGN_RC_CD0UNIT, m_D0Unit);
	DDX_Text(pDX, IDC_DGN_RC_NQRB, m_strNqrb);
	DDX_Text(pDX, IDC_DGN_RC_NROW, m_strNrow);
	DDX_Text(pDX, IDC_DGN_RC_BCJR, m_strBcjr);
	//}}AFX_DATA_MAP
}

#define CDialog CDialogMove
BEGIN_MESSAGE_MAP(CDgnConColmBarBCJointDlg, CInternationalDlg)
#undef CDialog

	//{{AFX_MSG_MAP(CDgnConColmBarBCJointDlg)
	...
	ON_NOTIFY(UDN_DELTAPOS, IDC_DGN_RC_BCJRSPIN, OnDeltaposDgnRcBcjrspin)
	...
	ON_EN_CHANGE(IDC_DGN_RC_BCJR, OnChangeDgnRcBcjr)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDgnConColmBarBCJointDlg message handlers


BOOL CDgnConColmBarBCJointDlg::OnInitDialog() 
{
...
/*
void CDgnConColmBarBCJointDlg::Initial_brExistData(T_REBR_D Data)
{
	// Initialize Data.
	m_nNqrb = Data.nQrb;
	m_nNrow = Data.nRow;
...
*/
void CDgnConColmBarBCJointDlg::Initial_cExistData(T_REBC_D Data)
{
	// Initialize Data.
	m_nNqrb = Data.nQrb;
	m_nNrow = Data.nRow;
	m_nBcjr = Data.iBCJointRebarNum;
	m_strNqrb.Format(_T("%d"),m_nNqrb);
	m_strNrow.Format(_T("%d"),m_nNrow);
	m_strBcjr.Format(_T("%d"),m_nBcjr);
	// Initialize Bar Size by Default.
	int Index = m_BarSize.FindStringExact(-1,Data.MainRebarName);
	m_BarSize.SetCurSel(Index);
...
void CDgnConColmBarBCJointDlg::Initial_WithoutData()
{
...
double CDgnConColmBarBCJointDlg::Get_SectionDimension()
{
	T_SECT_D rData;
	rData.Initialize();
...
double CDgnConColmBarBCJointDlg::Get_ChangeUnitFactor()
{
	// cm.
...
void CDgnConColmBarBCJointDlg::Initial_Data()
{
	T_SECT_D rData;
	rData.Initialize();
	...
	m_BcjrSpin.SetRange(0,200);
}

void CDgnConColmBarBCJointDlg::Show_RectColumnShape()
{
...
void CDgnConColmBarBCJointDlg::Show_CircColumnShape()
{
...
void CDgnConColmBarBCJointDlg::OnDeltaposDgnRcNqrbspin(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CDgnConColmBarBCJointDlg::OnDeltaposDgnRcNrowspin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
...
void CDgnConColmBarBCJointDlg::OnDeltaposDgnRcBcjrspin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;  

	CString str=_T("");
	GetDlgItem(IDC_DGN_RC_BCJR)->GetWindowText(str);

	int Factor = (int)(pNMUpDown->iDelta);
	m_BcjrSpinNo = _ttoi(str)+Factor;

	if(m_BcjrSpinNo < 0)
	{
		if(Factor==1)	m_BcjrSpinNo = 0;
		else					m_BcjrSpinNo = 100;
	}
	else if(m_BcjrSpinNo > 100)  m_BcjrSpinNo = 0;

	str.Format(_T("%d"),m_BcjrSpinNo);
	GetDlgItem(IDC_DGN_RC_BCJR)->SetWindowText(str);

	*pResult = 0;
}

void CDgnConColmBarBCJointDlg::OnOK() 
{
...
void CDgnConColmBarBCJointDlg::OnChangeDgnRcCd0() 
{
...
void CDgnConColmBarBCJointDlg::OnChangeDgnRcNqrb() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	m_nNrow = _ttoi(m_strNrow);
...
void CDgnConColmBarBCJointDlg::OnChangeDgnRcBcjr() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	m_nNrow = _ttoi(m_strNrow);
...
void CDgnConColmBarBCJointDlg::OnChangeDgnRcNrow() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	m_nNrow = _ttoi(m_strNrow);
