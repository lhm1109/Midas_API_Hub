// DgnRebarPscMomentGrid.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRebarPscMomentGrid

CDgnRebarPscMomentGrid::CDgnRebarPscMomentGrid(CDgnRebarPscMoment* pParent)
{
	ASSERT(pParent);
	m_pParent = pParent;
...
CDgnRebarPscMomentGrid::~CDgnRebarPscMomentGrid()
{
}

//--------------------------------------------------------------------------
// Interface
void CDgnRebarPscMomentGrid::Initialize(CArray<T_RPSC_MBAR, T_RPSC_MBAR&> *pData, BOOL bSym)
{
	ASSERT(pData);
	m_pData = pData;
	m_bSymmetry = bSym;
...
void CDgnRebarPscMomentGrid::MakeItemEx()
{
	CWaitCursor wait;
...
void CDgnRebarPscMomentGrid::ChangeSymmetry(BOOL bSym)
{
	m_bSymmetry = bSym;
...
void CDgnRebarPscMomentGrid::UpdateBuffer(int nCmd, CArray<int, int> &aOldIndex, CArray<int, int> &aIndex, CArray<T_RPSC_MBAR, T_RPSC_MBAR&> &aData)
{
...
CString CDgnRebarPscMomentGrid::GetArrangeStr(int data)
{
...
int CDgnRebarPscMomentGrid::GetArrangeData(CString &value)
{
...
CString CDgnRebarPscMomentGrid::GetRefPosStr(int data)
{
...
int CDgnRebarPscMomentGrid::GetRefPosData(CString &value)
{
...
//--------------------------------------------------------------------------
// Implementation
void CDgnRebarPscMomentGrid::SetColInfo()
{
...
void CDgnRebarPscMomentGrid::SetValue(ROWCOL nRow, int index, T_RPSC_MBAR &data)
{
	CStringArray aValue;
...
void CDgnRebarPscMomentGrid::SetHeaderTitle()
{
	BOOL bOldLock = LockUpdate(TRUE);
	...
	CString aTitle[] = {_LS(IDS_DGN_FPSC_Dia), _LS(IDS_DGN_FPSC_Number), _LS(IDS_DGN_FPSC_Ref_Y), _LS(IDS_DGN_FPSC_Y), 
		_LS(IDS_DGN_FPSC_Ref_Z), _LS(IDS_DGN_FPSC_Z), _LS(IDS_DGN_FPSC_Spacing)};
	
	int aWidth[] = {70, 70, 75, 70, 70, 70, 70};
...
void CDgnRebarPscMomentGrid::ConvDataToStrArray(T_RPSC_MBAR &data, CStringArray &aValue)
{
//   1        2         3      4      5       6        7  
// ---------------------------------------------------------
//  Dia |  Number |  Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
...
BOOL CDgnRebarPscMomentGrid::InsertRecordByKey(int index, T_RPSC_MBAR &data)
{
...
BOOL CDgnRebarPscMomentGrid::DeleteRecordByKey(int index, T_RPSC_MBAR &data)
{
	ROWCOL ncRow, ncCol;
...
void CDgnRebarPscMomentGrid::SelectGridRow(long* aRow, long nSize)
{
...
BOOL CDgnRebarPscMomentGrid::GetValue(ROWCOL nRow, T_RPSC_MBAR &data)
{
	data.Initialize();
...
BOOL CDgnRebarPscMomentGrid::OnValidateField(ROWCOL nRow, ROWCOL nCol)
{
	CString value;
	int nColID;
...
// CTBBrowserWnd::ProcessKeys 참조
BOOL CDgnRebarPscMomentGrid::OnValidateRecord(ROWCOL nRow)
{
...
BOOL CDgnRebarPscMomentGrid::ValidateField(CString& value, int nColID)
{
//   1        2         3      4      5       6        7  
// ---------------------------------------------------------
//  Dia |  Number |  Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
...
BOOL CDgnRebarPscMomentGrid::ValidateRecord(CStringArray &aValue, CRowColArray &aCols)
{
	CString aTitle[] = {_LS(IDS_DGN_FPSC_Dia), _LS(IDS_DGN_FPSC_Number), _LS(IDS_DGN_FPSC_Ref_Y), _LS(IDS_DGN_FPSC_Y),
		_LS(IDS_DGN_FPSC_Ref_Z), _LS(IDS_DGN_FPSC_Z), _LS(IDS_DGN_FPSC_Spacing)};
...
BOOL CDgnRebarPscMomentGrid::OnAddNewRecord(ROWCOL nRow)
{
	BOOL bOldLock = LockUpdate(TRUE);
	if (!AddToDB(nRow))
	{
...
BOOL CDgnRebarPscMomentGrid::AddToDB(ROWCOL nRow)
{
	int nIndex;
	T_RPSC_MBAR data;
...
BOOL CDgnRebarPscMomentGrid::AddData(int nIndex, T_RPSC_MBAR &data)
{
...
BOOL CDgnRebarPscMomentGrid::OnModifyRecord(ROWCOL nRow)
{
	// 수정한다. 실패하면 에러 리턴
	BOOL bOldLock = LockUpdate(TRUE);
	if (!ModifyToDB(nRow))
...
BOOL CDgnRebarPscMomentGrid::ModifyToDB(ROWCOL nRow)
{
	int nIndex;
	T_RPSC_MBAR data;
...
BOOL CDgnRebarPscMomentGrid::ModifyData(int nOldIndex, int nIndex, T_RPSC_MBAR &data)
{
	CArray<int, int> aOldIndex, aIndex;
	CArray<T_RPSC_MBAR, T_RPSC_MBAR&> aData;
...
BOOL CDgnRebarPscMomentGrid::OnDeleteRecord(ROWCOL nRow)
{
	// save current row
	ROWCOL ncRow, ncCol;
	GetCurrentCell(ncRow, ncCol);
...
BOOL CDgnRebarPscMomentGrid::OnDeleteRecords(CRowColArray &awRows)
{
	// save current row
	ROWCOL ncRow, ncCol;
	GetCurrentCell(ncRow, ncCol);
...
BOOL CDgnRebarPscMomentGrid::DeleteFromDB(CRowColArray &awRows)
{
	int nIndex;
...
void CDgnRebarPscMomentGrid::OnChangedSelection(const CGXRange *changedRect, BOOL bIsDraggin, BOOL bKey)
{
	long nSize, nTmpSize;
	if (changedRect)
	{
...
BOOL CDgnRebarPscMomentGrid::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
	if (IsEdit()) 
	{
...
BOOL CDgnRebarPscMomentGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CDgnRebarPscMomentGrid::PasteFromClipboard(const CGXRange &range, CString* aDefVal, int nKeyCol, 
...
BOOL CDgnRebarPscMomentGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bReadOnlyMode;
	BOOL bCurrentRow;
	BOOL bCurrentCell;
...
BOOL CDgnRebarPscMomentGrid::MakeDataArray(CStringArray& value, CRowColArray& awCols, 
																		CArray<T_RPSC_MBAR, T_RPSC_MBAR&>& aData)
{
	T_RPSC_MBAR data;
	data.Initialize();
...
BOOL CDgnRebarPscMomentGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, T_RPSC_MBAR &data)
{
...
BOOL CDgnRebarPscMomentGrid::ConvStrArrayToData(CStringArray &aValue, T_RPSC_MBAR& data)
{
//   1        2         3      4      5       6        7  
// ---------------------------------------------------------
//  Dia |  Number |  Ref. Y |  Y  | Ref. Z |  Z  |  Spacing 
...
BEGIN_MESSAGE_MAP(CDgnRebarPscMomentGrid, CTBBrowserWnd)
	//{{AFX_MSG_MAP(CDgnRebarPscMomentGrid)
	//ON_WM_LBUTTONDOWN()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRebarPscMomentGrid message handlers

/*
// 마우스 클릭시 번호 컬럼이 헤더 컬럼처럼 동작하게 한다.
void CDgnRebarPscMomentGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
	ROWCOL nhRow, nhCol;
...
BOOL CDgnRebarPscMomentGrid::OnInsertEmptyRecord(ROWCOL nRow)
{
...
CString CDgnRebarPscMomentGrid::GetDefValue(int ix)
{
	if (ix == 0) return _T("");
...
BOOL CDgnRebarPscMomentGrid::HideColsByIntend(int nFromCol, int nToCol, BOOL bHide)
{
...
BOOL CDgnRebarPscMomentGrid::IsHiddenByIntend(int nCol)
{
	CGXStyle style;
	CString csHide;
...
int CDgnRebarPscMomentGrid::HitTest(CPoint& pt, ROWCOL* pnRow, ROWCOL* pnCol, CRect* rectHit)
{
...
// HideColsByIntend()함수를 도와주는 함수 
BOOL CDgnRebarPscMomentGrid::OnTrackColWidth(ROWCOL nCol)
{
...
// HideColsByIntend()함수를 도와주는 함수
BOOL CDgnRebarPscMomentGrid::HideCols(ROWCOL nFromCol, ROWCOL nToCol, BOOL bHide, BOOL *abHideArray, UINT flags, GXCmdType ctCmd)
{
...
BOOL CDgnRebarPscMomentGrid::Copy()
{
