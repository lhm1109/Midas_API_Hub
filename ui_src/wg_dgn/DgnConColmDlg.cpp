// DgnConColmDlg.cpp : implementation file
//
...
#include "DgnConColmBarBCJointDlg.h"
...
CDgnConColmDlg::CDgnConColmDlg(CWnd* pParent /*=NULL*/)
	: CMenuBarChildDlg(CDgnConColmDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDgnConColmDlg)
	m_nOption = -1;
	m_nTied = -1;
	//}}AFX_DATA_INIT
	m_iSubRebarYNum = 0;
	m_iSubRebarZNum = 0;
	m_iBCJointRebarNum = 0;
...
void CDgnConColmDlg::DoDataExchange(CDataExchange* pDX)
{
	CMenuBarChildDlg::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnConColmDlg)
	DDX_Control(pDX, IDC_DGN_RC_COLS, m_S);
	DDX_Control(pDX, IDC_DGN_SUBBARSIZE_COMBO, m_SubRebar);
	DDX_Control(pDX, IDC_DGN_RC_CBC, m_Bc);
	DDX_Control(pDX, IDC_DGN_RC_CD0, m_D0);
	DDX_Control(pDX, IDC_DGN_RC_CHC, m_Hc);
	DDX_Control(pDX, IDC_DGN_RC_CHCUNIT, m_HcUnit);
	DDX_Control(pDX, IDC_DGN_RC_CD0UNIT, m_D0Unit);
	DDX_Control(pDX, IDC_DGN_RC_CBCUNIT, m_BcUnit);
	DDX_Radio(pDX, IDC_DGN_COLUMN_AR, m_nOption);
	DDX_Radio(pDX, IDC_DGN_RC_CTIED, m_nTied);
	DDX_Control(pDX, IDC_DGN_CONC_CGRID, m_wndGrid);
	DDX_Control(pDX, IDC_DGN_CONC_SELECTGRID, m_SelectGrid.m_Grid);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CDgnConColmDlg, CMenuBarChildDlg)
	//{{AFX_MSG_MAP(CDgnConColmDlg)
	...
	ON_BN_CLICKED(IDC_DGN_RC_CTIED, OnDgnRcCtied)
	ON_BN_CLICKED(IDC_DGN_RC_CSPIRAL, OnDgnRcCspiral)
...
/////////////////////////////////////////////////////////////////////////////
// CDgnConColmDlg message handlers

void CDgnConColmDlg::OnDgnClose() 
{
	CTreeMenuBarBase::SetModeST(__TMMODE_TREEMENU__);
}

void CDgnConColmDlg::OnDgnColumnAr() 
{
	m_nOption = 0;			//	add/replace
	UpdateData(FALSE);
}

void CDgnConColmDlg::OnDgnColumnDel() 
{
	m_nOption = 1;			//	Delete
	UpdateData(FALSE);
}

BOOL CDgnConColmDlg::ErrorCheck()
{
	BOOL bCheck=TRUE;
...
void CDgnConColmDlg::OnDgnExecute() 
{
	UpdateData(TRUE);
	...
	else                  // Del
	{
	  ...
	  while(PosSect)
	  {
		...
		int nRebcSize = arRchKey.GetSize();
		if(nRebcSize > 0)
		{
			...
			{
				aRebcK[i].keymap = arRchKey[i];
			}
			
			if(!m_pDoc->m_pDataCtrl->DelRebc(aRebcK))
				bCheck = FALSE;
		}
...
BOOL CDgnConColmDlg::OnInitDialog() 
{
...
void CDgnConColmDlg::Initial_Unit()
{
...
void CDgnConColmDlg::Initial_GridWnd()
{
...
void CDgnConColmDlg::Write_GridTitle()
{
	CGXStyle style;
...
void CDgnConColmDlg::Write_GridItem(int Nqrb,int Nrow,CString Norb)
{
	m_wndGrid.GetParam()->EnableUndo(FALSE);
...
void CDgnConColmDlg::OnDgnRcCinputbar() 
{
	...
	if(bEC8CapaciyDesign)
	{
		...
		if(dlg.DoModal()==IDOK)
		{
		  ...
		  m_iSubRebarYNum = dlg.m_iSpaceYNum;
			...
			m_iBCJointRebarNum = dlg.m_nBcjr;
...
void CDgnConColmDlg::ShowBitmap(CString str)
{
	if(str==_T("SB"))
	{
		GetDlgItem(IDC_DGN_COLRECT_BIT)->ShowWindow(SW_SHOW);
		GetDlgItem(IDC_DGN_COLCIRCLE_BIT)->ShowWindow(SW_HIDE);
		GetDlgItem(IDC_DGN_COLPIPE_BIT)->ShowWindow(SW_HIDE);
	}
	else if(str==_T("SR"))
	{
		GetDlgItem(IDC_DGN_COLRECT_BIT)->ShowWindow(SW_HIDE);
		GetDlgItem(IDC_DGN_COLCIRCLE_BIT)->ShowWindow(SW_SHOW);
		GetDlgItem(IDC_DGN_COLPIPE_BIT)->ShowWindow(SW_HIDE);
	}
	else if(str==_T("P"))
	{
		GetDlgItem(IDC_DGN_COLRECT_BIT)->ShowWindow(SW_HIDE);
		GetDlgItem(IDC_DGN_COLCIRCLE_BIT)->ShowWindow(SW_HIDE);
		GetDlgItem(IDC_DGN_COLPIPE_BIT)->ShowWindow(SW_SHOW);
	}
}

void CDgnConColmDlg::Initial_ExistData()
{
	...
	m_iBCJointRebarNum = m_RchkData.iBCJointRebarNum;

	int Index = m_SubRebar.FindStringExact(-1,m_RchkData.SubRebarName);
...
void CDgnConColmDlg::Set_SectionShape(CString strShape)
{
	m_Shape = strShape;
	CString strSectShp=_T("");
	if(strShape==_T("SB"))			GetDlgItem(IDC_DGN_RC_CSHAPE)->SetWindowText(_LS(IDS_DGN_GRID_TEXT_SECT_RECT));
	else if(strShape==_T("SR"))	GetDlgItem(IDC_DGN_RC_CSHAPE)->SetWindowText(_LS(IDS_DGN_GRID_TEXT_SECT_CIR));
	else if(strShape==_T("P"))	GetDlgItem(IDC_DGN_RC_CSHAPE)->SetWindowText(_LS(IDS_DGN_GRID_TEXT_SECT_PIPE));
	else										GetDlgItem(IDC_DGN_RC_CSHAPE)->SetWindowText(_T(""));
}

void CDgnConColmDlg::Set_ShowSRSection()
{
	GetDlgItem(IDC_DGN_COLBC_STATIC)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_DGN_COLCOLON_STATIC)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_DGN_RC_CBC)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_DGN_RC_CBCUNIT)->ShowWindow(SW_HIDE);
}

void CDgnConColmDlg::Set_ShowSBSection()
{
	GetDlgItem(IDC_DGN_COLBC_STATIC)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_DGN_COLCOLON_STATIC)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_DGN_RC_CBC)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_DGN_RC_CBCUNIT)->ShowWindow(SW_SHOW);
}

void CDgnConColmDlg::Set_ShowPSection()
{
	GetDlgItem(IDC_DGN_COLBC_STATIC)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_DGN_COLCOLON_STATIC)->ShowWindow(SW_SHOW);
	GetDlgItem(IDC_DGN_RC_CBC)->ShowWindow(SW_SHOW);
...
void CDgnConColmDlg::OnDgnRcCtied() 
{
	m_nTied = 0;			//	Tied
	UpdateData(FALSE);
}

void CDgnConColmDlg::OnDgnRcCspiral() 
{
	m_nTied = 1;			//	Spiral
	UpdateData(FALSE);
}

void CDgnConColmDlg::OnDgnDesignSection() 
{
	// TODO: Add your control notification handler code here
	AfxGetMainWnd()->SendMessage(WM_COMMAND,MAKEWPARAM(ID_STRUCT_PROP_SECT4DGN,0));
}

void CDgnConColmDlg::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
...
void CDgnConColmDlg::UpdateBuffer()
{
	...
	while(pos != NULL)
	{
		...
		switch(nCmd)
		{
			...
			case(UR_SECD_MFD):
			case(UR_REBC_ADD):
			case(UR_REBC_DEL):  {bMFD  = TRUE; break;}
...
void CDgnConColmDlg::InitSectGrid()
{
	...
	m_SelectGrid.InitSectGrid(m_arColmData,4);
}

void CDgnConColmDlg::ResetSectGridData(BOOL bKeepChk/*FALSE*/)
{
	m_SelectGrid.ResetSectGridData(bKeepChk);
	ChangeSectionInfo();
}

void CDgnConColmDlg::ChangeSectionInfo()
{
	// Select Elem. On View
	SelectElemOnView();
	...
	if(m_pDoc->m_pAttrCtrl->GetRebc(m_RchkKey, m_RchkData))  Initial_ExistData();
...
LRESULT CDgnConColmDlg::OnChangeFocusSectionGrid(WPARAM wParam, LPARAM lParam)
{
	ChangeSectionInfo();
...
LRESULT CDgnConColmDlg::OnSelectCheckSectionGrid(WPARAM wParam, LPARAM lParam)
{
...
void CDgnConColmDlg::OnDgnColmSelectAll()
{
...
void CDgnConColmDlg::OnDgnColmUnSelectAll()
{
	ResetSectGridData();
...
void CDgnConColmDlg::SetData2Dlg()
{
	// UnSelect All Elem.
	...
	//  Space에 대한 것입니다.
	GetDlgItem(IDC_DGN_RC_COLS)->ShowWindow(SW_HIDE);
	//  Sub Rebar를 초기화시킵니다.
	...
	GetDlgItem(IDC_DGN_RC_CSHAPE)->EnableWindow(FALSE);
...
void CDgnConColmDlg::Initial_WithoutData()
{
	m_nTied = 0;
	m_D0.SetEditUnit(0.);
...
BOOL CDgnConColmDlg::SetDlg2Data()
{
	BOOL bCheck = TRUE;
	UpdateData(TRUE);
	...
	m_RchkData.iBCJointRebarNum	 = m_iBCJointRebarNum;
	...
	if(nRebcSize > 0)
	{
		...
		if(!m_pDoc->m_pDataCtrl->AddRebc(aRebcK, m_RchkData))
			bCheck = FALSE;
	}
...
void CDgnConColmDlg::SelectElemOnView()
{
	if(m_pDoc==NULL)	return;
	if(m_pDoc->m_pViewCtrl->SelectedElemCount()) m_pDoc->m_pViewCtrl->UnSelectAllElem(NULL,FALSE);
