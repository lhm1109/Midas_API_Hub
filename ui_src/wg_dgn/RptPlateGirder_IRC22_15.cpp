// RptPlateGirder_IRC22_15.cpp: implementation of the CRptPlateGirder_IRC22_15 class.
//
//////////////////////////////////////////////////////////////////////
...
CRptPlateGirder_IRC22_15::CRptPlateGirder_IRC22_15()
{
	...
	m_bAppTapered = FALSE;
	m_b1p3RhMy    = FALSE;
	m_bPBTFA4SH = CpgdD.bPBTFA4SH; // FALSE;
	m_bDistortion = FALSE;
...
void CRptPlateGirder_IRC22_15::SetUnitConvertFactor()
{
	// [Code Unit] : N, mm.
	...
	m_strUnitInertiaMoment.Format(_T("0.000000 %s"), _T("m©ù"));
	m_strUnitDegree.Format(_T("0.00 %s"), _T("¢ª"));
...
BOOL CRptPlateGirder_IRC22_15::Get_RptElemKey(ArrElemPairKey& aRptElemKey)
{
...
BOOL CRptPlateGirder_IRC22_15::Get_CheckResultAndPrintElemPositionType(ElemPairK ElemK)
{
...
void CRptPlateGirder_IRC22_15::SetDetailSheetPage(CMSExcel* pXL, CString strNameBefore, CString strNameCurrent)
{
...
void CRptPlateGirder_IRC22_15::PrintReport(CString strPath, int iPrintOpt, BOOL bSaveImage)
{
...
// 	m_pDoc->m_pUnitCtrl->SetUnitIndexCurrent(CngIndex);
	...
	m_strBaseBook = _T("plate_girder_IRC22_15_base.dll"); // .dll·Î ¼öÁ¤ÇØ¾ßÇÔ. 
	...
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Set_PrintMode(TRUE);
	...
	BOOL bCheckService[4];
	for ( i=0; i<2; i++ ) bCheckService[i] = m_CpgdD.bService[i];
	bCheckService[2] = m_CpgdD.bConsidermb7bm8;
	bCheckService[3] = m_CpgdD.bConsidermb9;
	RptData.Initialize();
	...
	for ( i=0; i<iDetlElem; i++ )
	{
		...
		for ( j=0; j<2; j++ ) // 0=I, 1=J
		{
			...
			T_PGBR_BASE PgbrBase1; PgbrBase1.Initialize();
			T_PGBR_IRC_BASE PgbrIRCBase; PgbrIRCBase.Initialize();
			if ( bCheckStrength[0] )
			{
				iChkSect=0;
				for ( k=0; k<2; k++ ) // Max/Min, 
				{
					...
					if ( m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetPgbrResultData(ElemK, nPosiNega, PgbrRatD) )
					{
						...
						m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_SavedResult4Design(ElemK, bPosiI, PgbrBase, PgbrIRCBase);
						PgbrBase1 = PgbrBase;
						if ( PgbrBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_BendingResistanceDetailResult4PlateGirder(ElemK, bPosiI, PgbrGd, PgbrBase, PgbrDetail) )
						{
							...
							PrintDesignInfo_Mod(pXL, iStRow, ElemK.first, j, RptData, SpscSect, PgbrIRCBase);
							PrintBendingResistance(pXL, iStRow, ++iChkSect, nPosiNega, PgbrGd, PgbrBase, PgbrDetail, RptData, SpscSect, PglsD, PgbrIRCBase);
							if ( PltbBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_LateralTorsionalBucklingResistanceDetailResult4PlateGirder(ElemK, bPosiI, PltbGd, PltbBase, PltbDetail) )
							{
								BOOL bPositive = PltbBase.dMy > -m_dZero ? TRUE : FALSE;
								PltbBase.dMa_Ed = PltbBase.dMa_Ed == 0. ? PgbrBase.dMa_Ed:PltbBase.dMa_Ed;
								PrintLateralBending_SteelOnly(pXL, iStRow, ++iChkSect, k, PltbGd, PltbBase, PltbDetail);
			...
			//           if(!bPositive)
			//           {
			//             if( m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_LateralTorsionalBucklingResistanceDetailResult4PlateGirder(ElemK, bPosiI, PltbGd, PltbBase, PltbDetail,TRUE))
			//             {
			...
			if ( bCheckStrength[1] ) // Shear Resistance
			{
				...
				if ( PrvsBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_VerticalShearResistanceDetailResult4PlateGirder(ElemK, bPosiI, PrvsGd, PrvsBase, PrvsDetail) )
				{
			...
			if ( bCheckStrength[3] ) // Transverse Force
			{
				...
				T_PGBR_BASE PgbrBase = PgbrBase1;
				m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_SavedResult4Design(ElemK, bPosiI, PgbrBase, PgbrIRCBasePRTF);
				//if(/*PrtfBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_TransverseForceResistanceDetailResult4PlateGirder(ElemK, bPosiI, PrtfBase, PrtfDetail)*/)
				{
					PrintTransverseForceResistance(pXL, iStRow, ++iChkSect, k, PrtfBase, PrtfDetail, PgbrIRCBasePRTF);
				}
			}
			if ( bCheckStrength[4] ) // Longitudinal Shear
			{
				...
				if ( PglsBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_LongitudinalShearResistanceDetailResult4PlateGirder(ElemK, bPosiI, PglsGd, PglsBase, PglsDetail) )
				{
					PrintLongitudinalShearResistance(pXL, iStRow, ++iChkSect, k, PglsGd, PglsBase, PglsDetail, PgbrIRCBase);
				}
			}
			if ( bCheckStrength[5] ) // Fatigue
			{
				...
				if ( PgrfBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_FatigueResistanceDetailResult4PlateGirder(ElemK, bPosiI, PgrfBase, PgrfDetail) )
				{
					PrintFatigueResistance(pXL, iStRow, ++iChkSect, k, PgrfBase, PgrfDetail, PgbrIRCBase);
				}
			}
			if ( bCheckService[0] )
			{
				...
				if ( PgbsBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_BeamStressDetailResult4PlateGirder(ElemK, bPosiI, PgbsBase, PgbsDetail) )
				{
					PrintStressLimit(pXL, iStRow, ++iChkSect, k, PgbsBase, PgbsDetail, PgbrIRCBase, bCheckService);
				}
			}
			if ( bCheckService[1] )
			{
				...
				if ( PlssBase.bChk && m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_LongitudinalShearResistanceServiceDetailResult4PlateGirder(ElemK, bPosiI, PlssGd, PlssBase, PlssDetail) )
				{
...
void CRptPlateGirder_IRC22_15::PrintTitle_Sub_Title_Shear_ResistanceChk(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintTitle_Sub_Title_Shear_Web(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintTitle_Sub_Title_Shear_LeftWeb(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintTitle_Sub_Title_Shear_Right_Web(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintTitle_Sub_Title_Stress_Limitation_Chk(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo(CMSExcel* pXL, int& iStRow, int iElemNo, int nPosi)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, _T("secprop_common"), iRowCnt, iColCnt);
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_Mod(CMSExcel* pXL, int& iStRow, int iElemNo, int nPosi, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect, T_PGBR_IRC_BASE& PgbrIRC)
{
	int iRowCnt = 0;
	int iColCnt = 0;

	CString strChapter   = _T("");
	CString strBaseSheet = _T("Sec_Prop");
	int iChapterIx = 1;
	int iSubChapterIx = 1;
	...
	////
	CString RptBlock;
	RptBlock = _T("secprop_common");  // SetValue_I_P_01  (pXL, RptBlock, ResData.Memb, stRow);

	pXL->GetCellIDByName(m_strBaseBook, strBaseSheet, RptBlock, Row_Ref, Col_Ref);
	iStRow += iRowCnt + 1;
	m_strBaseSheet = strBaseSheet;
	Row_Ref = 1; Col_Ref=-1;
	CString strCellname =  _T("Sec_prop_2");
	short Row=0, Col=0;
	//short Row_Ref=0, Col_Ref=0;
	short Row_Calc=0, Col_Calc=0;

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_2"), iElemNo, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_3"), nPosi == 0? _T("I"):_T("J"), Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_4"), _T("BEAM"), Row_Ref, Col_Ref, stRow);

	//Partial Safety factors
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_7_15"), PgbrIRC.SectProp.dGamma_cBS, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_8_15"), PgbrIRC.SectProp.dGamma_c_acci, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_9_15"), PgbrIRC.SectProp.dGamma_M0YB, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_10_15"), PgbrIRC.SectProp.dGamma_M1Ult, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_7_31"), PgbrIRC.SectProp.dGamma_s, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_8_31"), PgbrIRC.SectProp.dGamma_v, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_9_31"), PgbrIRC.SectProp.dGamma_fft, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_10_31"), PgbrIRC.SectProp.dGamma_Mft, Row_Ref, Col_Ref, stRow);
	//Section Size
	...
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_15_7"), PgbrIRC.SectProp.dBc, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_16_7"), PgbrIRC.SectProp.dtc, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_17_7"), PgbrIRC.SectProp.dHh, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_18_7"), PgbrIRC.SectProp.ddc, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_19_7"), PgbrIRC.SectProp.dfck, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_20_7"), PgbrIRC.SectProp.dEc, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_21_7"), PgbrIRC.SectProp.dfyk, Row_Ref, Col_Ref, stRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_25_7"), PgbrIRC.SectProp.dEffWidth, Row_Ref, Col_Ref, stRow);

	if ( PgbrIRC.SectProp.iEffWidthCase==0 || PgbrIRC.SectProp.iEffWidthCase==5 )
	{
		//No Span Informations
		CArray<UINT, UINT> aSpanK;
		m_pDoc->m_pAttrCtrl->GetSpanKeyList(aSpanK, 1);
		int nSize = aSpanK.GetSize();
		if ( nSize== 0 ) PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_25_10"), _T("****No Span Information***"), Row_Ref, Col_Ref, stRow);
		else PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_25_10"), _T("****No Effective Width Scale Factor***"), Row_Ref, Col_Ref, stRow);

		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_LHS"), _T("Bc"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_sign"), _T(""), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_rhs"), _T(""), Row_Ref, Col_Ref, stRow);

		//DeleteCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_sign")  , 0, Row_Ref, Col_Ref, stRow);

	}
	else if ( PgbrIRC.SectProp.iEffWidthCase==2 )
	{
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_LHS"), _T("L0/8 + X"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_sign"), _T("<="), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_rhs"), _T("B1/2 + X"), Row_Ref, Col_Ref, stRow);
		double dCalVal = (PgbrIRC.SectProp.dSLlo/8.) +  (PgbrIRC.SectProp.dBc/2.);
		dCalVal = min(dCalVal, PgbrIRC.SectProp.dBc);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_25_7"), dCalVal, Row_Ref, Col_Ref, stRow);
	}
	else if ( PgbrIRC.SectProp.iEffWidthCase==3 )
	{
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_LHS"), _T("Lo/4"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_sign"), _T("<="), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_rhs"), _T("(B1+B2)/2 + X"), Row_Ref, Col_Ref, stRow);
		double dCalVal = (PgbrIRC.SectProp.dSLlo/4.) +  (PgbrIRC.SectProp.dBc/2.);
		dCalVal = min(dCalVal, PgbrIRC.SectProp.dBc);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_25_7"), dCalVal, Row_Ref, Col_Ref, stRow);
	}
	else if ( PgbrIRC.SectProp.iEffWidthCase==4 )
	{
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_LHS"), _T("Lo/8 + X"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_sign"), _T("<="), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_rhs"), _T("B1/2 + X"), Row_Ref, Col_Ref, stRow);
		double dCalVal = (PgbrIRC.SectProp.dSLlo/8.) +  (PgbrIRC.SectProp.dBc/2.);
		dCalVal = min(dCalVal, PgbrIRC.SectProp.dBc);
	...
	//Top Layer
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_29_9"), PgbrIRC.SectProp.dTopDia, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_29_11"), PgbrIRC.SectProp.dTopSpaceing, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_29_16"), PgbrIRC.SectProp.dTopNos, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_29_18"), PgbrIRC.SectProp.dTopLayers, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_29_22"), PgbrIRC.SectProp.dTopCover, Row_Ref, Col_Ref, stRow);
	double dArea = 4.0*atan(1.0) * PgbrIRC.SectProp.dTopNos * PgbrIRC.SectProp.dTopDia * PgbrIRC.SectProp.dTopDia * PgbrIRC.SectProp.dTopLayers / 4.0;
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_29_25"), dArea, Row_Ref, Col_Ref, stRow);
	//Bot Layer
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_30_9"), PgbrIRC.SectProp.dBotDia, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_30_11"), PgbrIRC.SectProp.dBotSpaceing, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_30_16"), PgbrIRC.SectProp.dBotNos, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_30_18"), PgbrIRC.SectProp.dBotLayers, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_30_22"), PgbrIRC.SectProp.dBotCover, Row_Ref, Col_Ref, stRow);
	dArea = 4.0*atan(1.0) * PgbrIRC.SectProp.dBotNos* PgbrIRC.SectProp.dBotDia * PgbrIRC.SectProp.dBotDia * PgbrIRC.SectProp.dBotLayers / 4.0;
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_30_25"), dArea, Row_Ref, Col_Ref, stRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_33_12"), PgbrIRC.SectProp.dEta, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_33_17"), PgbrIRC.SectProp.dLambda, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_34_17"), PgbrIRC.SectProp.dfcds_BS, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_35_17"), PgbrIRC.SectProp.dfcds_Acci, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_37_17"), PgbrIRC.SectProp.dfyds_steel, Row_Ref, Col_Ref, stRow);

	//2.Girder Properties
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_41_14") , RptData.Memb.GIRDER.dbfc, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_41_7"), RptData.Memb.GIRDER.dbft, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_42_14") , RptData.Memb.GIRDER.dtfc, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_42_7"), RptData.Memb.GIRDER.dtft, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_43_7") , RptData.Memb.GIRDER.dD, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_43_14"), RptData.Memb.GIRDER.dtw, Row_Ref, Col_Ref, stRow);
	// 
	//   // °Å´õ-»óºÎÇÃ·£Áö
	CString strCompFlgThk = _T("");
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_46_10"), m_strMatNameSteel, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_46_14"), RptData.Memb.GIRDER.Top.dthick, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_46_17"), RptData.Memb.GIRDER.Top.dfy_or, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_46_20"), RptData.Memb.GIRDER.Top.dfu, Row_Ref, Col_Ref, stRow);
	strCompFlgThk = (m_dtfc < 20.0) ? _T("Less Than 20mm") : _T("Greater Than 20mm"); /*pExcel->SetValue(stRow+17, 21, strCompFlgThk);*/
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_46_23"), strCompFlgThk, Row_Ref, Col_Ref, stRow);
	// 
	//   // °Å´õ-ÇÏºÎÇÃ·£Áö
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_48_10"), m_strMatNameSteel, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_48_14"), RptData.Memb.GIRDER.Bottom.dthick, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_48_17"), RptData.Memb.GIRDER.Bottom.dfy_or, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_48_20"), RptData.Memb.GIRDER.Bottom.dfu, Row_Ref, Col_Ref, stRow);
	strCompFlgThk = (m_dtft < 20.0) ? _T("Less Than 20mm") : _T("Greater Than 20mm");// pExcel->SetValue(stRow+18,21, strCompFlgThk);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_48_23"), strCompFlgThk, Row_Ref, Col_Ref, stRow);

	//   // °Å´õ-ÇÏºÎÇÃ·£Áö
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_47_10"), m_strMatNameSteel, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_47_14"), RptData.Memb.GIRDER.Web.dthick, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_47_17"), RptData.Memb.GIRDER.Web.dfy_or, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_47_20"), RptData.Memb.GIRDER.Web.dfu, Row_Ref, Col_Ref, stRow);
	strCompFlgThk = (m_dtw < 20.0) ? _T("Less Than 20mm") : _T("Greater Than 20mm"); //pExcel->SetValue(stRow+19, 21, strCompFlgThk);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_47_23"), strCompFlgThk, Row_Ref, Col_Ref, stRow);

	double dVal = min(min(RptData.Memb.GIRDER.Top.dfy_or, RptData.Memb.GIRDER.Bottom.dfy_or), RptData.Memb.GIRDER.Web.dfy_or) / PgbrIRC.SectProp.dGamma_M0YB;
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_51_17"), dVal, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_52_17"), PgbrIRC.SectProp.dEs, Row_Ref, Col_Ref, stRow);
	double dGs = PgbrIRC.SectProp.dEs/ (2.0* (1 + 0.2));
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_53_17"), RptData.Memb.GIRDER.dSlen, Row_Ref, Col_Ref, stRow);

	//TRST Web
	switch ( PgbrIRC.TraWebStiff.iStiffCaseNo )
	{
	case 1:
	case 2:
		//For Flat print in B
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_9"), PgbrIRC.SectProp.dTRSTWebType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_11"), PgbrIRC.SectProp.dTRSTWebFy, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_18"), PgbrIRC.SectProp.dTRSTWebB, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_22"), PgbrIRC.SectProp.dTRSTWebType == 0 ? PgbrIRC.SectProp.dTRSTWebtw:PgbrIRC.SectProp.dTRSTWebtf, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_25"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_28"), PgbrIRC.SectProp.dTRSTWebc, Row_Ref, Col_Ref, stRow);
		//TRST BEaring
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_18"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_22"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_25"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_28"), _T("-"), Row_Ref, Col_Ref, stRow);
	...
	case 4:
		...
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_9"), PgbrIRC.SectProp.dTRSTWebType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_11"), PgbrIRC.SectProp.dTRSTWebFy, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_14"), PgbrIRC.SectProp.dTRSTWebH, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_18"), PgbrIRC.SectProp.dTRSTWebB, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_22"), PgbrIRC.SectProp.dTRSTWebtw, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_25"), PgbrIRC.SectProp.dTRSTWebtf, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_28"), PgbrIRC.SectProp.dTRSTWebc, Row_Ref, Col_Ref, stRow);
		//TRST BEaring
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), _T("-"), Row_Ref, Col_Ref, stRow);
	...
	case 5:
		//For Flat print in B
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_9"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_11"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_18"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_22"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_25"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_28"), _T("-"), Row_Ref, Col_Ref, stRow);
		//TRST BEaring
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_18"), PgbrIRC.SectProp.dTRSTBearingB, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_22"), PgbrIRC.SectProp.dTRSTBearingtw, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_25"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_28"), _T("-"), Row_Ref, Col_Ref, stRow);
		break;
	case 6:
		...
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_22"), PgbrIRC.SectProp.dTRSTBearingtw, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_25"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_28"), PgbrIRC.SectProp.dTRSTBearingc, Row_Ref, Col_Ref, stRow);
		break;
	case 7:
		//For Flat print in B
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_9"), _T("-"), Row_Ref, Col_Ref, stRow);
		...
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), PgbrIRC.SectProp.dTRSTBearingH, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_18"), PgbrIRC.SectProp.dTRSTBearingB, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_22"), PgbrIRC.SectProp.dTRSTBearingtw, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_25"), PgbrIRC.SectProp.dTRSTBearingtf, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_28"), _T("-"), Row_Ref, Col_Ref, stRow);
		break;
	case 8:
		//For Flat print in B
	...
	case 16:
		...
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_18"), PgbrIRC.SectProp.dTRSTWebB, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_22"), PgbrIRC.SectProp.dTRSTWebtf, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_25"), _T("-"), Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_28"), PgbrIRC.SectProp.dTRSTWebc, Row_Ref, Col_Ref, stRow);
		if ( PgbrIRC.TraWebStiff.iStiffCaseNo==9 || PgbrIRC.TraWebStiff.iStiffCaseNo==13 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		...
		else if ( PgbrIRC.TraWebStiff.iStiffCaseNo==10 || PgbrIRC.TraWebStiff.iStiffCaseNo==14 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		...
		else if ( PgbrIRC.TraWebStiff.iStiffCaseNo==11 || PgbrIRC.TraWebStiff.iStiffCaseNo==15 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), PgbrIRC.SectProp.dTRSTBearingH, Row_Ref, Col_Ref, stRow);
		...
		else if ( PgbrIRC.TraWebStiff.iStiffCaseNo==12 || PgbrIRC.TraWebStiff.iStiffCaseNo==16 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), PgbrIRC.SectProp.dTRSTBearingH, Row_Ref, Col_Ref, stRow);
	...
	case 24:
		...
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_25"), PgbrIRC.SectProp.dTRSTWebtf, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_56_28"), PgbrIRC.SectProp.dTRSTWebc, Row_Ref, Col_Ref, stRow);
		if ( PgbrIRC.TraWebStiff.iStiffCaseNo==17 || PgbrIRC.TraWebStiff.iStiffCaseNo==21 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		...
		else if ( PgbrIRC.TraWebStiff.iStiffCaseNo==18 || PgbrIRC.TraWebStiff.iStiffCaseNo==22 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), _T("-"), Row_Ref, Col_Ref, stRow);
		...
		else if ( PgbrIRC.TraWebStiff.iStiffCaseNo==19 || PgbrIRC.TraWebStiff.iStiffCaseNo==23 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), PgbrIRC.SectProp.dTRSTBearingH, Row_Ref, Col_Ref, stRow);
		...
		else if ( PgbrIRC.TraWebStiff.iStiffCaseNo==20 || PgbrIRC.TraWebStiff.iStiffCaseNo==24 )
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_9"), PgbrIRC.SectProp.dTRSTBearingType == 0 ? _T("Flat"):_T("T"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_11"), PgbrIRC.SectProp.dTRSTBearingFy, Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_57_14"), PgbrIRC.SectProp.dTRSTBearingH, Row_Ref, Col_Ref, stRow);
	...
	dVal = min(min(RptData.Memb.GIRDER.Top.dthick, RptData.Memb.GIRDER.Bottom.dthick), RptData.Memb.GIRDER.Web.dthick);
	if ( dVal > 10.0 ) PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_60_5"), _T("Plate thickness is greater than 10 mm, hence OK "), Row_Ref, Col_Ref, stRow);
	else if ( dVal > 8.0 ) PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_60_5"), _T("Plate thickness is less than 10 mm, should be accesible on both sides  "), Row_Ref, Col_Ref, stRow);
	else PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_60_5"), _T("Plate thickness should be greater than 8 mm as per Clause 504.7, IRC 24:2010"), Row_Ref, Col_Ref, stRow);
	//Shear Lag Effect
	if ( PgbrIRC.SectProp.iEffWidthCase==0 )
	{
		//No SHear Lag
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_70_5"), _T("****No Span Information. Shear Lag Effect Ignored***"), Row_Ref, Col_Ref, stRow);
	...
	{
		//Shear Lag Effect
		double dbot = RptData.Memb.GIRDER.dbfc;
		double dbob =  RptData.Memb.GIRDER.dbft;
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_63_7") , RptData.Memb.GIRDER.dbfc, Row_Ref, Col_Ref, stRow);
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_64_7"), RptData.Memb.GIRDER.dbft, Row_Ref, Col_Ref, stRow);
		double dL0 = PgbrIRC.SectProp.dSLlo;
		PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_65_7"), dL0, Row_Ref, Col_Ref, stRow);
		if ( dL0 > 0 )
		{
			if ( dbot>=dL0/20 && dbob<=dL0/20 )
			{
				//Case 2
				PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_70_5"), _T("If NG, then shear lag effect has to be considered,"), Row_Ref, Col_Ref, stRow);
				PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_70_17"), _T("btf="), Row_Ref, Col_Ref, stRow);
				PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_70_22"), dL0/20., Row_Ref, Col_Ref, stRow);

			}
			else if ( dbot<=dL0/20 && dbob>=dL0/20 )
			{
				//Case 3
				PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_71_5"), _T("If NG, then shear lag effect has to be considered,"), Row_Ref, Col_Ref, stRow);
				PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_71_17"), _T("bbf="), Row_Ref, Col_Ref, stRow);
				PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_71_22"), dL0/20., Row_Ref, Col_Ref, stRow);
		...
		else
		{
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_70_5"), _T("If NG, then shear lag effect has to be considered,"), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_70_17"), _T("btf="), Row_Ref, Col_Ref, stRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_70_22"), _T("0"), Row_Ref, Col_Ref, stRow);
	...
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_74_8") , PgbrIRC.SectProp.SteelSect.dArea, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_74_17"), PgbrIRC.SectProp.SteelSect.dIyy, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_74_26"), PgbrIRC.SectProp.SteelSect.dIzz, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_75_8") , PgbrIRC.SectProp.SteelSect.dtop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_75_17"), PgbrIRC.SectProp.SteelSect.dbot, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_76_8") , PgbrIRC.SectProp.SteelSect.dstop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_76_17"), PgbrIRC.SectProp.SteelSect.dsbot, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_76_26"), PgbrIRC.SectProp.SteelSect.dzp, Row_Ref, Col_Ref, stRow);
	//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("ds")   ,  RptData.Memb.Steel.dds       , Row_Ref, Col_Ref, stRow);

	// ´Ü¸é°è¼ö- ´Ü±âÇÕ¼º´Ü¸é
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_79_8") , PgbrIRC.SectProp.ShortTermCompo.dArea, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_79_17"), PgbrIRC.SectProp.ShortTermCompo.dIyy, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_79_26"), PgbrIRC.SectProp.ShortTermCompo.dIzz, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_80_8") , PgbrIRC.SectProp.ShortTermCompo.dtop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_80_17"), PgbrIRC.SectProp.ShortTermCompo.dbot, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_81_8") , PgbrIRC.SectProp.ShortTermCompo.dstop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_81_17"), PgbrIRC.SectProp.ShortTermCompo.dsbot, Row_Ref, Col_Ref, stRow);
	//PrintCellData(pExcel, m_strBaseBook, m_strBaseSheet_T("Sec_prop_76_26"), _T PgbrIRC.SectProp.SteelSect.dzp   (_T("ds_n"))   ,  RptData.ShortComp.dds      , Row_Ref, Col_Ref, stRow);

	// ´Ü¸é°è¼ö- Àå±âÇÕ¼º´Ü¸é
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_85_8") , PgbrIRC.SectProp.LongTermCompo.dArea, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_85_17"), PgbrIRC.SectProp.LongTermCompo.dIyy, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_85_26"), PgbrIRC.SectProp.LongTermCompo.dIzz, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_86_8") , PgbrIRC.SectProp.LongTermCompo.dtop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_86_17"), PgbrIRC.SectProp.LongTermCompo.dbot, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_87_8") , PgbrIRC.SectProp.LongTermCompo.dstop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_87_17"), PgbrIRC.SectProp.LongTermCompo.dsbot, Row_Ref, Col_Ref, stRow);

	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_90_8") , PgbrIRC.SectProp.Cracked.dArea, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_90_17"), PgbrIRC.SectProp.Cracked.dIyy, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_90_26"), PgbrIRC.SectProp.Cracked.dIzz, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_91_8") , PgbrIRC.SectProp.Cracked.dtop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_91_17"), PgbrIRC.SectProp.Cracked.dbot, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_92_8") , PgbrIRC.SectProp.Cracked.dstop, Row_Ref, Col_Ref, stRow);
	PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_92_17"), PgbrIRC.SectProp.Cracked.dsbot, Row_Ref, Col_Ref, stRow);

	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_74_8") )    ,  RptData.Memb.Steel.dA       , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_74_17")    ,  RptData.Memb.Steel.dINA       , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_74_26")  ,  RptData.Memb.Steel.dIzz     , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_75_8") ) ,  RptData.Memb.Steel.ddTop    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_75_17") ,  RptData.Memb.Steel.dSTop    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_76_8") ) ,  RptData.Memb.Steel.ddBot    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_76_17") ,  RptData.Memb.Steel.dSBot    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_76_26")   ,  RptData.Memb.Steel.dI       , Row_Ref, Col_Ref, stRow);
	//   //PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("ds")   ,  RptData.Memb.Steel.dds       , Row_Ref, Col_Ref, stRow);
	// 
	//   // ´Ü¸é°è¼ö- ´Ü±âÇÕ¼º´Ü¸é
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_79_8") )    ,  RptData.Memb.ShortComp.dA       , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_79_17")    ,  RptData.Memb.ShortComp.dINA       , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_79_26")  ,  RptData.Memb.ShortComp.dIzz     , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_80_8") ) ,  RptData.Memb.ShortComp.ddTop    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_80_17") ,  RptData.Memb.ShortComp.dSTop    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_81_8") ) ,  RptData.Memb.ShortComp.ddBot    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_81_17") ,  RptData.Memb.ShortComp.dSBot    , Row_Ref, Col_Ref, stRow);
	//   //PrintCellData(pExcel, m_strBaseBook, m_strBaseSheet_T("Sec_prop_76_26_T(", ")ds_n")   ,  RptData.ShortComp.dds      , Row_Ref, Col_Ref, stRow);
	// 
	//   // ´Ü¸é°è¼ö- Àå±âÇÕ¼º´Ü¸é
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_85_8") )    ,  RptData.Memb.LongComp.dA       , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_85_17")    ,  RptData.Memb.LongComp.dINA       , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_85_26")  ,  RptData.Memb.LongComp.dIzz     , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_86_8") ) ,  RptData.Memb.LongComp.ddTop    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_86_17") ,  RptData.Memb.LongComp.dSTop    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T(_T("Sec_prop_87_8") ) ,  RptData.Memb.LongComp.ddBot    , Row_Ref, Col_Ref, stRow);
	//   PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("Sec_prop_87_17") ,  RptData.Memb.LongComp.dSBot    , Row_Ref, Col_Ref, stRow);
		//Delete Unneccary Rows
	if ( PgbrIRC.SectProp.iEffWidthCase==0 )
	{
		//No SHear Lag
		DeleteCellData(pXL, m_strBaseBook, strBaseSheet, _T("Sec_prop_63_7"), 5, Row_Ref, Col_Ref, stRow);
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_Section(CMSExcel* pXL, int& iStRow, CString strBaseSheet, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect)
{
	// 1. Section Size
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionSize(CMSExcel* pXL, int& iStRow, CString strBaseSheet, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect)
{
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionImage_Compo_I(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionImage_Compo_Box(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionImage_Compo_Tub(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionSize_Compo_I(CMSExcel* pXL, int& iStRow, CString strBaseSheet, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::SetValue_I_P_01(CMSExcel* pExcel, CString& RptBlock, short &Row_Ref, short &Col_Ref, int& stRow, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect)
{
...
//   // °Å´õ- ¼³°è°­µµ
	...
	//   //   CString srtShape[3] = { _T("I-type_T(", ")T-type"),     _T("U-type")};
...
void CRptPlateGirder_IRC22_15::SetValue_I_P_Sect(CMSExcel* pExcel, CString& RptBlock, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, int& stRow)
{
	int rowCnt = CopyRange(pExcel, RptBlock, stRow);
...
void CRptPlateGirder_IRC22_15::SetValue_I_P_02_Title(CMSExcel* pExcel, CString& RptBlock, int& stRow)
{
	int rowCnt = CopyRange(pExcel, RptBlock, stRow);
	short Row_Ref = 0, Col_Ref = 0;
	// ±âÁØÀÌ µÇ´Â Block ½ÃÀÛÁ¡ °¡Á®¿Â´Ù.
...
void CRptPlateGirder_IRC22_15::SetValue_I_P_02_01(CMSExcel* pExcel, CString& RptBlock, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, int& stRow)
{
	int rowCnt = CopyRange(pExcel, RptBlock, stRow);
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionSize_Compo_Box(CMSExcel* pXL, int& iStRow, CString strBaseSheet, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionSize_Compo_Tub(CMSExcel* pXL, int& iStRow, CString strBaseSheet, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_SectionStiffness(CMSExcel* pXL, int& iStRow, CString strBaseSheet, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintDesignInfo_Material(CMSExcel* pXL, int& iStRow, CString strBaseSheet)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintBendingResistance(CMSExcel* pXL, int& iStRow, int iChapIndex, int nPosiNega, T_PGBR_BASE& PgbrGd, T_PGBR_BASE& PgbrBase, T_PGBR_DETAIL& PgbrDetail)
{
	//   int iRowCnt = 0;
	//   int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintBendingResistance(CMSExcel* pXL, int& iStRow, int iChapIndex, int nPosiNega, T_PGBR_BASE& PgbrGd, T_PGBR_BASE& PgbrBase, T_PGBR_DETAIL& PgbrDetail, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect, T_PGLS_D& PglsD, T_PGBR_IRC_BASE& PgbrIRCBase)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	// 	dMpl_Rd = PgbrBase.dMpl_Rd*m_dFactorMoment;

	CString strBaseSheet = _T("SLS_P");
	m_strBaseSheet = strBaseSheet;
	//pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, _T("Title_of_BendingResistance"), iRowCnt, iColCnt);
//     pXL->SetValue(iStRow, 2, _T("III"));
//     pXL->SetValue(iStRow, 3, _T("Strength Limit State"));
	pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, _T("SLS_P_NAME"), iRowCnt, iColCnt);

	iStRow += 1;

	short Row_Ref = 0, Col_Ref = 0;
	CString RptBlock = _T("SLS_P_DES") ;
	// ±âÁØÀÌ µÇ´Â Block ½ÃÀÛÁ¡ °¡Á®¿Â´Ù.
	pXL->GetCellIDByName(m_strBaseBook, m_strBaseSheet, RptBlock, Row_Ref, Col_Ref);
	CString strSubChapter=_T("");
	// Positive Moment
	...
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_4_10"), strLcomName, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_8_9"), PgbrBase.dMa_Ed*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_8_17"), PgbrBase.dMc_Ed*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_8_25"), PgbrGd.dNc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_8_28"), PgbrBase.dFx*m_dFactorForce, Row_Ref, Col_Ref, iStRow);


	if ( nPosiNega!=1 )
	{
		RptBlock = _T("SKS_N_DES") ;
		// ±âÁØÀÌ µÇ´Â Block ½ÃÀÛÁ¡ °¡Á®¿Â´Ù.
		pXL->GetCellIDByName(m_strBaseBook, m_strBaseSheet, RptBlock, Row_Ref, Col_Ref);
		CString strSubChapter=_T("");
		// Positive Moment
	...
	iStRow+=iRowCnt; iStRow += 1;

	//Minimum web and flange thk chk
	RptBlock = _T("SLS_P_THK_CHK") ;
	pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
	Row_Ref = 9; Col_Ref=0;
	//       
	//       pXL->SetValue(iStRow+2, 5, PgbrGd.dFx*m_dFactorForce);
	//       pXL->SetValue(iStRow+3, 5, PgbrBase.dFx*m_dFactorForce);
	//       pXL->SetValue(iStRow+4, 5, PgbrBase.dMa_Ed*m_dFactorMoment);
	//       pXL->SetValue(iStRow+5, 5, PgbrBase.dMc_Ed*m_dFactorMoment);
	BOOL bTrstWebStiff = PgbrIRCBase.TraWebStiff.iStiffCaseNo>=5 && PgbrIRCBase.TraWebStiff.iStiffCaseNo<=8 ? FALSE:TRUE;


	if ( PgbrIRCBase.TraWebStiff.iStiffCaseNo>0 && bTrstWebStiff )
	{
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_12_13"), _T("Yes"), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_13_13"), PgbrIRCBase.SectProp.dTRSTWebc, Row_Ref, Col_Ref, iStRow);
		double dHTR = m_dHw/m_dtw;
		bool bSpclcase = FALSE;
		if ( PgbrIRCBase.SectProp.dTRSTWebc > 0.74*m_dHw && PgbrIRCBase.SectProp.dTRSTWebc<m_dHw && PgbrIRCBase.SectProp.dTRSTWebc<1.5*m_dHw )
		{
			bSpclcase = TRUE;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_16_9"), PgbrIRCBase.SectProp.dTRSTWebc /m_dtw, Row_Ref, Col_Ref, iStRow);

		}
		else if ( PgbrIRCBase.SectProp.dTRSTWebc < 3.*m_dHw && PgbrIRCBase.SectProp.dTRSTWebc<m_dHw && PgbrIRCBase.SectProp.dTRSTWebc<1.5*m_dHw )
		{
			bSpclcase = TRUE;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_16_9"), dHTR, Row_Ref, Col_Ref, iStRow);
		}
		else PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_16_9"), dHTR, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_19_9"), dHTR, Row_Ref, Col_Ref, iStRow);

		double dLimFact1 = 200. * sqrt(250./RptData.Memb.GIRDER.Web.dfy_or);
		double dLimFact2 = 345. * sqrt(250./RptData.Memb.GIRDER.Top.dfy_or)* sqrt(250./RptData.Memb.GIRDER.Top.dfy_or);
		if ( PgbrIRCBase.SectProp.dTRSTWebc < m_dHw )
		{
			if ( !bSpclcase )
			{
				dLimFact1 = 270. * sqrt(250./RptData.Memb.GIRDER.Web.dfy_or);
				PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_16_13"), _T("270*ew"), Row_Ref, Col_Ref, iStRow);

			}
		}
		if ( PgbrIRCBase.SectProp.dTRSTWebc < 1.5*m_dHw )
		{


			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_19_13"), _T("345*ef"), Row_Ref, Col_Ref, iStRow);
			dLimFact2 = 345. * sqrt(250./RptData.Memb.GIRDER.Top.dfy_or);
		}
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_16_17"), dLimFact1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_19_17"), dLimFact2, Row_Ref, Col_Ref, iStRow);

	}
	else
	{
		//No TRST
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_12_13"), _T("No"), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_13_13"), _T("0.0"), Row_Ref, Col_Ref, iStRow);
		double dHTR = m_dHw/m_dtw;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_16_9"), dHTR, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_19_9"), dHTR, Row_Ref, Col_Ref, iStRow);
	...
	RptBlock = _T("SLS_P_SECT_CLASSI") ;
	pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
	Row_Ref = 21;
	//PgbrBase.iTopClass, PgbrBase.iWebClass, PgbrBase.iBotClass, PgbrBase.iSectClass,PgbrBase.dTopClassRat,PgbrBase.dBotClassRat,PgbrBase.dWebClassRat
	if ( max(PgbrBase.iBotClass, PgbrBase.iTopClass) == PgbrBase.iBotClass )
	{
		//Negative Moment
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_24_9"), GetClassString(PgbrBase.iBotClass), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_24_20"), PgbrBase.dBotClassRat, Row_Ref, Col_Ref, iStRow);

	}
	else
	{
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_24_9"), GetClassString(PgbrBase.iTopClass), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_24_20"), PgbrBase.dTopClassRat, Row_Ref, Col_Ref, iStRow);

	}

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_25_9"), GetClassString(PgbrBase.iWebClass), Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_25_20"), PgbrBase.dWebClassRat, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SLS_P_26_14"), GetClassString(PgbrBase.iSectClass), Row_Ref, Col_Ref, iStRow);
	...
	if ( iPNACond == 0 && PgbrBase.iSectClass == 1 ) iFinalReportCond = 1; //Case 1: PNA  in Slab  (Plastic)
	else if ( iPNACond == 1 && PgbrBase.iSectClass == 1 ) iFinalReportCond = 2; //Case 2: PNA  in Flange  (Plastic)
	else if ( iPNACond == 2 && PgbrBase.iSectClass == 1 ) iFinalReportCond = 3; //Case 2: PNA  in Web  (Plastic)
	else if ( iPNACond == 0 && PgbrBase.iSectClass == 2 ) iFinalReportCond = 4; //Case 4: PNA  in Slab  (Comapact)
	else if ( iPNACond == 1 && PgbrBase.iSectClass == 2 ) iFinalReportCond = 5; //Case 5: PNA  in Flange  (Comapact)
	else if ( iPNACond == 2 && PgbrBase.iSectClass == 2 ) iFinalReportCond = 6; //Case 6: PNA  in Web  (Comapact)
	else if ( iPNACond == 0 && PgbrBase.iSectClass == 3 ) iFinalReportCond = 7; //Case 7: PNA  in Slab  (Semi - Comapact)
	else if ( iPNACond == 1 && PgbrBase.iSectClass == 3 ) iFinalReportCond = 8; //Case 8: PNA  in Flange  (Semi - Comapact)
	else if ( iPNACond == 2 && PgbrBase.iSectClass == 3 ) iFinalReportCond = 9; //Case 9: PNA  in Web  (Semi - Comapact)
	else ASSERT(0);



	m_nCase_PNA = iFinalReportCond;
	m_dY_Bar_PNA = PgbrBase.dPlasticNA;
	int iClassSect = PgbrBase.iSectClass;

	if ( nPosiNega==1 ) // Positive
	{
		if ( iFinalReportCond==1 )
		{
			RptBlock = _T("SLS_P_CASE1") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 28;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_fc"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_fctot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_ftot"), PgbrDetail.dNg_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_pnares") , _T("PNA in the Slab") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_xu"), PgbrBase.dDistT, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_fcc"), PgbrDetail.dNg_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_ast"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE1_mplrdcal"), (PgbrBase.dMpl_Rd)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==2 )
		{
			RptBlock = _T("SLS_P_CASE2") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 78;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_fc"), PgbrDetail.dN_slab*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_fsctop"), PgbrBase.dNa_topc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_fsttop"), PgbrBase.dNa_topt*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_pnares") , _T("PNA in the Top Flange") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_xu"), PgbrBase.dXpl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_asc"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_ast"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_y"), PgbrBase.dDistW2, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_fctot"), (PgbrDetail.dN_slab+PgbrBase.dNa_top)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_ftot") , (PgbrBase.dNa_bot*m_dFactorForce+PgbrBase.dNa_web*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_mplrdcal") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE2_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);



		}
		else if ( iFinalReportCond==3 )
		{
			RptBlock = _T("SLS_P_CASE3") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 126;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_fc"), PgbrDetail.dN_slab*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_asctop"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_ascweb"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_ast"), PgbrBase.dDistB, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_pnares") , _T("PNA in the Web") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_xu"), PgbrBase.dXpl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_y") , PgbrBase.dDistW, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_fscweb"), PgbrBase.dNa_webc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_fstweb"), PgbrBase.dNa_webt*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_fctot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_ftot") , PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_mplrdcal") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE3_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==4 )
		{

			RptBlock = _T("SLS_P_CASE4") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 175;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_fc"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_fctot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_ftot"), PgbrDetail.dNg_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_pnares") , _T("PNA in the Slab") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_xu"), PgbrBase.dDistT, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_fcc"), PgbrDetail.dNg_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_ast"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE4_mplrdcal"), (PgbrBase.dMpl_Rd)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==5 )
		{
			RptBlock = _T("SLS_P_CASE5") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 224;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_fc"), PgbrDetail.dN_slab*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_fsctop"), PgbrBase.dNa_topc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_fsttop"), PgbrBase.dNa_topt*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_pnares") , _T("PNA in the Top Flange") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_xu"), PgbrBase.dXpl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_asc"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_ast"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_y"), PgbrBase.dDistW2, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_fctot"), (PgbrDetail.dN_slab+PgbrBase.dNa_top)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_ftot") , (PgbrBase.dNa_bot*m_dFactorForce+PgbrBase.dNa_web*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_mplrdcal") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE5_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==6 )
		{
			RptBlock = _T("SLS_P_CASE6") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 273;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_fc"), PgbrDetail.dN_slab*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_asctop"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_ascweb"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_ast"), PgbrBase.dDistB, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_pnares") , _T("PNA in the Web") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_xu"), PgbrBase.dXpl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_y") , PgbrBase.dDistW, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_fscweb"), PgbrBase.dNa_webc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_fstweb"), PgbrBase.dNa_webt*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_fctot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_ftot") , PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_mplrdcal") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE6_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==7 )
		{
			RptBlock = _T("SLS_P_CASE7") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 322;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_fc"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_fctot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_ftot"), PgbrDetail.dNg_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_pnares") , _T("PNA in the Slab") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_xu"), PgbrBase.dDistT, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_fcc"), PgbrDetail.dNg_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_ast"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE7_mplrdcal"), (PgbrBase.dMpl_Rd)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==8 )
		{
			RptBlock = _T("SLS_P_CASE8") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 370;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_fc"), PgbrDetail.dN_slab*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_fsctop"), PgbrBase.dNa_topc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_fsttop"), PgbrBase.dNa_topt*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_pnares") , _T("PNA in the Top Flange") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_xu"), PgbrBase.dXpl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_asc"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_ast"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_y"), PgbrBase.dDistW2, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_fctot"), (PgbrDetail.dN_slab+PgbrBase.dNa_top)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_ftot") , (PgbrBase.dNa_bot*m_dFactorForce+PgbrBase.dNa_web*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_mplrdcal") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE8_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==9 )
		{
			RptBlock = _T("SLS_P_CASE9") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 419;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_fc"), PgbrDetail.dN_slab*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_ac"), PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_asctop"), PgbrBase.dDistTc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_ascweb"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_ast"), PgbrBase.dDistB, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_pnares") , _T("PNA in the Web") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_xu"), PgbrBase.dXpl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_y") , PgbrBase.dDistW, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_fscweb"), PgbrBase.dNa_webc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_fstweb"), PgbrBase.dNa_webt*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_fctot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_ftot") , PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_mplrdcal") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_P_CASE9_med"), (PgbrBase.dMa_Ed + PgbrBase.dMc_Ed)*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);

		}
		iStRow+=iRowCnt+1;
		//Ignore Negative Bending
		RptBlock = _T("SLS_P_BR_NEG_IGNORED") ;
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);

	}
	else //Negative
	{
		if ( iFinalReportCond==3 ) //PNA in web (Plastic) (Negative)
		{
			RptBlock = _T("SLS_N_CASE1") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 468;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_fc"), PgbrDetail.dNr_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_fsttot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_fstot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_pnares") , _T("PNA in the Web") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_pnaalone"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_y") , PgbrBase.dDistW, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_z") , PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_zp") , PgbrBase.dDistT, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_mplrd") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE1_med"), (abs(PgbrBase.dMa_Ed) + abs(PgbrBase.dMc_Ed))*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==2 ) //PNA in Flange (Plastic) (Negative)
		{
			RptBlock = _T("SLS_N_CASE2") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 511;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_fc"), PgbrDetail.dNr_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_fsttot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_fstot"), (PgbrBase.dNa_bot+PgbrBase.dNa_web)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_pnares") , _T("PNA in the Flange") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_pnaalone"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_y") , PgbrBase.dDistW, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_z") , PgbrBase.dDistB, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_zp") , PgbrBase.dNa_botc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_mplrd") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE2_med"), (abs(PgbrBase.dMa_Ed) + abs(PgbrBase.dMc_Ed))*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==6 ) //PNA in web (Compact) (Negative)
		{
			RptBlock = _T("SLS_N_CASE3") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 551;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_fc"), PgbrDetail.dNr_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_fsttot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_fstot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_pnares") , _T("PNA in the Web") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_pnaalone"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_y") , PgbrBase.dDistW, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_z") , PgbrBase.dDistWc1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_zp") , PgbrBase.dNa_botc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_mplrd") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE3_med"), (abs(PgbrBase.dMa_Ed) + abs(PgbrBase.dMc_Ed))*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond==5 ) //PNA in Flange (Compact) (Negative)
		{
			RptBlock = _T("SLS_N_CASE4") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 593;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_fc"), PgbrDetail.dNr_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_fstop"), PgbrBase.dNa_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_fsweb"), PgbrBase.dNa_web*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_fsbot"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_fsttot"), PgbrDetail.dNg_top*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_fstot"), (PgbrBase.dNa_bot+PgbrBase.dNa_web)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			//PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_pnares") , _T("PNA in the Flange") , Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_pnaalone"), PgbrBase.dDistTt, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_y") , PgbrBase.dDistW, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_z") , PgbrBase.dDistB, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_zp") , PgbrBase.dNa_botc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_mplrd") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE4_med"), (abs(PgbrBase.dMa_Ed) + abs(PgbrBase.dMc_Ed))*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		else if ( iFinalReportCond>6 ) //PNA in Flange (Compact) (Negative)
		{
			RptBlock = _T("SLS_N_CASE5") ;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
			Row_Ref = 635;

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE5_xu"), PgbrDetail.dNg_top, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE5_xe"), PgbrBase.dNa_top, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE5_ic"), PgbrBase.dNa_web, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE5_fs"), PgbrBase.dNa_bot*m_dFactorForce, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE5_melrd") , PgbrBase.dMpl_Rd*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, m_strBaseSheet, _T("SLS_N_CASE5_med"), (abs(PgbrBase.dMa_Ed) + abs(PgbrBase.dMc_Ed))*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);


		}
		iStRow+=iRowCnt+1;
		//Ignore Positive Bending
		RptBlock = _T("SLS_P_BR_POS_IGNORED") ;
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
	}


	//Resistance to steel only bending moment
//    RptBlock = _T(_T("SLS_P_RLBM_STLONLY")   );
//    pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
	iStRow+=iRowCnt+1;
...
void CRptPlateGirder_IRC22_15::PrintStressInformationDB(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_STRESS_INFO_I& StrInfoD)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintStressInformation(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_GLINE_DGN_D& GLineDgn, T_DLINE_CALC_D& StressInfoD)
{
	...
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_GLineDgnPartData(GLineDgn, aTopFlgPartLine, aBotFlgPartLine, aWebPartLine);
	...
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_DLinePartData(StressInfoD, aTopFlangePart, aBotFlangePart, aWebPart);
...
void CRptPlateGirder_IRC22_15::PrintStressInfo4Class(CMSExcel* pXL, int& iStRow, CArray<T_GLINE_PART_DGN, T_GLINE_PART_DGN&>& aPartLine, CArray<T_DLINE_PART_D, T_DLINE_PART_D&>& aDgnLinePartD)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	for ( i=0; i<iPartSize; i++ )
	{
		...
		if ( iInternalSize>0 )
		{
			...
			for ( j=0; j<iInternalSize; j++ )
			{
				...
				m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_LineEndStress4LineDgnBase(DgnLineLBD.aDgnBaseD, dStrEnd1, dStrEnd2);
...
void CRptPlateGirder_IRC22_15::PrintClassificationofSection(CMSExcel* pXL, int& iStRow, int iTopCS, int iWebCS, int iBotCS, int iSectCS)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	// Classification of sections
...
void CRptPlateGirder_IRC22_15::PrintClassificationofSectionDetail(CMSExcel* pXL, int& iStRow, int iTopCS, int iWebCS, int iBotCS, int iSectCS, double dTopClassRat, double dBotClassRat, double dWebClassRat)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	// Classification of sections
...
void CRptPlateGirder_IRC22_15::PrintShearResistance(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PRVS_BASE& PrvsGd, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_ShearDgnForce(CMSExcel* pXL, int& iStRow, T_PRVS_BASE& PrvsGd, T_PRVS_BASE& PrvsBase)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	strBaseSheet = _T("Shear_Resistance"); CString RptBlock = _T("Shear_Resist_PRT1") ;
	pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
	short Row_Ref = 0, Col_Ref = 0;
	pXL->GetCellIDByName(m_strBaseBook, strBaseSheet, RptBlock, Row_Ref, Col_Ref); Col_Ref-=1;

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT1_LCOM"), strLcomName, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT1_maed"), PrvsBase.dMa_Ed*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT1_vaed"), PrvsGd.dFz*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT1_mced"), PrvsBase.dMy*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT1_vced"), PrvsBase.dFz*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT1_n"), PrvsBase.dFx*m_dFactorForce, Row_Ref, Col_Ref, iStRow);


	iStRow += iRowCnt + 1;
}

void CRptPlateGirder_IRC22_15::PrintShearResistance_Vpl_Rd_Calc(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, T_PRVS_BASE& PrvsBase)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Vpl_Rd_ChkRatio(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, T_PRVS_BASE& PrvsBase)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Vbw_Rd_Calc(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, T_PRVS_BASE& PrvsBase)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Vbw_Rd_Calc_Sum(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, T_PRVS_BASE& PrvsBase)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Vbw_Rd_Calc_Part(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, T_PRVS_Vbw_Rd_Param& Vbw_Rd_Param)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_PlasticRestance(CMSExcel* pXL, int& iStRow, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	CString strBaseSheet = _T("Shear_Resistance");
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_ShearBucklingCoeff(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_PRVS_BASE& PrvsGd, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	strBaseSheet = _T("Shear_Resistance");
	if ( PrvsBase.iTopClass == 0 )
	{
		//Case 0

		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE1"), iRowCnt, iColCnt);
		short Row_Ref = 0; short Col_Ref=0;

		Row_Ref = 239;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE1_dbyt"), PrvsBase.dEta1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE1_67e"), PrvsBase.dEta1_lim, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE1_vplrdcal"), PrvsBase.dEta3*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE1_vced"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE1_av"), PrvsBase.dIsl_org, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
		//if (PrvsBase.dEta1 <= PrvsBase.dEta1_lim )
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PSR"), iRowCnt, iColCnt);
			Row_Ref = 393;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PSR_vplrd"), PrvsBase.dVpl_Rd*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PSR_vced"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PSR_av"), PrvsBase.dAv, Row_Ref, Col_Ref, iStRow);
			iStRow += iRowCnt;
		}
		...
		if (m_bPBTFA4SH)
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4"), iRowCnt, iColCnt);
			Row_Ref = 316;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_cbyd"), PrvsBase.dtf, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_vtf"), abs(PrvsBase.dVb_Rd_org*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_vplrd"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_taucre1"), abs(PrvsBase.dXw), Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_kv1"), PrvsBase.dMax_Bar_Lamda_w, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_lambdaw1"), PrvsBase.dc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_taub1"), PrvsBase.dbf, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_vp"), PrvsBase.dEta3*m_dFactorForce, Row_Ref, Col_Ref, iStRow);


			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_fv"), PrvsBase.DetailResD[0].dbf, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_psi"), PrvsBase.DetailResD[0].dc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_phi"), PrvsBase.DetailResD[0].dIsl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_wtf"), PrvsBase.DetailResD[0].dIsl_org, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_sc"), PrvsBase.DetailResD[0].dMax_Bar_Lamda_w, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_mfrc"), PrvsBase.DetailResD[0].dtf*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_st"), PrvsBase.DetailResD[1].dbf, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_mfrt"), PrvsBase.DetailResD[1].dc*m_dFactorMoment, Row_Ref, Col_Ref, iStRow);

			iStRow += iRowCnt;
		}

	}
	else if ( PrvsBase.iTopClass == 1 )
	{
		//Case 1

		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE2"), iRowCnt, iColCnt);
		short Row_Ref = 0; short Col_Ref=0;

		Row_Ref = 252;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE2_dbyt"), PrvsBase.dEta1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE2_67e"), PrvsBase.dEta1_lim, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE2_vplrd"), PrvsBase.dEta3*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE2_vced"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE2_av"), PrvsBase.dIsl_org, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
		//if (PrvsBase.dEta1 <= PrvsBase.dEta1_lim )
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PSR"), iRowCnt, iColCnt);
	...
	else if ( PrvsBase.iTopClass == 2 )
	{
		//Case 2

		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3"), iRowCnt, iColCnt);
		short Row_Ref = 0; short Col_Ref=0;

		Row_Ref = 267;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_dbyt"), PrvsBase.dEta1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_67e"), PrvsBase.dEta1_lim, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_vplrd"), PrvsBase.dEta3*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_vced"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_av"), PrvsBase.dIsl_org, Row_Ref, Col_Ref, iStRow);

		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_vcr"), abs(PrvsBase.dIsl*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_vcrvced"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_taucre"), abs(PrvsBase.dXw), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_kv"), PrvsBase.dMax_Bar_Lamda_w, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_lambdaw"), PrvsBase.dc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE3_taub"), PrvsBase.dbf, Row_Ref, Col_Ref, iStRow);
	...
	else if ( PrvsBase.iTopClass == 3 )
	{
		//Case 3

		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A"), iRowCnt, iColCnt);
		short Row_Ref = 0; short Col_Ref=0;

		Row_Ref = 292;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_dbyt"), PrvsBase.dEta1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_67e"), PrvsBase.dEta1_lim, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_vplrd"), PrvsBase.dEta3*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_vced"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_av"), PrvsBase.dIsl_org, Row_Ref, Col_Ref, iStRow);

		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_vcr"), abs(PrvsBase.dIsl*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_vcrvced"), abs(PrvsBase.dV_Ed*m_dFactorForce), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_taucre"), abs(PrvsBase.dXw), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_kv"), PrvsBase.dMax_Bar_Lamda_w, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_lambdaw"), PrvsBase.dc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE4A_taub"), PrvsBase.dbf, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt+1;
		//Tension Field
	...
	else if ( PrvsBase.iTopClass == 4 )
	{
		//Case 4 Transverse stiffener is provided Rigid End  not provided 
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASE_NEW"), iRowCnt, iColCnt);
		short Row_Ref = 0; short Col_Ref=0;

		Row_Ref = 383;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASENEW_dbyt"), PrvsBase.dEta1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASENEW_67e"), PrvsBase.dEta1_lim, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_PRT2_CASENEW_kv"), PrvsBase.dMax_Bar_Lamda_w, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
		...
		//Rigid End Is Not Provided - Revise section
		//Asked to Remove(Shear_Resist_ShrBuckling_FAIL) Pratap/Sindhu 10.6.2020
		//pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_ShrBuckling_FAIL"), iRowCnt, iColCnt);
		//iStRow += iRowCnt;
	...
	//Interaction Part
	if ( PrvsBase.bEta1Ratio )
	{
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_NoHighShear"), iRowCnt, iColCnt);
		short Row_Ref = 0; short Col_Ref=0;

		Row_Ref = 347;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_NoHighShear_vrd"), abs(PrvsBase.dV_Rd)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_NoHighShear_ved"), abs(PrvsBase.dV_Ed)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_NoHighShear_rat"), PrvsBase.dRatio, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;

		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN"), iRowCnt, iColCnt);
		Row_Ref = 372;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_nrd"), abs(PrvsBase.dVbf_Rd)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_nrdlim"), abs(PrvsBase.dFx)*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_interaction"), (abs(PrvsBase.dFx)/abs(PrvsBase.dVbf_Rd))+PrvsBase.DetailResD[1].dIsl_org, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_ag"), PrvsBase.DetailResD[1].dIsl, Row_Ref, Col_Ref, iStRow);
	...
	{
		//High Shear Condition Present
		short Row_Ref = 0; short Col_Ref=0;
		if ( PrvsBase.iSectClass == 3 )//Section class 3
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_4"), iRowCnt, iColCnt);


			Row_Ref = 399;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_vrd_4"), PrvsBase.dV_Rd*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_ved_4"), PrvsBase.dV_Ed*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_rat_4"), PrvsBase.dRatio, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_mdv_4"), PrvsBase.DetailResD[1].dVbf_Rd*m_dFactorForce*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_mdvlim_4"), PrvsBase.DetailResD[1].dVbw_Rd*m_dFactorForce*m_dFactorForce*m_dFactorForce, Row_Ref, Col_Ref, iStRow);

		}
		else
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear"), iRowCnt, iColCnt);


			Row_Ref = 356;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_vrd"), PrvsBase.dV_Rd*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_ved"), PrvsBase.dV_Ed*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_rat"), PrvsBase.dRatio, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_mdv"), PrvsBase.DetailResD[1].dVbf_Rd*m_dFactorForce*m_dFactorForce*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_mdvlim"), PrvsBase.DetailResD[1].dVbw_Rd*m_dFactorForce*m_dFactorForce*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_beta"), PrvsBase.DetailResD[1].dMax_Bar_Lamda_w, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_HighShear_mfd"), PrvsBase.DetailResD[1].dtf*m_dFactorForce*m_dFactorForce*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		}

		iStRow += iRowCnt;
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN"), iRowCnt, iColCnt);
		Row_Ref = 372;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_nrd"), PrvsBase.dVbf_Rd*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_nrdlim"), PrvsBase.dFx*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_ag"), PrvsBase.DetailResD[1].dIsl, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Shear_Resist_MN_interaction"), PrvsBase.DetailResD[1].dIsl_org, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
	}
...
//       if (d_Alpha<1.0)
//       {
//         RptBlock = _T(_T("I_SH_02_Classify")     ); 
//         pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
//         short Row_Ref = 0, Col_Ref = 0;
...
//       RptBlock = _T(_T("I_SH_02_Vn_b_01")     ); 
//       pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
//       short Row_Ref = 0, Col_Ref = 0;
//       pXL->GetCellIDByName(m_strBaseBook, m_strBaseSheet, RptBlock, Row_Ref, Col_Ref);Col_Ref-=1;
...
//       double dPart2 = PrvsBase.dAv * dPart1;
//       RptBlock = _T(_T("I_SH_02_Vn_b_01_b")     ); 
//       pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
//       short Row_Ref = 0, Col_Ref = 0;
//       pXL->GetCellIDByName(m_strBaseBook, m_strBaseSheet, RptBlock, Row_Ref, Col_Ref);Col_Ref-=1;
...
//       double dPart2 = PrvsBase.dAv * dPart1;
//       RptBlock = _T(_T("I_SH_02_Vn_b_01_a")     ); 
//       pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
//       short Row_Ref = 0, Col_Ref = 0;
//       pXL->GetCellIDByName(m_strBaseBook, m_strBaseSheet, RptBlock, Row_Ref, Col_Ref);Col_Ref-=1;
...
//  
//   CString RptBlock = _T(_T("I_SH_02_Vn_c")     ); 
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_SBC_type1_lessthan_1(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, const T_PRVS_Vbw_Rd_Param& rVbwRdParam)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_SBC_type1_morethan_1(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, const T_PRVS_Vbw_Rd_Param& rVbwRdParam)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_SBC_type1_calc_param(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, const T_PRVS_Vbw_Rd_Param& rVbwRdParam)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_SBC_type2(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, const T_PRVS_Vbw_Rd_Param& rVbwRdParam)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Vbf_Rd_Calc(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow,
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Vbf_Rd_Calc_Sum(CMSExcel* pXL, const CString& strBaseSheet, int& iStRow, T_PRVS_BASE& PrvsBase)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_WebShearBucklingCoeff_GenSect(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_FlangeShearBucklingCoeff_GenSect(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_PRVS_BASE& PrvsGd, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_ContributionWeb(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_SubPanel_Lamda_w(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_PRVS_Vbw_Rd_Sub_Panel& VbwSubPanel)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_ContributionFlange(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Interation_M_V(CMSExcel* pXL, int& iStRow, CString strBaseSheet, int iClassSect, double deta1, double dEta1_lim, double deta3, double dRatio)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintShearResistance_Mf_Rd(CMSExcel* pXL, int& iStRow, CString strBaseSheet, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::PrintLateralTorsionalBucklingResistance(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PLTB_BASE& PltbGd, T_PLTB_BASE& PltbBase, T_PLTB_DETAIL& PltbDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	short Row_Ref=0, Col_Ref=0;
	...
	CString RptBlock = _T("LTB_PRT1") ;
	pXL->CopyRange(iStRow, 3, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
	Row_Ref = 108;
	Col_Ref-=2;
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betab"), PltbBase.dkc, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_llt"), PltbBase.dk1, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_it"), PltbBase.dRatio, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_iw"), PltbBase.dcc, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betaf"), PltbBase.dX_LT_org, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betaf_ifc"), PltbBase.dI_at, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betaf_ift"), PltbBase.dI_afz, Row_Ref, Col_Ref, iStRow);


	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_mcr"), PltbBase.dgamma, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_lambdalt"), PltbBase.dMu, Row_Ref, Col_Ref, iStRow);
	iStRow += iRowCnt;
	if ( PltbBase.dMu < 0.4 )
	{
		RptBlock = _T("LTB_PRT1_res") ;
		pXL->CopyRange(iStRow, 3, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
		iStRow += iRowCnt;
	}
	else
	{
		RptBlock = _T("LTB_PRT2") ;
		pXL->CopyRange(iStRow, 3, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
		Row_Ref = 125;
		Col_Ref-=1;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_philt"), PltbBase.dPHI, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_zetalt"), PltbBase.dm1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_mpl"), abs(PltbBase.dM_Rd) * m_dFactorForce/1000., Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_mr"), abs(PltbBase.dM1) * m_dFactorForce / 1000., Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
	}
	//For steel only section
	BOOL bPositive = PltbBase.dMy > -m_dZero ? TRUE : FALSE;
	if ( !bPositive )
	{
		RptBlock = _T("LTB_PRT3") ;
		pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
		Col_Ref=0;
		Row_Ref = 136;
		Col_Ref-=1;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT3_mrdcal"), PltbBase.dk2* m_dFactorForce / 1000, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT3_betab"), PltbBase.dkc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT3_mrd"), abs(PltbBase.dMa_Ed) * m_dFactorForce / 1000., Row_Ref, Col_Ref, iStRow);

		iStRow += iRowCnt;
		RptBlock = _T("LTB_PRT1") ;
		pXL->CopyRange(iStRow, 3, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
		Row_Ref = 108;
		Col_Ref=0; Col_Ref-=2;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betab"), PltbBase.dkc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_llt"), PltbBase.dm_min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_it"), PltbBase.dRatio, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_iw"), PltbBase.dcc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betaf"), PltbBase.dX_LT_org, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betaf_ifc"), PltbBase.dI_at, Row_Ref, Col_Ref, iStRow);
		...
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_mcr"), PltbBase.dgamma, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_lambdalt"), PltbBase.dAwc, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
		if ( PltbBase.dMu < 0.4 )
		{
			RptBlock = _T("LTB_PRT1_res") ;
		...
		else
		{
			...
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_zetalt"), PltbBase.dm1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_mpl"), abs(PltbBase.dM_Rd) * m_dFactorForce/1000., Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_mr"), abs(PltbBase.dMa_Ed) * m_dFactorForce / 1000., Row_Ref, Col_Ref, iStRow);
			iStRow += iRowCnt;
		}
	}

	// Title
/*  pXL->CopyRange(iStRow, 1, m_strBaseBook, _T("I_Sect_T(", ")Title_of_LTBResistance"), iRowCnt, iColCnt);*/
...
//   CString RptBlock = _T(_T("LTB_Common")     );   
//   pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
//   short Row_Ref = 0, Col_Ref = 0; 
//   // ±âÁØÀÌ µÇ´Â Block ½ÃÀÛÁ¡ °¡Á®¿Â´Ù.
//   pXL->GetCellIDByName(m_strBaseBook, strBaseSheet, RptBlock, Row_Ref, Col_Ref);
...
void CRptPlateGirder_IRC22_15::PrintLateralBending_SteelOnly(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PLTB_BASE& PltbGd, T_PLTB_BASE& PltbBase, T_PLTB_DETAIL& PltbDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	short Row_Ref=0, Col_Ref=0;
	...
	//if (bPositive)
	{
		...
		Row_Ref = 136;
		Col_Ref-=1;
		double dBetab = PltbBase.dkc;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT3_mrdcal"), PltbBase.dk2* m_dFactorForce / 1000, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT3_betab"), dBetab, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT3_mrd"), abs(PltbBase.dMa_Ed) * m_dFactorForce / 1000., Row_Ref, Col_Ref, iStRow);
		...
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betab"), dBetab, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_llt"), PltbBase.dm_min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_it"), PltbBase.dRatio, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_iw"), PltbBase.dcc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_betaf"), PltbBase.dX_LT_org, Row_Ref, Col_Ref, iStRow);
		...
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_mcr"), PltbBase.dgamma*m_dFactorForce*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT1_lambdalt"), PltbBase.dAwc, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
		if ( PltbBase.dMu < 0.4 )
		{
		...
		else
		{
			...
			Row_Ref = 125;
			Col_Ref-=1;
			double dMplBuck = PltbBase.dm1 * PltbBase.dk2;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_philt"), PltbBase.dPHI, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_zetalt"), PltbBase.dm1, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_mpl"), abs(dMplBuck) * m_dFactorForce/1000., Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LTB_PRT2_mr"), abs(PltbBase.dMa_Ed) * m_dFactorForce / 1000., Row_Ref, Col_Ref, iStRow);
			iStRow += iRowCnt;
		}
	}
...
void CRptPlateGirder_IRC22_15::PrintLateralTorsionalBucklingResistance_Neg(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PLTB_BASE& PltbGd, T_PLTB_BASE& PltbBase, T_PLTB_DETAIL& PltbDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	//if (PltbBase.dM_Ed<m_dZero) return;
...
void CRptPlateGirder_IRC22_15::PrintTransverseForceResistance(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PRTF_BASE& PrtfBase, T_PRTF_DETAIL& PrtfDetail, T_PGBR_IRC_BASE& PgbrIRCBasePRTF)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	// Title
	pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("TWS_PRT1"), iRowCnt, iColCnt);
	short Row_Ref = 0; short Col_Ref=0;

	Row_Ref = 83;
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT1_H"), PgbrIRCBasePRTF.TraWebStiff.dMinH, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT1_20tq"), PgbrIRCBasePRTF.TraWebStiff.d20tqe, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT1_14tq"), PgbrIRCBasePRTF.TraWebStiff.d14tqe, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT1_heff"), PgbrIRCBasePRTF.TraWebStiff.dMinHeff, Row_Ref, Col_Ref, iStRow);

	if ( PgbrIRCBasePRTF.TraWebStiff.iBearingStiffNos == 0 )  PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT1_stifftype"), _T("One Side"), Row_Ref, Col_Ref, iStRow);
	else PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT1_stifftype"), _T("Two Side"), Row_Ref, Col_Ref, iStRow);

	iStRow += iRowCnt;
	if ( PgbrIRCBasePRTF.TraWebStiff.iCbyDcond == 0 )
	{
		//Greater than sqrt(2)
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("TWS_PRT2_CASE1"), iRowCnt, iColCnt);
		Row_Ref = 92;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT2_CASE1_iys"), PgbrIRCBasePRTF.TraWebStiff.dIys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT2_CASE1_075dtw3"), PgbrIRCBasePRTF.TraWebStiff.d075dtw3, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
	}
	else
	{
		//Less than sqrt(2)
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("TWS_PRT2_CASE2"), iRowCnt, iColCnt);
		Row_Ref = 95;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT2_CASE2_iys"), PgbrIRCBasePRTF.TraWebStiff.dIys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT2_CASE2_15d2tw3c2"), PgbrIRCBasePRTF.TraWebStiff.d075dtw3, Row_Ref, Col_Ref, iStRow);
		iStRow += iRowCnt;
	}
	//Start from buckling chk

	if ( PgbrIRCBasePRTF.TraWebStiff.iStiffCaseNo <= 4 )
	{
		//No Bearing Stiffener

		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("TWS_PRT3"), iRowCnt, iColCnt);
		Row_Ref = 99;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_fqd"), PgbrIRCBasePRTF.TraWebStiff.dFqd*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_fq"), PgbrIRCBasePRTF.TraWebStiff.dFqdLim*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_hdashw"), PgbrIRCBasePRTF.TraWebStiff.dhdashw, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_aeff"), PgbrIRCBasePRTF.TraWebStiff.dAeff, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_iys"), PgbrIRCBasePRTF.TraWebStiff.dIysBuck, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_rys"), PgbrIRCBasePRTF.TraWebStiff.drys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_kl"), PgbrIRCBasePRTF.TraWebStiff.dKL, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_fcc"), PgbrIRCBasePRTF.TraWebStiff.dFcc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_lambda"), PgbrIRCBasePRTF.TraWebStiff.dLambda, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_phi"), PgbrIRCBasePRTF.TraWebStiff.dPhi, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_x"), PgbrIRCBasePRTF.TraWebStiff.dX, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_fcd"), PgbrIRCBasePRTF.TraWebStiff.dfcd, Row_Ref, Col_Ref, iStRow);

		//No Bearing Stiffener

		iStRow += iRowCnt;

		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_NO_BEARING"), iRowCnt, iColCnt);

	}
	else if ( PgbrIRCBasePRTF.TraWebStiff.iStiffCaseNo <= 8 )
	{

		//No Intermediate Stiffener
		pXL->CopyRange(iStRow-9, 0, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_NO_INTERMEDIATE"), iRowCnt, iColCnt);
		iStRow += iRowCnt-15;
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BEARING"), iRowCnt, iColCnt);
		Row_Ref = 117;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_lcomname"), (PgbrIRCBasePRTF.TraWebStiff.strLcombName), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_r"), PgbrIRCBasePRTF.TraWebStiff.dReaction*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_fcdw"), PgbrIRCBasePRTF.TraWebStiff.dFcdw*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_fcdwlim"), PgbrIRCBasePRTF.TraWebStiff.dReaction*m_dFactorForce, Row_Ref, Col_Ref, iStRow);

		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_b1"), PgbrIRCBasePRTF.TraWebStiff.db1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_n1"), PgbrIRCBasePRTF.TraWebStiff.dn1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_b1plusn1"), PgbrIRCBasePRTF.TraWebStiff.dn1+PgbrIRCBasePRTF.TraWebStiff.db1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_a"), PgbrIRCBasePRTF.TraWebStiff.dA, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_iys"), PgbrIRCBasePRTF.TraWebStiff.dBearingIys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_rys"), PgbrIRCBasePRTF.TraWebStiff.dBearingrys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_fcc"), PgbrIRCBasePRTF.TraWebStiff.dBearingfcc, Row_Ref, Col_Ref, iStRow);

		//Chk for local crushing of web
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_LC_fcdw"), PgbrIRCBasePRTF.TraWebStiff.dFcdwCrushing*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_LC_fcdwlim"), PgbrIRCBasePRTF.TraWebStiff.dReaction*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_LC_n2"), PgbrIRCBasePRTF.TraWebStiff.dCrushing_n2, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_LC_b1plusn2"), (PgbrIRCBasePRTF.TraWebStiff.dCrushing_n2 +PgbrIRCBasePRTF.TraWebStiff.db1), Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_LC_a"), PgbrIRCBasePRTF.TraWebStiff.dCrushing_A, Row_Ref, Col_Ref, iStRow);

		//Bearing Chk
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BC_fpsd"), PgbrIRCBasePRTF.TraWebStiff.dFpsd*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BC_fpsdlim"), PgbrIRCBasePRTF.TraWebStiff.dReaction*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BC_n"), PgbrIRCBasePRTF.TraWebStiff.dBearing_n, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BC_aq"), PgbrIRCBasePRTF.TraWebStiff.dAq, Row_Ref, Col_Ref, iStRow);

		//Buckling Chk
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fcd"), PgbrIRCBasePRTF.TraWebStiff.dBuck_fcd, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_fcdlim"), PgbrIRCBasePRTF.TraWebStiff.dBuck_fcd_Lim, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_hdasheff"), PgbrIRCBasePRTF.TraWebStiff.dBuck_hdasheff, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_hdashw"), PgbrIRCBasePRTF.TraWebStiff.dBuck_hdashw, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_aeff"), PgbrIRCBasePRTF.TraWebStiff.dBuck_Aeff, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_iys"), PgbrIRCBasePRTF.TraWebStiff.dBuck_Iys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_rys"), PgbrIRCBasePRTF.TraWebStiff.dBuck_rys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_kl"), PgbrIRCBasePRTF.TraWebStiff.dBuck_KL, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_fcc"), PgbrIRCBasePRTF.TraWebStiff.dBuck_fcc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_lambda"), PgbrIRCBasePRTF.TraWebStiff.dBuck_lambda, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_phi"), PgbrIRCBasePRTF.TraWebStiff.dBuck_phi, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BUCK_x"), PgbrIRCBasePRTF.TraWebStiff.dBuck_X, Row_Ref, Col_Ref, iStRow);
	...
	else
	{
		...
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_iys"), PgbrIRCBasePRTF.TraWebStiff.dBearingIys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_rys"), PgbrIRCBasePRTF.TraWebStiff.dBearingrys, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("TWS_PRT3_BR_fcc"), PgbrIRCBasePRTF.TraWebStiff.dBearingfcc*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
...
void CRptPlateGirder_IRC22_15::PrintLongitudinalShearResistance(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PGLS_BASE& PglsGd, T_PGLS_BASE& PglsBase, T_PGLS_DETAIL& PglsDetail, T_PGBR_IRC_BASE& PgbrIRCGd)
{
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	// Title
	pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1"), iRowCnt, iColCnt);
	short Row_Ref = 0; short Col_Ref=0;

	Row_Ref = 55;
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_veddl"), PgbrIRCGd.LotrShear.dVedDL*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vedll"), PgbrIRCGd.LotrShear.dVedLL*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_veddllcom"), PgbrIRCGd.LotrShear.strVedDlLcombMax, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vedlllcomb"), PgbrIRCGd.LotrShear.strVedLlLcombMax, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_slprov"), PgbrIRCGd.LotrShear.dSLProv, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_sllim"), PgbrIRCGd.LotrShear.dSLProvLim, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_hs"), PgbrIRCGd.LotrShear.dhs, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vled"), PgbrIRCGd.LotrShear.dVled, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_sl1req"), PgbrIRCGd.LotrShear.dSl1req, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_sl1prov"), PgbrIRCGd.LotrShear.dSl1prov, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_n"), PgbrIRCGd.LotrShear.dn, Row_Ref, Col_Ref, iStRow);
	double dQu =  PgbrIRCGd.LotrShear.dQu;
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_qu"), dQu, Row_Ref, Col_Ref, iStRow);
	//chk full shear connection

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_asl"), PgbrIRCGd.LotrShear.dAsl*m_dFactorRebarArea, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_aecslab"), PgbrIRCGd.LotrShear.dAecSlab*m_dFactorRebarArea, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_aecsteel"), PgbrIRCGd.LotrShear.dAecSteel*m_dFactorRebarArea, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_h1"), PgbrIRCGd.LotrShear.dH1*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_h2"), PgbrIRCGd.LotrShear.dH2*m_dFactorForce, Row_Ref, Col_Ref, iStRow);


	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_sl2req"), PgbrIRCGd.LotrShear.dSl2req, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_sl2prov"), PgbrIRCGd.LotrShear.dSl1prov, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_sl2h"), PgbrIRCGd.LotrShear.dH*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_sl2l"), PgbrIRCGd.LotrShear.dL, Row_Ref, Col_Ref, iStRow);

	//Longitudinal Shear 
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_VR"), PgbrIRCGd.LotrShear.dVR*m_dFactorForce, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_Vsmallr"), PgbrIRCGd.LotrShear.dVr, Row_Ref, Col_Ref, iStRow);//Converting from N/mm to kN/m is factor 1 CIVIL-3320

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_srreq"), PgbrIRCGd.LotrShear.dSrreq, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_srprov"), PgbrIRCGd.LotrShear.dSl1prov, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_qr"), PgbrIRCGd.LotrShear.dQr*m_dFactorForce, Row_Ref, Col_Ref, iStRow);//CIVIL-3320
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_slreq"), PgbrIRCGd.LotrShear.dSLreq, Row_Ref, Col_Ref, iStRow);

	//Transverse shear

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vlcase1"), PgbrIRCGd.LotrShear.dVlcase1, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vlcase2"), PgbrIRCGd.LotrShear.dVlcase2, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vll"), PgbrIRCGd.LotrShear.dVlL, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vlast"), PgbrIRCGd.LotrShear.dVlAst, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("LOTR_PRT1_vl25"), PgbrIRCGd.LotrShear.dVl25, Row_Ref, Col_Ref, iStRow);

	iStRow += iRowCnt;
	/*pXL->CopyRange(iStRow, 1, m_strBaseBook, _T("I_Sect_T(", ")resistance_to_longi_shear"), iRowCnt, iColCnt);*/

//   CString RptBlock = _T(_T("resistance_to_longi_shear")     ); 
//   pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
//   short Row_Ref = 0, Col_Ref = 0;
//   pXL->GetCellIDByName(m_strBaseBook, m_strBaseSheet, RptBlock, Row_Ref, Col_Ref);Col_Ref-=1;
//   
...
void CRptPlateGirder_IRC22_15::PrintFatigueResistance(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PGRF_BASE& PgrfBase, T_PGRF_DETAIL& PgrfDetail, T_PGBR_IRC_BASE& PgbrIRCGd)
{
	//
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	// Title
	short Row_Ref = 0; short Col_Ref=0;
	if ( PgbrIRCGd.Fatg.TopFlange.TopSide.y1 == 0. && PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Max==0. )
	{
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("fatr_NoLC"), iRowCnt, iColCnt);
	}
	else
	{
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("fatr_full"), iRowCnt, iColCnt);


		Row_Ref = 35;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_lcbmax"), PgbrIRCGd.Fatg.TopFlange.TopSide.strMaxLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_lcbmin"), PgbrIRCGd.Fatg.TopFlange.TopSide.strMinLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_y1"), PgbrIRCGd.Fatg.TopFlange.TopSide.y1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_z1"), PgbrIRCGd.Fatg.TopFlange.TopSide.z1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_sig1max"), PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_sig1min"), PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_tau1max"), PgbrIRCGd.Fatg.TopFlange.TopSide.dTau1Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_tf_tau1min"), PgbrIRCGd.Fatg.TopFlange.TopSide.dTau1Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_lcbmax"), PgbrIRCGd.Fatg.TopFlange.BotSide.strMaxLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_lcbmin"), PgbrIRCGd.Fatg.TopFlange.BotSide.strMinLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_y2"), PgbrIRCGd.Fatg.TopFlange.BotSide.y1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_z2"), PgbrIRCGd.Fatg.TopFlange.BotSide.z1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_sig2max"), PgbrIRCGd.Fatg.TopFlange.BotSide.dSigma2Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_sig2min"), PgbrIRCGd.Fatg.TopFlange.BotSide.dSigma2Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_tau2max"), PgbrIRCGd.Fatg.TopFlange.BotSide.dTau2Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_bf_tau2min"), PgbrIRCGd.Fatg.TopFlange.BotSide.dTau2Min, Row_Ref, Col_Ref, iStRow);

		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_lcbmax"), PgbrIRCGd.Fatg.BotFlange.TopSide.strMaxLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_lcbmin"), PgbrIRCGd.Fatg.BotFlange.TopSide.strMinLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_y1"), PgbrIRCGd.Fatg.BotFlange.TopSide.y1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_z1"), PgbrIRCGd.Fatg.BotFlange.TopSide.z1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_sig1max"), PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_sig1min"), PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_tau1max"), PgbrIRCGd.Fatg.BotFlange.TopSide.dTau1Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_tf_tau1min"), PgbrIRCGd.Fatg.BotFlange.TopSide.dTau1Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_lcbmax"), PgbrIRCGd.Fatg.BotFlange.BotSide.strMaxLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_lcbmin"), PgbrIRCGd.Fatg.BotFlange.BotSide.strMinLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_y2"), PgbrIRCGd.Fatg.BotFlange.BotSide.y1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_z2"), PgbrIRCGd.Fatg.BotFlange.BotSide.z1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_sig2max"), PgbrIRCGd.Fatg.BotFlange.BotSide.dSigma2Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_sig2min"), PgbrIRCGd.Fatg.BotFlange.BotSide.dSigma2Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_tau2max"), PgbrIRCGd.Fatg.BotFlange.BotSide.dTau2Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_B_bf_tau2min"), PgbrIRCGd.Fatg.BotFlange.BotSide.dTau2Min, Row_Ref, Col_Ref, iStRow);

		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_lcbmax"), PgbrIRCGd.Fatg.Web.TopSide.strMaxLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_lcbmin"), PgbrIRCGd.Fatg.Web.TopSide.strMinLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_y1"), PgbrIRCGd.Fatg.Web.TopSide.y1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_z1"), PgbrIRCGd.Fatg.Web.TopSide.z1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_sig1max"), PgbrIRCGd.Fatg.Web.TopSide.dSigma1Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_sig1min"), PgbrIRCGd.Fatg.Web.TopSide.dSigma1Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_tau1max"), PgbrIRCGd.Fatg.Web.TopSide.dTau1Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_tf_tau1min"), PgbrIRCGd.Fatg.Web.TopSide.dTau1Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_lcbmax"), PgbrIRCGd.Fatg.Web.BotSide.strMaxLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_lcbmin"), PgbrIRCGd.Fatg.Web.BotSide.strMinLCB, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_y2"), PgbrIRCGd.Fatg.Web.BotSide.y1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_z2"), PgbrIRCGd.Fatg.Web.BotSide.z1, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_sig2max"), PgbrIRCGd.Fatg.Web.BotSide.dSigma2Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_sig2min"), PgbrIRCGd.Fatg.Web.BotSide.dSigma2Min, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_tau2max"), PgbrIRCGd.Fatg.Web.BotSide.dTau2Max, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_W_bf_tau2min"), PgbrIRCGd.Fatg.Web.BotSide.dTau2Min, Row_Ref, Col_Ref, iStRow);


		//Correction Factor
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_corrfact_mewr"), PgbrIRCGd.Fatg.dMewR, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_corrfact_nsc"), PgbrIRCGd.Fatg.dNscCorr, Row_Ref, Col_Ref, iStRow);
		//Low Fatigue
		bool isFatigueCheckReq = false;
		double dtau = max(abs(PgbrIRCGd.Fatg.TopFlange.TopSide.dTau1Max), abs(PgbrIRCGd.Fatg.TopFlange.TopSide.dTau1Min));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.TopFlange.BotSide.dTau2Max));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.TopFlange.BotSide.dTau2Min));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.BotFlange.TopSide.dTau1Max));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.BotFlange.TopSide.dTau1Min));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.BotFlange.BotSide.dTau2Max));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.BotFlange.BotSide.dTau2Min));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.Web.TopSide.dTau1Max));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.Web.TopSide.dTau1Min));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.Web.BotSide.dTau2Max));
		dtau = max(dtau, abs(PgbrIRCGd.Fatg.Web.BotSide.dTau2Max));
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_lowfatg_f"), PgbrIRCGd.Fatg.df, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_lowfatg_27bygamma"), PgbrIRCGd.Fatg.d27gammamft, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_lowfatg_nsc"), PgbrIRCGd.Fatg.dNsc, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_lowfatg_nsc5"), PgbrIRCGd.Fatg.dNscLim, Row_Ref, Col_Ref, iStRow);
		if (PgbrIRCGd.Fatg.df > PgbrIRCGd.Fatg.d27gammamft || PgbrIRCGd.Fatg.dNsc > PgbrIRCGd.Fatg.dNscLim) isFatigueCheckReq = true;
		//Highest normal stress
		//PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("highest_normal_stress_obtained"), PgbrIRCGd.Fatg.df, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("highest_normal_stress"), PgbrIRCGd.Fatg.d27gammamft * PgbrIRCGd.Fatg.dMewR, Row_Ref, Col_Ref, iStRow);
		//Highest shear stress
		//PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("highest_shear_stresss_obtained"), PgbrIRCGd.Fatg.dtauf, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("highest_shear_stress"), PgbrIRCGd.Fatg.d27gammamft * (67/27), Row_Ref, Col_Ref, iStRow);
		if (PgbrIRCGd.Fatg.df > PgbrIRCGd.Fatg.d27gammamft * PgbrIRCGd.Fatg.dMewR || dtau > PgbrIRCGd.Fatg.d27gammamft * (67 / 27)) isFatigueCheckReq = true;
		//Fatigue Strength
		if (isFatigueCheckReq) 
		{
			iStRow += iRowCnt + 1;
			Row_Ref = 74;
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("fatigue_strength"), iRowCnt, iColCnt);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_ffn"), PgbrIRCGd.Fatg.dffn, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_tfn"), PgbrIRCGd.Fatg.dtfn, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_ff"), PgbrIRCGd.Fatg.dff, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_tf"), PgbrIRCGd.Fatg.dtauf, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_ffd"), PgbrIRCGd.Fatg.dfd, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_tfd"), PgbrIRCGd.Fatg.dtaufd, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_ffdlim"), PgbrIRCGd.Fatg.dfdlim, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("fatr_fatgstre_tfdlim"), PgbrIRCGd.Fatg.dtaulim, Row_Ref, Col_Ref, iStRow);
		}
	}
	// Title
//   pXL->CopyRange(iStRow, 2, m_strBaseBook, strBaseSheet, _T("fatr_full"), iRowCnt, iColCnt);
//   pXL->SetValue(iStRow, 1, m_iFatigueChapterIx);
//   iStRow += iRowCnt;
//   
//   CString RptBlock = _T(_T("FATIGUE_RESIST")     ); 
//   pXL->CopyRange(iStRow, 1, m_strBaseBook, strBaseSheet, RptBlock, iRowCnt, iColCnt);
//   short Row_Ref = 0, Col_Ref = 0;
//   pXL->GetCellIDByName(m_strBaseBook, m_strBaseSheet, RptBlock, Row_Ref, Col_Ref);Col_Ref-=1;
// 
...
void CRptPlateGirder_IRC22_15::PrintStressLimit(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PGBS_BASE& PgbsBase, T_PGBS_DETAIL& PgbsDetail, T_PGBR_IRC_BASE& PgbrIRCGd, BOOL bCheckService[4])
{
	int iRowCnt = 0;
	int iColCnt = 0;
	...
	pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("SL_SL_full"), iRowCnt, iColCnt);
	short Row_Ref = 0; short Col_Ref=0;

	Row_Ref = 42;
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_lcom"), PgbrIRCGd.Fatg.strLCLcomb, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_sigmaedser"), PgbrIRCGd.Fatg.dSigmaEdSer, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_tauedser"), PgbrIRCGd.Fatg.dTauEdser, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_sigmaedserlim"), PgbrIRCGd.Fatg.dSigmaEdSerLim, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_tauedserlim"), PgbrIRCGd.Fatg.dTauEdserLim, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_srss"), PgbrIRCGd.Fatg.dTotSqrt, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_srsslim"), PgbrIRCGd.Fatg.dSigmaEdSerLim, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_sigc"), PgbrIRCGd.Fatg.dSigmac, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_k1"), PgbrIRCGd.Fatg.dk1, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_k1fck"), PgbrIRCGd.Fatg.dk1fck, Row_Ref, Col_Ref, iStRow);

	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_sigs"), PgbrIRCGd.Fatg.dSigmaRF, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_k3"), PgbrIRCGd.Fatg.dk3, Row_Ref, Col_Ref, iStRow);
	PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_SL_k2fyk"), PgbrIRCGd.Fatg.dk3fyk, Row_Ref, Col_Ref, iStRow);
	iStRow += iRowCnt + 1;

	if ( bCheckService[2] )
	{
		if ( PgbrIRCGd.SectProp.iEffWidthCase==0 )
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("SL_DC_NOSPAN"), iRowCnt, iColCnt);
		}
		else
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("SL_DC_full"), iRowCnt, iColCnt);
			short Row_Ref = 0; short Col_Ref=0;

			Row_Ref = 60;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_DC_lcom"), PgbrIRCGd.Fatg.strLCDefl, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_DC_delta"), PgbrIRCGd.Fatg.dDeltaDL, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_DC_limit"), PgbrIRCGd.Fatg.dPhis, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_DC_deltalim"), PgbrIRCGd.Fatg.dDeltaDLLimit, Row_Ref, Col_Ref, iStRow);
	...
	{
		short Row_Ref = 0; short Col_Ref=0;
		pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("SL_CW_MAIN"), iRowCnt, iColCnt);
		Row_Ref = 117;
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_MAIN_TOP_LC"), PgbrIRCGd.Fatg.strLCBTop, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_MAIN_TOP_STRESS"), PgbrIRCGd.Fatg.dSigmaTop, Row_Ref, Col_Ref, iStRow);

		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_MAIN_BOT_LC"), PgbrIRCGd.Fatg.strLCBTop, Row_Ref, Col_Ref, iStRow);
		PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_MAIN_BOT_STRESS"), PgbrIRCGd.Fatg.dSigmaBot, Row_Ref, Col_Ref, iStRow);

		iStRow += iRowCnt + 1;
		if ( PgbrIRCGd.Fatg.dSigmaTop >0. ||  PgbrIRCGd.Fatg.dSigmaBot>0. )
		{
			pXL->CopyRange(iStRow, 0, m_strBaseBook, strBaseSheet, _T("SL_CW_full"), iRowCnt, iColCnt);


			Row_Ref = 67;
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_phi"), PgbrIRCGd.Fatg.dphi, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_c"), PgbrIRCGd.Fatg.dc, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_etasm"), PgbrIRCGd.Fatg.dEcmminusEcm, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_sigmas"), PgbrIRCGd.Fatg.dSigmaS, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_fcteff"), PgbrIRCGd.Fatg.dfcteff, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_ecm"), PgbrIRCGd.Fatg.dEcm, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_alphae"), PgbrIRCGd.Fatg.dEs/PgbrIRCGd.Fatg.dEcm, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_rhopeff"), PgbrIRCGd.Fatg.dRhopeff, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_as"), PgbrIRCGd.Fatg.dAs, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_aceff"), PgbrIRCGd.Fatg.dAceff, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_slim"), PgbrIRCGd.Fatg.dslimit, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_srmax"), PgbrIRCGd.Fatg.dSrmax, Row_Ref, Col_Ref, iStRow);

			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_k1"), PgbrIRCGd.Fatg.dk1spacing, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_k2"), PgbrIRCGd.Fatg.dk2spacing, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_k3"), PgbrIRCGd.Fatg.dk3spacing, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_k4"), PgbrIRCGd.Fatg.dk4spacing, Row_Ref, Col_Ref, iStRow);


			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_wk"), PgbrIRCGd.Fatg.dWk, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_wkmax"), PgbrIRCGd.Fatg.dWkmax, Row_Ref, Col_Ref, iStRow);
			PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_wmax"), PgbrIRCGd.Fatg.dWkmax, Row_Ref, Col_Ref, iStRow);

			if ( PgbrIRCGd.Fatg.nCrackClass == 0 ) PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_expclass"), _T("Moderate"), Row_Ref, Col_Ref, iStRow);
			else if ( PgbrIRCGd.Fatg.nCrackClass == 1 ) PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_expclass"), _T("Severe"), Row_Ref, Col_Ref, iStRow);
			else if ( PgbrIRCGd.Fatg.nCrackClass == 2 ) PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("Very SL_CW_expclass"), _T("Very Severe"), Row_Ref, Col_Ref, iStRow);
			else                                      PrintCellData(pXL, m_strBaseBook, strBaseSheet, _T("SL_CW_expclass"), _T("Extreme"), Row_Ref, Col_Ref, iStRow);
...
void CRptPlateGirder_IRC22_15::PrintLongitudinalShear_SLS(CMSExcel* pXL, int& iStRow, int iChapIndex, int iMaxMin, T_PLSS_BASE& PlssGd, T_PLSS_BASE& PlssBase, T_PLSS_DETAIL& PlssDetail)
{
	int iRowCnt = 0;
	int iColCnt = 0;
...
void CRptPlateGirder_IRC22_15::Print_CheckElemPosition(CMSExcel* pXL, int& iStRow, CString strBaseSheet, int iElemNo, int nPosi)
{
	int iRowCnt = 0;
	int iColCnt = 8;
...
BOOL CRptPlateGirder_IRC22_15::Get_DesignInfo_Section(ElemPairK ElemK, BOOL bPosiI)
{
	int nPosiId = bPosiI ? 0 : 1;
	CPG_SECT_ELEM SectElem;  SectElem.Initialize();
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_CpgSectElem(ElemK, SectElem);

	CPG_RBAR_ELEM RbarElem; RbarElem.Initialize();
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_CpgRbarElem(ElemK, RbarElem);
	//
	CPG_SCON_ELEM SconElem; SconElem.Initialize();
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_CpgSconElem(ElemK, SconElem);
	//

	CPG_TRST_ELEM TrstElem; TrstElem.Initialize();
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_CpgTrstElem(ElemK, TrstElem);

	BOOL bMembTemp=FALSE;
	CPG_MEMB_ELEM MembElemTemp;  MembElemTemp.Initialize();
	if ( !m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_CpgMembElem(ElemK, MembElemTemp) ) { bMembTemp=FALSE; }
	...
	T_PGBR_IRC_LCOM PgbrIRCRatD; PgbrIRCRatD.Initialize(); // PositiveÀÇ I,J¸¦ ¾ò¾î¿Â´Ù. 
	if ( m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetPgbrResultData_IRC(ElemK, nPosiNega, PgbrIRCRatD) )
	{
		T_PGBR_IRC_BASE PgbrGd   = PgbrIRCRatD.BeforePgbr[nPosiId];
		T_PGBR_IRC_BASE PgbrBase = PgbrIRCRatD.PgbrBase[nPosiId];
	...
	if ( nSectType != D_SECT_TYPE_COMPO_G )
	{
		...
		// girder
		m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_GirderStiffnessofCompo(bPosiI, SectD, GirderStiffD);
	...
	// Material
	m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_MaterialData(ElemK, m_dfy, m_dfy1, m_dEs, m_dfck, m_dEcm, m_dfsk, m_dErs);
...
//   m_dArb = dArb;//100.0;
	...
	if ( !m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_CpgMatlElem(ElemK, MatlElem) ) { return FALSE; }
...
CString CRptPlateGirder_IRC22_15::Get_String4PartShape(int nPartType)
{
	CString strPartType;
...
CString CRptPlateGirder_IRC22_15::Get_String4ShearLamdaw(BOOL bWeb, int iLamda_w_Type)
{
...
CString CRptPlateGirder_IRC22_15::Get_String4ShearSubPanelLamdaw(BOOL bWeb, int iLamda_w_Type)
{
	CString strLamdawType;
	if ( bWeb )
	{
...
CString CRptPlateGirder_IRC22_15::Get_String4ShearXwType(BOOL bWeb, int iEndSupportType, int iLamda_w_scope)
{
	CString strXwType;
	if ( bWeb )
	{
...
CString CRptPlateGirder_IRC22_15::GetLoadCombinationName(int OrgLcomK)
{
...
CString CRptPlateGirder_IRC22_15::GetStressPosition(int iPosition)
{
...
int CRptPlateGirder_IRC22_15::CopyRange(CMSExcel* pExcel, CString& RptBlock, int stRow)
{
...
void CRptPlateGirder_IRC22_15::SetRptData(ElemPairK ElemK, int nPosi, CSG_MEMB_POSD& InData, CSG_MEMB_RES_AASHTO_LRFD_D& RptData, T_SECT_STIFFNESS& SpscSect, T_PGBR_BASE& PgbrBase)
{
	//RptData Calculation
	...
	if ( nSectType == D_SECT_TYPE_COMPO_I )
	{
		if ( !bTapered )
		{
			dHw =  SectD.SectBefore.SectI.Size[0];
			dtw  = SectD.SectBefore.SectI.Size[1];
			dB1  = SectD.SectBefore.SectI.Size[2];
			dtf1 = SectD.SectBefore.SectI.Size[3];
			dB2  = SectD.SectBefore.SectI.Size[4];
			dtf2 = SectD.SectBefore.SectI.Size[5];
		}
		else
		{
 ...
 // m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_MaterialData(ElemK, m_dfy, m_dfy1, m_dEs, m_dfck, m_dEcm, m_dfsk, m_dErs);
	...
	// °Å´õ-»óºÎÇÃ·£Áö
	CPG_MATL_ELEM MatlElem;
	if ( !m_pDataCtrl->Get_PlateGirderPointer_IRC22_15()->Get_CpgMatlElem(ElemK, MatlElem) ) ASSERT(0);
...
void CRptPlateGirder_IRC22_15::SetRptData_SectStiffness(int nPosi, DGN_SECT_STIFFNESS & InData, CSG_SectionProperty& RptData)
{
...
void CRptPlateGirder_IRC22_15::ConvertCGSGlobalData(T_CPGD_D &CpgdD, S_CSGD_D &rData)
{
...
void CRptPlateGirder_IRC22_15::PrintCellData(CMSExcel* pExcel, const CString strCellname, CString strData, short &rRow_Ref, short &rCol_Ref, int& stRow)
{
	short Row =0, Col=0;
	//  short Row_Ref=0, Col_Ref=0;
	short Row_Calc=0, Col_Calc=0;
...
void CRptPlateGirder_IRC22_15::PrintCellData(CMSExcel* pExcel, const CString strCellname, double     dData, short &rRow_Ref, short &rCol_Ref, int& stRow)
{
	short Row=0, Col=0;
	//short Row_Ref=0, Col_Ref=0;
	short Row_Calc=0, Col_Calc=0;
...
void CRptPlateGirder_IRC22_15::PrintCellData(CMSExcel* pExcel, const CString strBaseBook, const CString strSheetName, const CString strCellname, CString strData, short &rRow_Ref, short &rCol_Ref, int& stRow)
{
	short Row =0, Col=0;
	//  short Row_Ref=0, Col_Ref=0;
	short Row_Calc=0, Col_Calc=0;
...
void CRptPlateGirder_IRC22_15::PrintCellData(CMSExcel* pExcel, const CString strBaseBook, const CString strSheetName, const CString strCellname, double     dData, short &rRow_Ref, short &rCol_Ref, int& stRow)
{
	short Row=0, Col=0;
	//short Row_Ref=0, Col_Ref=0;
	short Row_Calc=0, Col_Calc=0;
...
BOOL CRptPlateGirder_IRC22_15::CalcCellID(int stRow, short Row, short Col, short Row_Ref, short Col_Ref, short &rRow_Calc, short &rCol_Calc)
{
	BOOL bChk = TRUE;
...
CString CRptPlateGirder_IRC22_15::GetClassString(int iClass)
{
	CString strClass = _T("Plastic");

	if ( iClass==1 ) strClass = _T("Plastic");
	else if ( iClass==2 ) strClass = _T("Compact");
	else if ( iClass==3 ) strClass = _T("Semi Compact");
	else                strClass = _T("Revise Section");

	return strClass;

}

void CRptPlateGirder_IRC22_15::DeleteCellData(CMSExcel* pExcel, CString strBaseBook, CString strSheetName, CString strCellname, int  fwdDelRows, short &rRow_Ref, short &rCol_Ref, int& stRow)
{
	short Row=0, Col=0;
	//short Row_Ref=0, Col_Ref=0;
	short Row_Calc=0, Col_Calc=0;
	...
	pExcel->GetCellIDByName(strBaseBook, strSheetName, strCellname, Row, Col);
	bChk = CalcCellID(stRow, Row, Col, rRow_Ref, rCol_Ref, Row_Calc, Col_Calc);
	int nRowDel1 = Row_Calc+1;
	int nRowDel2 = nRowDel1+fwdDelRows;
	CString strDelCell1;  strDelCell1.Format(_T("A%d"), nRowDel1);
	CString strDelCell2;  strDelCell2.Format(_T("AH%d"), nRowDel2);
	if ( bChk ) pExcel->DeletCell(strDelCell1, strDelCell2);
