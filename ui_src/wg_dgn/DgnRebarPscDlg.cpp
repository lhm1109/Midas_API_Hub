// DgnRebarPscDlg.cpp : implementation file
//
...
CDgnRebarPscDlg::CDgnRebarPscDlg(CWnd* pParent /*=NULL*/)
	: CChildDialog(CDgnRebarPscDlg::IDD, pParent)
{
...
/*
void CDgnRebarPscDlg::SetParamData(T_RPSC_K RpscK)
{
	m_Key = RpscK;
	m_bModify = TRUE;
}*/

void CDgnRebarPscDlg::DoDataExchange(CDataExchange* pDX)
{
	CChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnRebarPscDlg)
...
BEGIN_MESSAGE_MAP(CDgnRebarPscDlg, CChildDialog)
	//{{AFX_MSG_MAP(CDgnRebarPscDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRebarPscDlg message handlers

BOOL CDgnRebarPscDlg::OnInitDialog() 
{
...
void CDgnRebarPscDlg::OnOK() 
{
	...
	{
		AfxMessageBox(_LS(IDS_DGN_RPSC_ERR_SHEAR_REBAR));
		return;
	}
...
// delete
void CDgnRebarPscDlg::OnDgnDelete() 
{
...
void CDgnRebarPscDlg::OnClickDgnSectList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
	int nIndex = m_wndList.GetNextItem( -1, LVNI_ALL | LVNI_SELECTED);
...
void CDgnRebarPscDlg::OnItemchangedDgnSectList(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CDgnRebarPscDlg::ChangeItem(int nIndex)
{
	// enable/disable
	m_btnAdd.EnableWindow(nIndex != -1);
	m_btnDel.EnableWindow(nIndex != -1);
...
void CDgnRebarPscDlg::UpdateView(BOOL bEndI)
{
...
void CDgnRebarPscDlg::LayoutChildDlg()
{
	CWnd* pPlaceHolder = GetDlgItem(IDC_DGN_PLACEHOLDER);
	ASSERT(pPlaceHolder);
	m_ctrlTab.SetPlaceHolder(pPlaceHolder);

	m_pMomentDlg = new CDgnRebarPscMoment(&m_wndSecView, 0, this);//Cpg일 경우에만 가운데 파라메타를 2로
	m_ctrlTab.AddTab(m_pMomentDlg, _LS(IDS_DGN_RPSC_MAIN_REBAR), CDgnRebarPscMoment::IDD, TRUE);

	m_pShearDlg = new CDgnRebarPscShear(this);
	m_ctrlTab.AddTab(m_pShearDlg, _LS(IDS_DGN_RPSC_SHEAR_REBAR), CDgnRebarPscShear::IDD, TRUE);

	m_ctrlTab.ShowTab(0);
}

void CDgnRebarPscDlg::SetHeaderTitle()
{
	CStringArray aTitle;
...
#define M_ADD_LIST_INFO(TITLE, WIDTH) aTitle.Add(TITLE); aWidth.Add(WIDTH)
	...
	M_ADD_LIST_INFO(_LS(IDS_DGN_RPSC_MAIN_REBAR),   85);
	M_ADD_LIST_INFO(_LS(IDS_DGN_RPSC_SHEAR_REBAR),  85);
...
void CDgnRebarPscDlg::MakeItemEx()
{
	CWaitCursor Cursor;
	...
	CArray<T_RPSC_K, T_RPSC_K> arRpscKeyList;
	
	m_pDoc->m_pAttrCtrl->GetRpscKeyList(arRpscKeyList);

	for(int i=0 ; i<arRpscKeyList.GetSize() ; i++)
	{
		m_pDoc->m_pAttrCtrl->GetSect(arRpscKeyList[i], Data);
		InsertItem(arRpscKeyList[i], Data);
	}	
}

BOOL CDgnRebarPscDlg::InsertItem(T_SECT_K Key, T_SECT_D &Data)
{
	T_RPSC_D RData;
...
int CDgnRebarPscDlg::FindInsertionPos(T_SECT_K nID)
{
...
CString CDgnRebarPscDlg::DataToStr(int i, T_SECT_K Key, T_SECT_D &Data, T_RPSC_D &RData)
{
	CString str;
...
void CDgnRebarPscDlg::OnClickCopyBtn()
{
