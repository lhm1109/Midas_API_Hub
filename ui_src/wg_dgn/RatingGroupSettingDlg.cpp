// RatingGroupSettingDlg.cpp : implementation file
//
...
CRatingGroupSettingDlg::CRatingGroupSettingDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CRatingGroupSettingDlg::IDD, pParent)
{
	m_pDoc = CDBDoc::GetDocPoint();	

	//{{AFX_DATA_INIT(CRatingGroupSettingDlg)
	m_dFactor = 1.0;
	m_bIEnd = FALSE;
	m_bJEnd = FALSE;
	//}}AFX_DATA_INIT
...
CRatingGroupSettingDlg::~CRatingGroupSettingDlg()
{
    _SAFE_DELETE(m_pDataCtrl);
}

void CRatingGroupSettingDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CRatingGroupSettingDlg)
...
BOOL CRatingGroupSettingDlg::OnInitDialog() 
{
	//PROFILING(_T("CRatingGroupSettingDlg::OnInitDialog()"));
...
void CRatingGroupSettingDlg::InitSelectGroupList() 
{
	//PROFILING(_T("CRatingGroupSettingDlg::InitSelectGroupList()"));
	...
	for(INT_PTR i=0; i<m_arGrupKey.GetSize(); i++)
	{
		bool isBeam = TRUE;
		bool isConcret = TRUE;
		bool isSect = FALSE;
		...
		for(int e=0 ; e<ElemKeyList.GetSize() ; e++)
		{
			...
			if(m_pDataCtrl->Get_DgnGenMbtp(ElemKeyList[e]) != 2) nBeamNG++;				
					
			//2. Group에 포함된 요소의 Material은 모두 콘크리트이어야 한다.(Steel이 하나라도 있음 안된다.)
			if(!(MatlD.Type==_T("C") || MatlD.Type==_T("SRC")))	
				nMatlNG++;
			//Group에 포함된 요소의 Section타입은 모두 PSC이어야 한다.
			if(sectData.nStype == D_SECT_TYPE_PSC)
			{        
				if(sectData.SectBefore.Shape!=D_SECT_SHAPE_PSC_CMPWEB) isSect = TRUE;
			}	
			else if (sectData.nStype == D_SECT_TYPE_COMPO_CI || sectData.nStype == D_SECT_TYPE_COMPO_CT || sectData.nStype == D_SECT_TYPE_COMPO_PC || sectData.nStype == D_SECT_TYPE_COMPO_G)
			{
				isSect = TRUE;
			}
			else if (sectData.nStype==D_SECT_TYPE_TAPERED)
			{
				if(sectData.SectBefore.nStype==D_SECT_TYPE_PSC && sectData.SectBefore.Shape!=D_SECT_SHAPE_PSC_CMPWEB)
					isSect = TRUE;
				else if (sectData.SectBefore.nStype == D_SECT_TYPE_COMPO_CI || sectData.SectBefore.nStype == D_SECT_TYPE_COMPO_CT ||
					       sectData.SectBefore.nStype == D_SECT_TYPE_COMPO_PC || sectData.SectBefore.nStype == D_SECT_TYPE_COMPO_G) isSect = TRUE;
			}

			if (isSect==FALSE) nSectNG++;
		}
				

		if(nBeamNG==0 && nMatlNG==0 && nSectNG==0)
		{
...
void CRatingGroupSettingDlg::SetListCtrlHeader()
{
	CString aTitle[] = {_LS(IDS_DGN_RATING_GROUP), _LS(IDS_DGN_RATING_FACTOR), _LS(IDS_DGN_RATING_POSITION)};  //_T("Group"), _T("Factor"), _T("Position")
...
int CRatingGroupSettingDlg::FindInsertionPos(T_BLRG_K nID)
{
	int nCount = m_listGroup.GetItemCount();
	for (int i=0; i < nCount; i++)
	{
        T_BLRG_K key = m_listGroup.GetItemData(i);
		if (nID < key) { return i; }
	}
	return nCount;
}

BOOL CRatingGroupSettingDlg::InsertListItem(T_BLRG_K Key)
{
	//DB에서 Data를 가져온다

	T_BLRG_D Data;

	m_pDoc->m_pAttrCtrl->GetBlrg(Key, Data);
...
CString CRatingGroupSettingDlg::DataToStr(int i, T_BLRG_K Key, T_BLRG_D &Data)
{  
	CString str;
	if(i == 0)
...
BOOL CRatingGroupSettingDlg::DeleteListItem(T_BLRG_K key)
{
	LVFINDINFO FindInfo;
...
BOOL CRatingGroupSettingDlg::ModifyListItem(T_BLRG_K key)
{
	T_BLRG_D Data;
...
void CRatingGroupSettingDlg::LoadDlgFactorPosition()
{
...
BOOL CRatingGroupSettingDlg::SaveDlgFactorPosition()
{
	UpdateData(TRUE);
...
BOOL CRatingGroupSettingDlg::SaveDlgSelectGroup()
{
	UpdateData(TRUE);
...
BEGIN_MESSAGE_MAP(CRatingGroupSettingDlg, CDialogMove)
	//{{AFX_MSG_MAP(CRatingGroupSettingDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CRatingGroupSettingDlg message handlers

void CRatingGroupSettingDlg::OnDgnRatingGroupAddBtn() 
{
	...
	CArray<T_BLRG_K, T_BLRG_K> arKey;

	for(int i=0 ; i<m_arSelectGrupKey.GetSize() ; i++)
	{
		T_BLRG_K key = m_arSelectGrupKey.GetAt(i);
		if(m_pDoc->m_pAttrCtrl->ExistBlrg(key))
		{
			arKey.Add(key);
		}
		else
		{
			if(m_pDoc->m_pDataCtrl->AddBlrg(key, m_MainData))
				InsertListItem(key);
		}
	}
...
void CRatingGroupSettingDlg::OnDgnRatingGroupModifyBtn() 
{
	// TODO: Add your control notification handler code here
	if(!SaveDlgSelectGroup())
		return;
	...
	for(int i=0 ; i<m_arSelectGrupKey.GetSize() ; i++)
	{
		...
		{
			if(m_pDoc->m_pDataCtrl->ModifyBlrg(key, key, m_MainData))
				ModifyListItem(key);
		}
	}
...
void CRatingGroupSettingDlg::OnDgnRatingGroupDeleteBtn() 
{
	// TODO: Add your control notification handler code here
	...
	for(int i=0 ; i<arKey.GetSize() ; i++)
	{
		if(m_pDoc->m_pAttrCtrl->ExistBlrg(arKey[i]))
		{
			if(m_pDoc->m_pDataCtrl->DelBlrg(arKey[i]))
				DeleteListItem(arKey[i]);  
		}
	}	
}

void CRatingGroupSettingDlg::GetSelectedItemList(CListCtrl* pList, CArray<int, int> &aSelItem)
{
	int  nItem = -1;
...
void CRatingGroupSettingDlg::OnItemchangedDgnRatingGroupList(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CRatingGroupSettingDlg::ChangeItem(int nIndex)
{
	//PROFILING(_T("CRatingGroupSettingDlg::ChangeItem(int nIndex)"));
	...
	m_pDoc->m_pAttrCtrl->GetBlrg(grupKey, m_MainData);
	LoadDlgFactorPosition();
}

void CRatingGroupSettingDlg::OnDgnRatingGroupClose() 
{
	// TODO: Add your control notification handler code here
	CDialogMove::OnOK();
}

void CRatingGroupSettingDlg::GetDBBlrg() 
{
	CArray<T_BLRG_K, T_BLRG_K> BlrgKeyList;

	m_pDoc->m_pAttrCtrl->GetBlrgKeyList(BlrgKeyList);

	for(int i=0 ; i<BlrgKeyList.GetSize() ; i++)
	{
		InsertListItem(BlrgKeyList[i]);
	}

	if(BlrgKeyList.GetSize()>0)
	{
		m_pDoc->m_pAttrCtrl->GetBlrg(BlrgKeyList[0], m_MainData);
		LoadDlgFactorPosition();
	}
