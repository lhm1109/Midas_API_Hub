// RatingLoadCombSteelDlg.cpp : implementation file
//
...
#include "RatingLoadCombSteelDlg.h"
...
CRatingLoadCombSteelDlg::CRatingLoadCombSteelDlg(CWnd* pParent )
	: CRatingLoadCombCommonDlg(CRatingLoadCombSteelDlg::IDD, pParent)
{
	// *^^*  여기를 바꾸시오.

	//{{AFX_DATA_INIT(CRatingLoadCombSteelDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	
	m_pLoadCombList = new CRatingLoadCombSteelMainGrid;

	m_pContentList = new CRatingLoadCombSteelSubGrid(0);
	m_pContentListMV = new CRatingLoadCombSteelSubGrid(1);
}

CRatingLoadCombSteelDlg::~CRatingLoadCombSteelDlg()
{
...
void CRatingLoadCombSteelDlg::DoDataExchange(CDataExchange* pDX)
{
	CRatingLoadCombCommonDlg::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CRatingLoadCombSteelDlg)


	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CRatingLoadCombSteelDlg, CRatingLoadCombCommonDlg)
	//{{AFX_MSG_MAP(CRatingLoadCombSteelDlg)
	ON_BN_CLICKED(IDC_BTN_COPY_TO_LCOM, OnCopyToLCom)	
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CRatingLoadCombSteelDlg message handlers
BOOL CRatingLoadCombSteelDlg::OnInitDialog() 
{
...
void CRatingLoadCombSteelDlg::UpdateBuffer()
{	
	...
	while(pos != NULL)
	{
		...
		if (nCmd != UR_RLCS_ADD && nCmd != UR_RLCS_DEL) continue;

		switch(nCmd)
		{
		case(UR_RLCS_ADD):
				// 삽입될 위치(nRow)를 찾아 넣는다.
				 pViewBuff->GetRlcs(nKey, Key, Data);
				 ((CRatingLoadCombSteelMainGrid*)m_pLoadCombList)->InsertRecordByKey(Key, Data);
				break;
		case(UR_RLCS_DEL):
				// 삭제될 위치(nRow)를 찾아서 지운다.
				pViewBuff->GetRlcs(nKey, Key, Data);
				((CRatingLoadCombSteelMainGrid*)m_pLoadCombList)->DeleteRecordByKey(Key, Data);
				break;
		default:
			break;
		}
...
void CRatingLoadCombSteelDlg::OnCopyToLCom()
{
	...
	CRatingLoadCombUtil::CopySteelRatingToLCom(m_pDoc);
}