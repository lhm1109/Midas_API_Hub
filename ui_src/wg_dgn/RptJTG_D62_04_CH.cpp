// RptJTG_D62_04_CH.cpp: implementation of the CRptJTG_D62_04_CH class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "..\DgnEngine\Src\DgnCalcBase\DgnCalcBase_PSC_Struct.h"
#include "RptJTG_D62_04_CH.h"
#include <winbase.h>
...
CRptJTG_D62_04_CH::CRptJTG_D62_04_CH()
{
    ...
    if(m_ChrcD.iDgnCode==RC_JTG_D62_04 || m_ChrcD.iDgnCode==RC_CJJ_11_2011)
    {
...
CString CRptJTG_D62_04_CH::GetFilePathName()
{
...
BOOL CRptJTG_D62_04_CH::Get_DetailBdcyPos(T_ELEM_K ElemK)
{
    //++++++++++++by maxiao
return TRUE; 
}

BOOL CRptJTG_D62_04_CH::Get_DetailBdcyNeg(T_ELEM_K ElemK)
{
    //++++++++++++by maxiao
return TRUE; 
}
...
//!/
BOOL CRptJTG_D62_04_CH::MakeData()
{
...
BOOL CRptJTG_D62_04_CH::MakeDataFromDB()
{
    GetMaterialProperty();
...
void CRptJTG_D62_04_CH::InitializeData()
{
    m_pRptData->m_TitleType[3] = 1; // Title4ÀÇ Çü½ÄÀ» (1)·Î º¯°æ
    m_iChapNum = 1;
}

BOOL CRptJTG_D62_04_CH::MakeDefaultStyle()
{
...
// 0-1. Ç¥Áö Data ±¸¼º.
BOOL CRptJTG_D62_04_CH::MakeCoverData()
{
...
// 0-2. ¸ñÂ÷ Data¸¦ ±¸¼º.
BOOL CRptJTG_D62_04_CH::MakeContents()
{
    return TRUE;  
}

// 1. ¼³°è±âÁØ Data¸¦ ±¸¼ºÇÔ.
BOOL CRptJTG_D62_04_CH::MakeCodes()
{
    ...
    if(m_ChrcD.iDgnCode == RC_CJJ_11_2011)
    {
...
// 2. ¼³°èÀÚ·á Data¸¦ ±¸¼ºÇÔ.
BOOL CRptJTG_D62_04_CH::MakeDesignParameter()
{
    T_CHAP_K ChapK, Chap2K;  
    T_CHAP_RPT ChapD; ChapD.Initialize();
    T_CHAP_RPT Chap2D; Chap2D.Initialize();
...
// 3. ÀçÁú Data¸¦ ±¸¼ºÇÔ.
BOOL CRptJTG_D62_04_CH::MakeMaterialData()
{
...
BOOL CRptJTG_D62_04_CH::MakeModelData()
{
    CString strTitle, strText;
...
BOOL CRptJTG_D62_04_CH::MakeLComData()
{
    CString strText;
...
BOOL CRptJTG_D62_04_CH::MakeCheckResultTable()
{
    ...
    if (chrcD.bDesignPrint[2]) {
        ...
        Chap2D = MakeChapter(2, iChap2DCount, _T("Ê©¹¤½×¶ÎÊÜÀ­Çø¸Ö½îµÄÀ­Ó¦Á¦ÑéËã"));
        ...
        Chap2D = MakeChapter(2, iChap2DCount, _T("Ê©¹¤½×¶ÎÖÐÐÔÖáÖ÷À­Ó¦Á¦ÑéËã"));
        if(MakeConstructStagecsncData(iChap2DCount, ParaD))
        {
            m_pRptData->AddParaData(ParaK, ParaD);
            Chap2D.arParaK.Add(ParaK);  
            m_pRptData->AddChapData(Chap2K, Chap2D);
    ...
    {
        Chap2D = MakeChapter(2, iChap2DCount, _T("Ê¹ÓÃ½×¶ÎÁºµÄÁÑ·ì¿í¶ÈÑéËã"));
    ...
    {
        Chap2D = MakeChapter(2, iChap2DCount, _T("ÁºµÄ×ÝÏò¸Ö½îÁ¿¹ÀËã"));
        if(MakeBrlrData(iChap2DCount, ParaD))
        {
            m_pRptData->AddParaData(ParaK, ParaD);
            Chap2D.arParaK.Add(ParaK);  
            m_pRptData->AddChapData(Chap2K, Chap2D);
    ...
    {
        Chap2D = MakeChapter(2, iChap2DCount, _T("ÁºµÄÆÕÍ¨¹¿½îÓÃÁ¿¹ÀËã"));
        if(MakeBrcrData(iChap2DCount, ParaD))
        {
            m_pRptData->AddParaData(ParaK, ParaD);
            Chap2D.arParaK.Add(ParaK);  
            m_pRptData->AddChapData(Chap2K, Chap2D);
    ...
    if(chrcD.bDesignPrint[8])  // 
    {
        Chap2D = MakeChapter(2, iChap2DCount, _T("Ê¹ÓÃ½×¶ÎÖùµÄÁÑ·ì¿í¶ÈÑéËã"));
        if(MakeServiceStageColumnCrackData(iChap2DCount, ParaD))
        {
            m_pRptData->AddParaData(ParaK, ParaD);
            Chap2D.arParaK.Add(ParaK);  
            m_pRptData->AddChapData(Chap2K, Chap2D);
    ...
    {
        Chap2D = MakeChapter(2, iChap2DCount, _T("ÖùµÄ¿¹À­³ÐÔØÁ¦ÑéËã"));
        if(MakeColumnCtcsData(iChap2DCount, ParaD))
        {
            m_pRptData->AddParaData(ParaK, ParaD);
            Chap2D.arParaK.Add(ParaK);  
            m_pRptData->AddChapData(Chap2K, Chap2D);
    ...
    {
        Chap2D = MakeChapter(2, iChap2DCount, _T("ÖùµÄ¿¹Ñ¹³ÐÔØÁ¦ÑéËã"));
        if(MakeColumnCscsData(iChap2DCount, ParaD))
        {
            m_pRptData->AddParaData(ParaK, ParaD);
            Chap2D.arParaK.Add(ParaK);  
            m_pRptData->AddChapData(Chap2K, Chap2D);
    ...
    if(chrcD.bDesignPrint[3])  // 
    {
        Chap2D = MakeChapter(2, iChap2DCount, _T("ÖùµÄ×ÝÏò¸Ö½îÁ¿¹ÀËã"));
        if(MakeCrlrData(iChap2DCount, ParaD))
        {
            m_pRptData->AddParaData(ParaK, ParaD);
            Chap2D.arParaK.Add(ParaK);  
            m_pRptData->AddChapData(Chap2K, Chap2D);
...
BOOL CRptJTG_D62_04_CH::MakeConcData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
    paraD.Initialize();
...
// 3.2. ÇÁ¸®½ºÆ®·¹½º Ã¶±Ù.
BOOL CRptJTG_D62_04_CH::MakePrestressRebar(int& iChapID, T_PARA_RPT& paraD)
{
    return TRUE;
...
BOOL CRptJTG_D62_04_CH::MakeTensionRebar(int& iChapID, T_PARA_RPT& paraD)
{
    //++++++++++++by maxiao
return TRUE;
}
...
BOOL CRptJTG_D62_04_CH::MakeShearRebar(int& iChapID, T_PARA_RPT& paraD)
{
    //++++++++++++++by maxiao
        return FALSE;
}
...
BOOL CRptJTG_D62_04_CH::MakeRebar(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
    paraD.Initialize();
...
// 4.5. Stage Data ±¸¼º.
BOOL CRptJTG_D62_04_CH::MakeStageGroupSummaryData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeElemGroupData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeBoundaryGroupData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeLoadGroupData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeServiceStageLoadData(int iChapID, T_CHAP_RPT& chapD)
{  
...
BOOL CRptJTG_D62_04_CH::MakeLiveLoadData(T_PARA_RPT& paraD)
{
    int iExistCount = 0;
...
BOOL CRptJTG_D62_04_CH::MakeSystemTempData(int& iChapID, T_PARA_RPT& paraD)
{  
    int iExistCount = 0;
...
BOOL CRptJTG_D62_04_CH::MakeNodalTempData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
...
BOOL CRptJTG_D62_04_CH::MakeElemTempData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
...
BOOL CRptJTG_D62_04_CH::MakeTempGradientData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
...
BOOL CRptJTG_D62_04_CH::MakeBeamSectTempData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;
...
BOOL CRptJTG_D62_04_CH::MakeSpecifiedDispData(T_PARA_RPT& paraD)
{
    int iExistCount = 0;
...
BOOL CRptJTG_D62_04_CH::MakeStaticLoadCaseData(int& iChapID, T_PARA_RPT& paraD)
{
...
BOOL CRptJTG_D62_04_CH::MakeResponseSpectrumData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = 0;  
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeTimeHistoryData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeMovingLoadData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeSettlementData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeGridLoadData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeLoadCombCaseData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
...
BOOL CRptJTG_D62_04_CH::MakeLoadCombData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
...
// 6.1. Ê©¹¤½×¶ÎÕý½ØÃæ·¨ÏòÓ¦Á¦ÑéËã.
BOOL CRptJTG_D62_04_CH::MakeConstructStageVertStressData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    CString aHeadNames1[] = { _LS(IDS_TB_PSCD_FYSG_JTG_ELEM), _LS(IDS_TB_PSCD_FYSG_JTG_PART),_LS(IDS_TB_PSCD_BDCW_JTG_TYPE), 
...
//  }
    ...
    thData.arTDK.Add(AddTDwithUnit(_LS(IDS_TB_PSCD_FYSG_JTG_FMAX), m_strStressUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_LS(IDS_TB_PSCD_FYSG_JTG_ALW), m_strStressUnit, 1, 1, 1));
...
//  tableD.arTHK.Add(thK);
    ...
    {
        T_CSCC_CH Cscc;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCsccData(m_arElemKeyList[i], Cscc);
        for(int j=0; j<4; j++)
        {
//      if (j%2 > 0) continue;
            tdData.Initialize();

            T_CSCC_CH_BASE CsccBase = Cscc.CsccBase[j];
...
//        strText = (j%2 > 0)? _T("×îÐ¡") : _T("×î´ó");     
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
                strText = GetLoadCombType(CsccBase.iMax);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
            
                T_STAG_D stagD;
                m_pDoc->m_pAttrCtrl->GetStag(CsccBase.StagK, stagD);
                tdData.arTDK.Add(AddTDString(stagD.StageName, 0, 1, 1, m_TDCenterCssK));

                strText = (CsccBase.bOK)? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
//
//        strText.Format(_T("%.3f"), CsccBase.dFT);
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
//
//        strText.Format(_T("%.3f"), CsccBase.dFTL);
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
//
//        strText.Format(_T("%.3f"), CsccBase.dFTR);
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CsccBase.dFMAX);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CsccBase.dALW);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
...
//        tdData.Initialize();
//
//        strText.Format(_T("%.3f"), CsccBase.dFB);
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
//
//        strText.Format(_T("%.3f"), CsccBase.dFBL);
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
//
//        strText.Format(_T("%.3f"), CsccBase.dFBR);
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
//
//        tdData.CssK = m_pRptData->m_DefaultCss.TDCssK;
//        m_pRptData->AddTRData(tdK, tdData);
...
// 6.2.Ê©¹¤½×¶ÎÊÜÀ­Çø¸Ö½îµÄÀ­Ó¦Á¦ÑéËã   
BOOL CRptJTG_D62_04_CH::MakeTensRebarStressData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    CString strText = _T("");
    ...
    CString aHeadNames1[] = { _T("µ¥Ôª"),_T("Î»ÖÃ"),_T("¸Ö½î"), _LS(IDS_TB_PSCD_FYPC_JTG_OK) };
    for(int i=0; i<4; i++)
    {
        thData.arTDK.Add(AddTDString(aHeadNames1[i], 1, 1, 1));
    }
    ...
    CString aHeadNames2[] = { _LS(IDS_TB_PSCD_FYPC_JTG_FDL1),_LS(IDS_TB_PSCD_FYPC_JTG_FLL),_LS(IDS_TB_PSCD_FYPC_JTG_AFDL1),_LS(IDS_TB_PSCD_FYPC_JTG_AFLL) };
    for(int i=0; i<4; i++)
    {
        thData.arTDK.Add(AddTDwithUnit(aHeadNames2[i], m_strStressUnit, 1, 1, 1));
    }
    ...
    //++++++++++++++++++++
    CArray<UINT, UINT> aCtstKey;
    m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCtstkeyList(aCtstKey);
    int nCtstCount = aCtstKey.GetSize();
    //+++++++++++++++++++++++
    for(int i=0; i<nCtstCount; i++)
    {
        T_CTST_CH ctst;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCtstData(aCtstKey[i], ctst);
        if(ctst.bCHK)
        {
            tdData.Initialize();    
            strText.Format(_T("%d"), ctst.iposi);
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

            strText = (ctst.ilayer<1)? _T("I") : _T("J");     
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

            strText.Format(_T("Rbar-%ld"), ctst.iRbarNa);         // elem
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

            strText = (ctst.bOK)? _T("OK") : _T("NG");
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

            strText.Format(_T("%.3f"), ctst.dDLL);
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

            strText.Format(_T("%.3f"), ctst.dFLL);
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

            strText.Format(_T("%.3f"), ctst.dADLL);
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

            strText.Format(_T("%.3f"), ctst.dAFLL);
            tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
...
// 6.3. Ê©¹¤½×¶ÎÖÐÐÔÖáÖ÷À­Ó¦Á¦ÑéËã
BOOL CRptJTG_D62_04_CH::MakeConstructStagecsncData(int& iChapID, T_PARA_RPT& paraD){
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    CString aHeadNames1[] = { _LS(IDS_TB_PSCD_FYSG_JTG_ELEM), _LS(IDS_TB_PSCD_FYSG_JTG_PART),
        /*_LS(IDS_TB_PSCD_FYSG_JTG_MAX_MIN), */_LS(IDS_TB_PSCD_FYSG_JTG_STAGE),//_LS(IDS_TB_PSCD_FYSG_JTG_STAGE),
    ...
    CString aHeadNames2[] = { _T("Sig_M"), _T("Sig_ML"),_T("Sig_MR") };
    for(int i=0; i<3; i++)
    {
        thData.arTDK.Add(AddTDwithUnit(aHeadNames2[i], m_strStressUnit, 1, 1, 1));   
    }
    ...
    {
        T_CSNC_CH Csnc;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCsncData(m_arElemKeyList[i], Csnc);
        for(int j=0; j<4; j++)
        {
            if (j%2 == 0) continue;// add by maxiao to remove the max value

            tdData.Initialize();

            T_CSNC_CH_BASE CsncBase = Csnc.CsncBase[j];
            ...
            if(CsncBase.bCHK)
            {
                ...
                T_STAG_D stagD;
                m_pDoc->m_pAttrCtrl->GetStag(CsncBase.StagK, stagD);
                tdData.arTDK.Add(AddTDString(stagD.StageName, 0, 1, 1, m_TDCenterCssK));
//        strText = GetLoadCombName(CsncBase.StagK);
//        tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (CsncBase.bOK)? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CsncBase.dFM);
                
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CsncBase.dFML);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CsncBase.dFMR);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CsncBase.dFMAX);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CsncBase.dALW);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
...
//6.4¡¡Ê¹ÓÃ½×¶ÎÁÑ·ì¿í¶ÈÑéËã
BOOL CRptJTG_D62_04_CH::MakeServiceStageCrackData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    thData.arTDK.Add(AddTDwithUnit(_LS(IDS_TB_PSCD_BDCW_JTG_FRT), m_strStressUnit, 1, 1, 1));
    ...
    {
        T_CCWS_CH Ccws;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcBeamCcwsData(m_arElemKeyList[i], Ccws);
        
        for(int j=0; j<4; j++)
        {
            tdData.Initialize();

            T_CCWS_CH_BASE CcwsBase = Ccws.ccwsBase[j];
            ...
            if(CcwsBase.bCHK)
            {
                ...
                strText = GetLoadCombName(CcwsBase.LcomK);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = GetLoadCombType(CcwsBase.iMax);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (CcwsBase.bOK)? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText.Format(_T("%.3f"), CcwsBase.dFRT);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
                
                double dWs[] = { CcwsBase.dWC, CcwsBase.dAWC };
                for(int k=0; k<2; k++)
                {
...
//6.5 ÁºµÄ×ÝÏò¸Ö½îÁ¿¹ÀËã
BOOL CRptJTG_D62_04_CH::MakeBrlrData(int& iChapID, T_PARA_RPT& paraD){
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    CString aHeadNames1[] = { _LS(IDS_TB_PSCD_TBAR_JTG_ELEM), _LS(IDS_TB_PSCD_TBAR_JTG_PART), 
            _T("×î´ó/×îÐ¡"), _LS(IDS_TB_PSCD_TBAR_JTG_LOAD),
            _LS(IDS_TB_PSCD_TBAR_JTG_TYPE), _LS(IDS_TB_PSCD_TBAR_JTG_OK) };
    for(int i=0; i<6; i++)
    {
        thData.arTDK.Add(AddTDString(aHeadNames1[i], 1, 1, 1));
    }
    thData.arTDK.Add(AddTDwithUnit(_LS(IDS_TB_PSCD_TBAR_JTG_Mj), m_strMomentUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("Ag_REQ_T"), m_arstrAreaUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("Ag_REQ_B"), m_arstrAreaUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("Ag_USE_T"), m_arstrAreaUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("Ag_USE_B"), m_arstrAreaUnit, 1, 1, 1));
    ...
    {
        T_BRLR_CH brlr;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcBrlrData(m_arElemKeyList[i], brlr);
        for(int j=0; j<4; j++)
        {
            tdData.Initialize();

            T_BRLR_CH_BASE BrlrBase = brlr.BrlrBase[j];
            ...
            if(BrlrBase.bCHK)
            {
                ...
                strText = GetLoadCombName(BrlrBase.LcomK);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = GetLoadCombType(BrlrBase.iMax);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (BrlrBase.bOK)? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                double dWs[] = { BrlrBase.dMd*m_dMomentkNM, BrlrBase.dREQTop, BrlrBase.dREQBot, BrlrBase.dUSETop, BrlrBase.dUSEBot };
                for(int k=0; k<5; k++)
                {
                    strText.Format(_T("%.4f"), dWs[k]);
                    tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
...
//6.6 ÁºµÄÆÕÍ¨¹¿½îÓÃÁ¿¹ÀËã
BOOL CRptJTG_D62_04_CH::MakeBrcrData(int& iChapID, T_PARA_RPT& paraD){
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    CString aHeadNames1[] = { _LS(IDS_TB_PSCD_TBAR_JTG_ELEM), _LS(IDS_TB_PSCD_TBAR_JTG_PART), 
            _LS(IDS_TB_PSCD_BDCY_JTG_MAX_MIN), _LS(IDS_TB_PSCD_TBAR_JTG_LOAD),
            _LS(IDS_TB_PSCD_TBAR_JTG_TYPE), _LS(IDS_TB_PSCD_TBAR_JTG_OK) };
    for(int i=0; i<6; i++)
    {
        thData.arTDK.Add(AddTDString(aHeadNames1[i], 1, 1, 1));
    ...
    thData.arTDK.Add(AddTDwithUnit(_T("Vj"), m_strForceUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_LS(IDS_TB_PSCD_TBAR_JTG_REQ), m_arstrAreaUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_LS(IDS_TB_PSCD_TBAR_JTG_USE), m_arstrAreaUnit, 1, 1, 1));
    ...
    {
        T_BRCR_CH brcr;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcBrcrData(m_arElemKeyList[i], brcr);
        for(int j=0; j<4; j++)
        {
            tdData.Initialize();

            T_BRCR_CH_BASE BrcrBase = brcr.BrcrBase[j];
            ...
            if(BrcrBase.bCHK)
            {
                ...
                strText = GetLoadCombName(BrcrBase.LcomK);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = GetLoadCombType(BrcrBase.iMax);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (BrcrBase.bOK)? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                double dWs[] = { BrcrBase.dVd*m_dForcekN, BrcrBase.dREQ, BrcrBase.dUSE };
                for(int k=0; k<3; k++)
                {
                    strText.Format(_T("%.4f"), dWs[k]);
                    tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
...
// 6.7. Ê¹ÓÃ½×¶Î¿¹ÍäÑéËã.
BOOL CRptJTG_D62_04_CH::MakeServiceStageMomentCheckData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    {
        T_CBCS_CH cbcs;      
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCbcsData(m_arElemKeyList[i], cbcs);
...
// 6.8. Ð±½ØÃæ¿¹¼ôÑéËã
BOOL CRptJTG_D62_04_CH::MakeServiceStageShearCheckData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    {
        T_CSIS_CH csis;  
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCsisData(m_arElemKeyList[i], csis);
...
// 6.9.¿¹Å¤ÑéËã
BOOL CRptJTG_D62_04_CH::MakeServiceStageTorsionCheckData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    {
        T_CTSS_CH Ctss;      
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCtssData(m_arElemKeyList[i], Ctss);
                    
        for(int j=0; j<6; j++)
        {
            tdData.Initialize();

            T_CTSS_CH_BASE ctssBase = Ctss.CtssBase[j];
            ...
            if(ctssBase.bCHK)
            {
                ...
                strText = GetLoadCombName(ctssBase.LcomK);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = GetLoadCombType(ctssBase.iMax);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (ctssBase.bOK)? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                double dFP1[] = { ctssBase.drTu, ctssBase.dTn, ctssBase.drVu, ctssBase.dVn };
                for(int k=0; k<4; k++)
                {
                ...
                strText = (fabs(ctssBase.dShrTorVal) > ctssBase.dTorSect)? _T("NG") : _T("OK") ;
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (fabs(ctssBase.dShrTorVal) <= ctssBase.dTorLoad)? _LS(IDS_TB_PSCD_CRMT_JTG_TORLOAD_SKIP) : _LS(IDS_TB_PSCD_CRMT_JTG_TORLOAD_CHECK);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
...
//6.4¡¡Ê¹ÓÃ½×¶ÎÁÑ·ì¿í¶ÈÑéËã
BOOL CRptJTG_D62_04_CH::MakeServiceStageColumnCrackData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    {
        T_CCWS_CH Ccws;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcColumnccwsData(m_arColumnElemKeyList[i], Ccws);
        
        for(int j=0; j<4; j++)
        {
			if (j==1 || j==3) continue;//½öÊä³ö×î´ó
            tdData.Initialize();
            ...
            if(CcwsBase.bCHK)
            {
                ...
                CString TopBot[2] = {_T("µ×"),_T("¶¥")};
				if (CcwsBase.iKind ==0 || CcwsBase.iKind ==1) 
					strText = TopBot[CcwsBase.iKind];
				else strText = _T("-");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
...
// 6.11.ÖùµÄÖá/Æ«ÐÄ¿¹À­ÑéËã
BOOL CRptJTG_D62_04_CH::MakeColumnCtcsData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    CString aHeadNames1[] = { _LS(IDS_TB_PSCD_BDCY_JTG_ELEM), _LS(IDS_TB_PSCD_BDCY_JTG_PART), _LS(IDS_TB_PSCD_BDCW_JTG_LOAD), 
        ...
        _T("rNd"), _T("e"), _T("e1"),  _T("Nn")};//, _T("Nn*e"), _T("Nn*e1")};

    CString aMaxMinName[]= {_T("ÖáÐÄ-Fxmax"),_T("Æ«ÐÄ-Fxmax(My)"),_T("Æ«ÐÄ-Fxmax(Mz)"),_T("Æ«ÐÄ-Mymax"),_T("Æ«ÐÄ-Mymin")};
    ...
    {
        T_CSTCS_CH Ctcs;      
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCtcsData(m_arColumnElemKeyList[i], Ctcs);
        ...
        for(int j=0; j<10; j++)
        {
            ...
            T_CSTCS_CH_BASE ctcsBase = Ctcs.cstcsBase[j];
            ...
            if(ctcsBase.bCHK)
            {
             strText.Format(_T("%d"), m_arColumnElemKeyList[i]);
                ...
                strText = GetLoadCombName(ctcsBase.LcomK);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText =aMaxMinName[j%5];
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (ctcsBase.bOK)? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

//        double dFP1[] = {ctcsBase.drNd,ctcsBase.drNde,ctcsBase.drNde1,ctcsBase.dNn};//,ctcsBase.dNne,ctcsBase.dNne1};
                for(int k=0; k<4; k++)
                {
                    if (ctcsBase.drNd >0 ) {//is not tensile
                        if (k ==0)  strText.Format(_T("%.3f"), ctcsBase.drNd * m_dForcekN); else strText = _T("-");
                    }else{
                        if (k ==0) strText.Format(_T("%.3f"), ctcsBase.drNd * m_dForcekN);
                        if (k ==1) strText.Format(_T("%.3f"), ctcsBase.drNd > -1.0E-07 ? 0.0 : ctcsBase.drNde /ctcsBase.drNd);
                        if (k ==2) strText.Format(_T("%.3f"), ctcsBase.drNd > -1.0E-07 ? 0.0 : ctcsBase.drNde1/ctcsBase.drNd);
                        if (k ==3) strText.Format(_T("%.3f"), ctcsBase.dNn * m_dForcekN);
                    }
                    tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
                }      
...
// 6.12.ÖùµÄÖá/Æ«ÐÄ¿¹Ñ¹ÑéËã
BOOL CRptJTG_D62_04_CH::MakeColumnCscsData(int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    {
        T_CSTCS_CH Cscs;      
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCscsData(m_arColumnElemKeyList[i], Cscs);
...
//6.13 ÖùµÄ×ÝÏò¸Ö½îÁ¿¹ÀËã
BOOL CRptJTG_D62_04_CH::MakeCrlrData(int& iChapID, T_PARA_RPT& paraD){
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    CString aHeadNames1[] = { _LS(IDS_TB_PSCD_TBAR_JTG_ELEM), _LS(IDS_TB_PSCD_TBAR_JTG_PART), 
            _LS(IDS_TB_PSCD_TBAR_JTG_LOAD),_LS(IDS_TB_PSCD_TBAR_JTG_TYPE),_LS(IDS_TB_PSCD_BDCY_JTG_OK)};
    for(int i=0; i<5; i++)
    {
        thData.arTDK.Add(AddTDString(aHeadNames1[i], 1, 1, 1));
    }
    thData.arTDK.Add(AddTDwithUnit(_T("Nd"), m_strForceUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("Myd"), m_strMomentUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("Mzd"), m_strMomentUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("¶¥_REQ"), m_arstrAreaUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("×ó_REQ"), m_arstrAreaUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("¶¥_USE"), m_arstrAreaUnit, 1, 1, 1));
    thData.arTDK.Add(AddTDwithUnit(_T("×ó_USE"), m_arstrAreaUnit, 1, 1, 1));
    ...
    {
        T_CRLR_CH crlr;
        m_pDoc->m_pPostCtrl->GetAnalysisResult()->GetCHRcCrlrData(m_arColumnElemKeyList[i], crlr);
        for(int j=0; j<4; j++)
        {
            tdData.Initialize();

            T_CRLR_CH_BASE CrlrBase = crlr.CrlrBase[j];
            ...
            if(CrlrBase.bCHK)
            {
                ...
                strText = GetLoadCombName(CrlrBase.LcomK);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = GetLoadCombType(CrlrBase.iMax);
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                strText = (CrlrBase.dREQTop<=CrlrBase.dUSETop && CrlrBase.dREQLeft<=CrlrBase.dUSELeft) ? _T("OK") : _T("NG");
                tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));

                double dWs[] = { CrlrBase.dNd*m_dForcekN,CrlrBase.dMyd*m_dMomentkNM,CrlrBase.dMzd*m_dMomentkNM,CrlrBase.dREQTop,CrlrBase.dREQLeft,CrlrBase.dUSETop,CrlrBase.dUSELeft};
                for(int k=0; k<7; k++)
                {
                    strText.Format(_T("%.3f"), dWs[k]);
                    tdData.arTDK.Add(AddTDString(strText, 0, 1, 1, m_TDCenterCssK));
                }
...
// T_FYCM_JTG Data Ãâ·Â°ü·Ã Á¤º¸¸¦ ±¸¼ºÇÔ.
BOOL CRptJTG_D62_04_CH::MakeFycmJTGData(BOOL bMax, int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
    ...
    for(int i=0; i<m_iElemNum; i++)
    {
        ...
        // Reference this index. (See struct T_FYCM_JTG) ´Ü±â : 0=I-MAX, 1=I-MIN, 2=J-MAX, 3=J-MIN. Àå±â : 4=I-MIN, 5=J-MIN
        if(bMax)
        {
            for(int j=4; j<6; j++)
            {
                ...
                if(fycmBase.bCHK)
                {
                    ...
                    for(int k=0; k<5; k++)
                    {
                        ...
                        int iSubIndex = (k>2)? 2 : 1;  // dFMAX, dALW´Â RowMerge ÇüÅÂ·Î Ãâ·Â.
...
// T_FYMS_JTG Data Ãâ·Â°ü·Ã Á¤º¸¸¦ ±¸¼ºÇÔ.
BOOL CRptJTG_D62_04_CH::MakeFymsJTGData(BOOL bMax, int& iChapID, T_PARA_RPT& paraD)
{
    int iExistCount = FALSE;
    paraD.Initialize();
...
CString CRptJTG_D62_04_CH::GetFileDate()
{ 
    CString strDate = _T("");
...
// Ãæ°Ý°è¼ö °ªÀ» °¡Á®¿È.
CString CRptJTG_D62_04_CH::GetImpactCoefficient()
{ 
...
CString CRptJTG_D62_04_CH::GetBeamSectTempLoadDirection(int iDir)
{
    CString strText = _T("");
    if(iDir == 0)
    {
...
CString CRptJTG_D62_04_CH::GetLcombActiveType(int iActiveType)
{
    CString strText = _T("");
...
CString CRptJTG_D62_04_CH::GetStageActivationType(int iCondition)
{
    CString strText = _T("");
...
CString CRptJTG_D62_04_CH::GetStageTotalType(int iSummary)
{
    return (iSummary == 1)? _T("O") : _T("");
}

// ¼³°è¿ë ÇÏÁßÁ¶ÇÕ Key¸¦ ÀÔ·Â¹Þ¾Æ ½ÇÁ¦ ÇÏÁßÁ¶ÇÕÀÇ ÀÌ¸§ ±¸ÇÔ.
CString CRptJTG_D62_04_CH::GetLoadCombName(T_LCOM_K lcomK)
{
    T_LCOM_K orglcomK;
    T_LCOM_D lcomD;
...
CString CRptJTG_D62_04_CH::GetStaticLoadCaseName(T_STLD_K stldK)
{
    T_STLD_D stldD;
...
CString CRptJTG_D62_04_CH::GetLoadCombType(int iMaxType)
{
...
CString CRptJTG_D62_04_CH::GetTopBottomType(int iLocType)
{
...
CString CRptJTG_D62_04_CH::GetMaxMinType(int iMaxType)
{
...
CString CRptJTG_D62_04_CH::GetMaxMinTypeShear(int iMaxType)
{
...
void CRptJTG_D62_04_CH::GetElementGroupList(T_STAG_K stagK, CArray<T_TEXT_K, T_TEXT_K>& arActiveK, CArray<T_TEXT_K, T_TEXT_K>& arDeactiveK)
{
    arActiveK.RemoveAll();
...
void CRptJTG_D62_04_CH::GetBoundaryGroupList(T_STAG_K stagK, CArray<T_TEXT_K, T_TEXT_K>& arActiveK, CArray<T_TEXT_K, T_TEXT_K>& arDeactiveK)
{
    arActiveK.RemoveAll();
    arDeactiveK.RemoveAll();
...
void CRptJTG_D62_04_CH::GetLoadGroupList(T_STAG_K stagK, CArray<T_TEXT_K, T_TEXT_K>& arActiveK, CArray<T_TEXT_K, T_TEXT_K>& arDeactiveK)
{
    arActiveK.RemoveAll();
    arDeactiveK.RemoveAll();
...
//   iNumber : ¹Ù²Ü ¼ýÀÚ.
CString CRptJTG_D62_04_CH::GetNumberCharacterForChina(int iNumber)
{
    CString result = _T("");
    switch(iNumber)
    {
...
//   iYY : ³âµµ.
//   iMM : ´Þ.
CString CRptJTG_D62_04_CH::GetFileDateForChina(int iYY, int iMM)
{  
    TCHAR i_str[32] = _T("");
...
BOOL CRptJTG_D62_04_CH::GetTendonMatlData(CString strTndnName, double& dEc, double& dDensity, double& dThermal)
{
	//++++++++++++++++++++++++++++++++++++++++++++
	// <Remember> Get Data by Code Units.
	T_UNIT_INDEX CurIndex;
...
CString CRptJTG_D62_04_CH::GetLiveLoadTypeDescription(CString strTypeName)
{
    CString result = _T("");
...
CString CRptJTG_D62_04_CH::GetLoadCaseDescription(CString strCaseName)
{
    CString result = _T("");
...
void CRptJTG_D62_04_CH::GetLoadCombinationCases(CMap<CString, LPCTSTR, CString, CString&>& mapLcom)
{
    ...
    case D_MOVE_CODE_FRANCE:
        {
            ...
            {
                T_MVLDfr_D mvldbfrD;
                if(m_pDoc->m_pAttrCtrl->GetMvldfr(arMvldfrKeyList[i], mvldbfrD))
                {
                    strText.Format(_T("M[%d]"), mvldbfrD.LoadCaseId);
                    mapLcom.SetAt(strText, mvldbfrD.LoadCaseName);
                }
            }
        }
        break;
...
CString CRptJTG_D62_04_CH::GetConstructionStageLoadCaseSymbol(T_SGLD_K sgldK)
{
...
BOOL CRptJTG_D62_04_CH::CheckExistTemperatureLoad()
{  
    int iTempCount = 0;
    CArray<T_STLD_K, T_STLD_K> arStldKeyList;
    m_pDoc->m_pAttrCtrl->GetStldKeyList(arStldKeyList);
...
BOOL CRptJTG_D62_04_CH::CheckExistResultTableItem()
{
    int iTableCount = 0;
...
// T_CHAP_RPT Data »ý¼º.
T_CHAP_RPT CRptJTG_D62_04_CH::MakeChapter(int iLevel, int iChapID, CString strText, T_CSS_K cssK/*0*/)
{
    T_CHAP_RPT chapD;
    chapD.Initialize();
    chapD.nLevel = iLevel;
...
BOOL CRptJTG_D62_04_CH::GetDesignStringPara(CStringArray& arrString)
{ 
    CString strText;
    // ¼³°è´Ü¸é·Â ¼ººÐ.  
...
BOOL CRptJTG_D62_04_CH::GetMaterialProperty()
{  
...
CString CRptJTG_D62_04_CH::GetModifyString(int iMaxLength, CString strText)
{
    CString str = strText;
...
T_TD_K CRptJTG_D62_04_CH::AddTDwithUnit(CArray<T_CHAR_K, T_CHAR_K>& arCharK, CString strUnit, int iTDType/*0*/, int iMergeCols/*1*/, int iMergeRows/*1*/, T_CSS_K CssK/*0*/)
{
...
T_TD_K CRptJTG_D62_04_CH::AddTDwithUnit(CArray<T_CHAR_K, T_CHAR_K>& arCharK, CArray<T_CHAR_K, T_CHAR_K>& arCharUnitK, int iTDType/*0*/, int iMergeCols/*1*/, int iMergeRows/*1*/, T_CSS_K CssK/*0*/)
{
    CArray<T_TEXT_K, T_TEXT_K> arTextK; arTextK.RemoveAll();
...
T_TD_K CRptJTG_D62_04_CH::AddTDwithUnit(LPCTSTR strText, CString strUnit, int iTDType/*0*/, int iMergeCols/*1*/, int iMergeRows/*1*/, T_CSS_K CssK/*0*/)
{
    CArray<T_TEXT_K, T_TEXT_K> arTextK; arTextK.RemoveAll();
...
T_TD_K CRptJTG_D62_04_CH::AddTDwithUnit(LPCTSTR strText, CArray<T_CHAR_K, T_CHAR_K>& arCharUnitK, int iTDType/*0*/, int iMergeCols/*1*/, int iMergeRows/*1*/, T_CSS_K CssK/*0*/)
{
    CArray<T_TEXT_K, T_TEXT_K> arTextK; arTextK.RemoveAll();
...
void CRptJTG_D62_04_CH::InitialarCharK()
{
    CString strText;
...
// 7.ÁºµÄÏêÏ¸¼ÆËã¹ý³Ì.
BOOL CRptJTG_D62_04_CH::MakeCheckBeamResultData()
{
    CString strPathName=m_pDoc->GetPathName();
    CFileCtrl FileCtrl(strPathName);
...
// 8. ÖùµÄÏêÏ¸¼ÆËã¹ý³Ì
BOOL CRptJTG_D62_04_CH::MakeCheckColumnResultData()
{
    CString strPathName=m_pDoc->GetPathName();
    CFileCtrl FileCtrl(strPathName);
