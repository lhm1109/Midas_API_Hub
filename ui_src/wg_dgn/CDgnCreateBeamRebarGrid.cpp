// CDgnCreateBeamRebarGrid.cpp: implementation of the CDgnCreateBeamRebarGrid class.
// (060123) sshan Create
//////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
// CDgnCreateBeamRebarGrid
CString CDgnCreateBeamRebarGrid::m_aCurDefVal[] = {_T(""), _T(""), _T(""), _T("")};


CDgnCreateBeamRebarGrid::CDgnCreateBeamRebarGrid()
{
...
BEGIN_MESSAGE_MAP(CDgnCreateBeamRebarGrid, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CDgnCreateBeamRebarGrid)
	ON_WM_RBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CDgnCreateBeamRebarGrid message handlers
void CDgnCreateBeamRebarGrid::Initialize(CArray<T_RBGC_MABR, T_RBGC_MABR&>* pData, CConcSectionCreateBeamRebar* pParent)
{
	...
	*/
	...
	{
		L_SetColWidth(1, 1, 0);
		L_SetColWidth(2, 2, 50);
		L_SetColWidth(3, 3, 70);
		L_SetColWidth(4, 5, 70);
		L_SetColWidth(6, 6, 70); // Hide
	}
	else
	{
		L_SetColWidth(1, 1, 0);
		L_SetColWidth(2, 2, 40);
		L_SetColWidth(3, 3, 50);
		L_SetColWidth(4, 5, 60);
		L_SetColWidth(6, 6, 50);
	}	
...
void CDgnCreateBeamRebarGrid::MakeItemEx()
{
	CWaitCursor wait;
...
BOOL CDgnCreateBeamRebarGrid::InsertRecordByKey(REBAR_KEY &key, T_RBGC_MABR &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CDgnCreateBeamRebarGrid::DeleteRecordByKey(REBAR_KEY &key, T_RBGC_MABR &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
void CDgnCreateBeamRebarGrid::UnitChanged(CDBDoc* pDoc)
{
...
/////////////////////////////////////////////////////////////////////////////
// CDgnCreateBeamRebarGrid Overridable Functions
void CDgnCreateBeamRebarGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	if (nSize <= 0)
	{
...
long CDgnCreateBeamRebarGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
...
BOOL CDgnCreateBeamRebarGrid::ValidateField(CString value, int nColID)
{
...
BOOL CDgnCreateBeamRebarGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CDgnCreateBeamRebarGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, REBAR_KEY &key, T_RBGC_MABR &data)
{
...
BOOL CDgnCreateBeamRebarGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CDgnCreateBeamRebarGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
	ASSERT(raKey.GetSize() == raData.GetSize());
	REBAR_KEY tKey;
	CArray<REBAR_KEY, REBAR_KEY> aKey;
	CArray<T_RBGC_MABR, T_RBGC_MABR&> aData;
	for (int i = 0; i < raKey.GetSize(); i++)
	{
		tKey = *((REBAR_KEY*)raKey.GetAt(i));
		aKey.Add(tKey);
		aData.Add(*((T_RBGC_MABR*)raData.GetAt(i)));
	}
...
BOOL CDgnCreateBeamRebarGrid::DeleteFromDB(ROWCOL nRow)
{
	REBAR_KEY key;
	T_RBGC_MABR data;
...
BOOL CDgnCreateBeamRebarGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CDgnCreateBeamRebarGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
...
BOOL CDgnCreateBeamRebarGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CDgnCreateBeamRebarGrid::GetNewKey()
{
	CString value;
	/*
...
CString CDgnCreateBeamRebarGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CDgnCreateBeamRebarGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CDgnCreateBeamRebarGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
																	CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	REBAR_KEY* pKey;
	T_RBGC_MABR* pData;
	pKey = new REBAR_KEY;
	pData = new T_RBGC_MABR;
	aKey.Add((void*)pKey);
...
void CDgnCreateBeamRebarGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	REBAR_KEY* pKey;
	T_RBGC_MABR* pData;
	...
	for (int i = 0; i < aKey.GetSize(); i++)
	{
		...
		pData = (T_RBGC_MABR*)aData.GetAt(i);
		delete pKey;
		delete pData;
	}
}
...
long CDgnCreateBeamRebarGrid::GetCountData()
{
	CDBDoc* pDoc = m_pDoc;
...
void CDgnCreateBeamRebarGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	CDBDoc* pDoc = m_pDoc;
	T_RBGC_MABR Data;
...
void CDgnCreateBeamRebarGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	//REBAR_KEY Key;
	T_RBGC_MABR Data;
	CDBDoc* pDoc = m_pDoc;
...
void CDgnCreateBeamRebarGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	REBAR_KEY Key;
	T_RBGC_MABR Data;
...
/////////////////////////////////////////////////////////////////////////////
// CDgnCreateBeamRebarGrid Implementation
BOOL CDgnCreateBeamRebarGrid::GetValue(ROWCOL nRow, REBAR_KEY &key, T_RBGC_MABR &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CDgnCreateBeamRebarGrid::SetValue(ROWCOL nRow, REBAR_KEY key, T_RBGC_MABR &data)
{
	//
	CRowColArray awCols;
...
BOOL CDgnCreateBeamRebarGrid::SetValue(CGXData* pData, ROWCOL nRow, REBAR_KEY key, T_RBGC_MABR &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CDgnCreateBeamRebarGrid::ConvStrToData(int i, CString& value, T_RBGC_MABR& data)
{
	switch(i)
	{
		case 0:
...
void CDgnCreateBeamRebarGrid::ConvDataToStr(int i, REBAR_KEY key, T_RBGC_MABR& data, CString& value)
{
	switch(i)
	{
		case 0:
...
void CDgnCreateBeamRebarGrid::MakeSearchKey(REBAR_KEY &key, T_RBGC_MABR &data, CStringArray &aKey)
{
	int nColID;
...
void CDgnCreateBeamRebarGrid::GetAllSelectedKey(CArray<REBAR_KEY, REBAR_KEY>& aSelKey)
{
	for (int i = 0; i < m_aSelectedKeys.GetSize(); i++)
		aSelKey.Add(m_aSelectedKeys[i]);
}

void CDgnCreateBeamRebarGrid::SetHeaderTitle(T_UNIT_SYSTEM &UnitSystem, BOOL bInit)
{
	// Set Title
	/*
	int aSize[11];
...
// 주1 참조
BOOL CDgnCreateBeamRebarGrid::OnTrackColWidth(ROWCOL nCol)
{
...
// 이동되는 것을 금지한다.
// 주1 참조
BOOL CDgnCreateBeamRebarGrid::OnSelDragColsDrop(ROWCOL nFirstCol, ROWCOL nLastCol, ROWCOL nDestCol)
{
...
/*
BOOL CDgnCreateBeamRebarGrid::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
/*
	if (IsEdit()) 
...
void CDgnCreateBeamRebarGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
...
/*
CString CDgnCreateBeamRebarGrid::GetLoadTypeCode(CString &csLoadTypeName)
{
	return m_pDoc->m_pLoadDB->GetLoadType(csLoadTypeName);
}

CString CDgnCreateBeamRebarGrid::GetLoadTypeName(CString &csLoadTypeCode)
{
...
BOOL CDgnCreateBeamRebarGrid::CheckCurrentRecordChanged()
{
	ROWCOL ncRow, ncCol;
...
BOOL CDgnCreateBeamRebarGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
...
void CDgnCreateBeamRebarGrid::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CDgnCreateBeamRebarGrid::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CDgnCreateBeamRebarGrid::OnChangeCurrentRecord(ROWCOL nRow)
{
	m_nCurrentRow = nRow;
...
BOOL CDgnCreateBeamRebarGrid::GetCurrentMaRb(REBAR_KEY& nKey)
{
	ROWCOL ncRow, ncCol;
...
BOOL CDgnCreateBeamRebarGrid::SetCurrentMaRb(REBAR_KEY key, T_RBGC_MABR& data)
{
	ROWCOL nRow, nCol;
...
void CDgnCreateBeamRebarGrid::DeleteRowByKey(REBAR_KEY key)
{
	REBAR_KEY keyData;
	T_RBGC_MABR data;
...
BOOL CDgnCreateBeamRebarGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CDgnCreateBeamRebarGrid::SaveGridData()
{
	...
	{
		MData.nLabel = _ttoi(GetValueRowCol(i, 2));
...
BOOL CDgnCreateBeamRebarGrid::OnInsertEmptyRecord(ROWCOL nRow)
{
...
BOOL CDgnCreateBeamRebarGrid::OnAddNewRecord(ROWCOL nRow)
{	
	return TRUE;   
}

REBAR_KEY CDgnCreateBeamRebarGrid::GetLastRebarKey()
{	
	REBAR_KEY key = 0;
	int RowCount = GetRowCount()-1;
	T_RBGC_MABR MData;
...
BOOL CDgnCreateBeamRebarGrid::GridCheckData()
{	
	int RowCount = GetRowCount()-1;
	BOOL result = TRUE;
...
BOOL CDgnCreateBeamRebarGrid::bCheck_int(CString str)
{	
...
BOOL CDgnCreateBeamRebarGrid::bCheck_double(CString str)
{	
	int iLength = str.GetLength();
	for(int i=0; i<iLength; i++)
	{
