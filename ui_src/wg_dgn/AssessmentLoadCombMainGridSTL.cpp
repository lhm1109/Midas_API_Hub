// AssessmentLoadCombMainGridSTL.cpp : implementation file
//
...
#include "AssessmentLoadCombSTLDlg.h"
...
/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGridSTL
CAssessmentLoadCombMainGridSTL::CAssessmentLoadCombMainGridSTL(CAssessmentLoadCombSTLDlg* pParent)
...
CAssessmentLoadCombMainGridSTL::~CAssessmentLoadCombMainGridSTL()
{
}

#define CWnd CTBCommon
BEGIN_MESSAGE_MAP(CAssessmentLoadCombMainGridSTL, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CAssessmentLoadCombMainGridSTL)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGridSTL message handlers
void CAssessmentLoadCombMainGridSTL::Initialize(CDBDoc* pDoc, T_UNIT_SYSTEM &UnitSystem)
{
	...
	m_pDoc->m_pAttrCtrl2->GetRcds(RcdsD);

	if (RcdsD.nRatingCode == STL_CS454_20)
	{
		csChoiceList.Format(_T("%s\n%s\n%s\n%s\n"),
...
BOOL CAssessmentLoadCombMainGridSTL::GetRowNoByKey(UINT key, ROWCOL& nRow)
{
	ROWCOL nColKey;
...
BOOL CAssessmentLoadCombMainGridSTL::GetInsertRowByKey(UINT key, ROWCOL& nRow)
{
	if (GetRowCount() == 1)
	{
		nRow = 1;
...
/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGridSTL Overridable Functions
void CAssessmentLoadCombMainGridSTL::OnChangedSelectionPublic(long *keys, int nSize)
{
	if (nSize <= 0)
	{
...
long CAssessmentLoadCombMainGridSTL::GetSelectedKeys(CArray<long, long> &caKey)
{
...
BOOL CAssessmentLoadCombMainGridSTL::ValidateField(CString value, int nColID)
{
	if (nColID >= GetColCount())
	{
		//SetWarningText(_LS(IDS_WG_CMD__ADDD___CAssessmentLoadCombMainGridSTL__ValidateFie));
...
BOOL CAssessmentLoadCombMainGridSTL::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
CString CAssessmentLoadCombMainGridSTL::GetDefValue(int nIndex)
{
	if (nIndex == 0) return _T("");
	return m_aCurDefVal[nIndex + 1];
}

CString* CAssessmentLoadCombMainGridSTL::GetDefValueArray()
{
	return m_aCurDefVal;
}

void CAssessmentLoadCombMainGridSTL::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0);

}

void CAssessmentLoadCombMainGridSTL::GetAllSelectedLcom(CArray<UINT, UINT>& aSelKey)
{
...
BOOL CAssessmentLoadCombMainGridSTL::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
...
BOOL CAssessmentLoadCombMainGridSTL::OnCanceledModify()
{
	ROWCOL ncRow, ncCol;
...
CString CAssessmentLoadCombMainGridSTL::GetKeyForRow(ROWCOL ncRow, ROWCOL nkCol)
{
	CString sKey;
...
BOOL CAssessmentLoadCombMainGridSTL::CheckCurrentRecordChanged()
{
	ROWCOL ncRow, nkCol;
	CString sKey;
...
BOOL CAssessmentLoadCombMainGridSTL::IsActiveCol(ROWCOL& nCol)
{
	ROWCOL nRow, nACol;
...
// 이동되는 것을 금지한다.
// 주1 참조
BOOL CAssessmentLoadCombMainGridSTL::OnSelDragColsDrop(ROWCOL nFirstCol, ROWCOL nLastCol, ROWCOL nDestCol)
{
...
void CAssessmentLoadCombMainGridSTL::SetHeaderTitle(T_UNIT_SYSTEM &UnitSystem, BOOL bInit)
{
	SetHeaderTitle2((int*)m_aUnit.GetData(), bInit, TRUE);
}

BOOL CAssessmentLoadCombMainGridSTL::OnPasteFromClipboard(const CGXRange &range)
{
...
void CAssessmentLoadCombMainGridSTL::OnLButtonDblClk(UINT nFlags, CPoint point)
{
...
void CAssessmentLoadCombMainGridSTL::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
...
void CAssessmentLoadCombMainGridSTL::SetModeEdit()
{
...
int CAssessmentLoadCombMainGridSTL::GetLimState(CString &value)
{
...
CString CAssessmentLoadCombMainGridSTL::GetLimStateStr(int nVal)
{
	if (nVal == 0)
	{
...
int CAssessmentLoadCombMainGridSTL::GetCombType(CString &value)
{
...
CString CAssessmentLoadCombMainGridSTL::GetCombTypeStr(int nVal)
{
	if (nVal == 0)
	{
...
void CAssessmentLoadCombMainGridSTL::GetCurSelKey(CArray<T_ALCS_K, T_ALCS_K>& aAlcsK)
{
	aAlcsK.RemoveAll();
	ROWCOL ncRow, ncCol;
...
BOOL CAssessmentLoadCombMainGridSTL::DeleteRecord(ROWCOL nRow)
{
...
BOOL CAssessmentLoadCombMainGridSTL::InsertRecordByKey(const T_ALCS_K &key, const T_ALCS_D &data)
{
	ROWCOL nRow;
...
BOOL CAssessmentLoadCombMainGridSTL::DeleteRecordByKey(const T_ALCS_K &key, const T_ALCS_D &data)
{
	ROWCOL nRow;
...
BOOL CAssessmentLoadCombMainGridSTL::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, T_ALCS_K &key, T_ALCS_D &data)
{
...
BOOL CAssessmentLoadCombMainGridSTL::AddToDB(ROWCOL nRow)
{
...
BOOL CAssessmentLoadCombMainGridSTL::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
	ASSERT(0);
...
BOOL CAssessmentLoadCombMainGridSTL::DeleteFromDB(ROWCOL nRow)
{
	T_ALCS_K key;
	T_ALCS_D data;
	data.Initialize();
...
BOOL CAssessmentLoadCombMainGridSTL::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CAssessmentLoadCombMainGridSTL::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
	nCol = 1;
...
BOOL CAssessmentLoadCombMainGridSTL::IsDataChange(T_ALCS_K key, T_ALCS_D& dataNew)
{
	T_ALCS_D dataOld;
...
BOOL CAssessmentLoadCombMainGridSTL::ModifyToDB(CStringArray& raOldKey,
...
BOOL CAssessmentLoadCombMainGridSTL::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols,
...
void CAssessmentLoadCombMainGridSTL::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	T_ALCS_K* pKey;
	T_ALCS_D* pData;
...
long CAssessmentLoadCombMainGridSTL::GetCountData()
{
	CDBDoc* pDoc = m_pDoc;
...
void CAssessmentLoadCombMainGridSTL::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	T_ALCS_K Key;
	T_ALCS_D Data;
	Data.Initialize();
...
void CAssessmentLoadCombMainGridSTL::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	T_ALCS_K Key;
	T_ALCS_D Data;
...
/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGridSTL Implementation
BOOL CAssessmentLoadCombMainGridSTL::GetValue(ROWCOL nRow, T_ALCS_K &key, T_ALCS_D &data)
{
	ROWCOL nCol;
	CString str;
	data.Initialize();
	...
	((CAssessmentLoadCombSubGridSTL*)m_pSubGrid)->GetData(data);
	m_pParent->GetData(data);

	return TRUE;
}

BOOL CAssessmentLoadCombMainGridSTL::SetValue(ROWCOL nRow, const T_ALCS_K &key, const T_ALCS_D &data)
{
	//
	CRowColArray awCols;
...
BOOL CAssessmentLoadCombMainGridSTL::SetValue(CGXData* pData, ROWCOL nRow, const T_ALCS_K &key, const T_ALCS_D &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CAssessmentLoadCombMainGridSTL::ConvStrToData(int i, CString& value, T_ALCS_D& data)
{
...
void CAssessmentLoadCombMainGridSTL::ConvDataToStr(int i, const T_ALCS_D& data, CString& value)
{
...
void CAssessmentLoadCombMainGridSTL::OnChangeCurrentRecord(ROWCOL nRow)
{
	ROWCOL nkCol;
	GetColumnNoFor(GetKeyColID(), nkCol);
...
BOOL CAssessmentLoadCombMainGridSTL::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
	if (IsEdit())
	{
...
CString CAssessmentLoadCombMainGridSTL::GetNewKey()
{
	int i;
