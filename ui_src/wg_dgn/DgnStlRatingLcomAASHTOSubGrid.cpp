// DgnStlRatingLcomAASHTOSubGrid.cpp: implementation of the CDgnStlRatingLcomAASHTOSubGrid class.
//
//////////////////////////////////////////////////////////////////////
...
CDgnStlRatingLcomAASHTOSubGrid::CDgnStlRatingLcomAASHTOSubGrid()
{
	//m_pMainGrid = 0;
...
BEGIN_MESSAGE_MAP(CDgnStlRatingLcomAASHTOSubGrid, CTBBrowserWnd)
	//{{AFX_MSG_MAP(CDgnStlRatingLcomAASHTOSubGrid)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()


/////////////////////////////////////////////////////////////////////////////
// CDgnStlRatingLcomAASHTOSubGrid message handlers
void CDgnStlRatingLcomAASHTOSubGrid::Initialize(CDBDoc* pDoc)
{
...
// Column별 헤더 이름을 설정한다.
void CDgnStlRatingLcomAASHTOSubGrid::SetHeaderTitle()
{
	CString title;
	int i;
	ROWCOL nCol;
...
void CDgnStlRatingLcomAASHTOSubGrid::SetLoadCaseColumnStyle(ROWCOL nCol)
{
	...
	// 
	int nStageNum = m_pDoc->m_pAttrCtrl->GetCountStag();
	BOOL bExistPlcb = m_pDoc->m_pAttrCtrl->ExistPlcb();

	BOOL bEnable = FALSE;
	if(m_nMainRow==1) 
	{
		if(nStageNum == 0 && bExistPlcb) bEnable = TRUE;
		else                            
		{
			bEnable = FALSE;
			szLoadCaseList.Empty();
		}
	}
	else bEnable = TRUE;
...
BOOL CDgnStlRatingLcomAASHTOSubGrid::IsMainAppendRow()
{
	return m_sKey == _LS(IDS_WG_CMD__ADDD__Append_Row);
}

BOOL CDgnStlRatingLcomAASHTOSubGrid::IsMainNewRow()
{
	return m_sKey == _LS(IDS_WG_CMD__ADDD__New_Row);
}

void CDgnStlRatingLcomAASHTOSubGrid::SetData(T_RTLD_D &Data)
{
	m_Data = Data;

	//////////////////////////////////////////////////////////////////////////
	CArray<T_LCOM_BASE, T_LCOM_BASE&> Combination; Combination.RemoveAll();
	...
	for (int i = 0; i < m_Data.Combination.GetSize(); i++)
	{
		...
		Combination.Add(m_Data.Combination[i]);
	}
	//////////////////////////////////////////////////////////////////////////
	...
	if (IsMainAppendRow() || IsMainNewRow())
	{
		...
		DeleteAllRecords();
		SetRecordCount(Combination.GetSize()+1);
		return;
	}
	
	BOOL bOldLock = LockUpdate(TRUE);
	DeleteAllRecords();
	SetRecordCount(Combination.GetSize());
	...
	{
		nRangeStart = D_STLD_MAXNUM*Combination[i].AnalType;
		sValue.Format(_T("%d"), nRangeStart+Combination[i].LoadCaseKey);
...
/*
void CDgnStlRatingLcomAASHTOSubGrid::SetData(CString sKey)
{
	m_sKey = sKey;
...
BOOL CDgnStlRatingLcomAASHTOSubGrid::OnAddNewRecord(ROWCOL nRow)
{
...
BOOL CDgnStlRatingLcomAASHTOSubGrid::OnModifyRecord(ROWCOL nRow)
{
...
BOOL CDgnStlRatingLcomAASHTOSubGrid::OnDeleteRecord(ROWCOL nRow)
{
...
BOOL CDgnStlRatingLcomAASHTOSubGrid::OnDeleteRecords(CRowColArray &awRows)
{
...
BOOL CDgnStlRatingLcomAASHTOSubGrid::OnCanceledAddNew()
{
	BOOL bRetVal = CTBBrowserWnd::OnCanceledAddNew();
	return bRetVal;
}

BOOL CDgnStlRatingLcomAASHTOSubGrid::OnCanceledModify()
{
	BOOL bRetVal = CTBBrowserWnd::OnCanceledModify();
	return bRetVal;
}

BOOL CDgnStlRatingLcomAASHTOSubGrid::OnInsertEmptyRecord(ROWCOL nRow)
{
...
void CDgnStlRatingLcomAASHTOSubGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
/*
void CDgnStlRatingLcomAASHTOSubGrid::SetRecordModified()
{
...
void CDgnStlRatingLcomAASHTOSubGrid::OnChangeStld()
{
	// 지워진 하중들은 제거한다.
	T_STLD_D stldData;
	T_SGLD_D sgldData;
...
void CDgnStlRatingLcomAASHTOSubGrid::GetCombItem(T_RTLD_D &data)
{
...
BOOL CDgnStlRatingLcomAASHTOSubGrid::OnValidateField(ROWCOL nRow, ROWCOL nCol)
{
	/*
...
void CDgnStlRatingLcomAASHTOSubGrid::EnableDisableGrid(int nMainRow)
{
	m_nMainRow = nMainRow;
