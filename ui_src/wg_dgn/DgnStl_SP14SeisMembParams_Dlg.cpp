...
CDgnStl_SP14SeisMembParams_Dlg::CDgnStl_SP14SeisMembParams_Dlg(CDBDoc* new_doc, CWnd* parent)
	: CMenuBarChildDlg(IDD, parent), doc{ new_doc } {
	ASSERT(doc);
}

void CDgnStl_SP14SeisMembParams_Dlg::DoDataExchange(CDataExchange* dex) {
	ASSERT(dex);
	CMenuBarChildDlg::DoDataExchange(dex);
	DDX_Radio(dex, IDC_DGN_STL_SP14SeisMembParams_AR_RDO, option);
	DDX_Text(dex, IDC_DGN_STL_SP14SeisMembParams_m_tr_1_EDT, data.m_tr_1_strength);
	DDX_Text(dex, IDC_DGN_STL_SP14SeisMembParams_m_tr_2_EDT, data.m_tr_2_lesser_flexibility);
	DDX_Text(dex, IDC_DGN_STL_SP14SeisMembParams_m_tr_3_EDT, data.m_tr_3_greater_flexibility);
	DDX_Text(dex, IDC_DGN_STL_SP14SeisMembParams_m_tr_4_EDT, data.m_tr_4_stability);
}

BEGIN_MESSAGE_MAP(CDgnStl_SP14SeisMembParams_Dlg, CDialog)
	ON_BN_CLICKED(IDC_DGN_CLOSE, OnDgnClose)
	ON_BN_CLICKED(IDC_DGN_EXECUTE, OnDgnExecute)
END_MESSAGE_MAP()

void CDgnStl_SP14SeisMembParams_Dlg::OnDgnClose() {
	CTreeMenuBarBase::SetModeST(__TMMODE_TREEMENU__);
}

void CDgnStl_SP14SeisMembParams_Dlg::OnDgnExecute() {
	...
	if (selected_keys.GetSize() > 0) {
		...
		if (option == 0) { // add or replace
			if (steel_keys.GetSize() > 0) {
				doc->m_pDataCtrl->Add_SP14SeisMembParams(steel_keys, data);
			} else {
				AfxMessageBox(_LSX(No selected steel elements!));
			}
		} else if (option == 1) { // delete
			doc->m_pDataCtrl->Del_SP14SeisMembParams(steel_keys);
		} else {
			ASSERT(FALSE);
		}
	} else {
