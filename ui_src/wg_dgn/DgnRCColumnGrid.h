#if !defined(__DGNRCCOLUMNGRID_H__)
#define __DGNRCCOLUMNGRID_H__
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCColumnGrid window
class CDgnRCColumnGrid : public CTBCommon
{
// Construction
public:
	CDgnRCColumnGrid();
...
// Operations
public:
	void Initialize(CRCColumnParameter* pParent);

	BOOL DeleteRecordByKey(T_ELEM_K &key, T_RCCO_D &data);
	BOOL InsertRecordByKey(T_ELEM_K &key, T_RCCO_D &data);
...
// Overrides
	...
	//{{AFX_VIRTUAL(CDgnRCColumnGrid)
	//}}AFX_VIRTUAL
public:
	virtual int  GetKeyColID() { return 0; }
	virtual void OnChangedSelectionPublic(long *keys, int nSize);

	virtual BOOL ValidateAndMakeRecord(CStringArray &value, CRowColArray &aCols, T_ELEM_K &key, T_RCCO_D &data);
	...
	// 외부에서 현재 레코드의 키 참조
	BOOL GetCurrentRcco(T_ELEM_K& nKey);
	// 외부에서 현재 레코드로 Current Cell 이동
	BOOL SetCurrentRcco(T_ELEM_K key, T_RCCO_D& data);
...
// Implementation
public:
	virtual ~CDgnRCColumnGrid();
	BOOL GetValue(ROWCOL nRow, T_ELEM_K &key, T_RCCO_D &data);
	BOOL SetValue(ROWCOL nRow, T_ELEM_K key, T_RCCO_D &data);
	BOOL SetValue(CGXData* pData, ROWCOL nRow, T_ELEM_K key, T_RCCO_D &data, CRowColArray &awCols);
	void ConvStrToData(int i, CString& value, T_RCCO_D& data);
	void ConvDataToStr(int i, T_ELEM_K key, T_RCCO_D& data, CString& value);

	void GetAllSelectedStld(CArray<T_ELEM_K, T_ELEM_K>& aStldKey);
	void MakeSearchKey(T_ELEM_K &key, T_RCCO_D &data, CStringArray &aKey);
...
private:
	...
	CRCColumnParameter* m_pParent;

	CString GetStringLoadCase(T_RCCG_D& data);
};
