// DgnRCPlateColDesignDlg.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCPlateColDesignDlg dialog
CDgnRCPlateColDesignDlg::CDgnRCPlateColDesignDlg(CWnd* pParent /*=NULL*/)
	: CMenuBarChildDlg(CDgnRCPlateColDesignDlg::IDD, pParent)
{
	m_pDoc = CDBDoc::GetDocPoint();
	m_Data.Initialize();
}

CDgnRCPlateColDesignDlg::~CDgnRCPlateColDesignDlg()
{
}

void CDgnRCPlateColDesignDlg::DoDataExchange(CDataExchange* pDX)
{
	CMenuBarChildDlg::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnRCPlateColDesignDlg)		  
...
BEGIN_MESSAGE_MAP(CDgnRCPlateColDesignDlg, CMenuBarChildDlg)
	//{{AFX_MSG_MAP(CDgnRCPlateColDesignDlg) 	          
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCPlateColDesignDlg message handlers
BOOL CDgnRCPlateColDesignDlg::OnInitDialog() 
{
...
void CDgnRCPlateColDesignDlg::SetCtrl()
{
...
void CDgnRCPlateColDesignDlg::InitCombo()
{
...
void CDgnRCPlateColDesignDlg::InitSbdoCombo(BOOL bKeepSel)
{
...
void CDgnRCPlateColDesignDlg::Data2Dlg()
{	
...
BOOL CDgnRCPlateColDesignDlg::Dlg2Data()
{
	UpdateData(TRUE);
...
void CDgnRCPlateColDesignDlg::ControlsEnableDisable()
{
	UpdateData(TRUE);
...
void CDgnRCPlateColDesignDlg::SetListCtrlHeader()
{
...
void CDgnRCPlateColDesignDlg::MakeItemEx()
{
	m_List.DeleteAllItems();

	int nItemCount = m_pDoc->m_pAttrCtrl2->GetCountPcdd();
	...
	int nCount = 0;
	pos = m_pDoc->m_pAttrCtrl2->GetStartPcdd();
	while(pos !=NULL)
	{
		m_pDoc->m_pAttrCtrl2->GetNextPcdd(pos, Key, Data);		  
		KeyBuf[nCount++] = Key;
	}
	...
	for(nCount = 0 ; nCount < nItemCount ; nCount++)
	{
		...
		m_pDoc->m_pAttrCtrl2->GetPcdd(Key, Data);
...
void CDgnRCPlateColDesignDlg::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	...
	case D_UPDATE_BUFFER_AFTER:	 
		...
		break;
	case D_UPDATE_UNIT:	
		m_pDoc->m_pUnitCtrl->ConvertUnitPcddPrevious(m_Data);
		break;
...
void CDgnRCPlateColDesignDlg::UpdateBuffer()
{
...
void CDgnRCPlateColDesignDlg::OnItemchanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	T_PCDD_D Data; Data.Initialize();
	if(!m_pDoc->m_pAttrCtrl2->GetPcdd(Key, Data)) return;
...
void CDgnRCPlateColDesignDlg::OnMod() 
{
	UpdateData(TRUE);	
	...
	T_PCDD_K PcddK = aSelKey[0];

	m_pDoc->m_pDataCtrl->ModifyPcdd(PcddK, PcddK, m_Data);
...
void CDgnRCPlateColDesignDlg::OnDel() 
{
	UpdateData(TRUE);	
	...
	m_pDoc->m_pDataCtrl->DelPcdd(PcddK);

	if(m_pDoc->m_pViewCtrl->SelectedNodeCount()) m_pDoc->m_pViewCtrl->UnselectAll(NULL);	
	if(m_pDoc->m_pViewCtrl->SelectedElemCount()) m_pDoc->m_pViewCtrl->UnselectAll(NULL);
}

void CDgnRCPlateColDesignDlg::OnAdd() 
{
	UpdateData(TRUE);	

	if(!Dlg2Data()) return;

	m_pDoc->m_pDataCtrl->AddPcdd(m_Data);

	if(m_pDoc->m_pViewCtrl->SelectedNodeCount()) m_pDoc->m_pViewCtrl->UnselectAll(NULL);	
	if(m_pDoc->m_pViewCtrl->SelectedElemCount()) m_pDoc->m_pViewCtrl->UnselectAll(NULL);
}

void CDgnRCPlateColDesignDlg::OnClose() 
{
	CTreeMenuBarBase::SetModeST(__TMMODE_TREEMENU__);
}

void CDgnRCPlateColDesignDlg::OnSbdoBtn()
{
...
void CDgnRCPlateColDesignDlg::OnRebarBtn()
{
	CDgnRCPlateColRebarDlg dlg;
	dlg.DoModal();
}

void CDgnRCPlateColDesignDlg::OnDgnRadioElemAvg() 
{
	ControlsEnableDisable();
}