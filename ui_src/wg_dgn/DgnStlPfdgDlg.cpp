...
IMPLEMENT_DYNAMIC(CDgnStlPfdgDlg, CDlgChild)

CDgnStlPfdgDlg::CDgnStlPfdgDlg(CWnd* pParent /*=NULL*/)
	: CDlgChild(CDgnStlPfdgDlg::IDD, pParent)
{
}

CDgnStlPfdgDlg::~CDgnStlPfdgDlg()
{
}

void CDgnStlPfdgDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDgnStlPfdgDlg, CDlgChild)
	ON_CBN_SELCHANGE(IDC_DGN_PFDN_LOAD_LEVEL,     OnSelectPfdl)
END_MESSAGE_MAP()


// CDgnStlPfdgDlg 消息处理程序


BOOL CDgnStlPfdgDlg::OnInitDialog()
{
    ...
    m_nDucLevel.AddString(_LS(IDS_TB_PFDL_4));
    m_nDucLevel.AddString(_LS(IDS_TB_PFDL_5));
...
BOOL CDgnStlPfdgDlg::ExistKey(const CArray<T_LCOM_BASE>& aSeis,T_LCOM_BASE nKey)
{
...
BOOL CDgnStlPfdgDlg::GetPfdnData(T_PFDN_D& PfdnD)
{
...
void CDgnStlPfdgDlg::OnSelectPfdl()
{
	UpdateData(TRUE);
	int nPfdl[] = {4,4,4,3,2,1,0};
	int nIndex = m_nPerforLevel.GetCurSel();
	m_nDucLevel.SetCurSel(nPfdl[nIndex]);
	double dMinPfmc[] = {1.1,0.9,0.7,0.55,0.45,0.35,0.28};
	m_dMinCoe = dMinPfmc[nIndex];
	UpdateData(FALSE);
}