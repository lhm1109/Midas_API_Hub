#ifndef __DGN_IDEA_RS_EXPORT_WRITE_H__
#define __DGN_IDEA_RS_EXPORT_WRITE_H__
...
class __MY_EXT_CLASS__ CDgnIDEAArchive : public CArchive
{
public:
	// Flag values
	CDgnIDEAArchive(LPCTSTR lpszTxtFile, CFile* pFile, UINT nMode, int nBufSize = 4096, void* lpBuf = NULL);
	virtual ~CDgnIDEAArchive();
	
	// insertion operations
	CDgnIDEAArchive& operator<<(BYTE by);
	CDgnIDEAArchive& operator<<(WORD w);
	CDgnIDEAArchive& operator<<(LONG l);
	CDgnIDEAArchive& operator<<(DWORD dw);
	CDgnIDEAArchive& operator<<(float f);
	CDgnIDEAArchive& operator<<(double d);
	
	CDgnIDEAArchive& operator<<(int i);
	CDgnIDEAArchive& operator<<(short w);
	CDgnIDEAArchive& operator<<(TCHAR ch);
	CDgnIDEAArchive& operator<<(unsigned u);
	CDgnIDEAArchive& operator<<(const CString& string);
...
class __MY_EXT_CLASS__ CDgnClassIREW
{
	// Construction
public:
	CDgnClassIREW();
	...
	void Initialize();
	BOOL ExportIDEAData(CDgnIDEAArchive& ar, KEYArray& aElement, CString& szMCTPath);
	BOOL ExportIDEADataTest(CDgnIDEAArchive& ar, CString& szMCTPath, CDgnClassIRED& rData);
...
protected:
	...
	// Write Data
	BOOL WriteBASC(CDgnIDEAArchive&  ar, CString& szMCTPath);
	BOOL WriteUNIT(CDgnIDEAArchive&  ar, S_UNIT_D&  UnitD);
	BOOL WriteNODE(CDgnIDEAArchive&  ar, UINT nKey, S_NODE_D& NodeD);
	BOOL WriteELEM(CDgnIDEAArchive&  ar, UINT nKey, S_ELEM_D& ElemD);
	BOOL WriteMEMB(CDgnIDEAArchive&  ar, UINT nKey, S_MEMB_D& MembD);
	BOOL WriteMtPr(CDgnIDEAArchive&  ar, S_MATL_PROP& MtPrD);
	BOOL WriteMTST(CDgnIDEAArchive&  ar, UINT nKey, S_MATL_STEEL_D&    MtstD);
	BOOL WriteMTCC(CDgnIDEAArchive&  ar, UINT nKey, S_MATL_CONCRETE_D& MtccD);
	BOOL WriteMTRB(CDgnIDEAArchive&  ar, UINT nKey, S_MATL_REBAR_INFO& MtrbD);
	BOOL WriteMTAD(CDgnIDEAArchive&  ar, UINT nKey, S_MATL_ADIF_D&     MtadD);

	BOOL WriteSTSF(CDgnIDEAArchive&  ar, S_SECT_STIFFNESS& StsfD);
	BOOL WriteSTPG(CDgnIDEAArchive&  ar, S_GSEC_POLYGON& StpgD);
	BOOL WriteSECT(CDgnIDEAArchive&  ar, UINT nKey, S_SECT_D& SectD);
	BOOL WriteELST(CDgnIDEAArchive&  ar, UINT nEKey, UINT nSKey, S_ELST_D& ElstD);
	BOOL WriteCSET(CDgnIDEAArchive&  ar, UINT nEKey, UINT nSKey, UINT nPKey, S_CSET_D& CsetD);
	BOOL WriteSTCS(CDgnIDEAArchive&  ar, UINT nKey, S_PROP_D& StcsD);
	BOOL WriteGSPO(CDgnIDEAArchive&  ar, S_GSPO_D& GspoD);
	BOOL WriteGSPI(CDgnIDEAArchive&  ar, S_GSPI_D& GspiD);
	BOOL WriteCSPT(CDgnIDEAArchive&  ar, UINT nSkey, UINT nPkey, UINT nIJ, S_GSEC_PART& CsptD, BOOL bPrintKey);
	BOOL WriteLINE(CDgnIDEAArchive&  ar, const CArray<S_GSEC_LINE, S_GSEC_LINE&>& aLine);
	BOOL WriteLNLP(CDgnIDEAArchive&  ar, const CArray<int, int>& aLineLoop);

	BOOL WriteCONS(CDgnIDEAArchive&  ar, UINT nKey, S_CONS_D& ConsD);
	BOOL WriteSTLD(CDgnIDEAArchive&  ar, UINT nKey, S_STLD_D& StldD);
	BOOL WriteLCBS(CDgnIDEAArchive&  ar, S_LCOM_BASE& LcomD);
	BOOL WriteLCOM(CDgnIDEAArchive&  ar, UINT nType, UINT nKey, S_LCOM_D& LcomD);

	BOOL WriteLENG(CDgnIDEAArchive&  ar, UINT nKey, S_LENG_D& LengD);
	BOOL WriteKFAC(CDgnIDEAArchive&  ar, UINT nKey, S_KFAC_D& KfacD);
	BOOL WriteLTSR(CDgnIDEAArchive&  ar, UINT nKey, S_LTSR_D& LtsrD);
	BOOL WriteCMFT(CDgnIDEAArchive&  ar, UINT nKey, S_CMFT_D& CmftD);
	BOOL WriteFMAG(CDgnIDEAArchive&  ar, UINT nKey, S_FMAG_D& FmagD);

	BOOL WriteDCON(CDgnIDEAArchive&  ar, S_DCON_D& DConD);
	BOOL WriteRDBM(CDgnIDEAArchive&  ar, UINT nKey, S_RDGN_BEAM& RdbmD);
	BOOL WriteRDCN(CDgnIDEAArchive&  ar, UINT nKey, S_RDGN_COLM& RbcnD);
	BOOL WriteRCBM(CDgnIDEAArchive&  ar, UINT nKey, S_RCHK_BEAM& RcbmD);
	BOOL WriteRCCN(CDgnIDEAArchive&  ar, UINT nKey, S_RCHK_COLM& RccnD);

	BOOL WriteREBT(CDgnIDEAArchive&  ar, UINT nKey, S_REBT_D&     RebtD);
	BOOL WriteRPMS(CDgnIDEAArchive&  ar, S_RPSC_MBAR_SSM& RpmsD);
	BOOL WriteRPSS(CDgnIDEAArchive&  ar, S_RPSC_SBAR_SSM& RpssD);
	BOOL WriteRPSC(CDgnIDEAArchive&  ar, UINT nKey, S_RPSC_SSM_D& RpscD);
	BOOL WriteTDIF(CDgnIDEAArchive&  ar, UINT nKey, UINT nIJ, S_TDIF_D&     TdifD);  

	// for 2D Element
	BOOL WriteTHIK(CDgnIDEAArchive&  ar, UINT nKey, S_THIK_D& iThik);
	BOOL WriteSTRP(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_STRP_DL& iStrp);
	BOOL WriteSTRPCS(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_STRP_CSDL& iStrp);

	// for construction Stage
	BOOL WriteCSLC(CDgnIDEAArchive&  ar, UINT nKey, S_STLD_D& iCslc);
	BOOL WriteCLAT(CDgnIDEAArchive&  ar, UINT nKey, S_STLD_D& iClat);  
	BOOL WriteSTAG(CDgnIDEAArchive&  ar, UINT nKey, S_STAG_D& iStag);
	BOOL WriteCSCS(CDgnIDEAArchive&  ar, UINT nKey, S_CSCS_D& iCscs);
	BOOL WriteGRUP(CDgnIDEAArchive&  ar, UINT nKey, S_GRUP_D& iGrup);
	BOOL WriteBNGR(CDgnIDEAArchive&  ar, UINT nKey, S_BNGR_D& iBngr);
	BOOL WriteLDGR(CDgnIDEAArchive&  ar, UINT nKey, S_LDGR_D& iLdgr);
	BOOL WriteTDLS(CDgnIDEAArchive&  ar, UINT nKey, UINT nStag, UINT nStep, S_TDLS_D& TdlsD);

	BOOL WriteREAC(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_REAC_D& ReacD);
	BOOL WriteDISP(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_DISP_D& DispD);
	BOOL WriteTFOR(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_TFOR_D& TforD);
	BOOL WriteBFOR(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_BFOR_D& BforD);
	BOOL WriteRECC(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_RECC_D& ReccD);
	BOOL WriteDSPC(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_DSPC_D& DspcD);
	BOOL WriteBCFO(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_BCFO_B& BcfoD);

	BOOL WriteDISPCS(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_DISP_CSD& DispD);
	BOOL WriteTFORCS(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_TFOR_CSD& TforD);
	BOOL WriteBFORCS(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_BFOR_CSD& BforD);

	BOOL WriteBFORPT(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_BFOR_PT& BforD);
	BOOL WriteBFORPTCC(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_BFOR_PTCC& BforD);
	BOOL WriteBFORPTCS(CDgnIDEAArchive&  ar, UINT nEKey, UINT nLKey, UINT nType, S_BFOR_PTCS& BforD);
