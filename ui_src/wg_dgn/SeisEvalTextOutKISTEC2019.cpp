...
CSeisEvalTextOutKISTEC2019::CSeisEvalTextOutKISTEC2019(void)
{
}
...
void CSeisEvalTextOutKISTEC2019::PrintEval1stConBeamLcom(T_ELEM_K ElemK, const T_EVAL1ST_BEAM_LCOM& EvalLcom)
{
...
void CSeisEvalTextOutKISTEC2019::PrintEval1stConColmLcom(T_ELEM_K ElemK, const T_EVAL1ST_COLM_LCOM& EvalLcom,
...
void CSeisEvalTextOutKISTEC2019::PrintEval1stConWallLcom(T_ELEM_K ElemK, const T_EVAL1ST_WALL_LCOM& EvalLcom,
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConBeam(T_ELEM_K ElemK, const T_UPHG_CONBEAM& Uphg)
{
    WriteOutline1(_LS(IDS_DGN_SE_EVALUATION_RESULT_I_END));
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConColm(T_ELEM_K ElemK, const T_UPHG_CONCOLM& Uphg)
{
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConWall(T_ELEM_K ElemK, const T_UPHG_CONWALL& Uphg)
{
...
void CSeisEvalTextOutKISTEC2019::PrintConWallCtrlAction(const T_CONWALL_INFO& Info, const CString& strComment)
{
    WriteOutline3(_LS(IDS_DGN_SE_CHECK_CONTROLLED_ACTION));
    WriteComment(strComment);

    const double dRatHwLw = Info.dLw < cEvalZero ? 3.0 + cEvalZero : Info.dHw / Info.dLw;
    
    if (dRatHwLw > 3.0 )
    {
        WriteParaFmt(_T("Hw / Lw = %12.4f > 3.0 -> %s"), dRatHwLw, _LS(IDS_DGN_SE_CONTROLLED_FLEXURE));
        return;
    }
    
    if ( dRatHwLw < 1.5 )
    {
        WriteParaFmt(_T("Hw / Lw = %12.4f < 1.5 -> %s"), dRatHwLw, _LS(IDS_DGN_SE_CONTROLLED_SHEAR));
        return;
    }
...
void CSeisEvalTextOutKISTEC2019::PrintEval1stConBeamLcomPos(T_ELEM_K ElemK, T_LCOM_K LcomK, const T_EVAL1ST_BEAM_POS& EvalPos)
{
...
void CSeisEvalTextOutKISTEC2019::PrintEval1stConColmLcomPos(T_ELEM_K ElemK, T_LCOM_K LcomK, const T_EVAL1ST_COLM_POS& EvalPos,
                                                            const T_PMCV_TABLE& Pmcvy, const T_PMCV_TABLE& Pmcvz)
{
    CSeisEvalDataTool Tool;
...
void CSeisEvalTextOutKISTEC2019::PrintEval1stConWallLcomPos(T_ELEM_K ElemK, T_LCOM_K LcomK, const T_EVAL1ST_WALL_POS& EvalPos,
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConBeamPos(T_ELEM_K ElemK, const T_UPHG_CONBEAM_POS& UphgPos)
{    
    CSeisEvalDataTool Tool;
    bool bUseSeisHook = Tool.IsUseSeisHook(ElemK);
    const T_CONBEAM_INFO& Info = UphgPos.Info;
    const CString& strComment = _T("KISTEC 2019, Table 5.4.6");
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConColmAxial(T_ELEM_K ElemK, const T_UPHG_PARAM& ParamComp, const T_UPHG_PARAM& ParamTens)
{
    const CString& strComment = _T("KISTEC 2019, Table 6.1.6");

    WriteOutline2(_LS(IDS_DGN_SE_UPDATE_HINGE_CONDITION));
    WriteOutline3(_LS(IDS_DGN_SE_1ST_YIELDING_STRAIN));
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConColmDir(T_ELEM_K ElemK, EN_DIR enDir, const T_UPHG_CONCOLM_DIR& UphgDir)
{
    const T_CONCOLM_INFO& Info = UphgDir.Info;
    const CString& strComment = _T("KISTEC 2019, Table 5.4.7");
    ...
    PrintConColmFailureMode(enDir, Info, _T("KISTEC 2019, Table 5.4.1"));
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConWallPosFlex(T_ELEM_K ElemK, const T_UPHG_CONWALL_POS& UphgPos)
{
    const T_CONWALL_INFO& Info = UphgPos.Info;
    const CString& strCommentAction = _T("KISTEC 2019, 5.5.4(1)");
    const CString& strComment = _T("KISTEC 2019, Table 5.5.4, 5.5.5");
...
void CSeisEvalTextOutKISTEC2019::PrintUphgConWallPosSher(T_ELEM_K ElemK, const T_UPHG_CONWALL_POS& UphgPos)
{
    const T_CONWALL_INFO& Info = UphgPos.Info;
    const CString& strCommentAction = _T("KISTEC 2019, 5.5.4(1)");
    const CString& strComment = _T("KISTEC 2019, Table 5.5.4, 5.5.5");
...
void CSeisEvalTextOutKISTEC2019::PrintConBeamMFact(T_ELEM_K ElemK, const T_EVAL1ST_BEAM_POS& Pos)
{
    ...
    else
    {
        ...
        const CString& strComment = _T("KISTEC 2019, Table 5.4.3");
...
void CSeisEvalTextOutKISTEC2019::PrintConColmMFactAxial(T_ELEM_K ElemK, const T_EVAL1ST_COLM_POS& Pos)
{
    ...
    {
        WriteOutline2Fmt(_T("%s (%s)"), _LS(IDS_DGN_SE_CALC_MFACT), _LS(IDS_DGN_SE_AXIAL));
        const CString& strComment = _T("KISTEC 2019, Table 6.1.4");

        PrintMFactCalc(Pos.MfacP.admFac, strComment);
    }
}

void CSeisEvalTextOutKISTEC2019::PrintConColmMFact(T_ELEM_K ElemK, EN_DIR enDir, const T_EVAL1ST_COLM_DIR& EvalDir)
{
    ...
    else
    {
        ...
        const CString& strComment = _T("KISTEC 2019, Table 5.4.4");
...
void CSeisEvalTextOutKISTEC2019::PrintConWallMFact(T_ELEM_K ElemK, const T_EVAL1ST_WALL_POS& Pos)
{
    const CString& strComment = _T("KISTEC 2019, Table 5.5.2, 5.5.3");
...
void CSeisEvalTextOutKISTEC2019::PrintConBeamStrength(const T_CONBEAM_INFO& Info)
{
    ...
    WriteComment(_T("KISTEC 2019, 5.4.3.1(1), (5.4.1)"));
    ...
    WriteComment(_T("KISTEC 2019, 5.4.3.1(2), (5.4.2)~(5.4.4)"));
    PrintConBeamStrengthShear(Info);
    WriteBlank();
}

void CSeisEvalTextOutKISTEC2019::PrintConColmStrength(const T_EVAL1ST_COLM_POS& Pos, const T_PMCV_TABLE& Pmcvy, 
                                                      const T_PMCV_TABLE& Pmcvz, int nShrMethod)
{
    ...
    PrintConColmStrengthShear(nShrMethod, Pos.Dirz, _LSX(y), _T("KISTEC 2019, (5.4.5)"));
    WriteBlank();

    WriteOutline3Fmt(_T("%s (%s)"), _LS(IDS_DGN_SE_SHEAR_STRENGTH), GetLocalDir(EN_DIR_Z));
    PrintConColmStrengthShear(nShrMethod, Pos.Diry, _LSX(z), _T("KISTEC 2019, (5.4.5)"));
    WriteBlank();
}


void CSeisEvalTextOutKISTEC2019::PrintConWallStrength(const T_EVAL1ST_WALL_POS& Pos, const T_PMCV_TABLE& Pmcvy)
{
    WriteOutline2(_LS(IDS_DGN_SE_CALC_STRENGTH));
...
void CSeisEvalTextOutKISTEC2019::PrintConWallForcAction(const T_CONWALL_INFO& Info, const CString& strComment)
{
    const CString& strCommentAction = _T("KISTEC 2019, 5.5.4(1)");
