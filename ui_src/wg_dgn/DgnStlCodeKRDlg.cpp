// DgnStlCodeKRDlg.cpp : implementation file
//
...
CDgnStlCodeKRDlg::CDgnStlCodeKRDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CDgnStlCodeKRDlg::IDD, pParent)
{
	m_EnvelopeData.Initialize();

	//{{AFX_DATA_INIT(CDgnStlCodeKRDlg)	
	//}}AFX_DATA_INIT
	...
	m_aPrintCtrlLst.Add(IDC_DGN_SECT_CHECK4);
	m_aPrintCtrlLst.Add(IDC_DGN_SECT_CHECK5);
	m_aPrintCtrlLst.Add(IDC_DGN_SELECT_ALL_BTN);
	m_aPrintCtrlLst.Add(IDC_DGN_UNSELECT_ALL_BTN);
}

void CDgnStlCodeKRDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnStlCodeKRDlg)
	...
	DDX_Control(pDX, IDC_DGN_SECT_CHECK4, m_chkSectCheck4);
	DDX_Control(pDX, IDC_DGN_SECT_CHECK5, m_chkSectCheck5);
...
BEGIN_MESSAGE_MAP(CDgnStlCodeKRDlg, CInternationalDlg)
#undef CDialog
	//{{AFX_MSG_MAP(CDgnStlCodeKRDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CDgnStlCodeKRDlg message handlers

BOOL CDgnStlCodeKRDlg::OnInitDialog() 
{
...
// 	m_DesignCode.AddString(_T("DDd"));
// 	m_DesignCode.UpdateData(TRUE);
// 	int nCount = m_DesignCode.GetCount();
...
void CDgnStlCodeKRDlg::Initial_Data()
{  
	T_DSTL_D rData;
	rData.Initialize();  
	...
	m_chkSectCheck4.SetCheck(rData.bPrintChk[3]);
	m_chkSectCheck5.SetCheck(rData.bPrintChk[4]);
...
void CDgnStlCodeKRDlg::InitCtrl()
{
	// Design Code Combo Box
	m_DesignCode.ResetContent();
	m_DesignCode.SetItemData(m_DesignCode.AddString(_T("KSCE-LSD15")), 0); // 0 : KSCE-LSD15
	m_DesignCode.SetItemData(m_DesignCode.AddString(_T("KSCE-ASD10")), 1); // 1 : KSCE-ASD10
	m_DesignCode.SetItemData(m_DesignCode.AddString(_T("KSCE-ASD05")), 2); // 2 : KSCE-ASD05
    m_DesignCode.SetItemData(m_DesignCode.AddString(STLCODE_KDS_24_14_30_2019), 3); 
	...
	{
		if(DstlD.DesignCode.CompareNoCase(_T("KSCE-LSD15"))==0){nCurSel = 0;}
		else if(DstlD.DesignCode.CompareNoCase(_T("KSCE-ASD10"))==0){nCurSel = 1;}
		else if(DstlD.DesignCode.CompareNoCase(_T("KSCE-ASD05"))==0){nCurSel = 2;}
        else if ( DstlD.DesignCode.CompareNoCase(STLCODE_KDS_24_14_30_2019)==0 ) { nCurSel = 3; }
		else {nCurSel = 0;}
	}
	else
	{
...
void CDgnStlCodeKRDlg::EnableDisable_Ctrl()
{
...
void CDgnStlCodeKRDlg::Initial_ExistData()
{
...
void CDgnStlCodeKRDlg::OnOK() 
{
	...
	rData.bPrintChk[3] = m_chkSectCheck4.GetCheck();
	rData.bPrintChk[4] = m_chkSectCheck5.GetCheck();	
...
void CDgnStlCodeKRDlg::OnCancel() 
{
	// TODO: Add extra cleanup here
	CDialogMove::OnCancel();
}

void CDgnStlCodeKRDlg::OnChangeDgnCodeCmb() 
{
	EnableDisable_Ctrl();

}

void CDgnStlCodeKRDlg::OnDgnEnvelopeBtn() 
{
...
void CDgnStlCodeKRDlg::OnDgnSelectAllBtn() 
{
	m_chkSectCheck1.SetCheck(TRUE);
	m_chkSectCheck2.SetCheck(TRUE);
	m_chkSectCheck3.SetCheck(TRUE);
	m_chkSectCheck4.SetCheck(TRUE);
	m_chkSectCheck5.SetCheck(TRUE);
}

void CDgnStlCodeKRDlg::OnDgnUnSelectAllBtn() 
{
	m_chkSectCheck1.SetCheck(FALSE);
	m_chkSectCheck2.SetCheck(FALSE);
	m_chkSectCheck3.SetCheck(FALSE);
	m_chkSectCheck4.SetCheck(FALSE);
	m_chkSectCheck5.SetCheck(FALSE);
}

void CDgnStlCodeKRDlg::OnDgnSectionForceRadio() 
{
	if(m_radioSectForce1.GetCheck())
