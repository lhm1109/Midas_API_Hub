...
CCPGFatigueParamTabJDlg_CSA::CCPGFatigueParamTabJDlg_CSA(CWnd* pParent /*=NULL*/)
	: CChildDialog(CCPGFatigueParamTabJDlg_CSA::IDD, pParent)
{
	//{{AFX_DATA_INIT(CCPGFatigueParamTabJDlg_CSA)
...
void CCPGFatigueParamTabJDlg_CSA::DoDataExchange(CDataExchange* pDX)
{
	CChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCPGFatigueParamTabJDlg_CSA)
	...
	DDX_Control(pDX, IDC_DGN_CPG_FATIGUE_Y_EDT,                m_edtDesignLife);
	DDX_Control(pDX, IDC_DGN_FATIGUE_ND_EDT,                   m_edtNdValue);
	DDX_Control(pDX, IDC_DGN_FATIGUE_P_EDT,                    m_edtPValue);
...
BEGIN_MESSAGE_MAP(CCPGFatigueParamTabJDlg_CSA, CChildDialog)
	//{{AFX_MSG_MAP(CCPGFatigueParamTabJDlg_CSA)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_DGN_FATIGUE_AUTO_CHK, &CCPGFatigueParamTabJDlg_CSA::OnBnClickedDgnFatigueAutoChk)
END_MESSAGE_MAP()


/////////////////////////////////////////////////////////////////////////////
// CCPGFatigueParamTabJDlg_CSA message handlers

BOOL CCPGFatigueParamTabJDlg_CSA::OnInitDialog() 
{
...
BOOL CCPGFatigueParamTabJDlg_CSA::DestroyWindow() 
{
	Dlg2Data();
...
void CCPGFatigueParamTabJDlg_CSA::InitCombo()
{
...
void CCPGFatigueParamTabJDlg_CSA::OnWarpingStressTypeRdo() 
{
	ControlsEnableDisable();
}

void CCPGFatigueParamTabJDlg_CSA::Data2Dlg()
{
	...
	m_edtDesignLife.SetEditUnit(m_Data.dDesignLife);
	m_edtNdValue.SetEditUnit(m_Data.dNd);
	m_edtPValue.SetEditUnit(m_Data.dp);
	m_bAutoChk = m_Data.bAutoChk;
	UpdateData(FALSE);
}

BOOL CCPGFatigueParamTabJDlg_CSA::Dlg2Data()
{
	UpdateData(TRUE);
	m_Data.bAutoChk         = m_bAutoChk;
	m_Data.dWeight          = m_edtWeightTruck.GetEditValue();
	m_Data.dDesignLife      = m_edtDesignLife.GetEditValue();	
	m_Data.dNd             = m_edtNdValue.GetEditValue();
	m_Data.dp              = m_edtPValue.GetEditValue();
	m_Data.nGirderType      = m_cmbGirderType.GetItemData(m_cmbGirderType.GetCurSel());   //수정필요
	m_Data.nADTT            = m_cmbDailyTruck.GetItemData(m_cmbDailyTruck.GetCurSel());   //수정필요

	return TRUE;
}

void CCPGFatigueParamTabJDlg_CSA::GetCpfpData()
{
	Dlg2Data();
...
void CCPGFatigueParamTabJDlg_CSA::SetCpfpData()
{
	Data2Dlg();
}

void CCPGFatigueParamTabJDlg_CSA::ControlsEnableDisable()
{
	UpdateData(TRUE);
	...
	GetDlgItem(IDC_DGN_CPG_FATIGUE_GIRDER_TYPE_CMB)->EnableWindow(bEnable);
	...
	GetDlgItem(IDC_DGN_FATIGUE_ND_STC)->EnableWindow(bEnable);
	GetDlgItem(IDC_DGN_FATIGUE_ND_EDT)->EnableWindow(bEnable);
	...
	if(m_bAutoChk == TRUE && m_pParent->m_nOption==0 && m_pParent->m_bBothIJ)
	{
		GetDlgItem(IDC_DGN_CPG_FATIGUE_GIRDER_TYPE_CMB)->EnableWindow(TRUE);
		GetDlgItem(IDC_DGN_FATIGUE_ND_STC)->EnableWindow(FALSE);
		GetDlgItem(IDC_DGN_FATIGUE_ND_EDT)->EnableWindow(FALSE);
	}
	else if(m_bAutoChk == FALSE && m_pParent->m_nOption==0 && m_pParent->m_bBothIJ)
	{
	...
	else if(m_pParent->m_nOption!=0 ||  m_pParent->m_bBothIJ == FALSE)
	{
		GetDlgItem(IDC_DGN_CPG_FATIGUE_GIRDER_TYPE_CMB)->EnableWindow(FALSE);
		GetDlgItem(IDC_DGN_FATIGUE_ND_STC)->EnableWindow(FALSE);
		GetDlgItem(IDC_DGN_FATIGUE_ND_EDT)->EnableWindow(FALSE);
...
CRect CCPGFatigueParamTabJDlg_CSA::GetResizeHeight()
{	
	CRect r;
...
void CCPGFatigueParamTabJDlg_CSA::ResizeWindow()
{
//   m_chkGirderGroup.SetFoldState(FALSE);
//   m_chkDailyGroup.SetFoldState(FALSE);
...
LRESULT CCPGFatigueParamTabJDlg_CSA::OnFoldButtonClicked(WPARAM wParam, LPARAM lParam)
{
	m_pParent->ResizeWindow();
	return 0L;
}

void CCPGFatigueParamTabJDlg_CSA::OnBnClickedDgnFatigueAutoChk()
{
	UpdateData(TRUE);
	if(m_bAutoChk == FALSE)
	{
