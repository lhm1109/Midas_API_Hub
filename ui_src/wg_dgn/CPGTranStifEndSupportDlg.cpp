// CPGTranStifEndSupportDlg.cpp : implementation file
//
...
CCPGTranStifEndSupportDlg::CCPGTranStifEndSupportDlg(CWnd* pParent /*=NULL*/)
	: CDialogMove(CCPGTranStifEndSupportDlg::IDD, pParent)
{
	m_pDoc = CDBDoc::GetDocPoint();	



	//{{AFX_DATA_INIT(CCPGTranStifEndSupportDlg)
	m_iEndpost = 0;
	//}}AFX_DATA_INIT
}

CCPGTranStifEndSupportDlg::~CCPGTranStifEndSupportDlg()
{
	
}

void CCPGTranStifEndSupportDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCPGTranStifEndSupportDlg)
...
BOOL CCPGTranStifEndSupportDlg::OnInitDialog() 
{
	//PROFILING(_T("CCPGTranStifEndSupportDlg::OnInitDialog()"));
...
void CCPGTranStifEndSupportDlg::InitSelectSupportList() 
{
...
void CCPGTranStifEndSupportDlg::SetListCtrlHeader()
{
...
int CCPGTranStifEndSupportDlg::FindInsertionPos(T_CGTE_K nID)
{
	int nCount = m_list.GetItemCount();
	for (int i=0; i < nCount; i++)
	{
        T_CGTE_K key = m_list.GetItemData(i);
		if (nID < key) { return i; }
	}
	return nCount;
}

BOOL CCPGTranStifEndSupportDlg::InsertListItem(T_CGTE_K Key)
{
	//DB에서 Data를 가져온다

	T_CGTE_D Data;

	m_pDoc->m_pAttrCtrl->GetCgte(Key, Data);
...
CString CCPGTranStifEndSupportDlg::DataToStr(int i, T_CGTE_K Key, T_CGTE_D &Data)
{  
	CString str;
...
BOOL CCPGTranStifEndSupportDlg::DeleteListItem(T_CGTE_K key)
{
	LVFINDINFO FindInfo;
...
BOOL CCPGTranStifEndSupportDlg::ModifyListItem(T_CGTE_K key)
{
	T_CGTE_D Data;
...
void CCPGTranStifEndSupportDlg::LoadDlgData()
{
...
BOOL CCPGTranStifEndSupportDlg::SaveDlgFactorPosition()
{
	UpdateData(TRUE);
...
BOOL CCPGTranStifEndSupportDlg::SaveDlgSelectSupport()
{
	UpdateData(TRUE);
...
BEGIN_MESSAGE_MAP(CCPGTranStifEndSupportDlg, CDialogMove)
	//{{AFX_MSG_MAP(CCPGTranStifEndSupportDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CCPGTranStifEndSupportDlg message handlers

void CCPGTranStifEndSupportDlg::OnDgnAddBtn() 
{
	...
	{
		T_CGTE_K key = m_arSelectNodeKey.GetAt(i);
		
		if(!m_pDoc->m_pAttrCtrl->ExistCgte(key))
		{
			if(m_pDoc->m_pDataCtrl->AddCgte(key, m_MainData))
				InsertListItem(key);
		}		
	}	
}

void CCPGTranStifEndSupportDlg::OnDgnModifyBtn() 
{
	// TODO: Add your control notification handler code here
	if(!SaveDlgSelectSupport())
		return;
	...
	for(int i=0 ; i<m_arSelectNodeKey.GetSize() ; i++)
	{
		...
		if(m_pDoc->m_pAttrCtrl->ExistCgte(key))
		{
			if(m_pDoc->m_pDataCtrl->ModifyCgte(key, key, Data))
				ModifyListItem(key);
		}		
	}	
}

void CCPGTranStifEndSupportDlg::OnDgnDeleteBtn() 
{
	// TODO: Add your control notification handler code here
	if(!SaveDlgSelectSupport())
		return;
	...
	m_pDoc->m_pDataCtrl->DelCgte(m_arSelectNodeKey);
	
	for(int i=0 ; i<size ; i++)
	{
		T_CGTE_K key = m_arSelectNodeKey[0];
		
		DeleteListItem(key);				
	}	
}

void CCPGTranStifEndSupportDlg::GetSelectedItemList(CListCtrl* pList, CArray<int, int> &aSelItem)
{
	int  nItem = -1;
...
void CCPGTranStifEndSupportDlg::OnItemchangedDgnList(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CCPGTranStifEndSupportDlg::OnItemchangedDgnCheckList() 
{
	if(!SaveDlgSelectSupport())
		return;	
...
void CCPGTranStifEndSupportDlg::ChangeItem(int nIndex)
{
	//PROFILING(_T("CCPGTranStifEndSupportDlg::ChangeItem(int nIndex)"));
	...
	m_pDoc->m_pAttrCtrl->GetCgte(nodeKey, m_MainData);
	LoadDlgData();
	OnPostRadio();
}

void CCPGTranStifEndSupportDlg::OnDgnClose() 
{
	// TODO: Add your control notification handler code here
	CDialogMove::OnOK();
}

void CCPGTranStifEndSupportDlg::GetDBCgte() 
{
...
void CCPGTranStifEndSupportDlg::OnPostRadio() 
{
	UpdateData(TRUE);
