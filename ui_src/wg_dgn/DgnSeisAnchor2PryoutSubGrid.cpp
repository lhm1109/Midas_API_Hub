// DgnSeisAnchor2PryoutSubGrid.cpp : implementation file
...
CDgnSeisAnchor2PryoutSubGrid::CDgnSeisAnchor2PryoutSubGrid(CWnd* pParent/* = NULL*/)
{
	m_pParent = (CDgnSeisAnchor2PryoutSub*)pParent;

	m_pDoc = CDBDoc::GetDocPoint();

}

CDgnSeisAnchor2PryoutSubGrid::~CDgnSeisAnchor2PryoutSubGrid()
{
}

/////////////////////////////////////////////////////////////////////////////
// User Defined Finction
void CDgnSeisAnchor2PryoutSubGrid::SetData2Grid(T_ANEV_PRYOUT_D* pPryout, T_ANEV_D& AnevD)
{
	BOOL bOldLock = LockUpdate(TRUE);
	m_AnevD = AnevD;

	int nRow = pPryout->aFailSelLayer.GetSize();
	int nCol = 0;
	...
	EnableDisableGrid(!AnevD.Fail.bUser[1]);

	LockUpdate(bOldLock);
	if (!bOldLock) Redraw();
}

BOOL CDgnSeisAnchor2PryoutSubGrid::GetGrid2Data(T_ANEV_PRYOUT_D* pPryout)
{
	...
	for (int i = 0; i < nRow; i++)
	{
		...
		pPryout->aFailSelLayer.Add(Layer);
	}

	return TRUE;
}

void CDgnSeisAnchor2PryoutSubGrid::EnableDisableGrid(BOOL bEnable)
{
	int nRow = GetRowCount();
	int nCol = GetColCount();
	if (nRow == 0 || nCol == 0) return;
...
void CDgnSeisAnchor2PryoutSubGrid::SetDataToRow(ROWCOL nRow, CStringArray& raValue)
{
...
BOOL CDgnSeisAnchor2PryoutSubGrid::GetRowToData(ROWCOL nRow, CStringArray& raValue)
{
	int nColCount = GetColCount();
	ASSERT(nColCount == raValue.GetSize());
...
CString CDgnSeisAnchor2PryoutSubGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString strNum(_T(""));
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CDgnSeisAnchor2PryoutSubGrid, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CDgnSeisAnchor2PryoutSubGrid)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

///////////////////////////////////////////////////////////////////////////
// CDgnSeisAnchor2PryoutSubGrid message handlers

// 그리드 초기화 
void CDgnSeisAnchor2PryoutSubGrid::Initialize()
{
...
BOOL CDgnSeisAnchor2PryoutSubGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bCurrentRow;
	BOOL bCurrentCell;
	BOOL bEnabled;
...
void CDgnSeisAnchor2PryoutSubGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CDgnSeisAnchor2PryoutSubGrid::OnLButtonClickedRowCol(ROWCOL nRow, ROWCOL nCol, UINT nFlags, CPoint pt)
{
...
BOOL CDgnSeisAnchor2PryoutSubGrid::OnPasteFromClipboard(const CGXRange &range)
{
	CGXGridWnd x;
	CGXRange r;
	CGXRange toRange;
...
BOOL CDgnSeisAnchor2PryoutSubGrid::VerifyPasteRange(CGXRange& from, CGXRange& to)
{
...
int CDgnSeisAnchor2PryoutSubGrid::GetSelectedRowsCount(int top, int bottom)
{
	int toRows;
	toRows = bottom - top + 1;
	return toRows;
...
int CDgnSeisAnchor2PryoutSubGrid::GetSelectedColsCount(int left, int right)
{
	int toCols;
	toCols = right - left + 1;
	return toCols;
...
void CDgnSeisAnchor2PryoutSubGrid::ShowData()
{
	ROWCOL nRow, nCol;
...
void CDgnSeisAnchor2PryoutSubGrid::SetViewData()
{
	m_pParent->SetViewData();
}

BOOL CDgnSeisAnchor2PryoutSubGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CDgnSeisAnchor2PryoutSubGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CDgnSeisAnchor2PryoutSubGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
