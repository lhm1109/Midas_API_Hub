// DgnRCPlateBeamRebarSBarDlg.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCPlateBeamRebarSBarDlg dialog


CDgnRCPlateBeamRebarSBarDlg::CDgnRCPlateBeamRebarSBarDlg(T_RPBS_D* pData, CWnd* pParent /*=NULL*/)
	: CChildDialog(CDgnRCPlateBeamRebarSBarDlg::IDD, pParent)
{
...
CDgnRCPlateBeamRebarSBarDlg::~CDgnRCPlateBeamRebarSBarDlg()
{
	
}

void CDgnRCPlateBeamRebarSBarDlg::DoDataExchange(CDataExchange* pDX)
{
	CChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnRCPlateBeamRebarSBarDlg)  
...
BEGIN_MESSAGE_MAP(CDgnRCPlateBeamRebarSBarDlg, CChildDialog)
	//{{AFX_MSG_MAP(CDgnRCPlateBeamRebarSBarDlg)  
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCPlateBeamRebarSBarDlg message handlers

BOOL CDgnRCPlateBeamRebarSBarDlg::OnInitDialog() 
{
...
LRESULT CDgnRCPlateBeamRebarSBarDlg::OnBeforeEndEditing(WPARAM wParam, LPARAM lParam)
{
	Dlg2Data();
	ReCalcAs();
	return 1;
...
void CDgnRCPlateBeamRebarSBarDlg::OnClickedNumCTC()
{
	SetNumCTC(TRUE);
	ReCalcAs();
}

void CDgnRCPlateBeamRebarSBarDlg::SetNumCTC(BOOL bClick/*=TRUE*/)
{
...
void CDgnRCPlateBeamRebarSBarDlg::InitCtrl()
{
...
void CDgnRCPlateBeamRebarSBarDlg::InitCombo()
{
	CString str;
...
void CDgnRCPlateBeamRebarSBarDlg::InitGrid()
{
...
void CDgnRCPlateBeamRebarSBarDlg::InitUnit()
{
...
void CDgnRCPlateBeamRebarSBarDlg::ReCalcAs()
{
...
void CDgnRCPlateBeamRebarSBarDlg::Data2Dlg()
{
...
BOOL CDgnRCPlateBeamRebarSBarDlg::Dlg2Data()
{ 
	UpdateData(TRUE);
...
void CDgnRCPlateBeamRebarSBarDlg::SetDataToGrid()
{
	int nNumCTC;
	CDlgUtil::CtrlRadioGetCheck(this, m_aRdoNumCTC, nNumCTC);
	SetNumCTC();
...
void CDgnRCPlateBeamRebarSBarDlg::GetDataFromGrid()
{
	CString str = _T("");
	int nNumCTC;
	CDlgUtil::CtrlRadioGetCheck(this, m_aRdoNumCTC, nNumCTC);
...
void CDgnRCPlateBeamRebarSBarDlg::ControlsEnableDisable()
{ 
	
	
}
/*
BOOL CDgnRCPlateBeamRebarSBarDlg::CheckRebarData(T_RRPL_SUB_D &rData)
{
	// 이쪽은 안건드리는 걸로....
...
BOOL CDgnRCPlateBeamRebarSBarDlg::CheckRebarData_MainRebar01(T_RRPL_SUB_D &rData)
{
	// Rebar Check
	int  nInput = 0;
	BOOL bInput[3];
...
BOOL CDgnRCPlateBeamRebarSBarDlg::CheckRebarData_MainRebar02(T_RRPL_SUB_D &rData)
{
	int iChkResult = 0;
...
BOOL CDgnRCPlateBeamRebarSBarDlg::CheckPosiInputData_New(T_RRPL_SUB_D &rData)
{
...
BOOL CDgnRCPlateBeamRebarSBarDlg::CheckRebarData_Stirrup(T_RRPL_SUB_D &rData)
{
	double dZero = 1.0E-7;
...
void CDgnRCPlateBeamRebarSBarDlg::CalcAs(T_RPBS_D& RpbsD, double& dAsTop, double& dAsBot)
{
	dAsTop = 0.0;
	dAsBot = 0.0;
	...
	for(int i=0; i<RpbsD.nTopLayer; i++)
	{
		// Top.
		double dAs1=0.0, dAs2=0.0;
		if(RpbsD.strTopBarNa1[i]!=_T(""))
			dAs1 = m_pDoc->m_pMatlDB->Get_RebarArea(RpbsD.strTopBarNa1[i]);    
		//if(RpbsD.strTopBarNa2[i]!=_T(""))
		//  dAs2 = m_pDoc->m_pMatlDB->Get_RebarArea(RpbsD.strTopBarNa2[i]);
		...
		if(RpbsD.bNum)
		{
			dCTC = dUnitLen / RpbsD.nTopBarNum[i];
		}
		else
		{        
			dCTC = RpbsD.dTopBarCTC[i];      
		}
	...
	for (int i = 0; i < RpbsD.nBotLayer; i++)
	{
		// Bot.
		double dAs1=0.0, dAs2=0.0;
		if(RpbsD.strBotBarNa1[i]!=_T(""))
			dAs1 = m_pDoc->m_pMatlDB->Get_RebarArea(RpbsD.strBotBarNa1[i]);    
		//if(RpbsD.strBotBarNa2[i]!=_T(""))
		//  dAs2 = m_pDoc->m_pMatlDB->Get_RebarArea(RpbsD.strBotBarNa2[i]);
		//if(dAs2==0.0) dAs2 = dAs1;
		dAs2 = dAs1;
		...
		if(RpbsD.bNum)
		{
			dCTC = dUnitLen / RpbsD.nBotBarNum[i];
		}
		else
		{        
			dCTC = RpbsD.dBotBarCTC[i];      
		}
...
void CDgnRCPlateBeamRebarSBarDlg::PostNcDestroy() 
{	
...
void CDgnRCPlateBeamRebarSBarDlg::OnOK()
{
	if(!Dlg2Data()) return;
...
void CDgnRCPlateBeamRebarSBarDlg::OnCancel()
{
	// TODO: Add extra cleanup here
	CChildDialog::OnCancel();
}

void CDgnRCPlateBeamRebarSBarDlg::OnSelchangeDgnRcTopLayerCmb() 
{
...
void CDgnRCPlateBeamRebarSBarDlg::OnSelchangeDgnRcBotLayerCmb() 
{
...
void CDgnRCPlateBeamRebarSBarDlg::OnDestroy() 
{
	Dlg2Data();
