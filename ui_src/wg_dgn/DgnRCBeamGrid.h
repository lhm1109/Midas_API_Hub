#if !defined(__DGNRCBEAMGRID_H__)
#define __DGNRCBEAMGRID_H__
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCBeamGrid window
class CDgnRCBeamGrid : public CTBCommon
{
// Construction
public:
	CDgnRCBeamGrid();
...
// Operations
public:
	void Initialize(CRCBeamParameter* pParent);

	BOOL DeleteRecordByKey(T_ELEM_K &key, T_RCBE_D &data);
	BOOL InsertRecordByKey(T_ELEM_K &key, T_RCBE_D &data);
...
// Overrides
	...
	//{{AFX_VIRTUAL(CDgnRCBeamGrid)
	//}}AFX_VIRTUAL
public:
	virtual int  GetKeyColID() { return 0; }
	virtual void OnChangedSelectionPublic(long *keys, int nSize);

	virtual BOOL ValidateAndMakeRecord(CStringArray &value, CRowColArray &aCols, T_ELEM_K &key, T_RCBE_D &data);
	...
	// 외부에서 현재 레코드의 키 참조
	BOOL GetCurrentRcbe(T_ELEM_K& nKey);
	// 외부에서 현재 레코드로 Current Cell 이동
	BOOL SetCurrentRcbe(T_ELEM_K key, T_RCBE_D& data);
...
// Implementation
public:
	virtual ~CDgnRCBeamGrid();
	BOOL GetValue(ROWCOL nRow, T_ELEM_K &key, T_RCBE_D &data);
	BOOL SetValue(ROWCOL nRow, T_ELEM_K key, T_RCBE_D &data);
	BOOL SetValue(CGXData* pData, ROWCOL nRow, T_ELEM_K key, T_RCBE_D &data, CRowColArray &awCols);
	void ConvStrToData(int i, CString& value, T_RCBE_D& data);
	void ConvDataToStr(int i, T_ELEM_K key, T_RCBE_D& data, CString& value);

	void GetAllSelectedStld(CArray<T_ELEM_K, T_ELEM_K>& aStldKey);
	void MakeSearchKey(T_ELEM_K &key, T_RCBE_D &data, CStringArray &aKey);
...
private:
	...
	CRCBeamParameter* m_pParent;

	CString GetStringiServCond(int index);
	CString GetStringLoadCase(T_RCBG_D& data);
};
