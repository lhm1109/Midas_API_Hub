// RatingCaseMainGrid.cpp: implementation of the CRatingCaseMainGrid class.
//
//////////////////////////////////////////////////////////////////////
...
CString CRatingCaseMainGrid::m_aCurDefVal[] = {_T(""), _T(""), _T(""), _T(""), _T("")};


CRatingCaseMainGrid::CRatingCaseMainGrid()
{
...
BEGIN_MESSAGE_MAP(CRatingCaseMainGrid, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CRatingCaseMainGrid)
	ON_WM_RBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CRatingCaseMainGrid message handlers
void CRatingCaseMainGrid::Initialize(T_RTLD_D RateData[10], CRatingCaseDlg* pParent)
{
 ...
 // 컬럼 스타일 변경
	...
	*/
	...
	{
		InsertRecordByKey(i, m_pData->GetAt(i));
	}
	*/
...
void CRatingCaseMainGrid::SetCellStyle()
{
	LockUpdate(TRUE);
	...
	SetCoveredCellsRowCol(3,3,3,4);
	SetStyleRange(CGXRange(3,3,3,4),style);
	...
	SetStyleRange(CGXRange(8,3,8,4), CGXStyle()
			.SetVerticalAlignment(DT_VCENTER)
			.SetHorizontalAlignment(DT_RIGHT)
			.SetValueType(GX_VT_NUMERIC)
			.SetFormat(GX_FMT_FIXED)
	...
	/*
	SetStyleRange(CGXRange(3,4,6,4), CGXStyle()
	    .SetHorizontalAlignment(DT_RIGHT)
			.SetValueType(GX_VT_STRING)
			.SetFormat(GX_FMT_FIXED)
			.SetReadOnly(TRUE));
...
void CRatingCaseMainGrid::Set_Title()
{
	CString title=_T("");
...
void CRatingCaseMainGrid::LoadDlgData()
{
	SetValue(1, 0, RateLoad[0]);//DC
	SetValue(2, 1, RateLoad[1]);//DW
	SetValue(3, 2, RateLoad[2]);//Temp
	SetValue(4, 3, RateLoad[6]);//Temp.Gradient
	SetValue(5, 4, RateLoad[3]);//SEC
	SetValue(6, 5, RateLoad[4]);//Permanet
	SetValue(7, 6, RateLoad[5]);//User
	
	Redraw();
}

void CRatingCaseMainGrid::MakeItemEx()
{
	CWaitCursor wait;
...
BOOL CRatingCaseMainGrid::InsertRecordByKey(int &key, T_RTLD_D &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CRatingCaseMainGrid::DeleteRecordByKey(int &key, T_RTLD_D &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
void CRatingCaseMainGrid::UnitChanged(CDBDoc* pDoc)
{
...
/////////////////////////////////////////////////////////////////////////////
// CRatingCaseMainGrid Overridable Functions
void CRatingCaseMainGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	if (nSize <= 0)
	{
...
long CRatingCaseMainGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
...
BOOL CRatingCaseMainGrid::ValidateField(CString value, int nColID)
{
...
BOOL CRatingCaseMainGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CRatingCaseMainGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, int &key, T_RTLD_D &data)
{
...
BOOL CRatingCaseMainGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CRatingCaseMainGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
...
BOOL CRatingCaseMainGrid::DeleteFromDB(ROWCOL nRow)
{
	int key;
	T_RTLD_D data;
...
BOOL CRatingCaseMainGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CRatingCaseMainGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
...
BOOL CRatingCaseMainGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CRatingCaseMainGrid::GetNewKey()
{
	CString value;
...
CString CRatingCaseMainGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CRatingCaseMainGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}


BOOL CRatingCaseMainGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CRatingCaseMainGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	int* pKey;
	T_RTLD_D* pData;
...
long CRatingCaseMainGrid::GetCountData()
{
	CDBDoc* pDoc = m_pDoc;
...
void CRatingCaseMainGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	CDBDoc* pDoc = m_pDoc;
	T_RTLD_D Data;
...
void CRatingCaseMainGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	//int Key;
	T_RTLD_D Data;
	CDBDoc* pDoc = m_pDoc;
...
void CRatingCaseMainGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	int Key;
	T_RTLD_D Data;
...
/////////////////////////////////////////////////////////////////////////////
// CRatingCaseMainGrid Implementation
BOOL CRatingCaseMainGrid::GetValue(ROWCOL nRow, int &key, T_RTLD_D &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CRatingCaseMainGrid::SetValue(ROWCOL nRow, int key, T_RTLD_D &data)
{
	//
	CRowColArray awCols;
...
BOOL CRatingCaseMainGrid::SetValue(CGXData* pData, ROWCOL nRow, int key, T_RTLD_D &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CRatingCaseMainGrid::ConvStrToData(int i, CString& value, T_RTLD_D& data)
{
	if (i==1)  // Load Type
	{
		if(value == _LS(IDS_DGN_RATING_DC))
		{
...
void CRatingCaseMainGrid::ConvDataToStr(int i, T_RTLD_D& data, CString& value)
{
	if (i==1) // Load Type
	{
		if(data.LoadType == D_RCTYPE_DC)
			value = _LS(IDS_DGN_RATING_DC);
		else if(data.LoadType == D_RCTYPE_DW)
			value = _LS(IDS_DGN_RATING_DW);
		else if(data.LoadType == D_RCTYPE_TEMPERATURE)
			value = _LS(IDS_DGN_RATING_T);
		else if(data.LoadType == D_RCTYPE_SECONDARY)
			value = _LS(IDS_DGN_RATING_SEC);
		else if(data.LoadType == D_RCTYPE_PERMANENT)
			value = _LS(IDS_DGN_RATING_PERMANENT);
		else if(data.LoadType == D_RCTYPE_USER)
			value = _LS(IDS_DGN_RATING_USER_DEFINE);//_T("User Defined");
		else if(data.LoadType == D_RCTYPE_TG)
...
void CRatingCaseMainGrid::MakeSearchKey(int &key, T_RTLD_D &data, CStringArray &aKey)
{
	int nColID;
...
void CRatingCaseMainGrid::GetAllSelectedStld(CArray<int, int>& aSelKey)
{
	for (int i = 0; i < m_aSelectedKeys.GetSize(); i++)
		aSelKey.Add(m_aSelectedKeys[i]);
}

void CRatingCaseMainGrid::SetHeaderTitle(T_UNIT_SYSTEM &UnitSystem, BOOL bInit)
{
...
// 주1 참조
BOOL CRatingCaseMainGrid::OnTrackColWidth(ROWCOL nCol)
{
...
// 이동되는 것을 금지한다.
// 주1 참조
BOOL CRatingCaseMainGrid::OnSelDragColsDrop(ROWCOL nFirstCol, ROWCOL nLastCol, ROWCOL nDestCol)
{
...
/*
BOOL CRatingCaseMainGrid::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
/*
	if (IsEdit()) 
...
void CRatingCaseMainGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
...
/*
CString CRatingCaseMainGrid::GetLoadTypeCode(CString &csLoadTypeName)
{
	return m_pDoc->m_pLoadDB->GetLoadType(csLoadTypeName);
}

CString CRatingCaseMainGrid::GetLoadTypeName(CString &csLoadTypeCode)
{
...
BOOL CRatingCaseMainGrid::CheckCurrentRecordChanged()
{
	ROWCOL ncRow, ncCol;
...
BOOL CRatingCaseMainGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
...
void CRatingCaseMainGrid::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CRatingCaseMainGrid::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CRatingCaseMainGrid::OnChangeCurrentRecord(ROWCOL nRow)
{
	if(nRow>7)
		return;
...
void CRatingCaseMainGrid::SaveCurrentGrid()
{
	m_pSubGrid->GetCombItem(RateLoad[GetDataIndexfromRow(m_nCurrentRow)]);
	...
	//Temp.Gradient
	RateLoad[6].dFactor[0] = _tstof(GetValueRowCol(4, 3));
	RateLoad[6].dFactor[1] = _tstof(GetValueRowCol(4, 4));
	//SEC
	RateLoad[3].dFactor[0] = _tstof(GetValueRowCol(5, 3));
	RateLoad[3].dFactor[1] = _tstof(GetValueRowCol(5, 4));
	//Permanet
	RateLoad[4].dFactor[0] = _tstof(GetValueRowCol(6, 3));
	RateLoad[4].dFactor[1] = _tstof(GetValueRowCol(6, 4));
	//User
	RateLoad[5].dFactor[0] = _tstof(GetValueRowCol(7, 3));
	RateLoad[5].dFactor[1] = _tstof(GetValueRowCol(7, 4));	
}

int CRatingCaseMainGrid::GetDataIndexfromRow(int nRow)
{
	int nIndex = 0;
...
CString CRatingCaseMainGrid::GetKeyForRow(ROWCOL ncRow, ROWCOL nkCol)
{
	CString sKey;
...
BOOL CRatingCaseMainGrid::GetCurrentRcbe(int& nKey)
{
	ROWCOL ncRow, ncCol;
...
BOOL CRatingCaseMainGrid::SetCurrentRcbe(int key, T_RTLD_D& data)
{
	ROWCOL nRow, nCol;
...
BOOL CRatingCaseMainGrid::LockUpdateGrid(BOOL bUpdate)
{
	return LockUpdate(bUpdate);
}

void CRatingCaseMainGrid::SetRtldData(T_RTLD_D RateData[10])
{
	for(int i=0 ; i<7 ; i++)
	{
	  RateLoad[i] = RateData[i];
