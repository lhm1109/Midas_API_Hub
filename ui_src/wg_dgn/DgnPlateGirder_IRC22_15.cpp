// DgnPlateGirder_IRC22_15.cpp: implementation of the CDgnPlateGirder_IRC22_15 class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "..\DgnEngine\Src\DgnCalcBase\DgnCalcBase_PSC_Struct.h"
#include "DgnPlateGirder_IRC22_15.h"
...
CDgnPlateGirder_IRC22_15::CDgnPlateGirder_IRC22_15()
{
	...
	m_LcomPgbr_IRC_Map.InitHashTable(10001);
	//
	m_dPi = 4.0*atan(1.0);
...
void CDgnPlateGirder_IRC22_15::SetDataCtrlPointer(CCRCDataCtrl* pDataCtrl)
{
...
void CDgnPlateGirder_IRC22_15::InitPartialFactor()
{
	T_CPGD_D CpgdD;
	CpgdD.Initialize();  
	...
	m_dGamma_C_Acc    = CpgdD.dGamma_S; 
	m_dGamma_S    = CpgdD.dGamma_V;
	m_dGamma_M0   = CpgdD.dGamma_M0_IRC;
	m_dGamma_M1   = CpgdD.dGamma_M1_IRC;
	m_dGamma_V    = CpgdD.dGamma_Ff;
	m_dGamma_Fft   = CpgdD.dGamma_Mf;
	m_dGamma_Mft   = CpgdD.dGamma_Mfs;

	m_dGamma_Mfs  = CpgdD.dGamma_Mfs;
    m_dGamma_M_ser = CpgdD.dGamma_M_ser/0.0001;
...
BOOL CDgnPlateGirder_IRC22_15::Make_ClassSection(ElemPairK ElemK, T_PGBS_D& PgbsD, T_PGBR_LCOM& PgbrLcomD)
{
...
BOOL CDgnPlateGirder_IRC22_15::Make_ClassInD(BOOL bPosiI, double aGirderForce[6], double aCptForce[6], CPG_SECT_ELEM& rSectD, T_CLASS_IN_D_IRC_15& ClassInD)
{
...
BOOL CDgnPlateGirder_IRC22_15::Make_ClassInD_I(BOOL bPosiI, double aGirderForce[6], double aCptForce[6], CPG_SECT_ELEM& rSectD, T_CLASS_IN_D_IRC_15& ClassInD)
{
	// Get Force
...
BOOL CDgnPlateGirder_IRC22_15::Make_ClassInD_Box(BOOL bPosiI, double aGirderForce[6], double aCptForce[6], CPG_SECT_ELEM& rSectD, T_CLASS_IN_D_IRC_15& ClassInD)
{
	ClassInD.Initialize();
...
BOOL CDgnPlateGirder_IRC22_15::Make_ClassInD_Tub(BOOL bPosiI, double aGirderForce[6], double aCptForce[6], CPG_SECT_ELEM& rSectD, T_CLASS_IN_D_IRC_15& ClassInD)
{
	ClassInD.Initialize();
...
BOOL CDgnPlateGirder_IRC22_15::Get_BendingResistanceForPlateGirder(ElemPairK ElemK, T_PGBR_LCOM& PgbrLcomD)
{	
	//+++++++++++++++++++++++++++++++++++++++
	// CurrUnit -> CodeUnit.
	//  m_pDataCtrl->Get_BendingResistanceForPlateGirderCurrToCode(PgbrLcomD);
...
BOOL CDgnPlateGirder_IRC22_15::Get_BendingResistanceForPlateGirder(ElemPairK ElemK, T_PGBR_IRC_LCOM& PgbrLcomD)
{	
	//+++++++++++++++++++++++++++++++++++++++
	// CurrUnit -> CodeUnit.
	//  m_pDataCtrl->Get_BendingResistanceForPlateGirderCurrToCode(PgbrLcomD);
	...
	for(int i=0; i<2; i++)
	{
		...
		if(PgbrLcomD.PgbrBase[i].bChk==FALSE) continue; 
		//T_PGBR_IRC_DETAIL PgbrDetail;
		//if(!Calc_BendingResistance(ElemK, bPosiI, MatlElem, SectElem, arRbarPosi, arLstiPosi, SconElem, TrstPosi, ClassD, PgbrLcomD.PgbrBase[i], PgbrDetail)) continue;    
		m_LcomPgbr_IRC_Map.SetAt(ComfKey,PgbrLcomD.PgbrBase[i]);
	}
	//+++++++++++++++++++++++++++++++++++++++
	// CodeUnit -> CurrUnit.
	//m_pDataCtrl->Get_BendingResistanceForPlateGirderCodeToCurr(PgbrLcomD);
...
BOOL CDgnPlateGirder_IRC22_15::Get_VerticalShearResistanceForPlateGirder(ElemPairK ElemK, T_PRVS_D& PrvsD)
{
	//+++++++++++++++++++++++++++++++++++++++
	// CurrUnit -> CodeUnit.
...
BOOL CDgnPlateGirder_IRC22_15::Get_LateralTorsionalBucklingResistanceForPlateGirder(ElemPairK ElemK, T_PLTB_D& PltbD)
{
	//+++++++++++++++++++++++++++++++++++++++
	// CurrUnit -> CodeUnit.
...
BOOL CDgnPlateGirder_IRC22_15::Get_TransverseForceResistanceForPlateGirder(ElemPairK ElemK, T_PRTF_D& PrtfD)
{
	//+++++++++++++++++++++++++++++++++++++++
	// CurrUnit -> CodeUnit.
...
BOOL CDgnPlateGirder_IRC22_15::Get_LongitudinalShearResistanceForPlateGirder(ElemPairK ElemK, T_PGLS_D& PglsD)
{
	//+++++++++++++++++++++++++++++++++++++++
	// CurrUnit -> CodeUnit.
...
BOOL CDgnPlateGirder_IRC22_15::Get_FatigueResistanceForPlateGirder(ElemPairK ElemK, T_PGRF_D& PgrfD, double dt_Ld)
{
	//+++++++++++++++++++++++++++++++++++++++
	// CurrUnit -> CodeUnit.
...
BOOL CDgnPlateGirder_IRC22_15::Get_BeamStressResistanceForPlateGirder(ElemPairK ElemK, T_PGBS_D& PgbsD)
{
	return TRUE;
}

BOOL CDgnPlateGirder_IRC22_15::Get_DesignResultExist(BOOL* abChkItem)
{
	return TRUE;
}

BOOL CDgnPlateGirder_IRC22_15::Get_PlateGirderDgnResult(BOOL* abChkItem)
{
	//MainFunction
	//////////////////////////////////////////////////////////////////////////
	...
	m_pDataCtrl->GetPgdElemKeyList(aElemPairK);
	int iTotalElement = aElemPairK.GetSize();
	...
	{
		auto ElemK = aElemPairK[i];

        m_Pgbr_IRC_Base.Initialize();
	...
	if(Get_CpgFrdtElem(ElemK, FrdtElem)) {bLamdaData=TRUE;}
		...
		CArray<UINT, UINT> aSpanK;
		m_pDoc->m_pAttrCtrl->GetSpanKeyList(aSpanK, 1);  // m_nOption -> 0:PSC, 1:Composite, 2:Stl Girder
		int nSize = aSpanK.GetSize();
		int iEffWidCase = 0;
		for(int p=0; p<nSize; ++p)
		{
			T_SPAN_K SpanK = aSpanK.GetAt(p);
			T_SPAN_D SpanD;
			if(!m_pDoc->m_pAttrCtrl->GetSpan(SpanK, SpanD)) { ASSERT(0); continue; }
			//nDirection - inner direction of multiple girders
			//SpanD.aElemBase[i].nSupport
			int nSubSpanNum = SpanD.aElemBase.GetSize();
			for (int m=0;m<nSubSpanNum;m++)
			{
				if (SpanD.aElemBase[m].ElemK == ElemK.first)
			{
				//Found correct span containing this element
					if (SpanD.aElemBase[m].nSupport == 2 && SpanD.nDirection <=1) iEffWidCase =1;
					else if(SpanD.aElemBase[m].nSupport == 2 && SpanD.nDirection > 1) iEffWidCase =2;
					else if(SpanD.aElemBase[m].nSupport == 1 && SpanD.nDirection <=1) iEffWidCase =3;
				else iEffWidCase =4;
			}
		}
		...
		T_EWSF_D EwsfD;EwsfD.Initialize();//Effective width calculation
		BOOL bEwsfOK =  m_pDoc->m_pAttrCtrl->GetEwsfElem(ElemK.first, EwsfD);


		//Set Section Data
		SetSectionPropData(ElemK,SectElem.SectD,SectElem,MatlElem.MatdD);
        m_dBeffLength = m_dBc ;//* EwsfD.dZtopFactor;
	...
	// 4. Get Member Data
		...
		m_dEc = MatlElem.dEcm;
		m_dfyk = MatlElem.MatdD.MainRebarData.B_fy;
	// 5. Get Design Force
	int nFortype = 0;
	...
	T_PGBR_IRC_LCOM PgbrIRCrat; PgbrIRCrat.Initialize();
	  ...
	  PlssRat.PlssBase[j].ElemK		= ElemK.first;
		PgbrIRCrat.PgbrBase[j].ElemK = ElemK.first;
		}
	...
	for(int j=0; j < iForceLcomSize; j++)
	{
		...
		{
            T_LCOM_D LcomData;      
            if (m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL, iOrgLcomNo, LcomData)) 
		{
                bULSLcom = LcomData.nActive==1 ? TRUE:FALSE;
                bSLSLcom = LcomData.nActive==2 ? TRUE:FALSE;
		...
		T_PGBR_IRC_LCOM PgbrIRCLcomD; PgbrIRCLcomD.Initialize();
		...
		{		    
			...
			T_PGBR_IRC_BASE PgbrIRCGd, PgbrIRCBase; 
			if(bIsGenSect)
			{
			...
			m_ClassBase = ClassD;
			//
			SetPgbrBaseData(bChkIJ[k], ElemK, nPosi, iDgnLcomNo, iOrgLcomNo, iMaxMinType, aGirderForce, aCptForce, PgbrGd, PgbrBase, PgbrIRCGd, PgbrIRCBase);        
			//SetPgIRCData(bChkIJ[k], ElemK, nPosi, iDgnLcomNo, iOrgLcomNo, iMaxMinType, aGirderForce, aCptForce, PgbrGd, PgbrBase,PgbrGdIRC);        
			PgbrIRCBase.SectProp.iEffWidthCase = iEffWidCase;

				if (PgbrIRCGd.SectProp.dSLlo == 0.)
				{
					CArray<UINT, UINT> aSpanK;
					m_pDoc->m_pAttrCtrl->GetSpanKeyList(aSpanK, 1);  // m_nOption -> 0:PSC, 1:Composite, 2:Stl Girder
					int nSize = aSpanK.GetSize();
					...
					for(int p=0; p<nSize; ++p)
					{
						...
						for (int m=0;m<nSubSpanNum;m++)
						{
							...
							{
								//Found correct span containing this element
								if(SpanD.aElemBase[m].nSupport == 1 ) dSpanLength= SpanD.aSpan[m];
								else dSpanLength+= SpanD.aSpan[0];
							}
						}

					}
					PgbrIRCGd.SectProp.dSLlo =dSpanLength;// max(dSpanLength,m_dTotalLength);
				}
				

				
				if (PgbrIRCBase.SectProp.iEffWidthCase==0 || PgbrIRCBase.SectProp.iEffWidthCase==5)
				{
					m_dBeffLength = PgbrIRCGd.SectProp.dBc;
				}
				else if (PgbrIRCBase.SectProp.iEffWidthCase==2)
				{
					double dCalVal = (PgbrIRCGd.SectProp.dSLlo/8.) +  (PgbrIRCGd.SectProp.dBc/2.);
					dCalVal = min(dCalVal , PgbrIRCGd.SectProp.dBc) ;
					m_dBeffLength = dCalVal;
				}
				else if (PgbrIRCBase.SectProp.iEffWidthCase==3)
				{
					double dCalVal = (PgbrIRCGd.SectProp.dSLlo/4.) +  (PgbrIRCGd.SectProp.dBc/2.);
					dCalVal = min(dCalVal , PgbrIRCGd.SectProp.dBc) ;
					m_dBeffLength = dCalVal;
				}
				else if (PgbrIRCBase.SectProp.iEffWidthCase==4)
				{
					double dCalVal = (PgbrIRCGd.SectProp.dSLlo/8.) +  (PgbrIRCGd.SectProp.dBc/2.);
					dCalVal = min(dCalVal , PgbrIRCGd.SectProp.dBc) ;
					m_dBeffLength = dCalVal;
			...
			PgbrIRCBase.SectProp.dEffWidth = m_dBeffLength;
			PgbrGd.dNc = dFzz;
			...
			PgbrIRCLcomD.BeforePgbr[k] = PgbrIRCGd;  
			PgbrIRCLcomD.PgbrBase[k]	= PgbrIRCGd;  
            if (m_Pgbr_IRC_Base.LotrShear.strLcomName.GetLength()+m_Pgbr_IRC_Base.Fatg.strLCLcomb.GetLength()==0 && j+k==0)
            {
			m_Pgbr_IRC_Base   =PgbrIRCGd;
			Get_SavedResult4Design(ElemK,bPosiI,PgbrBase,m_Pgbr_IRC_Base);
            }
			...
			if(bULSLcom && bCheckStrength[1])
			{
				...
				if(!bIsGenSect) {bPitchOk = Calc_VerticalShearResistance(ElemK, bPosiI, MatlElem, SectElem, arRbarPosi, arLstiPosi, TrstPosi, TsesPosi, ClassD, PrvsGd, PrvsBase, PrvsDetail,PgbrBase,PgbrDetail);				  }
			...
			if(bULSLcom && !bIsGenSect && bCheckStrength[3])
			{
				...
				if(bCalcPrtf)
				{
                    T_PGBR_IRC_BASE PgbrIRCBasePRTF; PgbrIRCBasePRTF.Initialize();
                    if (m_Pgbr_IRC_Base.LotrShear.strLcomName.GetLength()>0 && m_Pgbr_IRC_Base.Fatg.strLCLcomb.GetLength()>0)
                    {
                        PgbrIRCBasePRTF = m_Pgbr_IRC_Base;
                    }
                    else
                    {
                        Get_SavedResult4Design(ElemK,bPosiI,PgbrBase,PgbrIRCBasePRTF);
                    }
			...
			if(bSLSLcom && !bIsGenSect && bCheckStrength[5])
			{
                ...
                BOOL bFati = TRUE;
                if (!m_pForcCtrl->m_aFatiLcomId.Lookup(iOrgLcomNo, FatiLcomId)) {bFati = FALSE;}
                if (bFati)
                {
				T_PGRF_DETAIL PgrfDetail; 
		...
		if(bULSLcom)
		{
			...
			if(!pPGD->WriteBendingResistanceResult_IRC(ElemK, iDgnLcomNo, iMaxMin, PgbrIRCLcomD)){ASSERT(0);}
	  ...
	  // Critical for each Element
		...
		if(!pPGD->WriteBendingResistanceResult_IRC(ElemK, iDgnLcomNo, iMaxMin, PgbrIRCrat)){ASSERT(0);}
...
BOOL CDgnPlateGirder_IRC22_15::Get_BendingResistanceDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PGBR_BASE& PgbrGd, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_VerticalShearResistanceDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PRVS_BASE& PrvsGd, const T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	BOOL bRebar=FALSE;
	BOOL bShearConnector=FALSE;
	BOOL bLongStiff=FALSE;
	...
	if(!bIsGenSect) {Calc_VerticalShearResistance(ElemK, bPosiI, MatlElem, SectElem, arRbarPosi, arLstiPosi, TrstPosi, TsesPosi, ClassD, TempPrvsGd, TempPrvsBase, PrvsDetail,PgbrBase,PgbrDetail);				  }
...
BOOL CDgnPlateGirder_IRC22_15::Get_LateralTorsionalBucklingResistanceDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PLTB_BASE& PltbGd, T_PLTB_BASE& PltbBase, T_PLTB_DETAIL& PltbDetail,BOOL bspcl)
{
	BOOL bRebar=FALSE;
	BOOL bShearConnector=FALSE;
	BOOL bLongStiff=FALSE;
...
BOOL CDgnPlateGirder_IRC22_15::Get_TransverseForceResistanceDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PRTF_BASE& PrtfBase, T_PRTF_DETAIL& PrtfDetail)
{
	BOOL bRebar=FALSE;
	BOOL bShearConnector=FALSE;
	BOOL bLongStiff=FALSE;
...
BOOL CDgnPlateGirder_IRC22_15::Get_LongitudinalShearResistanceDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PGLS_BASE& PglsGd, const T_PGLS_BASE& PglsBase, T_PGLS_DETAIL& PglsDetail)
{
	BOOL bRebar=FALSE;
	BOOL bShearConnector=FALSE;
	BOOL bLongStiff=FALSE;
...
BOOL CDgnPlateGirder_IRC22_15::Get_FatigueResistanceDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PGRF_BASE& PgrfBase, T_PGRF_DETAIL& PgrfDetail)
{
	CDBDoc* pDoc = CDBDoc::GetDocPoint();
	ASSERT(pDoc); 
...
BOOL CDgnPlateGirder_IRC22_15::Get_BeamStressDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PGBS_BASE& PgbsBase, T_PGBS_DETAIL& PgbsDetail)
{
	BOOL bRebar=FALSE;
	BOOL bShearConnector=FALSE;
	BOOL bLongStiff=FALSE;
	...
	iDgnLcomNo = PgbsBase.ChkGirder.LcomK == 0 ? PgbsBase.ChkSlab.LcomK : PgbsBase.ChkGirder.LcomK;
	if(!m_pForcCtrl->Get_LcomType(iDgnLcomNo, strOrgLcomNa, iOrgLcomNo, iSerLcomCount, iMaxMinType))
	{
		ASSERT(0);
		return FALSE;
...
BOOL CDgnPlateGirder_IRC22_15::Get_LongitudinalShearResistanceServiceDetailResult4PlateGirder(ElemPairK ElemK, BOOL bPosiI, const T_PLSS_BASE& PlssGd, const T_PLSS_BASE& PlssBase, T_PLSS_DETAIL& PlssDetail)
{
	BOOL bRebar=FALSE;
	BOOL bShearConnector=FALSE;
	BOOL bLongStiff=FALSE;
...
void CDgnPlateGirder_IRC22_15::Set_PrintMode(BOOL bPrint)
{
	m_bPrintMode = bPrint;
}

BOOL CDgnPlateGirder_IRC22_15::IsPrintMode()
{
	return m_bPrintMode;
}
...
BOOL CDgnPlateGirder_IRC22_15::Calc_BendingResistance(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem,
																						 ...
																						 T_PGBR_BASE& PgbrGd, T_PGBR_BASE& PgbrBase, T_PGBR_DETAIL& PgbrDetail)
{
	...
	double deta,dlambda;
	CalEtaLambda(dfck,deta,dlambda);
	double dfcd		=  0.67*deta*dlambda* dfck/dgam_c;
	...
	double dfyd = min(dfyt,min(dfyb,dfyw)) / dgam_M0;
	double dFyFinal = min(dfyt,min(dfyb,dfyw))/m_dGamma_M0;
	// 3. Force   
	...
	//For Plastic Moment zp
	double dbbf =  m_dB1  , dbtf  = m_dB2;
	double dtbf =  m_dTF1 , dttf  = m_dTF2;
	double  dtw   = m_dTw;
	double tf_top=dttf,
		tf_bottom=dtbf,
		tw=dtw,
		...
		bf_top=dbtf,
		bf_bottom = dbbf;
		T_PGBR_IRC_BASE PgbrIRCGd;PgbrIRCGd.Initialize();
	Get_Plastic_Steel_Results(bf_top, tw, bf_bottom, tf_top,  dw,tf_bottom,PgbrIRCGd.SectProp.SteelSect );
	Get_Elastic_Steel_Results(bf_top, tw, bf_bottom, tf_top,  dw,tf_bottom,PgbrIRCGd.SectProp.SteelSect );

	 m_ddccase3  = Get_ddcfromybar(bf_top, tw, bf_bottom, tf_top,  dw,tf_bottom,PgbrIRCGd.SectProp.SteelSect ) + dHh + 0.5*dtc;
	// 6. Check Non-linear
	...
	double dNa_top = (iClassSect==3 )? dB1Sect3*dTF1*dfyd:dB1*dTF1*dfyd;
	double dNa_bot = (iClassSect==3 && !bPositive)? dB2Sect3*dTF2*dfyd:dB2*dTF2*dfyd;
	double dNa_web = (iClassSect==3 )? dhwSect3*dTw_t*dfyd:dhw*dTw_t*dfyd;
		...
		if(bPositive) // Positive일 경우는 강도 계산할 때 압축콘크리트 중심(Xpl*0.5)에서 계산한다.
		{
			if(dPlasticNA > dH) // PNA in Slab
			{
				 ...
				 // {
						...
						double da_c    = (dEta/2.)*dxu;dDistWc1 = da_c;
...
// 
//             double da_c    = (dEta/2.)*dXpl;dDistWc1 = da_c;
				...
				if(dPlasticNA > dH-dTF1) // PNA in Top Flange
				{
				 // if (iClassSect==3)
					//{
						...
						double da_c    = dXpl - (dEta*dtc/2.);dDistWc1 = da_c;
...
//           {
						...
						double dTft = dPlasticNA - (dH-dTF1);
						dNa_webc = (dxu-dtc-dHh-dTF1)*dTw_t*dfyd;
						dNa_webt = (dtc+dHh+dTF1+dhw-dxu)*dTw_t*dfyd;
						...
						double da_c    = dxu - (dEta*dtc/2.);dDistWc1 = da_c;
...
//           }
		...
		else // Negative moment, Negative일 경우는 강도 계산할 때 PlasticNA가 기준
		{
			if (iClassSect == 3)
			{
				...
				T_PGBR_IRC_BASE PgbrIRCGdTemp;PgbrIRCGdTemp.Initialize();
				
				Get_Elastic_Steel_Results(bf_top, tw, dB1Sect3, tf_top,  dw,tf_bottom,PgbrIRCGdTemp.SectProp.SteelSect );
				double dxu = PgbrIRCGdTemp.SectProp.SteelSect.dbot;dNg_top = dxu;
				...
				double dxe = dTopArea * (dhw + dTF2 + dTF1 - dxu +dtc  - dCover )/ (PgbrIRCGdTemp.SectProp.SteelSect.dArea + dTopArea);
				dNa_top = dxe; 
				double dIc = PgbrIRCGdTemp.SectProp.SteelSect.dIyy + (PgbrIRCGdTemp.SectProp.SteelSect.dArea * dxe*dxe) + dTopArea * pow( (dhw + dTF2 + dTF1 - dxu +dtc - dCover - dxe ),2);
				dNa_web = dIc;
				double dfs = abs(dMa_Ed) * dxu / PgbrIRCGdTemp.SectProp.SteelSect.dIyy ;
				dNa_bot = dfs;
				dMpl_Rd = abs(dMa_Ed) + (dfyd - 0.001*dfs) * dIc / (dhw + dTF2 + dTF1 - dxu + dxe ) ;
			}
			else
			{
				...
				else
				{
					if(dPlasticNA > dH-dTF1) // PNA in Top Flange
					{
						...
						double dfacty = ((dTensfsdAsi_sum*0.5 - dNa_web*0.5)/dB1/ dfyd) + 0.5*dhw  ;dDistW=dfacty;
						double dfactz = (dhw + dTF2 + dTF1)*0.5 +dtc- dCover -  dfacty*0.5 ;dDistB=dfactz;
						double dpnaalone = (((((dNa_top + dNa_web + dNa_bot)/2.) - (dNa_top))/dfyd)/dtw) + dTF1 ;dDistTt=dpnaalone;

						dMpl_Rd = (PgbrIRCGd.SectProp.SteelSect.dzp * dfyd) + (dTensfsdAsi_sum*dfactz)  ;
						dNa_botc = PgbrIRCGd.SectProp.SteelSect.dzp;
	...
	//           if (iClassSect==3)
						//{
							...
							double dfacty = dTensfsdAsi_sum/(2.0*dTw_t*dfyd);dDistW=dfacty;
							double dModZpfact = dTF1 + (((0.5*GirderStiff.Area) - (dB1*dTF1))/dTw_t);dDistTt=dModZpfact;
							double dfactxu = dtc+dHh + dTF1+ ( dModZpfact*0.5) - dfacty;dDistB=dfactxu;
							double dfactz = (dhw + dTF2 + dTF1)*0.5 +dtc- dCover -  dfacty*0.5;//dDistT=PgbrIRCGd.SectProp.SteelSect.dzp;
							double dfactzp = 0;
						 ...
						 dDistWc1 = dfactz;
							...
							dMpl_Rd = PgbrIRCGd.SectProp.SteelSect.dzp * dfyd + dfactz*dTensfsdAsi_sum; //dNa_top*dDistT + dNa_webc*dDistWc + dNa_webt*dDistWt+dNa_bot*dDistB + dCompfsdAsidsi + dTensfsdAsidsi;
							dN_slab = dTensfsdAsi_sum;  
	...
	// Non-linear Resistance
	 ...
	 else
 	{
 		...
 		else
 		{
			 ...
			 else 	PgbrBase.dM_Rd = fabs(PgbrBase.dMpl_Rd);
				if (iClassSect == 3 ) PgbrBase.dMel_Rd = PgbrBase.dMpl_Rd;
	...
	PgbrBase.dNa_webc2		= dNa_webc2;    //For MbRd    
	...
	PgbrBase.dWebClassRat		= ClassD.dBTR[4] == 0 ? dhw/dTw_t:ClassD.dBTR[4];
...
BOOL CDgnPlateGirder_IRC22_15::Get_BetaReductionFactor(double dXpl, double dh, double& dBeta)
{  
...
BOOL CDgnPlateGirder_IRC22_15::Calc_PlasticResistance(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, T_PGBR_BASE& PgbrBase, double dPlasticNA, double& dMpl_Rd, BOOL bNeglectWeb)
{
	// 1. Material.
	T_MATD_D& MatdD	= MatlElem.MatdD;
	double dfy		= MatdD.Data1.Design.S_Fy1;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_ElasticResistance(ElemPairK ElemK, BOOL bPosiI, BOOL bPositive, double aForceGd[6], double aForce[6], CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi,
...
double CDgnPlateGirder_IRC22_15::Calc_kmin(BOOL bPositive, double dk_rbar, double dk_slab, double dk_top, double dk_bot)
{
...
BOOL CDgnPlateGirder_IRC22_15::Calc_NonLinearResistance(ElemPairK ElemK, BOOL bPosiI, T_PGBR_BASE& PgbrBase)
{
...
BOOL CDgnPlateGirder_IRC22_15::Calc_VerticalShearResistance(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi,
																									 ...
																									 T_CLAS_BASE& ClassD, T_PRVS_BASE& PrvsGd, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail,T_PGBR_BASE& PgbrBase, T_PGBR_DETAIL& PgbrDetail)
{
	...
	double dfy		= MatdD.Data1.Design.S_Fy1;
	double dfy1		= MatdD.Data1.Design.S_Fy2;
	double dfy2		= MatdD.Data1.Design.S_Fy3;
	double dgam_M0	= m_dGamma_M0;
    ...
    double dfyw		= Get_fy4Thick(dfy, dfy1,dfy2, dTw);
...
//   }
	...
	double dFactc = 0;
	BOOL bUseStiffBearing = FALSE;BOOL bUseStiffWeb=FALSE;
	...
	for (int i=0; i<aKeySize; ++i)
	{
		...
		if (m_pDoc->m_pAttrCtrl2->GetCgtsSsm(aKeyList[i], CgtsSSM))
		{
			...
			m_mCsgTrst.SetAt(aKeyList[i], TrstD);

			if (m_mCsgTrst.Lookup(aKeyList[i], TrstD))
			{
				dFactc = max(TrstD.TrstPos[nPosI].dPitchWeb,TrstD.TrstPos[nPosI].dPitchBearing) ;

			}


		}
//     bUseStiffBearing = CgtsSSM.aStiffener[i].bUseStifBearing;
//     bUseStiffWeb     = CgtsSSM.aStiffener[i].bUseStifWeb; 
	}
	T_SECT_K SectK = m_pDoc->m_pAttrCtrl->GetSectKey(SectD.SName);
	CSG_TRST_SECT TrstSect;
	if (m_mCsgTrst.Lookup(SectK, TrstSect))
	{
		 dFactc = max(TrstSect.TrstPos[nPosI].dPitchWeb,TrstSect.TrstPos[nPosI].dPitchBearing) ;
		 bUseStiffBearing = TrstSect.TrstPos[nPosI].bUseStifBearing;
		 bUseStiffWeb     =TrstSect.TrstPos[nPosI].bUseStifWeb;
	}
	//Check Rigid End POst
	BOOL bENdPOstStiffIEnd = FALSE, bENdPOstStiffJEnd = FALSE;
	CPG_TSES_ELEM TsesElem; TsesElem.Initialize();    
	if(Get_CpgTsesElem(ElemK, TsesElem)) 
	{
	 
		bENdPOstStiffIEnd = TsesElem.TsesPosi[nPosI].iEndpost == 0 ? TRUE:FALSE;
		bENdPOstStiffJEnd = TsesElem.TsesPosi[nPosI].iEndpost == 0 ? TRUE:FALSE;
	}

	

	BOOL bStiffProvided = bUseStiffWeb;//(bUseStiffBearing == TRUE || bUseStiffWeb == TRUE) ? TRUE :FALSE;
	BOOL bRigidEndPost  = (bENdPOstStiffIEnd == TRUE || bENdPOstStiffJEnd == TRUE) ? TRUE :FALSE;
	if (dFactc == 0.)
	{
		CString strErrMsg = _LS(IDS_DB_CPG_IRC15_NOPITCH_TRST);
		GSaveHistoryFormatNF(strErrMsg,(int)SectK,SectD.SName);		
		return FALSE;
	}
	double dRatcbyd = dFactc/m_dHw ; 
	double dHTR    = dHw/dTw_t;
	double dLimHTR = 67 * dEpsi;
	int iCaseNo = 0;
	double dkv = 0.,dTau_cre = 0.,dLambda_w = 0., dTau_b = 0.;
	if (bStiffProvided == FALSE)
	{
		if ( bRigidEndPost == FALSE)
		{
			if (dHTR < dLimHTR)  iCaseNo = 0;
			else                 iCaseNo = 1;
		}
		else                   iCaseNo = 2;
	...
	{
		//Transverse Stiffener is provided
		iCaseNo = 3;
		 if ( bRigidEndPost == FALSE) iCaseNo = 4;//End Post is not provided
	}

	if ( iCaseNo!=0)
	{
		//calculate kv
		if(dRatcbyd<1.0)	{dkv = 4.0+5.35/pow(dRatcbyd,2);}
		else				{dkv = 5.35+4.0/pow(dRatcbyd,2);}  
		if (bStiffProvided == FALSE) dkv = 5.35;
		dLimHTR = iCaseNo!=2 ? 67. * dEpsi * sqrt(dkv/5.35): 67. * dEpsi;
		
		//dLamda = dhw/(37.4*dt*dEpsi*sqrt(dk_tau));
		dTau_cre = dkv*m_dPi*m_dPi*m_dEs/(12.0*(1.0-pow(m_dPoisson,2)) * pow(dHTR,2));
		dLambda_w = sqrt((m_dfy) / (sqrt(3.0)*dTau_cre));
		if(dLambda_w<0.8)
		{
			dTau_b = dfyw/sqrt(3.0);
			
		}
		else if(0.8 <  dLambda_w + m_dZero && dLambda_w < 1.2)
		{
			dTau_b = (1.0 - 0.8*(dLambda_w-0.8))*(dfyw/sqrt(3.0));    
			
		}
		else if(dLambda_w>1.2)
		{
			dTau_b =dfyw/(dLambda_w*dLambda_w*sqrt(3.0));    
			
		}
	}
 double dVcr = dAv*dTau_b;
	//Consider PostCritical Shear Buckling using Tension Field Actions
	double dfv = 0.,dPsi = 0.,dPhi = 0.;
	double dWtf = 0., dSc =0., dMfrc = 0.;
	double dSt = 0.,dMfrt = 0.,dVtf =0.;

	T_CPGD_D CpgdD; CpgdD.Initialize();  
	if(!m_pDoc->m_pAttrCtrl2->GetDgnCpgd(CpgdD)){ASSERT(0);}
	if (CpgdD.bPBTFA4SH && bStiffProvided)
	{
		dPhi    =  1.0 / tan(1./dRatcbyd) ;
		dPsi    =  1.5 *  dTau_b * sin(2.0 *dPhi  );

		dMfrc   = 0.25 * m_dB1 * m_dTF1 * m_dTF1 * dfy * (1.0 - pow(dN_Ed / (m_dB1 * m_dTF1 * dfy / dgam_M0),2));
		dMfrt   = 0.25 * m_dB1 * m_dTF1 * m_dTF1 * dfy * (1.0 - pow(dN_Ed / (m_dB1 * m_dTF1 * dfy / dgam_M0),2));
        dSc     =  (2. / sin(dPhi) ) * sqrt(abs(dMfrc / (dfyw * dTw )));
        dSt     =  (2. / sin(dPhi) ) * sqrt(abs(dMfrt / (dfyw * dTw )));
		dWtf    =  m_dHw * cos(dPhi) + ((dFactc - dSc - dSt )* sin(dPhi)) ;
		dfv     =  sqrt((dfyw*dfyw) - (3.*dTau_b*dTau_b) + (dPsi*dPsi)) - dPsi;

		dVtf    =  (dAv*dTau_b) + (0.9 * dWtf * dTw * dfv * sin(dPhi) );
		if (dVtf>dVpl_Rd) dVtf = dVpl_Rd;
	...
	double dZp = GirderStiff.Czp;
    double dNrd = dAg * dfyw / m_dGamma_M0;

	//IRC112 Calculated Values
	T_PGBR_IRC_BASE PgbrIRCGd;PgbrIRCGd.Initialize();
	Get_CalcIRCData(ElemK,bPosiI,PrvsBase.LcomK,PgbrIRCGd);

	int iInteractionCase = 0;//For High SHear Condition
	double dBeta = 0.,dmFd = 0.,dMdv = 0.,dMdvLim = 0.;
	if (dRatio < 0.6) iInteractionCase = 0;
	else
	{
			iInteractionCase = 1;
			//calculate beta nad mfd,mdv
			dBeta = pow(((2.0 * fabs(dV_Ed)/dVpl_Rd ) - 1.),2);
			dmFd  = (PgbrDetail.dNg_bot*PgbrBase.dDistWc1	) + ((PgbrBase.dNa_top+PgbrBase.dNa_bot)*PgbrBase.dDistTc) ;
			dMdv  = ClassD.iClassSect==3 ? PgbrIRCGd.SectProp.ShortTermCompo.dsbot*m_dfy/dgam_M0:PgbrBase.dMpl_Rd - dBeta*( PgbrBase.dMpl_Rd  - dmFd) ;
			dMdvLim = (abs(PgbrBase.dMa_Ed) + abs(PgbrBase.dMc_Ed)) ; 
	}
	...
	PrvsBase.bOK = FALSE;
	int icounter = 0;
	if (PgbrIRCGd.TraWebStiff.dIys >=PgbrIRCGd.TraWebStiff.d075dtw3) icounter++;
	if (PgbrIRCGd.TraWebStiff.dFqd >=PgbrIRCGd.TraWebStiff.dFqdLim) icounter++;
	if (PgbrIRCGd.TraWebStiff.dFpsd >=PgbrIRCGd.TraWebStiff.dReaction) icounter++;
	if (PgbrIRCGd.TraWebStiff.dBuck_fcd >=PgbrIRCGd.TraWebStiff.dBuck_fcd_Lim) icounter++;

	if (icounter==4 && dRatio<=1.0)PrvsBase.bOK = TRUE;
	else PrvsBase.bOK = FALSE;

	// 7. Save Vertical Shear Check Result
	//PrvsBase.iTopClass		= iCaseNo;
	...
	PrvsBase.dVbf_Rd			= dNrd;//ResD.dVbf_Rd;
	PrvsBase.dVb_Rd			= dVb_Rd;
	PrvsBase.dV_Rd			= dV_Rd;

	PrvsBase.dEta1			= dHTR;
	PrvsBase.dEta1_lim		= dLimHTR;
	PrvsBase.dEta3			= dVpl_Rd;
	...
	PrvsBase.dAv				= dAv;        
	PrvsBase.deta				= dEta;       
	PrvsBase.dIsl_org			= dAv;   

	PrvsBase.dIsl				= dVcr;         
	PrvsBase.dXw				= dTau_cre;        
	PrvsBase.dMax_Bar_Lamda_w = dkv;
	PrvsBase.dc				= dLambda_w;
	PrvsBase.dbf				= dTau_b;         


	PrvsBase.dtf				= dRatcbyd;         
	PrvsBase.dVb_Rd_org		= dVtf;  
	PrvsBase.dVb_Rd_Lim		= ResD.ShearResD[0].dVb_Rd_Lim;  
	PrvsBase.dCombinedEffe	= ResD.ShearResD[0].dCombinedEffe;
	PrvsBase.bEta1Ratio		= iInteractionCase == 0 ? TRUE:FALSE;
	...
	PrvsBase.DetailResD[0].dbf = dfv ;
	PrvsBase.DetailResD[0].dc = dPsi ;
	PrvsBase.DetailResD[0].dIsl = dPhi ;
	PrvsBase.DetailResD[0].dIsl_org = dWtf ;
	PrvsBase.DetailResD[0].dMax_Bar_Lamda_w = dSc ;
	PrvsBase.DetailResD[0].dtf   = dMfrc;
	
	PrvsBase.DetailResD[1].dbf = dSt ;
	PrvsBase.DetailResD[1].dc = dMfrt ;

	PrvsBase.DetailResD[1].dIsl = dAg ;
	PrvsBase.DetailResD[1].dIsl_org = PgbrBase.dRatio == 0. ? dRatio:PgbrBase.dRatio  ;
	PrvsBase.DetailResD[1].dMax_Bar_Lamda_w = dBeta;
	PrvsBase.DetailResD[1].dtf = dmFd;
	PrvsBase.DetailResD[1].dVbf_Rd = dMdv;
	PrvsBase.DetailResD[1].dVbw_Rd = dMdvLim;
	...
	if(IsPrintMode())
	{
	  ...
	  for(int nWeb = 0; nWeb < nWebSize; ++nWeb)
	  {
		  ...
		  const T_VERTICAL_SHAER_RES_UNIT_D_IRC_15& Res = ResD.ShearResD[nWeb];
...
BOOL CDgnPlateGirder_IRC22_15::Get_Eta4SteelMaterial(int iMatClass, double dgam0, double dgam1, double& dEta)
{
	dEta = 0.0;
...
double CDgnPlateGirder_IRC22_15::Get_Eta4SteelGrade(int nMatClass, double dFy1)
{
...
BOOL CDgnPlateGirder_IRC22_15::Calc_LateralTorsionalBucklingResistance(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem,
															  CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi,
															  ...
															  T_PLTB_DETAIL& PltbDetail)
{
	...
	PltbBase.dMa_Ed = PgbrBase.dMa_Ed;
	///////////////////////////////////////////////////////////////////////
	...
	//Unbraced Length Data
// 
//   T_LENG_D LengD;
//   BOOL bLeng = m_pDoc->m_pAttrCtrl->GetLeng(ElemK, LengD);
...
//     LtbdElem.LtbdPosi[j].da       = CgltD.LTdata[j].da;
	...
	if (dLengthData[0]<=0.)
	{
		...
		for(int p=0; p<nSize; ++p)
		{
			...
			dLengthData[0] =dSpanLength;
			dLengthData[1] =dSpanLength;
		}
...
// 	double dLamdaLtIRC= sqrt(1.0*dIxx_g*m_dfy /dMcrIRC );
	...
	double dBetaB =  iClassSect >= 3 ? min(PgbrIRCGd.SectProp.SteelSect.dstop,PgbrIRCGd.SectProp.SteelSect.dsbot)/PgbrIRCGd.SectProp.SteelSect.dzp: 1.0;
	double dMrdCal = dBetaB * PgbrIRCGd.SectProp.SteelSect.dzp*dFyFinal;
	double dL = max(dLengthData[0],dLengthData[1]);
	double dIt = ((dB1*dTF1*dTF1*dTF1) + (dB2*dTF2*dTF2*dTF2) + (dHw*dTw*dTw*dTw))/3.0 ;
	double dBeta_f = (dB2*dTF2*dTF2*dTF2) / ((dB2*dTF2*dTF2*dTF2)+(dB1*dTF1*dTF1*dTF1) );
	//if (dB2 != dB1)
	{
        if (bPositiveM)      
		{
			//positive bending      
			dBeta_f = dTF1*pow(dB1,3) /(dTF2*pow(dB2,3)+dTF1*pow(dB1,3) ) ;
		}
		else
		{
			//Negative Bending
			dBeta_f = dTF2*pow(dB2,3) /(dTF2*pow(dB2,3)+dTF1*pow(dB1,3) ) ;
		}
	}
	double dIw = (1.0 - dBeta_f)*dBeta_f*PgbrIRCGd.SectProp.SteelSect.dIzz*pow((dHw + (0.5*dTF2) + (0.5*dTF1) ),2);
	double dMcrIRC = (dL <= 0.) ? 0. : sqrt((m_dPi*m_dPi*dEs*PgbrIRCGd.SectProp.SteelSect.dIzz/(dL*dL))*(77000.0*dIt + ((m_dPi*m_dPi*dEs*dIw/(dL*dL)))));
	double dLamdaLtIRC= (dMcrIRC <= 0.) ? 0. : sqrt(dBetaB*PgbrIRCGd.SectProp.SteelSect.dzp*dFyFinal /dMcrIRC );

	double  dAlpha_LT	= Get_ImperfectionFactor(4);  
	double dPHI_LT	= Get_PHI_LT(dAlpha_LT, dLamdaLtIRC);

	double dX_LT = Get_X_LT(dPHI_LT,dLamdaLtIRC,dX_LT_N_org);
	double dNb_Rd = dX_LT*dMpl_Rd;
	//For Pure Steel Section
	double dL_Steel =  dLengthData[bPosiI == TRUE ? 0:1] > 0. ? dLengthData[bPosiI == TRUE ? 0:1] :  m_dTotalLength;
	double dMcrIRC_Steel = (dL_Steel <= 0.) ? 0. : sqrt((m_dPi*m_dPi*dEs*PgbrIRCGd.SectProp.SteelSect.dIzz/(dL_Steel*dL_Steel))*(77000.0*dIt + ((m_dPi*m_dPi*dEs*dIw/(dL_Steel*dL_Steel)))));
	double dLamdaLtIRC_Steel= dLamdaLtIRC;//(dMcrIRC_Steel <= 0.) ? 0. :sqrt(1.0*PgbrIRCGd.SectProp.SteelSect.dzp*dFyFinal /dMcrIRC_Steel );

	double dPHI_LT_Steel	= Get_PHI_LT(dAlpha_LT, dLamdaLtIRC_Steel);

	double dX_LT_Steel = Get_X_LT(dPHI_LT_Steel,dLamdaLtIRC_Steel,dX_LT_N_org);

	double dMb_Rd = dX_LT_Steel * dMpl_Rd;
	//
	...
	{
		CString strExecuteMsg;
		strExecuteMsg.Format(_T("Check Effective Length of Lateral Torsional Buckling."));
		GSaveHistoryNF(strExecuteMsg);
	}
	...
	PltbBase.dMcr			= dMcrIRC/0.001;           
	PltbBase.dRatio		= dIt;
	PltbBase.dM_Rd		= dNb_Rd;         // M_Rd               [Unit=Moment]
	PltbBase.dcc			= dIw;           // Cd/l               [Unit=Stress]
	PltbBase.dgamma		= dMcrIRC;        //                    [Unit=None]
	PltbBase.dMu			= dLamdaLtIRC;           // V1/V2 or V1/V2     [Unit=None] 위에는 dNu라고 되어 있음.
	PltbBase.dPHI			= dPHI_LT;          //                    [Unit=None]
	PltbBase.dm1			= dX_LT;
	PltbBase.dm2			= dNb_Rd;           //                    [Unit=None]
	//For pure steel section
	PltbBase.dm_min		= dL_Steel;
	PltbBase.dm			= dMcrIRC_Steel;            //                    [Unit=None]
	PltbBase.dAwc			= dLamdaLtIRC_Steel;          //                    [Unit=Area]
	PltbBase.dAf			= dPHI_LT_Steel;           //                    [Unit=Area]
	PltbBase.dLamda_LT	= dX_LT_Steel;     //                    [Unit=None]
	PltbBase.dAlpha_LT	= dMb_Rd;     //                    [Unit=None]

	PltbBase.dPHI_LT		= dPHI_LT;       //                    [Unit=None]
	PltbBase.dX_LT_org	= dBeta_f;     //                    [Unit=None]
	PltbBase.dX_LT		= dX_LT;         //                    [Unit=None]
	PltbBase.dX_LT_N		= dX_LT_N;
	PltbBase.dk1			= dL;
	PltbBase.dk2			= dMrdCal;
	PltbBase.dks			= dCombRat;           //                    [Unit=None]
	PltbBase.dI_at			= (dTF2*pow(dB2,3) ) / 12.;         //                    [Unit=L^4]
	PltbBase.dI_afz			= (dTF1*pow(dB1,3) ) / 12.;        //                    [Unit=L^4]
	PltbBase.dkc			= dBetaB;
	PltbBase.dMa_Ed			= PgbrBase.dMa_Ed;
	if(IsPrintMode())
	{    
...
BOOL CDgnPlateGirder_IRC22_15::Calc_LateralTorsionalBucklingResistance_Neg(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem,
																																	CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi,
																																	CPG_TRST_POSI& TrstPosi, CPG_TSES_POSI& TsesPosi, CPG_LTBD_POSI& LtbdPosi, 
																																	CPG_MEMB_POSI& MembPosi, T_CLAS_BASE& ClassD, T_PLTB_BASE& PltbBase, 
																																	T_PLTB_DETAIL& PltbDetail,double dmaed)
{
	...
	PltbBase.dNb_Rd		= dNb_Rd;        
	PltbBase.dMb_Rd		= dMb_Rd;        
	PltbBase.dInterRat	= PltbBase.iSectClass < 3 ? 1.:dCombRat;   
	PltbBase.dMcr			= dMcr/0.001; //For Table Output  CUnitCtrl::ConvertUnitPltbBaseOut(T_PLTB_BASE& rData)     Mcr is multiplied twice 38479
...
double CDgnPlateGirder_IRC22_15::Get_Zj_Second_Value(double dH, double dHw, double dtw, double dB1, double dtf1, double dB2, double dtf2, 
...
double CDgnPlateGirder_IRC22_15::Get_ShearCenter_H(double dh, double dt1, double db1, double dt2, double db2)
{  
...
double CDgnPlateGirder_IRC22_15::Get_fy4Thick(double dfy, double dfy1,double dfy2, double dt)
{
	//IS2062:2011 Table 2 Clause 5,10.3,10.3.1,11.3.1,12.2,12.4
    double dfy4thick = 0.;//dt<40.0 || fabs(dt-40.0)<m_dZero ? dfy : dfy1;
	if (dt<20.) dfy4thick = dfy;
	else if (dt>40) dfy4thick = dfy2;
	else dfy4thick = dfy1;
	return dfy4thick;
}

BOOL CDgnPlateGirder_IRC22_15::Get_WidthThickRatio4Web(BOOL bStiffened, double deta, double dhw, double dtw, double dEpsi, double dk_tau, double& dCheck_lim, BOOL& bCheck)
{
...
double CDgnPlateGirder_IRC22_15::Get_Vbw_Rd(double dXw, double dfyw, double dAvw, double dGamma_M1)
{
...
double CDgnPlateGirder_IRC22_15::Get_Vbw_Rd_Limit(double dEta, double dfyw, double dAvw, double dGamma_M1)
{
	if(dGamma_M1<=m_dZero) {return 0.0;}
...
double CDgnPlateGirder_IRC22_15::Get_tau_b_Rd(double dX, double dfyf, double dGamma_M1)
{
	if(dGamma_M1<=m_dZero) {return 0.0;}
...
double CDgnPlateGirder_IRC22_15::Get_ReductonFactorOfFlange4N_Ed(double dGamma_M2, double dfy_f1, double dfy_f2, double dN_Ed, double dAf1, double dAf2)
{
...
int CDgnPlateGirder_IRC22_15::Get_Lamda_w_Type4Shear(BOOL bLongStiff, CPG_TRST_POSI& TrstPosi)
{
	int iType=0;  
...
double CDgnPlateGirder_IRC22_15::Get_Alpha4Lamda_w(double da, double dhw)
{
	double dAlpha = dhw < m_dZero ? 0.0 : da/dhw;
	return dAlpha;
}

BOOL CDgnPlateGirder_IRC22_15::IsMaterialS420orS460(double dfy)
{
	BOOL bMatOK	= FALSE;
...
double CDgnPlateGirder_IRC22_15::Get_RatioOfInteractionMV(double deta1, double dBendingRat, double deta3)
{
	double dRatio = deta1+(1.0-dBendingRat)*pow((2.0*deta3-1.0),2);
	return dRatio;
}

BOOL CDgnPlateGirder_IRC22_15::GetStressInfo(bool bPosiI, const T_SECT_D& crSectD, double aGirderForce[6], double aCptForce[6], T_SECT_STIFFNESS& GdStiffD, T_SECT_STIFFNESS& StiffD, T_STRESS_INFO_I& rData)
{
...
BOOL CDgnPlateGirder_IRC22_15::Calc_TransverseForceResistance(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem,
													 CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi,  
																										 CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi, 
...
BOOL CDgnPlateGirder_IRC22_15::Calc_LongitudinalShearResistance(ElemPairK ElemK, BOOL bPosiI, BOOL bSLS, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CPG_SCON_POSI& SconPosi, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi,
																											 CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi, CPG_TRST_POSI& TrstPosi, CPG_TSES_POSI& TsesPosi,                                 
																											 T_CLAS_BASE& ClassD, double dks, T_PGLS_BASE& PglsGd, T_PGLS_BASE& PglsBase, T_PGLS_DETAIL& PglsDetail)
{
	...
	double dHh	= m_pDgnSectUtil->Get_Size_Hh(&SectD);
	double dHt	= m_pDgnSectUtil->Get_Size_Ht(&SectD, bPosiI);
	//Here force should be taken from shear connecters VedDL and VedLL
	int i=0, j=0;
	CMapEx<T_LCOM_K, T_LCOM_K, T_LCOM_K, T_LCOM_K&> mVedDLLcomKey;
	CMapEx<T_LCOM_K, T_LCOM_K, T_LCOM_K, T_LCOM_K&> mVedLLLcomKey;
	T_LCMT_D LcmtD; LcmtD.Initialize();
	if (!m_pDoc->m_pAttrCtrl2->GetLcmt(LcmtD))
	{
		
		m_pDoc->m_pAttrCtrl2->GetLcmtbyLcomType(D_LCOMTYPE_STEEL, LcmtD);
	}  
	//IRC112 Calculated Values
	T_PGBR_IRC_BASE tmpIRCCal;tmpIRCCal.Initialize();
	Get_CalcIRCData(ElemK,bPosiI,PglsBase.LcomK,tmpIRCCal);
		// Dimension
	int nVedDLLcomSize = LcmtD.Base[0].aStrength.GetSize();//VedDL
	double dVedDL = 0;
	for (i=0; i<nVedDLLcomSize; ++i)
	{
		T_LCOM_K nStrnKey = LcmtD.Base[0].aStrength[i];    
		_DGN_FORC_CRC MembEndForce;    
		m_pForcCtrl->Get_LcomDataForDesign_LcomK(ElemK, nStrnKey, TRUE, FALSE, MembEndForce);
		double dMax = max( abs(MembEndForce.dFzz[0]),abs( MembEndForce.dFzz[1]));
		dVedDL = max(dMax,dVedDL);
			//

	}

	int nVedLLLcomSize = LcmtD.Base[0].aService.GetSize();//VedLL
	double dVedLL = 0;
	for (i=0; i<nVedLLLcomSize; ++i)
	{
		T_LCOM_K nStrnKey = LcmtD.Base[0].aService[i];
		_DGN_FORC_CRC MembEndForce;    
		m_pForcCtrl->Get_LcomDataForDesign_LcomK(ElemK, nStrnKey, TRUE, FALSE, MembEndForce);
		double dMax = max( abs(MembEndForce.dFzz[0]),abs( MembEndForce.dFzz[1]));
		dVedLL = max(dMax,dVedLL);
	}
	...
	T_CGSC_D STUDdata; STUDdata.Initialize();
	if (!m_pDoc->m_pAttrCtrl->GetCgsc(ElemK.first,STUDdata)){ASSERT(0);}
	...
	//For LongiTudinal Shear Chk
	int iCounter = 0;
	if ( tmpIRCCal.LotrShear.dVlcase1 >=  tmpIRCCal.LotrShear.dVled ) iCounter++;
	if ( tmpIRCCal.LotrShear.dVlcase2 >=  tmpIRCCal.LotrShear.dVled ) iCounter++;
	if ( tmpIRCCal.LotrShear.dVlAst >=  tmpIRCCal.LotrShear.dVl25 ) iCounter++;
	...
	PglsBase.dV_L_Ed		= tmpIRCCal.LotrShear.dVedDL;
	PglsBase.dv_L_Ed2		=   tmpIRCCal.LotrShear.dVedLL;//m_dpgls_shear;     // Slab로 계산된.    
	PglsBase.dP_Rd1		= tmpIRCCal.LotrShear.dSl1req;//dP_Rd1;
	PglsBase.dP_Rd2		= dP_Rd2;
	PglsBase.dP_Rd		= dP_Rd;         
	PglsBase.iCheck_P_Rd	= iCheck_P_Rd;
	PglsBase.dv_L_Rd		= tmpIRCCal.LotrShear.dSl2req;//dv_L_Rd;     // P_Rd*(Stud개수)/간격
	PglsBase.dv_Ed		= tmpIRCCal.LotrShear.dSLProv;//dv_Ed;       // v_Ed(Slab의..)      
	...
	PglsBase.bOK			= tmpIRCCal.LotrShear.dSLreq > 75.0 + m_dZero ? TRUE : FALSE;
	PglsBase.dAlpha		= (double)iCounter;
	PglsBase.dAzI			= dP_Rd_SLS;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_FatigueResistance(ElemPairK ElemK, BOOL bPosiI, double dt_Ld, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CPG_SCON_POSI& SconPosi, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi,
...
{  
	...
	//IRC112 Calculated Values
	T_PGBR_IRC_BASE PgbrIRCGd;PgbrIRCGd.Initialize();
	Get_CalcIRCData(ElemK,bPosiI,PgrfBase.LcomK,PgbrIRCGd);
	...
	double dParam1 = 27.0/m_dGamma_Mft;
...
// 
	...
	dF_jd	  =dCorrection_Factor*dF_fn/dGamma_mft;
	dTau_jd =dCorrection_Factor*dT_fn/dGamma_mft;
	...
	PgrfBase.dLamda_v     = PgbrIRCGd.Fatg.dfd;      // λv                 [Unit=None]
	PgrfBase.dLamda_v1    = PgbrIRCGd.Fatg.dtaufd;     // λv1                [Unit=None]
	PgrfBase.dLamda_v2    = PgbrIRCGd.Fatg.dfdlim;     // λv2                [Unit=None]
	PgrfBase.dLamda_v3    = PgbrIRCGd.Fatg.dtaulim ;     // λv3                [Unit=None]
	PgrfBase.dLamda_v4    = PgbrIRCGd.Fatg.dfdlim > 0 ? (PgbrIRCGd.Fatg.dfd/PgbrIRCGd.Fatg.dfdlim) : 0;     // λv4                [Unit=None]
	...
	PgrfBase.dRatio       = PgbrIRCGd.Fatg.dtaulim > 0 ? (PgbrIRCGd.Fatg.dtaufd/PgbrIRCGd.Fatg.dtaulim) : 0; ;        // rFf*Δτ_E,2/(Δτ_c/rMf,s)  [Unit=None]  
	PgrfBase.dv_L_Ed      = dv_L_Ed;
	PgrfBase.dFsc         = dFsc;
	PgrfBase.dAsc         = dA_sc;
	PgrfBase.bOK			    = (PgrfBase.dRatio < 1.0 || PgrfBase.dLamda_v4 < 1.0) ? FALSE : TRUE;

	return TRUE;
}

BOOL CDgnPlateGirder_IRC22_15::Calc_StressCheck4SLS(ElemPairK ElemK, BOOL bPosiI, BOOL bIsGenSect, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi,
																					 const T_GENL_DATA& GLineData, const T_GLINE_DGN_D& GLineDgn, double dk1, double dk2, double dk3, T_PGBS_BASE& PgbsBase, T_PGBS_DETAIL& PgbsDetail)
{
	...
	if (dMaxStress == DBL_MAX || dMaxStress == DBL_MIN) dMaxStress = 0.;
	if (dMaxStressRat == DBL_MAX || dMaxStressRat == DBL_MIN) dMaxStress = 0.;
	...
	BOOL bChkSlabStress = dMaxSlabStressRat > 1.0 ? FALSE : TRUE;
	 for(int i=0; i<4; i++) {PgbsBase.ChkSlab.aForce[i] = aSlabStress4[i];}
...
BOOL CDgnPlateGirder_IRC22_15::Calc_LongitudinalShearResistance4SLS(ElemPairK ElemK, BOOL bPosiI, BOOL bIsGenSect, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CPG_SCON_POSI& SconPosi, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi,
																													 CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi, CPG_TRST_POSI& TrstPosi, CPG_TSES_POSI& TsesPosi,                                 
																													 T_CLAS_BASE& ClassD, double dks, T_PLSS_BASE& PlssGd, T_PLSS_BASE& PlssBase, T_PLSS_DETAIL& PlssDetail)
{
	...
	T_PGLS_DETAIL PglsDetail;PglsDetail.Initialize();
	...
	//IRC112 Calculated Values
	T_PGBR_IRC_BASE PgbrIRCGd;PgbrIRCGd.Initialize();
	Get_CalcIRCData(ElemK,bPosiI,PlssBase.LcomK,PgbrIRCGd);

	// Modify by GAY. PMS:4216. ('12.05.18). 데이터 할당 누락부분 수정.
	PlssBase.iSectClass	= PgbrIRCGd.Fatg.nCrackClass ; //DEFLCW
	...
	PlssBase.dP_Rd1		= PgbrIRCGd.LotrShear.dVR     ;
	PlssBase.dP_Rd2		= PgbrIRCGd.Fatg.dWk     ;//DEFLCW
	PlssBase.dP_Rd		= PgbrIRCGd.Fatg.dWkmax      ;//DEFLCW
	//PlssBase.dP_Rd_SLS	= PglsDetail.dP_Rd_SLS;
	PlssBase.iCheck_P_Rd	= PgbrIRCGd.Fatg.iDeflCtrlLCNum;//DEFLCW
	PlssBase.dv_L_Rd		= PgbrIRCGd.Fatg.dDeltaDL    ;  //DEFLCW     
	PlssBase.dv_Ed		= PgbrIRCGd.LotrShear.dSl1prov      ;
	PlssBase.dv_Ed_lim	= PgbrIRCGd.LotrShear.dSrreq  ;
	PlssBase.dRatio		= fabs(PglsBase.dv_L_Rd) < m_dZero? fabs(PglsBase.dv_L_Ed/m_dZero): fabs(PglsBase.dv_L_Ed/PglsBase.dv_L_Rd);  
	PlssBase.dAzI			= PgbrIRCGd.Fatg.dDeltaDLLimit; // //DEFLCW  
	PlssBase.bOK			= fabs(PglsBase.dv_Ed_lim) <  fabs(PglsBase.dv_Ed) ? FALSE : TRUE;

//   PlssBase.strFeqLcom = PgbrIRCGd.Fatg.iDeflCtrlLCNum;
//   PlssBase.dDeltaDlll    =  PgbrIRCGd.Fatg.dDeltaDL;
//   PlssBase.dDeltaDlll_lim= PgbrIRCGd.Fatg.dDeltaDLLimit;
// 
//   if (PgbrIRCGd.Fatg.nCrackClass == 0)      PlssBase.strCrackLcom =_T("Moderate") ;
//   else if (PgbrIRCGd.Fatg.nCrackClass == 1) PlssBase.strCrackLcom =_T("Severe")      ;
//   else if (PgbrIRCGd.Fatg.nCrackClass == 2) PlssBase.strCrackLcom =_T("Very Severe") ;
//   else                                      PlssBase.strCrackLcom =_T("Extreme")     ;
//   PlssBase.dWk           = PgbrIRCGd.Fatg.dWk;
//   PlssBase.dWkMax        =  PgbrIRCGd.Fatg.dWkmax;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_BendingResistance4GenSect(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CPG_TRST_POSI& TrstPosi,
...
BOOL CDgnPlateGirder_IRC22_15::Calc_VerticalShearResistance4GenSect(ElemPairK ElemK, BOOL bPosiI, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi, CPG_TRST_POSI& TrstPosi, CPG_TSES_POSI& TsesPosi,
																													 T_CLAS_BASE& ClassD, const T_GENL_DATA& GLineData, const T_GLINE_DGN_D& GLineDgn, T_PRVS_BASE& PrvsGd, T_PRVS_BASE& PrvsBase, T_PRVS_DETAIL& PrvsDetail)
{
	...
	double dfy3		= MatdD.Data1.Design.S_Fy3;
	double dfu  = MatdD.Data1.Design.S_Fu;
	double dEs  = MatdD.Data1.Analysis.Elast;
	 // Concrete
	double dfck  = MatlElem.dfck;
...
//   Get_Eta4SteelMaterial(iMatClass, dgam_M0, dgam_M1, dEta);
	...
	BOOL bAfter = TRUE;
    Get_EffectiveSectData(bPosiI, bAfter, dfy1, dfy2,dfy3, da, aForce, SectElem, GLineData, DLineInfo, EffGLineD, EffGLineDgn);    
	...
	for(i=0; i<iPartSize; i++)
	{
        ...
        double dfy_p = Get_fy4Thick(dfy1, dfy2,dfy3, dt_p);
		...
		for(j=0; j<iInternalSize; j++)
		{
			...
			for(k=0; k<iSubPanelSize; k++)
			{
                ...
                double dfy_si = Get_fy4Thick(dfy1, dfy2,dfy3, dt_si);
            ...
            double dfy_i = Get_fy4Thick(dfy1, dfy2,dfy3, dt_i);
...
BOOL CDgnPlateGirder_IRC22_15::Calc_LongitudinalShearResistance4GenSect(ElemPairK ElemK, BOOL bPosiI, BOOL bSLS, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CPG_SCON_POSI& SconPosi, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi,
																															 CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi, CPG_TRST_POSI& TrstPosi, CPG_TSES_POSI& TsesPosi,                                 
																															 T_CLAS_BASE& ClassD, double dks, T_PGLS_BASE& PglsGd, T_PGLS_BASE& PglsBase, T_PGLS_DETAIL& PglsDetail)
{
	// Material.
...
BOOL CDgnPlateGirder_IRC22_15::Get_PlasticMoment4GenSect(BOOL bPosiI, BOOL bPositive, CPG_MATL_ELEM& MatlElem, T_SECT_SECTBASE_D& SectBaseD, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, const T_GENL_DATA& GLineData, const T_GLINE_DGN_D& GLineDgn, double dPlasticNA, double& dMpl_Rd, T_PGBR_DETAIL& PgbrDetail)
{
	// Material.
	T_MATD_D MatdD = MatlElem.MatdD;
	 // Steel.
	...
	Get_PlasticMomentOfDividedPart(dfy1, dfy2,dfy3, dCzm_NAz, PolyD, aTopLine, dMpl_gt, dNg_top);
	Get_PlasticMomentOfDividedPart(dfy1, dfy2,dfy3, dCzm_NAz, PolyD, aBotLine, dMpl_gb, dNg_bot);
	if(aHCutLine.GetSize()>0)
        Get_PlasticMomentOfHoriDividedLine(dfy1, dfy2,dfy3, dCzm_NAz, PolyD, aHCutLine, aHCutTopThick, dMpl_lgt, dMpl_lgb, dNg_ltop, dNg_lbot);
...
BOOL CDgnPlateGirder_IRC22_15::Get_Mf_Rd_PlasticMoment4GenSect(BOOL bPosiI, BOOL bPositive, CPG_MATL_ELEM& MatlElem, T_SECT_SECTBASE_D& SectBaseD, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, const T_GENL_DATA& GLineData, T_GLINE_DGN_D& GLineDgn, double& dMf_Rd)
{
	int i=0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_ElasticResistance4GenSect(ElemPairK ElemK, BOOL bPosiI, BOOL bPositive, double aForceGd[6], double aForce[6], CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CPG_TRST_POSI& TrstPosi,
																										const T_GENL_DATA& GLineData, const T_GLINE_DGN_D& GLineDgn, T_PGBR_BASE& PgbrBase, T_PGBR_DETAIL& PgbrDetail)
{
	...
	{
        Get_EffectiveSectData(bPosiI, bAfter, dfy1, dfy2,dfy3, da, dForce2, SectElem, GLineData, DLineInfo, EffGLineD, EffGLineDgn);      
...
double CDgnPlateGirder_IRC22_15::Get_Mel_RdOfElastResistance(double dfy, double dFx, double dMy, double dA, double dIy, double dz)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectivePropertyForClass4(ElemPairK ElemK, BOOL bPosiI, BOOL bPositive, const T_SECT_SECTBASE_D& SectBase, T_GENL_DATA& EffGLineD, T_SECT_STIFFNESS& GirderEffStiff, T_SECT_STIFFNESS& CompoEffStiff)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_PlasticMomentOfDividedPart(double dfy1, double dfy2,double dfy3, double dCzm_NAz, const T_GSEC_POLYGON& PolyD, const CArray<T_GSEC_LINE, T_GSEC_LINE&>& aLineData, double& dMpl, double& dNgir)
{
	dMpl=0.0;
	int i=0;
...
//  CSectUtil::Get_MaxMinPoint4LineType(TempPolyD, aTempLineData, dOutMaxPt, dOutMinPt);
	...
	for(i=0; i<iLineSize; i++)
	{
		...
		double dAl = dbi*dt;    
        double dfy = Get_fy4Thick(dfy1, dfy2,dfy3, dt);
...
BOOL CDgnPlateGirder_IRC22_15::Get_PlasticMomentOfHoriDividedLine(double dfy1, double dfy2,double dfy3, double dCzm_NAz, const T_GSEC_POLYGON& PolyD, const CArray<T_GSEC_LINE, T_GSEC_LINE&>& aLineData, const CArray<double, double&>& aCutTopThick,
...
double CDgnPlateGirder_IRC22_15::Get_Lamda3_DesignLifeInYears(int iYears) 
{
	double dLamda=0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_NonDimensionalSlenderness(double dL, double db, double dfy, double dEs, double dm, double dAwc, double dAf, double& dLamda)
{
...
double CDgnPlateGirder_IRC22_15::Get_PHIform(double dM1, double dM2, double dNu)
{
	double dMrat = 0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_m_Value(double dNu, double dPHI, double dgamma, double& dm1, double& dm2, double& dm_min, double& dm)
{
...
double CDgnPlateGirder_IRC22_15::Get_PHI_LT(double dAlpha, double dLamda_LT)
{
	double dPHI_LT = 0.5*(1.0+dAlpha*(dLamda_LT-0.2)+pow(dLamda_LT,2));
	return dPHI_LT;
}

double CDgnPlateGirder_IRC22_15::Get_X_LT(double dPHI_LT, double dLamda_LT, double& dX_LT_org)
{
	double dX_LT = 0.0;
...
int CDgnPlateGirder_IRC22_15::Get_BucklingCurveForCrossSection(double dh, double db, BOOL bIsSectI/* = TRUE*/)
{
	int iType;
...
double CDgnPlateGirder_IRC22_15::Get_ImperfectionFactor(int iCase)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_IslOfLongiStiff(int iType, double dh, double dt, double dtw, double dEpsi, double& dIsl)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_IslOfLongiStiff(const CPG_LSTI_DATA& InD, double dtw, double dEpsi, double& dIsl)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_IslOfLongStfn4GS(double dEpsi, T_GLINE_STIFFENER& StfnD, double dtw, double& dIsl)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_SlendernessParamLamda(int iType, int iLongStiffSize, double dhw, double da, double dt, double dIsl, double dEpsi, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_ShearBucklingCoeff(int iLongStiffSize, double dhw, double da, double dt, double dIsl, double& dk_tau, double& dk_tau_st, double& dk_tau_st_Limit)
{ 
...
BOOL CDgnPlateGirder_IRC22_15::Get_ContributionFromWebFactor(int iEndpostType, double dEta, double dLamda, double& dXw, int& iLamda_w_scope,double dfyw)
{
	//IRC22 Pg.21
	...
	else if(iEndpostType==1 || iEndpostType==2) // Non-rigid end post
	{
		if(dLamda<0.8)
		{
			dXw = dfyw/sqrt(3.0);
...
BOOL CDgnPlateGirder_IRC22_15::Get_bf4c_DB(bool bPosiI, T_SECT_D& SectD, bool bTopF, int nWebPart, double dEpsilon, double& dbf)
{
	dbf = 0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_ClassifyClassSection(BOOL bPosiI, double aGirderForce[6], double aCptForce[6], CPG_MATL_ELEM& MatlElem, 
											   ...
											   T_DLINE_CALC_D& DLineCalcD, T_CLAS_BASE& ClassD, ElemPairK ElemK)
{
	...
	{    
        ...
        Calc_ClassifyClassSection4GenSect(bChkRestrained, dfy, dfy1,dfy3, aGirderForce, aCptForce, dPlasticNA, GirderStiff, SectStiffD, PureSect, GLineData, GLineDgn, DLineCalcD);
	...
	else
	{
		...
		T_CLASS_IN_D_IRC_15 ClassInD;ClassInD.Initialize();
        ...
        Get_PlasticNA_inclSectclass(bPosiI, bPositiveMy,dL, MatlElem, SectD, arRbarPosi, dPlasticNA,ClassD.iClassSect);
		SetPlasticNA(dPlasticNA);
...
//   if (dMy < m_dZero)
//   {
//     //Negative Moment consider only Bottom and Web
// //     ClassD.iTopFlgClass	= iClassFlg_T;
// //     ClassD.iBotFlgClass	= iClassFlg_B;
// //     ClassD.iFlangeClass = iClassFlg;
// //     ClassD.iWebClass	= iClassWeb;
//     ClassD.iClassSect	= max(ClassD.iBotFlgClass,ClassD.iWebClass);
//   }
//   else
//   {
//     ClassD.iClassSect	= max(ClassD.iTopFlgClass,ClassD.iWebClass);
//   }
	return TRUE;
}

BOOL CDgnPlateGirder_IRC22_15::Get_SectionClass(bool bPosiI, BOOL bChkRestrained, const T_SECT_D& SectD, const T_CLASS_IN_D_IRC_15& crClassInD, 
									   const T_MATL_DESIGN& DgnMatD, T_CLAS_BASE& ClassD)
{
	...
	{
		iClass = 0;
		const T_CLASS_IN_UNIT_D_IRC_15& ClassInD = crClassInD.aTopFlangeD[nTop];
		CLASS_BASE_UNIT_IRC_15 ClassResD;
	...
	{
		iClass = 0;
		const T_CLASS_IN_UNIT_D_IRC_15& ClassInD = crClassInD.aBotFlangeD[nBot];
		CLASS_BASE_UNIT_IRC_15 ClassResD;
		if(ClassInD.nType == 0)		{Calc_OutstandClassUnit(ClassInD, dfy, dfy1, iClass, ClassResD);}
		else if(ClassInD.nType == 1){Calc_InternalPlateClassUnit(bPosiI, SectD, ClassInD, dfy, dfy1, iClass, ClassResD,false);}
		else {ASSERT(0); return FALSE;}
	...
	{
		iClass = 0;
		const T_CLASS_IN_UNIT_D_IRC_15& ClassInD = crClassInD.aWebD[nWeb];
		CLASS_BASE_UNIT_IRC_15 ClassResD;
...
double CDgnPlateGirder_IRC22_15::interpolate(double x1, double x2, double p1, double p2, double x)
{
	return ( p1 + (p2-p1)*(x-x1)/(x2-x1) ) ;
}

BOOL CDgnPlateGirder_IRC22_15::Get_PlasticNA(BOOL bPosiI, BOOL bPositive, double dL,CPG_MATL_ELEM& MatlElem, const T_SECT_D& crSectD, 
...
// 	}
	...
	// 7. Calc. Plastic Bending Resistance
	double dfyd = min(dfyd_w,min(dfyd_b,dfyd_t));

	double dConcF	=    dfcd* dBc*dtc;
    double dStopfF	= dB1*dTf1*dfyd;
    double dSbotfF	= dB2*dTf2*dfyd;
    double dSwebF	= dH*dTw_t*dfyd;
	...
	for(int i=0; i < arRbarPosi.GetSize(); i++)
	{
		_DGN_RBAR_CRC Rbar = arRbarPosi[i];
		double dAsi = Rbar.dArea;
		dConcF_neg += dAsi*dfsd;
	}
    
	double dStopfF_neg = dfyd * dB1*dTf1;
	double dSbotfF_neg = dB2*dTf2*dfyd;
	double dSwebF_neg	 = dh*dTw_t*dfyd;
	...
	double dIRCConcF	= 0.67*dfck*dBeffFact*dBc*dIRCxu/m_dGamma_C;
	double dIRCStopfF	= dB1*dTf1*dfy/1.1;
	double dIRCSbotfF	= dB2*dTf2*dfy/1.1;
	double dIRCSwebF	= bNeglectWeb ? 0.0 : (dh*dTw_t*dfy/1.1);
	double dGirderF = bPositive==TRUE ? dStopfF + dSbotfF + dSwebF : dStopfF_neg + dSbotfF_neg + dSwebF_neg;  
...
BOOL CDgnPlateGirder_IRC22_15::Get_PlasticNA4GenSect(BOOL bPosiI, BOOL bPositive, CPG_MATL_ELEM& MatlElem, T_SECT_SECTBASE_D& SectBaseD, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, 
																						const T_GENL_DATA& GLineData, const T_GLINE_DGN_D& GLineDgn, double& dPlasticNA)
{
	...
	double dfy  = MatlElem.MatdD.Data1.Design.S_Fy1;
	double dfy1 = MatlElem.MatdD.Data1.Design.S_Fy2;
	double dfy3 = MatlElem.MatdD.Data1.Design.S_Fy3;
	double dfck = MatlElem.dfck;
	double dfsk = MatlElem.MatdD.MainRebarData.B_fy;
	 double dfcd  = 0.67* dfck/dgam_c;
	double dfsd = dgam_sr==0.0 ? dfsk : dfsk/dgam_sr;
	...
	do 
	{
		...
		if(bPositive)
		{
			...
			dFtc = 0.0;
            Get_ForceLineGS(dfy, dfy1,dfy3, PolyD, aTopLine, dFcg); // dFcg = dfyd * dAg_top;
            Get_ForceLineGS(dfy, dfy1,dfy3, PolyD, aBotLine, dFtg); // dFtg = dfyd * dAg_bot;
            Get_TopBotForceLineGS(dfy, dfy1,dfy3, PolyD, aHCutLine, aHCutTopThick, dFcg_cl, dFtg_cl);
			dFtr = dfsd * dAr_bot;
		}
		else
		{
			...
			dFtc = 0.0;
            Get_ForceLineGS(dfy, dfy1,dfy3, PolyD, aBotLine, dFcg); // dFcg = dfyd * dAg_bot;
            Get_ForceLineGS(dfy, dfy1,dfy3, PolyD, aTopLine, dFtg); // dFtg = dfyd * dAg_top; 
            Get_TopBotForceLineGS(dfy, dfy1,dfy3, PolyD, aHCutLine, aHCutTopThick, dFtg_cl, dFcg_cl);
			dFtr = dAr_top * dfsd;
...
BOOL CDgnPlateGirder_IRC22_15::Get_PlasticNAEff(BOOL bPosiI, BOOL bPositive, CPG_MATL_ELEM& MatlElem, const T_SECT_D& crSectD, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveSectionProperty(BOOL bPosiI, double dMy, CPG_MATL_ELEM& MatlElem, CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, T_CLAS_BASE& ClassD, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveSectionProperty4DB(BOOL bPosiI, double* aForce, double da, CPG_MATL_ELEM& MatlElem, 
													  CPG_SECT_ELEM& SectElem, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, 
													  ...
													  BOOL bCompoStiffData/*=FALSE*/)
{
    ...
    Get_EffeSectPropBeforeDB(bPosiI, dfy1, dfy2,dfy3, aForce, SectD, da, GirderStiff, arLstiPosi, GirderEffeStiffD, RhoC_Before);
    ...
    Get_EffeSectPropAfterDB(bPosiI, dfy1, dfy2, dfy3,aForce, da, MatlElem, SectElem, arRbarPosi, arLstiPosi, CompoEffeStiffD, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffeSectPropBeforeDB(bool bPosiI, double dfy1, double dfy2,double dfy3, double* aForce, T_SECT_D& SectD, double da, 
											   ...
											   T_SECT_STIFFNESS& EffeStiff, T_DGNLINE_LBD_BR& RhoC)
{
	...
	double dtf2	= m_pDgnSectUtil->Get_Size_tfb(&SectD, bPosiI);
	double dh	= m_pDgnSectUtil->Get_Size_hw(&SectD, bPosiI);
	double dfyt	= Get_fy4Thick(dfy1, dfy2,dfy3, dtf1);
	double dfyb	= Get_fy4Thick(dfy1, dfy2,dfy3, dtf2);
	double dfyw	= Get_fy4Thick(dfy1, dfy2,dfy3, dtw);
	double dAg	= GirderStiff.Area;
    ...
    Get_LineDgnInfo4SectDB(bPosiI, dfy1, dfy2,dfy3, dEs, dPoisson, aForce, SectD, da, GirderStiff, arLstiPosi, aLineDgnBase, LineLBD); 
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffeSectPropAfterDB(BOOL bPosiI, double dfy1, double dfy2,double dfy3, double* aForce, double da, 
											  ...
											  T_DGNLINE_LBD_BR& RhoC, BOOL bCompoStiffData/*=FALSE*/)
{
    ...
    Get_EffeSectPropBeforeDB(bPosiI, dfy1, dfy2,dfy3, aForce, SectD, da, CompoStiff, arLstiPosi, CompoGiderEffeStiffD, RhoC);
...
BOOL CDgnPlateGirder_IRC22_15::Get_StiffnessOfEffeSectCompo(T_SECT_INFO_D& SectInfoD, T_SECT_STIFFNESS& EffeStiff)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveCzmData4Sect(bool bPosiI, BOOL bPositive, T_SECT_D& SectD, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_LineDgnInfo4SectDB(bool bPosiI, double dfy1, double dfy2,double dfy3, double dEs, double dPoisson, double* aForce, T_SECT_D& SectD, 
																						 ...
																						 CArray<T_GLINE_DGN_B,T_GLINE_DGN_B&>& aLineDgnBase, T_DGNLINE_LBD& LineLBD)
{
	...
	for(int i=0; i < iLineDgnSize; i++)
	{
		...
		dbs = LineDgn.db;
		dts = LineDgn.dt;
        double dfyi = Get_fy4Thick(dfy1, dfy2,dfy3, dts);
...
BOOL CDgnPlateGirder_IRC22_15::Get_LineInfo4SectDB(bool bPosiI, T_SECT_D& SectD, T_SECT_STIFFNESS& AppStiffD, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi, 
										  CArray<T_GLINE_DGN_B,T_GLINE_DGN_B&>& aLineDgnBase)
{
	double dH	= m_pDgnSectUtil->Get_Size_H(&SectD, bPosiI);
	double dtw	= m_pDgnSectUtil->Get_Size_tw(&SectD, CDgnSectUtil::EN_WEB_THK_BOTH, bPosiI);
...
double CDgnPlateGirder_IRC22_15::Get_InertiaOfStiffener(double db, double dt)
{
	double dIy = dt*pow(db,3)/12.0;
	return dIy;
}

BOOL CDgnPlateGirder_IRC22_15::Get_StressOfPosition(double aGirderForce[6], double aCptForce[6], T_SECT_STIFFNESS& GdStiffD, T_SECT_STIFFNESS& StiffD, double aStress[4])
{
	double dArea_a = GdStiffD.Area;
	double dIy_a   = GdStiffD.Ryy;
	double dIz_a   = GdStiffD.Rzz;
...
BOOL CDgnPlateGirder_IRC22_15::Get_ConvertPoint4Centroid(double dCym, double dCzm, CArray<T_GLINE_DGN_B,T_GLINE_DGN_B&>& aLineDgnBase)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_InternalWebClassClassification(bool bPosiI, int iBotClass, double aForce[6], double dStress[4], CPG_MATL_ELEM& MatlElem, 
...
BOOL CDgnPlateGirder_IRC22_15::Calc_InternalPlateClassUnit(bool bPosiI, const T_SECT_D& SectD, const T_CLASS_IN_UNIT_D_IRC_15& ClassInD, double dfy, double dfy1,
													int& iClass, CLASS_BASE_UNIT_IRC_15& ClassBase, bool bWeb)
{
	// Initialize 
	iClass = 0;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_OutstandClassUnit(const T_CLASS_IN_UNIT_D_IRC_15& ClassInD, double dfy, double dfy1, 
											 int& iClass,CLASS_BASE_UNIT_IRC_15& ClassBase)
{
	ClassBase.Initialize();
	iClass = 0;
	...
	if (dStressInner > pow(10,100) || dStressOuter > pow(10,100)) return FALSE;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_ClassifyClassSection4GenSect(BOOL bChkRestrained, double dfy1, double dfy2, double dfy3,double* aGirderForce, double* dForce, double dPlasticNA, T_SECT_STIFFNESS& GirStiffD, T_SECT_STIFFNESS& CptStiffD, T_SECT_STIFFNESS& OrgStiffD,
																												const T_GENL_DATA& GLineData, const T_GLINE_DGN_D& GLineDgn, T_DLINE_CALC_D& DLineCalcD)
{
	...
	if(bChkRestrained)
	{
		...
		{    
			...
			int iTempBotClass=1;
            Calc_PartLineClassSection4GenSect(dfy1, dfy2, dfy3,iTempBotClass, aGirderForce, dForce, dPlasticNA, GirStiffD, CptStiffD, OrgStiffD, GLineData, PartDgnD, DLinePartD);
	...
	else
	{
		...
		{    
            ...
            Calc_PartLineClassSection4GenSect(dfy1, dfy2, dfy3, iFlangeBotClass, aGirderForce, dForce, dPlasticNA, GirStiffD, CptStiffD, OrgStiffD, GLineData, PartDgnD, DLinePartD);
			DLineCalcD.aPartLine.SetAt(i, DLinePartD);
			int iPartClass=DLinePartD.iClass;
			iWebCount++;
			if(iWebCount==1) iWebClass = iPartClass;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_PartLineClassSection4GenSect(double dfy1, double dfy2,double dfy3, int iBotClass, double* aGirderForce, double* dForce, double dPlasticNA, T_SECT_STIFFNESS& GirStiffD, T_SECT_STIFFNESS& CptStiffD, T_SECT_STIFFNESS& OrgStiffD, const T_GENL_DATA& GLineData, const T_GLINE_PART_DGN& PartDgnD,
																												T_DLINE_PART_D& DLinePartD)
{
	int i=0;
	...
	for(i=0; i<iOutstandSize; i++)
	{
		...
		int iLineClass=0;
        Calc_SubLineClassSection4GenSect(nPartType, dfy1, dfy2,dfy3, iBotClass, aGirderForce, dForce, dPlasticNA, GirStiffD, CptStiffD, OrgStiffD, GLineData, LineDstD, DLineLbd, iLineClass);
...
BOOL CDgnPlateGirder_IRC22_15::Calc_SubLineClassSection4GenSect(int nPartType, double dfy1, double dfy2, double dfy3,int iBotClass, double* aGirderForce, double* dForce, double dPlasticNA, T_SECT_STIFFNESS& GirStiffD, T_SECT_STIFFNESS& CptStiffD, T_SECT_STIFFNESS& OrgStiffD, const T_GENL_DATA& GLineData, const T_GLINE_DST& LineDst, T_DGNLINE_LBD& DLineLbd, int& iClass)
{
	iClass=0;
	int i=0;
	double dFx_a=aGirderForce[0];
    ...
    double dfy = Get_fy4Thick(dfy1, dfy2,dfy3, dt_t);
...
BOOL CDgnPlateGirder_IRC22_15::Is_Sig1IsFree4Outstand(BOOL bIs1_Sig1)
{
	return bIs1_Sig1 ? FALSE : TRUE; // Outstand일 경우 0번째에 자유단이 아닌 다른 Line과 연결된 vertex가 있다. 
}

double CDgnPlateGirder_IRC22_15::Get_InternalBucklingfactor(double dPsi)
{  
...
double CDgnPlateGirder_IRC22_15::Get_OutstandBucklingfactor(int iType, double dPsi)
{
	double dk_sig=0.0;
	if(iType==1)
	{
...
int CDgnPlateGirder_IRC22_15::Get_WebPlaticClass(double dct, double Eps, double dAlpha)
{
	int iClass = 0;
...
int CDgnPlateGirder_IRC22_15::Get_WebElasticClass(double dAlpha, double dct, double Eps, double dPsi)
{
	int iClass = 0;
...
int CDgnPlateGirder_IRC22_15::Get_PartClass(int iStreType, double dct, double dEpsi, double dAlpha, double dk)
{
	int iClass=1;
	double dLimit1 = 0.0;
	double dLimit2 = 0.0;
...
int CDgnPlateGirder_IRC22_15::Get_InternalPartClass(BOOL bChkPlastic, int iStreType, double dct, double dEpsi, double dAlpha, double dPsi)
{
	int iClass=0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_ReductionFactorRho(BOOL bInternal, double dBar_b, double dt, double dLamda_p, double dPsi, double& dRho)
{
	dRho = 1.0;
...
double CDgnPlateGirder_IRC22_15::Get_Lamda_p(double db, double dt, double dEpsilon, double dk_sig)
{
...
double CDgnPlateGirder_IRC22_15::Get_Epsilon(double dfy)
{
	if(dfy<=0.0)
	{
		ASSERT(0);
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveSectData(BOOL bPosiI, BOOL bAfter, double dfy1, double dfy2,double dfy3, double da, double* dForce, CPG_SECT_ELEM& SectElem, const T_GENL_DATA& GLineData, T_DLINE_CALC_D& DLineInfo, T_GENL_DATA& EffGLineD, T_GLINE_DGN_D& EffGLineDgn)
{
	double dFx = dForce[0];
	double dMy = dForce[4];
	double dMz = dForce[5];
    ...
    Get_CalcSectInfo4EffeSect(dfy1, dfy2,dfy3, da, dForce, dCzm_gap, aStiff, GLineData, GLineDgn, DLineInfo);
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveSectInfo4PartLine(double* aForce, double dCzm_gap, double dStiff[3], const T_GENL_DATA& GLineData, const T_GLINE_PART_DGN& PartDgnD, const T_DLINE_PART_D& DLinePart, T_GENL_DATA& EffGLineD)
{
	int i=0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveSectInfo4PlateElements(int iClass, double* aForce, double dCzm_gap, double dStiff[3], const T_GENL_DATA& GLineData, T_GLINE_DST& GLineDST, T_DGNLINE_LBD& LineLBD, T_GENL_DATA& EffGLineD)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_ConvertVertexId(int vs, int v1, int v2, int& vc1, int& vc2)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_ConvertSectStiffenerD(const T_DGN_STL_STIFFENER_PART& InD, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectivePanelInformation(int iClass, BOOL bInternal, int iStressType, double* aForce, double dCzm_gap, double dStiff[3], const T_GENL_DATA& GLineData, const T_GLINE_BASE& LineBaseD, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveSect4PartLine(double dfy1, double dfy2,double dfy3, double* dForce, double dArea, double dIy, double dIz, const T_GENL_DATA& GLineData, const T_GLINE_PART_DGN& PartDgnD, 
																								 T_GENL_DATA& EffGLineD, T_GLINE_PART_DGN& EffPartDgnD)
{
	...
	for(i=0; i<iOutstandSize; i++)
	{
		...
		T_GLINE_DST LineDstD = PartDgnD.aOutstand[i];
        Get_EffectiveSection4StiffenedPlateElements(dfy1, dfy2,dfy3, dForce, dArea, dIy, dIz, GLineData, LineDstD, EffGLineD, EffLineDst);
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveWidth4Internal(double dPsi, double db, double dRho, double& dbeff, double& dbe1, double& dbe2)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveWidth4Outstand(int nType, double dPsi, double dc, double dRho, double& dbeff)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_CalcSectInfo4EffeSect(double dfy1, double dfy2,double dfy3, double da, double* dForce, double dCzm_gap, double dStiff[3], const T_GENL_DATA& GLineData, const T_GLINE_DGN_D& GLineDgn, T_DLINE_CALC_D& DlineInfo)
{
	T_GSEC_POLYGON GsecPolyD = GLineData.PolyD;
	CArray<T_GSEC_LINE, T_GSEC_LINE&> aLine;
	aLine.Copy(GLineData.aLine);
	...
	for(i=0; i<iPartSize; i++)
	{
		...
		T_GLINE_PART_DGN PartDgnD = aPartLine[i];
        Get_CalcSectPartLine4EffeSect(dfy1, dfy2,dfy3, m_dEs, m_dPoisson, da, dForce, dCzm_gap, dStiff, GLineData, PartDgnD, DLinePart);
...
BOOL CDgnPlateGirder_IRC22_15::Get_CalcSectPartLine4EffeSect(double dfy1, double dfy2,double dfy3, double dE, double dPoisson, double da, double* aForce, double dCzm_gap, double dStiff[3], const T_GENL_DATA& GLineData, const T_GLINE_PART_DGN& PartDgnD, T_DLINE_PART_D& DLinePart)
{
	int i=0, j=0, k=0;
	...
	for(i=0; i<iOutstandSize; i++)
	{
        ...
        Get_LocalBucklingDataOfSubPanel(dfy1, dfy2,dfy3, dE, dPoisson, da, aForce, dCzm_gap, dStiff, GLineData, LineDstD, DLineLbd);
	...
	for(i=0; i<iInternalSize; i++)
	{
        ...
        Get_LocalBucklingDataOfSubPanel(dfy1, dfy2,dfy3, dE, dPoisson, da, aForce, dCzm_gap, dStiff, GLineData, LineDstD, DLineLbd);
        double dfyi=Get_fy4Thick(dfy1, dfy2,dfy3, LineDstD.dt);
		...
		for(j=0; j<nStfnSizeOfInt; j++)
		{
			...
			{        
                ...
                Get_LocalBucklingDataOfSubPanel(dfy1, dfy2,dfy3, dE, dPoisson, da, aForce, dCzm_gap, dStiff, GLineData, StfnLineDst, StfnDLineLbd);
...
BOOL CDgnPlateGirder_IRC22_15::Get_ElasticCriticalPlateBucklingData(double dfy, double da, double dE, double dPoisson, double* aForce, T_DGNLINE_LBD& DLineLbd)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_ConvertCompPanelDgnLineLBD(const T_DGNLINE_LBD& DLineLbd, T_DGNLINE_LBD& CompDLineLbd)
{
	int iLineStressType = DLineLbd.iLineStressType;
	int iPanelSize = DLineLbd.aDgnBaseD.GetSize();
...
BOOL CDgnPlateGirder_IRC22_15::Get_ColumnBucklingData(double dfy, double da, double dE, double dPoisson, BOOL bIsOpenStfn, double* aForce,
											 T_DGNLINE_LBD& DLineLbd)
{
	int iPanelSize = DLineLbd.aDgnBaseD.GetSize();
...
BOOL CDgnPlateGirder_IRC22_15::Get_Sigma_cr_p_4Stiffener(double da, double dE, double dPoisson, double* aForce, const T_DGNLINE_LBD& CompDLineLbd,
												T_DGNLINE_LBD& DLineLbd)
{
	int iCompPanelSize= DLineLbd.iCompPanelSize;
...
BOOL CDgnPlateGirder_IRC22_15::Get_Sigma_cr_p_4MultifleStiffener(double da, double dE, double dPoisson, double* aForce, T_DGNLINE_LBD& DLineLbd)
{
	int i=0;    
	double dFx = aForce[0];
	double dMy = aForce[4];
...
double CDgnPlateGirder_IRC22_15::Get_Sigma_E(double dEs, double db, double dt, double dPoisson)
{
	double dPi = m_dPI;
...
double CDgnPlateGirder_IRC22_15::Get_k_sigma_p(double dalpha, double dgamma, double dPsi, double ddelta, double& dalpha_lim)
{
...
double CDgnPlateGirder_IRC22_15::Get_Ip_4BendingOfPlate(double db, double dt, double dPoisson)
{
...
double CDgnPlateGirder_IRC22_15::Get_Width4StiffenedPlates(int iNtype, double db, double dPsi, BOOL bSup)
{
	double dwidth=0.0;
...
int CDgnPlateGirder_IRC22_15::Get_StreeTypeOfStiffenedPlates(double df1, double df2, int& iStrType2)
{
	int iType=-1;
...
BOOL CDgnPlateGirder_IRC22_15::Get_AreaAndEffArea4Line(const T_DGNLINE_LBD& DgnLineLBD, double& dAc, double& dAc_eff_loc)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_MomentofAreaOfFlange(int iChkPartIx, double dCzm_gap, T_SECT_STIFFNESS& StiffD, const T_GLINE_DGN_D& GLineDgn, double& dQflg)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_Nc_fofSlab(BOOL bPoisI, double dfcd, double dXpl, double dHt, T_SECT_SECTBASE_D& SectBaseD, const T_GLINE_DGN_D& GLineDgn, double& dNc_f)
{
...
int CDgnPlateGirder_IRC22_15::Get_StiffenerSize(const T_GLINE_DST& GLineDST)
{
	int iStfnSize=0;
...
double CDgnPlateGirder_IRC22_15::Get_Lamda_p4PlateTypeBehaviour(double dBeta_A_c, double dfy, double dSigma_cr_p)
{
...
double CDgnPlateGirder_IRC22_15::Get_Sigma_cr_c4ColumnTypeBehaviour(double dEs, double dt, double dnu, double da)
{
...
double CDgnPlateGirder_IRC22_15::Get_Sigma_cr_sl4ColumnTypeBehaviour(double dEs, double dI_sl, double dA_sl, double da)
{
...
double CDgnPlateGirder_IRC22_15::Get_Sigma_cr_sl4SimplifiedModel(double dE, double dPoisson, double dt, double db, double db1, double db2, 
...
double CDgnPlateGirder_IRC22_15::Get_xi(double dSigma_cr_p, double dSigma_cr_c)
{
...
double CDgnPlateGirder_IRC22_15::Get_Rho_c_FinalReductionFactor(double dRho, double dXc, double dxi)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectiveSection4StiffenedPlateElements(double dfy1, double dfy2,double dfy3, double* dForce, double dArea, double dIy, double dIz, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_EffectivePanelInfo(BOOL bInternal, int iStressType, BOOL b1stIdx, int ve, double* dForce, double dStiff[3], double dbe1, double dbe2, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_LocalBucklingDataOfSubPanel(double dfy1, double dfy2,double dfy3, double dE, double dPoisson, double da, double* dForce, double dCzm_gap, double dStiff[3], const T_GENL_DATA& GLineData, 
																											const T_GLINE_DST& LineDst, T_DGNLINE_LBD& LineLBD)
{
	...
	for(i=0; i<iLineDgnSize; i++)
	{
		...
		double dApi = dbi*dti;
		dAp_sum += dApi;
        double dfyi = Get_fy4Thick(dfy1, dfy2,dfy3, dti);
		double dEpsilon = Get_Epsilon(dfyi);
		db_t += dbi;
		if(i==0) dt_t = dti;
	...
	if(bInternal && iSubPanelSize>1)
	{
		...
		for(i=0; i<iSubPanelSize-1; i++)
		{
            ...
            double dfyi = Get_fy4Thick(dfy1, dfy2, dfy3,dt_t);
			double dEpsilon = Get_Epsilon(dfyi);
...
void CDgnPlateGirder_IRC22_15::Set_CalcSubPanel(BOOL bGrossWidth, int iNtype, double dbb, double dti, T_SUB_PANEL_BASE& rData)
{
	if(bGrossWidth)
	{
		if(iNtype==Stiffened_Plate_inf) 
...
int CDgnPlateGirder_IRC22_15::Get_StressType4EffSect(BOOL bCompStr, BOOL bDbCurve, BOOL bIs1_Sig1)
{
	int iStrType=-1;
	if(bCompStr)
	{
...
BOOL CDgnPlateGirder_IRC22_15::Get_CompWidthOfDoubleCurvatrue(T_GLINE_DGN_B& LineDgnB, double& dbcc)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_CompWidthOfDoubleCurvatrue(double* dForce, double dCzm_gap, double dArea, double dIy, double dIz, const T_GENL_DATA& GLineData, const T_GLINE_BASE& LineBaseD, double& dbc)
{
	// b1stIdx : 첫번째 Index부터냐...
	int i=0;
	int iLineSize = LineBaseD.aLineIx.GetSize();  
...
int CDgnPlateGirder_IRC22_15::Get_ConnectedVertexId(int v1, int v2, int v3, int v4)
{
	int vcn=-1;
...
BOOL CDgnPlateGirder_IRC22_15::Get_PointCoor4beff(double dbe, double dx1, double dy1, double dx2, double dy2, double& dx, double& dy)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_CompLengthOfPlastic(BOOL bPositive, double dy1, double dz1, double dy2, double dz2, double& dLen_c, double& dLen_t)
{
	double dc=0.0, dt=0.0;
	double dLen = CMathFunc::mathLength(dy1, dz1, dy2, dz2);
	if(dz1*dz2<0.0) // 
...
double CDgnPlateGirder_IRC22_15::Get_Psi(double dSig1, double dSig2)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_Psi(double dStr1, double dStr2, double& dSig1, double& dSig2, double& dPsi)
{
	BOOL bIs1_Sig1=FALSE;
	Get_f1_f2(dStr1, dStr2, dSig1, dSig2, bIs1_Sig1);
	dPsi = Get_Psi(dSig1, dSig2);
...
BOOL CDgnPlateGirder_IRC22_15::Get_GLineDgnPartData(const T_GLINE_DGN_D& GLineDgnD, CArray<T_GLINE_PART_DGN,T_GLINE_PART_DGN&>& aTopFlangePart, CArray<T_GLINE_PART_DGN,T_GLINE_PART_DGN&>& aBotFlangePart, CArray<T_GLINE_PART_DGN,T_GLINE_PART_DGN&>& aWebPart)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_DLinePartData(const T_DLINE_CALC_D& DLineInfo, CArray<T_DLINE_PART_D,T_DLINE_PART_D&>& aTopFlangePart, CArray<T_DLINE_PART_D,T_DLINE_PART_D&>& aBotFlangePart, CArray<T_DLINE_PART_D,T_DLINE_PART_D&>& aWebPart)
{
	aTopFlangePart.RemoveAll();
	aBotFlangePart.RemoveAll();
	aWebPart.RemoveAll();
...
BOOL CDgnPlateGirder_IRC22_15::Get_CompositeGeneralSectionInfo(BOOL bPosiI, const T_SECT_D& SectD, double& dZbar_sb, double& dCzm_sb, double& dCzp_sb, double& dHt, double& dArea_sb, double& dBc_sb, double* aYcr, double* aZcr)
{
	BOOL bTapered = m_pDgnSectUtil->Is_TaperedSect(&SectD);
	T_SECT_SECTBASE_D SectBaseD = bTapered && !bPosiI ? SectD.SectBefore.SectJ : SectD.SectBefore.SectI;
...
double CDgnPlateGirder_IRC22_15::Get_Height4Gen(BOOL bPosiI)
{
	double dHt = bPosiI ? m_dHt[0] : m_dHt[1];
	return dHt;
}

double CDgnPlateGirder_IRC22_15::Get_ZcentroidofSlab(BOOL bPosiI)
{
	double dZbar_slab = bPosiI ? m_dZbar_sb[0] : m_dZbar_sb[1];
	return dZbar_slab;
}
double CDgnPlateGirder_IRC22_15::Get_CzmofSlab(BOOL bPosiI)
{
	double dCzm_slab = bPosiI ? m_dCzm_sb[0] : m_dCzm_sb[1];
	return dCzm_slab;
}
double CDgnPlateGirder_IRC22_15::Get_CzpofSlab(BOOL bPosiI)
{
	double dCzp_slab = bPosiI ? m_dCzp_sb[0] : m_dCzp_sb[1];
	return dCzp_slab;
}
double CDgnPlateGirder_IRC22_15::Get_AreaofSlab(BOOL bPosiI)
{
	double dArea_slab = bPosiI ? m_dArea_sb[0] : m_dArea_sb[1];
	return dArea_slab;
}
double CDgnPlateGirder_IRC22_15::Get_BcofSlab(BOOL bPosiI)
{
...
void CDgnPlateGirder_IRC22_15::Set_LineDgnBase(int iLineType, double db, double dt, double dy1, double dz1, double dy2, double dz2, double df1, double df2,
...
BOOL CDgnPlateGirder_IRC22_15::Calc_Vb_Rd(bool bPosiI, CPG_MATL_ELEM& MatlElem, T_SECT_D& SectD,  CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi,  
								 CPG_TRST_POSI& TrstPosi, CPG_TSES_POSI& TsesPosi, T_PRVS_BASE& PrvsBase, T_CLAS_BASE& ClassD, 
								 double& dVb_Rd, T_VERTICAL_SHAER_RES_D_IRC_15& ResD)
{
	// 1. Initailize
	dVb_Rd = 0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_Vbw_Rd(bool bPosiI, CPG_MATL_ELEM& MatlElem, T_SECT_D& SectD, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi,  
...
BOOL CDgnPlateGirder_IRC22_15::Calc_Vbf_Rd(bool bPosiI,T_MATD_D& MatdD, T_SECT_D& SectD, T_PRVS_BASE& PrvsBase, T_CLAS_BASE& ClassD,
											double& dVbf_Rd, T_VERTICAL_SHAER_RES_D_IRC_15& ResD)
{
	// 1. Initailize
	dVbf_Rd = 0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Calc_Lambda_w(bool bPosiI, CPG_MATL_ELEM& MatdElemD, T_SECT_D& SectD, CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&>& arLstiPosi,  
									CPG_TRST_POSI& TrstPosi, CPG_TSES_POSI& TsesPosi, double& dLambda_w, 
									T_VERTICAL_SHAER_RES_UNIT_D_IRC_15& ResD)
{
	// 1. Initailize
	dLambda_w	= 0.0;
	...
	double dfy3				= MatdElemD.MatdD.Data1.Design.S_Fy3;
...
void CDgnPlateGirder_IRC22_15::Set_Calc_Vbw_Param(double dAlpha, double dk_tau, double dk_tau_st, double dk_tau_st_limit, double dIsl, double dhw, double dt, 
...
void CDgnPlateGirder_IRC22_15::Set_Calc_Vbw_Param2(int iLamda_w_scope, double dXw, double dVbw_Rd, double dV_Rd, double dVb_Rd_lim, double dV_Edi, double deta3, T_PRVS_Vbw_Rd_Param& rData)
{
...
void CDgnPlateGirder_IRC22_15::Set_Calc_Vbw_SubPanel_Param(int nLongStfnSize, double dhw, double dt, double dAlpha, double dk_tau, double dk_tau_st, double dk_tau_st_lt, double dIsl, double dLamda_w, T_PRVS_Vbw_Rd_Sub_Panel& rData)
{
...
void CDgnPlateGirder_IRC22_15::Set_Calc_FlangeShearstress_Param(double dtau_Ed, double dtau_b_Rd, double dV_Edi, double dQi, double dIyyi, double dtf, T_PRVS_Vbw_Rd_Param& rData)
{
...
void CDgnPlateGirder_IRC22_15::SetPgbrBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, double* aForceGd, double* aForce, T_PGBR_BASE& rPgbrGd, T_PGBR_BASE& rData, T_PGBR_IRC_BASE& rPgbrIRCGd, T_PGBR_IRC_BASE& rDataIRC)
{
	...
	rPgbrIRCGd.bChk     = rDataIRC.bChk     = bChk;
	rPgbrIRCGd.ElemK    = rDataIRC.ElemK    = ElemK.first;
	rPgbrIRCGd.nPosi    = rDataIRC.nPosi    = nPosi;
	rPgbrIRCGd.LcomK	  = rDataIRC.LcomK	   = iDgnLcomNo;
	rPgbrIRCGd.OrgLcomK = rDataIRC.OrgLcomK = iOrgLcomNo;		    
	rPgbrIRCGd.iMax	    = rDataIRC.iMax	   = iMaxMinType;


	//IRC Data

	rPgbrIRCGd.SectProp.dGamma_cBS     = m_dGamma_C     ;
	rPgbrIRCGd.SectProp.dGamma_c_acci  = m_dGamma_C_Acc ;
	rPgbrIRCGd.SectProp.dGamma_M0YB    = m_dGamma_M0     ;
	rPgbrIRCGd.SectProp.dGamma_M1Ult   = m_dGamma_M1    ;
	rPgbrIRCGd.SectProp.dGamma_s       = m_dGamma_S    ;
	rPgbrIRCGd.SectProp.dGamma_v       = m_dGamma_V     ;
	rPgbrIRCGd.SectProp.dGamma_fft     = m_dGamma_Fft   ;
	rPgbrIRCGd.SectProp.dGamma_Mft     = m_dGamma_Mft   ;


	rPgbrIRCGd.SectProp.dBc = m_dBc;
	rPgbrIRCGd.SectProp.dtc= m_dtc	;
	rPgbrIRCGd.SectProp.dHh= m_dHh	;
	rPgbrIRCGd.SectProp.ddc = m_ddc;
	rPgbrIRCGd.SectProp.dfck = m_dfck;
	rPgbrIRCGd.SectProp.dEc = m_dEc ;
	rPgbrIRCGd.SectProp.dfyk= m_dfyk;

	
	rPgbrIRCGd.SectProp.dEs = m_dEs;

	//Reinforcement Details at report stage

	//Design Strength
	double deta=0.0,dlambda=0.0;
	CalEtaLambda(m_dfck,deta,dlambda);
	rPgbrIRCGd.SectProp.dEta = dlambda;
	rPgbrIRCGd.SectProp.dLambda = deta;
	rPgbrIRCGd.SectProp.dfcds_BS = 0.67*m_dfck / rPgbrIRCGd.SectProp.dGamma_cBS;
	rPgbrIRCGd.SectProp.dfcds_Acci = 0.67*m_dfck / rPgbrIRCGd.SectProp.dGamma_c_acci;
	rPgbrIRCGd.SectProp.dfyds_steel = rPgbrIRCGd.SectProp.dfyk / m_dGamma_S;

	//Girder Properties at report stage
	//Design Strength at report stage
...
// void CDgnPlateGirder_IRC22_15::SetPgIRCData(BOOL bChk, T_ELEM_K ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, double* aForceGd, double* aForce, T_PGBR_BASE& rPgbrGd, T_PGBR_BASE& rData,T_PG_IRC22_15& rIRCdata)
// {
//   rPgbrGd.bChk     = rData.bChk     = bChk;
//   rPgbrGd.ElemK    = rData.ElemK    = ElemK;
//   rPgbrGd.nPosi    = rData.nPosi    = nPosi;
//   rPgbrGd.LcomK	   = rData.LcomK	   = iDgnLcomNo;
//   rPgbrGd.OrgLcomK = rData.OrgLcomK = iOrgLcomNo;		    
//   rPgbrGd.iMax	   = rData.iMax	   = iMaxMinType;
//   
//   rIRCdata.SectProp.dBc = m_dBc;
// 
// 
// }

BOOL CDgnPlateGirder_IRC22_15::SetRhocBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, const T_PGBR_DETAIL& PgbrDetail, T_RHOC_BASE& Rho_C)
{
...
void CDgnPlateGirder_IRC22_15::SetPrvsBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, double* aForceGd, double* aForce, T_PRVS_BASE& rPrvsGd, T_PRVS_BASE& rData)
{
...
void CDgnPlateGirder_IRC22_15::SetPltbBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, double* aForceGd, double* aForce,
...
void CDgnPlateGirder_IRC22_15::SetPrtfBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, double* aForce, T_PRTF_BASE& rData)
{
	rData.bChk = bChk;
...
void CDgnPlateGirder_IRC22_15::SetPglsBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, double* aForceGd, double* aForce, T_PGLS_BASE& rPglsGd, T_PGLS_BASE& rData)
{
	rData.bChk = bChk;
	rData.ElemK = ElemK.first;
	rData.nPosi = nPosi;
...
void CDgnPlateGirder_IRC22_15::SetPgfrBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, double* aForce, T_PGRF_BASE& rData)
{
	rData.bChk = bChk;
	rData.ElemK = ElemK.first;
	rData.nPosi = nPosi;
...
void CDgnPlateGirder_IRC22_15::SetPgbsBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, int iLcomType,
...
void CDgnPlateGirder_IRC22_15::SetPlssBaseData(BOOL bChk, ElemPairK ElemK, int nPosi, int iDgnLcomNo, int iOrgLcomNo, int iMaxMinType, int iLcomType, double* aForceGd, double* aForce, T_PLSS_BASE& rPlssGd, T_PLSS_BASE& rData)
{
	rData.bChk = bChk;
	rData.ElemK = ElemK.first;
	rData.nPosi = nPosi;
...
BOOL CDgnPlateGirder_IRC22_15::Get_MaterialData(ElemPairK ElemK, double& dfy, double& dfy1, double& dEs, double& dfck, double& dEcm, double& dfsk, double& dErs)
{
	CPG_MATL_ELEM MatlElem;  
	if(!Get_CpgMatlElem(ElemK, MatlElem)) {return FALSE;}
...
BOOL CDgnPlateGirder_IRC22_15::Get_LineEndStress4LineDgnBase(const CArray<T_GLINE_DGN_B,T_GLINE_DGN_B&>& aDgnBaseD, double& dStr1, double& dStr2)
{
...
BOOL CDgnPlateGirder_IRC22_15::Get_ForceLineGS(double dfy1, double dfy2,double dfy3, const T_GSEC_POLYGON& PolyD, const CArray<T_GSEC_LINE, T_GSEC_LINE&>& aLine, double& dTotalForce)
{
	dTotalForce = 0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_TopBotForceLineGS(double dfy1, double dfy2,double dfy3, const T_GSEC_POLYGON& PolyD, const CArray<T_GSEC_LINE, T_GSEC_LINE&>& aLine, const CArray<double, double&>& aTopThick, 
...
BOOL CDgnPlateGirder_IRC22_15::Get_PartLineId(int nPartId, const T_GLINE_DGN_D& GLineDgnD, CArray<int,int&>& aWebLineId)
{
	int i=0,j=0,k=0;
	int iPartLineSize = GLineDgnD.aPartLine.GetSize();  
	for(i=0; i<iPartLineSize; i++)
...
double CDgnPlateGirder_IRC22_15::Get_ShearStress(double dV, double dQ, double dIyy, double dbw)
{
	double dtau = dIyy*dbw==0.0 ? 0.0 : fabs(dV)*dQ/(dIyy*dbw);
	return dtau;
}

double CDgnPlateGirder_IRC22_15::Get_Q_FirstMomentOfGirderCentroid(double* aSize, double dZcen_g, double dZcen_c)
{
	double dQ=0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_ThicknessOfWeb4GenSect(const T_GLINE_DGN_D& GLineDgn, double& dbw,  double& dAweb)
{
	dbw=0.0, dAweb=0.0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_ThicknessOfFlange4GenSect(const T_GLINE_DGN_D& GLineDgn, double& dt_top, double& dt_bot)
{
	dt_top = dt_bot = 0.0;
	int iPartSize = GLineDgn.aPartLine.GetSize();  
...
BOOL CDgnPlateGirder_IRC22_15::IsValueZero(double dValue)
{
	BOOL bIsZero=FALSE;
...
BOOL CDgnPlateGirder_IRC22_15::Get_SectPropOfDLinePart(CArray<T_DLINE_PART_D,T_DLINE_PART_D&>& aDLinePart, double& dArea, double& dt)
{
	int i=0, j=0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_SectPropOfLineLBD(CArray<T_DGNLINE_LBD,T_DGNLINE_LBD&>& aDLineLBD, double& dArea, double& dt)
{
	int i=0;
...
void CDgnPlateGirder_IRC22_15::GetPureSectStiffD(const CPG_SECT_ELEM& SectElemD, BOOL bPosI, T_SECT_STIFFNESS& rData)
{
	if(bPosI)	{rData = SectElemD.SectPosi[0].PureSect;}
	else		{rData = SectElemD.SectPosi[1].PureSect;}
}

void CDgnPlateGirder_IRC22_15::GetNonComSectStiffD(const CPG_SECT_ELEM& SectElemD, BOOL bPosI, T_SECT_STIFFNESS& rData)
{
	if(bPosI)	{rData = SectElemD.SectPosi[0].NonConSect;}
	else		{rData = SectElemD.SectPosi[1].NonConSect;}
}

void CDgnPlateGirder_IRC22_15::GetRebarSectStiffD(const CPG_SECT_ELEM& SectElemD, BOOL bPosI, T_SECT_STIFFNESS& rData)
{
	if(bPosI)	{rData = SectElemD.SectPosi[0].RebarSect;}
	else		{rData = SectElemD.SectPosi[1].RebarSect;}
}

double CDgnPlateGirder_IRC22_15::GetPlasticNA()
{
	return m_dPlasticNA;
}

void CDgnPlateGirder_IRC22_15::SetPlasticNA(const double& dPlasticNA)
{
	m_dPlasticNA = dPlasticNA;
}

void CDgnPlateGirder_IRC22_15::Convert_VerbShearResD(const T_VERTICAL_SHAER_RES_UNIT_D_IRC_15& InD, T_PRVS_BASE_UNIT& rData)
{
...
double CDgnPlateGirder_IRC22_15::GetOutStandFlgLineLengD(BOOL bTop, BOOL bLeft, bool bPosiI, const T_SECT_D& crSectD)
{
...
BOOL CDgnPlateGirder_IRC22_15::GetCentFlgLinePosD(bool bPosiI, BOOL bTop, BOOL bLeft, const T_SECT_D& crSectD, const T_SECT_STIFFNESS& crStiffD, 
...
BOOL CDgnPlateGirder_IRC22_15::GetCentWebLinePosD(bool bPosiI, BOOL bLeft, const T_SECT_D& crSectD, const T_SECT_STIFFNESS& crStiffD, 
											double& dy1, double& dz1, double& dy2, double& dz2)
{
	dy1 = dy2 = dz1 = dz2 = 0.0;
	CDgnSectUtil::EN_WEB_THK enWebThk = bLeft ? CDgnSectUtil::EN_WEB_THK_LEFT : CDgnSectUtil::EN_WEB_THK_RIGHT;
...
// Detail Print
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
BOOL CDgnPlateGirder_IRC22_15::Execute_Detail_SteelPlateGirder(ArrElemPairKey& arElemK, BOOL* bPgdChk)
{
...
BOOL CDgnPlateGirder_IRC22_15::Detail_Header(ElemPairK ElemK)
{
...
BOOL CDgnPlateGirder_IRC22_15::Detail_SectionProperty_Material(ElemPairK ElemK, BOOL bPosiI)
{
	rptwofstream fout(m_strTestDgnFile, ios::app);
...
BOOL CDgnPlateGirder_IRC22_15::Detail_BendingResistance(ElemPairK ElemK, BOOL bPosiI)
{
	rptwofstream fout(m_strTestDgnFile, ios::app);
...
BOOL CDgnPlateGirder_IRC22_15::Detail_VerticalShearResistance(ElemPairK ElemK, BOOL bPosiI)
{
	rptwofstream fout(m_strTestDgnFile, ios::app);
...
BOOL CDgnPlateGirder_IRC22_15::Detail_LateralTorsionalBucklingResistance(ElemPairK ElemK, BOOL bPosiI)
{
	rptwofstream fout(m_strTestDgnFile, ios::app);
...
BOOL CDgnPlateGirder_IRC22_15::Detail_TransverseForceResistance(ElemPairK ElemK, BOOL bPosiI)
{
	rptwofstream fout(m_strTestDgnFile, ios::app);
...
BOOL CDgnPlateGirder_IRC22_15::Detail_LongitudinalShearResistance(ElemPairK ElemK, BOOL bPosiI)
{
	rptwofstream fout(m_strTestDgnFile, ios::app);
...
BOOL CDgnPlateGirder_IRC22_15::Detail_FatigueResistance(ElemPairK ElemK, BOOL bPosiI)
{
	rptwofstream fout(m_strTestDgnFile, ios::app);
...
CString CDgnPlateGirder_IRC22_15::Get_String_Format(double dValue, int iNum)
{
	CString strFormat;
...
void CDgnPlateGirder_IRC22_15::Print_EffectivePolyAndLineData(T_GSEC_POLYGON& PolyD, CArray<T_GSEC_LINE, T_GSEC_LINE&>& aEffLine)
{
...
int CDgnPlateGirder_IRC22_15::Get_SubPannelCalcType(int nLongStfnSize)
{
...
void CDgnPlateGirder_IRC22_15::Calc_r1r2(double& dr1, double& dr2)
{
	//PROFILING(_T("CDgnPlateGirder_IRC22_15::Calc_r1r2"));
...
void CDgnPlateGirder_IRC22_15::Calc_StressWeb_H(double& dStrTop, double& dStrBot)
{
	//PROFILING(_T("CDgnPlateGirder_IRC22_15::Calc_StressWeb_H"));
...
double CDgnPlateGirder_IRC22_15::Calc_HTR()  // for web
{
	// Get HTR
	double dHTR=0.0;

	if(m_iType==0)			dHTR = (m_dH-m_dtf1)/ m_dTw_t;				  	// L.
	else if(m_iType==1)	dHTR = (m_dH-m_dtf1-m_dtf2)/ m_dTw_t;		// C.
	else if(m_iType==2)	dHTR = (m_dH-m_dtf1-m_dtf2)/ m_dTw_t;		// H.
	else if(m_iType==3)	dHTR = (m_dH-m_dtf1)/ m_dTw_t;				  	// T.
	else if(m_iType==4)	dHTR = (m_dH-m_dtf1-m_dtf2)/ m_dTw_t;		// B.
	else if(m_iType==5)	dHTR = m_dH/m_dB1;										// P.
	else if(m_iType==6)	dHTR = 1.0;												  	// SR.
	else if(m_iType==7)	dHTR = m_dH/m_dB1;										// SB.
	else if(m_iType==8)	dHTR = (m_dH-m_dtf1)/(2.*m_dTw_t);			// 2L.
	else if(m_iType==9)	dHTR = (m_dH-2.*m_dtf1)/(2.*m_dTw_t);	  // 2C.
	else if(m_iType==45)dHTR = (m_dH-m_dtf1-m_dtf2)/ m_dTw_t;		// HC.
	else ASSERT(0);

	return dHTR;
}

double CDgnPlateGirder_IRC22_15::Calc_BTR()  // for flenge
{
	// Get BTR
	double dBTR=0.0;
...
double CDgnPlateGirder_IRC22_15::Calc_Class_General()
{
	double dBeta2f,  dBeta2w,  dBeta3f,  dBeta3w;
	//PROFILING(_T("CDgnPlateGirder_IRC22_15::Calc_Class_General"));
	// C, H, B, SB, 2C
	// IS:800-2007 Table 2. p.18
...
double CDgnPlateGirder_IRC22_15::Calc_Class_General(int &iClassF,int &iClassW)
{
	double dBeta2f,  dBeta2w,  dBeta3f,  dBeta3w;
	//PROFILING(_T("CDgnPlateGirder_IRC22_15::Calc_Class_General"));
	// C, H, B, SB, 2C
...
double CDgnPlateGirder_IRC22_15::Calc_Class_General(int &iClassF,int &iClassW,double &dfy,double &dBTR_calg)
{
	double dBeta2f,  dBeta2w,  dBeta3f,  dBeta3w;
	//PROFILING(_T("CDgnPlateGirder_IRC22_15::Calc_Class_General"));
	// C, H, B, SB, 2C
...
double CDgnPlateGirder_IRC22_15::Calc_Class_P_SR()
{
...
double CDgnPlateGirder_IRC22_15::Calc_BTR_HC1()  // for Flange
{
	// Get BTR
	double dBTR=0.0;
	// Same Get_BTR() at CACSCode_EuroCode3.
...
double CDgnPlateGirder_IRC22_15::Calc_BTR_HC2()  // for Flange
{
	// Get BTR
	double dBTR=0.0;
	// Same Get_BTR() at CACSCode_EuroCode3.
...
double CDgnPlateGirder_IRC22_15::Calc_BTR_HC3()  // for Flange
{
	// Get BTR
	double dBTR=0.0;
	// Same Get_BTR() at CACSCode_EuroCode3.
...
void CDgnPlateGirder_IRC22_15::Get_StaticForceMoment(int ElemNo, int LcaseNo, T_LCOM_D_UL& LcomUlData) 
{
	//////////////////////////////////////////////
...
BOOL CDgnPlateGirder_IRC22_15::SetSectionPropData(ElemPairK ElemK,const T_SECT_D SectD,CPG_SECT_ELEM& SectElem,T_MATD_D& MatdD)
{
	//Slab Properties
	BOOL bPosiI = TRUE;
	T_SECT_STIFFNESS NonConSect;	NonConSect.Initialize();
	...
	m_dH		= m_pDgnSectUtil->Get_Size_H(&SectD, bPosiI);  
	m_dB1	= m_pDgnSectUtil->Get_Size_bft(&SectD, bPosiI);
	m_dTw	= m_pDgnSectUtil->Get_Size_tw(&SectD, CDgnSectUtil::EN_WEB_THK_LEFT, bPosiI);
	m_dTw_t	= m_pDgnSectUtil->Get_Size_tw(&SectD, CDgnSectUtil::EN_WEB_THK_BOTH, bPosiI);
	m_dTF1	= m_pDgnSectUtil->Get_Size_tft(&SectD, bPosiI);
	m_dB2	= m_pDgnSectUtil->Get_Size_bfb(&SectD, bPosiI);
	m_dTF2	= m_pDgnSectUtil->Get_Size_tfb(&SectD, bPosiI);
	m_dhw	= m_pDgnSectUtil->Get_Size_hw(&SectD, bPosiI);
	m_dBc	= m_pDgnSectUtil->Get_Size_Bc(&SectD); 
	m_dtc	= m_pDgnSectUtil->Get_Size_tc(&SectD); 
	m_dHh	= m_pDgnSectUtil->Get_Size_Hh(&SectD); 
	m_dHt_sectProp	= m_pDgnSectUtil->Get_Size_Ht(&SectD, bPosiI);
	m_ddc  = GirderStiff.Czp + m_dHh + 0.5*m_dtc;
	
	 m_ddc1  = GirderStiff.Czm ;
	 m_dZp = m_dB1*m_dTF1*(m_dH-m_dTF1) + 0.25*m_dTw*(m_dH-2*m_dTF1)*(m_dH-2*m_dTF1);
	//for elastic section zpe
	double dpart1= m_dTF1+(((0.5*GirderStiff.Area) - (m_dB1*m_dTF1))/m_dTw_t);
	double dytop = ( (m_dB1*m_dTF1) >(0.5*GirderStiff.Area))? dpart1:(dpart1-0.5*m_dTF1);
	double dyweb  = (m_dB1*m_dTF1 + (m_dTw_t*m_dhw) > (0.5*GirderStiff.Area))? (dpart1-m_dTF1-0.5*m_dhw):0;
	double dybot  = 0;
	BOOL bfact1 = (m_dB1*m_dTF1)>(0.5*GirderStiff.Area) ?TRUE:FALSE;
	BOOL bfact2 = (((m_dB1*m_dTF1) + (m_dTw_t*m_dhw))<(0.5*GirderStiff.Area))? TRUE:FALSE;
	if (bfact1 && bfact2) dybot = dpart1-m_dTF1-m_dTF2 ;
	else dybot = dpart1-m_dhw-m_dTF1-0.5*m_dTF2 ;
	double dWebfact = dyweb>0? ((m_dTw_t*dyweb*dyweb/4.0)+(m_dTw_t*(m_dhw-dyweb)*(m_dhw-dyweb)/4.0)) :m_dTw_t*m_dhw;
	double dzpe = dWebfact+ (m_dB1*m_dTF1*abs(dytop) + m_dB2*m_dTF2*abs(dybot));

	m_dfck		= MatdD.Data2.Design.C_fc;
	double deta=0.0,dlambda=0.0;
	CalEtaLambda(m_dfck,deta,dlambda);
	m_dfcds_bs = 0.67*deta*dlambda*m_dfck/m_dGamma_C;
	m_dfcds_acc = 0.67*deta*dlambda*m_dfck/m_dGamma_C_Acc;
	m_dfyd_steel = m_dfyk/m_dGamma_S;
	//m_dEc = MatD.Da ;
	//m_dFyk;
//   double dfyt = MatdD.Data1.DesignTopFlange.S_Fy1;//dTF1 < 40.0 + m_dZero ? dfy : dfy1;
//   double dfyb = MatdD.Data1.DesignBotFlange.S_Fy1; //dTF2 < 40.0 + m_dZero ? dfy : dfy1;
...
BOOL CDgnPlateGirder_IRC22_15::CalEtaLambda(double dfck,double& dEta,double& dLambda)
{
	dEta = 0.0;
	if      (dfck < 60.0+cDGN_Zero)  dEta = 0.80;
	else if (dfck < 110.0+cDGN_Zero) dEta = 0.80 - (dfck-60.0)/500.0;
...
BOOL CDgnPlateGirder_IRC22_15::Get_SavedResult4Design(ElemPairK ElemK, BOOL bPosiI, const T_PGBR_BASE& PgbrBase, T_PGBR_IRC_BASE& rData)
																																				 
{
	...
	ArrElemPairKey aElemPairK;
	m_pDataCtrl->GetPgdElemKeyList(aElemPairK);
	m_pDataCtrl->GetElemKeyList4ElemPair(EN_EL_BEAM, aElemPairK, arCheckElemK);
	int iTotalElement = 1;//arCheckElemK.GetSize();

	int iDgnStatus = pDoc->GetDesignStatus();
	pDoc->SetDesignStatus(6);
	if (m_PgbrBase.dPlasticNA == 0. && m_PgbrBase.iSectClass == 0) m_PgbrBase = PgbrBase;
	T_CPGD_D CpgdD; CpgdD.Initialize();  
	if(!pDoc->m_pAttrCtrl2->GetDgnCpgd(CpgdD)){ASSERT(0);}
	BOOL bCheckStrength[6]={0};
	BOOL bCheckService[2]={0};
	...
	for(int i=0; i<iTotalElement; i++)
	{
		...
		T_EWSF_D EwsfD;EwsfD.Initialize();//Effective width calculation
		BOOL bEwsfOK =  m_pDoc->m_pAttrCtrl->GetEwsfElem(ElemK.first, EwsfD);
		if (!bEwsfOK) iEffWidCase =5;
		
		//Set Section Data
		SetSectionPropData(ElemK,SectElem.SectD,SectElem,MatlElem.MatdD);
		m_dBeffLength = m_dBc * EwsfD.EwsfSubY.dZtopFactor;
		...
		int iMaxMin=0, iDgnLcomNo = 0, iOrgLcomNo = 0, iSerLcomCount = 0, iMaxMinType = 0, iServLcomType = 0;
		int iForceLcomSize = 1;//aForceLcom.GetSize();	
		CString strOrgLcomNa=_T("");
		double dC4 = 0.0;
		...
		for(int j=0; j < iForceLcomSize; j++)
		{
			...
			//if(bULSLcom && bCheckStrength[2] && !bIsGenSect)
				...
				T_LCOM_D LcomData;      
				m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL, iDgnLcomNo, LcomData);
				m_pDoc->m_pPostCtrl->SelectLoadCombNew(LcomData); // Load Combi를 셋팅시킴.
				T_REAC_D ReacMaxData;ReacMaxData.Initialize();
				T_NODE_K NodeK = ElemD.elnod[bPosiI==TRUE?0:1];
				m_pDoc->m_pPostCtrl->GetReacNew(NodeK,NULL , NULL, &ReacMaxData);	// reaction을 가져옴.

//         
//         T_NODE_K NodeK = ElemD.elnod[bPosiI==TRUE?0:1];
//         T_REAC_D ReacMaxData;  ReacMaxData.Initialize();
//         m_pDoc->m_pPostCtrl->GetReacNew(NodeK,NULL , NULL, &ReacMaxData); // Load Combi에 대한 반력 
				 double dFz = ReacMaxData.dblReac[2]; 
			
			// Jaeoh. Modification. 이대근 부장님 요청 
			CPG_FORC_STAG ForceStage;      ForceStage.Initialize();
            if(!m_pDataCtrl->Get_GirderCSLoadOfLCOM(StctD, ElemK, nGdLastStagK, nLastStepNo, iDgnLcomNo, ForceStage)) {ASSERT(0);}

			for(int k=0; k < 1; k++)	// Repeat I-End & J-End.
			{
				...
				//Rebar Data
				T_SECT_D& SectD = SectElem.SectD;
				T_SECT_K SectK = pDoc->m_pAttrCtrl->GetSectKey(SectD.SName);
				T_RPSC_SSM_D RpscSSMD; RpscSSMD.Initialize();
				T_RPSC_SSM_D RpscSSMData; RpscSSMData.Initialize();
				 if(!pDoc->m_pAttrCtrl2->GetRpsdSsmByConsiderRbar(TRUE,ElemD.elpro, RpscSSMD)) { ASSERT(0); return FALSE; }
				RpscSSMData = RpscSSMD;
			 
				// Rebar
				T_RPSC_MBAR_SSM MbarD; MbarD.Initialize();
				T_RPSC_MBAR_SSM MbarDPrev; MbarDPrev.Initialize();
				CArray<T_GSEC_RBAR,T_GSEC_RBAR&> aRbarData;
				CArray<T_GSEC_RBAR,T_GSEC_RBAR&> aRbarDataPrev;

				CArray<T_RPSC_MBAR_SSM, T_RPSC_MBAR_SSM&> aRpscSSMDTop; 
				CArray<T_RPSC_MBAR_SSM, T_RPSC_MBAR_SSM&> aRpscSSMDBot; 
				aRpscSSMDTop.RemoveAll();aRpscSSMDBot.RemoveAll();
				aRbarData.RemoveAll();  
				
				double dMoveY, dMoveZ,dPosition=0.;
				if(! CSectUtil::GetConvertLength(SectD,dPosition, dMoveY, dMoveZ)) { ASSERT(0); return FALSE; }; //COnsider Centroidal Point
				double dTopDistFrmCG = 0.;
				double dBotDistFrmCG = 0.;
				dTopDistFrmCG = m_dHw + m_dHh + m_dtc+m_dTF1+m_dTF2 - dMoveZ;
				dBotDistFrmCG = m_dHw + m_dHh + m_dTF1 + m_dTF2 - dMoveZ;

				aRpscSSMDTop.Copy(RpscSSMD.Mbar[1]);
				aRpscSSMDBot.Copy(RpscSSMD.Mbar[0]);
				
				double dPhiNumerator =0.;
				double dPhiDenominator =0.;
				//Dist betn top and bot rebar
								
				double dTopBotDist = RpscSSMD.Mbar[0].GetAt(0).nType != 1 ? 0.5*(m_dtc - abs(RpscSSMD.Mbar[0].GetAt(0).aPassage[0].dPointZ - RpscSSMD.Mbar[0].GetAt(1).aPassage[0].dPointZ)):0.;
				double dTopRightLeftDist =RpscSSMD.Mbar[0].GetAt(0).nType != 1 ? 0.5*(m_dBc - abs(RpscSSMD.Mbar[0].GetAt(1).Start.dPointY - RpscSSMD.Mbar[0].GetAt(1).End.dPointY)):0.;
				double dBotRightLeftDist =RpscSSMD.Mbar[0].GetAt(0).nType != 1 ? 0.5*(m_dBc - abs(RpscSSMD.Mbar[0].GetAt(0).Start.dPointY - RpscSSMD.Mbar[0].GetAt(0).End.dPointY)):0.;
				double dRbarCenter = 0;double dTop = DBL_MIN;double dBot = DBL_MAX;
				double dHzTop = DBL_MAX;double dHzBot = DBL_MAX;
				int iTopLayers = 0,iBotLayers = 0;
				double dTopCoverTemp,dBotCoverTemp;
				if (RpscSSMD.Mbar[0].GetSize() ==1)
				{
					for(int i=0; i<RpscSSMD.Mbar[0].GetSize(); i++)
					{
						MbarD = RpscSSMD.Mbar[0].GetAt(i);           

						aRbarData.RemoveAll();
						CSectUtil::Get_ConvertRpscMbarSSMToGsecRbar(MbarD, aRbarData);           

						dTop = max(dTop,aRbarData[0].dyz[1]);
						dBot = min(dBot,aRbarData[0].dyz[1]);
					}
					dTopCoverTemp=dTopDistFrmCG-dTop;
					dBotCoverTemp = dBot-dBotDistFrmCG;
				}
				else
				{
					for(int i=1; i<RpscSSMD.Mbar[0].GetSize(); i++)
					{
						MbarD = RpscSSMD.Mbar[0].GetAt(i);
						MbarDPrev = RpscSSMD.Mbar[0].GetAt(i-1);

						aRbarData.RemoveAll();aRbarDataPrev.RemoveAll();
						CSectUtil::Get_ConvertRpscMbarSSMToGsecRbar(MbarD, aRbarData);
						CSectUtil::Get_ConvertRpscMbarSSMToGsecRbar(MbarDPrev, aRbarDataPrev);

						dTop = max(dTop,max(aRbarData[0].dyz[1],aRbarDataPrev[0].dyz[1]));
						dBot = min(dBot,min(aRbarData[0].dyz[1],aRbarDataPrev[0].dyz[1]));
				...
				dRbarCenter = dTop == DBL_MIN && dBot == DBL_MAX? (dTopDistFrmCG+dBotDistFrmCG)*0.5:0.5*(dTop+dBot);
				if (RpscSSMD.Mbar[0].GetSize() ==1)
				{//Single Rebar Layer - Always consider at top
					for(int i=0; i<RpscSSMD.Mbar[0].GetSize(); i++)
					{
						MbarD = RpscSSMD.Mbar[0].GetAt(i);
						...
						if (MbarD.nType == 0)
						{
							if (aRbarData[0].dyz[1] > (dTopDistFrmCG+dBotDistFrmCG)*0.5)
							{//TOP R/F
								
									iTopLayers++;
									dHzTop = min(dHzTop,abs(aRbarData[0].dyz[0] ));               

							}

							else if (aRbarData[0].dyz[1] < (dTopDistFrmCG+dBotDistFrmCG)*0.5 )
							{//BOT R?F
								
								
									iBotLayers++;
									dHzBot =min(dHzBot, abs(aRbarData[0].dyz[0]));
				...
				else
				{
					for(int i=1; i<RpscSSMD.Mbar[0].GetSize(); i++)
					{
						...
						if (MbarD.nType == 0)
						{
							if (aRbarData[0].dyz[1] > dRbarCenter && aRbarDataPrev[0].dyz[1] > dRbarCenter )
							{//TOP R/F
								if ( aRbarData[0].dyz[1] == aRbarDataPrev[0].dyz[1])
								{
									iTopLayers+= iTopLayers >=1 ? 0:1;
									dHzTop = min(dHzTop,sqrt(pow((aRbarData[0].dyz[0] - aRbarDataPrev[0].dyz[0]),2)));
								}
								else
								{
									iTopLayers++;
							...
							else if (aRbarData[0].dyz[1] < dRbarCenter && aRbarDataPrev[0].dyz[1] < dRbarCenter)
							{//BOT R?F
								if (aRbarData[0].dyz[1] == aRbarDataPrev[0].dyz[1])
								{
									iBotLayers+= iBotLayers >= 1 ? 0:1;
									dHzBot =min(dHzBot, sqrt(pow((aRbarData[0].dyz[0] - aRbarDataPrev[0].dyz[0]),2)));
								}
								else
								{
									iBotLayers++;
				...
				for(int i=0; i<RpscSSMD.Mbar[0].GetSize(); i++)
				{
					...
					aRbarData.RemoveAll();
					CSectUtil::Get_ConvertRpscMbarSSMToGsecRbar(MbarD, aRbarData);
					if (MbarD.nType == 0)//Point Type rebar definition
					{
						if (aRbarData[0].dyz[1] > dRbarCenter)
						{//TOP R/F
							PgbrIRCGd.SectProp.dTopDia =pDoc->m_pMatlDB->Get_RebarDia(MbarD.strBar);
							PgbrIRCGd.SectProp.dTopSpaceing =  dHzTop;//MbarD.nNumorCTC >= 1 ? MbarD.dCTC :  MbarD.dSpacing;
							PgbrIRCGd.SectProp.dTopNos += 1. ; //MbarD.nNumorCTC >= 1 ? aRbarData.GetCount():MbarD.nNum;
							PgbrIRCGd.SectProp.dTopLayers =iTopLayers;// PgbrIRCGd.SectProp.dTopDia > 0 ? max(1,aRpscSSMDTop.GetSize()) : aRpscSSMDTop.GetSize();
							PgbrIRCGd.SectProp.dTopCover = MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dTopRightLeftDist):MbarD.dRefZ_A;
							dPhiNumerator+= PgbrIRCGd.SectProp.dTopNos * pow(PgbrIRCGd.SectProp.dTopDia,2);
							dPhiDenominator+= PgbrIRCGd.SectProp.dTopNos *PgbrIRCGd.SectProp.dTopDia;
						}
						else
						{//BOT R?F
							PgbrIRCGd.SectProp.dBotDia =pDoc->m_pMatlDB->Get_RebarDia(MbarD.strBar);
							PgbrIRCGd.SectProp.dBotSpaceing = dHzBot;//MbarD.nNumorCTC >= 1 ? MbarD.dCTC :  MbarD.dSpacing;
							PgbrIRCGd.SectProp.dBotNos += 1.0;// MbarD.nNumorCTC >= 1 ? aRbarData.GetCount():MbarD.nNum;
							PgbrIRCGd.SectProp.dBotLayers = iBotLayers;//PgbrIRCGd.SectProp.dBotDia > 0 ? max(1,aRpscSSMDBot.GetSize()) : aRpscSSMDBot.GetSize();
							PgbrIRCGd.SectProp.dBotCover =  MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dBotRightLeftDist):MbarD.dRefZ_A;
							dPhiNumerator+= PgbrIRCGd.SectProp.dBotNos * pow(PgbrIRCGd.SectProp.dBotDia,2);
							dPhiDenominator+= PgbrIRCGd.SectProp.dBotNos *PgbrIRCGd.SectProp.dBotDia;
						}

					}
					else if (MbarD.nType >= 1)//Line Type rebar definition
					{
						if (RpscSSMD.Mbar[0].GetSize() ==1)
						{
							if (iTopLayers+iBotLayers>1)
							{
							if (aRbarData[0].dyz[1] > (dTopDistFrmCG+dBotDistFrmCG)*0.5 )
							{//TOP R/F
								PgbrIRCGd.SectProp.dTopDia =pDoc->m_pMatlDB->Get_RebarDia(MbarD.strBar);
								PgbrIRCGd.SectProp.dTopSpaceing =  MbarD.nNumorCTC >= 1 ? MbarD.dCTC :  MbarD.dSpacing;
								PgbrIRCGd.SectProp.dTopNos = MbarD.nNumorCTC >= 1 ? aRbarData.GetCount():MbarD.nNum;
								PgbrIRCGd.SectProp.dTopLayers = iTopLayers;//PgbrIRCGd.SectProp.dTopDia > 0 ? max(1,aRpscSSMDTop.GetSize()) : aRpscSSMDTop.GetSize();
								PgbrIRCGd.SectProp.dTopCover = dTopCoverTemp;//MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dTopRightLeftDist):MbarD.dRefZ_A;
								dPhiNumerator+= PgbrIRCGd.SectProp.dTopNos * pow(PgbrIRCGd.SectProp.dTopDia,2);
								dPhiDenominator+= PgbrIRCGd.SectProp.dTopNos *PgbrIRCGd.SectProp.dTopDia;
							}
							...
							{//BOT R?F
								...
								PgbrIRCGd.SectProp.dBotDia =pDoc->m_pMatlDB->Get_RebarDia(MbarD.strBar);
								PgbrIRCGd.SectProp.dBotSpaceing =MbarD.nNumorCTC >= 1 ? MbarD.dCTC :  MbarD.dSpacing;
								PgbrIRCGd.SectProp.dBotNos =  MbarD.nNumorCTC >= 1 ? aRbarData.GetCount():MbarD.nNum;
								PgbrIRCGd.SectProp.dBotLayers = iBotLayers;//PgbrIRCGd.SectProp.dBotDia > 0 ? max(1,aRpscSSMDBot.GetSize()) : aRpscSSMDBot.GetSize();
								PgbrIRCGd.SectProp.dBotCover = dBotCoverTemp;// MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dBotRightLeftDist):MbarD.dRefZ_A;
								dPhiNumerator+= PgbrIRCGd.SectProp.dBotNos * pow(PgbrIRCGd.SectProp.dBotDia,2);
								dPhiDenominator+= PgbrIRCGd.SectProp.dBotNos *PgbrIRCGd.SectProp.dBotDia;
                            ...
                            {
							//If Only one layer of rebar is present consider it at top
                                //TOP R/F CIVIL-2601
                                PgbrIRCGd.SectProp.dTopDia =pDoc->m_pMatlDB->Get_RebarDia(MbarD.strBar);
                                PgbrIRCGd.SectProp.dTopSpaceing =  MbarD.nNumorCTC >= 1 ? MbarD.dCTC :  MbarD.dSpacing;
                                PgbrIRCGd.SectProp.dTopNos = MbarD.nNumorCTC >= 1 ? aRbarData.GetCount():MbarD.nNum;
                                PgbrIRCGd.SectProp.dTopLayers = iTopLayers;//PgbrIRCGd.SectProp.dTopDia > 0 ? max(1,aRpscSSMDTop.GetSize()) : aRpscSSMDTop.GetSize();
                               // PgbrIRCGd.SectProp.dTopCover = aRpscSSMDTop.ElementAt(0).dRefZ_A;//dTopCoverTemp;//MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dTopRightLeftDist):MbarD.dRefZ_A;
								PgbrIRCGd.SectProp.dTopCover = dTopCoverTemp;//MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dTopRightLeftDist):MbarD.dRefZ_A;
                                dPhiNumerator+= PgbrIRCGd.SectProp.dTopNos * pow(PgbrIRCGd.SectProp.dTopDia,2);
                                dPhiDenominator+= PgbrIRCGd.SectProp.dTopNos *PgbrIRCGd.SectProp.dTopDia;
						...
						else
						{
							...
							{//TOP R/F
								...
								PgbrIRCGd.SectProp.dTopLayers = iTopLayers;//PgbrIRCGd.SectProp.dTopDia > 0 ? max(1,aRpscSSMDTop.GetSize()) : aRpscSSMDTop.GetSize();
								PgbrIRCGd.SectProp.dTopCover = MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dTopRightLeftDist):MbarD.dRefZ_A;
								if (PgbrIRCGd.SectProp.dTopCover == 0.) PgbrIRCGd.SectProp.dTopCover = abs(dTopDistFrmCG) - abs(dTop);// - abs(PgbrIRCGd.SectProp.dTopDia/2.);
								dPhiNumerator+= PgbrIRCGd.SectProp.dTopNos * pow(PgbrIRCGd.SectProp.dTopDia,2);
								dPhiDenominator+= PgbrIRCGd.SectProp.dTopNos *PgbrIRCGd.SectProp.dTopDia;
							}
							else
							{//BOT R?F
								...
								PgbrIRCGd.SectProp.dBotLayers = iBotLayers;//PgbrIRCGd.SectProp.dBotDia > 0 ? max(1,aRpscSSMDBot.GetSize()) : aRpscSSMDBot.GetSize();
								PgbrIRCGd.SectProp.dBotCover =  MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dBotRightLeftDist):MbarD.dRefZ_A;
								if (PgbrIRCGd.SectProp.dBotCover == 0.) PgbrIRCGd.SectProp.dBotCover = abs(dBot) - abs(dBotDistFrmCG);// - abs(PgbrIRCGd.SectProp.dBotDia / 2.);
								dPhiNumerator+= PgbrIRCGd.SectProp.dBotNos * pow(PgbrIRCGd.SectProp.dBotDia,2);
								dPhiDenominator+= PgbrIRCGd.SectProp.dBotNos *PgbrIRCGd.SectProp.dBotDia;
							}
						}
				...
				//Bottom LoopTraWebStiff.dReaction
				//for(int i=0; i<RpscSSMD.Mbar[0].GetSize(); i++)
				//{
				//  MbarD = RpscSSMD.Mbar[0].GetAt(i);
				...
				// /* if (MbarD.Start.dPointZ > 0 && MbarD.End.dPointZ > 0)
				//  {
				//    PgbrIRCGd.SectProp.dTopDia =pDoc->m_pMatlDB->Get_RebarDia(MbarD.strBar);
				//    PgbrIRCGd.SectProp.dTopSpaceing = MbarD.dSpacing;
				//    PgbrIRCGd.SectProp.dTopNos = MbarD.nNum;
				//    PgbrIRCGd.SectProp.dTopLayers =  PgbrIRCGd.SectProp.dTopDia > 0 ? max(1,aRpscSSMDTop.GetSize()) : aRpscSSMDTop.GetSize();
				//    PgbrIRCGd.SectProp.dTopCover = MbarD.dRefZ_A;
				//    dPhiNumerator+= PgbrIRCGd.SectProp.dTopNos * pow(PgbrIRCGd.SectProp.dTopDia,2);
				//    dPhiDenominator+= PgbrIRCGd.SectProp.dTopNos *PgbrIRCGd.SectProp.dTopDia;
				//  }
				//  else*/
				...
				//    PgbrIRCGd.SectProp.dBotSpaceing =MbarD.nNumorCTC >= 1 ? MbarD.dCTC :  MbarD.dSpacing;
				//    PgbrIRCGd.SectProp.dBotNos =  MbarD.nNumorCTC >= 1 ? aRbarData.GetCount():MbarD.nNum;
				//    PgbrIRCGd.SectProp.dBotLayers = PgbrIRCGd.SectProp.dBotDia > 0 ? max(1,aRpscSSMDBot.GetSize()) : aRpscSSMDBot.GetSize();
				//    PgbrIRCGd.SectProp.dBotCover =  MbarD.nNumorCTC >= 1 ? min(dTopBotDist,dBotRightLeftDist):MbarD.dRefZ_A;
				//    dPhiNumerator+= PgbrIRCGd.SectProp.dBotNos * pow(PgbrIRCGd.SectProp.dBotDia,2);
				//    dPhiDenominator+= PgbrIRCGd.SectProp.dBotNos *PgbrIRCGd.SectProp.dBotDia;
				//  }
				...
				PgbrIRCGd.Fatg.dphi = dPhiDenominator == 0. ? 0 : dPhiNumerator/dPhiDenominator;
				PgbrIRCGd.Fatg.dc   = max(PgbrIRCGd.SectProp.dBotCover,PgbrIRCGd.SectProp.dTopCover);
				//End Rebar Data1
				m_pForcCtrl->Get_Stld(ElemK, iDgnLcomNo, 0, FALSE, ForceLcase, StressAdd, StressLcase, StssLcase,
					ForcePart1, StressPart1, ForcePart2, StressPart2);
				m_dpgls_shear =ForceLcase.dFzz[0];
				...
				m_ClassBase = ClassD;
				//
				SetPgbrBaseData(bChkIJ[k], ElemK, nPosi, iDgnLcomNo, iOrgLcomNo, iMaxMinType, aGirderForce, aCptForce, PgbrGd, PgbrBase, PgbrIRCGd, PgbrIRCGd);      
				PgbrIRCGd.SectProp.iEffWidthCase = iEffWidCase;
				if (PgbrIRCGd.SectProp.dSLlo == 0.)
				{
					CArray<UINT, UINT> aSpanK;
					m_pDoc->m_pAttrCtrl->GetSpanKeyList(aSpanK, 1);  // m_nOption -> 0:PSC, 1:Composite, 2:Stl Girder
				...
				if (PgbrIRCGd.SectProp.iEffWidthCase==0 || PgbrIRCGd.SectProp.iEffWidthCase==5)
				{
					m_dBeffLength = PgbrIRCGd.SectProp.dBc;
				}
				else if (PgbrIRCGd.SectProp.iEffWidthCase==2)
				{
					double dCalVal = (PgbrIRCGd.SectProp.dSLlo/8.) +  (PgbrIRCGd.SectProp.dBc/2.);
					dCalVal = min(dCalVal , PgbrIRCGd.SectProp.dBc) ;
					m_dBeffLength = dCalVal;
				}
				else if (PgbrIRCGd.SectProp.iEffWidthCase==3)
				{
					double dCalVal = (PgbrIRCGd.SectProp.dSLlo/4.) +  (PgbrIRCGd.SectProp.dBc/2.);
					dCalVal = min(dCalVal , PgbrIRCGd.SectProp.dBc) ;
					m_dBeffLength = dCalVal;
				}
				else if (PgbrIRCGd.SectProp.iEffWidthCase==4)
				{
					double dCalVal = (PgbrIRCGd.SectProp.dSLlo/8.) +  (PgbrIRCGd.SectProp.dBc/2.);
					dCalVal = min(dCalVal , PgbrIRCGd.SectProp.dBc) ;
					m_dBeffLength = dCalVal;
				}
				PgbrIRCGd.SectProp.dEffWidth = m_dBeffLength;
				PgbrGd.dNc = dFzz;
				nPosi = k;
				//수평보강재
				CSG_TRST_SECT TrstSect;
				if (m_mCsgTrst.Lookup(SectK, TrstSect))
				{
					//rData.Trst = TrstSect.TrstPos[nPosi];
					PgbrIRCGd.SectProp.dTRSTWebType   = TrstSect.TrstPos[nPosi].nStiffenerTypeWeb;
					PgbrIRCGd.SectProp.dTRSTWebFy     = TrstSect.TrstPos[nPosi].dFyWeb;
					PgbrIRCGd.SectProp.dTRSTWebc    = TrstSect.TrstPos[nPosi].dPitchWeb;
					if (PgbrIRCGd.SectProp.dTRSTWebType == 1 )
                    {//T
						PgbrIRCGd.SectProp.dTRSTWebH    = TrstSect.TrstPos[nPosi].dSizeWeb[0];
						PgbrIRCGd.SectProp.dTRSTWebB    = TrstSect.TrstPos[nPosi].dSizeWeb[1];
						PgbrIRCGd.SectProp.dTRSTWebtw   = TrstSect.TrstPos[nPosi].dSizeWeb[2];
						PgbrIRCGd.SectProp.dTRSTWebtf   = TrstSect.TrstPos[nPosi].dSizeWeb[3];
					}
					else
					{
					//Flat
                       
						PgbrIRCGd.SectProp.dTRSTWebB    = TrstSect.TrstPos[nPosi].dSizeWeb[0];
						PgbrIRCGd.SectProp.dTRSTWebtw   = TrstSect.TrstPos[nPosi].dSizeWeb[1];
                        PgbrIRCGd.SectProp.dTRSTWebtf   = TrstSect.TrstPos[nPosi].dSizeWeb[2] == 0. ? PgbrIRCGd.SectProp.dTRSTWebtw : TrstSect.TrstPos[nPosi].dSizeWeb[2];
						PgbrIRCGd.SectProp.dTRSTWebH    = TrstSect.TrstPos[nPosi].dSizeWeb[3]  == 0. ? PgbrIRCGd.SectProp.dTRSTBearingB : TrstSect.TrstPos[nPosi].dSizeBearing[3];
					}
					

					PgbrIRCGd.SectProp.dTRSTBearingType   = TrstSect.TrstPos[nPosi].nBearingType;
					PgbrIRCGd.SectProp.dTRSTBearingFy     = TrstSect.TrstPos[nPosi].dFyBearing;
					PgbrIRCGd.SectProp.dTRSTBearingc      = TrstSect.TrstPos[nPosi].dPitchBearing;
					if (PgbrIRCGd.SectProp.dTRSTBearingType == 1)
                    {//T
						PgbrIRCGd.SectProp.dTRSTBearingH     = TrstSect.TrstPos[nPosi].dSizeBearing[0];
						PgbrIRCGd.SectProp.dTRSTBearingB      = TrstSect.TrstPos[nPosi].dSizeBearing[1];
						PgbrIRCGd.SectProp.dTRSTBearingtw     = TrstSect.TrstPos[nPosi].dSizeBearing[2];
						PgbrIRCGd.SectProp.dTRSTBearingtf     = TrstSect.TrstPos[nPosi].dSizeBearing[3];
					}
					else
                    {//Flat
                        
						PgbrIRCGd.SectProp.dTRSTBearingB      = TrstSect.TrstPos[nPosi].dSizeBearing[0];
						PgbrIRCGd.SectProp.dTRSTBearingtw     = TrstSect.TrstPos[nPosi].dSizeBearing[1];
                        PgbrIRCGd.SectProp.dTRSTBearingtf     = TrstSect.TrstPos[nPosi].dSizeBearing[2] == 0. ? PgbrIRCGd.SectProp.dTRSTBearingtw :TrstSect.TrstPos[nPosi].dSizeBearing[2] ;
						PgbrIRCGd.SectProp.dTRSTBearingH      = TrstSect.TrstPos[nPosi].dSizeBearing[3] == 0. ? PgbrIRCGd.SectProp.dTRSTBearingB : TrstSect.TrstPos[nPosi].dSizeBearing[3];
					}
				...
				double PIE=atan(1)*4;
				double dGamma_cbs=  m_dGamma_C,//1.50,
					dGamma_cacce=     m_dGamma_C_Acc,
					dGamma_smo=       m_dGamma_S,
					dGamma_sm1=       m_dGamma_M0;
				double dGamma_s=    m_dGamma_M1,
					dGamma_v=         m_dGamma_V,
					dGamma_fft=       m_dGamma_Fft,
					dGamma_mft=       m_dGamma_Mft;
				...
				double dTopDia  = PgbrIRCGd.SectProp.dTopDia, 
					dTopSpac      = PgbrIRCGd.SectProp.dTopSpaceing, 
					dTopNo        = PgbrIRCGd.SectProp.dTopNos,
					dTopLayer     = PgbrIRCGd.SectProp.dTopLayers,
					dTopCover     = PgbrIRCGd.SectProp.dTopCover,
					dTop_Area     = dTopDia*dTopDia*dTopNo*PIE/4.;
				double dBotDia  = PgbrIRCGd.SectProp.dBotDia, 
					dBotSpac      = PgbrIRCGd.SectProp.dBotSpaceing,
					dBotNo        = PgbrIRCGd.SectProp.dBotNos,
					dBotLayer     = PgbrIRCGd.SectProp.dBotLayers,
					dBotCover     = PgbrIRCGd.SectProp.dBotCover,
					dBot_Area     =dBotDia*dBotDia*dBotNo*PIE/4.;
				...
				double dbbf =  m_dB1  , dbtf  = m_dB2;
				double dtbf =  m_dTF1 , dttf  = m_dTF2;
				double dhw  =  m_dhw , dtw   = m_dTw;

				double dIt = ((m_dB1*m_dTF1*m_dTF1*m_dTF1) + (m_dB2*m_dTF2*m_dTF2*m_dTF2) + (m_dhw*m_dTw*m_dTw*m_dTw))/3.0 ;

				const T_MATD_D& MatdD = MatlElem.MatdD;

				double dtopflange_fy = max(MatdD.Data1.DesignTopFlange.S_Fy,MatdD.Data1.DesignTopFlange.S_Fy1) ,dtopflange_fu= MatdD.Data1.Design.S_Fu;
				double dweb_fy       = max(MatdD.Data1.DesignWeb.S_Fy,MatdD.Data1.DesignWeb.S_Fy1) ,		 dweb_fu= MatdD.Data1.Design.S_Fu;
				double dbotflange_fy = max(MatdD.Data1.DesignBotFlange.S_Fy ,MatdD.Data1.DesignBotFlange.S_Fy1) ,	 dbotflange_fu= MatdD.Data1.Design.S_Fu;
				...
				double tf_top=dttf,
					...
					bf_top=dbtf,
					bf_bottom = dbbf,
					D=dw+tf_top/2 +tf_bottom/2,
					fyd_tf=dtopflange_fy,
					fyd_w = dweb_fy,
					fyd_bf = dbotflange_fy,
					fu = dtopflange_fu,     // 540 Input
					mu = 0.3,
					d_girder=tf_top+tf_bottom+dw; // depth of girder



				// Modular Ratio
				double dConcreteGrade=dfck, 
					dEConcrete = dEc,//22000.0*pow(((dConcreteGrade+10)/12.5),0.3),
					dESteel = m_dEs,
					dGSteel = 77000.0,//dESteel/ (2.0* (1 + 0.2)),
					dModular_ratio_per = max((dESteel/(0.5*dEConcrete)),15.0) ,
					dModular_ratio_Trans = max((dESteel/dEConcrete),7.5);

				//
				double thickness_slab =dtc,
					slab_width =dBc,
					Area_Concrete = thickness_slab*slab_width,
					Cg_slab_top = thickness_slab/2,
					Equ_width_steel_pre = Area_Concrete/(dModular_ratio_per*thickness_slab),
					Equ_width_steel_trans = Area_Concrete/(dModular_ratio_Trans*thickness_slab);

				Get_Elastic_Steel_Results(bf_top, tw, bf_bottom, tf_top,  dw,tf_bottom,PgbrIRCGd.SectProp.SteelSect );
				Get_Plastic_Steel_Results(bf_top, tw, bf_bottom, tf_top,  dw,tf_bottom,PgbrIRCGd.SectProp.SteelSect );

				Get_Short_Term_Composite_Results(Equ_width_steel_trans,  bf_top,  tw,  bf_bottom,  dtc,  tf_top,   dw,  tf_bottom,  dModular_ratio_Trans,  slab_width,PgbrIRCGd.SectProp.ShortTermCompo );

				Get_Long_Term_Composite_Results(Equ_width_steel_pre, bf_top, tw, bf_bottom, dtc, tf_top, dw, tf_bottom, dModular_ratio_per, slab_width,PgbrIRCGd.SectProp.LongTermCompo );

				Get_Cracked_Results(bf_top,  tw,  bf_bottom,  dtc,  tf_top,   dw,  tf_bottom,  dModular_ratio_Trans,  slab_width,  dTopDia,  dTopNo,  dBotNo,  dBotSpac,   dTopSpac,   dTopCover,  dHh,  d_girder,  dBotDia,PgbrIRCGd.SectProp.Cracked );

				//**Transverse Stiffener
				double dFyStiffener =0,dFyBearingStiff =0;
				double dtf = 0.;
				double dtw1 = 0.;
				
				if (TrstSect.TrstPos[nPosi].bUseStifBearing == TRUE || TrstSect.TrstPos[nPosi].bUseStifWeb == TRUE)
				{
					if (TrstSect.TrstPos[nPosi].bUseStifBearing == FALSE && TrstSect.TrstPos[nPosi].bUseStifWeb == TRUE)
					{//Only Intermediate
						PgbrIRCGd.TraWebStiff.dMinH     = PgbrIRCGd.SectProp.dTRSTWebH  ;
						dFyStiffener                    = TrstSect.TrstPos[nPosi].dFyWeb;
						if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 && TrstSect.TrstPos[nPosi].nBearingType == 0)
						{
							//T Shape
							double dtw = PgbrIRCGd.SectProp.dTRSTWebtw == 0.?PgbrIRCGd.SectProp.dTRSTWebtf : PgbrIRCGd.SectProp.dTRSTWebtw ;
                            PgbrIRCGd.TraWebStiff.d20tqe    = 20.*dtw*sqrt(250./PgbrIRCGd.SectProp.dTRSTWebFy );
                            PgbrIRCGd.TraWebStiff.d14tqe    = 14.*dtw*sqrt(250./PgbrIRCGd.SectProp.dTRSTWebFy );
							PgbrIRCGd.TraWebStiff.dMinHeff  = PgbrIRCGd.TraWebStiff.dMinH > PgbrIRCGd.TraWebStiff.d20tqe ? PgbrIRCGd.TraWebStiff.d20tqe:min(PgbrIRCGd.TraWebStiff.dMinH,PgbrIRCGd.TraWebStiff.d14tqe);

						}
						else
						{
							//Flat Shape
							PgbrIRCGd.TraWebStiff.dMinH     = PgbrIRCGd.SectProp.dTRSTWebB;
							double dtf = PgbrIRCGd.SectProp.dTRSTWebtf == 0.?PgbrIRCGd.SectProp.dTRSTWebtw : PgbrIRCGd.SectProp.dTRSTWebtf ;
                            PgbrIRCGd.TraWebStiff.d20tqe    = 20.*dtf*sqrt(250./PgbrIRCGd.SectProp.dTRSTWebFy );
                            PgbrIRCGd.TraWebStiff.d14tqe    = 14.*dtf*sqrt(250./PgbrIRCGd.SectProp.dTRSTWebFy );
							PgbrIRCGd.TraWebStiff.dMinHeff  = PgbrIRCGd.SectProp.dTRSTWebB > PgbrIRCGd.TraWebStiff.d20tqe ? PgbrIRCGd.TraWebStiff.d20tqe:min(PgbrIRCGd.SectProp.dTRSTWebB,PgbrIRCGd.TraWebStiff.d14tqe);

						}
					}
					else if (TrstSect.TrstPos[nPosi].bUseStifBearing == TRUE && TrstSect.TrstPos[nPosi].bUseStifWeb == FALSE  )
					{//Only Bearing
						PgbrIRCGd.TraWebStiff.dMinH     = PgbrIRCGd.SectProp.dTRSTBearingH  ;
						dFyBearingStiff                    = TrstSect.TrstPos[nPosi].dFyBearing;
						if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 && TrstSect.TrstPos[nPosi].nBearingType == 0)
						{
							//T Shape
							double dtw = PgbrIRCGd.SectProp.dTRSTWebtw == 0.?PgbrIRCGd.SectProp.dTRSTWebtf : PgbrIRCGd.SectProp.dTRSTWebtw ;
                            PgbrIRCGd.TraWebStiff.d20tqe    = 20.*dtw*sqrt(250./PgbrIRCGd.SectProp.dTRSTBearingFy );
                            PgbrIRCGd.TraWebStiff.d14tqe    = 14.*dtw*sqrt(250./PgbrIRCGd.SectProp.dTRSTBearingFy );
							PgbrIRCGd.TraWebStiff.dMinHeff  = PgbrIRCGd.SectProp.dTRSTBearingH;//PgbrIRCGd.TraWebStiff.dMinH > PgbrIRCGd.TraWebStiff.d20tqe ? PgbrIRCGd.TraWebStiff.d20tqe:min(PgbrIRCGd.TraWebStiff.dMinH,PgbrIRCGd.TraWebStiff.d14tqe);
						...
						else
						{
							...
							double dtf = PgbrIRCGd.SectProp.dTRSTWebtf == 0.?PgbrIRCGd.SectProp.dTRSTWebtw : PgbrIRCGd.SectProp.dTRSTWebtf ;
							PgbrIRCGd.TraWebStiff.dMinH     = PgbrIRCGd.SectProp.dTRSTWebB;
                            PgbrIRCGd.TraWebStiff.d20tqe    = 20.*dtf*sqrt(250./PgbrIRCGd.SectProp.dTRSTBearingFy );
                            PgbrIRCGd.TraWebStiff.d14tqe    = 14.*dtf*sqrt(250./PgbrIRCGd.SectProp.dTRSTBearingFy );
							PgbrIRCGd.TraWebStiff.dMinHeff  = PgbrIRCGd.SectProp.dTRSTBearingH;//PgbrIRCGd.SectProp.dTRSTBearingH > PgbrIRCGd.TraWebStiff.d20tqe ? PgbrIRCGd.TraWebStiff.d20tqe:min(PgbrIRCGd.SectProp.dTRSTBearingH,PgbrIRCGd.TraWebStiff.d14tqe);
					...
					{//Intermediate + Bearing
						PgbrIRCGd.TraWebStiff.dMinH     = PgbrIRCGd.SectProp.dTRSTWebH  ;
						dFyBearingStiff                    = TrstSect.TrstPos[nPosi].dFyWeb;
						if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 && TrstSect.TrstPos[nPosi].nBearingType == 0)
						{
							//T Shape
							double dtw = PgbrIRCGd.SectProp.dTRSTWebtw == 0.?PgbrIRCGd.SectProp.dTRSTWebtf : PgbrIRCGd.SectProp.dTRSTWebtw ;
					...
					PgbrIRCGd.TraWebStiff.iStiffType= PgbrIRCGd.SectProp.dTRSTWebType == 0 ? 0.:1.;
					PgbrIRCGd.TraWebStiff.iTransvNos  = TrstSect.TrstPos[nPosi].nBearingPairNum;
					dtf = PgbrIRCGd.SectProp.dTRSTWebtf == 0. ? PgbrIRCGd.SectProp.dTRSTBearingtf:PgbrIRCGd.SectProp.dTRSTWebtf;
					dtw1 = TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 ?  PgbrIRCGd.SectProp.dTRSTWebtw: PgbrIRCGd.SectProp.dTRSTBearingtw;
					double dL[5]={0,},dD[5]={0,},dYy[5]={0,};
					double dA1=0.,dA2=0.,dA3=0.,dIyy1=0.,dIyy2=0.,dIyy3=0  ;
					double dA1B=0.,dA2B=0.,dA3B=0.,dIyy1B=0.,dIyy2B=0.,dIyy3B=0  ;

					int iTrstCaseNo = 0;
					
					if ( TrstSect.TrstPos[nPosi].bUseStifWeb == TRUE && TrstSect.TrstPos[nPosi].bUseStifBearing == FALSE)
					{//Only Intermediate
						if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 0 && TrstSect.TrstPos[nPosi].nStifNumWeb == 0 ) iTrstCaseNo = 1;//Transverse Flat One Stiffener
						else if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 0 && TrstSect.TrstPos[nPosi].nStifNumWeb == 1 ) iTrstCaseNo = 2;//Transverse Flat Two Stiffener
						else if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 && TrstSect.TrstPos[nPosi].nStifNumWeb == 0 ) iTrstCaseNo = 3;//Transverse T One Stiffener
						else if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 && TrstSect.TrstPos[nPosi].nStifNumWeb == 1 ) iTrstCaseNo = 4;//Transverse T Two Stiffener

					}          
					else if (TrstSect.TrstPos[nPosi].bUseStifBearing == TRUE && TrstSect.TrstPos[nPosi].bUseStifWeb == FALSE)
					{//Only Bearing
						if      (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 5;//Bearing Flat One Stiffener
						else if (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 6;//Bearing Flat Two or more Stiffener
						else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 7;//Bearing T One Stiffener
						else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 8;//Bearing T Two or more Stiffener

					}
					else if (TrstSect.TrstPos[nPosi].bUseStifBearing == TRUE && TrstSect.TrstPos[nPosi].bUseStifWeb == TRUE)
					{
						if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 0 &&  TrstSect.TrstPos[nPosi].nStifNumWeb == 0) //Transverse Flat One Stiffener
							{
								if      (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 9;//Bearing Flat One Stiffener
								else if (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 10;//Bearing Flat Two or more Stiffener
								else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 11;//Bearing T One Stiffener
								else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 12;//Bearing T Two or more Stiffener
							}
						else if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 0 &&  TrstSect.TrstPos[nPosi].nStifNumWeb == 1) //Transverse Flat Two Stiffener
						{
							if      (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 13;//Bearing Flat One Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 14;//Bearing Flat Two or more Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 15;//Bearing T One Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 16;//Bearing T Two or more Stiffener
						}
						else if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 &&  TrstSect.TrstPos[nPosi].nStifNumWeb == 0) //Transverse T One Stiffener
						{
							if      (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 17;//Bearing Flat One Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 18;//Bearing Flat Two or more Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 19;//Bearing T One Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 20;//Bearing T Two or more Stiffener
						}
						else if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 &&  TrstSect.TrstPos[nPosi].nStifNumWeb == 1) //Transverse T Two Stiffener
						{
							if      (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 21;//Bearing Flat One Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 0 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 22;//Bearing Flat Two or more Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum == 1) iTrstCaseNo = 23;//Bearing T One Stiffener
							else if (TrstSect.TrstPos[nPosi].nBearingType == 1 && TrstSect.TrstPos[nPosi].nBearingPairNum >  1) iTrstCaseNo = 24;//Bearing T Two or more Stiffener
						}
					...
					case 1:
                        ...
                        dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] =PgbrIRCGd.SectProp.dTRSTWebtf == 0.?PgbrIRCGd.SectProp.dTRSTWebtw : PgbrIRCGd.SectProp.dTRSTWebtf;dYy[0]= 0.5*PgbrIRCGd.SectProp.dTRSTWebB;

						//dL[1] = dtf;dD[1] = PgbrIRCGd.SectProp.dTRSTWebB ;dYy[1]=(dtf +m_dTw)*0.5;
						//dL[2] = PgbrIRCGd.SectProp.dTRSTBearingtf;dD[2] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? PgbrIRCGd.SectProp.dTRSTWebB:0. ;dYy[2]=(dtf +m_dTw)*0.5;
						//
						//dL[3] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? dL[0]:0.;dD[3] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? dD[0]:0.;dYy[3]=(0.5*(PgbrIRCGd.TraWebStiff.dMinH -dtf) + dtf+m_dTw*0.5 );
						dL[4] = 20.*PgbrIRCGd.SectProp.dTRSTWebtw  ;dD[4] = PgbrIRCGd.SectProp.dTRSTWebtw;dYy[4]=PgbrIRCGd.SectProp.dTRSTWebtw*0.5 ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1,dA2,dA3,dIyy1,dIyy2,dIyy3);
						break;
					case 2:
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTWebtf;dYy[0]= (0.5*PgbrIRCGd.SectProp.dTRSTWebB)+(0.5*PgbrIRCGd.SectProp.dTRSTWebtw);

						//dL[1] = dtf;dD[1] = PgbrIRCGd.SectProp.dTRSTWebB ;dYy[1]=(dtf +m_dTw)*0.5;
						//dL[2] = PgbrIRCGd.SectProp.dTRSTBearingtf;dD[2] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? PgbrIRCGd.SectProp.dTRSTWebB:0. ;dYy[2]=(dtf +m_dTw)*0.5;
						//
						dL[3] = PgbrIRCGd.TraWebStiff.dMinHeff;dD[3] = PgbrIRCGd.SectProp.dTRSTWebtf;dYy[3]=(0.5*PgbrIRCGd.SectProp.dTRSTWebB)+(0.5*PgbrIRCGd.SectProp.dTRSTWebtw);
                        dL[4] = 20.*m_dTw ;dD[4] = m_dTw;dYy[4]= 0. ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1,dA2,dA3,dIyy1,dIyy2,dIyy3);
						break;
					case 3:
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff-PgbrIRCGd.SectProp.dTRSTWebtf ;dD[0] = PgbrIRCGd.SectProp.dTRSTWebtw;dYy[0]= 0.5*dL[0]+PgbrIRCGd.SectProp.dTRSTWebtf;

						dL[1] = PgbrIRCGd.SectProp.dTRSTWebtf;dD[1] = PgbrIRCGd.SectProp.dTRSTWebB ;dYy[1]=(dL[1]*0.5);
						//dL[2] = PgbrIRCGd.SectProp.dTRSTBearingtf;dD[2] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? PgbrIRCGd.SectProp.dTRSTWebB:0. ;dYy[2]=(dtf +m_dTw)*0.5;
						//
						//dL[3] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? dL[0]:0.;dD[3] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? dD[0]:0.;dYy[3]=(0.5*(PgbrIRCGd.TraWebStiff.dMinH -dtf) + dtf+m_dTw*0.5 );
						dL[4] = 20.*m_dTw ;dD[4] = m_dTw;dYy[4]=m_dTw*0.5 ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1,dA2,dA3,dIyy1,dIyy2,dIyy3);
						break;
					case 4:
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff-PgbrIRCGd.SectProp.dTRSTWebtf ;dD[0] = PgbrIRCGd.SectProp.dTRSTWebtw;dYy[0]= 0.5*dL[0]+PgbrIRCGd.SectProp.dTRSTWebtf+0.5*m_dTw;

						dL[1] = PgbrIRCGd.SectProp.dTRSTWebtf;dD[1] = PgbrIRCGd.SectProp.dTRSTWebB ;dYy[1]=0.5*(dL[1]+m_dTw);
						dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];
						//
						dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
					 dL[4] = 20.*m_dTw ;dD[4] = m_dTw;dYy[4]=0. ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1,dA2,dA3,dIyy1,dIyy2,dIyy3);
						break;
					case 5:
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw;

//             dL[1] = PgbrIRCGd.SectProp.dTRSTWebtf;dD[1] = PgbrIRCGd.SectProp.dTRSTWebB ;dYy[1]=0.5*(dL[1]+m_dTw);
//             dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];
//             //
						dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
						dL[4] = 20.*m_dTw ;dD[4] = m_dTw;dYy[4]=0.5*m_dTw ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);
						break;
				 
					case 6:
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw;
						dL[1] =  dL[0];dD[1] = dD[0]  ;dYy[1]=0.5*(dL[1]+m_dTw);
						dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];           
						dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
						dL[4] = ((TrstSect.TrstPos[nPosi].nBearingPairNum-1)*TrstSect.TrstPos[nPosi].dPitchBearing)+2.*20.*m_dTw/2.  ;dD[4] = m_dTw;dYy[4]=0.5*m_dTw ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);
						break;

					case 7:
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff-PgbrIRCGd.SectProp.dTRSTBearingtf ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw+PgbrIRCGd.SectProp.dTRSTBearingtf;
						dL[1] = PgbrIRCGd.SectProp.dTRSTBearingtf ;dD[1] = PgbrIRCGd.SectProp.dTRSTBearingB  ;dYy[1]=0.5*(dL[1]+m_dTw);
						dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];           
						dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
						dL[4] = 20.*m_dTw ;dD[4] = m_dTw;dYy[4]=0. ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);
					...
					case 8:
						...
						dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];           
						dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
						dL[4] = ((TrstSect.TrstPos[nPosi].nBearingPairNum-1)*TrstSect.TrstPos[nPosi].dPitchBearing)+2.*20.*m_dTw/2.  ;dD[4] =m_dTw  ;dYy[4]=0. ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);
						break;
					case 9:
					case 10:
					case 11:
					case 12:          
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTWebtf;dYy[0]= 0.5*PgbrIRCGd.SectProp.dTRSTWebB;
						...
						dL[4] = 20.*PgbrIRCGd.SectProp.dTRSTWebtw  ;dD[4] = PgbrIRCGd.SectProp.dTRSTWebtw;dYy[4]=PgbrIRCGd.SectProp.dTRSTWebtw*0.5 ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1,dA2,dA3,dIyy1,dIyy2,dIyy3);
						if (iTrstCaseNo==9 || iTrstCaseNo==13 || iTrstCaseNo==17 || iTrstCaseNo==21 )
						{
							dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw;
							dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
							dL[4] = m_dTw ;dD[4] = 20.*dL[4];dYy[4]=0.5*dL[4] ;
							Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);
						}
						else if (iTrstCaseNo==10 || iTrstCaseNo==14 || iTrstCaseNo==18 || iTrstCaseNo==22)
						{
							dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw;
							dL[1] =  dL[0];dD[1] = dD[0]  ;dYy[1]=0.5*(dL[1]+m_dTw);
							dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];           
							dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
							dL[4] = m_dTw ;dD[4] = ((TrstSect.TrstPos[nPosi].nBearingPairNum-1)*TrstSect.TrstPos[nPosi].dPitchBearing)+2.*20.*dL[4]/2.;dYy[4]=0.5*dL[4] ;
							Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);
						}
						else if (iTrstCaseNo==11 || iTrstCaseNo==15 || iTrstCaseNo==19 || iTrstCaseNo==23)
						{
							dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff-PgbrIRCGd.SectProp.dTRSTBearingtf ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw+PgbrIRCGd.SectProp.dTRSTBearingtf;
							dL[1] = PgbrIRCGd.SectProp.dTRSTBearingtf ;dD[1] = PgbrIRCGd.SectProp.dTRSTBearingB  ;dYy[1]=0.5*(dL[1]+m_dTw);
							dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];           
							dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
							dL[4] = m_dTw ;dD[4] = 20.*dL[4];dYy[4]=0. ;
							Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);

						}
						else if (iTrstCaseNo==12 || iTrstCaseNo==16 || iTrstCaseNo==20 || iTrstCaseNo==24)
						{
							dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff-PgbrIRCGd.SectProp.dTRSTBearingtf ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw+PgbrIRCGd.SectProp.dTRSTBearingtf;
							dL[1] = PgbrIRCGd.SectProp.dTRSTBearingtf ;dD[1] = PgbrIRCGd.SectProp.dTRSTBearingB  ;dYy[1]=0.5*(dL[1]+m_dTw);
							dL[2] = dL[1];dD[2] = dD[1] ;dYy[2]=dYy[1];           
							dL[3] = dL[0];dD[3] = dD[0];dYy[3]=dYy[0];
							dL[4] = m_dTw ;dD[4] =  ((TrstSect.TrstPos[nPosi].nBearingPairNum-1)*TrstSect.TrstPos[nPosi].dPitchBearing)+2.*20.*dL[4]/2.;dYy[4]=0. ;
							Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1B,dA2B,dA3B,dIyy1B,dIyy2B,dIyy3B);
						}
						break;
					case 13:
					...
					case 15:
					case 16:
						dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTWebtf == 0. ? PgbrIRCGd.SectProp.dTRSTWebtw:PgbrIRCGd.SectProp.dTRSTWebtf;dYy[0]= (0.5*PgbrIRCGd.SectProp.dTRSTWebB)+(0.5*PgbrIRCGd.SectProp.dTRSTWebtw);

						//dL[1] = dtf;dD[1] = PgbrIRCGd.SectProp.dTRSTWebB ;dYy[1]=(dtf +m_dTw)*0.5;
						//dL[2] = PgbrIRCGd.SectProp.dTRSTBearingtf;dD[2] = PgbrIRCGd.TraWebStiff.iStiffType == 0 ? PgbrIRCGd.SectProp.dTRSTWebB:0. ;dYy[2]=(dtf +m_dTw)*0.5;
						//
						dL[3] = PgbrIRCGd.TraWebStiff.dMinHeff;dD[3] = PgbrIRCGd.SectProp.dTRSTWebtf == 0. ? PgbrIRCGd.SectProp.dTRSTWebtw:PgbrIRCGd.SectProp.dTRSTWebtf;dYy[3]=(0.5*PgbrIRCGd.SectProp.dTRSTWebB)+(0.5*PgbrIRCGd.SectProp.dTRSTWebtw);
						dL[4] = 20.*PgbrIRCGd.SectProp.dTRSTWebtw  ;dD[4] = PgbrIRCGd.SectProp.dTRSTWebtw;dYy[4]= 0. ;
						Get_Transverse_Stiffener_Sect_Prop(dL,dD,dYy,dA1,dA2,dA3,dIyy1,dIyy2,dIyy3);
						if (iTrstCaseNo==9 || iTrstCaseNo==13 || iTrstCaseNo==17 || iTrstCaseNo==21 )
						{
							dL[0] = PgbrIRCGd.TraWebStiff.dMinHeff ;dD[0] = PgbrIRCGd.SectProp.dTRSTBearingtw;dYy[0]= 0.5*dL[0]+0.5*m_dTw;
					...
					PgbrIRCGd.TraWebStiff.dIys      = dIyy1;
					PgbrIRCGd.TraWebStiff.iStiffCaseNo = iTrstCaseNo;
					double dFactcbyd = PgbrIRCGd.SectProp.dTRSTWebc / m_dhw;
                    double dthk = min(PgbrIRCGd.SectProp.dTRSTWebtw,PgbrIRCGd.SectProp.dTRSTBearingtw);
					if (dFactcbyd >= sqrt(2.0)) 
					{ 
						PgbrIRCGd.TraWebStiff.iCbyDcond = 0;
						PgbrIRCGd.TraWebStiff.d075dtw3  = 0.75 * PgbrIRCGd.SectProp.ddc*pow(dthk,3);

					}
					else
					{
						PgbrIRCGd.TraWebStiff.iCbyDcond = 1;
						PgbrIRCGd.TraWebStiff.d075dtw3  = 1.5 * pow(PgbrIRCGd.SectProp.ddc,3)*pow(dthk,3) / pow(PgbrIRCGd.SectProp.dTRSTWebc ,2);
					}

					PgbrIRCGd.TraWebStiff.dhdashw      =  20. * m_dTw;          
					PgbrIRCGd.TraWebStiff.dIysBuck     =  dIyy3;          
					PgbrIRCGd.TraWebStiff.dKL          = 0.7*m_dhw;
					PgbrIRCGd.TraWebStiff.dAlpha       = 0.49;
					...
					case 15:
					case 16:
						PgbrIRCGd.TraWebStiff.dAeff        =  dA3 ;
						PgbrIRCGd.TraWebStiff.drys         =  sqrt(PgbrIRCGd.TraWebStiff.dIysBuck /PgbrIRCGd.TraWebStiff.dAeff   );
						PgbrIRCGd.TraWebStiff.dFcc         = m_dPI * m_dPI * m_dEs / pow(PgbrIRCGd.TraWebStiff.dKL/PgbrIRCGd.TraWebStiff.drys ,2);
						PgbrIRCGd.TraWebStiff.dLambda      = sqrt(dtopflange_fy/PgbrIRCGd.TraWebStiff.dFcc         );
						PgbrIRCGd.TraWebStiff.dPhi         = 0.5*(1 + PgbrIRCGd.TraWebStiff.dAlpha * (PgbrIRCGd.TraWebStiff.dLambda - 0.2) + pow(PgbrIRCGd.TraWebStiff.dLambda,2) );
						dXcal                              = 1./ (PgbrIRCGd.TraWebStiff.dPhi + sqrt(pow(PgbrIRCGd.TraWebStiff.dPhi,2) - pow(PgbrIRCGd.TraWebStiff.dLambda ,2)));
						PgbrIRCGd.TraWebStiff.dX           =  dXcal > 1. ? 1.:dXcal;
                        PgbrIRCGd.TraWebStiff.dfcd         =  PgbrIRCGd.TraWebStiff.dX*max(dFyStiffener,dFyBearingStiff) / m_dGamma_M0;
                        PgbrIRCGd.TraWebStiff.dFqd        = PgbrIRCGd.TraWebStiff.dfcd  * PgbrIRCGd.TraWebStiff.dMinHeff  *PgbrIRCGd.SectProp.dTRSTWebtw;
						if (PgbrIRCGd.TraWebStiff.dFqd == 0.)PgbrIRCGd.TraWebStiff.dFqd        = PgbrIRCGd.TraWebStiff.dfcd  * PgbrIRCGd.TraWebStiff.dMinHeff  * PgbrIRCGd.SectProp.dTRSTWebtf;
					...
					case 24:
						...
						dXcal                              = 1./ (PgbrIRCGd.TraWebStiff.dPhi + sqrt(pow(PgbrIRCGd.TraWebStiff.dPhi,2) - pow(PgbrIRCGd.TraWebStiff.dLambda ,2)));
						PgbrIRCGd.TraWebStiff.dX           =  dXcal > 1. ? 1.:dXcal;
						PgbrIRCGd.TraWebStiff.dfcd         =  PgbrIRCGd.TraWebStiff.dX*PgbrIRCGd.SectProp.dTRSTWebFy / m_dGamma_M0;
						PgbrIRCGd.TraWebStiff.dFqd        = PgbrIRCGd.TraWebStiff.dfcd  * PgbrIRCGd.TraWebStiff.dMinHeff  * PgbrIRCGd.SectProp.dTRSTWebtw;
						if (PgbrIRCGd.TraWebStiff.dFqd == 0.)PgbrIRCGd.TraWebStiff.dFqd        = PgbrIRCGd.TraWebStiff.dfcd  * PgbrIRCGd.TraWebStiff.dMinHeff  * PgbrIRCGd.SectProp.dTRSTWebtf;
					...
					// 4. Calc. Plastic resistance to vertical shear, EN 1993-1-1: 2005, 6.2.6 (50page)
					double dAv	 = m_dHw*m_dTw_t;
					double dVpl_Rd = m_dGamma_M0 < m_dZero ? 0.0 : dAv*(dweb_fy/sqrt(3.0))/m_dGamma_M0;         
					PgbrIRCGd.TraWebStiff.strLcombName = strOrgLcomNa;
					PgbrIRCGd.TraWebStiff.dReaction    = abs(dFzz);//max(dV1,dV2);
					PgbrIRCGd.TraWebStiff.dFqdLim   = (PgbrIRCGd.TraWebStiff.dReaction  - dVpl_Rd ) / m_dGamma_M0 ; 

					//bearing stiffener
					PgbrIRCGd.TraWebStiff.db1         = 2.0*(0.5* PgbrIRCGd.SectProp.dTRSTBearingtw+  m_dTF1 );
					PgbrIRCGd.TraWebStiff.dn1         = 0.5*(m_dTF1+m_dTF2 + m_dHw);
					PgbrIRCGd.TraWebStiff.dA          = (PgbrIRCGd.TraWebStiff.db1+PgbrIRCGd.TraWebStiff.dn1)*m_dTw ; 
					PgbrIRCGd.TraWebStiff.dBearingIys = dIyy3B;//(PgbrIRCGd.TraWebStiff.db1+PgbrIRCGd.TraWebStiff.dn1)*pow(m_dTw,3)/12.;
					PgbrIRCGd.TraWebStiff.dBearingrys = sqrt(PgbrIRCGd.TraWebStiff.dBearingIys/ PgbrIRCGd.TraWebStiff.dA);
					PgbrIRCGd.TraWebStiff.dK          = 0.7;
					PgbrIRCGd.TraWebStiff.dBearingfcc = m_dPI * m_dPI * m_dEs / pow(PgbrIRCGd.TraWebStiff.dK*m_dHw/PgbrIRCGd.TraWebStiff.dBearingrys,2) ;
					PgbrIRCGd.TraWebStiff.dFcdw       =  PgbrIRCGd.TraWebStiff.dBearingfcc*PgbrIRCGd.TraWebStiff.dA;

					//chk local crushing of web
					PgbrIRCGd.TraWebStiff.dCrushing_n2 = m_dtf1 * 2.5;
					PgbrIRCGd.TraWebStiff.dCrushing_A          = (TrstSect.TrstPos[nPosi].nBearingPairNum*2)* m_dTw_t *  (PgbrIRCGd.TraWebStiff.dCrushing_n2 +PgbrIRCGd.TraWebStiff.db1 );
					PgbrIRCGd.TraWebStiff.dFcdwCrushing=  PgbrIRCGd.TraWebStiff.dCrushing_A * dtopflange_fy / m_dGamma_M0 ;
					//Bearing Check
					PgbrIRCGd.TraWebStiff.dBearing_n   = TrstSect.TrstPos[nPosi].nBearingPairNum *2. ;
					// if (TrstSect.TrstPos[nPosi].nStiffenerTypeWeb == 1 )
					 {
						 PgbrIRCGd.TraWebStiff.dAq          = TrstSect.TrstPos[nPosi].nBearingType == 0 ? dA1B: TrstSect.TrstPos[nPosi].nBearingPairNum*dA1B;//PgbrIRCGd.TraWebStiff.dBearing_n  * (PgbrIRCGd.SectProp.dTRSTWebH*PgbrIRCGd.SectProp.dTRSTWebtw  + PgbrIRCGd.SectProp.dTRSTBearingB *PgbrIRCGd.SectProp.dTRSTBearingtf );

					 }
//            else
//            {
//              PgbrIRCGd.TraWebStiff.dAq          = PgbrIRCGd.TraWebStiff.dBearing_n  * (PgbrIRCGd.SectProp.dTRSTBearingH*PgbrIRCGd.SectProp.dTRSTBearingtw  + PgbrIRCGd.SectProp.dTRSTWebH *PgbrIRCGd.SectProp.dTRSTWebB );
// 
//            }
					PgbrIRCGd.TraWebStiff.dFpsd        = PgbrIRCGd.TraWebStiff.dAq *dFyBearingStiff/ (0.8 * m_dGamma_M0);
					PgbrIRCGd.TraWebStiff.iBearingStiffNos  =  max(max(TrstSect.TrstPos[nPosi].nStifNumBot,TrstSect.TrstPos[nPosi].nStifNumTop),TrstSect.TrstPos[nPosi].nStifNumWeb);
					...
					//Buckling Check
					PgbrIRCGd.TraWebStiff.dBuck_hdasheff = min(20.* PgbrIRCGd.SectProp.dTRSTBearingtw, TrstSect.TrstPos[nPosi].nBearingType == 0 ? PgbrIRCGd.SectProp.dTRSTBearingB:PgbrIRCGd.SectProp.dTRSTBearingH  );
					PgbrIRCGd.TraWebStiff.dBuck_hdashw   = dD[4];//m_dTw*20.;
					PgbrIRCGd.TraWebStiff.dBuck_Aeff     = dA3B == 0.? PgbrIRCGd.TraWebStiff.dA  :dA3B;//(PgbrIRCGd.TraWebStiff.dBuck_hdasheff*max(PgbrIRCGd.SectProp.dTRSTWebB, PgbrIRCGd.SectProp.dTRSTWebtw)) + (PgbrIRCGd.TraWebStiff.dBuck_hdashw*m_dTw); 
					PgbrIRCGd.TraWebStiff.dBuck_Iys      = dIyy3B;
					PgbrIRCGd.TraWebStiff.dBuck_rys      = PgbrIRCGd.TraWebStiff.dBuck_Aeff == 0. ? 0.:sqrt(PgbrIRCGd.TraWebStiff.dBuck_Iys / PgbrIRCGd.TraWebStiff.dBuck_Aeff);
					PgbrIRCGd.TraWebStiff.dBuck_KL       = 0.7 * m_dHw;
					PgbrIRCGd.TraWebStiff.dBuck_alpha    = 0.49;
					PgbrIRCGd.TraWebStiff.dBuck_fcc      = m_dPI * m_dPI * m_dEs / pow(PgbrIRCGd.TraWebStiff.dBuck_KL/PgbrIRCGd.TraWebStiff.dBuck_rys ,2);
					PgbrIRCGd.TraWebStiff.dBuck_lambda   = PgbrIRCGd.TraWebStiff.dBuck_fcc  == 0. ? 0.:sqrt(dtopflange_fy/PgbrIRCGd.TraWebStiff.dBuck_fcc         );
					PgbrIRCGd.TraWebStiff.dBuck_phi      = 0.5*(1 +( PgbrIRCGd.TraWebStiff.dBuck_alpha * (PgbrIRCGd.TraWebStiff.dBuck_lambda - 0.2)) + pow(PgbrIRCGd.TraWebStiff.dBuck_lambda,2) );
					PgbrIRCGd.TraWebStiff.dBuck_X        =  min(abs(1./ (PgbrIRCGd.TraWebStiff.dBuck_phi + sqrt(pow(PgbrIRCGd.TraWebStiff.dBuck_phi,2) - pow(PgbrIRCGd.TraWebStiff.dBuck_lambda ,2)))),1.);
					PgbrIRCGd.TraWebStiff.dBuck_fcd      =  PgbrIRCGd.TraWebStiff.dBuck_X*dFyBearingStiff / m_dGamma_M0;
					PgbrIRCGd.TraWebStiff.dBuck_fcd_Lim  =  PgbrIRCGd.TraWebStiff.dReaction/PgbrIRCGd.TraWebStiff.dBuck_Aeff;
				...
				for (i=0; i<nVedDLLcomSize; ++i)
				{
					...
					_DGN_FORC_CRC MembEndForce;    
					m_pForcCtrl->Get_LcomDataForDesign_LcomK(ElemK, nStrnKey, TRUE, FALSE, MembEndForce);
                    GetShear_Rslt_FSUM(ElemK, nStrnKey);
					double dMax = max( abs(m_aValue->GetAt(0)),abs( m_aValue[1].GetAt(0)));
					
					//
					if (dMax>dVedDL)
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						T_LCOM_D rDataTemp;rDataTemp.Initialize();
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
                        PgbrIRCGd.LotrShear.strLcomName = rDataTemp.LoadCombName;//LcomebName;
                        PgbrIRCGd.LotrShear.strVedDlLcombMax = rDataTemp.LoadCombName;//LcomebName;
					}
					dVedDL = max(dMax,dVedDL);
				}
				...
				for (i=0; i<nVedLLLcomSize; ++i)
				{
					...
					if (dMax>dVedLL)
					{
						...
						T_LCOM_D rDataTemp;rDataTemp.Initialize();
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						PgbrIRCGd.LotrShear.strLcombType = rDataTemp.LoadCombName;//LcomebName;
                        PgbrIRCGd.LotrShear.strVedLlLcombMax = rDataTemp.LoadCombName;//LcomebName;
					}
					dVedLL = max(dMax,dVedLL);
				}
				...
				int iNum			= SconPosi.iNum;
				double dhsc		= SconPosi.dhsc;
                double dfu_sc		= m_pDoc->m_pUnitCtrl->ConvertCurUnitData2TgtUnit(D_UNITSYS_BASE_FORCE, 0, D_UNITSYS_BASE_FORCE, SconPosi.dfu*1000.);//SconPosi.dfu;
				double dDia_sc	= SconPosi.dDia;
				double dSt_sc		= SconPosi.dSt;
				double dSpace		= SconPosi.dSpace;
				double dQr      = SconPosi.dQr;
                double dAconAst = SconPosi.dAst*10000.;
                double dPlaneLen= m_pDoc->m_pUnitCtrl->ConvertCurUnitData2TgtUnit(0, D_UNITSYS_BASE_LENGTH, D_UNITSYS_BASE_LENGTH, SconPosi.dPlaneLen*10000.);//SconPosi.dPlaneLen;

				PgbrIRCGd.LotrShear.dVedDL = dVedDL;
				PgbrIRCGd.LotrShear.dVedLL = dVedLL;
				PgbrIRCGd.LotrShear.dSLProv = dSt_sc;
				PgbrIRCGd.LotrShear.dSLProvLim = 21. * m_dTF1 *sqrt(250./dtopflange_fy ) ;
				PgbrIRCGd.LotrShear.dhs        = dhsc;
				double dPart1     = (PgbrIRCGd.LotrShear.dVedDL*PgbrIRCGd.SectProp.LongTermCompo.dArea *(dtbf + dttf + dhw + dHh + 0.5*thickness_slab - PgbrIRCGd.SectProp.LongTermCompo.dbot ) / PgbrIRCGd.SectProp.LongTermCompo.dIyy ) ;
				double dPart2     = (PgbrIRCGd.LotrShear.dVedLL*PgbrIRCGd.SectProp.ShortTermCompo.dArea *(dtbf + dttf + dhw + dHh + 0.5*thickness_slab - PgbrIRCGd.SectProp.ShortTermCompo.dbot ) / PgbrIRCGd.SectProp.ShortTermCompo.dIyy ) ;
				PgbrIRCGd.LotrShear.dVled = dPart1+dPart2;
				PgbrIRCGd.LotrShear.dn    = iNum;
				PgbrIRCGd.LotrShear.dQu    =  dfu_sc;
                PgbrIRCGd.LotrShear.dSl1req= PgbrIRCGd.LotrShear.dVled == 0. ? 0.:PgbrIRCGd.LotrShear.dn *PgbrIRCGd.LotrShear.dQu*1000./PgbrIRCGd.LotrShear.dVled;
				PgbrIRCGd.LotrShear.dSl1prov= dSt_sc;
				//chk full hsear connection
				PgbrIRCGd.LotrShear.dAsl    = dTop_Area+dBot_Area;

				int iPNACond = 0;       
			 
				
				if (m_PgbrBase.dPlasticNA > m_dH ) iPNACond = 0;// PNA in Slab
				else if (m_PgbrBase.dPlasticNA > m_dH - m_dtf1 ) iPNACond = 1;// PNA in Top Flange
				else if(m_PgbrBase.dPlasticNA < m_dH-m_dtf1 && PgbrBase.dPlasticNA > m_dtf2) iPNACond = 2;// PNA in Web
				else if(m_PgbrBase.dPlasticNA < m_dtf2) iPNACond = 3;// PNA in Bottom Flange
				

				int iFinalReportCond = 0;

				if      (iPNACond == 0 && m_PgbrBase.iSectClass == 1) iFinalReportCond = 1; //Case 1: PNA  in Slab  (Plastic)
				else if (iPNACond == 1 && m_PgbrBase.iSectClass == 1) iFinalReportCond = 2; //Case 2: PNA  in Flange  (Plastic)
				else if (iPNACond == 2 && m_PgbrBase.iSectClass == 1) iFinalReportCond = 3; //Case 2: PNA  in Web  (Plastic)
				else if (iPNACond == 0 && m_PgbrBase.iSectClass == 2) iFinalReportCond = 4; //Case 4: PNA  in Slab  (Comapact)
				else if (iPNACond == 1 && m_PgbrBase.iSectClass == 2) iFinalReportCond = 5; //Case 5: PNA  in Flange  (Comapact)
				else if (iPNACond == 2 && m_PgbrBase.iSectClass == 2) iFinalReportCond = 6; //Case 6: PNA  in Web  (Comapact)
				else if (iPNACond == 0 && m_PgbrBase.iSectClass == 3) iFinalReportCond = 7; //Case 7: PNA  in Slab  (Semi - Comapact)
				else if (iPNACond == 1 && m_PgbrBase.iSectClass == 3) iFinalReportCond = 8; //Case 8: PNA  in Flange  (Semi - Comapact)
				else if (iPNACond == 2 && m_PgbrBase.iSectClass == 3) iFinalReportCond = 9; //Case 9: PNA  in Web  (Semi - Comapact)
				


				if (iFinalReportCond == 1 || iFinalReportCond == 4 || iFinalReportCond == 7)
				{
					 if      (iFinalReportCond == 1 ) PgbrIRCGd.LotrShear.dAecSlab = dBeff *m_PgbrBase.dDistT;
					 else                             PgbrIRCGd.LotrShear.dAecSlab = dBeff *m_dtc;//m_PgbrBase.dXpl;

				}
				else
				{
					PgbrIRCGd.LotrShear.dAecSteel = SectElem.SectD.SectBefore.Matl_Elast == 0.? (dBeff * thickness_slab):(dBeff * thickness_slab/SectElem.SectD.SectBefore.Matl_Elast);//CIVIL-3320
				}
				 
				PgbrIRCGd.LotrShear.dH1 = PgbrIRCGd.LotrShear.dAsl * m_dfyk  / m_dGamma_C_Acc ;
				PgbrIRCGd.LotrShear.dH2 = 0.36 * max(PgbrIRCGd.LotrShear.dAecSlab,PgbrIRCGd.LotrShear.dAecSteel) * dfck;
				PgbrIRCGd.LotrShear.dH  = min(PgbrIRCGd.LotrShear.dH2,PgbrIRCGd.LotrShear.dH1);
				PgbrIRCGd.LotrShear.dL  =  max(m_dBc/2.,dSpanLength/2.) ;
				PgbrIRCGd.LotrShear.dSl2req =  PgbrIRCGd.LotrShear.dH > 0 ? (PgbrIRCGd.LotrShear.dn *PgbrIRCGd.LotrShear.dQu * PgbrIRCGd.LotrShear.dL*1000. / PgbrIRCGd.LotrShear.dH) : 0.;
				//Longitudinal Shear Fatigue

				int nVrFatLcomSize = LcmtD.Base[0].aFatigue.GetSize();//VedDL
				double dVr = 0;
				for (i=0; i<nVrFatLcomSize; ++i)
				{
					T_LCOM_K nStrnKey = LcmtD.Base[0].aFatigue[i];    
					_DGN_FORC_CRC MembEndForce;    
					m_pForcCtrl->Get_LcomDataForDesign_LcomK(ElemK, nStrnKey, TRUE, FALSE, MembEndForce);
					double dMax = max( abs(MembEndForce.dFzz[0]),abs( MembEndForce.dFzz[1]));
					double dMin = min( abs(MembEndForce.dFzz[0]),abs( MembEndForce.dFzz[1]));
					//
					if (dMax> dVr)
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(iDgnLcomNo, LcomebName, iMaxmin);
						T_LCOM_D rDataTemp;rDataTemp.Initialize();
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						PgbrIRCGd.LotrShear.strLcomName = rDataTemp.LoadCombName;//LcomebName;
				...
				PgbrIRCGd.LotrShear.dVR =  dVr;
				PgbrIRCGd.LotrShear.dVr =  PgbrIRCGd.LotrShear.dVR * max(PgbrIRCGd.LotrShear.dAecSlab,PgbrIRCGd.LotrShear.dAecSteel) * (m_PgbrBase.dXpl - 0.5*thickness_slab) / PgbrIRCGd.SectProp.ShortTermCompo.dIyy;
				//Input for Qr
				PgbrIRCGd.LotrShear.dQr = dQr;
				PgbrIRCGd.LotrShear.dSrreq = PgbrIRCGd.LotrShear.dVr == 0. ? 0: PgbrIRCGd.LotrShear.dQr*PgbrIRCGd.LotrShear.dn  / PgbrIRCGd.LotrShear.dVr;

				double dMinSpacing = min(min(600.,3.*thickness_slab),PgbrIRCGd.LotrShear.dhs );
				double dMinSPacingProv = min(min(PgbrIRCGd.LotrShear.dSrreq==0.?999999:PgbrIRCGd.LotrShear.dSrreq,PgbrIRCGd.LotrShear.dSl2req),PgbrIRCGd.LotrShear.dSl1req);
				if (dMinSPacingProv < 75.) PgbrIRCGd.LotrShear.dSLreq = 75.;
				else PgbrIRCGd.LotrShear.dSLreq = dMinSPacingProv;
			
				//Transverse Shear Chk
				PgbrIRCGd.LotrShear.dVlL = dPlaneLen == 0.? 2.0*PgbrIRCGd.LotrShear.dhs + bf_top : dPlaneLen;
				PgbrIRCGd.LotrShear.dVlAst = dAconAst;
                PgbrIRCGd.LotrShear.dVl25  = m_dfyk == 0. ? 0.:2.5* PgbrIRCGd.LotrShear.dVled  / m_dfyk ;
				PgbrIRCGd.LotrShear.dVlcase1 = 0.632 * PgbrIRCGd.LotrShear.dVlL*sqrt(m_dfck);
				PgbrIRCGd.LotrShear.dVlcase2 = 0.232 * PgbrIRCGd.LotrShear.dVlL*sqrt(m_dfck) + (0.1*PgbrIRCGd.LotrShear.dVlAst*m_dfyk * (0.5*iNum));
				

				// 12. Fatigue Resistance
				T_STRESS_INFO_I StressInfoDMax,StressInfoDMin;
				T_SECT_STIFFNESS NonConSect;	NonConSect.Initialize();
				T_SECT_STIFFNESS RebarSect;	RebarSect.Initialize();	
				T_SECT_STIFFNESS GirderStiff;	GirderStiff.Initialize();
				...
				GetRebarSectStiffD(SectElem, bPosiI, RebarSect);
				Get_GirderStiffnessofCompo(bPosiI, SectD, GirderStiff);
				CMapEx<T_LCOM_K, T_LCOM_K, T_LCOM_K, T_LCOM_K&> mFatiguecomKey;         

				int nFatigueLcomSize = LcmtD.Base[0].aFatigue.GetSize();//Fatigue
				double dFatg = 0;
				for (i=0; i<nFatigueLcomSize; ++i)
				{
					T_LCOM_K nStrnKey = LcmtD.Base[0].aFatigue[i];    
					_DGN_FORC_CRC MembEndForce;    
                    T_DGN_LCOM_ID FatiLcomId;
                    BOOL bFati = TRUE;
                    if (!m_pForcCtrl->m_aFatiLcomId.Lookup(nStrnKey, FatiLcomId)) {bFati = FALSE;}
					m_pForcCtrl->Get_LcomDataForDesign_LcomK(ElemK, FatiLcomId.aDgnLcomId[0], TRUE, FALSE, MembEndForce);
					
					double dMax = max( (MembEndForce.dFzz[0]),( MembEndForce.dFzz[1]));
					double dMin = min( (MembEndForce.dFzz[0]),( MembEndForce.dFzz[1]));
					//
					if (dMax>dVedDL)
					{
						CString LcomebName = _T("");
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						//PgbrIRCGd.LotrShear.strLcomName = rDataTemp.LoadCombName;//LcomebName;
                        //PgbrIRCGd.LotrShear.strLcomName = LcomebName;
					}
					dFatg = max(dMax,dVedDL);

					CPG_FORC_STAG ForceStage;      ForceStage.Initialize();
				    if (!m_pDataCtrl->Get_GirderCSLoadOfLCOM(StctD, ElemK, nGdLastStagK, nLastStepNo, FatiLcomId.aDgnLcomId[0], ForceStage)) { ASSERT(0); }
					
					double dFxx = MembEndForce.dFxx[k];
					double dFyy = MembEndForce.dFyy[k];
					double dFzz = MembEndForce.dFzz[k];
					double dMux = MembEndForce.dMux[k];
					double dMuy = MembEndForce.dMuy[k];
					double dMuz = MembEndForce.dMuz[k];
					double dMa_Ed = ForceStage.MaxForce.dMuy[k];
					double aGirderForceMax[6] = {ForceStage.MaxForce.dFxx[k], ForceStage.MaxForce.dFyy[k], ForceStage.MaxForce.dFzz[k],
						ForceStage.MaxForce.dMux[k], ForceStage.MaxForce.dMuy[k], ForceStage.MaxForce.dMuz[k]};
					double aGirderForceMin[6] = {ForceStage.MinForce.dFxx[k], ForceStage.MinForce.dFyy[k], ForceStage.MinForce.dFzz[k],
						ForceStage.MinForce.dMux[k], ForceStage.MinForce.dMuy[k], ForceStage.MinForce.dMuz[k]};
					m_dPu = dFxx;
					m_dMuy = dMuy;
					m_dMuz = dMuz;
					double aForce[6] = {dFxx, dFyy, dFzz, dMux, dMuy, dMuz};
					double aCptForceMax[6]={0.0};
					double aCptForceMin[6]={0.0};
					for(int l = 0 ; l < 6; ++l){aCptForceMax[l] = aForce[l] - aGirderForceMax[l];}
					for(int l = 0 ; l < 6; ++l){aCptForceMin[l] = aForce[l] - aGirderForceMin[l];}
					T_CLASS_IN_D_IRC_15 ClassInD;
					Make_ClassInD(bPosiI, aGirderForceMax, aCptForceMax, SectElem, ClassInD);
					Make_ClassInD(bPosiI, aGirderForceMin, aCptForceMin, SectElem, ClassInD);

					BOOL bPositive = dMuy > -m_dZero ? TRUE : FALSE;
					T_SECT_STIFFNESS SectStiffD = bPositive ? RebarSect : NonConSect;
					GetStressInfo(bPosiI, SectD, aGirderForceMax, aCptForceMax, GirderStiff, SectStiffD, StressInfoDMax);	  
					GetStressInfo(bPosiI, SectD, aGirderForceMin, aCptForceMin, GirderStiff, SectStiffD, StressInfoDMin);
					double dTopStressMax = 0.5*(StressInfoDMax.FlgTL.dStr1 + StressInfoDMax.FlgTR.dStr1);
 					double dBotStressMax = 0.5*(StressInfoDMax.FlgBL.dStr1 + StressInfoDMax.FlgBR.dStr1);
					double dWebStressMax = StressInfoDMax.Web.dStr1;

					double dTopStressMin = 0.5*(StressInfoDMin.FlgTL.dStr1 + StressInfoDMin.FlgTR.dStr1);
					double dBotStressMin = 0.5*(StressInfoDMin.FlgBL.dStr1 + StressInfoDMin.FlgBR.dStr1);
					double dWebStressMin = StressInfoDMin.Web.dStr1;
					...
					double aSize[6] = {dH,dTw,dB1,dTF1,dB2,dTF2};
					double ddc  = GirderStiff.Czp + dHh + 0.5*dtc;
					double dQMax = Get_Q_FirstMomentOfGirderCentroid(aSize,GirderStiff.Czp,ddc);
					double tau1Max = Get_ShearStress(dMax,dQMax,GirderStiff.Ryy,dTw);
					double tau1Min = Get_ShearStress(dMin,dQMax,GirderStiff.Ryy,dTw);


					//Save Results
					//Top Flange Top Max
					if (abs(dTopStressMax) > abs(PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Max))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						T_LCOM_D rDataTemp;rDataTemp.Initialize();
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Max = dTopStressMax;
						PgbrIRCGd.Fatg.TopFlange.TopSide.dTau1Max   = tau1Max;
						PgbrIRCGd.Fatg.TopFlange.TopSide.strMaxLCB  = LcomebName;
						PgbrIRCGd.Fatg.TopFlange.TopSide.y1         = StressInfoDMax.FlgTR.dy1;
						PgbrIRCGd.Fatg.TopFlange.TopSide.z1         = StressInfoDMax.FlgTR.dz1;

					}
					//Top Flange Bot Max
					if (abs(dTopStressMax) > abs(PgbrIRCGd.Fatg.TopFlange.BotSide.dSigma2Max))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.TopFlange.BotSide.dSigma2Max = dTopStressMax;
						PgbrIRCGd.Fatg.TopFlange.BotSide.dTau2Max   = tau1Max;
						PgbrIRCGd.Fatg.TopFlange.BotSide.strMaxLCB  = LcomebName;
						PgbrIRCGd.Fatg.TopFlange.BotSide.y1         = StressInfoDMax.FlgTR.dy2;
						PgbrIRCGd.Fatg.TopFlange.BotSide.z1         = StressInfoDMax.FlgTR.dz2;

					}
					//Top Flange Top Min
					if (abs(dTopStressMin) > abs(PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Min))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Min = dTopStressMin;
						PgbrIRCGd.Fatg.TopFlange.TopSide.dTau1Min   = tau1Min;
						PgbrIRCGd.Fatg.TopFlange.TopSide.strMinLCB  = LcomebName;
						PgbrIRCGd.Fatg.TopFlange.TopSide.y1         = StressInfoDMin.FlgTR.dy1;
						PgbrIRCGd.Fatg.TopFlange.TopSide.z1         = StressInfoDMin.FlgTR.dz1;

					}
					//Top Flange Bot Min
					if (abs(dTopStressMin) > abs(PgbrIRCGd.Fatg.TopFlange.BotSide.dSigma2Min))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.TopFlange.BotSide.dSigma2Min = dTopStressMin;
						PgbrIRCGd.Fatg.TopFlange.BotSide.dTau2Min   = tau1Min;
						PgbrIRCGd.Fatg.TopFlange.BotSide.strMinLCB  = LcomebName;
						PgbrIRCGd.Fatg.TopFlange.BotSide.y1         = StressInfoDMin.FlgTR.dy2;
						PgbrIRCGd.Fatg.TopFlange.BotSide.z1         = StressInfoDMin.FlgTR.dz2;

					}
					//Bottom Flange 
				 //Bottom Flange Top Max
					if (abs(dBotStressMax) > abs(PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Max))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Max = dBotStressMax;
						PgbrIRCGd.Fatg.BotFlange.TopSide.dTau1Max   = tau1Max;
						PgbrIRCGd.Fatg.BotFlange.TopSide.strMaxLCB  = LcomebName;
						PgbrIRCGd.Fatg.BotFlange.TopSide.y1         = StressInfoDMax.FlgBR.dy1;
						PgbrIRCGd.Fatg.BotFlange.TopSide.z1         = StressInfoDMax.FlgBR.dz1;

					}
					//Bottom Flange Bot Max
					if (abs(dBotStressMax) > abs(PgbrIRCGd.Fatg.BotFlange.BotSide.dSigma2Max))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.BotFlange.BotSide.dSigma2Max = dBotStressMax;
						PgbrIRCGd.Fatg.BotFlange.BotSide.dTau2Max   = tau1Max;
						PgbrIRCGd.Fatg.BotFlange.BotSide.strMaxLCB  = LcomebName;
						PgbrIRCGd.Fatg.BotFlange.BotSide.y1         = StressInfoDMax.FlgBR.dy2;
						PgbrIRCGd.Fatg.BotFlange.BotSide.z1         = StressInfoDMax.FlgBR.dz2;

					}
					//Bottom Flange Top Min
					if (abs(dBotStressMin) > abs(PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Min))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Min = dBotStressMin;
						PgbrIRCGd.Fatg.BotFlange.TopSide.dTau1Min   = tau1Min;
						PgbrIRCGd.Fatg.BotFlange.TopSide.strMinLCB  = LcomebName;
						PgbrIRCGd.Fatg.BotFlange.TopSide.y1         = StressInfoDMin.FlgBR.dy1;
						PgbrIRCGd.Fatg.BotFlange.TopSide.z1         = StressInfoDMin.FlgBR.dz1;

					}
					//Bottom Flange Bot Min
					if (abs(dBotStressMin) > abs(PgbrIRCGd.Fatg.BotFlange.BotSide.dSigma2Min))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.BotFlange.BotSide.dSigma2Min = dBotStressMin;
						PgbrIRCGd.Fatg.BotFlange.BotSide.dTau2Min   = tau1Min;
						PgbrIRCGd.Fatg.BotFlange.BotSide.strMinLCB  = LcomebName;
						PgbrIRCGd.Fatg.BotFlange.BotSide.y1         = StressInfoDMin.FlgBR.dy2;
						PgbrIRCGd.Fatg.BotFlange.BotSide.z1         = StressInfoDMin.FlgBR.dz2;

					}
					//Web
					//Web Top Max
					if (abs(dWebStressMax) > abs(PgbrIRCGd.Fatg.Web.TopSide.dSigma1Max))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.Web.TopSide.dSigma1Max = dWebStressMax;
						PgbrIRCGd.Fatg.Web.TopSide.dTau1Max   = tau1Max;
						PgbrIRCGd.Fatg.Web.TopSide.strMaxLCB  = LcomebName;
						PgbrIRCGd.Fatg.Web.TopSide.y1         = StressInfoDMax.Web.dy1;
						PgbrIRCGd.Fatg.Web.TopSide.z1         = StressInfoDMax.Web.dz1;

					}
					//WebBot Max
					if (abs(dWebStressMax) > abs(PgbrIRCGd.Fatg.Web.BotSide.dSigma2Max))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.Web.BotSide.dSigma2Max = dWebStressMax;
						PgbrIRCGd.Fatg.Web.BotSide.dTau2Max   = tau1Max;
						PgbrIRCGd.Fatg.Web.BotSide.strMaxLCB  = LcomebName;
						PgbrIRCGd.Fatg.Web.BotSide.y1         = StressInfoDMax.Web.dy2;
						PgbrIRCGd.Fatg.Web.BotSide.z1         = StressInfoDMax.Web.dz2;

					}
					//Web Top Min
					if (abs(dWebStressMin) > abs(PgbrIRCGd.Fatg.Web.TopSide.dSigma1Min))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.Web.TopSide.dSigma1Min = dWebStressMin;
						PgbrIRCGd.Fatg.Web.TopSide.dTau1Min   = tau1Min;
						PgbrIRCGd.Fatg.Web.TopSide.strMinLCB  = LcomebName;
						PgbrIRCGd.Fatg.Web.TopSide.y1         = StressInfoDMin.Web.dy1;
						PgbrIRCGd.Fatg.Web.TopSide.z1         = StressInfoDMin.Web.dz1;

					}
					//Web Bot Min
					if (abs(dWebStressMin) > abs(PgbrIRCGd.Fatg.Web.BotSide.dSigma2Min))
					{
						CString LcomebName = _T("");
						int iMaxmin;
						m_pForcCtrl->Get_LcomType(nStrnKey, LcomebName, iMaxmin);
						...
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);
						LcomebName = rDataTemp.LoadCombName;//LcomebName;
						PgbrIRCGd.Fatg.Web.BotSide.dSigma2Min = dWebStressMin;
						PgbrIRCGd.Fatg.Web.BotSide.dTau2Min   = tau1Min;
						PgbrIRCGd.Fatg.Web.BotSide.strMinLCB  = LcomebName;
						PgbrIRCGd.Fatg.Web.BotSide.y1         = StressInfoDMin.Web.dy2;
						PgbrIRCGd.Fatg.Web.BotSide.z1         = StressInfoDMin.Web.dz2;

					}

				}
				//Correction factor
				 T_CGFR_D CgfrD;CgfrD.Initialize();
				 if(m_pDoc->m_pAttrCtrl->GetCgfr(ElemK.first,CgfrD))
				 {
					 PgbrIRCGd.Fatg.dMewR      =  CgfrD.FRdata[nPosi-1].dCorrect_fact;
					 PgbrIRCGd.Fatg.dNscCorr   =  CpgdD.dLoadCycles;
					 PgbrIRCGd.Fatg.df         =  max(max(abs(PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Max),abs(PgbrIRCGd.Fatg.TopFlange.TopSide.dSigma1Min)),max(abs(PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Max),abs(PgbrIRCGd.Fatg.BotFlange.TopSide.dSigma1Min)));
					 PgbrIRCGd.Fatg.d27gammamft=  27. / m_dGamma_Mft;
					 PgbrIRCGd.Fatg.dNsc       =  2.E6;//CpgdD.dLoadCycles;
					 PgbrIRCGd.Fatg.dNscLim    = PgbrIRCGd.Fatg.df == 0.? 0: 5.E6 * pow(((27./m_dGamma_Mft)/(m_dGamma_Fft*PgbrIRCGd.Fatg.df )),3.);//CIVIL-3267

					 PgbrIRCGd.Fatg.dffn       =  CgfrD.FRdata[nPosi-1].dLamda_v2;
					 PgbrIRCGd.Fatg.dtfn       =  CgfrD.FRdata[nPosi-1].dLamda_v4;
					 PgbrIRCGd.Fatg.dff       =  PgbrIRCGd.Fatg.dffn * pow(5000000/PgbrIRCGd.Fatg.dNsc,1.0/3.0);
					 PgbrIRCGd.Fatg.dtauf       =  PgbrIRCGd.Fatg.dtfn * pow(5000000/PgbrIRCGd.Fatg.dNsc,1.0/5.0);//CIVIL-5418
					 PgbrIRCGd.Fatg.dfd        =  PgbrIRCGd.Fatg.dMewR* PgbrIRCGd.Fatg.dff /m_dGamma_Mft;
					 PgbrIRCGd.Fatg.dtaufd      =  PgbrIRCGd.Fatg.dMewR* PgbrIRCGd.Fatg.dtauf /m_dGamma_Mft;
					 PgbrIRCGd.Fatg.dfdlim      =  PgbrIRCGd.Fatg.df;
					 PgbrIRCGd.Fatg.dtaulim     =  max(max(abs(PgbrIRCGd.Fatg.Web.TopSide.dSigma1Max),abs(PgbrIRCGd.Fatg.Web.TopSide.dSigma1Min)),max(abs(PgbrIRCGd.Fatg.Web.TopSide.dSigma1Max),abs(PgbrIRCGd.Fatg.Web.TopSide.dSigma1Min)));
				...
				{
                    CString strOrgLcomNa; int iOrgLcomNo; int iSerLcomCount; int iMaxMinType;
                    m_pForcCtrl->Get_LcomType(i+1,strOrgLcomNa,iOrgLcomNo,iSerLcomCount,iMaxMinType);
					int iTServLcomType = m_pForcCtrl->Get_ServLcomType(i+1);
                    if (iTServLcomType==0) continue;
                    
					//Part wise Stresses
					T_LCOM_D LcomData;      
					m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL, iOrgLcomNo, LcomData);
					m_pDoc->m_pPostCtrl->SelectLoadCombNew(LcomData, 0, 0);
					T_STRB_D StrbD;StrbD.Initialize();
					m_pDoc->m_pPostCtrl->GetStrbPartNew(ElemK.first, 1, &StrbD, 0, 0);
					//StrbD.GetMaximumStress_Torsional()
					T_STRB_D StrbD1;StrbD1.Initialize();
					m_pDoc->m_pPostCtrl->GetStrbPartNew(ElemK.first, 2, &StrbD1, 0, 0);
					StrbD1.calcMaximumCombinedStress();
					T_STRB_D StrbD2;StrbD2.Initialize();
					m_pDoc->m_pPostCtrl->GetStrbNew(ElemK.first, &StrbD2,  NULL, NULL, TRUE, NULL, ElemK.second);
					if(iTServLcomType==3) // Characteristic
					{
						
						//Check worst Stress
						ADGNSTRESS aStressLcomTemp;
						ADGNSTRSADD aStressAddTemp;    
						if(!m_pForcCtrl->Get_LcomDataForStress(ElemK, TRUE, FALSE, aStressLcomTemp, aStressAddTemp, i+1))	{continue;}
						for (int p=0;p<aStressLcomTemp.GetSize();p++)
						{
							_DGN_STRS_CRC& StressCrc = aStressLcomTemp[i];
							double dTopStressMax = k == 1 ?  max(abs(StressCrc.dStl[nPosi]),abs(StressCrc.dStr[nPosi])): max(abs(StressCrc.dStl[nPosi-1]),abs(StressCrc.dStr[nPosi-1]));
							double dBotStressMax = k == 1 ?  max(abs(StressCrc.dSbl[nPosi]),abs(StressCrc.dSbr[nPosi])): max(abs(StressCrc.dSbl[nPosi-1]),abs(StressCrc.dSbr[nPosi-1]));
							double dMaxStress = 0.;
							CString LcomebName = _T("");
							if (abs(dTopStressMax)>abs(dBotStressMax) ) {dMaxStress = dTopStressMax;}
							else {dMaxStress = dBotStressMax;}
							if (abs(dMaxStress) > abs( PgbrIRCGd.Fatg.dSigmaEdSer))
							{
								
								int iMaxmin;
								m_pForcCtrl->Get_LcomType(i+1, LcomebName, iMaxmin);

								PgbrIRCGd.Fatg.strLCLcomb = LcomebName;
								PgbrIRCGd.Fatg.dSigmaEdSer = dMaxStress;

								_DGN_FORC_CRC MembEndForce;    
								m_pForcCtrl->Get_LcomDataForDesign_LcomK(ElemK, iOrgLcomNo, TRUE, FALSE, MembEndForce);
								...
								double ddc  = GirderStiff.Czp + dHh + 0.5*dtc;
								double dQMax = Get_Q_FirstMomentOfGirderCentroid(aSize,GirderStiff.Czp,ddc);
								PgbrIRCGd.Fatg.dTauEdser = PgbsBase.ChkGirder.dtau_w;//Get_ShearStress(dMax,dQMax,GirderStiff.Ryy,dTw);

							}
							//For Part 2 (Concrete part)
							double dVal = k == 1 ? StrbD1.dblStress[3][14]:StrbD1.dblStress[0][14];
							if (abs(dVal) > abs( PgbrIRCGd.Fatg.dSigmac))
							{
								PgbrIRCGd.Fatg.dSigmac          = dVal;
								PgbrIRCGd.Fatg.dk1              = CpgdD.dk1;
								PgbrIRCGd.Fatg.strLimitRF       = LcomebName ;
							}
							//Reinforcement
							if  (abs( PgbsBase.ChkBar.dStress_bar ) > abs( PgbrIRCGd.Fatg.dSigmaRF))
							{
								PgbrIRCGd.Fatg.dSigmaRF          = PgbsBase.ChkBar.dStress_bar;
                                PgbrIRCGd.Fatg.dk3              = CpgdD.dk2;
							}
					...
					else if (iTServLcomType==2)//Frequent
					{
						T_ELEM_D ElemD;
						m_pDoc->m_pAttrCtrl->GetElem(ElemK.first, ElemD);
						T_NODE_K NodeK1 = ElemD.elnod[nPosi];            

						//           m_pPostCtrl->SelectLoadCombNew(LcomD);
						//           m_pPostCtrl->GetDispNew(NodeK1, &Node1MaxDisp, &Node1MinDisp, NULL);
						//           m_pPostCtrl->GetDispNew(NodeK2, &Node2MaxDisp, &Node2MinDisp, NULL);
						T_LCOM_D LcomData;      
						m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL, iOrgLcomNo, LcomData);

						T_STLD_D Lcase;
						Lcase.Initialize();
						int aSIze = LcomData.aCombination.GetSize();
						BOOL bDL=FALSE,bLL=FALSE,bImpact=FALSE,bSIDL=FALSE;
						for (int iSz=0;iSz<aSIze;iSz++)
						{
                           unsigned int iLcaseKey = LcomData.aCombination[iSz].LoadCaseKey;
                           unsigned int iAnalType = LcomData.aCombination[iSz].AnalType;
                           ...
                           if(iAnalType==D_LCOM_STATIC)
                           {
                               ...
                               T_STLD_D StldD;
                               StldD.Initialize();
                               if(!m_pDoc->m_pPostCtrl->GetStld(iLcaseKey,StldD))	{ASSERT(0); continue;}
                               if(StldD.LoadCaseType == _T("D")  ) bDL = TRUE; // Dead Load.
                               else if(StldD.LoadCaseType == _T("L")) bLL = TRUE; 
                               else if(StldD.LoadCaseType == _T("DW") || Lcase.LoadCaseType == _T("DC") ) bSIDL = TRUE; 
                               else if(StldD.LoadCaseType == _T("IL")) bImpact = TRUE; 
						...
						T_DISP_D DMax1, DMin1;
						//if(!m_pDoc->m_pPostCtrl->GetDispNew_Lcom(LcomData, NodeK1, DMax, DMin)) return FALSE;
						double dMaxDeflnNodes = 0.;

						m_pDoc->m_pPostCtrl->GetDispNew(NodeK1, &DMax1, &DMin1, NULL);              
						dMaxDeflnNodes = DMax1.dblDisp[2];
						
						if (abs(dMaxDeflnNodes) != abs(PgbrIRCGd.Fatg.dDeltaDL) && abs(dMaxDeflnNodes) > abs(PgbrIRCGd.Fatg.dDeltaDL))
						{
							CString LcomebName = _T("");
							int iMaxmin;
							m_pForcCtrl->Get_LcomType(iOrgLcomNo, LcomebName, iMaxmin);
							PgbrIRCGd.Fatg.iDeflCtrlLCNum=iOrgLcomNo;
							PgbrIRCGd.Fatg.strLCDefl = strOrgLcomNa;
							PgbrIRCGd.Fatg.dDeltaDL  = dMaxDeflnNodes;

						}

					}
					else if (iTServLcomType==1)//Quasi Permanant
					{
						//Check worst Stress
						ADGNSTRESS aStressLcomTemp;
						ADGNSTRSADD aStressAddTemp;    
						if(!m_pForcCtrl->Get_LcomDataForStress(ElemK, TRUE, FALSE, aStressLcomTemp, aStressAddTemp, iOrgLcomNo))	{continue;}
						for (int p=0;p<aStressLcomTemp.GetSize();p++)
						{
							_DGN_STRS_CRC& StressCrc = aStressLcomTemp[p];
							double dTopStressMax = 0.5*(StressCrc.dStl[nPosi] + StressCrc.dStr[nPosi] );
							double dBotStressMax = 0.5*(StressCrc.dSbl[nPosi] + StressCrc.dSbr[nPosi]);
							double dMaxStress = 0.;
							if (abs(dTopStressMax)>abs(dBotStressMax) ) {dMaxStress = dTopStressMax;}
							else {dMaxStress = dBotStressMax;}
							if (abs(dMaxStress) > abs( PgbrIRCGd.Fatg.dSigmaS))
							{
								CString LcomebName = _T("");
								int iMaxmin;
								m_pForcCtrl->Get_LcomType(i+1, LcomebName, iMaxmin);

								PgbrIRCGd.Fatg.strLCBTop = LcomebName;
								PgbrIRCGd.Fatg.strLCBBot = LcomebName;
								 
								PgbrIRCGd.Fatg.dSigmaTop = 0.5*( PgbsBase.ChkSlab.aForce[0]+PgbsBase.ChkSlab.aForce[1]);
								PgbrIRCGd.Fatg.dSigmaBot = 0.5*( PgbsBase.ChkSlab.aForce[2]+PgbsBase.ChkSlab.aForce[3]);

								_DGN_FORC_CRC MembEndForce;    
								m_pForcCtrl->Get_LcomDataForDesign_LcomK(ElemK, i+1, TRUE, FALSE, MembEndForce);
								double dMax = max( (MembEndForce.dFzz[0]),( MembEndForce.dFzz[1]));
								...
								double ddc  = GirderStiff.Czp + dHh + 0.5*dtc;
								double dQMax = Get_Q_FirstMomentOfGirderCentroid(aSize,GirderStiff.Czp,ddc);
								PgbrIRCGd.Fatg.dSigmaS = PgbsBase.ChkGirder.dtau_w;//Get_ShearStress(dMax,dQMax,GirderStiff.Ryy,dTw);
							}
				...
				PgbrIRCGd.Fatg.dSigmaEdSerLim   = 0.87 * m_dfy / m_dGamma_M_ser;
				PgbrIRCGd.Fatg.dTauEdserLim     = 0.87 * m_dfy / (sqrt(3.)*m_dGamma_M_ser);
                PgbrIRCGd.Fatg.dTotSqrt         = sqrt(pow(PgbrIRCGd.Fatg.dSigmaEdSer ,2) +3.* pow(PgbrIRCGd.Fatg.dTauEdser ,2 ) );
				//In the concrete of the slab
				PgbrIRCGd.Fatg.strLCConc        = PgbrIRCGd.Fatg.strLCLcomb ;
				//calculation dSigmac   and SigmaS
				double dSigmaC_Conc = (PgbrIRCGd.Fatg.dSigmaEdSer * (0.5*m_pDgnSectUtil->Get_Size_Ht(&SectD, bPosiI)) / (0.5*m_pDgnSectUtil->Get_Size_H(&SectD, bPosiI))) - PgbrIRCGd.Fatg.dSigmaEdSer;
				double dSigmaS_Steel= PgbrIRCGd.Fatg.dSigmaEdSer + (PgbrIRCGd.Fatg.dSigmaEdSer*(m_pDgnSectUtil->Get_Size_H(&SectD, bPosiI)+PgbrIRCGd.SectProp.dBotCover)/m_pDgnSectUtil->Get_Size_H(&SectD, bPosiI)) ;
				
				
				PgbrIRCGd.Fatg.dk1fck              = dk1*m_dfck;
				
				
                PgbrIRCGd.Fatg.dk3fyk              = PgbrIRCGd.Fatg.dk3*m_dfyk;
				
				//Deflection Control
				PgbrIRCGd.Fatg.dPhis            = CpgdD.dSteelStressLim == 0.87 ? 600 :CpgdD.dSteelStressLim;
                PgbrIRCGd.Fatg.dDeltaDLLimit    = PgbrIRCGd.SectProp.iEffWidthCase==0 ? 0.:(PgbrIRCGd.SectProp.dSLlo/PgbrIRCGd.Fatg.dPhis);
                
				//Crack Width
				PgbrIRCGd.Fatg.dfcteff          =  MatlElem.dfctm;
				PgbrIRCGd.Fatg.dEcm             =  MatlElem.dEcm;
				PgbrIRCGd.Fatg.dEs             =  m_dEs;
				PgbrIRCGd.Fatg.dAs              =   PgbrIRCGd.LotrShear.dAsl ;
				PgbrIRCGd.Fatg.dAceff              =   m_dBeffLength *m_PgbrBase.dXpl ;
				PgbrIRCGd.Fatg.dRhopeff         = PgbrIRCGd.Fatg.dAceff == 0. ? 0:PgbrIRCGd.Fatg.dAs /PgbrIRCGd.Fatg.dAceff ;
				PgbrIRCGd.Fatg.dkt              = 0.5;
				//calculating S_max
				PgbrIRCGd.Fatg.dslimit          =   5. * (PgbrIRCGd.Fatg.dc + 0.5*PgbrIRCGd.Fatg.dphi);
				PgbrIRCGd.Fatg.dk1spacing       =   0.8 ; 
				PgbrIRCGd.Fatg.dk2spacing       =   0.5 ; 
				PgbrIRCGd.Fatg.dk3spacing       =   CpgdD.dConcStressLim ; 
				PgbrIRCGd.Fatg.dk4spacing       =   CpgdD.dk3 ; 
				PgbrIRCGd.Fatg.dSrmax           =   PgbrIRCGd.Fatg.dRhopeff  == 0. ? 0:(PgbrIRCGd.Fatg.dk3spacing *PgbrIRCGd.Fatg.dc ) + (PgbrIRCGd.Fatg.dk1spacing*PgbrIRCGd.Fatg.dk2spacing*PgbrIRCGd.Fatg.dk4spacing*PgbrIRCGd.Fatg.dphi/PgbrIRCGd.Fatg.dRhopeff);
				PgbrIRCGd.Fatg.dEcmminusEcm     =   (PgbrIRCGd.Fatg.dSigmaS  - (PgbrIRCGd.Fatg.dkt*(PgbrIRCGd.Fatg.dfcteff/PgbrIRCGd.Fatg.dRhopeff)*(1.0 +(PgbrIRCGd.Fatg.dEs*PgbrIRCGd.Fatg.dEcm/PgbrIRCGd.Fatg.dEs) ))) / PgbrIRCGd.Fatg.dEs ;
				PgbrIRCGd.Fatg.dX               =   0.6 * PgbrIRCGd.Fatg.dSigmaS  / PgbrIRCGd.Fatg.dEs;
				PgbrIRCGd.Fatg.dWk              =   PgbrIRCGd.Fatg.dSrmax *min(abs(PgbrIRCGd.Fatg.dEcmminusEcm),PgbrIRCGd.Fatg.dX  );
				PgbrIRCGd.Fatg.nCrackClass      =   CpgdD.nConsiderPv;

				if      (CpgdD.nConsiderPv == 0)  PgbrIRCGd.Fatg.dWkmax = 0.3;
				else if (CpgdD.nConsiderPv == 1)  PgbrIRCGd.Fatg.dWkmax = 0.3;
				else      PgbrIRCGd.Fatg.dWkmax = 0.2;
...
void CDgnPlateGirder_IRC22_15::ConvertCsgCalcCgtsData(T_CGTS_SSM_D *pCgtsSSM, CSG_TRST_SECT &rData)
{
...
void CDgnPlateGirder_IRC22_15::ConvertCsgCalcCgtsPosData(T_CGTS_SUB_SSM &CgtsSub, CSG_TRST_POSD &rData)
{
	...
	BOOL CDgnPlateGirder_IRC22_15::Get_Elastic_Steel_Results(double bf_top,double tw,double bf_bottom,double tf_top, double dw,double tf_bottom,T_PG_IRC22_15_ELP& SectProp )
	{

		// Elastic Steel section // Function  //
		double Flangetop_L=bf_bottom,
			Web_L=tw,
			FlangeBot_L =bf_top ;

		double Flangetop_D= tf_bottom,
			Web_D=dw,
			FlangeBot_D=tf_top;

		double Flangetop_Iself=Flangetop_L*pow(Flangetop_D,3)/12,
			Web_Iself=Web_L*pow(Web_D,3)/12,
			FlangeBot_Iself=FlangeBot_L*pow(FlangeBot_D,3)/12;

		double Flangetop_Yx=Flangetop_D/2+Web_D+FlangeBot_D,
			Web_Yx=Web_D/2+FlangeBot_D,
			FlangeBot_Yx=FlangeBot_D/2;

		double Flangetop_A=Flangetop_L*Flangetop_D,
			Web_A=Web_L*Web_D,
			FlangeBot_A=FlangeBot_L*FlangeBot_D,
			Total_A =Flangetop_A+Web_A+FlangeBot_A;

		double Flangetop_AYx=Flangetop_Yx*Flangetop_A,
			Web_AYx=Web_Yx*Web_A,
			FlangeBot_AYx=FlangeBot_Yx*FlangeBot_A,
			Total_AYx =Flangetop_AYx+Web_AYx+FlangeBot_AYx;

		double Flangetop_Yx_=Total_AYx/Total_A,
			Web_Yx_=Total_AYx/Total_A,
			FlangeBot_Yx_=Total_AYx/Total_A;

		double Flangetop_Ixx=Flangetop_Iself+Flangetop_A*pow((Flangetop_Yx-Flangetop_Yx_),2),
			Web_Ixx=Web_Iself+Web_A*pow((Web_Yx-Web_Yx_),2),
			FlangeBot_Ixx=FlangeBot_Iself+FlangeBot_A*pow((FlangeBot_Yx-FlangeBot_Yx_),2),
			Total_Ixx=Flangetop_Ixx+Web_Ixx+FlangeBot_Ixx;

		double  Flangetop_Yy=0,
			Web_Yy=0,
			FlangeBot_Yy=0;

		double  Flangetop_AYy2=Flangetop_A*Flangetop_Yy*Flangetop_Yy,
			Web_AYy2=Web_A*Web_Yy*Web_Yy,
			FlangeBot_AYy2=FlangeBot_A*FlangeBot_Yy*FlangeBot_Yy;

		double Flangetop_Iyy=Flangetop_D*pow(Flangetop_L,3)/12 + Flangetop_AYy2 ,
			Web_Iyy=Web_D*pow(Web_L,3)/12 + Web_AYy2 ,
			FlangeBot_Iyy=FlangeBot_D*pow(FlangeBot_L,3)/12 +FlangeBot_AYy2 ,
			Total_Iyy =Flangetop_Iyy+Web_Iyy+FlangeBot_Iyy;

		double yx_bot=Total_AYx/Total_A,
			yx_top = Flangetop_D+Web_D+FlangeBot_D-yx_bot,
			IgrossX =Total_Ixx,
			Ztop = IgrossX/yx_top,
			ZBottom = IgrossX/yx_bot,
			rxx = sqrt(IgrossX/Total_A),
			ryy = sqrt(Total_Iyy/Total_A);    



		SectProp.dArea =  Total_A; 
		SectProp.dIyy  =  IgrossX;
		SectProp.dIzz  =  Total_Iyy;
			SectProp.dtop  =  yx_top ;
			SectProp.dbot   = yx_bot ;
			SectProp.dstop  = Ztop;
			SectProp.dsbot  = ZBottom;
	...
	BOOL CDgnPlateGirder_IRC22_15::Get_Plastic_Steel_Results(double bf_top,double tw,double bf_bottom,double tf_top, double dw,double tf_bottom,T_PG_IRC22_15_ELP& SectProp )
	{

		// Elastic Steel section // Function  //
		double Flangetop_b= bf_bottom,
			Web_b=tw,
			FlangeBot_b=bf_top;
		...
		double Flanfetop_sumd = Flangetop_d,
			web_sumd = Flangetop_d+Web_d,
			Flangebot_sumd = Flangetop_d+Web_d+FlangeBot_d;

		double Flangetop_A=Flangetop_b*Flangetop_d,
			Web_A=Web_b*Web_d,
			FlangeBot_A=FlangeBot_b*FlangeBot_d,
			Total_A =Flangetop_A+Web_A+FlangeBot_A;

		double Flangetop_SumA=Flangetop_A,
			Web_SumA=Flangetop_A+Web_A,
			FlangeBot_SumA=Web_SumA+FlangeBot_A,
			Total_SumA =Flangetop_SumA+Web_SumA+FlangeBot_SumA;

		double dA_by2= 0.5*Total_A;

		int ival1=0,ival2=0,ival3=0;
		double dval1=0.,dval2=0,dval3=0;

		if (Flangetop_SumA>dA_by2) ival1=1;
		if (ival1==1)   dval1 = dA_by2/Flangetop_d;
		
		if (Web_SumA>dA_by2 && ival1==0) ival2=1;
		if (ival2==0 && ival1==1) dval2= 0;
		else dval2=(dA_by2-Flangetop_SumA)/Web_b;
		
		
		if (FlangeBot_SumA>dA_by2 && ival1==0 && ival2==0) ival3=1;
		if (ival3==0 && ival2==1) dval3= 0;
		else dval3=(dA_by2-Web_SumA)/FlangeBot_b;

		double dFlangeTop_EATOP = dval1>0?dval1:0;
		double dWeb_EATOP = dval2>0? dval2+Flanfetop_sumd:0;
		double dFlangeBot_EATOP = dval3>0? dval3+web_sumd:0;

		double dybartop = dFlangeTop_EATOP+dWeb_EATOP+dFlangeBot_EATOP;
		double dyabrbot = Flangetop_d+Web_d+FlangeBot_d-dybartop;

		double dFlangeTop_y = ival1 == 0 ? (dybartop-0.5*Flangetop_d):dybartop;
		double dFWeb_y = ival2 == 0 ? 0:abs(dybartop-0.5*Web_d-Flangetop_d);
		double dFlangeBot_y = ival3 == 0 ? abs(dybartop-0.5*FlangeBot_d-Web_d-Flangetop_d):abs(dybartop-Web_d-Flangetop_d);

		double dWeb_bd2by4 = ival2 == 0 ?  dFWeb_y*Web_A : ((Web_b*dFWeb_y*dFWeb_y/4)+(Web_b*pow((Web_d-dFWeb_y),2)/4));

		double dFlangeTop_Ay = dFlangeTop_y*Flangetop_A;
		double dFlangeBot_Ay = dFlangeBot_y*FlangeBot_A;

		SectProp.dzp    = dFlangeTop_Ay+dWeb_bd2by4+dFlangeBot_Ay;
	...
	double CDgnPlateGirder_IRC22_15::Get_ddcfromybar(double bf_top,double tw,double bf_bottom,double tf_top, double dw,double tf_bottom,T_PG_IRC22_15_ELP& SectProp )
	{

		// Elastic Steel section // Function  //
		double Flangetop_b=bf_top,
			Web_b=tw,
			FlangeBot_b=bf_bottom;

		double Flangetop_d=tf_top,
			Web_d=dw,
			FlangeBot_d=tf_bottom;

		double Flanfetop_sumd = tf_top,
			web_sumd = tf_top+Web_d,
			Flangebot_sumd = tf_top+Web_d+FlangeBot_d;
		...
		double dybartop = dFlangeTop_EATOP+dWeb_EATOP+dFlangeBot_EATOP;
		double dyabrbot = tf_top+Web_d+FlangeBot_d-dybartop;

		double dFlangeTop_y = ival1 == 0 ? (dybartop-0.5*tf_top):dybartop;
		double dFWeb_y = ival2 == 0 ? 0:abs(dybartop-0.5*Web_d-tf_top);
		double dFlangeBot_y = ival3 == 0 ? abs(dybartop-0.5*FlangeBot_d-Web_d-tf_top):abs(dybartop-Web_d-tf_top);
	...
	BOOL CDgnPlateGirder_IRC22_15::Get_Short_Term_Composite_Results(double Equ_width_steel_trans,double bf_top,double tw,double bf_bottom,double dtc,double tf_top, double dw,double tf_bottom,double dModular_ratio_Trans,double slab_width,T_PG_IRC22_15_ELP& SectProp )
	{
		double Slab_L=Equ_width_steel_trans,
			Flangetop_L=bf_bottom ,
			Web_L=tw,
			FlangeBot_L=bf_top;

		double Slab_D=dtc,
			Flangetop_D= tf_bottom,
			Web_D=dw,
			FlangeBot_D=tf_top;

		double Slab_Iself=Slab_L*pow(Slab_D,3)/12,
			Flangetop_Iself= Flangetop_L*pow(Flangetop_D,3)/12,
			Web_Iself=Web_L*pow(Web_D,3)/12,
			FlangeBot_Iself=FlangeBot_L*pow(FlangeBot_D,3)/12;

		double Slab_Yx =Slab_D/2+Flangetop_D+Web_D+FlangeBot_D,
			Flangetop_Yx=Flangetop_D/2+Web_D+FlangeBot_D,
			Web_Yx=Web_D/2+FlangeBot_D,
			FlangeBot_Yx=FlangeBot_D/2;

		double Slab_A=Slab_L*Slab_D,
			Flangetop_A=Flangetop_L*Flangetop_D,
			Web_A=Web_L*Web_D,
			FlangeBot_A=FlangeBot_L*FlangeBot_D,
			Total_A =Slab_A+Flangetop_A+Web_A+FlangeBot_A;

		double Slab_AYx=Slab_Yx*Slab_A,
			Flangetop_AYx=Flangetop_Yx*Flangetop_A,
			Web_AYx=Web_Yx*Web_A,
			FlangeBot_AYx=FlangeBot_Yx*FlangeBot_A,
			Total_AYx =Slab_AYx+Flangetop_AYx+Web_AYx+FlangeBot_AYx;

		double Slab_Yx_=Total_AYx/Total_A,
			Flangetop_Yx_=Total_AYx/Total_A,
			Web_Yx_=Total_AYx/Total_A,
			FlangeBot_Yx_=Total_AYx/Total_A;

		double  Slab_Ixx=Slab_Iself+Slab_A*pow((Slab_Yx-Slab_Yx_),2),
			Flangetop_Ixx=Flangetop_Iself+Flangetop_A*pow((Flangetop_Yx-Flangetop_Yx_),2),
			Web_Ixx=Web_Iself+Web_A*pow((Web_Yx-Web_Yx_),2),
			FlangeBot_Ixx=FlangeBot_Iself+FlangeBot_A*pow((FlangeBot_Yx-FlangeBot_Yx_),2),
			Total_Ixx=Slab_Ixx+Flangetop_Ixx+Web_Ixx+FlangeBot_Ixx;
		...
		double Slab_AYy2=Slab_A*Slab_Yy*Slab_Yy,
			Flangetop_AYy2=Flangetop_A*Flangetop_Yy*Flangetop_Yy,
			Web_AYy2=Web_A*Web_Yy*Web_Yy,
			FlangeBot_AYy2=FlangeBot_A*FlangeBot_Yy*FlangeBot_Yy;

		double Slab_Iyy=(Slab_D/dModular_ratio_Trans)*pow(slab_width,3)/12 + Slab_AYy2 ,  // Slab width
			Flangetop_Iyy=Flangetop_D*pow(Flangetop_L,3)/12 + Flangetop_AYy2 ,
			Web_Iyy=Web_D*pow(Web_L,3)/12 + Web_AYy2 ,
			FlangeBot_Iyy=FlangeBot_D*pow(FlangeBot_L,3)/12 +FlangeBot_AYy2 ,
			Total_Iyy =Slab_Iyy+Flangetop_Iyy+Web_Iyy+FlangeBot_Iyy;

		double yx_bot=Total_AYx/Total_A,
			yx_top = Flangetop_D+Web_D+FlangeBot_D-yx_bot,
			IgrossX =Total_Ixx,
			Ztop_slab= IgrossX/yx_top,
			Ztop = IgrossX/yx_top,
			ZBottom = IgrossX/yx_bot,
			rxx = sqrt(IgrossX/Total_A),
			ryy = sqrt(Total_Iyy/Total_A);
	...
	BOOL CDgnPlateGirder_IRC22_15::Get_Long_Term_Composite_Results(double Equ_width_steel_pre,double bf_top,double tw,double bf_bottom,double dtc,double tf_top, double dw,double tf_bottom,double dModular_ratio_per,double slab_width,T_PG_IRC22_15_ELP& SectProp )
	{
		double Slab_L=Equ_width_steel_pre,
			Flangetop_L=bf_bottom ,
			Web_L=tw,
			FlangeBot_L=bf_top;
		...
		double Slab_Iyy=(Slab_D/dModular_ratio_per)*pow(slab_width,3)/12 + Slab_AYy2 ,  // Slab width 
			Flangetop_Iyy=Flangetop_D*pow(Flangetop_L,3)/12 + Flangetop_AYy2 ,
			Web_Iyy=Web_D*pow(Web_L,3)/12 + Web_AYy2 ,
			FlangeBot_Iyy=FlangeBot_D*pow(FlangeBot_L,3)/12 +FlangeBot_AYy2 ,
			Total_Iyy =Slab_Iyy+Flangetop_Iyy+Web_Iyy+FlangeBot_Iyy;
	...
	BOOL  CDgnPlateGirder_IRC22_15::Get_Cracked_Results(double bf_top,double tw,double bf_bottom,double dtc,double tf_top, double dw,double tf_bottom,double dModular_ratio_Trans,double slab_width,double dTopDia,double dTopNo,double dBotNo,double dBotSpac, double dTopSpac, double dTopCover,double dHh,double d_girder,double dBotDia,T_PG_IRC22_15_ELP& SectProp )
	{
		...
		double
			RTop_Iself=dTopNo*(atan(1)*4/4)*pow(dTopDia/2,4),
			RBot_Iself=dBotNo*(atan(1)*4/4)*pow(dBotDia/2,4),
			Flangetop_Iself= Flangetop_L*pow(Flangetop_D,3)/12,
			Web_Iself=Web_L*pow(Web_D,3)/12,
			FlangeBot_Iself=FlangeBot_L*pow(FlangeBot_D,3)/12;

		double RTop_Yx =d_girder+Slab_D+dHh-dTopCover,
			RBot_Yx = d_girder+dHh+dTopCover,
			Flangetop_Yx=Flangetop_D/2+Web_D+FlangeBot_D,
			Web_Yx=Web_D/2+FlangeBot_D,
			FlangeBot_Yx=FlangeBot_D/2;

		double RTop_A =dTopNo*(atan(1)*4/4)*dTopDia*dTopDia,
			RBot_A = dBotNo*(atan(1)*4/4)*dBotDia*dBotDia,
			Flangetop_A=Flangetop_L*Flangetop_D,
			Web_A=Web_L*Web_D,
			FlangeBot_A=FlangeBot_L*FlangeBot_D,
			Total_A =RTop_A+RBot_A+Flangetop_A+Web_A+FlangeBot_A;

		double  RTop_AYx =  RTop_Yx*RTop_A,
			RBot_AYx =  RBot_Yx*RBot_A,
			Flangetop_AYx=Flangetop_Yx*Flangetop_A,
			Web_AYx=Web_Yx*Web_A,
			FlangeBot_AYx=FlangeBot_Yx*FlangeBot_A,
			Total_AYx =RTop_AYx+RBot_AYx +Flangetop_AYx+Web_AYx+FlangeBot_AYx;

		double RTop_Yx_ = Total_AYx/Total_A,
			RBot_Yx_ = Total_AYx/Total_A,
			Flangetop_Yx_=Total_AYx/Total_A,
			Web_Yx_=Total_AYx/Total_A,
			FlangeBot_Yx_=Total_AYx/Total_A;

		double  RTop_Ixx = RTop_Iself+RTop_A*pow((RTop_Yx-RTop_Yx_),2),
			RBot_Ixx = RBot_Iself+RBot_A*pow((RBot_Yx-RBot_Yx_),2),
			Flangetop_Ixx=Flangetop_Iself+Flangetop_A*pow((Flangetop_Yx-Flangetop_Yx_),2),
			Web_Ixx=Web_Iself+Web_A*pow((Web_Yx-Web_Yx_),2),
			FlangeBot_Ixx=FlangeBot_Iself+FlangeBot_A*pow((FlangeBot_Yx-FlangeBot_Yx_),2),
			Total_Ixx=RTop_Ixx+RBot_Ixx+Flangetop_Ixx+Web_Ixx+FlangeBot_Ixx;
		...
		double RTop_AYy2 = RTop_A*RTop_Yy*RTop_Yy,
			RBot_AYy2 =RBot_A*RBot_Yy*RBot_Yy,
			Flangetop_AYy2=Flangetop_A*Flangetop_Yy*Flangetop_Yy,
			Web_AYy2=Web_A*Web_Yy*Web_Yy,
			FlangeBot_AYy2=FlangeBot_A*FlangeBot_Yy*FlangeBot_Yy;

		double  RTop_Iyy = dTopNo < 0.0001 ? RTop_Iself : RTop_Iself   +  2*(RTop_A/dTopNo)    *(pow((dTopSpac/2),2))    *    (dTopNo/2)     *       (2*(dTopNo/2) +1)    *     (2*(dTopNo/2)-1)/3  ,
			RBot_Iyy = dBotNo < 0.00001 ? RBot_Iself : RBot_Iself+2*(RBot_A/dBotNo)*(pow((dBotSpac/2),2))*(dBotNo/2)*(2*(dBotNo/2) +1)*(2*(dBotNo/2)-1)/3  ,
			Flangetop_Iyy=Flangetop_D*pow(Flangetop_L,3)/12 + Flangetop_AYy2 ,
			Web_Iyy=Web_D*pow(Web_L,3)/12 + Web_AYy2 ,
			FlangeBot_Iyy=FlangeBot_D*pow(FlangeBot_L,3)/12 +FlangeBot_AYy2 ,
			Total_Iyy =RTop_Iyy+RBot_Iyy+Flangetop_Iyy+Web_Iyy+FlangeBot_Iyy;
	...
	BOOL CDgnPlateGirder_IRC22_15::Set_Forc24(int iPosiNo, _DGN_FORC_CRC& ForcCrc2, _DGN_FORC_CRC& ForcCrc4)
	{
	...
	BOOL CDgnPlateGirder_IRC22_15::Cng_DgnForcCrc(_DGN_FORC_CRC& ForcCrc)
	{
		for(int i=0; i<cPOSI_CRC; i++)
		{
			ForcCrc.dFxx[i] = (ForcCrc.dFxx[i]);
			ForcCrc.dFyy[i] = (ForcCrc.dFyy[i]);
			ForcCrc.dFzz[i] = (ForcCrc.dFzz[i]);
			ForcCrc.dMux[i] = (ForcCrc.dMux[i]);
			ForcCrc.dMuy[i] = (ForcCrc.dMuy[i]);
			ForcCrc.dMuz[i] = (ForcCrc.dMuz[i]);
		}
		return TRUE;
	}

	 BOOL CDgnPlateGirder_IRC22_15::Get_Transverse_Stiffener_Sect_Prop(double dL[5],double dD[5],double dYy[5],double& dA1,double& dA2,double& dA3,double& dIyy1,double& dIyy2,double& dIyy3)
	 {
		 double dIself[5],dTempA[5],dTempAy2[5],dTempIyy[5];
		 dA1 = dA2 = dA3 = 0.;
		 dIyy1 = dIyy2 = dIyy3 =0.;

		 for(int i=0;i<5;i++)
		 {
			 dIself[i] = dD[i] * pow(dL[i],3) / 12. ;
			 dTempA[i] = dL[i] * dD[i] ;
			 dTempAy2[i] = dTempA[i] * pow(dYy[i],2);
			 dTempIyy[i] = dTempAy2[i] + dD[i] * pow(dL[i],3) / 12. ;
		 }
		 for(int i=0;i<=3;i++) dA1 += dTempA[i];
		 dA2 = dTempA[4];
		 dA3 = dA1+dA2;

		 for(int i=0;i<=3;i++) dIyy1 += dTempIyy[i];
		 dIyy2 = dTempIyy[4];
		 dIyy3 = dIyy2 + dIyy1;
	 ...
	 BOOL CDgnPlateGirder_IRC22_15::Get_CalcIRCData(ElemPairK ElemK, BOOL bPosiI, int iDgnSelLcomNo,T_PGBR_IRC_BASE& retPgbr_IRC)
	 {
		 BOOL bRebar=FALSE;
		 BOOL bShearConnector=FALSE;
		 BOOL bLongStiff=FALSE;
		 ...
		 CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&> arRbarPosi;
		 arRbarPosi.Copy(RbarElem.RbarPosi[k].arRbarPosi);    
		 // Longitudinal Stiffener  dTopCover
		 CArray<CPG_LSTI_DATA,CPG_LSTI_DATA&> arLstiPosi;
		 arLstiPosi.Copy(LstiElem.LstiPosi[k].arLstiPosi);
		 // Transverse Stiffener
		 CPG_TRST_POSI TrstPosi = TrstElem.TrstPosi[k];
         ...
         if (m_Pgbr_IRC_Base.LotrShear.strLcomName.GetLength()>0 && m_Pgbr_IRC_Base.Fatg.strLCLcomb.GetLength()>0)
         {
             retPgbr_IRC = m_Pgbr_IRC_Base;
         }
         else
         {
		 T_PGBR_IRC_BASE Pgbr_IRC_Temp;   
		 Get_SavedResult4Design(ElemK,bPosiI,m_PgbrBase,Pgbr_IRC_Temp);
             retPgbr_IRC = Pgbr_IRC_Temp;
         }
...
BOOL CDgnPlateGirder_IRC22_15::Get_PlasticNA_inclSectclass(BOOL bPosiI, BOOL bPositive, double dL,CPG_MATL_ELEM& MatlElem, const T_SECT_D& crSectD, 
											 CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, double& dPlasticNA,int iSectClass, BOOL bNeglectWeb)
{
	// 1. Initialize
	dPlasticNA = 0.0;
	double dPlasticNAIRC = 0.0;
	...
	double dConcF	=    dfcd* dBc*dtc;
	double dStopfF	=  (iSectClass==3 )? dB1Sect3*dTF1*dfyd:dB1*dTF1*dfyd;;//dB1*dTf1*dfyd;
	double dSbotfF	=  (iSectClass==3 && !bPositive)? dB2Sect3*dTF2*dfyd:dB2*dTF2*dfyd;//dB2*dTf2*dfyd;
	double dSwebF	=  (iSectClass==3 )? dhwSect3*dTw_t*dfyd:dhw*dTw_t*dfyd;//dH*dTw_t*dfyd;
...
void CDgnPlateGirder_IRC22_15::GetAllLoadcase(CStringArray &aLoadName, CArray<UINT, UINT> &aLoadData, 
																	 CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	BOOL bStageMode = m_pDoc->m_pStagCtrl->IsBaseStag() == FALSE;
	BOOL bFinalStage = m_pDoc->m_pStagCtrl->IsFinalStag();

	//GetAllCsld(bStageMode, bFinalStage, aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllStld(bStageMode, bFinalStage, aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllSplc(aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllEslc(aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllThis(aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllMvld(aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllSmlc(aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllGilc(aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllLcom(bStageMode, bFinalStage, aLoadName, aLoadData, aLkindData, aMinMaxData);
	GetAllEnvl(bStageMode, bFinalStage, aLoadName, aLoadData, aLkindData, aMinMaxData);
}

/*void CDgnPlateGirder_IRC22_15::GetAllCsld(BOOL bStageMode, BOOL bFinalStage, CStringArray &aLoadName, 
															 CArray<UINT, UINT> &aLoadData, CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	// construction loadcase
	if (bStageMode && !bFinalStage)
	{
		CArray<T_STLD_K, T_STLD_K> aSgld;
		T_STLD_K SgldKey;
		T_STLD_D SgldData;
		m_pDoc->m_pPostCtrl->GetStageInfo()->GetSgldKeyListPost(aSgld);
		for (int i = 0; i < aSgld.GetSize(); i++)
		{
			SgldKey = aSgld.GetAt(i);
			m_pDoc->m_pPostCtrl->GetStageInfo()->GetSgldPost(SgldKey, SgldData);
			aLoadName.Add(SgldData.LoadCaseName+_T("(CS)"));
			aLoadData.Add(SgldKey);
			aLkindData.Add(D_LOADCASE_STAGE);
			aMinMaxData.Add(2); // max 값 읽어와야 함
		}
	}
}
*/

void CDgnPlateGirder_IRC22_15::GetAllStld(BOOL bStageMode, BOOL bFinalStage, CStringArray &aLoadName, 
															 CArray<UINT, UINT> &aLoadData, CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	if (m_pDoc->m_pPostCtrl->GetPointDataCount()->m_nNumStld == 0) return;

	// static loadcase
	// 정적하중은 Stage 모드에서 뺀다.
	// Final Stage에서는 시공 타입의 정적 하중은 뺀다.
	if (!bStageMode || bFinalStage)
	{
		CArray<T_STLD_K, T_STLD_K> aStld;
		T_STLD_K StldKey;
		T_STLD_D StldData;
		...
		for (int i = 0; i < aStld.GetSize(); i++)
		{
			...
			if (bFinalStage && (StldData.LoadCaseType == _T("CS") || StldData.LoadCaseType == _T("TCS")) )
				continue;
			aLoadName.Add(StldData.LoadCaseName+_T("(ST)"));
			aLoadData.Add(StldKey);
			aLkindData.Add(D_LOADCASE_STATIC);
			aMinMaxData.Add(2); // max 값 읽어와야 함
		}
	}
}

void CDgnPlateGirder_IRC22_15::GetAllSplc(CStringArray &aLoadName, CArray<UINT, UINT> &aLoadData, 
															 CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	if (m_pDoc->m_pPostCtrl->GetPointDataCount()->m_nNumSplc == 0) return;

	// response spectrum loadcase
	CArray<T_SPLC_K, T_SPLC_K> aSplc;
	T_SPLC_K SplcKey;
	T_SPLC_D SplcData;
	m_pDoc->m_pAttrCtrl->GetSplcKeyList(aSplc);
	for (int i = 0; i < aSplc.GetSize(); i++)
	{
		SplcKey = aSplc.GetAt(i);
		m_pDoc->m_pAttrCtrl->GetSplc(SplcKey, SplcData);
		aLoadName.Add(SplcData.LoadCaseName+_LS(IDS_TB_RESULTVIEW__RS_));
		aLoadData.Add(SplcKey);
		aLkindData.Add(D_LOADCASE_SPECTRUM);
		aMinMaxData.Add(2); // max 값 읽어와야 함
	}
}

void CDgnPlateGirder_IRC22_15::GetAllEslc(CStringArray &aLoadName, CArray<UINT, UINT> &aLoadData, 
															 CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	// Response Spectrum Accidental Eccentricity Moment
	CArray<T_STLD_K, T_STLD_K> aReam;
	T_STLD_K ReamKey;
	T_STLD_D ReamData;
	m_pDoc->m_pPostCtrl->GetRaemKeyList(aReam);
	for (int i = 0; i < aReam.GetSize(); i++)
	{
		ReamKey = aReam.GetAt(i);
		m_pDoc->m_pPostCtrl->GetRaem(ReamKey, ReamData);
		aLoadName.Add(ReamData.LoadCaseName+_LS(IDS_TB_RESULTVIEW__ES_));
		aLoadData.Add(ReamKey);
		aLkindData.Add(D_LOADCASE_ECCENSPEC);
		aMinMaxData.Add(2); // max 값 읽어와야 함
	}
}

void CDgnPlateGirder_IRC22_15::GetAllThis(CStringArray &aLoadName, CArray<UINT, UINT> &aLoadData, 
															 CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	// time history loadcase
	if (m_pDoc->m_pPostCtrl->GetPointDataCount()->m_nNumThis == 0) return;

	// time history loadcase
	T_THIS_K ThisKey;
	T_THIS_D ThisData;
	CArray<T_THIS_K, T_THIS_K> aThis;
	m_pDoc->m_pAttrCtrl->GetThisKeyList(aThis);
	for (int i = 0; i < aThis.GetSize(); i++)
	{
		ThisKey = aThis.GetAt(i);
		m_pDoc->m_pAttrCtrl->GetThis(ThisKey, ThisData);

		aLoadData.Add(aThis.GetAt(i));
		aLkindData.Add(D_LOADCASE_HISTORY);
		aMinMaxData.Add(0);  // all (절대값 큰 것)
		aLoadName.Add(ThisData.LoadCaseName+_T("(TH:all)"));

		aLoadData.Add(aThis.GetAt(i));
		aLkindData.Add(D_LOADCASE_HISTORY);
		aMinMaxData.Add(1);  // max
		aLoadName.Add(ThisData.LoadCaseName+_T("(TH:max)"));

		aLoadData.Add(aThis.GetAt(i));
		aLkindData.Add(D_LOADCASE_HISTORY);
		aMinMaxData.Add(-1); // min
		aLoadName.Add(ThisData.LoadCaseName+_T("(TH:min)"));
	}
}

void CDgnPlateGirder_IRC22_15::GetAllMvld(CStringArray &aLoadName, CArray<UINT, UINT> &aLoadData, 
															 CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	if (m_pDoc->m_pPostCtrl->GetPointDataCount()->m_nNumMvld == 0) return;
	...
	case D_MOVE_CODE_BRAZIL:
		{
			...
			T_MVLD_K MvldKey;
			T_MVLD_D MvldData;
			CArray<T_MVLD_K, T_MVLD_K> aMvld;
			m_pDoc->m_pAttrCtrl->GetMvldKeyList(aMvld);
			for (int i = 0; i < aMvld.GetSize(); i++)
			{
				MvldKey = aMvld.GetAt(i);
				m_pDoc->m_pAttrCtrl->GetMvld(MvldKey, MvldData);

				aLoadData.Add(aMvld.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(0);  // all (절대값 큰 것)
				aLoadName.Add(MvldData.LoadCaseName+_T("(MV:all)"));

				aLoadData.Add(aMvld.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(1);  // max
				aLoadName.Add(MvldData.LoadCaseName+_T("(MV:max)"));

				aLoadData.Add(aMvld.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(-1); // min
				aLoadName.Add(MvldData.LoadCaseName+_T("(MV:min)"));
			}
		}
		break;
	case D_MOVE_CODE_CHINA:
		{
			// moving loadcase for Chinese Version
			T_MVLDch_K MvldchKey;
			T_MVLDch_D MvldchData;
			CArray<T_MVLDch_K, T_MVLDch_K> aMvldch;
			m_pDoc->m_pAttrCtrl->GetMvldchKeyList(aMvldch);
			for (int i = 0; i < aMvldch.GetSize(); i++)
			{
				MvldchKey = aMvldch.GetAt(i);
				m_pDoc->m_pAttrCtrl->GetMvldch(MvldchKey, MvldchData);

				aLoadData.Add(aMvldch.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(0);  // all (절대값 큰 것)
				aLoadName.Add(MvldchData.LoadCaseName+_T("(MV:all)"));

				aLoadData.Add(aMvldch.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(1);  // max
				aLoadName.Add(MvldchData.LoadCaseName+_T("(MV:max)"));

				aLoadData.Add(aMvldch.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(-1); // min
				aLoadName.Add(MvldchData.LoadCaseName+_T("(MV:min)"));
			}
		}
		break;
	case D_MOVE_CODE_JAPAN:
		{
			// moving loadcase for japan version
			T_MVLDjp_K MvldjpKey;
			T_MVLDjp_D MvldjpData;
			CArray<T_MVLDjp_K, T_MVLDjp_K> aMvldjp;
			m_pDoc->m_pAttrCtrl->GetMvldjpKeyList(aMvldjp);
			for (int i = 0; i < aMvldjp.GetSize(); i++)
			{
				MvldjpKey = aMvldjp.GetAt(i);
				m_pDoc->m_pAttrCtrl->GetMvldjp(MvldjpKey, MvldjpData);

				aLoadData.Add(aMvldjp.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(0);  // all (절대값 큰 것)
				aLoadName.Add(MvldjpData.LoadCaseName+_T("(MV:all)"));

				aLoadData.Add(aMvldjp.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(1);  // max
				aLoadName.Add(MvldjpData.LoadCaseName+_T("(MV:max)"));

				aLoadData.Add(aMvldjp.GetAt(i));
				aLkindData.Add(D_LOADCASE_MOVING);
				aMinMaxData.Add(-1); // min
				aLoadName.Add(MvldjpData.LoadCaseName+_T("(MV:min)"));
			}
		}
		break;
	case D_MOVE_CODE_INDIA:
		{
			// moving loadcase for india version
			T_MVLDid_K MvldKey;
			T_MVLDid_D MvldData;
			CArray<T_MVLDid_K, T_MVLDid_K> aMvld;
			m_pDoc->m_pAttrCtrl->GetMvldidKeyList(aMvld);
			for (int i = 0; i < aMvld.GetSize(); i++)
			{
				MvldKey = aMvld.GetAt(i);
				m_pDoc->m_pAttrCtrl->GetMvldid(MvldKey, MvldData);
	...
	case D_MOVE_CODE_BS:
		{
			...
			T_MVLDbs_K MvldKey;
			T_MVLDbs_D MvldData;
			CArray<T_MVLDbs_K, T_MVLDbs_K> aMvld;
			m_pDoc->m_pAttrCtrl->GetMvldbsKeyList(aMvld);
			for (int i = 0; i < aMvld.GetSize(); i++)
			{
				MvldKey = aMvld.GetAt(i);
				m_pDoc->m_pAttrCtrl->GetMvldbs(MvldKey, MvldData);
	...
	case D_MOVE_CODE_FRANCE:
		{
			// moving loadcase for France version
			T_MVLDfr_K MvldKey;
			T_MVLDfr_D MvldData;
			CArray<T_MVLDfr_K, T_MVLDfr_K> aMvld;
			m_pDoc->m_pAttrCtrl->GetMvldfrKeyList(aMvld);
			for (int i = 0; i < aMvld.GetSize(); i++)
			{
				MvldKey = aMvld.GetAt(i);
				m_pDoc->m_pAttrCtrl->GetMvldfr(MvldKey, MvldData);
	...
	case D_MOVE_CODE_TRANS:
		{
			...
			T_MVLDtr_K MvldKey;
			T_MVLDtr_D MvldData;
			CArray<T_MVLDtr_K, T_MVLDtr_K> aMvld;
			m_pDoc->m_pAttrCtrl->GetMvldtrKeyList(aMvld);
			for (int i = 0; i < aMvld.GetSize(); i++)
			{
				MvldKey = aMvld.GetAt(i);
...
void CDgnPlateGirder_IRC22_15::GetAllSmlc(CStringArray &aLoadName, CArray<UINT, UINT> &aLoadData, 
															 CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	if (m_pDoc->m_pPostCtrl->GetPointDataCount()->m_nNumSmlc == 0) return;

	// settlement loadcase
	T_SMLC_K SmlcKey;
	T_SMLC_D SmlcData;
	CArray<T_SMLC_K, T_SMLC_K> aSmlc;
	m_pDoc->m_pAttrCtrl->GetSmlcKeyList(aSmlc);
	for (int i = 0; i < aSmlc.GetSize(); i++)
	{
		SmlcKey = aSmlc.GetAt(i);
		m_pDoc->m_pAttrCtrl->GetSmlc(SmlcKey, SmlcData);

		aLoadData.Add(aSmlc.GetAt(i));
		aLkindData.Add(D_LOADCASE_SETTLE);
		aMinMaxData.Add(0);  // all (절대값 큰 것)
		aLoadName.Add(SmlcData.LoadCaseName+_T("(SM:all)"));

		aLoadData.Add(aSmlc.GetAt(i));
		aLkindData.Add(D_LOADCASE_SETTLE);
		aMinMaxData.Add(1);  // max
		aLoadName.Add(SmlcData.LoadCaseName+_T("(SM:max)"));

		aLoadData.Add(aSmlc.GetAt(i));
		aLkindData.Add(D_LOADCASE_SETTLE);
		aMinMaxData.Add(-1); // min
		aLoadName.Add(SmlcData.LoadCaseName+_T("(SM:min)"));
	}
}

void CDgnPlateGirder_IRC22_15::GetAllGilc(CStringArray &aLoadName, CArray<UINT, UINT> &aLoadData, 
																		CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	if (m_pDoc->m_pPostCtrl->GetPointDataCount()->m_nNumGmld == 0) return;

	// grid influence line load case
	CArray<T_GILC_K, T_GILC_K> aGilc;
	T_GILC_K GilcKey;
	T_GILC_D GilcData;
	m_pDoc->m_pAttrCtrl->GetGilcKeyList(aGilc);
	for(int i=0; i<aGilc.GetSize(); i++)
	{
		GilcKey = aGilc.GetAt(i);
		m_pDoc->m_pAttrCtrl->GetGilc(GilcKey, GilcData);
		if (!m_pDoc->m_pAttrCtrl->IsGilcMaxMinType(GilcData))
		{
			aLoadName.Add(GilcData.LoadCaseName+_LS(IDS_TB_RESULTVIEW__GI_));
			aLoadData.Add(GilcKey);
			aLkindData.Add(D_LOADCASE_GRIDMODEL);
			aMinMaxData.Add(2); // max 값 읽어와야 함
		}
		else
		{
			aLoadName.Add(GilcData.LoadCaseName+_T("(GI:all)"));
			aLoadData.Add(GilcKey);
			aLkindData.Add(D_LOADCASE_GRIDMODEL);
			aMinMaxData.Add(0); // all (절대값 큰 것)

			aLoadName.Add(GilcData.LoadCaseName+_T("(GI:max)"));
			aLoadData.Add(GilcKey);
			aLkindData.Add(D_LOADCASE_GRIDMODEL);
			aMinMaxData.Add(1); // max 

			aLoadName.Add(GilcData.LoadCaseName+_T("(GI:min)"));
			aLoadData.Add(GilcKey);
			aLkindData.Add(D_LOADCASE_GRIDMODEL);
			aMinMaxData.Add(-1); // min
		}
...
void CDgnPlateGirder_IRC22_15::GetAllLcom(BOOL bStageMode, BOOL bFinalStage, CStringArray &aLoadName, 
															 CArray<UINT, UINT> &aLoadData, CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	if (bStageMode && !bFinalStage) return ; // MNET:3541-GYKIM-20101130 : 시공단계 모드에서는 하중조합을 리스트에 포함시키지 않는다.  
	// load combination
	// Final Stage에서는 모든 하중조합, Final Stage 아니면 CS로만 구성된 하중조합
	//if (!bStageMode || bFinalStage)
	{
		...
		int nLcaseType[] = {
			D_LOADCASE_COMB_GENERAL, D_LOADCASE_COMB_STEEL, D_LOADCASE_COMB_CONCRETE, D_LOADCASE_COMB_SRC, D_LOADCASE_COMB_FDN, D_LOADCASE_COMB_ALUMINUM, D_LOADCASE_COMB_STLCOMP,
		};
		CString csLcomTypeName[] = {_LS(IDS_TB_RESULTVIEW_CB), _LS(IDS_TB_RESULTVIEW_CBS), _LS(IDS_TB_RESULTVIEW_CBC), _LS(IDS_TB_RESULTVIEW_CBR), _LS(IDS_TB_RESULTVIEW_CBF), _LS(IDS_TB_RESULTVIEW_CBA), _LS(IDS_TB_RESULTVIEW_CBSC),};
		...
		for (int k = 0; k < sizeof(nLcomType)/sizeof(int); k++)
		{
			...
			if (!bStageMode || bFinalStage)
				...
				m_pDoc->m_pAttrCtrl->GetLcomKeyListOnlySgld(nLcomType[k], aLcomKey);
			int nSize = aLcomKey.GetSize();
			for (int i = 0; i < nSize; i++)
			{
				LcomKey = aLcomKey.GetAt(i);
				m_pDoc->m_pAttrCtrl->GetLcom(nLcomType[k], LcomKey, LcomData);
				if (LcomData.nActive == 0) continue;

				aLoadData.Add(aLcomKey.GetAt(i));
				aLkindData.Add(nLcaseType[k]);
				if (m_pDoc->m_pAttrCtrl->IsMaxMinComb(LcomData))
				{
					aMinMaxData.Add(0);  // all(절대값 큰 것)
					aLoadName.Add(LcomData.LoadCombName+_T("(")+csLcomTypeName[k]+_T(":all)"));
					aLoadData.Add(aLcomKey.GetAt(i));
					aLkindData.Add(nLcaseType[k]);
					aMinMaxData.Add(1);  // max;
					aLoadName.Add(LcomData.LoadCombName+_T("(")+csLcomTypeName[k]+_T(":max)"));
					aLoadData.Add(aLcomKey.GetAt(i));
					aLkindData.Add(nLcaseType[k]);
					aMinMaxData.Add(-1); // min
					aLoadName.Add(LcomData.LoadCombName+_T("(")+csLcomTypeName[k]+_T(":min)"));
				}
				else
				{
					aMinMaxData.Add(2); // max 값 읽어와야 함
					aLoadName.Add(LcomData.LoadCombName+_T("(")+csLcomTypeName[k]+_T(")"));
				}
			}
		}
	}
...
void CDgnPlateGirder_IRC22_15::GetAllEnvl(BOOL bStageMode, BOOL bFinalStage, CStringArray &aLoadName, 
															 CArray<UINT, UINT> &aLoadData, CArray<UINT, UINT> &aLkindData, CArray<int, int> &aMinMaxData)
{
	// Envelope
	if (!bStageMode || bFinalStage)
	{
		T_ENVL_K EnvlKey;
		T_ENVL_D EnvlData;
		CArray<T_ENVL_K, T_ENVL_K> aEnvl;
		m_pDoc->m_pAttrCtrl->GetEnvlKeyList(aEnvl);
		for (int i = 0; i < aEnvl.GetSize(); i++)
		{
			EnvlKey = aEnvl.GetAt(i);
			m_pDoc->m_pAttrCtrl->GetEnvl(EnvlKey, EnvlData);

			aLoadData.Add(aEnvl.GetAt(i));
			aLkindData.Add(D_LOADCASE_ENVELOPE);
			aMinMaxData.Add(0);  // all (절대값 큰 것)
			aLoadName.Add(EnvlData.EnvelopeName+_T("(EN:all)"));

			aLoadData.Add(aEnvl.GetAt(i));
			aLkindData.Add(D_LOADCASE_ENVELOPE);
			aMinMaxData.Add(1);  // max
			aLoadName.Add(EnvlData.EnvelopeName+_T("(EN:max)"));

			aLoadData.Add(aEnvl.GetAt(i));
			aLkindData.Add(D_LOADCASE_ENVELOPE);
			aMinMaxData.Add(-1); // min
			aLoadName.Add(EnvlData.EnvelopeName+_T("(EN:min)"));
		}
	}
}

void CDgnPlateGirder_IRC22_15::GetShear_Rslt_FSUM(ElemPairK ElemK,T_LCOM_K nStrnKey)
{
    m_aLoadName.RemoveAll();
    m_aLoadData.RemoveAll();
    m_aLkindData.RemoveAll();
    m_aMinMaxData.RemoveAll();
    GetAllLoadcase(m_aLoadName, m_aLoadData, m_aLkindData, m_aMinMaxData);
    T_FSUM_D data;data.Initialize();
    CArray<T_FSUM_K, T_FSUM_K> raKey;raKey.SetSize(1);
    raKey.Add(1);
    data.nElemType = 2;
    data.csName = _T("1") ;
    data.nType =1 ;
    data.nMethod =1 ;
    T_ELEM_D ElemData;
    data.aElem.SetSize(1);
    //for (int i=0; i<iTotalElement; i++) 
    {
        data.aElem[0] = ElemK.first;//arCheckElemK[i];
        m_pDoc->m_pAttrCtrl->GetElem(data.aElem[0], ElemData);
    }
    data.aPart.Add(0);data.aPart.Add(7);
    T_LCMT_D LcmtD; LcmtD.Initialize();
    if (!m_pDoc->m_pAttrCtrl2->GetLcmt(LcmtD))
    {
    ...
    int nVedDLLcomSize = 1;//LcmtD.Base[0].aStrength.GetSize();//VedDL
    for (int im = 0;im<nVedDLLcomSize;im++)
    {
        ...
        T_LCOM_D rDataTemp;rDataTemp.Initialize();
        m_pDoc->m_pAttrCtrl->GetLcom(D_LCOMTYPE_STEEL,nStrnKey,rDataTemp);           
        for (int ime = 0; ime < m_aLoadData.GetSize(); ime++)
        {
            if (m_aLoadName.GetAt(ime).Find(rDataTemp.LoadCombName) == 0)
            {
                data.aLoad.Add(m_aLoadData[ime]);
                data.aLkind.Add(m_aLkindData[ime]);
                data.aMinMax.Add(m_aMinMaxData[ime]);
            }
        }
    }       
    data.aItem.Add(2);
    //m_pDoc->m_pDataCtrl->AddFsum(data);

    //Retrieve results

    int nNum = 1 ;//raKey.GetSize();
    for (int i=0; i<nNum; i++)
    {
        //if (!m_pDoc->m_pAttrCtrl->GetFsum(raKey[i], data)) ASSERT(0);
        m_aKey.Add(raKey[i]);
        m_aData.Add(data);
    }
    ...
    m_aLkind.RemoveAll();
    m_aMinMax.RemoveAll();
    m_aLoadStep.RemoveAll();

    data = m_aData.GetAt(0);
    int nSize = data.aLoad.GetSize();
    for (int i=0; i<nSize; i++)
    {
        m_aLoad.Add(data.aLoad[i]);
        m_aLkind.Add(data.aLkind[i]);
        m_aMinMax.Add(data.aMinMax[i]);
        m_aLoadStep.Add(T_LCIX_STEP(i, 0));
    }

    m_aValue[0].RemoveAll();
    m_aValue[1].RemoveAll();
    ...
    T_LCOM_K LcomKey;
    T_LCOM_D LcomData;
    T_STRB_D StrbDataMax;
    T_STRB_D StrbDataMin;
    T_STRB_D StrbDataAbs;

    KeyIndex = 0;//m_nCurRec-1;
    ElptKey.keymap = m_aKey.GetAt(KeyIndex);
    ElemKey = ElemK.first ;//ElptKey.key.elem;
    nPart = ElptKey.key.part;

    int nKN2 = m_aLoadStep.GetSize();
    int m_nTgtFldIndex = 2;
    for (int i=0; i<nKN2; i++)
    {    
        int lcix = m_aLoadStep[i].lcix;
        int step = m_aLoadStep[i].step;
        LcomKey = nStrnKey;//m_aLoad.GetAt(lcix);
        nLkind = m_aLkind.GetAt(lcix);
        nMinMax = m_aMinMax.GetAt(lcix);
        int nAnalType =0;// GetAnalType(nLkind);
        m_pDoc->m_pPostCtrl->MakeLoadComb(nLkind, LcomKey, LcomData);
        m_pDoc->m_pPostCtrl->SelectLoadCombNew(LcomData, step, nAnalType);

        if (nMinMax == 0)
            m_pDoc->m_pPostCtrl->GetStrbNew(ElemKey, 0, 0, &StrbDataAbs,TRUE, NULL, ElemK.second);
       // else if (nMinMax == -1)
            m_pDoc->m_pPostCtrl->GetStrbNew(ElemKey, 0, &StrbDataMin, 0,TRUE, NULL, ElemK.second);
        //else
            m_pDoc->m_pPostCtrl->GetStrbNew(ElemKey, &StrbDataMax, 0, 0,TRUE, NULL, ElemK.second);

        if (nPart % 2 == 0)
        {
           double dval1 = StrbDataMax.dblForce[nPart/2][m_nTgtFldIndex];
           double dval2 = StrbDataMin.dblForce[nPart/2][m_nTgtFldIndex];
           double dMaxRes = fabs(dval1) >= fabs(dval2) ? dval1:dval2;
           double dMinRes = fabs(dval1) <= fabs(dval2) ? dval1:dval2;

            m_aValue[0].Add(dMaxRes);//StrbData.dblForce[nPart/2][m_nTgtFldIndex]);
            m_aValue[1].Add(dMinRes);//StrbData.dblForce[nPart/2][m_nTgtFldIndex]);
        }
        else
        {
            double dval1 =(StrbDataMax.dblForce[nPart/2][m_nTgtFldIndex+6]);
            double dval2 =(StrbDataMin.dblForce[nPart/2][m_nTgtFldIndex+6]);
            double dMaxRes = fabs(dval1) >= fabs(dval2) ? dval1:dval2;
            double dMinRes = fabs(dval1) <= fabs(dval2) ? dval1:dval2;
