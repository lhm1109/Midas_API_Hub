...
/////////////////////////////////////////////////////////////////////////////
// CConcSectionCreateRebar dialog


CConcSectionCreateRebar::CConcSectionCreateRebar(CWnd* pParent /*=NULL*/)
	: CChildDialog(CConcSectionCreateRebar::IDD, pParent)
{
	m_pDoc = 0;
	m_pDoc = CDBDoc::GetDocPoint();
	//{{AFX_DATA_INIT(CConcSectionCreateRebar)
	m_nTypeofCreation = 0;
	...
	m_pRebarGrid = new CDgnCreateRebarGrid();
...
void CConcSectionCreateRebar::SetDlgViewPanel(I_DlgViewPanel* pIDVP)
{
	this->m_pIDVP = pIDVP;
}

CConcSectionCreateRebar::~CConcSectionCreateRebar()
{
...
void CConcSectionCreateRebar::DoDataExchange(CDataExchange* pDX)
{
	CChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CConcSectionCreateRebar)
	DDX_Control(pDX, IDC_DGN_REBARDB_COMBO_SHEAR, m_CobxShearRebarDia);
	...
	DDX_Control(pDX,IDC_DGN_LENGTH_UNIT_STATIC6, m_wndLengthUnit6);
	...
	DDX_Control(pDX, IDC_DGN_CIR_TYPE_NUM_OF_REBAR_EDIT, m_edtCirTypeNum);
	...
	DDX_CBString(pDX, IDC_DGN_REBARDB_COMBO_SHEAR, m_strShearRebarDia);
	DDX_Control(pDX, IDC_DGN_SHEAR_SPACE_EDT, m_dShearRebarXStep);
	DDX_Text(pDX, IDC_DGN_SHEAR_Y_STEP_EDIT, m_strShearRebarYNum);
	DDX_Text(pDX, IDC_DGN_SHEAR_Z_STEP_EDIT, m_strShearRebarZNum);
...
BOOL CConcSectionCreateRebar::OnInitDialog() 
{
	...
	RebarKey = m_pParent->m_pParent->GetMainRebarI_LastNum();
	SetAsRebarArea();
...
BEGIN_MESSAGE_MAP(CConcSectionCreateRebar, CChildDialog)
	//{{AFX_MSG_MAP(CConcSectionCreateRebar)
	...
	ON_BN_CLICKED(IDC_DGN_REDO_BUTTON, OnDgnRedoButton)
	ON_BN_CLICKED(IDC_DGN_UNDO_BUTTON, OnDgnUndoButton)
	ON_UPDATE_COMMAND_UI(IDC_DGN_UNDO_BUTTON    ,OnUpdateUI)
	ON_CBN_SELCHANGE(IDC_DGN_REBARDB_COMBO_P, OnSelchangeDgnRebardbCombo)
	ON_NOTIFY(UDN_DELTAPOS, IDC_DGN_SHEAR_Y_STEP_SPIN, OnDeltaposDgnShearYStepSpin)
	ON_NOTIFY(UDN_DELTAPOS, IDC_DGN_SHEAR_Z_STEP_SPIN, OnDeltaposDgnShearZStepSpin)
	ON_CBN_EDITCHANGE(IDC_DGN_REBARDB_COMBO_SHEAR, OnEditchangeDgnRebardbComboShear)
	...
	ON_UPDATE_COMMAND_UI(IDC_DGN_REDO_BUTTON    ,OnUpdateUI)
...
/////////////////////////////////////////////////////////////////////////////
// CConcSectionCreateRebar message handlers

void CConcSectionCreateRebar::ControlMan(BOOL bInit)
{
	...
	if(bFirst)
	{
		CDlgUtil::GetCtrlIDByIncRect(this,arCtrlPType,IDC_DGN_TYPE_POINT_CTRL_HOLDER,FALSE);
		CDlgUtil::GetCtrlIDByIncRect(this,arCtrlLType,IDC_DGN_TYPE_LINEAR_CTRL_HOLDER,FALSE);
		CDlgUtil::GetCtrlIDByIncRect(this,arCtrlCType,IDC_DGN_TYPE_CIRCULAR_CTRL_HOLDER,FALSE);
		bFirst = FALSE;
	}
...
void CConcSectionCreateRebar::OnClickTypeRadioBtn() 
{
	//데이타를 해당 타입에 맞게 세팅한다.
	//코드 작성해야함
	UpdateData(TRUE);
...
void CConcSectionCreateRebar::SetCreationType()
{
...
BOOL CConcSectionCreateRebar::DestroyWindow() 
{
...
void CConcSectionCreateRebar::SetInitUnit()
{
...
void CConcSectionCreateRebar::OnDgnCreateButton() 
{	
	addDelMod = 0;
...
void CConcSectionCreateRebar::OnDgnDeleteButton() 
{
	...
	for(int i=0 ; i<selectKeyList.GetSize() ; i++)
	{
		...
		//판넬뷰에서 해당 철근 그림을 지운다
		m_pParent->DeleteRebarData(selectKeyList[i], TRUE);
	}
	m_pRebarGrid->LockUpdate(bOldLock);
	if (!bOldLock) m_pRebarGrid->Redraw();
...
void CConcSectionCreateRebar::OnDgnRedoButton() 
{
	
}

void CConcSectionCreateRebar::OnDgnUndoButton() 
{
	
}

void CConcSectionCreateRebar::OnUpdateUI(CCmdUI*pCmdUI)
{
	
}


void CConcSectionCreateRebar::OnChangeLineTypeDivNum() //분할 개수 변경 되었을 때  
{
...
void CConcSectionCreateRebar::OnOnChangeCirTypeDivNum() //분할 개수 변경 되었을 때 
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CChildDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CConcSectionCreateRebar::OnChangeCirTypeAngle() // Circle Type의 Arc Angle이 변경되었을때 
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CChildDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CConcSectionCreateRebar::OnChangePointTypeParam() //Edit Control의 내용이 변경되었을 때 
{
	//---------------------------------------------------
...
void CConcSectionCreateRebar::OnChangeLineTypeParam() //Edit Control의 내용이 변경되었을 때 
{
	//---------------------------------------------------
	//KeyPressed 상태이면 화면을 Update하고 그렇지 않으면 
	//화면을 갱신하지 않는다. ........................... 
...
void CConcSectionCreateRebar::OnChangeCirTypeParam() //Edit Control의 내용이 변경되었을 때 
{
	/*
	//---------------------------------------------------
	//KeyPressed 상태이면 화면을 Update하고 그렇지 않으면 
...
void CConcSectionCreateRebar::OnSelchangeDgnRebardbCombo()  //Diameter Combo box 변경 
{	
	UpdateData(TRUE);
	int Index;
	...
	{
		int nRNum = m_edtCirTypeNum.GetEditValueInt();
...
void CConcSectionCreateRebar::OnOutofmemoryDgnShearYStepSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
...
void CConcSectionCreateRebar::OnDeltaposDgnShearYStepSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	CDlgUtil::SetSpinValue(this,IDC_DGN_SHEAR_Y_STEP_EDIT,2,100,pNMHDR,pResult);
	*pResult = 0;
}

void CConcSectionCreateRebar::OnDeltaposDgnShearZStepSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	CDlgUtil::SetSpinValue(this,IDC_DGN_SHEAR_Z_STEP_EDIT,2,100,pNMHDR,pResult);
	*pResult = 0;
}

void CConcSectionCreateRebar::OnEditchangeDgnRebardbComboShear() 
{
	// TODO: Add your control notification handler code here
	
}

BOOL CConcSectionCreateRebar::Data2Dlg()
{
	/*
	m_Dt.SetEditUnit(0.);
	m_Db.SetEditUnit(0.);
...
BOOL CConcSectionCreateRebar::Dlg2Data()
{
...
void CConcSectionCreateRebar::UpdateBuffer()
{
	REBAR_KEY    Key;
	T_REBT_MABR    Data;
...
void CConcSectionCreateRebar::AddRebarDrawData() 
{
	REBAR_KEY    Key;
	T_REBT_MABR    Data;
	...
	{
		m_MabrDataList.GetNextAssoc(Pos,Key,Data);	
		m_pParent->AddRebarData(Key, Data, TRUE);		
	}
}

double CConcSectionCreateRebar::GetZ(double Y, double a, double b, double c, double d) 
{
	//기울기
	double m;
	double n;
...
void CConcSectionCreateRebar::SetInitData(CArray<T_REBT_MABR,T_REBT_MABR&>& MainRebarDataList)
{
	m_MabrDataList.RemoveAll();
	RebarKey = 0;
...
void CConcSectionCreateRebar::SaveDlgData()
{
	Dlg2Data();
}

int CConcSectionCreateRebar::GetComboRebarIndex(CString strRebarname)
{
...
void CConcSectionCreateRebar::RemoveMainRebarDataList(REBAR_KEY Key, T_REBT_MABR Data)
{
	m_pRebarGrid->DeleteRecordByKey(Key, Data);
...
void CConcSectionCreateRebar::SetRebarListData(CMap<REBAR_KEY,REBAR_KEY,T_REBT_MABR,T_REBT_MABR>& m_MDataList)
{
	m_MabrDataList.RemoveAll();
...
void CConcSectionCreateRebar::OnDgnRedrawButton() 
{
	...
	while(Pos)
	{
		...
		m_pParent->m_pParent->ReDrawRebar(Key, ReberData, TRUE);			
	}

	SetAsRebarArea();
}

void CConcSectionCreateRebar::SetAsRebarArea() 
{
	double RebarArea = 0.;
