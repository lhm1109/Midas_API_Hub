// CDgnGenSeisFactor_IS18168.cpp : implementation file
//
...
CDgnGenSeisFactor_IS18168::CDgnGenSeisFactor_IS18168(CWnd* pParent /*=NULL*/)
	: CDialogMove(CDgnGenSeisFactor_IS18168::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDgnGenSeisFactor_IS18168)
	m_bHideSpecialGroup = FALSE;
	m_strConCode.Empty();
	//}}AFX_DATA_INIT
}

void CDgnGenSeisFactor_IS18168::DoDataExchange(CDataExchange* pDX)
{
	CDialogMove::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnGenSeisFactor_IS18168)
...
BEGIN_MESSAGE_MAP(CDgnGenSeisFactor_IS18168, CDialog)
#undef CDialog

	//{{AFX_MSG_MAP(CDgnGenSeisFactor_IS18168)
...
/////////////////////////////////////////////////////////////////////////////
// CDgnGenSeisFactor_IS18168 message handlers

void CDgnGenSeisFactor_IS18168::SetListCtrlHeader()
{
...
BOOL CDgnGenSeisFactor_IS18168::GetSelectedItem(int &nIndex)
{
...
void CDgnGenSeisFactor_IS18168::MakeItemEx()
{
...
CString CDgnGenSeisFactor_IS18168::DataToStr(int i, T_SOSF_BASE &data)
{
	CString value;
	if (i == 0)
	{
		...
		else if (data.nLoadCaseType == 2)
		{
			T_STLD_K key = data.LoadCaseKey;
			T_STLD_D rData;
			m_pDoc->m_pAttrCtrl->GetStld(key, rData);
			value = rData.LoadCaseName + _T("(LL)");
		}
	}
...
void CDgnGenSeisFactor_IS18168::SetItem(int nIndex, T_SOSF_BASE &data)
{
	LVITEM lvitem;
	CString str;
	for (int i = 0; i < 2; i++)
...
BOOL CDgnGenSeisFactor_IS18168::ValidItem(T_SOSF_BASE& data)
{
	...
	if (data.nLoadCaseType == 0 && data.dScaleFactor != 2.5 && data.dScaleFactor != 3.0) {
		//m_pDoc->DisplayHistoryMessage(_LSX());
		AfxMessageBox(_LS(IDS_WG_CMD__ADDD_Invalid_scale_factor_seis_is18168));
		return FALSE;
	}
	if (data.nLoadCaseType == 1 && data.dScaleFactor != 2.5 && data.dScaleFactor != 3.0) {
		AfxMessageBox(_LS(IDS_WG_CMD__ADDD_Invalid_scale_factor_seis_is18168));
		return FALSE;
	}
	if (data.nLoadCaseType == 2 && data.dScaleFactor != 0.25 && data.dScaleFactor != 0.5) {
		AfxMessageBox(_LS(IDS_WG_CMD__ADDD_Invalid_scale_factor_live_is18168));
		return FALSE;
	}
	return TRUE;
}

BOOL CDgnGenSeisFactor_IS18168::Dlg2Item(T_SOSF_BASE &data)
{
	...
	if (Index != -1)
	{
		...
		iLoadCaseType = GetLoadCaseType(Index);
		...
		else if (iLoadCaseType == 2)
		{
			strLCName.TrimRight(_T(")"));
			strLCName.TrimRight(_T("L"));
			strLCName.TrimRight(_T("L"));
			strLCName.TrimRight(_T("("));
...
BOOL CDgnGenSeisFactor_IS18168::Item2Dlg(int nIndex, T_SOSF_BASE &data)
{
...
void CDgnGenSeisFactor_IS18168::Initial_LCaseComboBox()
{
	...
	//Adding LL case to Drop DOwn
	arKeyList.RemoveAll();
	m_pDoc->m_pAttrCtrl->GetStldKeyList(arKeyList);
	iCount = m_pDoc->m_pAttrCtrl->GetCountStld();
	...
	if (iCount > 0)
	{
		for (int i = 0; i < iCount; i++)
		{
			...
			{
				m_LoadCase.AddString(strLoadCase + _T("(LL)"));

				_DGN_SEIS_LCTP_IS18168 LCType;
				LCType.index = m_LoadCase.GetCount() - 1;
				LCType.iLoadCaseType = 2; // 0: Static,		1: Response Spectrum   2:LL
				m_arLCtype.Add(LCType);
			}
...
BOOL CDgnGenSeisFactor_IS18168::OnInitDialog()
{
	...
	GetDlgItem(IDC_WG_DGN_STATIC6)->EnableWindow(FALSE);
	GetDlgItem(IDC_DGN_VERT_FAC2)->EnableWindow(FALSE);

	return TRUE;  // return TRUE unless you set the focus to a control
				  // EXCEPTION: OCX Property Pages should return FALSE
}

void CDgnGenSeisFactor_IS18168::OnOK()
{
	...
	//Special combination generation should have at least one seismic load case
	int iCountSeisLoadCases = 0;
	for (int i = 0; i < m_Data.aLoadPattern.GetSize(); i++)
	{
		int iLCaseType = m_Data.aLoadPattern.GetAt(i).nLoadCaseType;
		if (iLCaseType == 0 || iLCaseType == 1) {
			iCountSeisLoadCases++;
		}
	}
	if (iCountSeisLoadCases <= 0)
	{
		AfxMessageBox(_LS(IDS_WG_CMD__ADDD_Add_atleast_one_seis_loadcase));
		return;
	}

	//GEN-9379 - If Design code is not selected then DSTL will be null.Hence handled at IS 18168 Dlg Level	
	T_DSTL_D rData1;
	rData1.Initialize();
	m_pDoc->m_pAttrCtrl->GetDstl(rData1);
	if (rData1.DesignCode != _T("IS:800-2007"))
	{
		rData1.DesignCode = CDBLib::GetConvertCodeName(_T("IS:800-2007"));
		m_pDoc->m_pDataCtrl->AddDstl(rData1);
		m_pDoc->m_pDataCtrl->AddDstlLog(rData1);
	}
...
void CDgnGenSeisFactor_IS18168::OnCancel()
{
	// TODO: Add your control notification handler code here
	CDialogMove::OnCancel();
}

void CDgnGenSeisFactor_IS18168::OnClickLoadCaseBtn()
{
	// TODO: Add your control notification handler code here
	AfxGetMainWnd()->SendMessage(WM_COMMAND, MAKEWPARAM(ID_STRUCT_STATIC_LOADCASE, 0));
}

void CDgnGenSeisFactor_IS18168::OnDgnGenSeisAdd()
{
	// TODO: Add your control notification handler code here
	T_SOSF_BASE data;
	/*
		if (m_Data.nNumPdelCase >= D_PDEL_NUMPDEL)
		{
	...
	*/
	...
	for (int i = 0; i < m_Data.aLoadPattern.GetSize(); i++)
	{
		...
		if (SosfBase.LoadCaseKey == data.LoadCaseKey && SosfBase.nLoadCaseType == data.nLoadCaseType)//==0)
		{
			CString csLoadCase;
		...
		/*
		else if (SosfBase.LoadCaseKey == data.LoadCaseKey && data.nLoadCaseType==1)
		{
			CString csLoadCase;
			m_LoadCase.GetWindowText(csLoadCase);
			CString msg;
...
void CDgnGenSeisFactor_IS18168::OnDgnGenSeisModify()
{
	// TODO: Add your control notification handler code here
	int nIndex;
...
void CDgnGenSeisFactor_IS18168::OnDgnGenSeisDelete()
{
	// TODO: Add your control notification handler code here
	int nIndex;
	if (!GetSelectedItem(nIndex))
...
void CDgnGenSeisFactor_IS18168::OnChangedCurrentItem(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CDgnGenSeisFactor_IS18168::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	HWND hWnd;
...
void CDgnGenSeisFactor_IS18168::UpdateBuffer()
{
...
BOOL CDgnGenSeisFactor_IS18168::CheckLoadCaseName(CString str, int& Index)
{
	BOOL bCheck = TRUE;
...
int CDgnGenSeisFactor_IS18168::GetLoadCaseType(int iIndex)
{
	int iType;
	for (int i = 0; i < m_arLCtype.GetSize(); i++)
	{
		_DGN_SEIS_LCTP_IS18168 LCtype = m_arLCtype.GetAt(i);
...
/*
CString CDgnGenSeisFactor_IS18168::GetLoadCaseName(int iIndex)
{
	CString StrName = m_ListCtrl.GetItemText(iIndex,0);
	return StrName;
}
*/
void CDgnGenSeisFactor_IS18168::EnableCtrl()
{
	if (m_strConCode != CONCODE_NSCP_2015) { return; }

	CArray<UINT, UINT> aCtrlEnable;
	aCtrlEnable.Add(IDC_WG_DGN_STATIC1);
	aCtrlEnable.Add(IDC_DGN_VERT_FAC1);
	aCtrlEnable.Add(IDC_WG_DGN_STATIC7);
	//aCtrlEnable.Add(IDC_DGN_SPS_SDS);
	CDlgUtil::CtrlEnableDisable(this, aCtrlEnable, FALSE);
}
void CDgnGenSeisFactor_IS18168::ShowHideCtrl()
{
