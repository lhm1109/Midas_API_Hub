// DgnConCodeNewChild_1_Dlg.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CDgnConCodeNewChild10Dlg dialog


CDgnConCodeNewChild10Dlg::CDgnConCodeNewChild10Dlg(CDgnConCodeNewDlg* pParent /*=NULL*/, T_DCON_D* pData, BOOL bExistData/*=FALSE*/)
	: CDconChildDialog(CDgnConCodeNewChild10Dlg::IDD, pParent, pData, bExistData)
{
	//{{AFX_DATA_INIT(CDgnConCodeNewChild10Dlg)
// 	m_pData = pData;
...
void CDgnConCodeNewChild10Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDconChildDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDgnConCodeNewChild10Dlg)
...
BEGIN_MESSAGE_MAP(CDgnConCodeNewChild10Dlg, CDconChildDialog)
	//{{AFX_MSG_MAP(CDgnConCodeNewChild10Dlg)
...
/////////////////////////////////////////////////////////////////////////////
// CDgnConCodeNewChild10Dlg message handlers

BOOL CDgnConCodeNewChild10Dlg::OnInitDialog() 
{
...
void CDgnConCodeNewChild10Dlg::SetComboCdAndIe()
{
...
void CDgnConCodeNewChild10Dlg::SetComboCdAngle_ACI318()
{
	m_CmbCd.ResetContent();
...
void CDgnConCodeNewChild10Dlg::SetComboBox()
{
...
BOOL CDgnConCodeNewChild10Dlg::UseAlpha2Colm(const CString& strCode)
{
    return CDgnCodeCtrl::IsConCodeUseAlpha2ColmSSWB(strCode) ? TRUE : FALSE;
}

BOOL CDgnConCodeNewChild10Dlg::UseBeamColmJointDesign(const CString& strCode)
{
...
void CDgnConCodeNewChild10Dlg::AlignControl()
{
	//m_grpWallType.InitControl(this, CDgnConCodeNewChild10Dlg::IDD, IDC_DGN_WALLTYPE_N0, TRUE, TRUE);
}

void CDgnConCodeNewChild10Dlg::SetShowPMCurveMethod(const CString& strCode)
{
...
void CDgnConCodeNewChild10Dlg::InitCtrl()
{
	m_grpSeisParam.InitControl(this, CDgnConCodeNewChild10Dlg::IDD, IDC_DGN_SEIS_PARAM_GRB, TRUE, TRUE);
	...
	EnableWindowByNTC_LDS();
	OnBCJoint();
}

void CDgnConCodeNewChild10Dlg::OnDgnRcctrlSpecial() 
{
...
void CDgnConCodeNewChild10Dlg::GetInitValue(CString strCode)
{
	if(!m_bExistData)
	{
		m_iAlphaType1 = 1;
...
void CDgnConCodeNewChild10Dlg::OnChangeFrameType()
{
	UpdateData(TRUE);
...
void CDgnConCodeNewChild10Dlg::OnChangeAlphaType()
{
	UpdateData(TRUE);
...
void CDgnConCodeNewChild10Dlg::OnChkSpecialWall()
{
	UpdateData(TRUE);
	EnableWindowBySpecialWall();
}

void CDgnConCodeNewChild10Dlg::OnChangeBndrElemMethodType()
{
	UpdateData(TRUE);
	EnableWindowByBndrElemMethod();
}

void CDgnConCodeNewChild10Dlg::EnableWindowByBeamColmJointDesign(const CString& strCode, BOOL bChangeValue/*=TRUE*/)
{
...
void CDgnConCodeNewChild10Dlg::EnableWindowByBndrElemMethod()
{
...
void CDgnConCodeNewChild10Dlg::EnableWindowByNTC_LDS()
{
	CString strCode = CDBLib::GetConvertCodeName(m_pData->DesignCode);
	if(strCode != CONCODE_NTC_DCEC_2023) return;

	const BOOL bEnable = m_bSpecialEQ && m_iFrameType!=2 ? TRUE : FALSE;
	CDlgUtil::CtrlEnableDisable(this, m_aShearWallType, bEnable);
	CDlgUtil::CtrlEnableDisable(this, m_aShearforDesign, bEnable);
	CDlgUtil::CtrlEnableDisable(this, m_arCtrlAlphaColm, bEnable);
}

void CDgnConCodeNewChild10Dlg::OnChangeTorsion()
{
...
void CDgnConCodeNewChild10Dlg::OnUpdateByCode()
{
	UpdateData(TRUE);
...
void CDgnConCodeNewChild10Dlg::SetShowExposure(const CString& strCode)
{
...
void CDgnConCodeNewChild10Dlg::SetShowBeamDeflection(const CString& strCode)
{
...
void CDgnConCodeNewChild10Dlg::SetAlpha2Colm(const CString& strCode)
{
	...
	CDlgUtil::CtrlMoveDistY(this, m_aSelectPosGroup, nDistY);
...
void CDgnConCodeNewChild10Dlg::UpdateChildData()
{
...
void CDgnConCodeNewChild10Dlg::ChangeDesignCode()
{
...
void CDgnConCodeNewChild10Dlg::ResizeChildDlg()
{
	CString strCode= CDBLib::GetConvertCodeName(m_pData->DesignCode);

	BOOL bShowFrame = strCode==CONCODE_NTC_DCEC_2023 ? TRUE : m_iFrameType != 2;
...
void CDgnConCodeNewChild10Dlg::ReAlingShearDesignCtrl()
{
...
void CDgnConCodeNewChild10Dlg::EnableWindowBySpecialWall()
{
	if(m_bSpecialWall)
	{ 
...
void CDgnConCodeNewChild10Dlg::SetEQCtrl(const CString& strCode)
{
	CString strTxt=_T("");
	int iStatus	= GetDataStatus(strCode);
...
BOOL CDgnConCodeNewChild10Dlg::Dlg2Data()
{
	UpdateData(TRUE);
	...
	m_dCd = _tstof(strNewValue);
	m_CmbIe.GetWindowText(strNewValue);
	m_dIe = _tstof(strNewValue);
...
void CDgnConCodeNewChild10Dlg::OnBCJoint()//PMS:4562 (Tel. 4022) MNET:XXXX-BSCHOI-20121108 #1
{
	UpdateData(TRUE);
...
LRESULT CDgnConCodeNewChild10Dlg::OnFoldButtonClicked(WPARAM wParam, LPARAM lParam)
{
	BOOL bSeisParamFolded = m_grpSeisParam.GetFoldState();
	ResizeChildDlg();
	if(!bSeisParamFolded)
...
void CDgnConCodeNewChild10Dlg::SetControlText(const CString& strCode)
{
	BOOL bNTC_DECE_2023 = (strCode == CONCODE_NTC_DCEC_2023) ? TRUE : FALSE;
	
	CString strFrame1 = _LS(IDS_DGN_SPECIAL_MOMENT_FRAMES);
	CString strFrame2 = _LS(IDS_DGN_INTERMEDIATE_MOMENT_FRAMES);
	CString strFrame3 = _LS(IDS_DGN_ORDINARY_MOMENT_FRAMES);
	CString strWall   = _LS(IDS_DGN_SPECIAL_RC_STRUCTURAL_WALLS);
	if (bNTC_DECE_2023)
	{
		strFrame1 = _LS(IDS_DGN_HIGH_DUCTILITY_FRAME);
		strFrame2 = _LS(IDS_DGN_MEDIUM_DUCTILITY_FRAME);
		strFrame3 = _LS(IDS_DGN_LOW_DUCTILITY_FRAME);
		strWall   = _LS(IDS_DGN_DUCTILITY_WALL);
	}
	
	GetDlgItem(IDC_DGN_EQ_ACI1)->SetWindowText(strFrame1);
	GetDlgItem(IDC_DGN_EQ_ACI2)->SetWindowText(strFrame2);
	GetDlgItem(IDC_DGN_EQ_ACI3)->SetWindowText(strFrame3);
	GetDlgItem(IDC_DGN_WALLTYPE_N1)->SetWindowText(strWall);
}