// DgnSeisAnchor2SupportGrid.cpp : implementation file
...
CDgnSeisAnchor2SupportGrid::CDgnSeisAnchor2SupportGrid(CWnd* pParent/* = NULL*/)
{

	m_pParent = (CDgnSeisAnchorEvalItem2NewSubSupportDlg*)pParent;

}

CDgnSeisAnchor2SupportGrid::~CDgnSeisAnchor2SupportGrid()
{
}

/////////////////////////////////////////////////////////////////////////////
// User Defined Finction
void CDgnSeisAnchor2SupportGrid::SetData2Grid(T_ANEV_REG_SUPP_TYPE_D& Data)
{
	BOOL bOldLock = LockUpdate(TRUE);

	int nRow = Data.aLayer.GetSize();
	int nCol = 0;

	if (nRow > 0)
	{
		nCol = Data.aLayer[0].aBrevType.GetSize(); // 여기서는 NxM 행렬로 값이 채워져 있다.
	}
	...
	CString strSupp;
	strSupp.Format(_T("%s\n%s\n%s\n%s\n"), _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_1), _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_2), _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_3), _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_4));
	SetStyleRange(CGXRange().SetCols(1, nCol), CGXStyle()
		.SetControl(GX_IDS_CTRL_CBS_DROPDOWNLIST)  // GX_IDS_CTRL_TABBED_COMBOBOX
		.SetChoiceList(strSupp)
	...
	for (int i = 0; i < nRow; i++)
	{
		...
		{
			if      (Data.aLayer[i].aBrevType[j] == 0) aValue[j] = _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_1);
			else if (Data.aLayer[i].aBrevType[j] == 1) aValue[j] = _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_2);
			else if (Data.aLayer[i].aBrevType[j] == 2) aValue[j] = _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_3);
			else if (Data.aLayer[i].aBrevType[j] == 3) aValue[j] = _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_4);
			else if (Data.aLayer[i].aBrevType[j] ==-1) aValue[j] = _T("None");
		}
...
BOOL CDgnSeisAnchor2SupportGrid::GetGrid2Data(T_ANEV_REG_SUPP_TYPE_D& Data)
{
	...
	T_ANEV_REG_SUPP_LAYER_D Layer; Layer.Initialize();
	Layer.aBrevType.SetSize(nCol);
	...
	for (int i = 0; i < nRow; i++)
	{
		...
		for (int j = 0; j < nCol; j++)
		{
			if      (aValue[j] == _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_1)) Layer.aBrevType[j] = 0;
			else if (aValue[j] == _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_2)) Layer.aBrevType[j] = 1;
			else if (aValue[j] == _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_3)) Layer.aBrevType[j] = 2;
			else if (aValue[j] == _LS(IDS_DGN_SEIS_ANCHOR_SUPPPOT_BREV_TYPE_4)) Layer.aBrevType[j] = 3;
			else if (aValue[j] == _T("None"))                                   Layer.aBrevType[j] =-1;	
		}

		Data.aLayer.Add(Layer);
	}

	return TRUE;
}

void CDgnSeisAnchor2SupportGrid::SetDataToRow(ROWCOL nRow, CStringArray& raValue)
{
	...
	for (int i = 1; i <= nColCount; ++i)
	{
		...
		if(raValue[i - 1] == _T("None"))
		{
			SetStyleRange(CGXRange(nRow, i), CGXStyle().SetEnabled(FALSE));
		}
	}
}

BOOL CDgnSeisAnchor2SupportGrid::GetRowToData(ROWCOL nRow, CStringArray& raValue)
{
	int nColCount = GetColCount();
	ASSERT(nColCount == raValue.GetSize());
...
CString CDgnSeisAnchor2SupportGrid::GetEditingValue(ROWCOL nRow, ROWCOL nCol)
{
	CString strNum(_T(""));
	BOOL bGet = FALSE;
	ROWCOL ncRow, ncCol;
...
BEGIN_MESSAGE_MAP(CDgnSeisAnchor2SupportGrid, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CDgnSeisAnchor2SupportGrid)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

///////////////////////////////////////////////////////////////////////////
// CDgnSeisAnchor2SupportGrid message handlers

// 그리드 초기화 
void CDgnSeisAnchor2SupportGrid::Initialize()
{
...
BOOL CDgnSeisAnchor2SupportGrid::GetStyleRowCol(ROWCOL nRow, ROWCOL nCol, CGXStyle& style, GXModifyType mt, int nType)
{
	BOOL bCurrentRow;
	BOOL bCurrentCell;
	BOOL bEnabled;
...
void CDgnSeisAnchor2SupportGrid::OnModifyCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CDgnSeisAnchor2SupportGrid::OnPasteFromClipboard(const CGXRange &range)
{
	CGXGridWnd x;
	CGXRange r;
	CGXRange toRange;
...
BOOL CDgnSeisAnchor2SupportGrid::VerifyPasteRange(CGXRange& from, CGXRange& to)
{
...
int CDgnSeisAnchor2SupportGrid::GetSelectedRowsCount(int top, int bottom)
{
	int toRows;
	toRows = bottom - top + 1;
	return toRows;
...
int CDgnSeisAnchor2SupportGrid::GetSelectedColsCount(int left, int right)
{
	int toCols;
	toCols = right - left + 1;
	return toCols;
...
void CDgnSeisAnchor2SupportGrid::ShowData()
{
	ROWCOL nRow, nCol;
...
void CDgnSeisAnchor2SupportGrid::SetViewData()
{
	m_pParent->SetViewData();
}

BOOL CDgnSeisAnchor2SupportGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
	CRowColArray awRows;
...
BOOL CDgnSeisAnchor2SupportGrid::SetRightValidCell(ROWCOL nRow, ROWCOL nCol)
{
...
BOOL CDgnSeisAnchor2SupportGrid::SetLeftValidCell(ROWCOL nRow, ROWCOL nCol)
{
