...
#if !defined(AFX_DGNKSCE_USD05_H__INCLUDED_)
#define AFX_DGNKSCE_USD05_H__INCLUDED_
...
class CDgnKSCE_USD05 : public CDgnPscCommon
{
// Member Variables.
protected:
	CPrtKSCE_USD05 m_Print;
//  CPrtKSCE_USD10* m_Print_ksce10;
	CPrtKSCE_USD10 m_Print_ksce10;
...
// Member Struct.
protected:
	struct _PSC_ALWS_POSI
	{
		double dfti[2];   // 0=Top, 1=Bottom
		double dfci;
		double dfte[2];   // 0=Top, 1=Bottom
		double dfce;
	  ...
	  void Initialize()
	  {
		...
		_PSC_ALWS_POSI& operator = (const _PSC_ALWS_POSI& rData)
	  {
			for(int i=0; i<2; i++) 
			{
				dfti[i] = rData.dfti[i];
...
public:
	CDgnKSCE_USD05();
	virtual ~CDgnKSCE_USD05();
	void SetDataCtrlPointer(CCRCDataCtrl* pDataCtrl);

	CPrtKSCE_USD05* Get_PrtKscePointer();
	CPrtKSCE_USD10* Get_PrtKscePointer10();
	...
	BOOL Get_KsceBdcy(ElemPairK ElemK, T_BDCY_KSCE& BdcrKsce, CMSExcel* pXL=NULL, BOOL bI=TRUE, BOOL bJ=TRUE);		//  6.   휨파괴안전도Y (BND_CR)
	...
	BOOL Get_KsceFsms(ElemPairK ElemK, T_FPMS_JTG& FpmsKsce);
	...
	BOOL Get_AllowbleStressOfConc(BOOL bPre, T_MATD_D& MatdD, T_SPSC_SECT& SpscSect, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbar, _PSC_ALWS_POSI& AlwsPosi);
	BOOL Get_AllowbleStressOfConc(BOOL bPre, double df_ci, T_MATD_D& MatdD, T_SPSC_SECT& SpscSect, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbar, CArray<_DGN_TNDN_CRC,_DGN_TNDN_CRC&>& arTndn, double* dfi, double* dfe);
	BOOL Get_AllowbleStressofPC(T_TDNA_K TdnaK, T_FYPC_KSCE& FypcKsce);
	BOOL Get_CrackCheck(BOOL bTopCalc, int iCrackType, T_MATD_D& MatdD, double dbw, double dFlgThk[2], double dpMomPos[2][2], T_SPSC_SECT& SpscSect, CArray<T_SPSC_COOR,T_SPSC_COOR>& arOuter, CArray<T_SPSC_COOR_AR*,T_SPSC_COOR_AR*>& arInner, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbar,
	...
	BOOL Get_AllowableStressofLcom(BOOL bStage, ElemPairK ElemK, double dfci, double* dAlw);
	...
	// 강축(Y)
	BOOL Get_FlexureStrength(BOOL bYaxis, BOOL bPosiI, BOOL bIter, BOOL bPositive, double dPhib, _PSC_MATL_ELEM& MatlElem, _PSC_SECT_POSI &SectPosD, T_SPSC_SECT& SpscSect, CArray<T_SPSC_COOR,T_SPSC_COOR>& arOuter, CArray<T_SPSC_COOR_AR*,T_SPSC_COOR_AR*>& arInner, T_SPSC_SECT& SpscgSect,
													 double drp, double dbw, double dFlgThk[2], double dpMomPos[2][2], CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CArray<_DGN_TNDN_CRC,_DGN_TNDN_CRC&>& arTndnPosi, T_BDCY_KSCE_BASE& BdcrKsce, CMSExcel* pXL=NULL, BOOL bApwM=FALSE, BOOL bRatingPrt=FALSE);
	// 약축(Z)
	BOOL Get_FlexureStrength(BOOL bYaxis, BOOL bPosiI, BOOL bIter, BOOL bPositive, double dPhib, _PSC_MATL_ELEM& MatlElem, T_SPSC_SECT& SpscSect, CArray<T_SPSC_COOR,T_SPSC_COOR>& arOuter, CArray<T_SPSC_COOR_AR*,T_SPSC_COOR_AR*>& arInner, T_SPSC_SECT& SpscgSect,
													 double dbw, double dFlgThk[2], double dpMomPos[2][2], CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CArray<_DGN_TNDN_CRC,_DGN_TNDN_CRC&>& arTndnPosi, T_BDCZ_KSCE_BASE& BdcrKsce, CMSExcel* pXL=NULL );
	...
	BOOL CalcTorsion_Pcp_Acp(CArray<T_SPSC_COOR,T_SPSC_COOR>& arOuter, double& dPcp, double& dAcp);
	...
	BOOL Get_ReqTendon(ElemPairK ElemK, UINT iIJ, UINT iTopBot, _PSC_ALWS_POSI& AlwsPosi, double dFlgThk[2], 
										 T_SPSC_SECT& SpscgSect, T_SPSC_SECT& SpscSect, CArray<_DGN_RBAR_CRC,_DGN_RBAR_CRC&>& arRbarPosi, CArray<_DGN_TNDN_CRC,_DGN_TNDN_CRC&>& arTndnPosi, T_TNDN_KSCE_BASE& TndnKsce);
	...
	void Get_Phi_Flexure(double dc, double ddt, double& dPhi);

	void Set_CMSExcel_KSCE10(CMSExcel* pXL, CString strBaseBook, int nCodePSC);
...
#endif // !defined(AFX_DGNKSCE_USD05_H__INCLUDED_)
