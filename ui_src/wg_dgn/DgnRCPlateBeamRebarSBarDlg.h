...
class CDgnRCPlateBeamRebarSBarDlg : public CChildDialog
{
public:
	CDgnRCPlateBeamRebarSBarDlg(T_RPBS_D* pData, CWnd* pParent = NULL);
	~CDgnRCPlateBeamRebarSBarDlg();
	enum { IDD = IDD_DGN_RC_PLATE_BEAM_REBAR_MBAR_CHILD_DLG };
...
private:
	...
	void InitCtrl();
	void SetNumCTC(BOOL bClick=TRUE);
	void CalcAs(T_RPBS_D& RchkBeamD, double& dAsTop, double& dAsBot);
...
private:
	CDBDoc* m_pDoc;
	T_RPBS_D* m_pData; // I만 사용, Layer 최대갯수 3개
	int m_iThik;
};
