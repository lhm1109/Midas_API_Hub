...
#if !defined(AFX_RPTJTG_D62_04_H__INCLUDED_)
#define AFX_RPTJTG_D62_04_H__INCLUDED_
...
class __MY_EXT_CLASS__ CRptJTG_D62_04 : public CReportBase , public CPrtKSCE_USD03  
{
public:
	CRptJTG_D62_04();
	virtual ~CRptJTG_D62_04();
...
protected:
	...
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeConstructStageVertStressData(int& iChapID, T_PARA_RPT& paraD);

	// 6.2. 인장구역철근의 인장응력 검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeTensRebarStressData(int& iChapID, T_PARA_RPT& paraD);

	// 6.3. 사용단계 정단면 균열 검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageCrackData(int& iChapID, T_PARA_RPT& paraD);

	// 6.4. 사용단계 경사단면 균열 검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageCrackSlopeData(int& iChapID, T_PARA_RPT& paraD);

	// 6.5. 사용단계 정단면 압축응력 검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageCompStressData(int& iChapID, T_PARA_RPT& paraD);

	// 6.6. 사용단계 경사단면 압축응력 검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageCompStressSlopeData(int& iChapID, T_PARA_RPT& paraD);

	// 6.7. 사용단계 균열폭 검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageCrackWidthData(int& iChapID, T_PARA_RPT& paraD);

	// 6.8. 사용단계 정단면 휨검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageMomentCheckData(int& iChapID, T_PARA_RPT& paraD);

	// 6.9. 사용단계 경사단면 전단검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageShearCheckData(int& iChapID, T_PARA_RPT& paraD);

	// 6.10. 사용단계 비틀림 검토 Data를 구성함.
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeServiceStageTorsionCheckData(int& iChapID, T_PARA_RPT& paraD);

	/************************************************************************/
	/* check compression at service stage //add by maxiao 2008-01-15        */
	/************************************************************************/
	BOOL MakeServiceStageCompressionCheckData(int& iChapID, T_PARA_RPT& paraD);
	
	// T_FYCM_JTG Data 출력관련 정보를 구성함.
	//   bMax : 최대값/최소값 출력여부. (TRUE : 사용단계 정단면 균열검토, FALSE : 사용단계 정단면 압축응력 검토)
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeFycmJTGData(BOOL bMax, int& iChapID, T_PARA_RPT& paraD);

	// T_FYMS_JTG Data 출력관련 정보를 구성함.
	//   bMax : 최대값/최소값 출력여부. (TRUE : 사용단계 경사단면 압축응력 검토, FALSE : 사용단계 경사단면 균열 검토)
	//   iChapID : Chapter index. Data가 구성되는 경우에는 iChapID++로 return.
	//   paraD : 구성된 Paragraph Data.
	//   RETURN : Data가 구성되면 TRUE, 아니면 FALSE.   
	BOOL MakeFymsJTGData(BOOL bMax, int& iChapID, T_PARA_RPT& paraD);
...
protected:
	...
	*/
	...
	BOOL Get_DetailBdcyPos(ElemPairK ElemK);
	BOOL Get_DetailBdcyNeg(ElemPairK ElemK);
...
#endif // !defined(AFX_RPTJTG_D62_04_H__INCLUDED_)
