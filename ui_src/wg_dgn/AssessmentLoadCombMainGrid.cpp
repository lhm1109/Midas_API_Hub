// AssessmentLoadCombMainGrid.cpp : implementation file
//
...
#define D_ALCOM_COL_GAMMA_F3	5
#define D_ALCOM_COL_DESC		6

/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGrid
CAssessmentLoadCombMainGrid::CAssessmentLoadCombMainGrid(CAssessmentLoadCombPSCDlg* pParent)
:m_bElastStag(FALSE), m_bTB10002_1_05(FALSE)
{
	...
	m_aCurDefVal[D_ALCOM_COL_GAMMA_F3	- 1] = _T("1.0");
	...
	AddColInfo(_LS(IDS_DGN_GAMMA_F3),				  t_real,    0);
...
CAssessmentLoadCombMainGrid::~CAssessmentLoadCombMainGrid()
{
}

#define CWnd CTBCommon
BEGIN_MESSAGE_MAP(CAssessmentLoadCombMainGrid, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CAssessmentLoadCombMainGrid)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGrid message handlers
void CAssessmentLoadCombMainGrid::Initialize(CDBDoc* pDoc, T_UNIT_SYSTEM &UnitSystem)
{
	...
	SetStyleRange(CGXRange().SetCols(D_ALCOM_COL_GAMMA_F3), CGXStyle()
		.SetHorizontalAlignment(DT_RIGHT)
		.SetValueType(GX_VT_NUMERIC)
		.SetPlaces(4));
	...
	csChoiceList.Format(_T("%s\n%s\n"), 
		GetLimStateStr(0), GetLimStateStr(1));
	...
	if(RacdD.nRatingCode == CS_454_20_PSC_RATING )
	{
		csChoiceList.Format(_T("%s\n%s\n%s\n%s\n"), 
			GetCombTypeStr(0), GetCombTypeStr(1), GetCombTypeStr(2), GetCombTypeStr(3), GetCombTypeStr(4));
	}
	else
	{
		csChoiceList.Format(_T("%s\n%s\n%s\n%s\n%s\n"), 
			GetCombTypeStr(0), GetCombTypeStr(1), GetCombTypeStr(2), GetCombTypeStr(3), GetCombTypeStr(4), GetCombTypeStr(5));
	}
	...
	L_SetColWidth(D_ALCOM_COL_NAME		, 100);
	L_SetColWidth(D_ALCOM_COL_LIM_STATE	, 82);
	L_SetColWidth(D_ALCOM_COL_COMB_TYPE	, 90);
	L_SetColWidth(D_ALCOM_COL_GAMMA_F3	, 80);
	L_SetColWidth(D_ALCOM_COL_DESC		, 90);
...
BOOL CAssessmentLoadCombMainGrid::GetRowNoByKey(UINT key, ROWCOL& nRow)
{
	ROWCOL nColKey;
...
BOOL CAssessmentLoadCombMainGrid::GetInsertRowByKey(UINT key, ROWCOL& nRow)
{
	if(GetRowCount() == 1)
	{
		nRow = 1;
...
/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGrid Overridable Functions
void CAssessmentLoadCombMainGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	if (nSize <= 0)
	{
...
long CAssessmentLoadCombMainGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
...
BOOL CAssessmentLoadCombMainGrid::ValidateField(CString value, int nColID)
{
	if (nColID >= GetColCount())
	{
		//SetWarningText(_LS(IDS_WG_CMD__ADDD___CAssessmentLoadCombMainGrid__ValidateFie));
...
BOOL CAssessmentLoadCombMainGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;

	for (int i = D_ALCOM_COL_NAME - 1; i < D_ALCOM_COL_GAMMA_F3; i++)
	{
		nPos = aCols[i]-1;
		if (value[nPos] == _T(""))
...
CString CAssessmentLoadCombMainGrid::GetDefValue(int nIndex)
{
	if(nIndex == 0) return _T("");
	return m_aCurDefVal[nIndex + 1];
}

CString* CAssessmentLoadCombMainGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

void CAssessmentLoadCombMainGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	ASSERT(0);

}

void CAssessmentLoadCombMainGrid::GetAllSelectedLcom(CArray<UINT, UINT>& aSelKey)
{
...
BOOL CAssessmentLoadCombMainGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
...
BOOL CAssessmentLoadCombMainGrid::OnCanceledModify()
{
	ROWCOL ncRow, ncCol;
...
CString CAssessmentLoadCombMainGrid::GetKeyForRow(ROWCOL ncRow, ROWCOL nkCol)
{
	CString sKey;
...
BOOL CAssessmentLoadCombMainGrid::CheckCurrentRecordChanged()
{
	ROWCOL ncRow, nkCol;
	CString sKey;
...
BOOL CAssessmentLoadCombMainGrid::IsActiveCol(ROWCOL& nCol)
{
	ROWCOL nRow, nACol;
...
// 이동되는 것을 금지한다.
// 주1 참조
BOOL CAssessmentLoadCombMainGrid::OnSelDragColsDrop(ROWCOL nFirstCol, ROWCOL nLastCol, ROWCOL nDestCol)
{
...
void CAssessmentLoadCombMainGrid::SetHeaderTitle(T_UNIT_SYSTEM &UnitSystem, BOOL bInit)
{
	SetHeaderTitle2((int*)m_aUnit.GetData(), bInit, TRUE);
}

BOOL CAssessmentLoadCombMainGrid::OnPasteFromClipboard(const CGXRange &range)
{
...
void CAssessmentLoadCombMainGrid::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CAssessmentLoadCombMainGrid::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CAssessmentLoadCombMainGrid::SetModeEdit()
{
...
int CAssessmentLoadCombMainGrid::GetLimState(CString &value)
{
...
CString CAssessmentLoadCombMainGrid::GetLimStateStr(int nVal)
{
	if(nVal == 0)
	{
...
int CAssessmentLoadCombMainGrid::GetCombType(CString &value)
{
...
CString CAssessmentLoadCombMainGrid::GetCombTypeStr(int nVal)
{	
	if(nVal == 0)
	{
...
void CAssessmentLoadCombMainGrid::GetCurSelKey(CArray<T_ALCS_K, T_ALCS_K>& aAlcsK)
{
	aAlcsK.RemoveAll();
	ROWCOL ncRow, ncCol;
...
BOOL CAssessmentLoadCombMainGrid::DeleteRecord( ROWCOL nRow )
{
...
BOOL CAssessmentLoadCombMainGrid::InsertRecordByKey(const T_ALCS_K &key, const T_ALCS_D &data)
{
	ROWCOL nRow;
...
BOOL CAssessmentLoadCombMainGrid::DeleteRecordByKey(const T_ALCS_K &key, const T_ALCS_D &data)
{
	ROWCOL nRow;
...
BOOL CAssessmentLoadCombMainGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, T_ALCS_K &key, T_ALCS_D &data)
{
...
BOOL CAssessmentLoadCombMainGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CAssessmentLoadCombMainGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
	ASSERT(0);
...
BOOL CAssessmentLoadCombMainGrid::DeleteFromDB(ROWCOL nRow)
{
	T_ALCS_K key;
	T_ALCS_D data;
	data.Initialize();
...
BOOL CAssessmentLoadCombMainGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CAssessmentLoadCombMainGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
	nCol = 1;
...
BOOL CAssessmentLoadCombMainGrid::IsDataChange(T_ALCS_K key, T_ALCS_D& dataNew)
{
	T_ALCS_D dataOld;
...
BOOL CAssessmentLoadCombMainGrid::ModifyToDB(CStringArray& raOldKey, 
...
BOOL CAssessmentLoadCombMainGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CAssessmentLoadCombMainGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	T_ALCS_K* pKey;
	T_ALCS_D* pData;
...
long CAssessmentLoadCombMainGrid::GetCountData()
{
	CDBDoc* pDoc = m_pDoc;
...
void CAssessmentLoadCombMainGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	T_ALCS_K Key;
	T_ALCS_D Data;
	Data.Initialize();
	...
	while(pos != NULL)
	{
		...
		if (Data.nRatingType == 0) continue;
...
void CAssessmentLoadCombMainGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	T_ALCS_K Key;
	T_ALCS_D Data;
...
/////////////////////////////////////////////////////////////////////////////
// CAssessmentLoadCombMainGrid Implementation
BOOL CAssessmentLoadCombMainGrid::GetValue(ROWCOL nRow, T_ALCS_K &key, T_ALCS_D &data)
{
	ROWCOL nCol;
	CString str;
	data.Initialize();
	...
	((CAssessmentLoadCombSubGrid*)m_pSubGrid)->GetData(data);
	m_pParent->GetData(data);
	
	return TRUE;
}

BOOL CAssessmentLoadCombMainGrid::SetValue(ROWCOL nRow, const T_ALCS_K &key, const T_ALCS_D &data)
{
	//
	CRowColArray awCols;
...
BOOL CAssessmentLoadCombMainGrid::SetValue(CGXData* pData, ROWCOL nRow, const T_ALCS_K &key, const T_ALCS_D &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CAssessmentLoadCombMainGrid::ConvStrToData(int i, CString& value, T_ALCS_D& data)
{
	...
	else if (i == D_ALCOM_COL_GAMMA_F3 - 1) data.dGammaF3 = _tstof(value);
	else if (i == D_ALCOM_COL_DESC - 1) data.strDescription = value;	
}

void CAssessmentLoadCombMainGrid::ConvDataToStr(int i, const T_ALCS_D& data, CString& value)
{
	...
	else if (i == D_ALCOM_COL_GAMMA_F3 - 1) value.Format(_T("%g"), data.dGammaF3);
	else if (i == D_ALCOM_COL_DESC - 1) value = data.strDescription;	
}

// Main Grid, Sub Grid간의 동기화  
void CAssessmentLoadCombMainGrid::OnChangeCurrentRecord(ROWCOL nRow)
{
	ROWCOL nkCol;
	GetColumnNoFor(GetKeyColID(), nkCol);
...
BOOL CAssessmentLoadCombMainGrid::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
	if (IsEdit()) 
	{
...
CString CAssessmentLoadCombMainGrid::GetNewKey()
{
	int i;
