// CPGLoadApplicationDlg.cpp: implementation of the CCPGLoadApplicationDlg class.
//
//////////////////////////////////////////////////////////////////////
...
CCPGLoadApplicationDlg::CCPGLoadApplicationDlg(CWnd* pParent /*=NULL*/)
	: CMenuBarChildDlg(CCPGLoadApplicationDlg::IDD, pParent)
{
	...
	m_aPositionCtrl.Add(IDC_DGN_CPG_LOAD_IJBOTH_CHK);
	m_aPositionCtrl.Add(IDC_DGN_CPG_LOAD_TAB);
	m_aPositionCtrl.Add(IDC_DGN_CPG_LOAD_APPLY);
	m_aPositionCtrl.Add(IDC_DGN_CPG_LOAD_CLOSE);
}

void CCPGLoadApplicationDlg::DoDataExchange(CDataExchange* pDX)
{
	CMenuBarChildDlg::DoDataExchange(pDX);

	//{{AFX_DATA_MAP(CCPGLoadApplicationDlg)
	DDX_Radio  (pDX, IDC_DGN_CPG_OPT_ADD_RDO,      m_nOption);
	DDX_Check  (pDX, IDC_DGN_CPG_LOAD_IJBOTH_CHK,  m_bBothIJ);
	DDX_Control(pDX, IDC_DGN_CPG_LOAD_TAB,         m_Tab);
	DDX_Control(pDX, IDC_DGN_CPG_LOAD_IJBOTH_CHK,  m_Ctrl_BothIJ);
	DDX_Radio(pDX, IDC_DGN_CPG_ELEM_RDO, m_nElemType);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CCPGLoadApplicationDlg, CMenuBarChildDlg)
	//{{AFX_MSG_MAP(CCPGLoadApplicationDlg)
	ON_BN_CLICKED(IDC_DGN_CPG_OPT_ADD_RDO,      OnDgnCPGLoadAppAdd)
	ON_BN_CLICKED(IDC_DGN_CPG_OPT_DEL_RDO,      OnDgnCPGLoadAppDel)
	ON_BN_CLICKED(IDC_DGN_CPG_LOAD_IJBOTH_CHK,  OnBothIJChk)
...
BOOL CCPGLoadApplicationDlg::OnInitDialog() 
{
	...
	// I
	m_pSubDlg_I = new CCPGLoadApplicationTabIDlg(this);
	m_Tab.AddTab(m_pSubDlg_I, _LS(IDS_DGN_CPG_I), CCPGLoadApplicationTabIDlg::IDD, TRUE);

	// J
	m_pSubDlg_J = new CCPGLoadApplicationTabJDlg(this);
	m_Tab.AddTab(m_pSubDlg_J, _LS(IDS_DGN_CPG_J), CCPGLoadApplicationTabJDlg::IDD, TRUE);		
	...
	//GetDlgItem(IDC_DGN_CPG_LOAD_CLOSE)->ShowWindow(SW_HIDE);
	//GetDlgItem(IDC_DGN_CPG_LOAD_APPLY)->ShowWindow(SW_HIDE);

	return TRUE;  // return TRUE unless you set the focus to a control
	              // EXCEPTION: OCX Property Pages should return FALSE
}

// void CCPGLoadApplicationDlg::OnDgnClose() 
// {
// 	// TODO: Add your control notification handler code here
// 	CTreeMenuBarBase::SetModeST(__TMMODE_TREEMENU__);
// }

void CCPGLoadApplicationDlg::Execute()
{
	...
	if (m_nElemType == ELEM_TYPE_ELEMENT)
	{
		...
		if (aSelKey.GetSize() > 0)
		{
			...
			{
				Dlg2Data();
				if (m_pDoc->m_pDataCtrl->AddCptf(aSelKey, m_Data))	Initial_SelectItem();
			}
			else	// Delete
			{
				if (m_pDoc->m_pDataCtrl->DelCptf(aSelKey))	Initial_SelectItem();
			}
		}
	...
	else
	{
		...
		if (aSelKey.GetSize() > 0)
		{
			...
			{
				Dlg2Data();
				if (m_pDoc->m_pDataCtrl->AddCptv(aSelKey, m_Data))	Initial_SelectItem();
			}
			else	// Delete
			{
				if (m_pDoc->m_pDataCtrl->DelCptv(aSelKey))	Initial_SelectItem();
			}
		}
		else	AfxMessageBox(_LS(IDS_DGN_CPG_BEAM_NON_MEMBER));
	}
...
void CCPGLoadApplicationDlg::Initial_SelectItem() 
{
	// Unselected All.
...
void CCPGLoadApplicationDlg::OnBothIJChk() 
{
...
BOOL CCPGLoadApplicationDlg::Dlg2Data()
{
...
void CCPGLoadApplicationDlg::OnSelchangeTab(NMHDR* pNMHDR, LRESULT* pResult) 
{
	m_bJTabEnable = (m_Ctrl_BothIJ.GetCheck() == 0) ? TRUE : FALSE;
}

void CCPGLoadApplicationDlg::OnDgnCPGLoadAppAdd() 
{
	m_nOption  = 0;			//	add/replace
	UpdateData(FALSE);
}

void CCPGLoadApplicationDlg::OnDgnCPGLoadAppDel() 
{
	m_nOption  = 1;			//	Delete
	UpdateData(FALSE);
}

void CCPGLoadApplicationDlg::OnDgnCPGElemTypeRdo()
{
	UpdateData(TRUE);
...
void CCPGLoadApplicationDlg::AlignControl()
{
	T_CPGD_D CpgdD;
	CpgdD.Initialize();
	...
	CWnd* pMomentFactorGroup = GetDlgItem(IDC_DGN_CPG_LOAD_IJBOTH_CHK);
...
void CCPGLoadApplicationDlg::ShowHideControls()
{
