// DgnRCBeamGrid.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCBeamGrid
CString CDgnRCBeamGrid::m_aCurDefVal[] = {_T(""), _T(""), _T(""), _T("")};


CDgnRCBeamGrid::CDgnRCBeamGrid()
{
	m_pDoc = CDBDoc::GetDocPoint();

	m_RcbeData.InitHashTable(101);  
...
BEGIN_MESSAGE_MAP(CDgnRCBeamGrid, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CDgnRCBeamGrid)
	ON_WM_RBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCBeamGrid message handlers
void CDgnRCBeamGrid::Initialize(CRCBeamParameter* pParent)
{
...
void CDgnRCBeamGrid::MakeItemEx()
{
	CWaitCursor wait;
	...
	//int nRecCount = m_pData->GetCount();
...
BOOL CDgnRCBeamGrid::InsertRecordByKey(T_ELEM_K &key, T_RCBE_D &data)
{
	T_RCBE_D Data;
	if(m_RcbeData.Lookup(key, Data))//해당 키의 데이터가 있으면 Add하지 않는다
		return FALSE;
	m_RcbeData.SetAt(key, data);
...
BOOL CDgnRCBeamGrid::DeleteRecordByKey(T_ELEM_K &key, T_RCBE_D &data)
{
	T_RCBE_D Data;
	if(!m_RcbeData.Lookup(key, Data))//해당 키의 데이터가 없으면 Delete 하지 않는다.
		return FALSE;
	m_RcbeData.RemoveKey(key);
...
void CDgnRCBeamGrid::UnitChanged(CDBDoc* pDoc)
{
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCBeamGrid Overridable Functions
void CDgnRCBeamGrid::OnChangedSelectionPublic(long *keys, int nSize)
{
	if (nSize <= 0)
	{
...
long CDgnRCBeamGrid::GetSelectedKeys(CArray<long, long> &caKey)
{
...
BOOL CDgnRCBeamGrid::ValidateField(CString value, int nColID)
{
...
BOOL CDgnRCBeamGrid::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CDgnRCBeamGrid::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, T_ELEM_K &key, T_RCBE_D &data)
{
...
BOOL CDgnRCBeamGrid::AddToDB(ROWCOL nRow)
{
...
BOOL CDgnRCBeamGrid::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
	ASSERT(raKey.GetSize() == raData.GetSize());
	T_ELEM_K tKey;
	CArray<T_ELEM_K, T_ELEM_K> aKey;
	CArray<T_RCBE_D, T_RCBE_D&> aData;
	for (int i = 0; i < raKey.GetSize(); i++)
	{
		tKey = *((T_ELEM_K*)raKey.GetAt(i));
		aKey.Add(tKey);
		aData.Add(*((T_RCBE_D*)raData.GetAt(i)));
	}
...
BOOL CDgnRCBeamGrid::DeleteFromDB(ROWCOL nRow)
{
	T_ELEM_K key;
	T_RCBE_D data;
...
BOOL CDgnRCBeamGrid::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CDgnRCBeamGrid::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
...
BOOL CDgnRCBeamGrid::ModifyToDB(CStringArray& raOldKey, 
...
CString CDgnRCBeamGrid::GetNewKey()
{
	CString value;
...
CString CDgnRCBeamGrid::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CDgnRCBeamGrid::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CDgnRCBeamGrid::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CDgnRCBeamGrid::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	T_ELEM_K* pKey;
	T_RCBE_D* pData;
	...
	for (int i = 0; i < aKey.GetSize(); i++)
	{
		...
		pData = (T_RCBE_D*)aData.GetAt(i);
		delete pKey;
		delete pData;
	}
}

long CDgnRCBeamGrid::GetCountData()
{
	CDBDoc* pDoc = m_pDoc;
...
void CDgnRCBeamGrid::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	CDBDoc* pDoc = m_pDoc;
	T_RCBE_D Data;
...
void CDgnRCBeamGrid::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	//T_ELEM_K Key;
	T_RCBE_D Data;
	CDBDoc* pDoc = m_pDoc;
...
void CDgnRCBeamGrid::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	T_ELEM_K Key;
	T_RCBE_D Data;
...
/////////////////////////////////////////////////////////////////////////////
// CDgnRCBeamGrid Implementation
BOOL CDgnRCBeamGrid::GetValue(ROWCOL nRow, T_ELEM_K &key, T_RCBE_D &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CDgnRCBeamGrid::SetValue(ROWCOL nRow, T_ELEM_K key, T_RCBE_D &data)
{
	//
	CRowColArray awCols;
...
BOOL CDgnRCBeamGrid::SetValue(CGXData* pData, ROWCOL nRow, T_ELEM_K key, T_RCBE_D &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CDgnRCBeamGrid::ConvStrToData(int i, CString& value, T_RCBE_D& data)
{/*
...
CString CDgnRCBeamGrid::GetStringiServCond(int index)
{
	CString result = _T("");
	switch(index)
	{
		case 0:
			result = _T("건조환경(0.4mm)");
		case 1:
			result = _T("건조환경(0.006tc)");
		case 2:
			result = _T("습윤환경(0.3mm)");
		case 3:
			result = _T("습윤환경(0.005tc)");
		case 4:
			result = _T("부식성환경(0.004tc)");
		case 5:
			result = _T("고부식성환경(0.0035tc)");				
	}

	return result;
}

CString CDgnRCBeamGrid::GetStringLoadCase(T_RCBG_D& data)
{
	CString result = _T("");
...
void CDgnRCBeamGrid::ConvDataToStr(int i, T_ELEM_K key, T_RCBE_D& data, CString& value)
{
	switch(i)
	{
		case 1:
		...
		//수정됨(과거에는 7개의 컬럼으로 그리드가 이루어져있었음
		/*
		case 1:
			value.Format(_T("%d"), key);
			break;
		...
		case 3:
			...
			break;
		case 4:
			value = GetStringiServCond(data.iServCond);
			break;
		case 5:
			value = _T("");
			if(data.bPosi[0])
...
void CDgnRCBeamGrid::MakeSearchKey(T_ELEM_K &key, T_RCBE_D &data, CStringArray &aKey)
{
	int nColID;
...
void CDgnRCBeamGrid::GetAllSelectedStld(CArray<T_ELEM_K, T_ELEM_K>& aSelKey)
{
	for (int i = 0; i < m_aSelectedKeys.GetSize(); i++)
		aSelKey.Add(m_aSelectedKeys[i]);
}

void CDgnRCBeamGrid::SetHeaderTitle(T_UNIT_SYSTEM &UnitSystem, BOOL bInit)
{
...
// 주1 참조
BOOL CDgnRCBeamGrid::OnTrackColWidth(ROWCOL nCol)
{
...
// 이동되는 것을 금지한다.
// 주1 참조
BOOL CDgnRCBeamGrid::OnSelDragColsDrop(ROWCOL nFirstCol, ROWCOL nLastCol, ROWCOL nDestCol)
{
...
/*
BOOL CDgnRCBeamGrid::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
/*
	if (IsEdit()) 
...
void CDgnRCBeamGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
...
/*
CString CDgnRCBeamGrid::GetLoadTypeCode(CString &csLoadTypeName)
{
	return m_pDoc->m_pLoadDB->GetLoadType(csLoadTypeName);
}

CString CDgnRCBeamGrid::GetLoadTypeName(CString &csLoadTypeCode)
{
...
BOOL CDgnRCBeamGrid::CheckCurrentRecordChanged()
{
	ROWCOL ncRow, ncCol;
...
BOOL CDgnRCBeamGrid::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
...
void CDgnRCBeamGrid::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CDgnRCBeamGrid::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CDgnRCBeamGrid::OnChangeCurrentRecord(ROWCOL nRow)
{
	m_nCurrentRow = nRow;
...
BOOL CDgnRCBeamGrid::GetCurrentRcbe(T_ELEM_K& nKey)
{
	ROWCOL ncRow, ncCol;
...
BOOL CDgnRCBeamGrid::SetCurrentRcbe(T_ELEM_K key, T_RCBE_D& data)
{
	ROWCOL nRow, nCol;
...
BOOL CDgnRCBeamGrid::LockUpdateGrid(BOOL bUpdate)
{
