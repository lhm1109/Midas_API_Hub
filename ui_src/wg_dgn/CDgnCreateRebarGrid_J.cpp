// CDgnCreateRebarGrid_J.cpp: implementation of the CDgnCreateRebarGrid_J class.
// (060123) sshan Create
//////////////////////////////////////////////////////////////////////
...
//#include "GCurI_FiberModel.h"          
//#include "I_FiberModel.h"
...
CString CDgnCreateRebarGrid_J::m_aCurDefVal[] = {_T(""), _T(""), _T(""), _T("")};


CDgnCreateRebarGrid_J::CDgnCreateRebarGrid_J()
{
...
BEGIN_MESSAGE_MAP(CDgnCreateRebarGrid_J, CWnd)
#undef CWnd
	//{{AFX_MSG_MAP(CDgnCreateRebarGrid_J)
	ON_WM_RBUTTONDBLCLK()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONDBLCLK()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CDgnCreateRebarGrid_J message handlers
void CDgnCreateRebarGrid_J::Initialize(CArray<T_REBT_MABR, T_REBT_MABR&>* pData, CConcSectionCreateRebar_J* pParent)
{
...
void CDgnCreateRebarGrid_J::MakeItemEx()
{
	CWaitCursor wait;
...
BOOL CDgnCreateRebarGrid_J::InsertRecordByKey(REBAR_KEY &key, T_REBT_MABR &data)
{
	// 삽입 위치를 찾는다.
	ROWCOL nRow;
...
BOOL CDgnCreateRebarGrid_J::DeleteRecordByKey(REBAR_KEY &key, T_REBT_MABR &data)
{
	// 레코드를 찾는다.
	ROWCOL nRow;
...
void CDgnCreateRebarGrid_J::UnitChanged(CDBDoc* pDoc)
{
...
/////////////////////////////////////////////////////////////////////////////
// CDgnCreateRebarGrid_J Overridable Functions
void CDgnCreateRebarGrid_J::OnChangedSelectionPublic(long *keys, int nSize)
{
	if (nSize <= 0)
	{
...
long CDgnCreateRebarGrid_J::GetSelectedKeys(CArray<long, long> &caKey)
{
...
BOOL CDgnCreateRebarGrid_J::ValidateField(CString value, int nColID)
{
...
BOOL CDgnCreateRebarGrid_J::ValidateRecord(CStringArray &value, CRowColArray &aCols)
{
	CString msg = _T("");
	int nPos;
...
BOOL CDgnCreateRebarGrid_J::ValidateAndMakeRecord(CStringArray &value, CRowColArray& aCols, REBAR_KEY &key, T_REBT_MABR &data)
{
...
BOOL CDgnCreateRebarGrid_J::AddToDB(ROWCOL nRow)
{
...
BOOL CDgnCreateRebarGrid_J::AddToDB(CArray<void*, void*>& raKey, CArray<void*, void*>& raData)
{
...
BOOL CDgnCreateRebarGrid_J::DeleteFromDB(ROWCOL nRow)
{
	REBAR_KEY key;
	T_REBT_MABR data;
...
BOOL CDgnCreateRebarGrid_J::DeleteFromDB(CRowColArray& awRows)
{
...
BOOL CDgnCreateRebarGrid_J::ModifyToDB(ROWCOL nRow)
{
	ROWCOL nCol;
...
BOOL CDgnCreateRebarGrid_J::ModifyToDB(CStringArray& raOldKey, 
...
CString CDgnCreateRebarGrid_J::GetNewKey()
{
	CString value;
	/*
...
CString CDgnCreateRebarGrid_J::GetDefValue(int nIndex)
{
	return m_aCurDefVal[nIndex];
}

CString* CDgnCreateRebarGrid_J::GetDefValueArray()
{
	return m_aCurDefVal;
}

BOOL CDgnCreateRebarGrid_J::MakeKeyAndDataArray(CStringArray& value, CRowColArray& awCols, 
...
void CDgnCreateRebarGrid_J::DeleteKeyAndDataArray(CArray<void*, void*>& aKey, CArray<void*, void*>& aData)
{
	REBAR_KEY* pKey;
	T_REBT_MABR* pData;
...
long CDgnCreateRebarGrid_J::GetCountData()
{
	CDBDoc* pDoc = m_pDoc;
...
void CDgnCreateRebarGrid_J::MakeItemFiltered(long nFilteredNum, long* aFilteredKeys, CRowColArray& awCols)
{
	CDBDoc* pDoc = m_pDoc;
	T_REBT_MABR Data;
...
void CDgnCreateRebarGrid_J::MakeItemNormal(long nNumData, CRowColArray& awCols)
{
	//REBAR_KEY Key;
	T_REBT_MABR Data;
	CDBDoc* pDoc = m_pDoc;
...
void CDgnCreateRebarGrid_J::MakeItemEditingRecord(ROWCOL ncRow, CStringArray& EditingRecord)
{
	CDBDoc* pDoc = m_pDoc;
	REBAR_KEY Key;
	T_REBT_MABR Data;
...
/////////////////////////////////////////////////////////////////////////////
// CDgnCreateRebarGrid_J Implementation
BOOL CDgnCreateRebarGrid_J::GetValue(ROWCOL nRow, REBAR_KEY &key, T_REBT_MABR &data)
{
	ROWCOL nCol;
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
BOOL CDgnCreateRebarGrid_J::SetValue(ROWCOL nRow, REBAR_KEY key, T_REBT_MABR &data)
{
	//
	CRowColArray awCols;
...
BOOL CDgnCreateRebarGrid_J::SetValue(CGXData* pData, ROWCOL nRow, REBAR_KEY key, T_REBT_MABR &data, CRowColArray &awCols)
{
	//
	CString str;
	for (int i = 0; i < GetColCount(); i++)
...
void CDgnCreateRebarGrid_J::ConvStrToData(int i, CString& value, T_REBT_MABR& data)
{
	switch(i)
	{
		case 0:
...
void CDgnCreateRebarGrid_J::ConvDataToStr(int i, REBAR_KEY key, T_REBT_MABR& data, CString& value)
{
	switch(i)
	{
		case 0:
...
void CDgnCreateRebarGrid_J::MakeSearchKey(REBAR_KEY &key, T_REBT_MABR &data, CStringArray &aKey)
{
	int nColID;
...
void CDgnCreateRebarGrid_J::GetAllSelectedStld(CArray<REBAR_KEY, REBAR_KEY>& aSelKey)
{
	for (int i = 0; i < m_aSelectedKeys.GetSize(); i++)
		aSelKey.Add(m_aSelectedKeys[i]);
}

void CDgnCreateRebarGrid_J::SetHeaderTitle(T_UNIT_SYSTEM &UnitSystem, BOOL bInit)
{
	// Set Title
	/*
	int aSize[11];
...
// 주1 참조
BOOL CDgnCreateRebarGrid_J::OnTrackColWidth(ROWCOL nCol)
{
...
// 이동되는 것을 금지한다.
// 주1 참조
BOOL CDgnCreateRebarGrid_J::OnSelDragColsDrop(ROWCOL nFirstCol, ROWCOL nLastCol, ROWCOL nDestCol)
{
...
/*
BOOL CDgnCreateRebarGrid_J::OnSelDragRowsDrop(ROWCOL nFirstRow, ROWCOL nLastRow, ROWCOL nDestRow)
{
	// 수정 모드이면 에러 리턴
/*
	if (IsEdit()) 
...
void CDgnCreateRebarGrid_J::OnLButtonDown(UINT nFlags, CPoint point)
{
...
/*
CString CDgnCreateRebarGrid_J::GetLoadTypeCode(CString &csLoadTypeName)
{
	return m_pDoc->m_pLoadDB->GetLoadType(csLoadTypeName);
}

CString CDgnCreateRebarGrid_J::GetLoadTypeName(CString &csLoadTypeCode)
{
...
BOOL CDgnCreateRebarGrid_J::CheckCurrentRecordChanged()
{
	ROWCOL ncRow, ncCol;
...
BOOL CDgnCreateRebarGrid_J::ProcessKeys(CWnd *pSender, UINT nMessage, UINT nChar, UINT nRepCnt, UINT flags)
{
...
void CDgnCreateRebarGrid_J::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CDgnCreateRebarGrid_J::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CDgnCreateRebarGrid_J::OnChangeCurrentRecord(ROWCOL nRow)
{
	m_nCurrentRow = nRow;
...
BOOL CDgnCreateRebarGrid_J::GetCurrentMaRb(REBAR_KEY& nKey)
{
	ROWCOL ncRow, ncCol;
...
BOOL CDgnCreateRebarGrid_J::SetCurrentMaRb(REBAR_KEY key, T_REBT_MABR& data)
{
	ROWCOL nRow, nCol;
...
void CDgnCreateRebarGrid_J::DeleteRowByKey(REBAR_KEY key)
{
	REBAR_KEY keyData;
	T_REBT_MABR data;
...
BOOL CDgnCreateRebarGrid_J::OnPasteFromClipboard(const CGXRange &range)
{
...
BOOL CDgnCreateRebarGrid_J::SaveGridData()
{
...
BOOL CDgnCreateRebarGrid_J::OnInsertEmptyRecord(ROWCOL nRow)
{
...
BOOL CDgnCreateRebarGrid_J::OnAddNewRecord(ROWCOL nRow)
{	
	return TRUE;   
}

REBAR_KEY CDgnCreateRebarGrid_J::GetLastRebarKey()
{	
	REBAR_KEY key = 0;
	int RowCount = GetRowCount()-1;
	T_REBT_MABR MData;
...
BOOL CDgnCreateRebarGrid_J::GridCheckData()
{	
	int RowCount = GetRowCount()-1;
	BOOL result = TRUE;
...
BOOL CDgnCreateRebarGrid_J::bCheck_int(CString str)
{	
...
BOOL CDgnCreateRebarGrid_J::bCheck_double(CString str)
{	
	int iLength = str.GetLength();
	for(int i=0; i<iLength; i++)
	{
