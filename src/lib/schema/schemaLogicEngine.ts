...
export interface SchemaLogicRules {
    ...
    enumDetectionRules?: EnumDetectionRule[];
    // 🔥 v1.4: conditionRegistry는 top-level로 이동됨
}

// 🔥 condition type taxonomy (전역 SSOT - top-level conditionRegistry)
...
// 🔥 section registry entry (전역 SSOT - top-level sectionRegistry)
export interface SectionRegistryEntry {
    id: string;  // SECTION_GENERAL
    name: string;
    description?: string;
    isDefault?: boolean;
}

// 🔥 Enum 판정 규칙 (구조화된 condition)
export interface EnumDetectionRule {
    id: string;
...
export interface StructuredCondition {
    type: string;  // conditionRegistry에 등록된 타입만 유효
    markerId?: string;  // type='has-marker'일 때 markerRegistry.id 참조
    sectionId?: string;  // type='assign-section'일 때 sectionRegistry.id 참조
    field?: string;
}
    ...
    const loadPromise = (async () => {
        try {
            ...
            const rules: SchemaLogicRules = {
                ...
                visibilityRules: schemaLogic.visibilityRules || [],
                requiredCalculationRules: schemaLogic.requiredCalculationRules || {},
                validationLayerRules: schemaLogic.validationLayerRules || {},
                descriptionBuildingRules: schemaLogic.descriptionBuildingRules || {},
                enumDetectionRules: schemaLogic.enumDetectionRules || [],
                // 🔥 v1.4: conditionRegistry는 top-level parsed.conditionRegistry에서 별도 로드
            };

            cachedRulesMap.set(cacheKey, rules);
            loadingMap.delete(cacheKey);

            console.log(`✅ Loaded schema logic from ${cacheKey}/shared.yaml (schemaLogic namespace)`);
            return rules;
        } catch (error) {
