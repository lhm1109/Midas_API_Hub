...
export function calculateFieldRuntimeStates(
  ...
  for (const section of sections) {
    for (const field of section.fields) {
      // 🔥 1. visible 계산
      const visible = evaluateVisibleWhen(field.ui?.visibleWhen, formValues);
      ...
      if (field.children && Array.isArray(field.children)) {
        for (const child of field.children) {
          const childVisible = evaluateVisibleWhen(child.ui?.visibleWhen, formValues);
          ...
          stateMap[child.key] = {
            ...
            enabled: childVisible, // 현재는 visible과 동일, 추후 분리 가능
          };
        }
      }
    }
