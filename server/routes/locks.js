  ...
  try {
    ...
    if (existingLock) {
      if (existingLock.locked_by === userId) {
        // 같은 사용자면 갱신
        const expiresAt = new Date(Date.now() + 3 * 60 * 1000).toISOString(); // 3분 (하트비트 간격보다 길게)
        const now = new Date().toISOString();
...
router.delete('/user/:userId/all', async (req, res) => {
  try {
    ...
    console.log('🔓 Releasing all locks for user:', userId);
    ...
    console.log(`✅ Released ${endpointLocks?.length || 0} endpoint locks and ${versionLocks?.length || 0} version locks`);
