...
function normalizeProvider(providerRaw) {
  try {
...
function createProgressReporter(scope = "task") {
  const startedAt = Date.now();
  let spinnerIndex = 0;
  return {
...
async function spawnCaptureAsync(command, args, opts = {}) {
  const prepared = prepareSpawn(command, args);
...
function resolveToolBinary(tool) {
  if (tool === "codex") {
    ...
    if (where.status === 0) {
      const hit = where.stdout
        .split(/\r?\n/g)
        .map((s) => s.trim())
        .find((s) => s.length > 0 && fs.existsSync(s));
      if (hit) return hit;
    }

    fail(
      "codex executable not found. Set BAT_RUN_CODEX_BIN or add codex to PATH."
    );
  }
  ...
  if (tool === "claude") {
    ...
    fail(
      "claude executable not found. Set BAT_RUN_CLAUDE_BIN or add claude to PATH."
    );
  }
...
function runAiCommand(provider, prompt, images, model, opts = {}) {
  if (provider === "codex") {
...
async function commandMcp(rawArgs) {
...
async function commandApi(rawArgs) {
  const [sub = "help", ...rest] = rawArgs;
  const opts = parseArgs(rest);
  const jsonOutput = Boolean(opts.json);
...
function commandSchemaCreate(rawArgs) {
  const opts = parseArgs(rawArgs);
  const provider = normalizeProvider(opts.provider);
  const progress = createProgressReporter("schema");
  ...
  const codeQueries = asArray(opts["code-search"]).map((q) => String(q).trim()).filter(Boolean);
  const schemaName =
...
function main() {
  ...
  // Fallback: treat remaining raw args as the first user prompt in shell mode.
  const opts = parseArgs(args);
  startWrapperShell(opts).catch((err) => fail(String(err?.message ?? err)));
}
